...
- **它怎么运作：** 赛事总监暂停计时，之后每打完一手就从当前级别固定扣掉 ==2 分钟==，不管这手牌实际打了多久（WSOP 规则 126.a、126.c；TDA RP-8）——所以盲注在 bubble 期间照样往上走，只是按手数走、不按真实时间走。所有桌各发一手牌，任何一桌都要等到每一桌都打完了才开始下一手。如果**同一桌**有两个人在同一次 hand-for-hand 里出局，那手开始时筹码更少的那个拿更低（bubble）的名次；如果他们在**不同桌**出局，则视为并列，平分这两个名次的奖金。
- **拖延（Stalling）：** 每一个决定都把时间银行用满，好少看几手牌（然后弃进钱圈）。它之所以有用，正是因为长考在计时上完全免费——这手牌照样只扣 2 分钟，不管你是秒弃还是把时间银行烧光。大码没理由拖延——他们想要更多手牌来进攻。short stack 和中码有时会拖延来求生，==但过度拖延可能招来 clock call（叫时）或罚则==——长考要有分寸，别刻意把时间银行烧光。
