...
export const POST: Post = {
  ...
  tags: ["扑克 icm", "德州扑克 icm 是什么", "德州扑克 icm 意思", "德州扑克 icm 公式", "德州扑克 icm 计算", "icm vs chip ev", "icm 分钱", "icm 锦标赛策略"],
  content: `
ICM 第一次让我亏钱时，我压根不知道有这个东西。桌上剩四个人，三个进钱，我低头一看是一对 J、码量中等。我推了，chip leader 拿 A-10 跟注，我在 bubble 上出局，一分钱没拿到。==好些年我都把它归档成「这一推是错的」的证据。它并不错== ——我只是压根不知道 bubble 究竟*在哪一步*向你收费，而那恰恰就是锦标赛扑克里最重要的那个概念。
...
> **快速回答**
> ICM（独立筹码模型）用剩下的奖金结构和每个人的码量，把一份筹码换算成它真正值多少奖金。它只回答一个问题：如果锦标赛此刻就以这些码量结束，我这一份奖池实际值多少钱。筹码翻倍，钱不会翻倍——差的那一截，就是 ICM 要算的东西。
...
> **快速回答**
> 因为奖金摊在好几个名次上，而排在你下面的那几档已经锁定了——筹码翻倍并不会让你的奖金权益翻倍。用来活下来的头几个筹码值很多，用来争冠军的最后几个筹码值得少。所以一个握着一半筹码的人拿不到一半奖池——他不可能比第一名更好，却仍然可能出局。

假设三个奖金分别付 $50 / $30 / $20。你一进钱，就保证至少能拿 $20——这 $20 已经不会再丢，接下来每一个筹码追的都是更上面、也更难拿到的那几档。

这让筹码换钱的曲线 ==弯曲== 起来：越早的筹码越贵，越往后的筹码越便宜。
...
## ICM 公式怎么算？（Malmuth–Harville 模型）

> **快速回答**
> ICM 纯粹根据码量大小，给每个人分配落在每个名次的概率，再乘以对应奖金。夺冠概率就是你的码量占总筹码的比例；第 2 名要对每一个可能夺冠的人递归地算一遍。这套算法叫 Malmuth–Harville 模型，下面用三个人、$100 奖池真算一次。

这个名字来自两个人：落名次概率的数学出自 David Harville 在 1970 年代对赛马赔率的研究，Mason Malmuth 把它搬进了扑克。
...
> **快速回答**
> chip EV 拿筹码当尺子，ICM（也叫 $EV）拿真实奖金当尺子。锦标赛前期两者一致；逼近钱圈和决赛桌时分道扬镳——一次在 chip EV 里没问题的 coin flip，到了 ICM 下往往该直接弃掉，因为出局丢的是高于已锁定名次的那部分权益。

锦标赛前期，pay jump 又小又远，一个筹码基本就是一个筹码——你打 [chip EV](/zh/blog/holdem-equity "thumb:/images/holdem-equity-hero.webp")，不停地积累。逼近钱圈和 final table 时，ICM 就接管了。

经典的冲突是*跟*一次边缘的 all in。在 chip EV 里，为一个大底池打一次 coin flip 可能没问题、甚至很好——你赢的筹码和输的一样多。在 ICM 里它可能是个明确的 ==弃牌==，因为出局会让你失去你在**每一档高于已锁定名次**的奖金里的权益（已经锁定的那份保底本身仍然是你的）——而在 bubble 上，什么都还没锁定，丢的就是你的全部奖金权益——而你要赢的那些筹码，值得比面值更少。

我正是在这里把那对 J 想反了。税是收在*跟注*那一侧的，而它的反面才是 bubble 之所以能打的原因：正因为所有人的跟注范围都收紧了，你的 fold equity 比用筹码衡量时**更值钱**。在 bubble 上抢先推是中等码量的武器，不是它的漏洞——我只是撞上了那个唯一能跟得最宽的人，那是方差，不是策略错误。==chip EV 问的是"这会做大我的码量吗？"ICM 问的是"这会做大我的 bankroll 吗？"== ——而只有后者才真正兑现成钱。
...
> **快速回答**
> 「ICM 税」是你的筹码占比和真实钱占比之间的差——码量越头重脚轻，凭空蒸发的那一截越大。它以风险溢价的形式出现在每一次 all in 里：跟注要比 chip EV 的盈亏点多出一截胜率，才真的赚钱。交税最重的，是钱圈边上的中等码量。

在上面那个算例里，leader 的筹码说是 50%，钱却只说 38.4%：当大码量要交一笔 ==11.6 个点的 ICM 税==。
...
![一个中等码量的锦标赛牌手在钱圈 bubble 上向大码量的 all in 弃牌，画面里有筹码和一道奖金阶梯——正是 ICM 压力把一次正常的跟注变成弃牌的那一刻](/images/holdem-icm-pressure.webp "ICM 压力：中等码量弃牌，因为在 bubble 上出局会让他失去整个 min-cash 以及它上面的每一档奖金")

## bubble factor 与风险溢价：ICM 怎么改变你的推和跟

> **快速回答**
> bubble factor（泡沫系数）衡量的是：输掉一个底池比赢下同样的底池要多亏多少，它在每一次 pay jump 之前飙升。1.0 是筹码和钱同步（前期）；1.5 意味着出局的痛是赢的爽的 1.5 倍。实战就两条：大码量进攻，中短码收紧跟注、保持率先推。

泡沫系数越高，你要跟就得多出越多胜率——1.5 时要 60%，2.0 就要 67%。具体的换算表在 [钱圈指南](/zh/blog/holdem-bubble) 里。
...
> **快速回答**
> chip chop 按纯筹码占比分剩下的钱，ICM deal 按每个人的 ICM 美元价值分。前者偏袒大码量，后者对 short stack 更公平——同一张桌子，两种分法能差出几百美元。口诀：短码要 ICM，大码要 chip chop，答应之前先把数字跑一遍。

当玩家们同意提前结束锦标赛、把奖金分掉时，桌面上就是这两种方法——而懂得两者的区别，值真金白银。
...
> **快速回答**
> ICM 在逼近 pay jump 时最重要：钱圈、决赛桌 bubble、卫星赛，风险溢价在这几处到峰值。pay jump 还很远时影响最小：前中期、深码小盲、争冠 heads-up，这时用 chip EV 当近似、专心累积筹码就够。用得太早和该用时无视，两头都在漏钱。

在这些位置上靠它：

- **[进钱 bubble](/zh/blog/holdem-bubble "thumb:/images/holdem-bubble-hero.webp")** ——最大的一次跳跃，就是从 $0 到进钱，所以风险溢价在这里达到峰值。
- **final table 的 bubble，以及 final table 上每一次 pay jump** ——阶梯上的每一级都是真钱。
- **卫星赛（satellite）** ——最极端的情形：在送出多个席位的卫星赛里，每一个晋级席位价值都相同，所以一旦你有足够的筹码赢下一个席位，多出来的筹码就几乎*一文不值*，你几乎会弃掉一切（只送一个席位的 winner-take-all 卫星赛，要按 chip EV 去争第一）。

可以把 chip EV 当成够用的近似来打的时候是：

- **前期和中期**，此时下一次 pay jump 还是个遥远的抽象概念，积累筹码才是赢下锦标赛的关键。
- **筹码深、盲注小的打法**，此时你有空间靠牌技压制对手，而不是把钱全推进去。
- **争冠军的 heads-up**，此时只剩两个奖金，还在场上的那部分钱可以按 chip EV 来判断。
...
> **快速回答**
> ICM 是手上最好的简单模型，但只是近似：它假设每个玩家水平相同，除了码量之外几乎无视一切——牌技、位置、上涨的盲注都不在公式里。把它当成有力的初步估算、再按牌技和位置去调整，别把它当成物理定律。

它漏掉的东西，值得一条条摆出来：

- **牌技。** ICM 把码量相同的世界冠军和第一次打的新手当成相等。一个更强的玩家，他的筹码值得比模型说的更多。
- **位置。** button 上的 3 个大盲码量（坐在最好的位置上，还能自己挑时机、带着完整的 fold equity 全下）比 big blind 上同样的码量（三分之一已经交进池里，一两手之内就被迫全下）更值钱。ICM 看不见座位。
...
A. chip EV 用赢来或输掉的筹码衡量一个决策；ICM 用真实的钱衡量。它们在锦标赛前期一致，逼近钱圈时分道扬镳。在 bubble 上，出局丢的是你进钱的全部机会；一旦进了钱，丢的是高于已锁定名次的一切。一次在 chip EV 里没问题的 coin-flip all in，在 ICM 下可能是个明确的弃牌。
...
A. 你永远不会把它完全关掉，但在前期和中期、以及筹码深且盲注小的打法里，它的影响小到可以用 chip EV 当近似——这些位置 pay jump 还很远。争冠军的 heads-up 只剩两个奖金，所以第一名和第二名之间的差价可以按 chip EV 判断。即便如此，也要先看一眼奖金分配表和码量分布。

**Q. 最常见的 ICM 错误有哪些？**

A. 主要有三个。第一，*过度*套用 ICM——在 pay jump 还很远时就一路弃牌"去 ladder up"，而不是积累筹码。第二，作为 bubble 附近的中等码量跟得太宽，而那里恰恰是你风险溢价最高的地方——那时你还什么都没锁定，所以在那里出局丢的是你的全部权益，连 min-cash 都算在内。第三，当你是 short stack 时不先跑一遍数字就答应 chip chop（或当你是 leader 时答应 ICM deal）。ICM 是个后期工具：用得太早，或在 final table 上无视它，两头都在漏钱。

**Q. ICM 压力下拿 AJ 面对全压，需要多少权益才能跟？**

A. 比 chip EV 的盈亏平衡点更高。在 chip EV 里，一个像 40% 权益就能跟的边缘 all in，到了 ICM 下往往要 48-50% 才够——因为出局会让你失去高于已锁定名次的那部分 pay-jump 权益，而你赢来的筹码又值得比面值更少（这就是**风险溢价**）。所以拿 AJ 这种常被压制、翻牌前对上对手全压范围经常落后的边缘牌，在 bubble 或 final table 上很多本该是 chip-EV 跟注的位置，会翻转成 ICM 弃牌。你越短、pay jump 越大，需要的权益就越高。
...
> **快速回答**
> 如果只带走一句：锦标赛的筹码是一张对着固定奖金表的彩票，ICM 就是给这张彩票定价的方法。下面三条，是这句话在牌桌上的三种用法——差价从哪来、什么时候换成 $EV、分钱时怎么用。
