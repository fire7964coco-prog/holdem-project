...
> 드리프트 **90** · 추적불가 84 (✅ 236)
> ⚠ **드리프트 82 → 90(+8)은 «내용» 드리프트가 아니다** — FAQ 스키마 복구로 EN `texas-…-beginners`·
> `game-order`의 `updated`를 08-10으로 올린 결과다. 해당 미러들은 어차피 23편 소급 대상이니
> **그 배치에서 masterUpdated를 한 번에 정리하면 사라진다.** (de는 −2로 이미 해소됨)
...
>
> ### ✅ 다국어 FAQ 스키마 — **게이트 신설 + EN 마스터 복구 완료. 번역본 23편 남음**
> `npm run check:faq-schema`(셀프테스트 **16/16**) 신설. 25개 로케일 457편 전수.
> **EN 마스터 2편은 복구했다**(산출물 8·12문항 확인) → 🔴 **23편** 남음.
> **목록·판정기준·처방 = `docs/faq-schema-audit-2026-08-10.md`.**
> ⚠ 수치 이력: 41(오보) → 25(실측) → 23(EN 복구 후). 41은 `.ts` 원본을 CRLF째로 훑은 탓이다.
...
| # | 할 일 | 메모 |
|---:|---|---|
| 0 | 🔴 **다국어 FAQ 스키마 23편 소급** | `npm run check:faq-schema` 가 목록·유형·처방을 매번 출력한다. 유형 **A**(`###` 헤딩) 12편 · **B**(`A.` 접두 누락) 7편 · **C**(현지어 마커 `س./प्र./S.`) 4편. 전부 기계적. **EN 마스터는 이미 정본 형식**이라 그대로 따라가면 된다. 🔴 치환은 반드시 `.mjs` 파일로(§16 — 이번에 `node -e`로 빌드를 깼다) |
