...
### 1. 🔴🔴 다음 세션이 반드시 먼저 판단할 것 — "불법" 서술 48건 (사장님 결정 필요)

**posting.mdc의 「합법/불법 얘기 금지」는 2026-07-29 지시인데, 그 이전에 갱신된 글들이 아직 안 고쳐졌다.**
`holdem-bankroll-management`를 경화하다 2건을 발견해 고쳤고, 전수 grep을 돌리니 **7개 글에 48건**이 더 남아 있다.

| 글 | "불법" 등장 | updated |
|---|---:|---|
| **`holdem-pub-legal`** | **29건** | 2026-07-11 |
| `holdem-pub-guide` | 9건 | 2026-07-11 |
| `holdem-pub-first-visit-guide` | 5건 | 2026-07-11 |
| `holdem-pub-promotion` | 2건 | 2026-07-11 |
| `holdem-tournament-buy-in-cost` | 1건 | 2026-07-28 |
| `holdem-tournament-how-to-enter` | 1건 | 2026-07-28 |
| `holdem-tournament-schedule-check` | 1건 | 2026-07-28 |

⚠️ **`holdem-pub-legal`은 글 자체가 합법성을 다루는 글이다.** 문장 몇 개를 고치는 문제가 아니라
**글의 존재 방식**에 대한 판단이 필요하다(전면 재프레이밍 / 축소 / 유지 중 택일).
slug가 색인돼 있어 임의 삭제·변경은 금지(§11-1). **사장님께 물어보고 착수할 것. 혼자 정하지 말 것.**

★ 그리고 **홀덤펍 클러스터가 다음 경화 후보 1순위**인데, 그 클러스터가 정확히 이 문제의 진앙이다.
   순서상 이 결정을 먼저 받아야 홀덤펍 클러스터를 시작할 수 있다.
...
| **홀덤펍** | `lib/posts/holdem-pub-guide.ts` | `pub-first-visit`(227노출·CTR 9.69%)·`pub-legal`·`pub-promotion` → ★위 1번 결정 선행 |
| 스타팅 핸드·프리플랍 | `lib/posts.ts` 3540행 부근 | `holdem-starting-hand-range`(12노출·r23.8)·`when-to-fold-preflop` 등 |

**우선순위 제안**: **확률·수학**(필라가 r28.6으로 3페이지, 질문형 0%라 개선 여지 최대이고 §1 결정과 무관하게 바로 착수 가능) → 홀덤펍(§1 해결 후) → 스타팅 핸드.
...
### 4. ⚠️ 이번에 지키지 못한 것 (다음 세션이 메울 것)

**적대적 검수(§14-A 9항)를 돌리지 않았다.** 이번 세션은 Agent 툴 사용이 제한돼 있어
§13 검산을 전부 직접 손계산으로 수행했다(그 과정에서 위 4건을 잡았다).
→ **다음 세션에서 전략 4편에 Fable5 병렬 적대적 검수(검산형 + SEO형)를 소급 실행할 것.**
   앞선 세션 기록대로 **검산 에이전트가 놓친 판정 오류를 SEO 에이전트가 잡은 실례**가 있다.
