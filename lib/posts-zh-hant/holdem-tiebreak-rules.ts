...
export const POST: Post = {
  ...
  tags: ["德州撲克平手", "牌型一樣誰贏", "同對子誰贏", "兩對平手", "第五張決勝", "踢腳牌", "同花比大小", "花色大小", "平分底池"],
  content: `
...
> **快速解答**
> 三步是固定的：①先比牌型 ②再比組成牌型的牌 ③最後從最大到最小逐張比踢腳牌。而且==只要有一步分出勝負就結束==，後面的步驟根本不會跑到。三步走完還完全一樣，才是平分底池。
...
如果第3步把牌都比完了，兩手就是一模一樣，==g:底池平分==。至於籌碼之後怎麼分（畸零籌碼、三家平分、邊池），那是[平分底池規則](/zh-hant/blog/holdem-split-pot-rules)的範疇。第2步和第3步才是牌桌上吵架的地方，我們接下來就往那走。
...
> **快速解答**
> 第一張踢腳大的贏。一對用三張踢腳，從最大那張開始一次比一張——第一個不一樣的地方決定底池，後面的踢腳連上場的機會都沒有。

實際上大多數底池在==第一張就結束了==，第二、第三張踢腳連上場的機會都沒有。
...
> **快速解答**
> 每一種牌型都有自己的比較順序——有些要比到踢腳，有些完全由組成牌型的牌決定。下面這張表把十種牌型一次列齊，標記會顯示那種牌型有沒有用到踢腳。
...
> **快速解答**
> 兩對的比較有固定順序：先比大對，再比小對，最後才比那張單一踢腳。兩對剛好只帶一張踢腳，所以比完兩組對子之後，能吵的就只剩那一張牌了。

新手真正會賠錢的地方不在順序，而在==公牌自己成對==的時候：那一對會同時算給桌上每個人，你翻牌時領先的兩對可能就這樣被公牌疊掉，變成別人的兩對 —— 下面那個例子就是。
...
> **快速解答**
> 順子純粹只比最大那張牌，沒有第二張可以吵。A 在順子裡兩頭都能用，但當小的時候（A-2-3-4-5，也就是「輪子」）它是全場最小的順子，而 A-K-Q-J-10 是最大的。

公牌是 4♦ 3♣ 2♠ K♦ Q♥ 時，拿 A♠ 5♠ 的人做成輪子：5-4-3-2-A。拿 6♥ 5♥ 的人做成 6-5-4-3-2。==r:A在輪子裡當*小*==，所以 A-2-3-4-5（也就是大家常說的「12345」）座落在順子階梯的最底層 —— ==g:6高的順子贏。== 兩副最大張一樣的順子就是一模一樣，一樣的牌就平分。

階梯的另一端：==**德州撲克裡最大的順子是百老匯 —— A-K-Q-J-10**==。沒有任何順子贏得了它（同花或更上面的牌型當然還是壓得過），而輪子墊在最底層，所以場上每一副順子的位置，都只由它最大那張牌決定，落在這兩端之間。

輪子有兩件事*做不到*：A不能從中間繞回來（Q-K-A-2-3 不是順子），也不能同時當大又當小。同花走的是平行的規則 —— 五張由大到小全比、花色不算數，細節在[同花大還是順子大](/zh-hant/blog/holdem-flush-vs-straight)裡。
...
> **快速解答**
> 只要兩手牌的前四張一模一樣，那第五張就是整個底池。這種局面在公牌很強的時候特別常見——大家共用前四張，勝負全押在最後那一張旁牌上。
...
> **快速解答**
> 不算——至少在決定誰贏這件事上不算。花色在德州撲克裡只做一件事：五張同花色湊成同花。除此之外它沒有任何位階，所以兩手點數一張一張都對上的牌永遠平分底池，也沒有任何一張牌會因為花色而壓過另一張。

這個問題之所以一直有人問，是因為撲克裡確實存在花色順序 —— 只是不在「誰贏這個底池」這件事上。梭哈（Stud）和 Razz 用它決定誰先下注（bring-in）、還有那顆分不開的籌碼歸誰；德州撲克的標準規則這兩件都不用（唯一沾得上邊的是開局抽牌定按鈕，那決定的是座位 —— 見下方 FAQ）。

最乾淨的證據，就是那顆*分不開*的籌碼。2026 年 WSOP 錦標賽規則寫的是 ==g:*"In button games with 2 or more high or low hands, the odd chip goes to the first seat left of the button"*==（Rule 73）。就算底池實際上分不平，規則找的也是**座位**，不是花色 —— 同一條規則後半段那套用花色決定的方法，是寫給梭哈和 Razz 的。

還有一件值得知道的事：在德州撲克裡，兩副同花永遠是*同一種*花色，因為五張公牌是大家共用的，而牌面不可能同時擺出三張紅心和三張黑桃。所以「我的黑桃贏你的紅心」不是一條讓你輸掉的規則 —— 那是一手根本發不出來的牌。
...
> **快速解答**
> 如果你的底牌破不了公牌自己的最強五張，它們就不算數——當這件事對每個人都成立時，底池就平分。這也是為什麼「我有順子」不等於「我贏」：要看你的順子有沒有比檯面那副大。

![資訊圖：公牌 A-K-Q-J-10 對每個人都是最強五張，所以 9-7 的牌贏不過它，底池平分](/images/holdem-tiebreak-best5.webp "七張取五：當公牌本身就是最強的牌時，你的底牌掉出了最強五張")

看上面這副公牌：A♠ K♥ Q♣ J♦ 10♠，百老匯已經完成。你的 9♥ 7♠ *確實*能做出一副順子 —— K-Q-J-10-9 —— 但它**小於**攤在檯面上那副A高的順子，所以你的最強五張就是公牌本身。其他每個人也一樣。在河牌下注之前就看出這種牌面走向，本身就是一門功夫 —— 那就是[讀公牌](/zh-hant/blog/holdem-reading-the-board)。
...
A. 三道檢查照順序跑 —— 先比牌型，再比組成牌型的牌，最後從最大張往下比踢腳 —— 第一個不一樣的地方就結束。同樣重要的是哪些東西*永遠不進*比較：花色、誰最後下注、誰離按鈕比較近、每個人放了多少籌碼進去。只要五張牌點數一張一張都對上，不管下注過程發生過什麼，荷官就是把底池分掉。
...
A. 踢腳大的贏 —— 但先確認你的哪幾張牌真的進了最強五張。公牌 A-Q-7-3-2 配一對 A 時，拿 A-9 的人打出的是 A-A-Q-9-7：公牌那張 Q 排到他的 9 前面，所以 9 只是*第二張*踢腳。對上 A-K，底池在第一個踢腳位子就決定了，那張 9 從頭到尾沒被比到。踢腳有三個位子，但大多數底池在第一個就結束。

**Q. 兩人都兩對誰贏？**

A. 先比大對，再比小對，最後比那張單一踢腳 —— 所以 A 和 3 的兩對贏 K 和 Q 的兩對，就算第二對小很多也一樣。會讓人栽跟頭的是公牌本身就有兩對的牌面，像 K-K-9-9-5（沒有三張同花色，誰都湊不成同花）：除非有人手上有 K、有 9、口袋 5-5，或是口袋對比 9 大，否則每個人都是同樣的兩對，整手牌就縮成比那一張踢腳，全桌最大的底牌拿走底池 —— 而如果沒有任何人的底牌大過公牌的第五張，大家一起打公牌，底池平分。兩對就只有一張踢腳，從來不會是兩張。
...
A. 重要 —— 而且這是牌手輸掉「以為穩贏」的底池最常見的方式。會把整個底池押在最後一張牌上的經典地方：一對的第三張踢腳、兩對唯一的那張踢腳、同花最小的那張、公牌四條旁邊那張邊牌。它只有在公牌本身的牌大過你那張邊牌時才失去意義——有時是整副公牌都上場、你的底牌整個掉出這手牌，有時是一張底牌上場、另一張永遠不算數：A♠ 3♠ 對上 A♦ 2♦、公牌 A♥ K♣ Q♦ J♠ 9♥ 就是平分，兩邊打的都是 A-A-K-Q-J。
...
A. 能，而且實戰裡多半發生在順子大半已經攤在公牌上的時候。公牌 5♦ 6♣ 7♠ 8♥ 2♦：拿 9♣ 4♠ 的人做成 9-8-7-6-5，拿 4♥ 3♦ 的人用同樣那四張做成 8-7-6-5-4。兩個人都「做成順子」了，但從頭到尾只有最大那張被拿來比，所以 9 拿走底池。最大張一樣才是同一副順子，才會分池。

**Q. 兩人同樣是順子誰贏？順子一樣大會怎樣？**

A. 最大張較大的那副順子贏 —— Q-J-10-9-8 贏 J-10-9-8-7，因為順子只比最高那一張、沒有踢腳。兩副順子最大張一樣就是一模一樣，底池平分。這最常發生在順子大半靠公牌、雙方又補上同一端的時候。

**Q. 兩人都是同花誰贏？（花色能不能決勝？）**

A. 從最大張開始逐張比：A高的同花贏K高的同花，最大張一樣就比下一張，一路比完五張。==花色永遠不破平手==，所以五張點數全一樣就平分。花色只用在「判斷有沒有湊成同花（或同花順）」，不用來比大小。（在德州撲克裡兩副同花一定同一花色，因為大家共用公牌。）
...
A. 不會 —— 但德州撲克的牌桌上確實有一個地方會用到花色，只是跟底池無關。現金桌、以及大多數撲克室的館規，開局時每人抽一張牌決定按鈕從哪裡開始，兩張抽到同點數就用花色順序決定。（WSOP 錦標賽開局不抽牌：==錦標賽規則第 85 條== 規定按鈕從荷官右手邊第一位開始，只在剩三桌、兩桌、一桌時才抽牌定按鈕。）不管哪一種，決定的都是*座位*，從來不是牌。除了抽牌定按鈕——WSOP 規則書的名詞解釋寫明按「點數與花色」決定——錦標賽規則裡唯一的花色順序屬於梭哈和 Razz。兩手最強五張一張一張都對上，不管花色如何都平分底池。
