...
> **先に結論**
> **練習中はプレイチップ**、つまり0円からで十分です。海外のリアルマネーのホームゲームなら、全員が「失っても平気」と思える額 — $0.01／$0.02のブラインドで$2〜$5のバイインが目安。緊張するレートで始めると学習が止まります。トーナメントは仕組みが別で、固定のエントリー料を1回だけ払う形式です。
...
> **先に結論**
> コールする前に答える質問は1つ — **払う価格は、勝てる確率に見合っているか？** ポット$100に相手が$20ベットなら、$120を取るために$20を払う「6対1」で、7回に1回（約14％）勝てれば損はしません。自分の勝率は「アウツ×4（フロップ）／アウツ×2（ターン）」で一瞬で出せます。
