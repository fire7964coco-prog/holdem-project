...
### 솔버 랜딩·앱
- **MS 랜딩 후속 연결**: `/ms/solver`와 다국어 FAQ 정정은 `27c08021`로 운영 배포 완료(09-14 · 11언어/FAQ229 실측 통과 · IndexNow11 접수). 별도 솔버 저장소의 `outbound.ts`에 MS `/solver` 매핑이 아직 필요하다. 조사·현지 페르소나 근거 = `docs/ms-solver-landing-brief.md`, 배포 상세 = WORKLOG. 랜딩 미구현 잔여=hi.
