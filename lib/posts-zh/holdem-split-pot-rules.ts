...
:::tip[如果公共牌通杀而有人在河牌下注，**反射性弃牌才是错的**。如果没有任何牌能盖过公共牌，平分就是确定的，跟注仍然会把底池里原有的筹码拿回一半——弃牌等于白白把这一半送人。如果公共牌还可能被盖过，就用频率来算：面对满池下注，需要对手同样只有公共牌的概率达到约三分之二；面对半池下注，约一半就够。这道门槛其实很高：面对满池下注，只有当他握着真牌的比例**超过三分之一**时，弃牌才是对的——而在公共牌通杀的局面里，那才是常态。]:::
