...
| 7 | B | id strategy 8편 | + PDF 앵커 «9-max/6-max» 거짓 id 소급. ⚠ shc AJo는 판단 대기 #1 — 소급 금지. 🆕 + pt 원장 when-to-fold #41(«폴드 과다→밸류벳 출혈» 누수 뒤바뀜 — 자기모순형)의 id 거울 확인 |
...
| **검수장 pt 원장 편입 — 진행 중(세션 0~4 완주 · 2026-08-27)** | 🟢 파일럿+rules 5+rankings 6+odds 7+strategy 8 = **27/42편 · 1,691주장 판정**(OK 1,570 · RISKY ~36 · WRONG 3 — 전부 자기모순형 · UNV ~82). 잔여 = glossary 6 · tournament 9(8/29 EPT 전환 이후 배치). 🔴 **WRONG 2건(reading #38 «한 장 플레이»·tiebreak #44 «두 킥커 슬롯»)은 다로케일 상속이라 본체가 08-27 전수 선정정**(8로케일+7로케일 — 검수장 감시 앵커 🔴 점등 = 정정 반영 신호, 설계 의도) · when-to-fold #41은 세션 7(strategy) 때 id 거울 확인. **RISKY 모음은 검수장이 «세션 7 일괄 발신» 예정** — 도착 시 다로케일 소급 판정(split-pot #11 «보드플레이=최다» 한정어 등 id·EN 거울 다수). 진행표 = `홀덤검수/reports/계획-pt-42편-원장편입-2026-08-26.md`. 정본 = settled-decisions §5-B |
...
| ✅ **DFS 복구 완료(2026-08-27) — «인증 만료»는 오진이었다.** 자격증명은 유효(직접 REST 20000 Ok·유료 잔액 실재 — 사장님 지적이 맞았다). 진짜 원인 = `.mcp.json`의 `${VAR}` 확장이 참조하는 **환경변수가 시스템에 없었던 것** → 사용자 환경변수 등록 완료. ✅ **교차검증 예약 이행 완료(세션 5)** — id 15종·fr 10종 REST 재실측 전 항목 라쿠와 일치(각 뱅크 머리 갱신) | ⚠ **MCP 프로세스는 여전히 40100**(이 CLI가 env 등록 전에 기동) — 다음 CLI 재시작 후 MCP 정상 예상, 그전엔 curl REST 직접(처방 = `docs/rakko-playbook.md` 2026-08-27 절 · settled-decisions §6) |
