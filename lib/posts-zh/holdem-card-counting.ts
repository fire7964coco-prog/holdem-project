...
你能看到的每一张牌，都排除了一些可能性。在德扑里，一张 out 不可能躺在牌面上——真躺在那儿，你这手牌就已经成了——所以真正要追踪的==已死的牌==，是那些在牌面*之外*露出来的：不小心闪出来的一张、进 muck 之前被亮开的一手、你恰好瞄到的邻座弃牌。每一张你看见过的，都是你已经没有了的一张 out。为它们做调整，是好牌手在每条街都会保持的、一种持续而无声的习惯。这也是算牌，只不过是不需要一个累加计数的那一种。
...
> **快速回答**
> 不违规。在自己脑子里数 outs、算赔率、用阻挡牌，是正常的扑克技术，不算外部协助。德扑里你面对的是别的牌手，牌房只抽水（rake），这和 21 点算牌冲着庄家去完全不同。真正要留意的是牌局进行中用设备、听别人出主意——这些由各个牌房、各项赛事自己的规则来管。
...
在德扑里，明着发出来的只有那五张共用的公共牌——其他你能看到的都是意外（闪出来的一张牌、弃牌时被亮开的一手），所以可追踪的东西很少。梭哈奖励的，恰恰是 21 点算牌手擅长的那种追牌功夫——这是扑克里最接近电影那个版本的玩法。
...
A. 不会——只要是你在脑子里数自己的 outs、用阻挡牌。在一个对其他牌手的游戏里，这些都是正常技术；而且不管谁赢，牌房都只收抽水（rake），和冲着庄家去的 21 点算牌不是一回事。
