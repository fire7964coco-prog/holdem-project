...
### ▶▶ 🔴 이 세션의 본체 = **검수장 조치목록 순위4 — `korea-poker-marathon` 러닝 실측 4**

> ✅ **순위3 `wpt-australia` WRONG 14는 2026-08-13에 종료됐다**(`af6c77c`) — 경위·교훈은
> `WORKLOG.md` 2026-08-13 (7). 실측은 **de 14건이 아니라 8로케일 전수**였고 EN이 발원지였다.

- **순위4 `korea-poker-marathon` 러닝 실측 4**(원장 #17 #46 #47 #75) — 종목 **11KM**(글은 10K×3자리) ·
  접수마감 **7/10(금) + 6/26 조기마감**(글은 「15. Jul」×2) · #17 날짜.
  ⚠ **desc가 이미 마감된 대회 참가를 권한다**(RISKY #1) — SERP 노출 필드라 함께 고칠 것.
  ⚠ 그 밖의 RISKY: 「nur für Ausländer」 무한정 반복 3자리(#3 #7 #8 — 재외국민 한정구 탈락) ·
  #33 「durch den Deoksugung-Palast」(도로 코스가 궁 «안»을 통과하지 않는다 → vorbei) · #61 서브4 전제.
  🔒 **국적 예외 3건(#57 #68 #69)은 8/16 묶음이니 이번에 열지 마라.**
  ⚠ **JTBC 3경로 403** — 출발시각 07:30은 2차에만 있다. 1차 못 열면 **주장 자체를 빼라**(고유명을
  다른 고유명으로 바꾸면 오류가 이동할 뿐이다). YTN 공식은 검수장이 열었다.
...
🔴 **분모를 지시서대로 믿지 마라 — 3연속 깨졌다**(순위1·2·3 전부). 착수 전 **전 로케일 grep으로 직접 세라.**
그리고 **grep 0건을 「전부 고쳤다」로 읽지 마라** — 순위3에서 패턴이 두 번 미탐했다
(es 비용표 «각주» · zh FAQ 「2 周」는 공백 때문에 「两周」와 안 걸렸다). **FAQ·각주는 직접 열어라.**

🟢 **`starpoker.com.au`는 200으로 열린다**(worldpokertour.com만 403). 대용량·이미지형 PDF는
`pdftoppm`으로 렌더해 읽으면 된다 — 순위3에서 이 방법으로 Player Guide 39p를 판독했다.
🔴 **표의 날짜 귀속을 `pdftotext`로 판정하지 마라** — 병합셀이라 날짜가 엉뚱한 행에 붙는다. 이미지로 봐라.
...
| **검수장 재검수 원장** | 42편 2,988주장 · OK 2,558 · **WRONG 30 · RISKY 116 · 미해결 146** — 이 중 **순위1·2·3이 닫혔다**(WRONG 20/30 처리 · 남은 건 순위4의 4건 + 순위5 RISKY + 🔒8/16 국적축 5건) |
