...
export const POST: Post = {
  ...
  tldr: "limp（平跟）就是翻前不加注也不弃牌，只用跟大盲的方式进池。open-limp（开池平跟，第一个进池）几乎永远是错的——limp 没法不战而收下盲注，你交出了主动权，还会被高手针对惩罚。但 limp 也不是永远都错：补齐小盲、跟在别人后面 over-limp 投机牌、以及一些现场牌局和锦标赛短码情形，都是正当的例外。",
  category: "strategy",
  date: "2026-07-05",
  updated: "2026-09-26",
...
**limp（平跟）** 就是翻前不加注、也不弃牌，只用*跟*大盲的方式进池。听起来人畜无害，偶尔也确实没问题——但 ==r:open-limp（开池平跟）当第一个进池的人== 是这游戏里最常见、也最烧钱的习惯之一。下面就把 limp 到底是什么、为什么它通常在亏钱、哪些具体位置其实是对的（它*并非*永远都错），以及强手怎么把你的 limp 变成他们的利润，一次讲清楚。把这一个概念搞对，比大多数人以为的跨度都大——它是一套赢家[德州扑克策略](/zh/blog/holdem-strategy "thumb:/images/holdem-strategy-hero.webp")里的第三个决策，就排在选好座位和起手牌之后。
...
> **快速回答**
> limp（平跟）就是翻前只跟等额的大盲进池——不加注。它只在还没人加注时成立；已经有人加注你再跟上，那叫跟注，不叫 limp。第一个进池就 limp（open-limp）几乎永远是错的，跟在别人后面 limp（over-limp）才有说得过去的用武之地。

**limp 的本质是“不加注地进池”——既不弃牌，也不施压。**你投入最低的钱去看一个翻牌。关键在于，limp 只适用于*还没有人加注*的时候：如果已经有人加注、你去跟上，那叫**跟注**，不是 limp。这个词专指用被动、最便宜的方式进入一个没被加注的底池。

有两个词大家常混在一起，值得分清。**limper（爱平跟的人）** 是指用跟大盲的方式进入未加注底池的人。**calling station（跟注站）** 是指跟得太多、很少加注也很少弃牌的人——这个习惯在翻牌、转牌、河牌上最明显。它们常常指同一个松被动的玩家，但强调的是不同的习惯——一个说的是你怎么*进*池，另一个主要说的是你怎么*继续*打。如果这些词里有哪个把你绕晕了，一份[扑克术语词汇表](/zh/blog/holdem-glossary)能帮你理清剩下的行话。
...
> **快速回答**
> open-limp = 第一个进池只跟大盲；over-limp = 已经有人 limp 之后再跟。前者交出主动权，后者是打折进多人底池——两个词说的是完全不同的两件事。
...
> **快速回答**
> open-limp 让你放弃四样东西：不战而收下盲注的机会（limp 时是零）、下注主动权、对底池大小和位置的控制，以及不被看穿——习惯性 limp 的范围又封顶又透明，高手一眼就知道该怎么攻击。

当你 open-limp 时，你放弃的东西多得惊人。具体如下：

1. **limp 没法不战而收下盲注。** 这是最要命的一条。当你第一个进池就*加注*时，所有人都可能弃牌，你不战而收下盲注——白拿的钱，而且相当一部分时候都能拿到。当你 limp，这个数字是**零**。你等于给自己上了枷锁，非得成牌或者往后赢下不可；你把最干净的赢法直接扔了。
...
3. **你造出一个膨胀的多人底池——往往还处于不利位置。** limp 会招来更多跟注者，还让大盲花很少就进来。看翻牌的人越多，你的牌就越不值钱；如果你是从前面位置 limp，那你在每条街上都对着几乎整桌人处于*不利位置*，还毫无主动权。那是全桌最烂的座位。
...
> **快速回答**
> 加注能不战而收下盲注；limp 永远不能。开池加注给你两条赢路——所有人翻前弃牌，或带着主动权往后拿下；limp 只留下第二条、更难走的那条，还让大盲花很少就能拿着随手的牌来看翻牌。
...
加注而不是 limp，整套道理归结为一个不对称。当你开池加注，你给了自己*两条*赢路——所有人翻前弃牌，或者你带着进攻方的主动权往后拿下。limp 只给你留下第二条、也更难走的路，还剥掉了让翻前进攻能盈利的那部分弃牌率（fold equity）。
...
> **快速回答**
> limp 并非永远都错，但只有四种情形：未加注底池里补齐小盲、over-limp、非常被动的现场局、锦标赛的短码后位。open-limp 几乎总是错，小盲位补齐除外。

这里就是教条走过头的地方。诚实、现代的答案是——limp *并非*永远都错，例外就是下面这几种具体情形：
...
| 位置 | 为什么这里 limp 没问题 |
|:---|:---|
| **补齐小盲（未加注底池）** | 还没有人加注，你的钱已经进去一半，身后只有大盲会行动——“加注或弃牌”的规则在打折价面前失效了。面对加注就是另一个问题了：3bet 或弃牌。 |
| **over-limp 投机牌** | 跟在别的 limper 后面拿小对子或同花连张，你能拿到极好的赔率，在多人底池里翻出巨怪。 |
| **非常被动的现场低级别** | 如果对手只拿巨牌才加注、从不惩罚 limper，你就能用投机牌花很少看翻牌、兑现胜率。 |
| **短码的后位（锦标赛）** | 在锦标赛的短码深度下——远低于标准现金局的 100BB——现代求解器会给按钮位开发出 open-limp 的范围，那种情况下加注收益很小，而 limp 能压低你的成本。 |
...
> **快速回答**
> limp 再加注（limp-raise）是先 limp、等身后有人加注再反加的陷阱，通常拿 AA、KK 这种巨牌做。它在会思考的对手眼里已经太透明——直接喊出 TT+、AK/AQ 的窄范围——所以只能当偶尔的工具，不能当惯用招。

在一张激进的桌子上拿 AA 或 KK 这种巨牌**先 limp、等身后加注再反加**，既能把底池做大，又能把自己伪装得很弱。
...
> **快速回答**
> 是的——在大多数牌局里，open-limp 就是一块写着“这里有个弱、被动的玩家”的招牌。老练的玩家不只注意到它，还会攻击它：用隔离加注逼你单挑、对着你封顶的范围不停 c-bet 榨薄价值、再用位置在翻后碾压你。

![六人桌示意图——红色标出的座位只用一枚筹码 limp 入池，四个座位弃牌并打上叉、盲注筹码留在桌上，按钮位用金色推出一堆大得多的筹码，箭头指回那个 limper](/images/holdem-limping-isolation-raise.webp "一枚筹码就能进池——但这个底池最后要花你多少，是按钮位的人说了算")

它之所以是个这么烧钱的习惯，是因为强手的攻击有一整套手法：
...
解法简单得让人舒心：**默认加注或弃牌，把 limp 只留给上面那些具体位置。** 你一停掉 open-limp，就不再是全桌最好啃的靶子——而这，恰好就是把你和 [fish（鱼）](/zh/blog/holdem-fish "thumb:/images/holdem-fish-hero.webp")区分开来的第一件事。
...
> **快速回答**
> 线上和更硬的牌局里，open-limp 几乎无可辩解——总有人来 iso-raise 你，GTO 基线就是 100BB 别 open-limp（小盲位补齐除外）。非常被动的现场低级别里代价小得多，但从前面位置 open-limp 依旧是最糟的一种。

有一点老实的提醒，因为环境能改变一切。在**线上和更硬的牌局**里，上面那三招是默认配置，不是偶尔遇上：桌上几乎每个人都在等着惩罚 limp，所以 ==r:一个正常的 100BB 牌局里，open-limp 基本上没有站得住的理由==。上文那四种情形里，只有补齐小盲能原样搬到这种牌局——那里你再补半个大盲就能看翻牌，身后又只剩大盲一个人有加注权，被隔离的代价小得多。

在**非常被动的现场低级别牌局**里，那是另一个世界。如果桌上惯常让 limper 花很少看翻牌、没人惩罚他们，那用投机牌 limp 的代价就小得多——你不会被隔离，还能用那些不太想面对加注的牌去兑现胜率。它仍然不是*最优*——枪口位那几个座位尤其站不住，因为你身后还有七八个人有加注权——但代价很小，在一个“全家福”底池里钓暗三还能印钞。读懂你的桌子：牌局越软越被动，你能蒙混过关的 limp 就越多；牌局越硬，你就越应该严格地加注或弃牌。
...
A. open-limp 放弃了很多：你没法像加注那样不战而收下盲注，你交出了让进攻方能靠持续下注赢池的主动权，还招来一个你往往在不利位置打的膨胀多人底池。除此之外，它把你标记成弱玩家，于是强对手会加注来隔离并剥削你。

**Q. limp 拉一堆人进池，为什么反而更亏？（用数字看）**

A. 因为多人底池会稀释你手牌的胜率。就算是最强的 AA，单挑一个对手时约有 **85% 胜率**；可一旦有 5 个人跟你到河牌，AA 的胜率就掉到 **约 49%**——不到一半。limp 恰恰在做这件事：它招来更多跟注者、造出便宜的多人底池，让你即便拿着好牌也很难兑现价值。加注则相反——它把人赶走、缩小战场，让你的领先真正值钱。这就是 limp 常在悄悄漏筹码的量化原因。

**Q. limp 有没有作为一种好策略的时候？**

A. 有，在特定位置。从小盲补齐、跟在别的 limper 后面 over-limp 小对子和同花连张这类投机牌、非常被动的现场低级别牌局，以及一些锦标赛里短码的按钮位情形，都是正当的。几乎永远错的是 open-limp——当第一个进池的人却选择只跟而不加注。
...
A. 在正常的现金局里几乎永远不该。一手牌如果好到值得玩，通常就好到值得加注；如果不够好，就弃。少数例外是极其被动、你不会被惩罚的现场牌局，以及一些求解器识别出的锦标赛短码后位情形。作为默认，加注或弃牌，跳过 open-limp。

**Q. 在小盲位 limp 可以吗？**

A. 通常可以——在未加注的底池里，补齐小盲是最说得过去的 limp 之一。你的钱已经进去一半，身后只有大盲能行动，你还拿到了一个价钱，所以平常“加注或弃牌”的逻辑在这里不一样。你是补齐、加注还是弃牌，取决于你的牌和大盲的打牌倾向，但在这里 limp 远不像在其他位置 open-limp 那样是个错误。（面对加注时，小盲应该 3bet 或弃牌，而不是平跟。）

**Q. limper 和 calling station（跟注站）有什么区别？**

A. limper 是翻前只跟大盲进入未加注底池的人——说的是他怎么*进*池。calling station 是跟得太多、很少加注也很少弃牌的人，哪条街都一样——这个标签主要说的是他怎么*继续*，尤其是翻牌之后。同一个松被动的玩家常常两样都干，但这两个词强调的是不同的习惯，不该混着用。

**Q. 老是 limp 的人在牌桌上叫什么？**

A. 通常叫“鱼（fish）”——对弱的、长期输钱的玩家的统称——或者叫“菜鸟（donk）”，意思是打得差。（“跟注站”常被扣在同一个人头上，但那个词涵盖的是在任何一条街上都跟得太多，而不是专指 open-limp 这个习惯。）习惯性 open-limp 是新手最明显的破绽之一，这也正是强手会用隔离加注专门盯着 limper 打的原因。不想被贴上这个标签，就默认加注或弃牌。

---

## 要记住的 3 件事

1. **limp 就是跟大盲而不加注**——而 open-limp，也就是第一个进池，几乎永远是错的：limp 没法不战而收下盲注，你交出了主动权，还把自己标记成好啃的靶子。
2. **但它*并非*永远都错。** 补齐小盲、跟在别人后面 over-limp 投机牌，以及被动的现场或锦标赛短码情形，都是正当的例外。那句教条的“永不 limp”言过其实。
