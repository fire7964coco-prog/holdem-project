...
export const POST: Post = {
  ...
  tags: ["德州撲克術語", "術語大全", "中英文對照", "德州撲克英文術語", "撲克名詞", "德撲術語", "玩家類型", "水上水下"],
  content: `
我第一次坐進現場局的時候，整桌人簡直像在說另一種語言。有人「槍口位」，另一個人「3-bet 了 CO」，荷官問我要不要「分兩次發（run it twice）」，等我拿著一對 K 輸牌，別人還告訴我「這都算不上 Bad Beat，只是個 Cooler（冤家牌）」。我像聽懂了一樣點頭。其實我一句都沒懂。

撲克有它自己的一套詞，懂這套詞能幫你兩件事：讓你看起來不像個「魚（Fish）」，也讓你真正跟得上那些能幫你贏錢的策略。這份德州撲克英文術語中英對照表，收的是牌桌上真會冒出來的詞——按 ==g:你實際會怎麼遇到它們== 來分組，而不是一股腦塞進一堵從 A 到 Z 的大牆裡。先看大家最愛搞混的那幾個詞，再按你需要的分類翻。凡是有深入指南的術語，你都能找到一條直達連結。
...
> **快速解答**
> 先整理這 8 個詞的理由很單純：其餘九十幾個術語聽不懂頂多尷尬，這 8 個一搞混就直接讓你付錢——把過牌當跟注、把明三條當暗三條，錯的是動作，不是詞彙。
...
## 下注動作術語（Betting Actions）

> **快速解答**
> 這一節別按順序背。它列的是輪到你時能做的每一件事；其餘各節則是在講這些動作發生在哪個座位、湊成什麼牌、賭掉多少錢。先把這 20 個動作分成「不投錢、跟上、加大、放棄」四類，整桌的話就聽得懂一半。
...
| 術語 | 意思 |
|:---|:---|
| **過牌（Check）** | 不下注、把行動權讓給下一位——只有你不需要再補籌碼跟上下注時才行。 |
...
> **快速解答**
> 與其記座位名稱，不如記「誰在誰之後說話」：按鈕位翻後最後；三人以上時，小盲翻後最先、槍口位翻前最先。單挑則由小盲兼按鈕，翻後最後行動。台灣現場多半直接喊 CO、HJ 這些英文縮寫，中文名稱知道就好。
...
| **小盲位（Small Blind, SB）** | 三人以上時，在按鈕左邊交強制注，翻牌後第一個行動（翻牌後最差的座位）。單挑時小盲就在按鈕位，翻牌後最後行動。 |
| **大盲位（Big Blind, BB）** | 兩個盲注中較大的那個；級別用盲注大小命名（$1/$2），而一個大盲是衡量籌碼量的標準單位。 |
| **槍口位（UTG, Under the Gun）** | 翻牌前第一個行動——需要最緊的開牌範圍。 |
| **CO（Cutoff／關煞位）** | 按鈕右邊；第二好的座位，很適合偷盲。台灣多半直接講「CO」。 |
| **HJ（Hijack／切入位）** | 按鈕右邊第二個；中期位（MP），就在關煞位之前。台灣多半直接講「HJ」。 |
| **LJ（Lojack／劫機位）** | HJ 右邊；同樣是中期位（MP）（名稱會隨桌型變動）。 |
...
> **快速解答**
> 最貴的兩組混淆都在「牌是用哪張做出來的」：暗三條靠口袋對，明三條靠一張底牌配牌面一對；後門是還缺兩張牌，送尾是那兩張真的都到了。順子只要記兩端——百老匯最大、輪子最小。
...
| **[Cooler（冤家牌）](/zh-hant/blog/holdem-cooler)** | 大牌輸給更大的牌、卻沒有任何失誤——是撲克裡最無奈的一種輸法。 |
| **[Bad Beat（爆冷門）](/zh-hant/blog/holdem-bad-beat)** | 你是大熱門，卻被一張幸運牌超車輸掉。 |
...
| **[抽水（Rake）](/zh-hant/blog/holdem-rake)** | 撲克室從大多數底池抽的一小份。 |
| **抽水返還（Rakeback）** | 把你付過的抽水退一部分回來的回饋。 |
| **Bad Beat 大獎（Bad beat jackpot）** | 一手非常強的牌在攤牌時輸掉才賠出的累積獎金——[怎麼觸發](/zh-hant/blog/holdem-bad-beat)。 |
| **[盲抓／強抓（Straddle）](/zh-hant/blog/holdem-straddle)** | 一個可選的額外盲注（通常 2× BB），買下翻牌前最後的行動權。 |
| **現金桌（Cash game）** | 真實面額的籌碼，隨時進出，盲注固定。 |
| **無限注（NLH）／限注（Limit）** | 無限注的首次下注最低為一個大盲，最高到全部籌碼（籌碼不足一個大盲時可全下）；限注把每次下注鎖在固定大小。德州幾乎都是無限注；PLO 採用的底池限注，把下注或加注的上限設為底池大小。 |
...
> **快速解答**
> 三個數據就能讀出對手：VPIP 看他多鬆、PFR 看他多兇，AF 則是（下注＋加注）÷跟注。PFR 永遠不會高於 VPIP，兩者差距越大，這個人越被動。至於禮儀那幾條，數據看不出來，可是一旦犯了，現場局沒人想再跟你同桌。
...
| **水上／水下** | 台灣現場局俗語：這一場你淨贏就是「水上」，淨輸就是「水下」。在「局抽」抽水制下，有些場子只跟水上（贏錢）的人收。 |
| **冷牌（Cold deck）** | 發出 Cooler 的一副倒楣牌（原指老千事先排好的牌）。 |
| **別敲魚缸（Don't tap the glass）** | 別去教育、數落弱玩家——你會把送錢的人嚇跑。 |
| **分兩次發（Run it twice）** | 全下的玩家把剩下的公牌發兩次，各賭一半底池，用來降低運氣波動——只限現金桌，且所有相關玩家都得同意。 |
| **單挑（Heads-up）** | 一對一打——兩人局，或錦標賽最後兩人。 |
| **搶先加注（RFI, Raise First In）** | 範圍與數據用語裡對開池加注的簡稱：前面沒人進池時，你有多常搶先加注入池。 |
...
A. 這是台灣現場局的俗語。這一場（或這一局）你淨贏、帳面是正的，就叫「水上」；淨輸、帳面是負的，就叫「水下」。在採「局抽」（每一局結算時才抽）的撲克室裡，有些場子只跟水上、也就是贏錢的玩家收，輸家不再另外被抽。這和按時間收的「時間抽水」不一樣——那種不管你有沒有贏，時間到了就得付。
...
A. 過牌是不投任何籌碼、把行動權讓下去，而且只有你不需要再補籌碼跟上下注時才允許。跟注是跟上別人已經下的注。過牌免費；跟注要補上你距目前下注還差的金額。把兩者搞混，是新手最常犯的錯。
