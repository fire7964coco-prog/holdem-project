...
3. **🟢 펍 허브 승격**: 현 2편(펍 첫방문·합법)→클러스터 몇 편 추가 시 7번째 허브. (펍=한국 검색 강점)
   ...
   - **⚠️ 사용자 확인(먼저 처리 권장)**: ①연락처=개인 Gmail 직노출 대신 **구글 폼 또는 contact@holdemmaster.com(ImprovMX/Cloudflare 무료 포워딩)** 로 교체 강력 권고 — 총판·머니상 스팸 표적. 텔레그램 핸들 있으면 보조 기입. ②히어로 커스텀 이미지 교체(현재 `holdem-raise-how-much-pub.webp` 재사용). ③GSC 색인 `/blog/holdem-pub-promotion`. ④실제 소개 펍이 쌓이면 펍 허브로 승격.
