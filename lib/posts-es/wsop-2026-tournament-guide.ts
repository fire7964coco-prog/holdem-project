import type { Post } from "../posts";

export const POST: Post = {
  slug: "wsop-2026-tournament-guide",
  seoTitle: "Un joyero de Cancún, US$449.067 y el brazalete: la WSOP 2026",
  title: "WSOP 2026 — guía completa: los brazaletes latinos, la visa desde México y cómo inscribirse",
  desc: "Ganó Lucas Jumalon (US$10.000.000); el mejor hispanohablante, Antonio Galiana, 12.º. Brazaletes hispanos, visa desde México y cómo clasificar desde US$0,50.",
  tldr: "El Main Event de la 57.ª WSOP se resolvió el 5 de agosto: ganó Lucas Jumalon, estadounidense de 22 años, por US$10.000.000. El mejor clasificado de habla hispana fue Antonio Galiana (España) en el puesto 12.º, con US$510.000 — no hubo hispanohablantes en la mesa final. La serie dejó 251.899 entradas —récord histórico— y US$469.975.059 repartidos en 35.157 puestos pagados, con tres brazaletes hispanohablantes: Ciro González (México, US$449.067), Adrian Mateos (España, US$4.334.411) y Juan Rodriguez (Perú, US$673.011). Ojo con el papeleo: México no está en el Visa Waiver Program, así que con pasaporte mexicano hace falta visa y el ESTA no aplica.",
  category: "tournament",
  date: "2026-06-16",
  updated: "2026-08-15",
  masterUpdated: "2026-08-15",
  keepImagesInBody: true,
  readTime: "12 min",
  emoji: "🏆",
  image: "/images/wsop-2026-tournament-guide-hero.webp",
  imageAlt: "Sala de torneo de la WSOP 2026 en Las Vegas, con decenas de mesas de tapete verde llenas de jugadores y torres de fichas bajo los focos",
  tags: [
    "WSOP 2026",
    "brazalete WSOP mexicano",
    "Ciro González WSOP",
    "Adrian Mateos sexto brazalete",
    "cómo inscribirse en la WSOP",
    "visa para jugar la WSOP",
    "matrícula consular WSOP",
    "clasificatorios WSOP GGPoker",
    "Main Event WSOP 2026 mesa final",
    "impuestos premios póker México",
  ],
  content: `
Ciro González viajó a Las Vegas con un amigo para celebrar sus 60 años. El plan era jugar el Evento #61, el $1.000 Super Seniors, y poco más. Cayó en el Día 2.

Su amigo insistió en que se inscribiera en otro torneo, el Evento #65: $1.500 Freezeout No-Limit Hold'em. ==g:Se llevó US$449.067 y un brazalete.== Joyero, de Cancún, y hasta esa semana ==r:jamás había cobrado una cifra de cinco dígitos==.

Esa es, para nosotros, la portada de la 57.ª World Series of Poker. Y no fue la única historia hispana del verano.

¿Nunca has jugado un torneo en vivo de este tamaño? Empieza por [la guía de torneos](/es/blog/holdem-tournament "thumb:/images/holdem-tournament-hero.webp") — entradas, ciegas, registro tardío y el ritmo real de un Día 1.

---

> **Respuesta rápida**
> Los eventos de brazalete de la 57.ª WSOP se jugaron del ==**26 de mayo al 15 de julio de 2026**== en el Horseshoe y el Paris de Las Vegas, y la mesa final del Main Event se disputó tres semanas después, del 3 al 5 de agosto. ==**251.899 entradas**== (récord histórico) y ==**US$469.975.059**== repartidos en 35.157 puestos pagados. Tres brazaletes hispanohablantes: ==g:Ciro González (México), Adrian Mateos (España) y Juan Rodriguez (Perú)==. El Main Event lo ganó ==**Lucas Jumalon**== el ==**5 de agosto**== (US$10.000.000) y el mejor de habla hispana fue ==r:**Antonio Galiana (España), 12.º**==. ==r:Con pasaporte mexicano necesitas visa: México no está en el Visa Waiver Program.==

---

## México suma una página dorada: el brazalete de Ciro González

El Evento #65 ($1.500 Freezeout No-Limit Hold'em) juntó ==**2.617 entradas**== y un pozo de premios de ==**US$3.474.067**==. Freezeout: sin recompras, una bala y a casa.

González llegó a la mesa final y se llevó el primer brazalete de su carrera, además de ==g:US$449.067==. Según **Poker Noticias**, es ==**el sexto brazalete en la historia de México**==.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Puesto | Jugador | País | Premio |
|:---:|:---|:---|---:|
| ==**1.º**== | ==g:**Ciro González**== | ==g:**México**== | ==g:**US$449.067**== |
| 2.º | Kyle Lin | EE. UU. | US$299.218 |
| 4.º | **Octavio Borra** | **Argentina** | US$163.838 |
| 9.º | **Leonardo Alves** | **Brasil** | US$42.801 |

</div>

Tres banderas de La Armada Latina en la misma mesa final — México arriba, Argentina en el cuarto puesto y Brasil abriendo el reparto. No es un dato menor: en un field de 2.617 entradas, llegar a las últimas nueve sillas ya es un logro estadístico enorme, y ahí es donde el [ICM](/es/blog/holdem-icm "thumb:/images/holdem-icm-hero.webp") empieza a mandar más que las cartas.

---

## Mateos entra en el club de los seis, y lo hace más joven que nadie

La otra gran historia hispana del verano llevó el Evento #41: $250.000 Super High Roller No-Limit Hold'em. Solo ==**56 entradas**== y un pozo de premios de ==**US$13.720.000**==.

**Adrian Mateos**, jugador del Winamax Team Pro, se llevó ==g:US$4.334.411== y su ==**sexto brazalete**==. Es ==g:**el jugador más joven de la historia en llegar a seis**==.

Basta con leer la mesa final para entender el tamaño del asunto:

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Puesto | Jugador | Premio |
|:---:|:---|---:|
| ==**1.º**== | ==g:**Adrian Mateos**== | ==g:**US$4.334.411**== |
| 2.º | Bryn Kenney | US$2.776.634 |
| 3.º | David Einhorn | US$1.862.941 |
| 4.º | Sean Winter | US$1.312.037 |
| 5.º | Jason Koon | US$972.375 |
| 8.º | Phil Ivey | US$553.270 |

</div>

Dejó atrás a **Phil Ivey**, a **Jason Koon**, a **Sean Winter** y a **David Einhorn**, y cerró el mano a mano contra **Bryn Kenney**. ==r:No hay campo más duro en toda la serie que un Super High Roller de US$250.000==, y ahí es donde Mateos firmó el sexto.

---

## Y Perú también levantó: Juan Rodriguez en el Seniors High Roller

**Juan Rodriguez (Perú)** ganó el Evento #39: $5.000 Seniors High Roller No-Limit Hold'em, por ==g:US$673.011==.

Con él se cierra la cuenta del verano: ==**tres brazaletes hispanohablantes en 2026**==, repartidos entre México, España y Perú.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Evento | Campeón | País | Premio |
|:---|:---|:---|---:|
| #41 — $250.000 Super High Roller NLH | Adrian Mateos | España | ==g:US$4.334.411== |
| #39 — $5.000 Seniors High Roller NLH | Juan Rodriguez | Perú | ==g:US$673.011== |
| #65 — $1.500 Freezeout NLH | Ciro González | México | ==g:US$449.067== |

</div>

---

## La 57.ª WSOP en cifras: récord de entradas, pero no de premios

La serie se jugó entre el **Horseshoe Las Vegas** y el **Paris Las Vegas**, con **100 eventos** en el calendario. Con el Main Event resuelto el 5 de agosto, ==**los 100 brazaletes ya tienen dueño**==.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Dato | 2026 |
|:---|---|
| Edición | 57.ª WSOP |
| Fechas | ==**26 mayo – 15 julio 2026**== |
| Sedes | Horseshoe Las Vegas + Paris Las Vegas |
| Eventos | 100 (**los 100 brazaletes entregados**) |
| Entradas totales | ==g:**251.899**== (récord histórico) |
| Premios repartidos | ==**US$469.975.059**== |
| Puestos pagados | **35.157** |
| Récord de premios | ==r:sigue siendo 2025: **US$481.761.919**== |
| Evento con más gente | #63 $1.000 Mystery Millions — ==**22.811 entradas**== |
| Ladies Championship | 1.475 entradas |

</div>

Fíjate en el matiz, porque es el titular que casi todo el mundo se come: ==**la WSOP 2026 batió el récord de entradas, no el de dinero repartido**==. Las 251.899 entradas son la marca más alta de la historia, pero los US$469.975.059 se quedaron por debajo de los US$481.761.919 de 2025. Más gente, más torneos baratos, más volumen — y aun así el pico de premios sigue en el año anterior.

---

## Un Main Event con 111 países y casi cuatro de cada diez jugadores de fuera

El Evento #82, el **$10.000 No-Limit Hold'em World Championship**, reunió ==**9.208 entradas**== y un pozo de premios de ==g:**US$87.568.080**==.

![Calendario de los vuelos del Día 1 del Main Event de la WSOP 2026, del 2 al 5 de julio, con las horas de inicio de cada jornada de salida](/images/wsop-2026-main-event-schedule.webp "Los vuelos del Día 1 del Main Event 2026 se repartieron entre el 2 y el 5 de julio")

Los **vuelos** del Día 1 se repartieron entre el **2 y el 5 de julio**. Y el dato que más nos interesa desde este lado del mapa: ==**111 países representados, récord de la historia del torneo**==, con ==g:**3.574 jugadores no estadounidenses (38,8 %)**==.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| # | País | Entradas |
|:---:|:---|---:|
| 1 | EE. UU. | 5.634 |
| 2 | Canadá | 371 |
| 3 | Reino Unido | 356 |
| 4 | Francia | 296 |
| 5 | Japón | 222 |
| 6 | ==**Brasil**== | ==g:**172**== |
| 7 | China | 169 |
| 8 | Israel | 154 |
| 9 | ==**España**== | ==g:**148**== |
| 10 | Alemania | 146 |

</div>

**Brasil sexto y España novena** en el conteo por países. Brasil es, con diferencia, la delegación latinoamericana más numerosa del Main Event.

### La estructura, en corto

- Stack inicial: ==**60.000**== fichas
- Niveles de ==**120 minutos**==
- ==r:**Freezeout**== — no hay recompras
- Edad mínima: ==**21 años**==

Si vas a jugar un torneo con niveles de dos horas, conviene tener el [ranking de manos](/es/blog/holdem-hand-rankings "thumb:/images/holdem-hand-rankings-hero.webp") tan interiorizado que no lo pienses: en un freezeout de esta profundidad no hay segunda bala para corregir un error de lectura.

## ¿Quién ganó el Main Event de la WSOP 2026?

**Lo ganó Lucas Jumalon, estadounidense de 22 años, por US$10.000.000.** La mesa final se jugó del 3 al 5 de agosto y se resolvió el día 5. Ahí se entregó el brazalete número 100 de la serie.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Puesto | Jugador | País | Premio |
|:---:|:---|:---|---:|
| 🏆 1.º | ==**Lucas Jumalon**== | EE. UU. | ==g:**US$10.000.000**== |
| 2.º | Lauri Saaskilahti | Finlandia | US$6.000.000 |
| 3.º | Gregor Mueller | Canadá | US$3.750.000 |
| 4.º | Michael Gagliano | EE. UU. | US$2.750.000 |
| 5.º | Hanming Feng | EE. UU. | US$2.250.000 |
| 6.º | Rami Hammoud | Canadá | US$1.750.000 |
| 7.º | Jamie Shaevel | EE. UU. | US$1.500.000 |
| 8.º | Mario Boos | Francia | US$1.250.000 |
| 9.º | Evagoras Evagorou | Chipre | ==**US$1.000.000**== |

</div>

**Sin hispanohablantes en la mesa final.** Revisamos directamente la columna *Country* de los puestos 1 al 50 en la página oficial de resultados: el mejor clasificado de habla hispana fue ==g:**Antonio Galiana (España), 12.º, con US$510.000**==. Ni México, ni Brasil, ni ningún país sudamericano aparece entre los 50 primeros (fecha de consulta: 6 de agosto de 2026; de los puestos 51 en adelante no revisamos país por país).

Duele un poco leerlo junto al conteo de entradas de arriba: Brasil metió 172 jugadores y España 148, y ninguno de los dos puso una bandera en la mesa final. Galiana se quedó a tres puestos de las últimas nueve sillas. El torneo se sigue decidiendo donde hay más volumen.

---

## Dónde ver la WSOP 2026: ESPN para la mesa final, YouTube para todo lo demás

Aquí hubo cambio, y grande: ==r:desde 2026 **PokerGO ya no emite en directo**== la WSOP (sigue encargándose de la producción). A cambio, ==g:**99 eventos se pueden ver gratis en el canal oficial de YouTube de la WSOP**==.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Día | Canal | Horario (ET) |
|:---|:---|:---|
| 3 de agosto | ESPN2 | 21:00–24:00 |
| 4 de agosto | ESPN | 21:00–24:00 |
| 5 de agosto | ESPN | ==**22:30–01:30**== |

</div>

---

## Con pasaporte mexicano no basta el ESTA: así se entra a Estados Unidos

Vamos con lo que de verdad decide si juegas o no. ==r:**México no está en el Visa Waiver Program**==, el programa de 42 países del Departamento de Seguridad Nacional (DHS) que permite viajar sin visa. Con pasaporte mexicano ==r:**necesitas visa**==; el ESTA no es una opción, porque el ESTA solo se puede pedir con pasaporte de un país del VWP.

Lo mismo aplica a ==r:**Argentina, Brasil, Colombia y Perú**==: tampoco están en la lista, también necesitan visa.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| País | ¿Visa Waiver Program? | Vía de entrada |
|:---|:---:|:---|
| ==**México**== | ==r:**NO**== | ==r:**Visa**== |
| Argentina | ==r:NO== | Visa |
| Brasil | ==r:NO== | Visa |
| Colombia | ==r:NO== | Visa |
| Perú | ==r:NO== | Visa |
| ==**Chile**== | ==g:**SÍ**== (desde el 31 de marzo de 2014) | ESTA |
| España | ==g:SÍ== (desde el 1 de octubre de 1991) | ESTA |

</div>

**Chile es el único país de América Latina en la lista.** Es la excepción, no la regla — y conviene saberlo porque más de una vez se cuenta al revés.

### Si te toca ESTA (Chile, España)

- Coste: ==**US$40,27**==
- Estancia de hasta **90 días**
- Hace falta pasaporte de un país del VWP
- Se paga con MasterCard, VISA, American Express, Discover (solo las marcas JCB y Diners Club) o PayPal
- El formulario lleva unos **23 minutos**
- La CBP recomienda pedirlo ==**"antes de comprar los billetes"**==

> ⚠️ **Y esto vale para todos.** La CBP lo dice textualmente: la autorización ESTA ==r:**no decide** si te dejan entrar==. Quien lo determina es el agente de CBP en el punto de llegada.

---

## La matrícula consular sirve en el mostrador de la WSOP — y solo a México

Este es, probablemente, el dato más útil de todo el artículo para un lector mexicano, y también para el paisano que ya vive en Estados Unidos. El reglamento oficial de la WSOP dice, textualmente:

> "If a Participant is not a U.S. citizen, Participant must have one of the following that is current, valid, and unexpired: passport, foreign government issued Driver's License (License must have a photograph, date of birth, and a valid expiration date), **Consular ID or Matrícula Consular Card (Mexico only)**, or U.S. Permanent Resident Card (PRC) is required. If the current, valid, and unexpired photo identification does not include a physical address, a second form of ID that indicates physical address will be required. This second form of ID can be a driver's license, utility bill, credit card statement, etc."

Traducido a lo práctico:

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Documento aceptado | Condición |
|:---|:---|
| Pasaporte | Vigente y sin caducar |
| Licencia de conducir extranjera | Con foto, fecha de nacimiento y caducidad válida |
| ==g:**Matrícula consular / ID consular**== | ==g:**Solo México**== |
| Green Card (U.S. Permanent Resident Card) | Vigente |

</div>

Dos cosas que hay que subrayar:

**1. La matrícula consular vale, y es una excepción exclusiva de México.** No aparece ningún otro país en esa cláusula del reglamento.

**2. Hace falta un segundo documento con domicilio.** Si tu identificación con foto ==r:no incluye una dirección física==, el reglamento exige un segundo documento que sí la lleve. ==r:Y los pasaportes no llevan domicilio==, así que si vas a identificarte con pasaporte, prepara también una licencia de conducir, un recibo de servicios o un estado de cuenta de tarjeta.

El listado completo de documentos y el paso a paso del registro los tienes en [cómo entrar al WSOP](/es/blog/como-entrar-al-wsop "thumb:/images/como-entrar-al-wsop-hero.webp"), que es la guía de procedimiento. Aquí nos quedamos con lo que cambia si el pasaporte es mexicano.

---

## Dos cuentas, una fila y un tope de US$10.000: cómo se inscribe uno en el Horseshoe

Aquí es donde se atasca el que llega por primera vez. Si buscas el procedimiento completo, está en [cómo entrar al WSOP](/es/blog/como-entrar-al-wsop); lo que sigue es el resumen y, sobre todo, las cifras de comisión que suelen pillar por sorpresa. El reglamento pide ==**dos cuentas**==, no una:

:::steps
Abre tu cuenta de Caesars Rewards | Es el programa del grupo que opera el Horseshoe y el Paris. Sin ella no pasas de la puerta del registro.
Abre tu cuenta de WSOP LIVE | Es la app de torneo. ==r:Ya no es Bravo Poker Live== — si el nombre que te suena es ese, estás mirando información vieja.
Pasa por verificación en persona | El área está en el **Versailles Ballroom**, del lado del Paris. Aunque hayas pagado la entrada desde la app, ==r:hay que presentarse ahí con tu documento==.
Llega una hora antes, pero solo la primera vez | La organización recomienda ese margen para la verificación inicial. ==g:Una vez verificado, ya te inscribes sin cola.==
:::

### Formas de pago (y sus trampas)

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Método | Límite / condición | ¿Para no residentes? |
|:---|:---|:---:|
| Tarjeta de crédito | ==r:**2 %** de comisión== · máx. US$10.000 por transacción | ✅ |
| ==g:**Tarjeta de débito**== | ==g:**sin comisión**== · mismo tope por transacción | ✅ |
| PayPal | ==r:**2 %** de comisión== (igual que el crédito) | ✅ |
| **Luxon Pay** | — | ✅ |
| ACH | ==r:solo residentes en EE. UU.== | ❌ |
| Transferencia bancaria | desde **US$100.000**, con aprobación previa | ✅ |
| Cheque de caja | con aprobación previa | ✅ |
| Cuenta de torneo (TA) | se abre en el Paris con efectivo o fichas | ✅ |

</div>

Haz la cuenta del Main Event: la entrada de US$10.000 con **tarjeta de crédito o PayPal** lleva un **2 % de comisión**, así que ==r:**salen US$10.200**==. Con ==g:**tarjeta de débito no hay comisión**==: US$10.000 exactos. Es la diferencia más barata de este artículo — US$200 por elegir bien el plástico. 🪶 Un detalle: las tarjetas de débito llevan **límite diario de consumo** puesto por tu banco, así que un cargo único de US$10.000 puede rebotar. Súbelo antes de viajar.

Y el tope de US$10.000 por transacción ==g:**se mide sobre el buy-in del evento, no sobre lo que acabas pagando**==: el reglamento pone de ejemplo que con una misma tarjeta puedes inscribirte al Main Event de US$10.000 **y además** al Monster Stack de US$1.500, pero no a un evento de US$50.000. Es decir, el 2 % no te deja fuera del tope. ⚠️ **La página de inscripción sigue publicando «3 %»** — aquí manda el reglamento de la temporada, pero él mismo avisa de que las comisiones pueden cambiar: mira el importe final en la pantalla de pago.

Tres reglas más que cuestan disgustos:

- ==r:**No aceptan tarjetas virtuales ni digitales.**== Plástico físico.
- El **nombre de la tarjeta debe coincidir con el del documento**.
- ==r:**Está prohibido inscribir a otra persona.**== Cada jugador se registra a sí mismo.

---

## En la mesa solo se habla inglés — pero no tienes que cantar tu jugada

Esta es la duda número uno del que viaja sin dominar el idioma, y la respuesta tranquiliza más de lo que la gente espera.

Mientras haya manos en juego, ==**en las mesas solo se habla inglés**==. Pero el vocabulario obligatorio son ==g:**siete palabras**==:

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Palabra oficial | Cuándo |
|:---|:---|
| **bet** | apostar |
| **call** | pagar |
| **fold** | retirarse |
| **check** | pasar |
| **all-in** | ir con todo |
| **pot** | ==solo en Pot-Limit== |
| **complete** | completar |

</div>

Y el detalle que quita el miedo escénico: ==g:**no hace falta cantar tu jugada en inglés**==. Las cartas deciden; la declaración verbal no obliga. Si llegas al showdown con la mejor mano, la mesa la lee, digas lo que digas.

### El resto del reglamento de mesa

- **Móvil**: puedes escribir en la mesa ==si no estás en la mano==. Para ==r:hablar por teléfono hay que alejarse al menos una mesa==.
- ==r:Nada de móvil, tablet, portátil ni funda de auriculares **encima de la mesa**.==
- **Auriculares**: permitidos ==**hasta las tres últimas mesas**==. ==r:Prohibidos los aparatos con funciones de IA.==
- **Gafas de sol y capucha**: sí. ==r:Taparse la cara: no.==
- **Reentrada**: solo en los eventos que la admiten, y ==r:únicamente con cero fichas==.

---

## De US$0,50 al Main Event: la escalera de clasificatorios

La propia web de la WSOP lo anuncia así: ==g:**"Qualify for as Little as $0.50"**==. El socio online oficial a nivel internacional es **GGPoker**, y el camino es una escalera de pasos.

![Escalera de clasificatorios de GGPoker para la WSOP 2026, desde el paso gratuito y el de US$0,50 hasta el Bracelet Pass de US$10.000](/images/wsop-2026-ggpoker-satellite.webp "La escalera de satélites de GGPoker: de US$0,50 al Bracelet Pass de US$10.000")

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Paso | Formato | Entrada |
|:---|:---|:---:|
| Step 0 | Gratis, entrando cada día | ==g:**US$0**== |
| Step 1 | SNG de 4 jugadores, all-in or fold | **US$0,50** |
| Step 2 | Spin & Gold | **US$2** |
| Step 3 | Target Stack Turbo | **US$10** |
| Step 4 | MTT normal | **US$150** |
| Premio | ==**Bracelet Pass**== | ==g:**US$10.000**== |

</div>

También hay atajos: se puede entrar **directo al Step 4 por US$150**, o pasar por el **Bracelet Step de US$1.000** o el **Super Step de US$2.500**.

> ⚠️ ==r:El número de plazas cambia cada temporada.== Consúltalo en la web oficial antes de sentarte a grindear la escalera.

### Satélites presenciales (1–6 de julio)

Si ya estás en Las Vegas, la otra vía es el satélite en vivo:

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Satélite | Entrada | Hora | Stack objetivo |
|:---|:---:|:---|:---:|
| Deepstack Step Satellite | US$150 | 10:00 | 200.000 |
| Landmark Satellite | US$585 | 10:00 y 16:00 | 200.000 |
| Turbo Satellite | US$1.100 | 8:00 | 150.000 |
| Landmark Satellite | US$2.200 | 19:00 | 200.000 |

</div>

Al alcanzar el stack objetivo eliges: ==g:**asiento de US$10.000 o US$10.000 en fichas**==. La segunda opción existe, y en más casos de los que parece es la buena.

---

## Lo que se lleva el fisco: primero México

Antes de contar las ganancias, cuenta lo que no es tuyo.

**México.** Los premios de póker en vivo llevan el ==**Impuesto por Obtención de Premios del 6 %**== más un ==**1 % de ISR**==.

**Estados Unidos.** Puede haber ==r:**retención en origen**== sobre los premios de no residentes. Existe y hay que contar con ella; ==r:no vamos a darte un porcentaje==, porque depende del caso.

**España.** Los ingresos por torneos, en vivo y online, son declarables. Sobre si se pueden compensar las pérdidas ==r:hay versiones contradictorias==, así que aquí no hacemos cuentas: consulta directamente con la Agencia Tributaria.

**Argentina, Colombia, Perú y Chile.** ==r:No tenemos un dato verificado y no lo vamos a inventar.== Consulta con un contador de tu país antes de viajar.

---

:::readnext[Sigue leyendo]
/es/blog/ept-barcelona-2026-guide | Guía del EPT Barcelona 2026 | /images/ept-barcelona-2026-guide-hero.webp
/es/blog/apt-incheon-2026-guide | Guía del APT Incheon 2026 | /images/apt-incheon-2026-guide-hero.webp
:::

## Preguntas frecuentes sobre la WSOP 2026

**Q. Soy mexicano, ¿me sirve el ESTA para ir a la WSOP?**

A. No. ==r:México no está en el Visa Waiver Program==, y el ESTA solo se puede solicitar con pasaporte de uno de los 42 países de esa lista del DHS. Con pasaporte mexicano hace falta visa. En América Latina, ==g:**Chile es el único país del VWP**== (desde el 31 de marzo de 2014).

**Q. ¿Puedo identificarme en la WSOP con la matrícula consular?**

A. Sí. El reglamento oficial acepta ==g:"Consular ID or Matrícula Consular Card (**Mexico only**)"==. Es una excepción exclusiva para México. Eso sí: si tu identificación con foto no incluye domicilio, ==r:te pedirán un segundo documento que sí lo tenga== (licencia de conducir, recibo de servicios, estado de cuenta de tarjeta).

**Q. ¿Cuánto cuesta de verdad inscribirse al Main Event con tarjeta?**

A. Depende del plástico. Con **tarjeta de crédito o PayPal** la casa cobra un **2 %**, así que son ==r:**US$10.200**==; con ==g:**tarjeta de débito no hay comisión**== y pagas US$10.000 exactos. Hay un tope de **US$10.000 por transacción**, medido sobre el buy-in del evento y no sobre el total cobrado — el reglamento pone de ejemplo que una misma tarjeta cubre el Main Event de US$10.000 **y** el Monster Stack de US$1.500, pero no un evento de US$50.000. Y ==r:no aceptan tarjetas virtuales ni digitales==: el nombre de la tarjeta debe coincidir con el de tu documento.

**Q. No hablo inglés. ¿Puedo jugar igual?**

A. Sí. Mientras hay manos en juego solo se habla inglés en las mesas, pero el vocabulario obligatorio son ==g:siete palabras: bet, call, fold, check, all-in, pot (solo en Pot-Limit) y complete==. Y ==g:no tienes que cantar tu jugada==: las cartas deciden, la declaración verbal no obliga.

**Q. ¿Cuál es la vía más barata para llegar al Main Event?**

A. La escalera de GGPoker, que arranca en un ==g:Step 0 gratuito== (entrando cada día) y sigue con pasos de US$0,50, US$2, US$10 y US$150 hasta el **Bracelet Pass de US$10.000**. Y si ya estás en Las Vegas, los satélites presenciales del 1 al 6 de julio, desde US$150.

**Q. ¿Dónde se puede ver la WSOP ahora que PokerGO no emite en directo?**

A. ==g:99 eventos se ven gratis en el canal oficial de YouTube de la WSOP.== La mesa final del Main Event se televisó: **3 de agosto en ESPN2** y **4 y 5 de agosto en ESPN** (21:00–24:00 ET; el día 5, de 22:30 a 01:30 ET).

---

## Fuentes y aviso

- **★Resultado final del Main Event (Evento #82)** *(consultado el 6 de agosto de 2026)*: [wsop.com/tournaments/result/619](https://www.wsop.com/tournaments/result/619/) — campeón Lucas Jumalon (EE. UU.) US$10.000.000, los nueve puestos con país y premio, **mejor hispanohablante: Antonio Galiana (España), 12.º, US$510.000** (columna *Country* de los puestos 1 al 50 revisada directamente), fecha de cierre 5 de agosto (el campo End Date de la página oficial indica «Aug 05 01:48 AM»), 9.208 entradas, 1.383 puestos pagados, **pozo US$87.568.080**
- **★Los 100 brazaletes entregados** *(consultado el 6 de agosto de 2026)*: [calendario oficial de la WSOP 2026](https://www.wsop.com/tournaments/2026-57th-annual-world-series-of-poker/) — se revisó la columna *Winner* de los eventos #1 al #100: **los 100 tienen campeón**, sin huecos
- Reglamento de inscripción, formas de pago e identificación: [wsop.com/registration](https://www.wsop.com/registration/)
- **★Reglamento oficial 2026 en PDF — la fuente de la comisión** *(consultado el 15 de agosto de 2026)*: [2026-WSOP-Tournament-Rules.pdf](https://wsop.gg-global-cdn.com/wsop/pdfs/2026-WSOP-Tournament-Rules.pdf) · Regla 5, textual: **2 % de comisión** con Visa, Mastercard, Discover, American Express **o PayPal**, **sin comisión en tarjeta de débito**, y "Fees are subject to change". ⚠️ **La página de inscripción sigue publicando «3 %» y no menciona PayPal ni débito** — tomamos el reglamento de la temporada como fuente. Comprueba el importe final en la pantalla de pago
- Retransmisión y cobertura de la serie: [wsop.com/news](https://www.wsop.com/news/)
- Calendario de emisión de la mesa final: [espnpressroom.com](https://espnpressroom.com/) · [espn.com](https://www.espn.com/)
- Análisis del Main Event por países: [pokerfuse.com](https://pokerfuse.com/)
- Totales de la serie (entradas, premios, puestos pagados): [cardplayer.com](https://www.cardplayer.com/)
- Adrian Mateos, Evento #41 y Ciro González, Evento #65: [pokernews.com](https://www.pokernews.com/)
- Lista completa de campeones 2026: [poker.org](https://www.poker.org/)
- Sexto brazalete en la historia de México: [pokernoticias.com](https://www.pokernoticias.com/)
- Visa Waiver Program y ESTA: [dhs.gov/visa-waiver-program](https://www.dhs.gov/visa-waiver-program) · [esta.cbp.dhs.gov](https://esta.cbp.dhs.gov/) · [cbp.gov](https://www.cbp.gov/)

> ⚠️ **Aviso.** Las normas de inscripción, los métodos de pago y las comisiones ==r:cambian cada temporada==, igual que el número de plazas de los clasificatorios online. Comprueba siempre la web oficial de la WSOP antes de inscribirte. Y recuerda: la autorización ESTA no decide si te dejan entrar en Estados Unidos — eso lo determina el agente de CBP a tu llegada.

---

## Guías relacionadas

<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/es/tournaments" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Calendario</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Torneos en vivo, fecha por fecha</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Qué se juega y dónde, temporada 2026</div>
  </a>
  <a href="/es/blog/holdem-tournament" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Lectura obligada</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Cómo funcionan los torneos de póker</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Entradas, ciegas, satélites y checklist del Día 1</div>
  </a>
  <a href="/es/blog/holdem-icm" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Mesa final</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">ICM: por qué en la mesa final no se juega igual</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">El modelo que decide los saltos de premio</div>
  </a>
</div>
`,
};
