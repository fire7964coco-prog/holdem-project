...
| WebP quality | 🔴 **82** (기본, `compress-images.mjs`) — 낮추려면 `--quality=NN`. **옛 «65 / `--aggressive` 60»은 폐기**(2026-08-30) |
| effort | **6** |
| 파일당 목표 | 🔴 **경직된 상한 금지 · 무제한 아님** — 게이트 `npm run check:images`(폭≥750→150KB · 폭<750→80KB). 옛 «≤60KB» 폐기 |
| 글당 합계 | **디스크 합계로 재지 말 것** — 아래 §전송량 기준 참조 |

### 🔴 인코딩 품질 = q82 · 경직된 상한만 금지(무제한 아님) — 2026-08-30 사장님 지시
...
| 항목 | q65 | **q82(정본)** | q88 | 판정 |
|---|---|---|---|---|
| **최대 국소오차** (눈에 띄는 아티팩트) | 35 | **20** | 19 | **q82가 이미 q88 수준** |
| 디스크 (49장 합계) | 1.88MB | **2.36MB** | 2.83MB | q88은 20% 더 쓰고 얻는 게 없다 |
| 🪶 **모바일 전송량** (640px AVIF·4장) | 157.7KB | **147.8KB** | 145.4KB | 🔴 **품질을 올리면 모바일이 오히려 6% 가벼워진다** |

🔴 **«저장 품질 ↑ = 모바일 느려짐»은 틀린 직관이다.** 원본 아티팩트가 적을수록 AVIF가 효율적으로
압축하기 때문에, q82는 모바일에도 **유리하다.** 눈에 띄는 아티팩트를 만드는 건 평균오차가 아니라
**최대 국소오차**이고, 그 값이 q82에서 이미 q88에 닿는다 — 그래서 q88이 아니라 q82다.

**데스크톱이 갈리는 이유** = 히어로 `sizes`가 `(max-width:768px) 100vw, (max-width:1200px) 90vw, 1200px`라
**축소 없이 원본 해상도를 그대로 받는다.** 전송량은 AVIF 재인코딩이라 원본 webp 용량과 무관하다.

🔴 **`compress:images`의 `WEBP_QUALITY`를 낮추지 마라** — 돌리는 순간 q82 자산이 통째로 되돌아간다.

### 🔴 모바일 속도의 진짜 부채 = «폭 750px 미만» 그룹 (품질이 아니다)

15장·713KB. 이들은 **AVIF로 변환하면 오히려 62% 커진다**(이미 q45~50으로 강압축된 세로 사진이라
재인코딩이 아티팩트까지 보존하려 든다) → next/image가 **원본을 그대로 내보낸다** = 디스크 = 전송량.

| 글 | 모바일이 통째로 받는 양 |
|---|---|
| `apt-incheon-2026-guide` | **405KB** (10장) |
| `korea-poker-marathon-2026` 계열 | **309KB** |

**처방은 품질 조정이 아니다** — ①정보 없는 영역 **크롭**(화질을 올리면서 용량이 준다)
②**더 큰 원본으로 교체**(1200px면 파이프라인이 줄여준다).
게이트 `npm run check:images`가 이 그룹을 따로 센다.
...
1. Squoosh에서 quality **82** 이상으로 저장하거나  
...
- 가로 1200px 초과 시 리사이즈  
- WebP quality **82**로 재인코딩 (낮추려면 `--quality=NN` — 🔴 낮추지 마라, §품질 절)  
...
- Photoshop — Export As → WebP, **Quality 82**  
...
- [ ] 🔴 **용량 상한 점검은 폐기** — 대신 `npm run check:images`를 돌린다(q82 인코딩 · 폭 1200px · 우회군 경보)  
- [ ] alt·title 캡션을 넣었는가  
...
| 용량 | 🔴 **경직된 상한 금지 · 게이트로 경보**(`check:images`) — 옛 «≤60KB» 폐기 |
...
| quality | 🔴 **82** (2026-08-30 — 옛 «65 / 80KB 초과 시 60»은 폐기) |
