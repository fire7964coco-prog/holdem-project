...
> 직전 회차 = **ja 회차 10 머지·배포**(`4291087c` · 레인 `dc12b1a4`). 경위 = WORKLOG 「2026-09-11 (4)」. 🟢 ja 레인은 지금 **회차 없음** — 사장님 지시 전까지 ja 파일을 만지지 마라.
...
| 무엇 | 규모 |
|---|---|
| `check:answer-echo` 🔴 **0건** · ja length **0**(회차 10 종결) · 🟠 **length zh 137 · zh-hant 53**(09-11 게이트를 로케일 규격으로 갈랐다 — zh·zh-hant **60~110** = 프로토콜 §4). 🔴 **헤드 미결 먼저: zh 상한 110이 맞는가** — zh 코퍼스 중앙값 107·p75 119라 137건이 «위로» 걸린다. 규격을 90~170으로 두면 같은 코퍼스가 68·131건 «아래로» 걸린다. 부채 «수»가 아니라 «규격»의 문제이니 레인 회차를 열기 **전에** 판정하라(후보: 중문 상한 130 · 근거는 EN 40~75단어 ↔ 중문 정보밀도 실측). 🪶 판정 뒤 zh·zh-hant는 ja 회차 10 브리프(`Holdem-ja/docs/harden-brief/ja-회차10-직답길이.md`) 방식 복제 + **네이티브 렌즈 1순위**(§3-K 1 — 압축은 조사를 먼저 깨뜨린다). 🔴 **블록을 고치면 LCS가 «다음 후보»로 옮겨 붙는다 — 고칠 때마다 게이트를 다시 돌려라**(§3-J 2) · 게이트는 «포인터 블록»·카드 글리프 중복을 원리상 못 본다(§3-K 5) | 중 |
| 🟠 **ja 회차 10이 헤드에 남긴 판정 3**(`harden-ja-진행` §5-H 3·5·10) — pot-odds L194 카드 행 中黒 이탈 · showdown 「マック 마너/손해」 FAQ 신설 여부(`ポーカー マック` 170 중 マナー·デメリット ≈40) · bubble 「複数シート」 4자리. 레인 몫(§2-J: 포인터 블록 3자리 · 常体 판정 · 「何で」 본문 잔여)은 회차 11에 | 소 |
| 🔴 **EN-먼저 18건 = ar 11(§2-A) + ja 5(§2-B) + 🆕 ja 회차 10의 2(§2-C: J#6 short-stack→pot-odds 링크 0 · J#7 tournament 표 Turbo/Mystery Bounty/Add-on 행 부재)** — `docs/en-first-queue.md`. **ar**: §13급 4 = 누적 재개방 «전원» 일반화(TDA §47A) · cards-speak FAQ 자기모순(TDA §14) · 올인 전원공개를 캐시까지 일반화(LA §149) · WSOP §117 룰북 미구분. **ja(J#1~J#5)**: `flush-vs-straight` 「Spot 2」 제목↔본문 불일치(🔴 · ja는 블록만 고쳐 둬 **지금 제목이 부정확**) · `all-in-rules` ミス4 국면 미명시 · `continuation-bet` 「97%」 사이즈 부재(**D유형**) · `strategy` 「콜엔 아무도 안 죽는다」 멀티웨이 거짓 · `bad-beat` 「50/50」(실제 43.2/56.8). 🔴 **EN을 고치면 8로케일 전파가 따라온다** — 회차를 따로 열어라 | 중~대 |
...
**레인 상태(여기만 유지)**: zh **42/42 완주** · zh-hant **42/42 완주**(회차 7 소급까지) · ja **회차 1~10 머지 완료**(회차 10 = `4291087c` · 레인 `dc12b1a4`) · 세 레인 전부 **회차 없음**.
