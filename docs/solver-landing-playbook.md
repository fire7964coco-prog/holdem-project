...
>
> **2026-09-14 로컬 작업 갱신**: 사용자 직접 지시로 `/ms/solver`를 신설해 소스상 **11개 언어**가 된다. 위 10개는 직전 배포 기준이며, 이번 MS와 기존 FAQ 정정의 검증·배포 상태는 WORKLOG를 확인한다. 랜딩 미구현 언어는 **hi**다. MS 조사·라벨·현지 전문가 페르소나 기록 = `docs/ms-solver-landing-brief.md`.
>
