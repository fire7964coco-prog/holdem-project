...
> ✅ **1단계**(미러 47곳) `a66ef97` · ✅ **2단계 A층 15/15** `3d51c1c`~`71f4b90` — **전건 승인·원장 반영 완료**
> ✅ **C3·C1·C2·C4-a·C6 완료 = 21/42** (`1e4223c` `fcaa258` `8f2e6ea` `3e97b2b` `739730d` `aa09f54`, 전부 푸시).
> 🔴 **다음은 C7**(6건) **→ C8**(7건) **→ C5**(6건) **→ C4-b**(2건) **— 남은 21건.**
> ⚠ **C4-b(`texas-…-beginners` #28·#29)와 C5의 `game-order` #31은 25개 로케일**이다 — 한 건이 25곳이니
> 8로케일 묶음과 섞지 말고 **별도 단위**로 잡아라. 원장 근거는 **34건 전부 읽어 정리해 뒀다**
...
### 상태 (2026-08-11 마감 · **A층 15 + C층 21건 반영 후에도 지표 불변** — 실측 재확인 · 푸시 완료)
...
| ja 원장 | 유효 **1368** / 무효 **12** — C2 4 + C4-a 3 + C6 5(재판정 «이후» 적용분). A층 15 · C3 4 · C1 4는 **전부 OK 반영 확인** ✅ |
...
## ▶ 검수장 인계 트랙 — **1·2단계 완료 · C층 21/42 진행 중 · B층은 판정 대기**
...
남은 것은 **C층 21건 · B층 7건**이다.
푸시하면 검수장이 재검수한다(확인: `node tools/ledger-check.mjs --locale ja`
→ 현재 유효 **1368** / 무효 **12** = C2 4 + C4-a 3 + C6 5).
