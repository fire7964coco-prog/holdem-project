...
export const POST: Post = {
  ...
  title: "Tabela de mãos iniciais e as melhores mãos do poker",
  ...
  tags: [
    ...
    "range de maos por posicao poker",
    "range de mãos poker pdf",
    "melhores mãos do poker pré flop",
    "tabelas gto de pré-flop",
    "mãos iniciais 6-max",
  ],
  ...
  content: `
Na minha primeira sessão ao vivo, peguei A♣ 4♦ e pensei "um ás — não deve ser tão ruim".

Paguei um aumento, errei o flop, paguei de novo, errei o turn. No river eu já tinha perdido 40 big blinds com nada.

Aqui está a matemática incômoda por trás dessa mão: ==o Texas Hold'em tem 169 tipos distintos de mãos iniciais — e cerca de 80% das mãos que você recebe deveriam ser foldadas no pré-flop.== Aprender quais mãos jogar — e de qual assento — é a maior evolução que iniciantes fazem no primeiro mês. A seleção de mãos é a segunda das [cinco decisões](/pt/blog/holdem-strategy) por trás de toda mão vencedora: acerte nela e cada street seguinte fica mais fácil.

Esta página é a versão tudo-em-um: as 10 melhores mãos iniciais, o que faz uma mão ser *boa* pra começo de conversa, a tabela completa de mãos iniciais por posição (9-max e 6-max), tabelas GTO de pré-flop vs tabelas de iniciante, um resumo em PDF pra imprimir e um quiz rápido pra você se testar.
...
~80% | Mãos que um iniciante deveria foldar no pré-flop
...
## As 10 melhores mãos iniciais do poker, ranqueadas

Estas são as melhores mãos iniciais do poker — as que você deveria quase sempre aumentar no pré-flop, de qualquer assento da mesa:

| Rank | Mão | Por que é forte |
|-----:|------|-----------------|
| 1 | AA | Melhor mão de pré-flop — favorita em ~85% sobre uma mão aleatória |
| 2 | KK | Só perde pra AA no pré-flop — ainda assim aumente e re-aumente |
...
## Tabela de mãos iniciais do poker por posição (tabela completa 9-max)

==Sua posição na mesa muda quais mãos dão lucro.==

De posição inicial, muitos jogadores ainda agem depois de você — então você precisa de mãos mais fortes. Do button, você age por último em toda street pós-flop, o que significa que dá pra jogar um range bem mais amplo com lucro.

Aqui está a tabela de mãos iniciais do poker pra uma mesa 9-max padrão — o range de mãos por posição que serve de referência padrão:
...
Quer a matriz completa — a grade 13×13 com cada uma das 169 mãos? A aba **Tabelas pré-flop** do [solver GTO gratuito do site](/pt/solver) traz os ranges detalhados assento a assento, direto no navegador. Pra um detalhamento de cada nome de assento (UTG, HJ, CO, BTN, SB, BB), veja o [guia de posições do poker](/pt/blog/holdem-positions "thumb:/images/holdem-positions-hero.webp").
...
O button é o melhor assento do poker. ==g:Você age por último no flop, no turn e no river em toda mão.== Essa vantagem te deixa adicionar com lucro:
...
==g:Parece chato. É essa a ideia.== A seleção tight-aggressive no pré-flop é a base de todo estilo vencedor de poker, do cash game iniciante aos torneios de high-stakes.

---

## Teste você mesmo: quiz de mãos no pré-flop

Três spots de tabela. Decida antes de espiar as respostas:

**1. 9-max, você está no UTG com A♠ J♦ (offsuit).** Aumentar ou foldar?
→ ==r:Fold.== AJo não passa o corte de iniciante do UTG — é dominado com frequência demais pelas mãos que pagam uma abertura de UTG. AJ*s* abre; AJo entra no range GTO cheio, mas só depois que o resto estiver automático.
...
Acertou os três? A grade completa por assento está na aba **Tabelas pré-flop** do [solver gratuito do site](/pt/solver) — e quando quiser levar o treino pro pós-flop, o **Treinador GTO** do mesmo app corrige a sua jogada spot a spot.

---

:::readnext[Continue lendo]
/pt/blog/holdem-hand-rankings | Ranking de mãos do poker | /images/holdem-hand-rankings-hero.webp
...
A. Existem 169 tipos distintos de mãos iniciais (13 pares, 78 suited, 78 offsuit) dentro de 1.326 combinações exatas de duas cartas. A matemática por trás dessas contas está no [guia de probabilidade do poker](/pt/blog/holdem-probability).
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  ...
  <a href="/pt/blog/holdem-positions" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Posições do poker: do UTG ao button</div>
  ...
  <a href="/pt/blog/holdem-tiebreak-rules" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Regras de kicker e desempate no poker</div>
