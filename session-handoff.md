...
> 마지막 갱신 2026-09-17 (본체 HEAD = zh-hant 계산기 회차 커밋 `cf2ba10f` + 마감 문서 · `/zh-hant/calculator` 재조준 배포(「手牌勝率」 탭·quickRef 6표·FAQ 18(TDA 2024 Rule 5 고유) · MB-060) · 다음 = **es 계산기 회차** · 미커밋 0).

## ▶ 다음 할 일 (사장님이 고른다)

1. 🧮 **es 계산기 랜딩 회차 — 첫 마디 「핸드오프 읽고 es 계산기 회차 시작해」** (🔴 **Opus 창에서 연다** · 모델 분담 = 브리프 §3-D(zh-hant 회차가 Fable 없이 완주 — 헤드·집필·렌즈 3종 Opus · 구조 추출 sonnet 2 · 전사 = `check:calc-parity`). 절차 = `docs/harden-brief/calculator-landing-rewrite.md` **§3-A**를 es로 + **§3-B·§3-C·§3-E가 남긴 것**(🔴 `npm run check:calc-parity -- es`를 렌즈 전에 · 탭 이름≠페이지 이름 · 방위어 금지 · FAQ 질문 뒤 명사 덧붙이기 금지 · **CTA 이름이 볼륨 0일 수 있다 → 제목 앞머리는 실측 머리어** · «calculator» 명사 갈래(calculadora de poker/de probabilidades/de odds)를 따로 재라 · fold equity 역어는 코퍼스 정본 grep · TDA 인용은 2024판). 🔴 es는 **LATAM 중립(`/es/`)** — 볼륨 location은 Mexico부터(메모리 es-track-latam · `docs/local-voice/es-latam.md`) · `language_code`는 볼륨을 안 나눈다(rakko-playbook) · 라쿠 location Mexico 가능 · 질문검색은 ja만 유효. 🔴 **합법성 축을 열지 마라**(es 컨셉 = 플레이어 · 메모리) — TDA 같은 «규정» 사실은 가능. 선례 = `ja/zh/zh-hant-calculator.md` · EN 정본 = `en-calculator.md`. 수치 EN 그대로(§13) · 정의형 FAQ 금지 · 딜러 렌즈 12항 동반. 끝나면 §0 es ✅ + 다음 로케일(pt)을 이 자리에.
...
4. **10/14 이후 — 배치 효과 4주 비교**(캘린더 등재됨). 기준선 `docs/post-placement-analysis-2026-09-16.md` §2 · 같은 창(`npm run analytics` + `ga-fetch --pages --days 28`). 모바일 홈 36.7%(60세션)는 09-16 배포 전 값 — 그때 재측정. 같은 창에서 `/ja/calculator`·`/zh/calculator`·`/zh-hant/calculator` GSC 쿼리 첫 판독(셋 다 신설 직후 0 · zh-hant는 「德州撲克勝率計算器」 260·「icm計算機」 70 축을 본다).
...
- 우편함 미처리는 전부 «→검수장»(MB-033~060 + S-022·023). 본체 몫 0. 검수장 회신이 오면 MA 요청부터 판정. 🔴 MB-054는 **MB-053 ②(상금풀 $87,568,080)를 뒤집는 통지** · MB-055는 **계산기 랜딩 10로케일 검수 청구**(ja·en·zh·zh-hant 몫은 MB-057·058·059·060 해시로 대체).
