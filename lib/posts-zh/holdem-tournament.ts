...
> **快速回答**
> 扑克锦标赛是这样一种比赛：所有人交同样的入场费（买入 / buy-in）、拿同样多的起始筹码，盲注按计时器上涨，一直打到有一个人赢下场上全部的筹码，通常前 10–15% 的人分奖池。和现金局最根本的差别只有一个——筹码不是钱，而你的最大损失就是那笔买入。

**和现金局比：**在现金局里你的筹码就是真钱，你随时可以离场；在 freezeout 锦标赛里你交出去的就那一笔，却在争一个大得多的奖池里的一份（换成重买或重进赛制，成本会往上走）。
...
> **快速回答**
> 报名时交的买入分成两部分：大头进奖池，小头是场馆的手续费——$109 写作 $100+$9。换来的是一份起始筹码，常见 10,000 到 50,000 枚，Level 1 通常有 100–300 个大盲深。这份筹码没有现金价值，它只是你的一条锦标赛命；报名时顺手要一份结构表，全场最有用的文件就是它。
...
大型现场赛通常把买入的 8–10% 作为手续费（小型每日赛往往抽得更多）——这里是 $109 里的 $9，约 8.3%。这笔手续费怎么运作（以及为什么线上和现场不一样），在 [rake 抽水怎么算](/zh/blog/holdem-rake) 里讲得很细。

作为交换，你会拿到一份**起始筹码（starting stack）**。枚数本身不重要，重要的是它相当于多少个大盲——这个数字决定你有多少打牌的空间。

**一份 10,000 的筹码不等于 $10,000。**真正重要的，是钱圈到来时你手里的筹码有没有比别人多。

每场锦标赛都会在一张**结构表（structure sheet）**上公布它的结构：起始筹码、盲注级别、每级时长、前注安排和奖金分配表。报名时问工作人员要一份——它是全场最有用的一份文件。
...
==g:经验法则：低于 20 个大盲，你就进入了 push-or-fold（要么推要么弃）的边缘，到 15 个大盲基本就只剩推或弃了。低于 10 个大盲，你几乎任何一手能打的牌——尤其在后位或小盲位——都必须在盲注把你啃光之前推出去。==
...
> **快速回答**
> 每场锦标赛都走同一条路：早期（100–200BB）有空间打牌；中期（30–60BB）前注上场，大部分人在这里出局；钱圈 bubble 最煎熬，离进钱只差一次淘汰；决赛桌上每淘汰一人奖金就大幅跳涨，ICM 主宰决策。四段打法各不相同。
...
> **快速回答**
> 最常见的赛制：freezeout 一次买入不可重买，成本固定；重买/重进赛可以在早期时间窗内再买；赏金赛和 PKO 每淘汰一人拿一份赏金；deepstack 筹码更深、级别更慢；卫星赛的奖品是更大赛事的席位而不是现金。第一次报名，选 freezeout。

| 赛制 | 怎么运作 | 适合谁 |
|:---|:---|:---|
| **Freezeout** | 一次买入，不可重买。破产 = 出局。 | 新手——成本固定 |
| **重买 / 重进（Rebuy / Re-entry）** | 在早期时间窗内再买筹码——重买赛里常常不用先破产 | bankroll 更厚的激进型玩家 |
| **赏金 / KO（Bounty / KO）** | 每淘汰一人赢一份固定现金赏金 | 打行动的人——每次击杀多一份收入 |
| **PKO（渐进式击杀）** | 赏金随每次击杀增长——一部分付给你，一部分加到你头上 | 高波动、大上限的玩家 |
| **Deepstack** | 起始筹码明显高于同系列的标准赛事，级别也更慢 | 想多打翻后的人 |
| **卫星赛（Satellite）** | 奖品是更大赛事的入场资格，不是现金 | 瞄准大赛的预算型玩家 |
| **Turbo / Hyper-Turbo（快速赛）** | 盲注级别比标准赛事短得多，码量很快变浅，早早进入 push/fold | 时间不多、习惯推或弃决策的玩家 |
| **Mystery Bounty（神秘赏金）** | 赏金赛的一种：每次击杀（通常从某个阶段起）抽一个随机奖金——多数很小，少数是头奖 | 想搏一笔大额单次派彩的玩家 |
...
deepstack 锦标赛让你的起始筹码相对盲注明显高于同系列的标准赛事，而且一般配上更长的盲注级别。**它没有统一的门槛数值**——“deepstack”始终是个相对说法。你要做的是翻开 structure sheet，算出自己 Level 1 的筹码相当于多少个大盲，再和标准赛事的 100–200 BB 比一比。更多筹码加更慢的钟，意味着更多翻后打法、更多从失误里恢复的空间，以及更长的比赛日。

**那重买和 add-on 呢？**在重买赛里，你可以在设定好的早期时间窗内再买筹码——很多赛事只要你的码量不高于起始筹码就行，不必先破产；add-on 则是一次性的可选加购筹码，通常在那个时间窗关闭时提供。在那之后，赛事就按 freezeout 的方式往下打。
...
> **快速回答**
> 卫星赛是一种小锦标赛，奖品不是现金，而是一张通往更大、更贵赛事的入场券——$500 的卫星赛，每 20 人送出若干个 $10,000 的主赛席位，一级一级往上串还能把门槛压到几美元。只要送出的席位不止一个，席位到手之后多出来的筹码就一文不值，所以打法和普通锦标赛相反；只送一个席位的那种是例外，还是争第一。
...
**链式卫星赛**能把门槛压得更低。$5 超级卫星 → $55 资格赛 → $215 赛事 → $1,050 的线上主赛。很多大赛的玩家，都是通过一条卫星赛链，用直接买入的零头挤进来的。

==g:卫星赛策略和普通锦标赛打法不一样——在一场送出多个同等席位的卫星赛里，一旦你的筹码足够锁定一个席位，就别再冒险了，哪怕好牌也弃掉，免得在 bubble 上破产。只送一个席位的 winner-take-all 卫星赛是例外：那种要按 chip EV 去争第一。==
...
> **快速回答**
> 报名一场锦标赛有三条路：到赌场扑克室的报名台出示证件、付买入、领座位卡；大型赛事提前在线预注册，到场核验证件就能打印座位卡；或者打赢一场卫星赛，用席位券换座位。报名费就是买入本身，报名台通常开赛前 1–3 小时开放，大赛建议提前一天锁定席位。
...
### 方式 B：线上预注册
大多数大型现场赛事让你提前在线报名：
- 在赛事平台上开个账号（比如 WSOP 用 WSOP LIVE app 外加一个 Caesars Rewards 账号，EPT/APPT 赛事用 PokerStars 软件大厅里的“Events”、“Live”标签页）
- 在线支付买入
...
**别掐着点来。**报名台通常在开赛前 1–3 小时开放，但大型赛事的队伍会比你想的长。
...
> **快速回答**
> 锦标赛策略按阶段走：早期 100BB+ 打紧、看位置，用能爆掉大对子的牌便宜看翻牌；中期 30–60BB 前注上场，放宽后位开牌、偷盲、防守大盲；20BB 以下 push-or-fold 接管；钱圈和决赛桌上，活下来比筹码更值钱。每一段都有专题指南接着讲。

一篇文章教不了完整的锦标赛策略——那是专题指南的活——但每套制胜方案都挂在同一副骨架上，下面按阶段拆开：
...
<div style="color:hsl(var(--foreground))">迟到报名关闭。公布最终参赛人数。确认奖池。</div>
</div>
<div style="display:grid;grid-template-columns:80px 1fr;gap:12px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
...
> **快速回答**
> 典型结构是前 10–15% 的选手进钱（钱圈率），下表四档都落在 12–13%；最低奖 1.5–2.2 倍买入，冠军拿奖池的 8–30%——场子越大，冠军占比越小。1,435 人的 WPT 冠军赛里，180 人进钱，冠军拿了奖池的约 14%。具体的分配表，报名时问结构表。
...
奖金分配表在开赛前就能查到，但最终发多少个名次、每个名次具体发多少，往往要等报名、重购和增购全部截止之后才确定。报名时问一份**结构表（structure sheet）**——它列出盲注级别、前注、起始筹码和奖金分配表。
...
> **快速回答**
> Day 1 你最常听到的三个词：ITM（进钱）、bubble（钱圈，离进钱只差一次淘汰）、structure sheet（结构表，列出盲注级别、前注和奖金）。下表 13 个术语覆盖桌上大部分黑话；完整的 A 到 Z 在 [poker 术语表](/zh/blog/holdem-glossary)。
...
| **PKO** | 渐进式击杀——赏金会增长的赏金赛 |
| **Mystery Bounty** | 神秘赏金——每次击杀的奖金随机抽取的赏金赛制 |
| **Turbo** | 快速赛——盲注级别短得多的结构；hyper-turbo 更短 |
| **Add-on** | 增购——重买期结束时向所有人提供的一次性额外买码，不看码量 |
...
> **快速回答**
> 出门前：带照片证件、买入加两成现金、需要的话带会员卡和预注册邮件，穿暖和。到场后：提前 30–45 分钟、打第一手前数筹码、要一份结构表、一手牌进行中别看手机。做到这几条，你第一次进场就不会显得一脸懵。
...
/zh/blog/holdem-bubble | 德州扑克钱圈怎么打 | /images/holdem-bubble-hero.webp
...
A. 在重买赛里，你可以在设定好的重买期内再买筹码——很多赛事只要码量不高于起始筹码就行，不必先破产——通常限于最初的几个盲注级别。add-on 则是一次性的可选加购筹码，通常在重买期结束时向所有人提供，不看筹码量。那个窗口一关，赛事就按 freezeout 往下打。具体规则因场馆而异，所以查结构表。

**Q. 扑克锦标赛靠什么赚钱？**

A. 场馆在每笔买入之外收一笔手续费——就是“$100+$9”里那个“+$9”。这笔手续费（大型现场赛通常约 8–10%，小型每日赛更高）是场馆的收入；而“$100”那部分全额进入玩家们争夺的奖池。所以一个锦标赛室靠的是报名量和这些手续费盈利，而不是奖金本身——奖金只是在玩家之间循环流转。

**Q. 在家办扑克锦标赛合法吗？**

A. 这取决于你所在的司法辖区。没有哪条经验法则能替你把这件事定下来——界线写在当地法条里，用一句话给你一个判定标准的指南，是在替你猜。各国、各州法律差异很大，办之前先查你当地的规定。
...
A. 不能。和现金局不同，锦标赛筹码没有现金价值，也不能中途兑现。如果你走人，你的筹码会留在场上继续付盲注和前注，直到它们被榨光。常规奖金只有在你打进拿钱（ITM）的名次时才发；不过在赏金赛和 PKO 里，你还能另外收到赏金。

**Q. 锦标赛的盲注多久升一次？怎么知道下一级是多少？**

A. 现场赛通常每 20–40 分钟升一级，线上快节奏赛可能几分钟就升一次。每一级的时长和它对应的盲注、前注，都白纸黑字写在**结构表（structure sheet）**上——报名时问工作人员要一份，你就能提前算出自己什么时候会被逼进短码。盲注按计时器不断上涨，是锦标赛逼出行动、并最终淘汰玩家的核心机制；你一枚筹码没输，码量也会随着盲注上涨而缩水。

**Q. 锦标赛更靠运气还是实力？**

A. 两者都有——但决定谁笑到最后的，长期来看是实力。单场锦标赛的方差极大：你可以打得毫无破绽，AA 还是被人翻盘出局，这正是为什么连顶尖职业选手都会经历很长一段没有大奖入账的日子。可放到几百场赛事的尺度上看，水平更高的人打进决赛桌的次数，远远超过纯运气能解释的比例。扑克是一门裹在短期运气里的技术游戏——而锦标赛只是把那份运气比现金局压缩得更浓罢了。
