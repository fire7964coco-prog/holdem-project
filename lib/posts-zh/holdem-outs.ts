...
> **快速回答**
> 常见听牌的补牌数：同花听牌 9 张，开口顺 8 张，同花加开口顺 15 张，同花加卡顺 12 张，两张超对 6 张，一对追两对或三条 5 张，卡顺 4 张，一张超对 3 张，口袋对子追暗三条 2 张。除了顶上两个组合听牌要减掉重叠，其余全是直来直去的减法。
...
> **快速回答**
> 组合听牌不能把两个补牌数直接相加，因为有些牌身兼两职。同花加开口顺：9 张同花补牌里已经包含两张同时凑成顺子的牌，所以是 9 + 6 = 15，不是 17；同花加卡顺是 9 + 3 = 12，不是 13。只要两个听牌共用一张牌，就减掉重叠的那部分。
...
**同花 + 卡顺**同理：四张卡顺牌里有一张是你的花色，已经在同花里数过了。这是玩家把补牌数多算的最常见原因，也是为什么表里那些组合行看起来比加总"少了一张"。
...
> **快速回答**
> 脏 outs（被污染的补牌）是凑成了你的牌却仍然可能输的牌：不是坚果的同花听牌、成对牌面上的同花补牌、撞上强牌时的超对。按全额去数会高估胜率，所以先数出原始补牌，再在换算之前把脏的那些打折——拿不准时，往下削。
...
🃁 | 超对撞上强牌 | 两张超对（A-K 在 Q-8-3 上）纸面上算 6 张补牌，但如果一个大加注摆明了是暗三条或两对，配对你的 A 往往不够好——数 3，顶多 4，不是 6
:::
