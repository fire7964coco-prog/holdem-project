...
| 🟢 **EN-먼저 18건 회차 종결(09-11 (7))** — EN 12편 정정 · 7로케일(de·es·id·ja·pt·zh·zh-hant) 전파 · 렌즈 4종+2차 교열 반영 · 배포. **잔여** = ① **ar 5편·fr 5편 미전파**(사장님 지시 · `check:drift` 🔴 5 = ar — 각 경화 트랙에서 EN diff로 받는다) ② **꼬리 15로케일 §13급 3건**(showdown 토너/캐시 한정 · all-in §47A 플레이어별 · cards-speak §14) — 판정 ⓒ 전파 회차 ③ 기각 4건(A#4A·A#4B·A#8·**J#4 = 참이라 원문 복원**)은 `en-first-queue` §2-A~2-C에 «되살리지 마라»로. 상세 = `en-first-queue.md` 잔여 표 | 소~중 |
