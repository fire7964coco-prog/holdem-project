...
export const POST: Post = {
  slug: "holdem-bubble",
  title: "德州扑克钱圈（bubble）怎么打——大码、中码、short stack 分别打法",
  seoTitle: "德扑钱圈 bubble 怎么打（按码量拆开讲）",
  desc: "到了钱圈，活下来比拿筹码更重要，正确打法整个反过来：跟注收紧、推牌保持宽。大码、中码、short stack 各自怎么打，为什么最被困死的是中码而不是短码，外加泡沫系数怎么算、卫星赛为什么连口袋 A 都要弃。",
  tldr: "钱圈（bubble）就是进钱前的那一段——再淘汰一个人，其余所有人就都进钱了。因为出局什么都拿不到，活下来的价值就超过了你能赢到的筹码——所以跟注范围收得很紧，而 shove（推）依然要宽。大码进攻，中码最被困死（不是 short stack），而在送出多个席位的卫星赛钱圈上，一旦你的席位锁定，什么都弃，连口袋 A 都弃。",
  category: "tournament",
  date: "2026-07-09",
  updated: "2026-09-13",
  ...
  imageAlt: "锦标赛牌桌上，钱圈上一小摞筹码和一座高耸的大码隔桌相对，背景是奖金阶梯——活下来比筹码更值钱的那一刻",
  tags: ["德州扑克 钱圈", "钱圈 意思", "德扑 bubble 策略", "bubble factor", "short stack bubble", "钱圈 bubble", "卫星赛 bubble", "hand for hand 扑克"],
  content: `
...
==在钱圈（bubble）上，多淘汰一个人，其余所有人就都进钱——所以在几手关键牌里，活下来比你能赢到的筹码更值钱。== 这一个事实就把正常的扑克整个翻了个底朝天，而几乎所有人都以同样的两种方式打错：大码进攻得不够，中码跟注得太多。这份指南就是按码量拆开讲的手册——大码、中码还是 short stack 各该怎么打，横跨你会遇到的三种不同的钱圈。
...
## 德州扑克里的钱圈（bubble）是什么？（以及“on the bubble”）

> **快速回答**
> 钱圈（bubble）就是进钱前的那一段：再淘汰一个人，所有还坐在位子上的人就都进入发钱的名次。发前 27 名的比赛，剩 28 个人时就到了钱圈——现在出局什么都拿不到，再挺过一次淘汰就锁定进钱。“on the bubble”说的就是卡在这一步。
...
> **快速回答**
> 因为锦标赛的筹码不是钱——你只能赢一个第一名，所以护住一份已锁定的进钱，比伸手去够更多筹码更值钱。这就是 ICM：临近奖金跳档时，出局的风险压过了赢下一次抛硬币的回报。落到牌桌上只有一句：跟注收紧，推牌保持宽。
...
## 你会遇到的 3 种钱圈：进钱 vs 决赛桌 vs 卫星赛

> **快速回答**
> 不是所有钱圈都一样：进钱 bubble 奖的是“进钱”这一跳，活下来溢价高但 min-cash 小，要继续累积；决赛桌 bubble 的 ICM 压力通常是全场最极端的；卫星赛 bubble 则反过来，席位一安全，多出来的筹码一文不值。搞混三者，是锦标赛里最烧钱的错误之一。

接下来按码量给的建议，会随你处在哪一种钱圈而变，所以先把三种分开：

- ==**进钱 bubble（money bubble）**== ——从什么都没有到 min-cash 的那一跳。活下来的溢价很高，但 min-cash 很小，所以你依然想为顶部大奖去*累积*。要施压，别光躲。
...
- ==**卫星赛钱圈（satellite bubble）**== ——那个另类。在送出多个席位的卫星赛里，每一个晋级席位发的奖 ==完全一样==。一旦你的码量大到足够安全，多出来的筹码就*一文不值*——所以正确打法几乎变成了正常 bubble 的反面（下面“弃 A”那条规则会细讲）。
...
> **快速回答**
> 毫不留情地进攻——大码在全桌的风险溢价最低，其他所有人都得敬着你的筹码。开池宽、3bet 轻，盯着最怕出局的中码打而不是最短的码，遇到抵抗就弃。打对了，大码在这一段几乎不用摊牌就能把筹码堆起来。

大码是钱圈的头号受益者。你能干掉任何人；没人能干掉你。所以把压力压上去：
...
做到这三条，钱圈这一段就是你整场比赛里成本最低的一段筹码增长期。
...
> **快速回答**
> 中码是全桌最被困死的位子——这正是几乎每篇文章都搞错的事实。按泡沫系数算，中码大到有真金白银的奖金权益可以输，又没短到有理由去赌。打法：跟注范围收到全桌最紧，继续偷下方短码的盲，但守住底线，别把自己弃成短码。

大家想当然地以为 short stack 感受到的压力最大。真正的数学说的是另一回事——下面“泡沫系数”一节会给出数字——受束缚最深的是 ==中码==。
...
> **快速回答**
> 要么 all in 要么弃牌——绝不 limp、也绝不跟到底——并且利用一个事实：short stack 的泡沫系数其实比中码更低。你本来就很可能出局，翻倍帮助极大，所以可以放开去赌，但要当那个推牌的人，不是跟注的人。有更短的码在场就等一等；自己就是最短的，就别等。

机制在完整的 [short stack 推/弃手册](/zh/blog/holdem-short-stack "thumb:/images/holdem-short-stack-hero.webp") 里讲透了，落到钱圈上是三条：
...
## 大筹码施压 vs 短筹码求生：正面撞上时谁占理

> **快速回答**
> 钱圈上最常见的对决，是大筹码往你身上推。大筹码是在拿一点点风险博别人的整场比赛，所以推得很宽；短筹码的泡沫系数低，该当抢先推的人而不是跟注的人；中等筹码最难受——门槛看对手是谁，面对 chip leader 按表要 75% 上下，达不到就弃，转身去偷更短的码。

这不是新规则，而是把上面几节的道理放到同一手牌里看：

- **站在大筹码一侧：** 你是在拿*一点点*风险去博*别人*的整场锦标赛。你推的时候可以很宽，因为对手就算手里有牌，光是“跟你就要赌上命”这件事，就足以逼他弃掉一大半本该跟的牌。所以你施压的对象要挑那些 ==输不起== 的人——被困死的中等筹码，而不是无所谓翻不翻倍的短筹码。
- **站在短筹码一侧：** 你的泡沫系数其实比中等筹码更低，所以当你决定进锅时，你是那个 ==抢先推== 的人，用 fold equity 逼别人弃——绝不是那个跟大筹码全下的人。有比你更短的码在场，就先弃、让他们去死。
- **站在中等筹码一侧（最难受）：** 大筹码正是冲着你来的。你没有短筹码那份“反正快死了”的自由，输掉筹码在真金白银上的代价又最大。所以除非牌力强过 ==你这个对手对应的泡沫系数门槛==，否则面对大筹码的推牌就弃——面对 chip leader，泡沫系数往往接近 3.0，按下面那张表的公式要 ==75%==，而不是 1.5 对应的 60%——底池里的死钱会把这个数往下拉，但方向不变：这是全桌最难跟的位置。弃掉之后，转身去偷你下方那些更短的码，把损失补回来。

一句话记牢：==大筹码用的是别人的命去赌，中等筹码用的是自己的奖金去赌==——这就是为什么同一手牌，大筹码敢推、中等筹码却得弃。

---

## 泡沫系数（bubble factor）和风险溢价：告诉你何时该弃的那个数

> **快速回答**
> 泡沫系数（bubble factor）衡量的是：输掉这个底池的损失，比赢下同一个底池的好处大出多少——它能直接换算成你跟注所需的额外胜率。1.0 是无压力（50% 就能跟），1.5 要 60%，1.7 要 63%。==r:表里的 1.5–1.7 是一个正经钱圈的下限，不是峰值==——对手是 chip leader 时会飙到 3.0 上下；反过来，底池里有死钱会把要求往下压。

bubble factor 为 1.0，意味着筹码和钱同步移动（锦标赛早期）。bubble factor 为 1.5，意味着 ==出局的痛，是赢下的爽的 1.5 倍==，所以你需要大得多的优势才能把筹码投进去。

有用的部分来了：你做一次跟注打平所需的胜率是 ==c · BF ÷（P + c · BF）==——**c** 是这次跟注要付的钱，**P** 是你赢下时能拿走的底池。当你押上的和你能赢的正好一样多时，它才收缩成常被引用的那个形式 ==BF ÷（1 + BF）==，下面这张表用的就是它。

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| 泡沫系数（bubble factor） | 输的痛…… | 所需胜率（无死钱时） |
|:--|:--:|:--:|
| 1.0（无压力） | 和赢的爽一样 | 50% |
| 1.3 | 1.3× | ==57%== |
| 1.5（进钱 bubble） | 1.5× | ==60%== |
...
> **快速回答**
> 进钱临近时，锦标赛会切换到 hand-for-hand（逐手制）：每一桌同时只打一手，打完互相等，专门用来阻止靠拖延混进钱圈。计时器停住，每一手固定折算 2 分钟，所以盲注照样往上走。大码在这一段最占便宜——别人都慢下来了，它还在一把把收盲注。

没有 hand-for-hand，慢桌的玩家可以一手接一手地弃，而快桌却在飞速烧穿钱圈。它把场子拉平：

- **它怎么运作：** 赛事总监暂停计时，之后每打完一手就从当前级别固定扣掉 ==2 分钟==，不管这手牌实际打了多久（WSOP 锦标赛规则 126.a、126.c；TDA RP-8-C、RP-8-D）——所以盲注在 bubble 期间照样往上走，只是按手数走、不按真实时间走。所有桌各发一手牌，任何一桌都要等到每一桌都打完了才开始下一手。如果**同一桌**有两个人在同一次 hand-for-hand 里出局，那手开始时筹码更少的那个拿更低（bubble）的名次；如果他们在**不同桌**出局，则在那个名次上并列（WSOP 锦标赛规则 126.b），实务上平分这两个名次的奖金。有一处两本规则写得一模一样：宣布进入 hand-for-hand 时**还没打完的那一手**，WSOP 126.c 和 TDA RP-8-A 都规定在那一手出局的所有人**分摊**派彩的名次。在指望往上爬一个名次之前，先问清楚这家牌室的规矩。
- **拖延（Stalling）：** 每一个决定都把时间银行用满，好少看几手牌（然后弃进钱圈）。但在 hand-for-hand 里这个指望是错的：长考在计时上完全免费——这手牌照样只扣 2 分钟，不管你是秒弃还是把时间银行烧光。大码没理由拖延——他们想要更多手牌来进攻。short stack 和中码有时会拖延来求生，==但过度拖延可能招来 clock call（叫时）或罚则==——长考要有分寸，别刻意把时间银行烧光。
...
> **快速回答**
> 在送出多个同等席位的卫星赛里，每一个席位的奖都一样，所以一旦你的码量在数学上安全了，就什么都弃——包括口袋 A。赢下一手只是拿到一个早已锁定的席位，输了却会出局：没有回报，只有风险。唯一的例外：你覆盖着对面那个 short stack，他一出局就打爆 bubble，而且就算这个底池输了你的席位也还稳。

这是扑克里最反直觉的一个局面，而它是对的。（只送一个席位的 winner-take-all 卫星赛是另一回事：那种要按 chip EV 去争第一。）三条规矩：

- **一旦你的席位在数学上安全了**（你进 bubble 进得够深，追不上你了），就每手都弃——是的，连 AA 和 KK 都弃——让更短的码去互相厮杀。盲注每升一级，这笔账都要重算一遍：前注一进场，“安全区”就会缩小。
- **别指望现场拖延。** 线上把时钟用满不花钱；现场为了爬名次刻意烧时间银行，按 WSOP 锦标赛规则 80 是明确可罚的——该条直接点名“为了在派彩上往上爬而故意烧掉时间银行”，并把它交给缩短时钟或规则 40、113、114 的罚则处理——按正常速度弃牌，把出局的风险留给短码。
- **唯一的例外：** 只有当你的码量覆盖（cover）对面那个 short stack、而他一出局就打爆 bubble、坐实你的席位时才跟——而且前提是就算这个底池输了，你的席位也依然稳保。

如果你从这一节只带走一件事：卫星赛不是一场正常的锦标赛。安全线以上的筹码一文不值，所以就照这个来打。

---

## 最大的钱圈错误：为 min-cash 而打

> **快速回答**
> 一路弃牌弃到 min-cash 感觉很安全，但那是拿锦标赛真正的钱去换它最小的那个奖。奖金头重脚轻，min-cash 是地板不是目标——钱在阶梯顶端，只有钱圈戳破时手里还有筹码的人才够得着。赢家在这里累积，输家在这里躲。

那一秒之后谁手里还有筹码，决定了他拿的是零头还是奖金。
...
**Q. “吃泡 / 破钱圈”（pay / burst the bubble）是什么意思？**

A. “泡沫（bubble）”就是进钱前的最后一个名次，所以在那里出局的人——也就是 bubble boy——什么都拿不到，而其余还在场的人全部进钱。“泡沫破裂”（burst the bubble）指的则是那最后一次淘汰：它一发生，剩下的每个玩家就都进了钱，紧张的求生压力也随之松开。人们说的“吃泡”（pay the bubble）指的是另一回事：有些比赛——或者剩下的玩家私下商量——会给泡沫上那个人一笔小小的安慰金。那是例外，不是规矩；传统上钱圈边上就是发零奖。

**Q. 钱圈边上该弃牌吗？**

A. 你该弃的*跟注*要比平时多得多，但不是什么都弃——而且你该继续推牌和偷盲。在 pay jump（奖金跳档）附近，活下来比筹码更值钱，所以跟到底然后出局才是那个烧钱的错误。把你的跟注范围收得很紧，同时保持你抢先进攻的范围宽。

**Q. short stack 感受到的钱圈压力最大吗？**
...
**Q. 什么是泡沫系数（bubble factor）？**

A. 泡沫系数衡量的是——按真金白银（ICM）来算，输掉一个底池给你造成的损失，比赢下同一个底池给你带来的好处，要大出多少。泡沫系数为 1.0 意味着筹码等于钱；为 1.5 意味着出局的痛是赢下的爽的 1.5 倍。它能换算成你做一次跟注所需要的胜率：c · BF ÷（P + c · BF），其中 c 是跟注要付的钱、P 是你能赢下的底池。押上的和能赢的正好一样多时，它就是 BF ÷（1 + BF）；但真实底池里有死钱，所以一次 10bb 的推、你跟 9bb 进一个 12bb 的底池，需要的大约是 53%。两种算法都高过筹码 EV 抛硬币给你的 50%——这正是为什么一次筹码 EV 上的抛硬币对局，到了泡沫期就变成了该弃。
...
A. 因为在送出多个席位的卫星赛里，每一个席位发的奖都一样，所以一旦你的码量稳稳进了 bubble 里，赢下一手牌不会给你带来任何额外的东西（你已经有你的席位了），而输了却会把你淘汰。全是风险、没有回报，连口袋 A 都弃在数学上就是对的。
...
> **快速回答**
> 钱圈上的一切归结为三句话：活下来只在那几手牌里胜过筹码；被困死的是中码不是 short stack；先搞清自己站在哪一种钱圈上。三句都做到，min-cash 就只是路过，不是终点。
...
3. **搞清你的钱圈类型。** 进钱、决赛桌和卫星赛这三种钱圈奖励的是不同的打法——而在卫星赛上，一个安全的码量什么都弃，连 A 都弃。
