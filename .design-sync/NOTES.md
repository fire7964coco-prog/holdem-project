...
- 🔴 **사이트 본체에 미해결 결함이 있다**: `background:var(--card)` 처럼 HSL 3요소 토큰을 색으로 직접 쓴
  자리가 **5,204곳(445파일)** 이라 라이브에서 안 먹는다(`hsl()` 로 감싸야 한다). 정상형은 53곳뿐.
  → 프리뷰를 authoring 할 때 **원본을 그대로 베끼면 깨진 CSS 를 디자인 시스템에 박아 넣게 된다.**
  conventions.md 에 「반드시 `hsl(var(--x))`」로 못박아 뒀다. 본체를 고치면 이 항목을 지워라.
