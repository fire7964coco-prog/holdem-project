...
export const POST: Post = {
  ...
  tldr: "キャッシュゲームではチップが現金価値を持ち、ブラインドは固定で、いつでも席を立てます。トーナメントではチップは生き残るための持ち点で、ブラインドが上昇し、賞金は最終順位で決まります。勝ち方も別物で、キャッシュはbb/100や時給で測る積み上げ型、トーナメントはROIで測るスパイク型。初心者は基礎を磨きやすいキャッシュゲームから始めるのが無難です。",
  category: "tournament",
  date: "2026-06-11",
  updated: "2026-09-13",
...
**ブラインドは固定です。** $1/$2のゲームは、5時間後も$1/$2のままです。2つの強制ベットを払う位置が、ハンドごとに時計回りに1席ずつ移るだけ。スモールブラインド、ビッグブラインド、あるいは「オプション」がまだ曖昧なら——ミスブラインドやストラドルのルールも含めて——[ポーカーのブラインドとは何か](/ja/blog/holdem-blind-meaning)で一気に押さえられます。
...
まったくの初心者なら、まず[テキサスホールデムのハンドの進み方](/ja/blog/holdem-game-order)と[ポーカー役の強さ](/ja/blog/holdem-hand-rankings)を押さえてください。基本ルールが身につけば、形式を選ぶのはずっと楽になります——そしてトーナメントに惹かれるなら、バイイン・ブラインドレベル・Day1の流れは[ポーカートーナメントの仕組み](/ja/blog/holdem-tournament "thumb:/images/holdem-tournament-hero.webp")を参照してください。
...
A. キャッシュゲームはテーブルの==上限までバイイン==するのが基本——海外のカジノの$1/$2なら上限はたいてい$200〜$300、つまり100〜150BBです(日本のアミューズメント店はチップを換金せず時間制の料金なので、この$表記は当てはまりません)。ディープスタックほどポストフロップの技術が活き、勝っているときに1スタック丸ごと取れます。ただし条件が2つ——バンクロールがそれを支えられること(上の20〜40バイインの目安はフルバイイン基準で、ショートでの参加を前提にしていません)、そしてポストフロップで自分のほうが上手いこと。どちらかが怪しいなら、浅めに買うのは初心者のミスではなく正当な選択です。ショートスタックは判断を単純にし、そのぶん取れる上限が下がるだけです。トーナメントは深さを主催者が決め、たいてい開始100〜300BBほど。ただしブラインドが上がるので、放っておいても25BB→10BB→プッシュ/フォールド圏へと縮んでいきます。要するに、==バンクロールとポストフロップの腕が許すならキャッシュは深くバイインし、トーナメントは自分のBB数が減っていくのを見ながら調整する==、ということです。
...
2. ==g:**キャッシュゲームでは基礎を磨きやすく、トーナメントではプレッシャーの中で判断する力が試される。**== どちらが華やかに聞こえるかではなく、目的で選びましょう。
