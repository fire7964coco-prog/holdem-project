...
> 🔴 **랜딩 SPEC에 «지원 언어 열거» 행을 두지 않는다**(2026-09-09 사장님 결정 · 솔버 S-013 · 2026-09-11 본체 이행). 언어별 랜딩은 그 언어 사용자만 보므로 «다른 언어판 12개» 열거는 정보값 0이고 언어를 추가할 때마다 8곳 갱신 비용만 남는다(M-100→M-107이 그 사례). 8곳(de·es·fr·id·ja·pt·zh·zh-hant)의 행을 삭제했고 ko·en에는 원래 없다 — **신설하지 마라.** «언어 대응 여부»는 FAQ 한 문항(ja 「画面は日本語に対応していますか」식)으로만 답한다.
> (경위: 2026-09-06까지는 «전환 가능»이 아니라 «다른 언어판이 있다»로만 쓰는 규칙이었다 — 폐기.)

> **2026-09-17 형제 랜딩 — 계산기 `/{locale}/calculator` 10개 신설(`64c0fd9a`)**: 솔버와 달리 **콘텐츠 복사본이 아니라 «공용 컴포넌트 + 로케일 사전»**이다(`components/calculator/calculator-tool.tsx` + `app/<loc>/calculator/dict.ts`). hreflang 12세트는 `lib/calculator-alternates.ts` **한 곳**(솔버처럼 12파일에 복사하지 않았다 — 로케일을 더할 때 한 줄). 등록 4곳(hub-routes · side-rail · sitemap · alternates)은 §3과 같다. 사전 작성 규율·sed 함정 = 메모리 `calculator-landings-shared-component` · WORKLOG 2026-09-17 (3). 🔴 §4-1 「가진 것부터 세어라」의 «없는 도구 라우트» 목록에서 **calculator는 빠졌다**(hand-chart·win-rate-quiz는 여전히 ko·en뿐).

> **2026-09-14 FAQ 동기화 보완**: 언어 선택기·다른 언어 목록은 FAQ에도 넣지 않는다.
> 계정 동기화 대상은 **교육 예제·오늘의 문제의 학습 기록**이다. **직접 계산한 스팟과 그 연습 기록은
> 로그인해도 이 기기에만 저장**된다. 이 범위는 SPEC 계정 행뿐 아니라 무료·로그인·저장 FAQ와
> 트레이너 본문까지 함께 맞춘다. FAQ는 화면과 FAQPage가 같은 배열을 쓰므로 문항 단독으로도
> 범위와 예외가 명확해야 한다.
