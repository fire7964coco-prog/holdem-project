...
新马、港澳牌桌上多直接喊英文原词（fold/call/raise/all-in），下面的中文译名便于理解，实战报牌以英文为主。
...
| 这对别说反了 | 用错会怎样 |
|:---|:---|
| **过牌（Check） vs 跟注（Call）** | 前面没人下注你才能过牌；一旦有人下了注，你想留在牌里就只能跟注或加注。把「过」当「跟」喊出来，荷官会按**没有下注可过**处理，容易搞出误会。 |
| **下注（Bet） vs 加注（Raise）** | 这一轮**第一个**投筹码叫下注；前面已经有人下了、你再往上加才叫加注。喊错了，荷官按你实际投入的动作判定，你以为在加注、其实只是补成跟注。 |
| **set（暗三条） vs trips（明三条）** | 两者牌力一样，但 **set 用一对口袋对子撞中公共牌**、隐蔽性极强，**trips 是你一张底牌配上公共牌上明摆着的一对**、对手一眼看得见。把 trips 当 set 去价值下注，常被对方拿更好的踢脚反杀。 |
| **关煞位（Cutoff / CO） vs 按钮位（Button / BTN）** | 按钮位在**你右手第一个**、翻后**最后**行动，是全桌最好的座位；CO 是按钮位**右边**、次好的座位。把 CO 当按钮位去偷盲、结果按钮位还没弃牌，你就成了无位置的那个。 |
...
| **拉线下注（String bet）** | 没声明就回手再取筹码——只有第一个动作算数（通常判为跟注）。先喊「加注」，分次推码就是合法的。 |
...
| **劫机位（Hijack / HJ）** | 按钮位右边第二个座位；中位（MP），就在关煞位之前。 |
| **Lojack（LJ）** | 劫机位右边；同样是中位（MP）（叫法会随桌上人数变动）。 |
