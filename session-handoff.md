...
### ▶ 0-◆. ★다음 세션 지정 작업 — 「사이트 기본기 6종」 한 호흡에 (사장님 지시 2026-08-04)

> 사장님 요청: *"다른 사이트들이랑 비교해봤을 때 바꿔야 할 것들"* 점검 → 6건 확정.
> *"1~6까지 다음 세션에서 한 호흡으로 하자"* — **쪼개지 말고 한 번에 간다.**
> 아래 수치는 2026-08-04에 로컬 빌드 산출물 + Playwright(360/1440)로 **직접 잰 값**이다.
> 다시 재느라 시간 쓰지 말 것. 코드는 아직 **하나도 안 건드렸다**.

**⚠ 시한 충돌 확인 필요**: `apt-incheon-2026-guide`는 **8/7 개막**이다. 6종을 먼저 하면
APT는 그 뒤가 된다. 세션 시작 때 사장님께 순서만 확인하고 진행할 것.

| # | 할 일 | 실측 근거 | 손댈 곳 |
|---:|---|---|---|
| **1** | **푸터 + 정책 페이지** | `<footer>` 전 페이지 **0개**(`SiteFooter()`가 `return null`). `/privacy` **404**, 이용약관·문의 **없음**. `/about`(228줄)은 존재하나 **어디서도 링크 안 됨** | `components/site-chrome.tsx` `SiteFooter`, 신규 `app/privacy`·`app/terms`·`app/contact` |
| **2** | **커스텀 404** | 지금은 Next 기본 화면 — 영어 한 줄 *"404: This page could not be found."* + 상단바뿐, 나머지 백지. **이탈 100%** | 신규 `app/not-found.tsx` |
| **3** | **데스크톱 본문 폭 제한** | 1440px에서 본문 **928px / 18px / 줄간격 1.63 → 한 줄 52자**. 한글 권장 38~42자(680~740px). 모바일은 302px·16px·**19자**로 오히려 좁음(좌우 여백 회수 여지) | `app/blog/[slug]/blog-post-client.tsx` 본문 래퍼. ★문단만 묶고 표·이미지는 넓게 두기 |
| **4** | **태그 링크화** | 글마다 태그 6개가 전부 `<span>` — 눌러도 반응 없음(`blog-post-client.tsx:333`) | 같은 파일 + `/blog?tag=` 필터(`blog-index-client.tsx`) |
| **5** | **사이트 검색** | 검색 입력이 **어디에도 없음**(`type="search"` 0건). 한국어 56 + 다국어 457편인데 카테고리 필터뿐 | 신규. 구현 비용이 1~4보다 크니 **마지막에** |
| **6** | **시각적 브레드크럼(블로그 글)** | `BreadcrumbList` JSON-LD는 **있는데** 화면엔 없음. `/rules/*`·`tournament-guide-post`엔 있어 사이트 내부에서도 불일치 | `app/blog/[slug]/blog-post-client.tsx` |
