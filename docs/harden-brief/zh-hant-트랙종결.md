...
> 🔴🔴 **마감 시 정정 (2026-09-09) — 이 브리프에 오류가 있었다. 다음 레인이 그대로 베끼지 마라.**
> **§0 실측 부록과 §2가 「블록총 = 대상 H2 **+ 1**(상단 블록)」이라 적고 자기점검 명령까지 그렇게 박았는데, 틀렸다.**
> 정본은 `docs/local-voice/zh-tw.md` §1-B **현행 절**의 「🔴 **상단 블록은 넣지 않는다** … `tldr` + 각 H2 블록,
> 이 둘만 둔다」이고, 바로 위 줄이 「회차 1·2의 12편은 트랙 종결 회차(7)에서 소급한다」다.
> **즉 상단 블록 12개 삭제가 이 회차의 마감 조건이었다.** C 구간 네이티브 렌즈가 잡았고, 실측이 지지했다 —
> 회차 3~6의 30편은 상단 블록 **0/30**, 회차 1·2만 12/12였다. → 삭제 완료, 12편 블록 = **110/110**.
> 🪶 **A 구간이 왜 놓쳤나**: 그 문장이 「### 1-B. 🔴 **폐기됨**」 표제 **아래**에 있어 절 전체를 폐기로 읽었다.
> 폐기된 것은 그 아래 `<details>` 안의 회차 1 판정뿐이다. **«폐기» 표제를 만나면 어디까지가 폐기인지 문장 단위로 확인해라.**
> 🪶 나머지 실측(직답 296 · 대상 H2 110 · 전환 47 / 신설 63 · 표기 12종)은 **전부 정확했다**(SEO 렌즈 교차확인).
