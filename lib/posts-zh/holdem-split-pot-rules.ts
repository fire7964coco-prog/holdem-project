import type { Post } from "../posts";

export const POST: Post = {
  slug: "holdem-split-pot-rules",
  title: "什么时候平分底池？德州扑克 Chop 规则",
  seoTitle: "赢了牌却只拿一半？德州扑克平局怎么分",
  desc: "德州扑克能打平吗？能——本文讲清底池到底何时平分：完全相同的五张牌、公共牌通杀、单数筹码规则，还有边池平分怎么算。",
  tldr: "能——扑克牌是可以打平的。当两名或更多玩家摊牌时亮出完全相同的最佳五张牌，底池就会平分（chop）。花色永远不能决定胜负，任何剩下的单数筹码归庄家按钮位左边第一个平局玩家。",
  category: "hand-rankings",
  date: "2026-06-13",
  updated: "2026-09-26",
  masterUpdated: "2026-09-26",
  keepImagesInBody: true,
  readTime: "12 分钟",
  emoji: "🃏",
  image: "/images/holdem-split-pot-hero.webp",
  imageAlt: "德州扑克平分底池——牌面 8♠ 8♥ 8♦ A♣ K♠，一方 J♠ 10♥ 对另一方 5♣ 2♦，因为两手牌都打不过公共牌，筹码被一条金线分成两半",
  tags: ["平分底池 德州扑克", "德州扑克能打平吗", "chop 是什么", "什么时候平分底池", "单数筹码规则", "公共牌通杀", "德扑 平局怎么分"],
  content: `
刚玩扑克那阵子，有一手牌我每条街都在领打——翻前加注，翻牌、转牌都下注，河牌被跟注。我翻开 J♠ 10♥。对手亮出 **5♣ 2♦**。“我赢了吧？”荷官一言不发，指了指牌面：==**8♠ 8♥ 8♦ A♣ K♠**==。==r:我们两个人的底牌都盖不过带 A-K 踢脚的三条 8==，于是荷官默默把底池分成了两半。

你本以为稳赢的底池，结果只拿到一半，确实很难受。但 ==g:平分底池是有清晰规则的==——而且它正好回答了新手问得最多的一个问题：**德州扑克能打平吗？**能。下面是它发生的每一种情况。

---

> **快速回答**
> 当摊牌时两名或更多玩家拥有**完全相同的最佳五张牌**，底池就会**平分**（也叫 **chop**）。筹码被均等分开。花色永远打不破平局，任何剩下的单数筹码归庄家按钮位左边第一个平局玩家。

---

### 核心数字

:::stripe
5 | 种德州扑克底池平分的情况
0 | 次平局在德州扑克中由花色决定
1 | 枚单数筹码——归按钮位左边第一个平局座位
:::

---

## 什么是平分底池？（“Chop”是同一回事吗？）

> **快速回答**
> 在摊牌这个场合，split pot 和 chop 是同一件事（锦标赛里谈拢分奖池的 chop 是另一回事，见文末 FAQ）：规则书里写 split pot，牌手嘴上说 chop（“我们 chop 吧”）。指的都是摊牌时两手牌分不出高下，荷官把筹码在他们之间均等分开。两个词大家都在搜，所以你会看到它们混着用。

基础在于：你的牌永远是七张里的 ==**最佳五张**==——你的两张底牌加上牌面的五张。每种五张牌型的高低排在哪里，[完整的德州扑克牌型大小](/zh/blog/holdem-hand-rankings "thumb:/images/holdem-hand-rankings-hero.webp")里都讲了。==r:当两名玩家的最佳五张在牌力上完全相同，谁都不算“更”赢==——底池平分，就这么简单。

---

## 德州扑克能打平吗？会平分底池的 5 种情况

> **快速回答**
> 德州扑克能打平，而且比新手想的更常见——因为大家共用同样的五张公共牌。平分的条件永远只有一条：最佳五张点数完全相同。下面把它最常见的五种发生方式拆开讲。

下面逐一看这五种方式。

### 1. 完全相同的最佳五张牌
两名玩家凑出一模一样的五张牌——点数相同，哪怕底牌花色不同。

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:20px 0">

| | 玩家 A | 玩家 B |
|---|---|---|
| **底牌** | K♠ 7♣ | K♥ 2♦ |
| **牌面** | K♦ K♣ Q♥ Q♦ J♠ | （相同） |
| **最佳五张** | ==g:K-K-K-Q-Q== | ==g:K-K-K-Q-Q → 平分== |

</div>

两人都用牌面加一张 K 凑成葫芦 KKK-QQ。==r:那些 K 的花色无关紧要。==

### 2. 公共牌通杀
五张公共牌本身对场上剩下的所有人来说已经是最强牌——就是我故事里那个 8-8-8-A-K 的底池。这种情况够常见，所以下面单独给它开一节。

### 3. 相同的顺子
两个顶张相同的顺子打平，不看花色。在牌面 7♣ 6♦ 5♥ K♠ 2♣ 上，A 的 9♠ 8♠ 和 B 的 9♥ 8♦ 都凑成 9-8-7-6-5——一样高，所以**平分**。

### 4. 相同的同花
花色没有高低之分，所以点数相同的两个同花打平。实战中这几乎总是意味着**牌面本身**就是一副五张同花。在 K♠ J♠ 8♠ 4♠ 2♠ 上，如果 A 拿 A♥ Q♦、B 拿 10♥ 9♦，两人都没有黑桃——最佳五张都是公共牌那副 K-J-8-4-2 同花（这就是“打公共牌”，play the board），**chop**。

==r:但在你下结论之前先检查一下：==你手里任何一张比牌面最小黑桃更大的黑桃都会改进这个同花。这里哪怕不起眼的 3♠ 都能凑成 K-J-8-4-3，==g:赢下整个底池==——而 A♠ 则凑成坚果同花。

### 5. 一直相同到最后一张踢脚
对子和两对通常由踢脚决定——但如果踢脚也一样，那就是平分。牌面 A♦ Q♠ 9♣ 6♥ 2♠，A♠ K♦ 对 A♥ K♣，两名玩家都拿到 ==g:A-A-K-Q-9== → **平分**。当踢脚*不同*时，更大的那张单独赢下——这个比较到底怎么一手手比，[平局与踢脚规则](/zh/blog/holdem-tiebreak-rules "thumb:/images/holdem-tiebreak-hero.webp")里讲得很清楚。

---

## 两名玩家能赢同一个底池吗？公共牌通杀时

> **快速回答**
> 能。只要五张公共牌本身就是场上所有人的最佳五张，就是公共牌通杀，还在牌局里的每个人都分这个底池——两个人也好，五个人也好。但先问一句：有没有人能用一张底牌打过牌面？能，就不是通杀。

公共牌通杀甚至不需要两手怪物牌。那就是我的 8-8-8-A-K：我的 J♠ 10♥ 和他的 5♣ 2♦ 打的都是公共牌那副带 A-K 踢脚的三条 8——五张完全相同，两手牌一亮出来就是 chop。极端情况是像 A♠ K♠ Q♠ J♠ 10♠（皇家同花顺）这样的牌面：任何底牌都改进不了它，所以 ==g:剩下的所有人一起 chop==。

> **检查方法：** *你的*最佳五张——至少用一张底牌——能打过牌面本身的五张吗？如果能，你就打你的牌。如果不能，那就是公共牌通杀，你很可能在 chop。系统地这样扫牌面的完整方法，在[怎么读牌面、找出你的最佳五张](/zh/blog/holdem-reading-the-board)里。

**而在牌桌上最要紧的一点：你的牌只有正面亮出来才算赢。** 一手被推进弃牌堆（muck）的牌就是死牌，哪怕它本来能平分——就算你打的是公共牌，也仍然必须把底牌正面亮出来，否则就等于放弃了属于你的那一份底池（WSOP 现金局规则第 172 条；WSOP 锦标赛规则在第 75 条里重申了同一点）。谁先亮牌、整个顺序怎么走，都在[摊牌规则](/zh/blog/holdem-showdown-rules)里。

:::tip[如果公共牌通杀而有人在河牌下注，**反射性弃牌才是错的**。如果没有任何牌能盖过公共牌，平分就是确定的，跟注仍然会把底池里原有筹码中属于你的那份拿回来（单挑时是一半）——弃牌等于白白把这一份送人。如果公共牌还可能被盖过，就用频率来算：单挑时，面对满池下注，需要对手同样只有公共牌的概率达到约三分之二；面对半池下注，约一半就够（若还有三人以上在池里，你平分到的份额变小，门槛会更高）。对跟注来说这道门槛很高，对弃牌却很低：面对满池下注，只要他握着真牌的比例**超过三分之一**，弃牌就是对的——而在公共牌仍有可能被盖过的河牌上，那才是常态。]:::

---

## 哪 3 件事永远打不破平局？

![公共牌 K♦ K♣ Q♥ Q♦ J♠，左边 K♠ 7♣、右边 K♥ 2♦，中间一个金色等号——两人凑出同样的葫芦 K-K-K-Q-Q，在德州扑克里花色永远不能决定胜负](/images/holdem-split-pot-suit-equals.webp "点数相同永远平分——德州扑克没有花色高低")

> **快速回答**
> 三件事永远打不破平局：花色（黑桃同花不赢红桃同花）、更大的底牌（进不了最佳五张就等于没有）、你用了几张底牌（只有七张里最强的五张算数）。大多数“等等，这凭什么平分？！”的争吵，都卡在这三条上。

### ❌ “我花色更大，所以我赢”
==r:黑桃同花**打不过**红桃同花。==德州扑克没有花色高低——==点数相同就平分，就这么简单==。（这一点常把那些从有花色高低的玩法转过来的玩家坑到。）

### ❌ “我底牌更大，所以我赢”
牌面 9♠ 8♦ 7♣ 6♥ 5♠——一副现成的顺子。你拿 A♠ K♦；对手拿 2♣ 3♥。==r:**平分。**==你们两个都只能打公共牌 9-8-7-6-5，因为 ==r:你那些大底牌根本进不了最佳五张==。大底牌只有在真正作为踢脚起作用时才有意义——[什么是踢脚、它什么时候起作用](/zh/blog/holdem-kicker)精确地划出了这条线。

### ❌ “我用了两张牌，他们只用一张”
==r:你用几张底牌无关紧要。==唯一算数的是七张里最强的五张。==g:如果两名玩家落在同样的最佳五张上，不管你是怎么凑到的，都是 chop。==

---

## 牌型一样，为什么他独赢我没份？踢脚决定的“假平分”陷阱

> **快速回答**
> 因为牌型的“大类”一样，不等于最佳五张一样。两人都是一对 A，但比到踢脚（kicker）那一张时分出了高下，牌力高的那手就独赢整个底池——这不是平分，是“假平分”陷阱。真正的平分要求五张一张不差。

这是新手最容易读错、也最容易在牌桌上吵起来的一种情况：两个人**看起来**都是“一对 A”，喊着要平分，结果荷官把整个底池推给了其中一个人。

我带过一个学生，就在这上面交过学费：他拿 A-J，对手拿 A-Q，牌面开出一张 A，他一看“都是对 A”就把筹码往中间一推想 chop——结果那 Q 顶掉了他的 J，他一份都没拿到。

真正的平分要求==**完整的最佳五张一张不差**==。只要有一张踢脚不同，牌力更高的那手就赢下整个底池。来逐张核对一手（七选五全部列出来）：

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:20px 0">

| | 玩家 A | 玩家 B |
|---|---|---|
| **底牌** | A♠ Q♦ | A♥ J♦ |
| **牌面** | A♦ K♠ 9♣ 6♥ 2♠ | （相同） |
| **最佳五张** | ==g:A-A-K-Q-9== | A-A-K-J-9 |
| **结果** | ==g:独赢整个底池== | 输 |

</div>

逐张比：两人都用底牌的 A 配上牌面的 A 凑成==一对 A==，第一张踢脚 **K** 从牌面拿，两边完全一样。分胜负的是**第二张**踢脚——A 的 **Q** 对 B 的 **J**，==g:Q 更大，A 独赢==；共用的那张 9 只是第三张踢脚，根本没轮到比。牌型（一对 A）确实一模一样，但这**不是平分**。踢脚到底怎么一张一张往下比、哪些牌型有几张踢脚有效，[什么是踢脚、它什么时候起作用](/zh/blog/holdem-kicker "thumb:/images/holdem-kicker-hero.webp")里讲得很细。

:::tip[判断“到底是真平分还是假平分”，别只看牌型名。把双方各自的最佳五张一张一张写出来对齐——只要有任何一张不同，牌力高的那手就整个赢下，不平分。]:::

---

## 多出来那枚筹码归谁？单数筹码规则

> **快速回答**
> 多出来的那枚==单数筹码==不会被撕成两半，也不看谁的牌更漂亮：它按座位归属。荷官先把它换成场上流通的最小面额（TDA 2024 第 20 条），实在分不平时，这枚零头才按座位发出去。所以“平分”极少是分毫不差的对半。

有时底池没法均分——两人之间一个 101 枚筹码的底池，每人 50 枚，剩一枚，而且没有半枚筹码这回事。标准规则是：==剩下的那一枚归**按钮左手边**第一个平局玩家==，也就是从按钮位顺时针数、第一个还没盖牌的赢家座位。

来看一个具体的例子（最小面额筹码是 $10）：一个 **$1000 的底池**由三名平局玩家 C、D、E 平分。$1000 ÷ 3 = $333.33，除不尽。按面额算就是 100 枚 $10 筹码分给三个人：==每人 33 枚 = $330==（33 × 3 = 99 枚 = $990），==还剩 1 枚 = $10== 的零头。这枚 $10 零头归谁？==g:从按钮位往左（顺时针）数，第一个没盖牌的赢家==。假设 C 坐在最靠近按钮左手的位置，那么 C 拿走这枚零头 → C 得 $340，D、E 各得 $330。

在三方平分、多出两枚单数筹码时，顺时针最早的两个座位各拿一枚。==r:各家规矩可能不同==——少数牌房改成按高牌或花色分单数筹码——所以真金白银时最好问一下荷官主管。==g:线上则由软件按位置自动分配。==

---

## 平分底池和边池分池有什么区别？（一张表分清）

> **快速回答**
> 平分底池是“牌一样，把同一个池子劈开”；边池分池是“下注金额不一样，把总奖池分层”。前者比的是牌，后者比的是各自投入的钱，两者可以在同一手牌里同时发生。

简体牌手最常混的两个词就是==**平分底池（split pot）**==和==**边池分池（side pot）**==——名字里都有个“分”，但它们是**完全不同的两件事**。

:::compare
平分底池（Split Pot）| 边池分池（Side Pot）
起因：两人以上**牌型完全相同** | 起因：有人**筹码不够、all in**
分的是：**同一个**底池，均等劈开 | 分的是：把总注**分层**成主池 + 边池
判定：比**牌**，一样就平分 | 判定：比**投入金额**，谁能争哪层
花色不影响 | 和牌型无关，只看谁投了钱
例：8-8-8-A-K 公共牌通杀，两人各拿一半 | 例：A all in 100，B/C 各 300 → 主池 + 边池
:::

搞混的根源在这：平分是**牌力**层面的结果（打平），边池是**筹码结构**层面的结果（有人全下）。它们互不排斥——下面这个例子里，A 和 B 因为牌一样**平分了主池**，同时因为 A 全下、B/C 更深，又多出一个只有 B/C 争的**边池**。同一手牌，两种“分”都上演了。

---

## 边池也会平分吗？有人 all in 时的平局

> **快速回答**
> 会。有人 all in 时筹码分成==**主池**==（所有人都有资格）和==**边池**==（只有继续下注的深码玩家），每个池子分别结算——赢下或 chop——只看有资格争那个池子的玩家里谁的牌最好。一个 all in 玩家永远只能赢或平分他实际投入过的那些池子。

举个具体例子：A all in 投入 100；B 和 C 各投入 300。这就是一个 **300 的主池**（100 × 3）和一个 **400 的边池**（200 + 200，只有 B 和 C）。牌面跑出 A♦ J♥ 7♠ 4♣ 2♥：

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:20px 0">

| 玩家 | 底牌 | 最佳五张 | 结果 |
|---|---|---|---|
| A（all in） | A♠ Q♦ | ==g:A-A-Q-J-7== | 平分主池 → 150 |
| B | A♣ Q♥ | ==g:A-A-Q-J-7== | 平分主池（150）+ 赢下边池（400） |
| C | K♦ K♠ | K-K-A-J-7 | 两池皆输 → 0 |

</div>

A 和 B 用一对 A 加相同踢脚打平，所以他们 ==g:平分主池==；边池只由 B 和 C 争夺，而 B 的一对 A 单独打过 C 的一对 K。==r:一个 all in 玩家永远只能赢下或 chop 他实际投入过的那些池子。==这些池子最初是怎么搭起来的——封顶、再加注资格、摊牌顺序——都在[all in 规则与边池指南](/zh/blog/holdem-all-in-rules)里。

---

## 底池会不会分成一半高牌、一半低牌？

在德州扑克里不会。你可能听过像 Omaha Hi-Lo 或 Stud Hi-Lo 这样的“分池玩法”，那里的底池设计成在最佳高牌和最佳合格低牌（八或更好）之间平分——而当没有低牌够格时，高牌一家通吃（scoop）。那是另一个游戏家族。==标准德州扑克只玩高牌==——底池*只在*最佳五张牌真正打平时才平分。

---

:::readnext[继续阅读]
/zh/blog/holdem-reading-the-board | 怎么读牌面、找出你的最佳五张 | /images/holdem-reading-the-board-hero.webp
/zh/blog/holdem-all-in-rules | All in 规则与边池详解 | /images/holdem-all-in-rules-hero.webp
:::

## FAQ

**Q. 德州扑克什么时候平分底池？**

A. 当摊牌时两名或更多玩家拥有完全相同的最佳五张牌，底池就会平分；筹码在他们之间均等分开。

**Q. 底池具体怎么分？**

A. 荷官把筹码在平局玩家之间均等分开。如果底池没法均分，剩下的先换成场上流通的最小面额，最后那枚单数筹码归庄家按钮位左边第一个平局玩家。花色永远不影响平分；当有人 all in 时，主池和每个边池都各自单独分。

**Q. 平分底池和边池分池有什么区别？**

A. 平分底池是因为**牌型完全相同**，把同一个底池均等劈开；边池分池是因为**有人筹码不够 all in**，把总奖池分层成主池加边池。平分比的是牌，边池比的是各自投入的金额——两者可以在同一手牌里同时发生。

**Q. 公共牌本身就是最大牌时，全桌一起平分吗？**

A. 只要没有任何一名玩家能用底牌打过公共牌本身的五张，那就是公共牌通杀，剩下的所有人一起平分——两个人也好、五个人也好。极端例子是公共牌直接摆出皇家同花顺（A♠ K♠ Q♠ J♠ 10♠），谁的底牌都改进不了，全桌 chop。但只要有人能用底牌延伸出更高的牌（比如公共牌顺子上面再接一张），那个人就独赢，不平分。

**Q. 牌型一样，为什么他独赢我却没份？**

A. 因为**踢脚**不同。牌型的“大类”（比如都是一对 A）一样，不代表最佳五张一样。只要比到某一张踢脚分出高下，牌力高的那手就赢下整个底池——这是“假平分”陷阱。真正的平分要求完整的最佳五张一张不差。

**Q. 葫芦、顺子或两对，会平分吗？**

A. 只有两手牌完全一样时才平分。两个葫芦要三条**和**对子都相同才平分——典型场景是公共牌上有两对、双方各自凑出同一副葫芦，就像上面的情形 1。两个顺子只有顶张相同才平分，两个两对只有两个对子加踢脚都相同才平分。其它任何情况都是更大的那手赢下整个底池。

**Q. 两名玩家牌一样就一定平分底池吗？**

A. 只有完整的最佳五张打平才平分——同样的对子加同样的踢脚，或者同样的顺子、同花、葫芦。只要有任何一张踢脚不同，更大的踢脚就赢下整个底池，而不是平分。

**Q. 扑克里“chopped pot”是什么意思？**

A. chopped pot 就是牌桌黑话里的平分底池。“chop”是牌手嘴上说的；“split pot”是规则书里的术语——两者都指在打平的牌之间均等分开底池。

**Q. 花色会不会决定谁赢下一次平分？**

A. 不会。德州扑克没有花色高低，所以完全相同的五张牌永远平分，不看花色。

**Q. 底池没法均分时，多出来那枚筹码归谁？**

A. 标准规则：庄家按钮位左边第一个平局玩家。少数牌房改成按高牌或花色分，所以各家规矩可能不同——线上软件会自动处理。

**Q. 超过两名玩家能平分一个底池吗？**

A. 能。如果三名或更多玩家都拿着完全相同的最佳五张牌，底池就在他们之间均等分开——最常见于公共牌对所有人通杀时。

**Q. 有人 all in 时，平分底池怎么处理？**

A. 底池会拆成一个主池和若干边池；每个池子分别赢下或 chop，根据有资格玩该特定池子的玩家中的最佳牌来定。

**Q. 边池怎么算？**

A. 每个玩家从一个对手身上最多只能赢到自己投入的那么多。如果 A all in 投入 100，B 和 C 各下 300，那么主池是 100 × 3 = 300（三人都有资格），边池是 200 × 2 = 400（只有 B 和 C）。每个池子再各自归该池有资格玩家中的最佳牌。

**Q. 谁有资格争边池？**

A. 只有往那个特定边池里投过筹码的玩家——也就是在别人 all in 之后还继续下注的人。一个 all in 玩家只对主池（以及他之前投入过的更早的边池）有资格，永远无权碰他没跟到的那部分筹码搭起来的边池。每个池子都归它自己那批有资格玩家中的最佳牌。

**Q. 能同时赢下主池和边池吗？**

A. 能。一个深码玩家如果拿到最佳牌，可以把主池和他有资格的每一个边池全部赢下——一口气通吃。相反，一个 all in 的短码玩家只能赢主池（外加他投入过的更早的边池），无论牌多强，都拿不到他没跟到的那部分筹码。

**Q. 锦标赛的 chop 和平分底池是一回事吗？**

A. 不是——同一个词，两件不同的事。摊牌时的平分底池是自动的：牌打平时荷官就分筹码。锦标赛的“chop”是剩下的玩家之间自愿的谈拢，把奖池分掉，通常按筹码量或 [ICM](/zh/blog/holdem-icm) 计算，只有所有人同意才发生。锦标赛的派彩有什么不同，可以看[锦标赛 vs 现金局](/zh/blog/holdem-tournament-vs-cash-game)。

---

## 关键要点

1. 对，==**德州扑克是能打平的**==——只要两名或更多玩家共用 ==**七张里完全相同的最佳五张**==，底池就平分（chop）。
2. ==r:**花色、更大的底牌、你用了几张牌**==——都永远打不破平局。
3. ==**单数筹码**==归按钮位左边第一个平局玩家，而 ==**边池**==与主池分开结算。

回顾一下顺序，看[完整的牌型大小](/zh/blog/holdem-hand-rankings)；掌握贴身近牌，看[踢脚与平局规则](/zh/blog/holdem-tiebreak-rules)；解决那个经典争论，看[同花和顺子哪个大](/zh/blog/holdem-flush-vs-straight)。

---

## 相关文章

<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/zh/blog/holdem-hand-rankings" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">牌型大小</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">德州扑克牌型大小——从大到小</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">全部 10 种牌型，带概率、例子和牌面谜题</div>
  </a>
  <a href="/zh/blog/holdem-tiebreak-rules" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">平局规则</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">踢脚与平局规则</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">踢脚如何决定同牌型摊牌</div>
  </a>
  <a href="/zh/blog/holdem-flush-vs-straight" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">牌力对决</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">同花和顺子哪个大？</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">头号困惑的数学与牌面场景</div>
  </a>
</div>
`.trim(),
};
