import type { Post } from "../posts";

export const POST: Post = {
  slug: "poker-casino-primera-vez",
  title: "Cómo jugar póker en un casino por primera vez: qué llevar, cómo sentarte y qué decir",
  seoTitle: "Póker en casino por primera vez: qué llevar y qué decir",
  desc: "La guía que nadie te da: cómo anotarte en la lista, dónde comprar fichas, las siete palabras que necesitas en la mesa y los errores que delatan al novato.",
  tldr: "Lo que más frena a la gente para entrar a una sala de póker no son las reglas del juego: es no saber el trámite. Dónde te anotas, dónde compras las fichas, qué se dice y cuándo. Son unos veinte minutos de trámite que nadie explica, y luego el juego es el mismo que ya conoces. Aquí va el recorrido completo, incluida la parte que más nos preocupa a los que hablamos español: en la mesa solo necesitas siete palabras en inglés, y están en el reglamento.",
  category: "rules",
  date: "2026-07-27",
  updated: "2026-07-27",
  keepImagesInBody: true,
  readTime: "12 min",
  emoji: "🎴",
  image: "/images/poker-casino-primera-vez-hero.webp",
  imageAlt: "Las siete palabras oficiales que se usan en una mesa de póker de casino: bet, call, fold, check, all-in, pot y complete",
  tags: [
    "como jugar poker en un casino",
    "poker casino primera vez",
    "etiqueta poker en vivo",
    "cuanto dinero llevar a una sala de poker",
    "como comprar fichas poker casino",
    "lista de espera poker",
    "propina al dealer poker",
    "errores principiante poker",
  ],
  content: `
La primera vez que fui a una sala de póker me quedé parado en la puerta como diez minutos. No porque no supiera jugar: llevaba años jugando en casa. **Era que no sabía el procedimiento.** ¿A quién le hablo? ¿Dónde compro fichas? ¿Y si me toca hablar y digo la palabra equivocada?

Nadie escribe eso. Todas las guías te explican qué le gana a qué, ==pero el juego no es lo que te da nervios la primera vez: es el trámite.== Y el trámite son veinte minutos.

Aquí va el recorrido completo, en el orden en que te va a pasar.

---

### Lo esencial

:::stripe
7 | palabras en inglés — las únicas que necesitas decir
20 | minutos de trámite antes de tu primera mano
100 | veces la ciega grande, la referencia de cuánto llevar
1 | identificación oficial con foto, siempre
:::

## Antes de salir de casa

Tres cosas, y ninguna se improvisa.

**La identificación.** Oficial y vigente, con foto. Te la van a pedir en la puerta o en la caja, y en Estados Unidos también cuando cobres un premio grande. Si estás en Estados Unidos y no eres ciudadano, el pasaporte es la opción que nunca falla; algunos casinos aceptan otros documentos, pero eso ya depende de cada casa. En México con tu **INE** basta, y en la entrada te van a registrar — es trámite de un minuto.

**La edad.** En casi todo Estados Unidos son 21 años, y en varios sitios eso significa que ni siquiera puedes estar parado en la zona de juego. En México y buena parte de Latinoamérica el mínimo suele ser 18, pero cada casino lo publica en sus reglas de la casa. Revísalo antes de manejar.

**El dinero, en efectivo.** La mayoría de las salas no te dejan pagar la entrada a la mesa con tarjeta. Y aquí viene la duda que todos tienen:

### Cuánto llevar

La referencia que usa casi todo el mundo es **100 veces la ciega grande (big blind)** — el *buy-in*, o la entrada. En una mesa de $1/$2, son $200.

*Por ejemplo:*

| Mesa | Ciega grande | Buy-in de referencia |
|---|---|---|
| $1/$2 | $2 | ~$200 |
| $1/$3 | $3 | ~$300 |
| $2/$5 | $5 | ~$500 |

*En México las mesas chicas suelen ser de 10/20 o 25/50 pesos, y la regla de las 100 ciegas aplica igual: entre $2,000 y $5,000 MXN por entrada.*

Pero **ese buy-in no es tu presupuesto.** Si llegas con una sola entrada y pierdes dos manos grandes, se acabó la noche. Lo que yo hago es llevar **tres entradas completas** y decidir antes de entrar que si se van las tres, me voy. El cajero automático del casino a las tres de la mañana es el peor consejero que existe.

La cuenta completa —buy-in, presupuesto de la noche y fondo de juego son tres números distintos— está en la [guía de cuánto dinero llevar](/es/blog/cuanto-dinero-llevar-poker "thumb:/images/cuanto-dinero-llevar-poker-hero.webp").

## La lista de espera

Esto es lo primero que sorprende: **no te sientas y ya.** Las mesas están llenas y hay que anotarse.

Hay dos formas, y conviene saber las dos:

:::steps
En el mostrador | Pasas al **mostrador de la sala** (el podio de la entrada), dices qué mesa quieres y te anotan en la lista. Si no sabes cuál pedir, di que quieres **la más chica que haya** — casi siempre $1/$2 o $1/$3. Te vocean cuando se desocupa un lugar.
Desde el celular, antes de salir | Muchas salas usan la aplicación **Bravo Poker Live**. Ahí ves qué partidas están corriendo en tiempo real, y muchas salas hasta te dejan anotarte a distancia. Choctaw, por decir algo, lo dice en su propia página: descarga la app para ver qué juegos hay disponibles. En México muchas salas mueven la lista por **WhatsApp** — pregunta al llegar por el grupo.
:::

Esa segunda opción te cambia el plan por completo. Si vas a manejar una hora, **abre la app antes de salir**: si a esa hora solo hay una mesa corriendo, más vale saberlo desde tu casa.

## Comprar fichas

Dos lugares, y el segundo es el que nadie te dice.

**En la caja del casino** (*cage*), como cualquier otra transacción. Sirve siempre.

**O en la mesa misma.** Te sientas, le dices al dealer cuánto quieres, pones el efectivo sobre la mesa y él llama al *chip runner*, que te trae las fichas. En muchas salas el efectivo juega mientras llegan las fichas — pero eso depende de la casa, así que pregúntale al dealer si tu billete juega.

:::note[Un detalle que confunde a todo el mundo: **el dinero no se le entrega en la mano al dealer.** Se pone sobre la mesa, extendido. No es desconfianza: es que las cámaras tienen que ver la transacción completa. Lo mismo aplica al revés cuando te dan cambio.]:::

## Sentarte a la mesa

Ya tienes asiento y fichas. Ahora hay una decisión que casi nadie explica:

**¿Esperas a que te llegue la ciega grande, o pagas para entrar ya?** Si te acabas de sentar, puedes esperar, sin jugar, a que te llegue la ciega grande — es gratis y es lo que yo recomiendo la primera vez, porque te da tres o cuatro manos para mirar. O puedes *postear* (pagar la ciega de entrada): pones el monto de la ciega grande y entras en la siguiente mano.

Y dos costumbres de la mesa que se notan de inmediato:

- **Las fichas grandes, adelante y visibles.** Nadie puede esconder cuánto tiene. Si apilas las de mayor valor hasta atrás, el dealer te va a pedir que las pases al frente.
- **Tus cartas se quedan sobre la mesa**, y muchos jugadores les ponen encima una ficha o un objeto pequeño para que el dealer no las recoja por error.

## Las siete palabras que necesitas

Aquí está la parte que más preocupa a quien no habla inglés, y la respuesta es mejor de lo que esperas.

El reglamento del WSOP —la serie más grande del mundo, y su reglamento está publicado en línea— trae una lista de **términos oficiales**, y es la mejor referencia que existe de lo que se dice en cualquier mesa:

:::stripe
bet | apostar
call | pagar
fold | retirarse
check | pasar
all-in | ir con todo
pot | el bote (solo en pot-limit)
complete | completar
:::

Siete palabras. El mismo reglamento las describe como *declaraciones simples, inequívocas, las de siempre*, y añade algo importante: **los términos regionales también valen**, pero usar lenguaje no estándar es **a tu propio riesgo**, porque pueden entenderte otra cosa.

:::note[Y un dato que conviene conocer si vas al WSOP: su reglamento tiene una **regla de solo inglés en la mesa mientras las cartas están en juego**. No es para excluir a nadie: es para que ningún jugador pueda pasarle información a otro en un idioma que el resto no entiende. En la práctica, con esas siete palabras ya cumples. Fuera de la mano y fuera de la mesa hablas lo que quieras.]:::

## Cómo se apuesta: la voz y las fichas

El reglamento lo dice con claridad: **se apuesta con la voz, con las fichas, o con las dos.** Y si haces las dos cosas, **manda la primera**. Si son simultáneas, una declaración verbal clara tiene prioridad.

En la práctica: **habla antes de tocar tus fichas.** Si dices "raise" y luego empujas, lo que dijiste manda y estás protegido. Si empujas primero y luego hablas, ya se definió tu apuesta.

### El error que delata al novato

Se llama *string bet*, y es el más común de todos.

El reglamento lo define como **intentar una apuesta o subida en varios movimientos**. Es decir: empujas unas fichas, y luego regresas por más. El dealer lo va a cantar y **solo va a valer el primer montón** — aunque tu intención fuera subir mucho más.

Cómo evitarlo, y es fácil:

:::steps
Dilo con la voz primero | "Raise to sixty". Listo. Después mueve las fichas con toda la calma del mundo.
O muévelas en un solo movimiento | Con las dos manos si hace falta, pero un solo empuje hacia el bote.
Nunca regreses por más fichas | Ese regreso por más fichas es exactamente lo que define el string bet.
:::

## El showdown

Al final de la mano, si quedan dos o más jugadores, se enseñan las cartas. Y aquí hay una regla que tranquiliza mucho al principiante:

**Las cartas hablan.** El reglamento dice que las declaraciones verbales del valor de tu mano **no son vinculantes** en el showdown — si tú dices "tengo un par" pero en realidad tienes un color, ganas igual, porque el dealer lee las cartas.

O sea: **no tienes que saber nombrar tu mano en inglés para cobrarla.** Enseñas las cartas y ya. (Eso sí, cantar mal tu mano a propósito para engañar a alguien sí se penaliza.)

Si todavía no tienes el ranking bien metido, [aquí está completo, de mayor a menor](/es/blog/holdem-hand-rankings "thumb:/images/holdem-hand-rankings-hero.webp").

## La propina al dealer

No es una regla, es costumbre. Lo corto: en mesas bajas de Estados Unidos se acostumbra **un dólar por bote ganado** — solo cuando ganas, no en cada mano. En México la costumbre existe pero es más relajada.

Cuánto se da en botes grandes, qué cambia en torneos, a quién más se le da y cuándo no hace falta dar nada está en la [guía de la propina al dealer](/es/blog/propina-dealer-poker "thumb:/images/propina-dealer-poker-hero.webp").

## Salir de la mesa

Cuando quieras. **En un cash game puedes levantarte en cualquier momento** — no tienes que esperar nada, y no le debes explicación a nadie.

Juntas tus fichas, le avisas al dealer, y llevas las fichas a la caja para cambiarlas por efectivo. Si es una cantidad fuerte, te van a pedir identificación otra vez.

:::note[Una diferencia que conviene tener clara antes de sentarte: en un **cash game** las fichas valen dinero real y te vas cuando quieras. En un **torneo** pagas una entrada fija, las fichas no se cambian por dinero, y solo cobras si llegas a los puestos pagados. Si nunca has jugado un torneo, [cómo funcionan está aquí](/es/blog/holdem-tournament "thumb:/images/holdem-tournament-hero.webp").]:::

## Errores de principiante (y ninguno es de estrategia)

Los que más veo, en orden de frecuencia:

:::steps
Actuar fuera de turno | Es el que más molesta a la mesa, porque le da información gratis a los que siguen. Mira al dealer: él señala a quién le toca.
Hacer string bet | Ya lo vimos. Habla primero o un solo movimiento.
Dar el dinero en la mano al dealer | Va sobre la mesa, siempre.
Esconder las fichas grandes | Van adelante y a la vista.
Sentarse con lo justo | Tres entradas, y si se van las tres, se acabó la noche.
Hablar de la mano mientras se juega | Aunque ya te hayas retirado, no se comenta una mano en curso. Ni para ayudar.
Guardarse fichas en la bolsa | Lo que está en la mesa es lo que juega. No puedes meterte fichas a la bolsa ni comprar más a media mano — si quieres subir tu stack, se hace entre manos.
Enseñar las cartas al tirarlas | Para foldear se deslizan boca abajo hacia el dealer, sin levantarlas. Si las enseñas, le das información gratis a toda la mesa.
:::

Ninguno de estos es de estrategia. Son de procedimiento — y por eso se aprenden en una noche.

:::readnext[Antes de tu primera mesa]
/es/blog/texas-holdem-rules-for-beginners | Reglas del Texas Hold'em para principiantes | /images/rules-texas-holdem.webp
/es/blog/holdem-hand-rankings | Ranking de manos, de mayor a menor | /images/holdem-hand-rankings-hero.webp
:::

## Preguntas frecuentes

**Q. ¿Cuánto dinero debo llevar a una sala de póker?**

A. La referencia habitual son 100 veces la ciega grande: unos $200 para una mesa de $1/$2. Pero eso es la entrada, no el presupuesto — lo prudente es llevar tres entradas completas y decidir antes de entrar que si se acaban, te vas.

**Q. ¿Necesito hablar inglés para jugar en Estados Unidos?**

A. Con siete términos te alcanza: bet, call, fold, check, all-in, pot y complete — son los oficiales del reglamento del WSOP, y en cualquier sala te los entienden. Y no necesitas nombrar tu mano: en el showdown las cartas hablan solas.

**Q. ¿Dónde compro las fichas?**

A. En la caja del casino, o directamente en la mesa: le dices al dealer cuánto quieres y pones el efectivo sobre la mesa. Nunca se le entrega el dinero en la mano — tiene que verlo la cámara.

**Q. ¿Cómo me anoto para jugar?**

A. En el mostrador de la sala, o desde el celular con una aplicación de lista de espera como Bravo Poker Live, que además te muestra qué partidas están corriendo en ese momento. Si la sala te queda lejos, revísala antes de salir.

**Q. ¿Qué es un string bet?**

A. Intentar una apuesta o subida en varios movimientos: empujas fichas y luego regresas por más. El dealer lo canta y solo cuenta el primer montón. Se evita declarando con la voz antes de tocar las fichas, o moviéndolas en un solo movimiento.

**Q. ¿Cuánto se le da de propina al dealer?**

A. No es una regla, es costumbre. En Estados Unidos lo habitual es un dólar por bote ganado en mesas bajas, y algo más en botes grandes. Se da cuando ganas, no en cada mano.

**Q. ¿La casa cobra algo por jugar?**

A. Sí: el rake (la comisión), un porcentaje de cada bote con un tope. En Estados Unidos suele rondar el 10% con un tope de unos $5. En México depende de la sala — hay casas que cobran porcentaje y otras que cobran por tiempo, así que pregunta el porcentaje y el tope al sentarte. No lo pagas aparte: sale del bote antes de que te lo entreguen.

**Q. ¿Puedo levantarme cuando quiera?**

A. En un cash game, sí, en cualquier momento y sin dar explicaciones. Juntas las fichas y las cambias en la caja. En un torneo es distinto: la entrada es fija y solo cobras si llegas a los puestos pagados.

**Q. ¿Qué edad piden?**

A. En casi todo Estados Unidos, 21 años — y en varios sitios ni siquiera puedes estar en la zona de juego siendo menor. En México y buena parte de Latinoamérica suelen ser 18. Cada casino lo publica en sus reglas de la casa.

**Q. ¿Hay código de vestimenta?**

A. En una sala de póker, casi nunca. Con ropa casual normal vas bien — playera, jeans y tenis es lo que trae la mitad de la mesa. Si el casino tiene restaurante o zona de espectáculos, ahí sí puede haber reglas distintas.

**Q. ¿Sirven bebidas en la mesa?**

A. Sí, pasan meseras a tomar la orden y no tienes que levantarte. En Estados Unidos las bebidas sin alcohol suelen ser cortesía y se acostumbra dejar un dólar de propina a quien te la trae; en México depende de la casa. Pide agua además de lo que tomes — son sesiones largas.

**Q. ¿Y si me equivoco en algo?**

A. Pasa, y no es grave. Los dealers están acostumbrados a jugadores nuevos y corrigen sin drama. Lo único que de verdad molesta a la mesa es actuar fuera de turno — si dudas, mira al dealer y espera su señal.

---

## Fuentes

- **Terminología oficial, métodos de apuesta, string bets, cartas que hablan y la regla de idioma**: [Reglamento del WSOP 2026 (PDF)](https://wsop.gg-global-cdn.com/wsop/pdfs/2026-WSOP-Tournament-Rules.pdf) — reglas 58, 59, 69, 90 y 103
- **Listas de espera por aplicación**: [Choctaw Casino](https://www.choctawcasinos.com/Durant/), como ejemplo de sala que lo indica en su propia página

## En resumen

El juego ya lo sabes. Lo que faltaba era el trámite, y son veinte minutos:

**Identificación y efectivo**, unas 100 ciegas grandes por entrada y tres entradas de presupuesto. **Te anotas** en el mostrador o desde la app antes de salir de casa. **Compras fichas** en la caja o en la propia mesa, siempre poniendo el dinero sobre el paño. Y en la mesa, **siete palabras**: bet, call, fold, check, all-in, pot, complete.

El resto se aprende mirando. Si vas a ir esta semana, repasa el [ranking de manos](/es/blog/holdem-hand-rankings "thumb:/images/holdem-hand-rankings-hero.webp") y [cómo funcionan las rondas de apuestas](/es/blog/holdem-betting-actions) — con eso llegas sabiendo lo que hay que saber.
`,
};

export default POST;
