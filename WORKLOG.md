...
**⚠ 같은 결함이 «컴포넌트»에도 있다 — 이번 범위 밖(인수인계 참조)**
산출물 609개 스윕에서 raw `<img src="/images/…">` **1,457건**이 남는다. 전부 컴포넌트 소산이다:
로케일 블로그 목록 카드 **42장/페이지(원본 합 1,514KB)** · 로케일 홈 12장(310KB, **6장 eager**) ·
대회 가이드 히어로(**eager + fetchPriority=high = LCP인데 원본 그대로**).
