...
**핵심 결손 94 → 🟢 0편** · **ar de es id ja pt zh zh-hant 8개 로케일 전부 🔴 0** ·
FAQ **175문항** 신설 · 링크 대상 **+81** · 결재 15건 전건 승격 · 결재 11·14 이행.
게이트 5종 수리·신설(썸네일 링크 · 불가능 링크 455건 제외 · ALLOW · FAQ 절 이탈 · 링크 손실 판별법) · 셀프테스트 7→21.

### 🔴🔴 다음 세션 «첫 작업» — updated 스탬프 감사 (사장님 지시 2026-09-10)

사장님 축어: *「현재 페이지들 보면 업데이트날짜가 전부 9/9 일로 되어있는데, 어제날짜거든? 어제까지 검수폴더에서
검수와 경화작업을 같이 했었는데 업데이트가 전체페이지로 다 덮어진건 아닌지 확인해봐」*

**🟢 1차 실측은 끝냈다 — 일괄 덮어쓰기는 «아니었다».**
09-09 구간 커밋 **72개를 전수 분해**해 파일별로 «내용이 바뀌었나 / 스탬프만 바뀌었나»를 갈랐다:

| 판정 | 편수 |
|---|---|
| 내용까지 실제로 바뀐 파일 | **311편** |
| 🔴 `updated`·`masterUpdated`**만** 바뀐 파일 | **1편** |

302편이 `2026-09-09`인 것은 **그날 실제로 그만큼 만졌기 때문**이다 — 09-09는 zh 회차 7(트랙 종결) +
zh-hant 회차 7 + ja 회차 7~8 + EN-먼저 대량 전파가 **한 날에 겹친** 날이라 규모가 맞는다.

**🔴 남은 실제 결함 1건 — 이걸 먼저 고쳐라**
`lib/posts-de/holdem-positions.ts` — 커밋 `52181008`이 **`masterUpdated`만 08-11 → 09-09로 올리고
본문은 한 줄도 안 바꿨다.** 이러면 `check:drift`가 그 글을 **영원히 ✅로 통과**시킨다.
처방 = EN `holdem-positions`와 전문 대조 → 실제 델타가 있으면 이식하고 스탬프 유지, 없으면 스탬프를 08-11로 되돌려라.
🪶 이 파일은 미결 절의 「de `holdem-position-play` masterUpdated 미갱신」과 **다른 파일이지만 같은 계열**이다 — 같이 봐라.

**🟠 남는 판단 1건 (사장님 우려의 나머지 절반)**
「302편이 같은 날짜」 자체는 데이터 사고가 아니지만, **독자·구글에 302편이 한날 갱신된 것처럼 보이는 것**은 별개다.
스탬프 정본 정책이 아직 없다 — 미결 절의 「회차 `updated` 스탬프를 «착수일/마감일» 중 무엇으로 볼지」와 **같은 뿌리**다.
판정할 것 셋: ① 회차 스탬프는 착수일인가 마감일인가 ② **전파본이 원본(EN) 날짜를 쓸 것인가 전파일을 쓸 것인가**
③ 한 회차가 300편을 만질 때 스탬프를 분산할 것인가. 정하면 프로토콜 §5-⑧에 한 줄.

**🪶 재현 명령**(다음 세션이 다시 재고 싶을 때 — 커밋 구간만 바꾸면 된다)
`git log --since=<from> --until=<to> --format=%H -- lib/` 로 커밋을 뽑고, 각 커밋의
`git show --unified=0 --format= <sha> -- lib/` 에서 **`updated:`·`masterUpdated:` 줄을 뺀 나머지 변경이 0인 파일**을 세면 된다.
🔴 이 검사는 게이트에 없다 — 만들 가치가 있으면 `check:stamp`로 신설해라.
...
| **B** faq | 🟢 **완료**(155문항) | — |
| **C** link | 🟢 **완료**(+81) | — |
| **ar** | 🟢 **완료**(FAQ +20 · 이미지 2 · 절 3 · 관련글 2편) | 🔴 **원어민 렌즈 미검사** — 아래 미결 참조 |
