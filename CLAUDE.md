...
### 세션 시작 시
1. `session-handoff.md` **통독** — «다음 할 일 + 미결»만 담는다.
   🔴 **통독이 «안 되면» 그 파일이 규율을 어긴 것이다.** 2026-09-10에 **288KB·1,648행**까지 불어
   Read 상한을 넘겼고, 그래서 세션마다 **«서로 다른 500행»만 보고** 시작하고 있었다(사장님 지적).
   누적보다 이쪽이 나쁘다 — 세션마다 다른 그림을 갖는다. 실사고 = 09-09 「팩 C 동결 중」 오판.
   **짧은 알림장으로 유지한다**(2026-09-15 사용자 재확인): 현재 맥락·바로 다음 행동·막힌 점·필수 문서 링크만.
   완료 이력과 규칙 전문을 누적하지 않는다. 장기 미결의 상세는 해당 대기열에 두고 여기에는 링크만 남긴다.
   **30KB는 허용 목표가 아니라 마지막 경고선**이다 — 게이트 `npm run check:handoff`.
...
### 🔀 세션 끊기·모델 변경은 «먼저 말한다» (2026-09-24 사장님 지시)
- **작업 성격이 바뀌고(예: EN 검수 → 로케일 번역 전파) 다음 작업이 길어질 것 같으면**, 한 묶음을 마감할 때
  «새 세션에서 하는 게 낫다»고 **묻기 전에 먼저** 말한다. 근거(맥락 포화·성격 전환)와 다음 세션 첫 지시문 한 줄을 같이 준다.
  단, 끊기 전에 커밋·핸드오프 갱신이 끝나 있어야 한다.
- **작업 중 모델을 바꾸는 게 낫다고 판단되면 그때 바로 말한다** — 예: 판단·검수·§13 검산은 상위 모델, 기계적 대량 치환·단순 반복은
  가벼운 모델. 어떤 모델로 무엇을 하고 왜인지 한 줄로. 모델 분담 원칙은 메모리 `translation-model-roles-workflow` 참조.
...
| 파일당 | **quality 82 · 경직된 상한 금지**(옛 «≤60KB·q65» 폐기). 단 **무제한이 아니다** — 게이트 `npm run check:images`가 «폭≥750은 150KB / 폭<750은 80KB»를 경보한다. 근거 §9-2-A |
...
🔴 **§9-2-A 인코딩 품질 = q82 · «경직된 상한»만 금지(무제한이 아니다)**
(2026-08-30 사장님 지시 축어: *「60KB를 강제하지마..상황에 맞게 해야지...이미지가 선명하게 보여야
유저들이 포스팅을 이해하는데 도움이 되지...조금 넘어도 괜찮아」* + *「특히 모바일을 조심해야해...
이미지는 거의 너가 만든거잖아?? 화면에서 유저들이 읽을때 뚜렷하기만 하면 됨...
그리고 모바일은 속도문제가 있으니 좀 주의하고」*)

**같은 날 이 규칙을 세 번 어겼다**: ①60KB 상한이 q40 강등을 만들고 ②상한을 뺀 뒤에도 q65 고정이
남아 49장이 7~9% 작아지고(세대 손실) ③반동으로 «상한 없음»이라 적어 **가드레일까지 지웠다.**

**기준은 «화면에서 뚜렷한가»이지 수치적 무손실이 아니다.** 이미지는 대부분 생성물이라
«원본 보존»은 근거가 약하다. 그래서 q88이 아니라 **q82**다 — 눈에 띄는 아티팩트를 만드는 것은
평균오차가 아니라 **최대 국소오차**인데, 그 값이 q65 **35** → q82 **20** → q88 19로
**q82에서 이미 q88 수준**에 닿고 용량은 20% 작다.

**🪶 모바일은 오히려 유리하다**(측정): 저장 q65 → q82로 올리면 모바일 전송량이 157.7KB → **147.8KB**로
**6% 준다.** 원본 아티팩트가 적을수록 AVIF가 효율적으로 압축하기 때문이다.
**저장 품질을 올리는 것과 모바일 속도는 상충하지 않는다.**

🔴 **모바일 속도의 진짜 부채는 «폭 750px 미만» 그룹이다**(15장·713KB). 이들은 AVIF로 변환하면
**오히려 62% 커져서**(이미 q45~50으로 강압축된 세로 사진) next/image가 **원본을 그대로 내보낸다**
→ 디스크 크기 = 전송량. `apt-incheon` 한 글이 **405KB**를 통째로 받는다. 처방은 품질이 아니라
...
- [ ] `session-handoff.md` 갱신 — 🔴 **3단 마감**(2026-08-24 재구조화): ① 이번 회차 경위는
      ...
      (미결은 해결될 때까지 해당 대기열에 보존하고 핸드오프에서는 링크로 넘긴다). 시한 항목은
      `docs/update-calendar.md`로. 핸드오프를 대청소하면 이전본을 `docs/handoff-archive/`에 보관
- [ ] **`npm run check:handoff`** — 🟠·🔴면 그 자리에서 대청소한다.
      🔴 **종결된 항목은 «취소선을 긋는 게» 아니라 «지운다».** 취소선만 그어 온 결과가 288KB였다
      (종결 표시 행만 103KB). 대기열 번호는 지워도 된다 — 경위는 WORKLOG가 정본이다
...
| **다음에 뭘 고칠까 — EN-먼저 작업 대기열** | `docs/en-first-queue.md` ← **작업 재료의 본체**(2026-09-10 신설). 핸드오프에는 요약만 있다 |
