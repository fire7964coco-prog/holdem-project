...
| 무엇 | 메모 |
|---|---|
| 🆕 🔴 **세션 9 발원 소급 묶음 — 4건**(각 로케일 다음 열 때 거울 · 임의 일괄 스윕 금지): ① **AJPC Samurai Circuit III «Des TBD» → 4–13 Des 확정**(pokercalendar 실측 08-28) — en·es·de·ja·zh·zh-hant·pt 7로케일의 `korea-poker-marathon` 잔존 ② **icm 버튼 로테이션 반전 정정**(«블라인드를 가장 늦게 낸다» 거짓 — BTN→SB→BB · EN·id `11a69d0f` 선정정) — de·ja·pt·zh·zh-hant 실측 5로케일 + es는 표기 변형 재grep ③ **wpt NSW «hingga/through Sep 2026» 자기모순 파편 삭제**(Aug 2027 일몰과 공존 불가 · EN·id 선정정) — de·ja 실측 잔존 ④ **marathon Chuncheon 축 2026 상태 장치**(tldr·직답·Opsi B·FAQ 2문·결론 — 매진 대회를 축으로 여행 설계가 남는 자리 · id 08-28이 정본) — EN+es·de·ja·zh·zh-hant·pt | EN icm·wpt updated=08-28이라 게이트 드리프트 🔴가 정상 신호다 — 되돌리지 마라. ①은 사실 갱신이라 «오류 아님·staleness»(TBD는 검증 시점엔 참) |
