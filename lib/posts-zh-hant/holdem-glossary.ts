...
| **HJ（Hijack／高劫持位）** | 按鈕右邊第二個；中期位（MP），就在關煞位之前。台灣多半直接講「HJ」。 |
| **LJ（Lojack／低劫持位）** | HJ 右邊；同樣是中期位（MP）（名稱會隨桌型變動）。 |
...
A. 最基本的是下注動作（過牌、下注、跟注、加注、蓋牌、全下）、四階段（翻牌、轉牌、河牌）、[位置](/zh-hant/blog/holdem-positions)（按鈕、小盲、大盲、槍口位），還有幾個牌型詞（堅果、踢腳牌、暗三條、頂對）。把上面那組「最容易搞混」的先搞懂——尤其是過牌 vs 跟注、暗三條 vs 明三條——你就幾乎聽得懂整桌的對話了。
