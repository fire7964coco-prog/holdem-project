...
🔴 **먼저 — 검수장 MA 요청 5건이 대기 중이다**(2026-09-21 사장님 전달 · 이게 최우선).
   지시 축어 = 「MA-147·MA-148과 후속 MA-158·MA-159·MA-163을 확인해. **이미 반영한 것은 실제 커밋·원문과 대조**하고, 남은 항목은 **채택·기각 판단 후 처리**해. 끝나면 **항목별 처리 결과와 커밋을 본체 발신함에 회신**해.」
   · 원문 = `~/Downloads/홀덤검수/mailbox/out-검수장.md` (다섯 행 전부 이 파일에 있다 · en·es·id·zh 폴더엔 없다)
   · 🔴 **검수 폴더는 읽기 전용이고 착수 전 `git -C ~/Downloads/홀덤검수 pull` 필수**(`docs/mailbox-protocol.md` 11행) — 지금 그 폴더엔 미커밋 `.cache/de/*`가 있으니 건드리지 마라
   · 요지: **147** 레인 재검증 3건 병합(en rules 5 · es glossary 6 · id glossary 6) · **148** zh+zh-hant strategy 8+8 병합 · **158** APPT 코리아 2026 종료(9/14) 결과 아카이브 전환 미처리(요청 2 · 통지 1) · **159** MB-069 이행 대조 완료(요청 2) · **163** **MB-071 Q10 변경분 판정 완료**(문자열 192개 전건 대조 · 잔여 요청 1)
   · 회신은 `mailbox/out-본체.md`에 **MB-072**로(게이트 `npm run check:mailbox`를 **쓰기 전에** 돌릴 것 · 🔴 `cat >>` 금지 — 아카이브 절 아래로 붙는다)
   · 🪶 **147·148은 「이미 반영」 주장을 대조하는 일이다** — 커밋 해시와 원문을 직접 열어라([[review-mechanize-not-repeat]] 「있다 보고 전에도 탐지방법부터 의심」)

1. **🧰 queue 다음 회차 — 사장님이 번호를 고른다.** 레인 창 첫 마디 = 「`HARDEN.md` 읽고 회차 Q<번호> 시작해」 · 회차 표 = `docs/harden-queue-진행.md` §1.
   후보 ⓐ **Q11 게이트 III — 재료 실측 완료(09-21 · 바로 열 수 있다)**: `check:calc-parity`는 `starting.summary`의 **행 개수만** 보고 행이 드는 **핸드 코드는 아무도 안 본다**(그래서 12사본 동시 오류가 통과했다) · 🔴 **«ko를 순회에 넣어라»는 한 줄로 안 된다** — ko는 사전 파일 자체가 없어(`calculator-client.tsx` 안 배열) **전용 추출기**가 필요하다 · 🪶 `seo.description` ≤160은 10/10 통과라 **부채가 아니다**(재발 방지용) ⓑ **Q8-a** pt 용어·문체 분열 소급 ⓒ **Q8-b** id·de 분열(es LATAM은 열지 않는다).
