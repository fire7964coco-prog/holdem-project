...
> **快速回答**
> 21 点算牌靠三个前提：一靴牌连打很多手、能看到大部分打出的牌、对面是规则固定的庄家。德扑把三个全打破了——每一手重新洗牌，什么都留不到下一手；底牌全盖着，只看得到几张公共牌；你打的是对手不是庄家，「大牌偏多」在这里毫无意义。
...
> **快速回答**
> 德扑里真正的算牌是三门活：数 outs——能把你这手牌变成赢牌的、尚未现身的牌，同花听牌有 9 张；用阻挡牌——你手里的牌减少了对手能拿到的组合，握着 A♠ 对手就凑不出坚果同花；追踪已死的牌——在牌面之外被亮出来的那些牌，闪牌、被亮开的弃牌，你看见一张就少一张 out。全在脑子里完成，全都合法。

### 数你的 outs

==out（出路牌）==是任何一张能把你这手牌变成大概率赢牌的、尚未现身的牌。一个同花听牌有==9 张 outs==（一种花色 13 张减去你能看到的 4 张）——牌面上那两张同花色的牌已经在这 9 张里减掉了，别再把它们当成已死的牌减第二次。用==二四法则==把 outs 换成一个大致胜率：还剩两张要发时乘 4，只剩一张时乘 2。

一个 9 张 outs 的同花听牌，到河牌为止大约有==g:35%==的概率成牌（9 × 4 = 36% 只是个快速估算——真实数字是 35.0%）。这个数字把剩下两张牌都算上了，所以它能决定一次跟注，只在你真的两张都看得到的时候——你在翻牌圈已经 all in、后面没得下注。要是翻牌圈面前有个下注、转牌还得再付一次，就只算下一张牌：==9 ÷ 47 = 19.1%==。完整的方法——脏 outs、组合听牌、精确百分比——都在 [数 outs 指南](/zh/blog/holdem-outs) 里，而每一种听牌背后的赔率则在 [概率表](/zh/blog/holdem-probability "thumb:/images/holdem-probability-hero.webp") 里。
...
你能看到的每一张牌，都排除了一些可能性。在德扑里，一张 out 不可能躺在牌面上——真躺在那儿，你这手牌就已经成了——所以真正要追踪的==已死的牌==，是那些在牌面*之外*露出来的：发牌失误闪出来的一张、进 muck 之前被亮开的一手、你恰好瞄到的邻座弃牌。每一张你看见过的，都是你已经没有了的一张 out。为它们做调整，是好牌手在每条街都会保持的、一种持续而无声的习惯。这也是算牌，只不过是不需要一个累加计数的那一种。
...
> **快速回答**
> 不违规。数 outs、算赔率、用阻挡牌在德扑里 100% 合法，因为这不过是脑子里的算术。21 点里算牌也不违法，只是赌场会把让庄家亏钱的人赶走；德扑里你打的是别的牌手，牌房只抽水（rake），没人有理由拦你。唯一的红线是作弊——做记号、串通、线上实时用 solver。

世上没有任何一条规则禁止你动脑子。这本身就是"技术"的定义。

大家容易搞混的是这一点：算牌在*21 点*里其实也不违法——它不是犯罪，只是心算而已。但在多数司法管辖区，赌场是私人生意，可以把疑似算牌的人赶走或拒之门外，因为算牌手让*庄家*亏钱。德扑把这套彻底反了过来：你打的是==别的牌手==，而不管谁赢，牌房只抽水（rake）。没有任何人有理由拦着你去数自己的 outs——笼罩着 21 点的那个"被赶出场"的问题，在这儿不会因为你算牌而找上你。（牌房当然保留因为别的原因拒绝任何人的权利；但在自己的听牌上做算术不在其中。）
...
德扑只亮出五张公共牌，所以这套只能用在牌面上。梭哈奖励的，恰恰是 21 点算牌手擅长的那种追牌功夫——这是扑克里最接近电影那个版本的玩法。
...
:::steps
每次听牌都数你的 outs | 一旦你有听牌，就数一数能凑成它的牌，然后乘起来——×4 只在两张牌都会发出来时用（你已经 all in，或者后面没得下注），否则就只算下一张牌 ×2。当这个胜率高过价格时，就跟
问一问你这手牌阻挡了什么 | 诈唬之前，先看看你是不是握着一张牌，能让对手最强的跟注牌变得不可能、或更不可能出现
按已死的牌做调整 | 把任何你在牌面之外看到的 out 减掉——发牌时闪出来的一张、亮开过的一手、恰好瞄到的邻座弃牌。是恰好瞄到，不是刻意去看——故意去看别人的底牌，在任何一家牌房都算违反礼仪。你能看到的牌，就是对手不可能拿到的牌
:::
...
A. 数 outs（能把你手牌变强的牌）、用阻挡牌（你手里握着、能减少对手组合数的牌），以及追踪已死的牌（你已经亲眼看见离场的 outs——发牌失误闪出来的一张、弃牌时被亮开的一手）。三者合起来，让你读出接下来大概会来什么、对手又不可能拿到什么。
...
A. 不会。数 outs、用阻挡牌是理所当然的、有技术含量的打法，而且不管谁赢牌房都只赚抽水（rake）——所以没理由拦你。这正是和 21 点最大的不同，21 点里赌场可以把算牌手赶走。
...
3. **这是一门本事，不是什么秘密。**没人会因此把你赶走。数你的 outs、问自己阻挡了什么、把你已经看见的已死的牌减掉——每一手都这么做。
