...
const SPEC: [string, string][] = [
  ...
  // 🟢 이 열거는 앱 셀렉터(8개)와 1:1이다 — zh 랜딩은 간체·번체를 따로 셀 수 있다.
  ["语言", "简体中文（另有繁体中文、韩语、英语、日语、西班牙语、葡萄牙语、德语）"],
...
const SPOT_GROUPS = [
  {
    ...
    items: [
      ...
      { board: "A♥7♦2♣", name: "干燥的 A 高牌面", note: "范围优势的教科书——A 这张牌正好打中开池方的范围，BB 在这里 98.2% 过牌" },
      { board: "K♠8♦3♣", name: "干燥的 K 高牌面", note: "和 A 高牌面比一比：K 高牌面同样是 BTN 占优，但过牌会稍微多一点。为什么呢？" },
      // 🟢 앱 zh 축어가 이미 정정본. 다만 «在 13 个案例里»의 편 수 하드코딩만
      //    «这套案例里»로 갈았다(RP-08 · M-046 P-1과 같은 처방).
      { board: "Q♠J♦T♠", name: "broadway 高张连张双色牌面", note: "看着像两边都打中的牌面。可 BB 的权益实现在这套案例里是最低的——77.9%，BTN 是 119.4%——BB 有 99.9% 都在过牌" },
      ...
      { board: "6♠5♥2♦", name: "低张彩虹牌面", note: "一场高张（overcard）之争。研究过牌加注怎么搭建的案例——画面上 BB 的第一个动作是 96.8% 过牌、3.2% 下注" },
    ],
  },
  {
    ...
    items: [
      ...
      { board: "A♦K♠2♥", name: "3bet 方占优的 A 高牌面", note: "对 3bet 范围（一堆 AK、AA、KK）来说最好的翻牌。小注之所以能压住对手整个范围，靠的是这个范围的形状——不是因为 SPR 低" },
      // 🟢 앱 zh 축어가 이미 정정본이다(98.4 · 0.8).
      //    🔴 100−99.1로 «빼서» 구하면 0.9가 나온다 — 개별 반올림값의 합은 100이 아니다(§4-B 주의).
      { board: "Q♥T♥7♠", name: "多变的双色牌面", note: "虽然是 3bet 底池，但这个牌面对跟注方也不差。可 3bet 方并不会收手——98.4% 的范围都用同一个 2/3 尺寸打出去，只有 0.8% 过牌" },
      ...
      { board: "8♦5♣2♠", name: "低张干燥牌面", note: "3bet 范围在这里连一个顶对都翻不出来——只有卡顺和后门听牌——可超对和 A 高牌照样在施压" },
    ],
  },
  {
    ...
    items: [
      ...
      { board: "K♥T♦6♠", name: "K 高带 T 的牌面", note: "盲位对战双方范围都宽，所以到了翻牌两边都比较弱。同样是 K 高牌面，和 BTN vs BB 时的频率比一比，差在哪里" },
      { board: "7♦6♦5♣", name: "低张连张双色牌面", note: "两个宽范围撞上一个连张性极强的牌面。两对、顺子、听牌满地都是——这是「手牌/听牌」面板最热闹的一个局面" },
      // 🟢 앱 zh 축어가 이미 정정본이다(88 대 66 · 80.1) — 明三条(trips) 용어도 코퍼스 정본과
      //    일치한다(`zh/holdem-glossary` 暗三条/明三条 구분 · M-034 「코퍼스 정본 먼저」).
      { board: "A♠A♥6♦", name: "A 对子牌面", note: "牌面上摆着两张 A 的特殊局面。明三条（trips）并不少见——SB 有 88 个组合，BB 只有 66 个，所以 SB 打出 80.1%。谁手里的 A 更多，就是这个牌面的全部" },
    ],
  },
];
