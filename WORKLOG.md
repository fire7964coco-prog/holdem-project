...
LCP는 거의 안 바뀐다(이미 🟢이고 첫 페인트는 CSS가 좌우). 이 작업의 값어치는 **INP·TBT**다 —
JS 14~16KB가 줄고, 하이드레이션 때 27KB 마크다운에 정규식 40여 개를 돌리던 일이 사라졌다.
전송량 감소는 덤이며, 다국어 457편에서 30% 가까이 줄어든 게 실질적으로 가장 크다.
