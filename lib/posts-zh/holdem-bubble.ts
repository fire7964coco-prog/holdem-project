...
- **拖延（Stalling）：** 每一个决定都把时间银行用满，好少看几手牌（然后弃进钱圈）。它之所以有用，正是因为长考在计时上完全免费——这手牌照样只扣 2 分钟，不管你是秒弃还是把时间银行烧光。大码没理由拖延——他们想要更多手牌来进攻。short stack 和中码有时会拖延来求生，==但过度拖延可能招来 clock call（叫时）或罚则==，所以要 tank（长考）也得有个分寸。
