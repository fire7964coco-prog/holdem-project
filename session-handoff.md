...
### 🟢 직전 회차 (2026-09-10 (3)·(4) · Opus 헤드) — 구조 결손 A·B·C축 완주
`f1a88b65`→`3f7e8b2e` 배포·라이브 확인 · 빌드 708p exit 0. 경위 = WORKLOG 「2026-09-10 (2)(3)(4)」.
**핵심 결손 94 → 2편(ar만)** · **de·es·id·ja·pt·zh·zh-hant 7개 로케일 전부 🔴 0** ·
FAQ **155문항** 신설 · 링크 대상 **+81** · 결재 15건 전건 승격 · 결재 11·14 이행.
게이트 5종 수리·신설(썸네일 링크 · 불가능 링크 455건 제외 · ALLOW · FAQ 절 이탈 · 링크 손실 판별법) · 셀프테스트 7→21.

### 🔴 1순위 — 구조 결손 전파 회차 (**A축 완료 · B/C축 잔여**)

`npm run check:structure`가 실증한 «EN 회차가 로케일에 통째로 안 간» 자리다. **본체는 `docs/en-first-queue.md` §0.**

| 축 | 실측(2026-09-10 (4) 갱신) | 남은 것 |
|---|---|---|
| **A** h2·row | 🟢 **완료** | — |
| **B** faq | 🟢 **완료**(155문항) | — |
| **C** link | 🟢 **완료**(+81) | — |
| **ar** | 🔴 **2편** — `hand-rankings`(h2 −1 · img −2 · faq −12) · `tournament-vs-cash-game`(h2 −1 · h3 −2 · row −6 · li −4 · faq −4) | 배치 진행 중이었음 — 결과 미확인이면 먼저 `npm run check:structure -- --locale=ar` |

🔴 **다음 회차 착수 규율(이번 사고에서 승격)**
1. 🔴 게이트 🔴를 «채우기» 전에 **`docs/locale-intentional-diffs.md`를 slug로 grep**해라. 행이 있으면 채우지 말고 ALLOW에 등재해 닫아라. **게이트는 그 파일을 읽지 않는다.**
2. 게이트의 «−N»은 착수 규모의 근거가 아니다 — **주제 단위로 짝지어라**(로케일 고유 문항이 개수를 상쇄한다).
3. 서브 배치 뒤에는 **헤드가 손실을 직접 재라** — `git show HEAD:<file>` 집합 차집합. 🔴 게이트는 재조준이 만든 손실을 못 본다.
4. **`masterUpdated`를 함부로 올리지 마라** — 축 하나만 검증하고 올리면 다음 회차가 그 글을 «최신»으로 믿고 건너뛴다.
...
| `check:drift` 🔴 **핵심 21편 잔여**(de 5 · id 5 · pt 4 · **ar 7** — ar은 결재 40으로 핵심 승격) — 꼬리 100은 🟠 | 소 |
...
| 실행 항목 | 어디서 |
|---|---|
| **ar 2편** — 권고 ⓐ대로 진행 중(배치). 🔴 링크는 게이트가 원리적으로 제외했다 — 남은 건 faq·h2·row·img | 위 1순위 표 |
| **결재 2-ⓐ** 값·계산 미반영 자리(사이드팟 200↔300 등) 전 로케일 | `docs/multilang-sync-plan.md` §0-B 절차 |
| **결재 7** 사진 5장 신규 제작 → EN 교체 → 8로케일 전파 | 이미지 회차(독립) |
| **결재 15** bad-beat 행 문구 «nine outs plus live side cards» 8로케일 | 값 불변(63%) · 문구만 |
