import type { Post } from "../posts";

export const POST: Post = {
  slug: "apt-incheon-2026-guide",
  title: "APT Incheon 2026 — Hasil, Juara, dan Rekor Entri",
  seoTitle: "Siapa juara APT Incheon 2026? Hasil & rekor 1.393 entri",
  desc: "APT Incheon 2026 usai dengan rekor 1.393 entri. Juaranya Stephen Pang (Hong Kong) membawa pulang KRW 411.180.000 (~$293.700). Hasil lengkap + panduan Korea.",
  tldr: "Main Event APT Incheon 2026 dimenangkan Stephen Pang dari Hong Kong, yang membawa pulang KRW 411.180.000 (~$293.700) setelah deal ICM tiga pemain, plus trofi Singa emas 24K. Field mencetak rekor 1.393 entri dengan total prize pool KRW 3.008.880.000. Runner-up Shoumin Zhang (Tiongkok, KRW 366.500.000 / ~$261.785), posisi ketiga Yifu He (AS). Tiga besar juga mendapat kursi Main Event APT Championship 2026 di Taipei (~$10.000 per kursi). Hanya untuk warga negara asing (warga negara Korea tanpa izin tinggal tetap di luar negeri tidak boleh masuk). Seoul berjarak ~1 jam — untuk stop berikutnya, trip poker dan trip Korea tetap trip yang sama.",
  category: "tournament",
  date: "2026-06-18",
  updated: "2026-09-09",
  masterUpdated: "2026-09-09",
  keepImagesInBody: true,
  readTime: "16 mnt",
  emoji: "🇰🇷",
  image: "/images/apt-incheon-2026-guide-hero.webp",
  imageAlt: "APT Incheon 2026 di Paradise City — ruang turnamen poker premium pada malam hari dengan meja berlapis kain hijau, tumpukan chip, dan siluet pelabuhan Incheon lewat jendela dari lantai sampai langit-langit",
  tags: [
    "APT Incheon 2026",
    "turnamen poker Korea 2026",
    "poker Paradise City Casino",
    "trip poker Seoul",
    "satelit Natural8 APT",
    "wisata poker Korea",
    "Asia poker tour Incheon",
    "trip poker Korea",
  ],
  content: `
Seoul sudah masuk daftar perjalanan semua orang sejak Parasite memenangkan Oscar, Squid Game menguasai setiap percakapan, dan BTS menjadi fenomena global. Kini ada satu alasan lagi untuk memesan tiket itu: ==**salah satu festival poker terbesar di Asia berlangsung tepat di sebelah Bandara Internasional Incheon**==.

APT Incheon 2026. 7–16 Agustus. Paradise City Casino. Total prize pool bergaransi $2,7 juta. Dan saat kartu sudah disimpan — bibimbap, malam di Sungai Han, jalan-jalan di istana, dan ayam goreng Korea sampai jam 4 pagi.

Trip poker dan trip Korea adalah trip yang sama. ==g:Paradise City hanya 10 menit dari bandara, dan Seoul berjarak 1 jam dengan subway.== Anda bisa datang hanya untuk kasino, hanya untuk jalan-jalan, atau keduanya — lokasinya membuat semuanya mudah.

:::note[**★Hasil akhir (16 Agustus 2026) — festival sudah usai.** Main Event dimenangkan **Stephen Pang** dari Hong Kong dengan **KRW 411.180.000 (~$293.700)** setelah deal ICM tiga pemain, menaklukkan field **rekor 1.393 entri** dengan total prize pool **KRW 3.008.880.000**. Runner-up: Shoumin Zhang (Tiongkok, KRW 366.500.000 / ~$261.785); ketiga: Yifu He (AS, KRW 257.000.000 / ~$183.570). Tiga besar juga mendapat kursi Main Event APT Championship 2026 di Taipei (~$10.000 per kursi). Jadwal dan cara registrasi di bawah tetap dipertahankan sebagai referensi untuk stop APT berikutnya. — Sumber: laporan resmi APT, *Hong Kong’s Stephen Pang Wins Record APT Incheon Main Event for KRW 411M (~USD 293K) After 3-Way Deal* (diverifikasi 17-08-2026)]:::

---

> **Jawaban singkat**
> APT Incheon 2026 sudah berlangsung ==**7–16 Agustus**== di Paradise City Casino, Incheon. Resor ini ==g:10 menit dengan shuttle gratis dari Terminal 1 Bandara Incheon== — berangkat setiap 20 menit, tanpa biaya. Buy-in Main Event: ==**KRW 2.500.000 (~$1.703)**==, bergaransi $1 juta. Kualifikasi online Natural8 menjamin total ==g:**74 kursi**==. ==r:Mega $35 dan OnLive tutup pada 2 Agustus, dan jalur terakhir (Step $20 → Mega $200) tutup 7 Agustus, hari pembukaan festival==. ==g:**Kualifikasi online sudah selesai sebelum Main Event dimulai 9 Agustus.**== Kasino ==r:hanya untuk warga negara asing== — warga negara Korea tanpa izin tinggal tetap di luar negeri tidak boleh masuk.

---

## Kenapa Korea di 2026? (Alasan di Luar Poker)

Sebelum bicara poker: Korea Selatan diam-diam sudah menjadi salah satu negara terbaik untuk dikunjungi di Asia, dan dunia mulai menyadarinya dengan cepat.

- **Keamanan:** Seoul secara konsisten masuk peringkat sebagai salah satu kota teraman di dunia. Jalan sendirian jam 3 pagi adalah hal biasa bagi warga lokal maupun turis.
- **Kuliner:** Street food, budaya BBQ, dan restoran berbintang Michelin di Seoul menyaingi ibu kota kuliner mana pun di dunia — dengan harga separuhnya.
- **Transportasi:** Sistem subway-nya lebih murah, lebih bersih, dan lebih andal daripada kebanyakan kota. Satu kartu T-money mengurus semuanya.
- **Budaya-K:** Istana Gyeongbokgung, Bukchon Hanok Village, dan skena musik live Hongdae adalah pengalaman budaya yang benar-benar berkelas dunia.
- **Biaya:** Dibanding Tokyo, Hong Kong, atau Sydney, Seoul luar biasa terjangkau — makan malam Korean BBQ lengkap dengan minuman kurang dari $20 per orang.

Dan untuk pengunjung internasional: ==g:sebagian besar pemegang paspor negara Barat masuk Korea tanpa visa atau lewat K-ETA gratis (otorisasi perjalanan elektronik) yang bisa diajukan online dalam hitungan menit.==

**Dan ada satu alasan yang hanya berlaku tahun ini.** Pengumuman resmi APT menyebutnya terang-terangan: *"with the APT turning twenty in 2026"* — APT genap dua puluh tahun di 2026. Tur ini lahir pada 2006, saat Antanas "Tony G" Guoga menjuarai Main Event perdana di Singapura. Musim ulang tahun ke-20 ditutup dengan APT Championship di Taipei, dan itulah sebabnya **kursi Championship yang dibagikan di Incheon tahun ini bernilai lebih dari biasanya**.

---

## APT Incheon 2026 — Turnamen Sekilas

<div style="overflow-x:auto;margin:20px 0">

| Detail | Info |
|---|---|
| **Tanggal** | ==**7–16 Agustus 2026**== |
| **Venue** | Paradise City Casino, Incheon, Korea Selatan |
| **Total GTD** | ==g:**Lebih dari KRW 4 miliar (~$2,7 juta USD)**== |
| **Buy-In Main Event** | KRW 2.500.000 ==**($1.703 USD)**== |
| **GTD Main Event** | ==g:KRW 1.500.000.000 (~$1 juta USD)== |
| **Kualifikasi Online** | ==r:**Sudah tutup — jalur terakhir (Step $20 → Mega $200) tutup 7 Agu**== |
| **Sponsor** | Natural8 |
| **Siapa yang Boleh Masuk** | ==r:Hanya warga negara asing — warga negara Korea tanpa izin tinggal tetap di luar negeri tidak boleh masuk== |
| **Syarat Usia** | 19+ dengan ID foto yang valid |
| **Registrasi** | registrations@apt.poker |

</div>

---

## Jadwal Acara Lengkap

<div style="overflow-x:auto;margin:20px 0">

| Acara | Tanggal | Buy-In (USD) | GTD (USD) |
|:---|:---|:---:|:---:|
| Korea National Cup | 7–8 Agu | ~$477 | ~$136K |
| Mystery Bounty Hunter | 8–9 Agu | ~$610 | ~$204K |
| APT Super High Roller | 9–10 Agu | ==r:~$10.200== | ~$340K |
| ==**APT Main Event**== | ==**9–15 Agu**== | ==**~$1.703**== | ==g:**~$1 juta**== |
| Superstar Challenge | 14–15 Agu | ~$23.800 | — |
| Zodiac Classic | 13–14 Agu | ~$2.000 | ~$340K |
| Mini Main Event | 14–15 Agu | ~$610 | ~$272K |
| APT High Roller | 15–16 Agu | ~$3.400 | ~$340K |
| Ultra Stack | 15–16 Agu | ~$477 | ~$102K |
| Micro Main Event | 16 Agu | ~$270 | — |

</div>

**Yang membuat APT Incheon unik:** Jadwalnya menampilkan format yang tidak akan Anda temukan di WPT atau EPT — "Choose Your Own Adventure" (BB yang memilih permainannya), Omaha 21, mix 12 permainan khusus stud, dan ==g:tujuh women's event termasuk Women's Main Event — lebih banyak daripada yang digelar kebanyakan tur besar dalam satu musim penuh==. Kalau Anda bosan dengan grind NLHE yang itu-itu saja, inilah festival yang tepat.

---

## Berapa nilai seat APT Championship?

Sepanjang festival ini dibagikan **sembilan seat APT Championship (APTC) Taipei 2026** — masing-masing bernilai **$10.000**. Hampir semua liputan hanya menyebut "tiga kursi di Main Event". Itu tidak lengkap, dan selisihnya menentukan event mana yang sebaiknya Anda ikuti.

<div style="overflow-x:auto;margin:20px 0">

| Event | Buy-in (USD) | Seat APTC |
|:---|:---:|:---:|
| **APT Main Event** | ~$1.703 | **3** (tiga besar) |
| APT Super High Roller | ~$10.200 | 1 |
| APT High Roller | ~$3.400 | 1 |
| Superstar Challenge | ~$23.800 | 1 |
| High Roller – Ultra Stack | ~$3.066 | 1 |
| **Mini Main Event** | **~$610** | **1** |
| Zodiac Classic | ~$2.000 | 1 |
| **Total** | | **9** |

</div>

==g:Perhatikan baris termurah.== **Mini Main Event ~$610** membawa satu seat utuh senilai $10.000. Memenangkannya sama sekali berbeda dari menembus tiga besar Main Event berisi seribu entri lebih — padahal tiketnya sama persis. Dari sembilan seat, **ini pintu masuk termurah**.

> ⚠️ **Dua event dengan nama mirip — jangan tertukar.** Seat diberikan oleh **#37 High Roller – Ultra Stack (KRW 4,5 juta, 10–11 Agustus)**. **#90 Ultra Stack (KRW 700 ribu, 15–16 Agustus)** yang ada di jadwal atas **tidak memberi seat APTC**. Namanya tumpang tindih, tetapi buy-in-nya berbeda lebih dari enam kali lipat.

**Ke mana tiket itu membawa Anda:** APT Championship 2026 berlangsung **13–29 November di Red Space, Taipei**. Main Event Championship digelar **23–27 November, freezeout $10.000 dengan garansi $5.000.000** — event APT terbesar sepanjang tahun. Dan berbeda dengan Paradise City, **Taipei tidak punya pembatasan kewarganegaraan untuk masuk**.

## Masih Sempat? — Yang Tersisa Cuma Step $20

Natural8 adalah partner online resmi APT dan menjalankan kampanye satelit ==**Journey to APT Incheon 2026**==. Ada dua jalur menuju Main Event:

<div style="background:rgba(255,248,210,0.08);border:1px solid rgba(255,240,180,0.25);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Rute | Kursi GTD | Cara Kerja |
|---|---|---|
| **Mega Satellite → Live Day 1** | 52 kursi | Menang online → mainkan Main Event live penuh dari Day 1 |
| **OnLive Day 1** | 22 kursi | Mainkan Day 1 online → jika Anda mengamankan chip, lanjut live dari Day 3 |

</div>

:::note[**★Status per 9 Agustus 2026 — kualifikasi online sudah selesai.** Mega $35 dan kualifikasi OnLive Day 1 tutup pada 2 Agustus, dan jalur terakhir, **Step $20 → Mega $200**, tutup pada **Jumat, 7 Agustus** — hari pembukaan seri. Angka 52 + 22 = 74 kursi di tabel itu adalah total terjamin untuk jalur-jalur tersebut. Rutenya tetap dicatat karena struktur yang sama kembali di stop APT berikutnya.]:::

**Jalur berjenjang, seperti saat itu berjalan:**

<div style="background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.10);border-radius:10px;padding:14px 20px;margin:16px 0;font-size:13px;line-height:1.8">
Step $20 → menangkan tiket ke Mega Satellite $200 → menangkan kursi Main Event (senilai ~$1.703) · hari terakhirnya 7 Agustus
</div>

==g:Natural8 juga menjalankan bonus payout 8%== untuk peserta yang lolos lewat satelit — kalau Anda memenangkan $10.000 di event live, Anda dapat tambahan $800 di atasnya. Perlu dicatat: **akses ke platform ini ditentukan negara domisili** dan tidak semua negara termasuk — periksa dulu apakah negara Anda muncul di formulir pendaftaran mereka. ⚠️ **Memakai VPN untuk menembus pembatasan wilayah melanggar ketentuan layanan** dan berisiko akun dibekukan beserta saldonya, jadi itu bukan jalan keluar. **Jalur yang selalu terbuka untuk siapa pun adalah mendaftar langsung ke penyelenggara di registrations@apt.poker** dengan membayar buy-in penuh.

> **Dulu OnLive itu apa:** lewat rute ini Day 1 bisa dimainkan dari zona waktu sendiri — terbang ke Korea baru perlu kalau bertahan sampai Day 3, jalur paling cerdas saat anggaran perjalanan ketat. ==r:Rutenya tutup 2 Agustus, jadi untuk edisi ini sudah bukan pilihan== — dicatat di sini karena layak diincar di stop APT berikutnya.

**Ada jalur ketiga yang hampir tidak pernah disebut panduan lain: Country Exclusives.** Natural8 menjalankan kualifikasi khusus berdasarkan kewarganegaraan yang membagikan **enam paket live** — dan paket bukan sekadar kursi: isinya **tiket Main Event (~$1.684) plus sekitar $500 biaya perjalanan**. Satelit biasa hanya memberi kursi, tiket pesawat urusan Anda sendiri. ==r:Kelayakan di sini ditentukan **kewarganegaraan dan dicek lewat KYC** — bukan alamat domisili yang Anda daftarkan, berbeda dengan akses akun yang memang bergantung pada negara domisili.== Satu-satunya bracket yang diumumkan untuk Incheon 2026, "N8TW x APT Incheon 2026", **hanya untuk warga negara Taiwan**. Cek dulu di lobby apakah ada edisi yang benar-benar bisa Anda ikuti sebelum menghitung jalur ini.

---

## Siapa saja yang benar-benar datang? — Komposisi field 2025

Sebelum memesan tiket pesawat, ada baiknya tahu meja seperti apa yang menanti. APT mempublikasikan rincian per negara, dan Main Event Incheon 2025 tampak seperti ini:

<div style="overflow-x:auto;margin:20px 0">

| # | Negara | Entries | Porsi |
|:---:|:---|---:|:---:|
| 1 | **Jepang** | 484 | 37,8% |
| 2 | **Tiongkok** | 299 | 23,3% |
| 3 | Thailand | 107 | 8,4% |
| 4 | Hong Kong | 106 | 8,3% |
| 5 | Taiwan | 49 | 3,8% |
| 6 | Rusia | 40 | 3,1% |
| 7 | Singapura | 37 | 2,9% |
| 8 | Vietnam | 29 | 2,3% |
| 9 | Amerika Serikat | 26 | 2,0% |
| 10 | **Malaysia** | **21** | **1,6%** |

</div>

==r:Jepang dan Tiongkok saja sudah 61% dari field.== Ada 27 negara yang hadir, dan menariknya **tetangga Asia Tenggara sudah masuk daftar** — Thailand ke-3, Singapura ke-7, Vietnam ke-8, Malaysia ke-10. Indonesia belum muncul di sepuluh besar.

Itu justru kabar bagus buat pemain Indonesia. Secara jarak Indonesia tidak lebih jauh dari negara-negara yang sudah rutin datang: **Jakarta–Incheon sekitar 7 jam penerbangan langsung** (Garuda, Korean Air, Asiana, dan T'way Air sejak April 2026), dibanding ~5,5 jam dari Bangkok dan ~6,5 jam dari Kuala Lumpur. ==g:Zona waktunya pun cuma **2 jam di depan WIB**, jadi jadwal Day 1 OnLive yang mengikuti waktu Korea masih jam manusiawi buat pemain Indonesia== — keunggulan nyata dibanding pemain Eropa yang harus mundur 7 jam. Dan ==g:Anda datang tanpa jejak sama sekali — tidak ada reg yang punya read atas Anda==, sementara banyak reguler di sirkuit ini sudah saling kenal bertahun-tahun.

**Angka 2025:** 1.281 entri dari **754 pemain unik** (sisanya re-entry), membentuk prize pool **KRW 2.766.960.000 (~$2,0 juta)** — Main Event terbesar yang pernah digelar APT di Korea sampai saat itu. Juaranya pemain Tiongkok **Junjie He** dengan ₩380.520.000 (~$275.140) plus satu kursi APT Championship, setelah kesepakatan ICM tiga arah bersama Kouki Okumiya dan Kuroda Kiyoto.

> **Baca angka entri dengan benar.** "1.281 entri" bukan 1.281 orang, melainkan 754 pemain plus re-entry. Field dengan re-entry selalu terlihat lebih besar dari kenyataannya.

---

## Venue: Paradise City Casino

![Interior Paradise City Casino — resor kasino mewah yang terhubung dengan Bandara Internasional Incheon dengan meja poker, mesin slot, dan lobi hotel kelas atas](/images/apt-incheon-2026-guide-venue.webp)

Paradise City bukan kasino yang berdiri sendiri — ini adalah ==**resor terpadu yang dibangun sebagai bagian dari perluasan Bandara Internasional Incheon**==, dirancang untuk menangkap lalu lintas transit dan kedatangan internasional.

**Apa artinya bagi Anda:**

- ==g:Bus shuttle gratis berjalan setiap 20 menit dari Terminal 1 (Gate 3C)== — ~10 menit, tanpa biaya, tanpa masalah cuaca
- Hotel bintang 5 (Hotel Paradise) di lokasi dengan akses langsung ke kasino
- Food hall internasional, spa, dan kompleks hiburan
- 45–60 menit ke pusat Seoul via kereta AREX dari bandara

**Aturan venue yang perlu diketahui:**
- Dress code: dilarang sandal jepit, dilarang piyama
- Dilarang membawa kamera atau memotret di dalam kasino
- ==r:Warga negara Korea tanpa izin tinggal tetap di luar negeri tidak boleh masuk== — yang lain cukup menunjukkan paspor di pintu masuk
- Usia: 19+ dengan ID

---

## Menuju Lokasi: Bandara Incheon ke Paradise City

Paradise City menjalankan transfer gratisnya sendiri dari bandara:

| Opsi | Perjalanan | Biaya | Catatan |
|:---|:---:|:---|:---|
| ==g:**Shuttle gratis (dari T1)**== | ==g:~10 mnt== | ==g:Gratis== | Opsi terbaik — Gate 3C, setiap 20 mnt (jam 5 pagi–11 malam) |
| Shuttle gratis (dari T2) | ~20 mnt | Gratis | Gate 4A, setiap 60 mnt |
| Taksi dari T1 | ~15–20 mnt | ~$15–20 | Argometer, tersedia 24/7 |
| AREX ke Stasiun Unseo → taksi | ~20 mnt total | ~$8 | Berguna jika jadwal kereta pas |

Kalau Anda transit melalui Incheon dan punya layover panjang: ==g:kasino bisa diakses saat transit== — cek dengan maskapai Anda soal persyaratan visa untuk kewarganegaraan Anda selama transit.

---

## Day Trip ke Seoul: 1 Jam dari Paradise City

Inilah kondisi yang tidak disadari kebanyakan pemain turnamen sampai mereka tiba: ==**Paradise City adalah markas Anda, dan Seoul hanya 1 jam jauhnya**==.

Anda bisa menginap di hotel kasino sepanjang trip — mainkan sesi Anda, tidur, ulangi. Atau Anda bisa melakukan yang dilakukan kebanyakan pemain reguler: selesaikan sesi pagi, naik subway, habiskan sore di Hongdae atau Myeongdong, lalu kembali ke meja pada malam hari. Transportasinya membuat semua itu benar-benar realistis.

**Dari Paradise City ke Seoul:**

<div style="overflow-x:auto;margin:14px 0">

| Rute | Tujuan | Perjalanan | Biaya |
|:---|:---|:---:|:---:|
| ==g:**Shuttle gratis → T1 → AREX Express**== | ==g:Seoul Station== | ==g:~53 mnt== | ==g:~₩11.000 (~$8)== |
| Shuttle gratis → T1 → AREX All-Stop | Hongdae (Hongik Univ.) | ~70 mnt | ~₩4.750 (~$3,50) |
| Bus Bandara 6001 | Myeongdong / City Hall | ~60 mnt | ~₩17.000 (~$13) |
| Bus Bandara 6015 | Myeongdong / Chungjeongno / Gongdeok / Mapo | tergantung lalu lintas | ~₩17.000 (~$13) |
| Taksi langsung | Hongdae / Myeongdong | ~50–60 mnt | ~$35–50 |

</div>

==g:Dua opsi AREX dari T1==: **Express** (non-stop ke Seoul Station dalam 43 mnt, ₩11.000 — lalu transfer ke Line 1 atau 4; untuk Line 2, naik All-Stop ke Hongik University saja) atau **All-Stop** (langsung ke Stasiun Hongdae/Hongik University tanpa transfer, ~60 mnt, ₩4.750). Kereta All-Stop berjalan setiap 6–10 menit, jam 5 pagi sampai tengah malam. Bus bandara berjalan langsung ke Myeongdong jika Anda lebih suka itu.

Berikut yang bisa dilakukan dengan 1–3 hari luang:

![Overview Google Maps menampilkan Paradise City di Incheon dan jaraknya ke pusat Seoul — sekitar 50 km di barat kota](/images/apt-incheon-2026-guide-map-wide.webp)

![Google Maps di-zoom ke pusat Seoul dengan Namsan (N Seoul Tower) ditandai — Hongdae di barat, Sungai Han di selatan](/images/apt-incheon-2026-guide-map-seoul.webp)

### Hari 1 — Seoul Lawas: Istana, Hanok & Street Food

**Pagi:** Mulai dari ==**Istana Gyeongbokgung**== (Stasiun Gyeongbokgung, Line 3). Dibangun tahun 1395, kompleks istananya sangat luas. Tiket masuk ~₩3.000 — atau sewa hanbok (~₩20.000) dari toko terdekat untuk ==g:masuk istana gratis==. Sisihkan 2 jam.

**Siang:** Berjalan 15 menit ke ==**Bukchon Hanok Village**== — kawasan permukiman di perbukitan berisi 900+ rumah tradisional Korea (hanok) yang berasal dari dinasti Joseon. Kunjungi antara jam 10 pagi–5 sore; ini masih kawasan permukiman jadi kebisingan diharapkan tetap rendah. Pemandangan dari puncak Gahoe-dong adalah spot paling banyak difoto di Seoul.

**Malam:** Menuju ==**Pasar Gwangjang**== (Stasiun Jongno 5-ga, Line 1) untuk makan malam. Pasar beratap tertua di Seoul ini punya lebih dari 5.000 kios — rute food tour-nya mencakup bindaetteok (pancake kacang hijau), bibimbap, mayak gimbap, dan sannakji (gurita hidup, opsional). ==g:Salah satu pengalaman kuliner terbaik di Asia dengan harga di bawah $10.==

---

### Hari 2 — Hongdae, Myeongdong & Cheonggyecheon

**Pagi:** ==**Ikseon-dong**== — kumpulan bangunan hanok berusia seabad yang diubah menjadi kafe brunch dan kedai kopi spesial, 10 menit jalan kaki dari Gyeongbokgung. Gang-gang kecil, belum ada turis, kopi enak. Cara bagus untuk memulai hari sebelum ramai.

**Sore:** Naik subway ke barat menuju ==**Hongdae**== (Stasiun Hongik University — Line 2, juga AREX langsung dari Incheon). Dinamai dari Hongik University, salah satu sekolah seni terbaik Korea, kawasan ini punya tiga zona: ==**walking street utama**== tempat crew dance K-pop dan busker tampil sejak sore; ==**Yeonnam-dong**== di utara, kumpulan kafe independen yang lebih tenang di sepanjang Gyeongui Line Forest Park (taman linier yang dibangun di atas rel kereta lama); dan ==**distrik klub**== antara Exit 6 dan 9 untuk nanti malam. Seluruh kawasan hidup sampai fajar — datang untuk energi sorenya, tinggal selama Anda mau.

<div style="display:flex;gap:6px;margin:14px 0">
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-hongdae.webp" alt="Pintu masuk subway Stasiun Hongik University (Hongdae) — Line 2, Airport Express, dan Gyeongui-Jungang, dengan sepeda sewaan terparkir di luar" loading="lazy" decoding="async" width="600" height="800" sizes="(max-width: 768px) 47vw, 333px" style="width:100%;height:auto;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Stasiun Hongdae — AREX All-Stop langsung dari Bandara Incheon (~60 mnt, tanpa transfer)</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-kbeauty.webp" alt="Toko Olive Young di Hongdae — jaringan kesehatan dan kecantikan terbesar Korea dengan ribuan produk K-beauty dan perawatan kulit" loading="lazy" decoding="async" width="600" height="800" sizes="(max-width: 768px) 47vw, 333px" style="width:100%;height:auto;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Olive Young — setiap brand K-beauty yang Anda lihat di TikTok</p>
  </div>
</div>

**Malam:** Subway ke ==**Myeongdong**== (Line 4, Exit 6). Kawasan paling banyak dikunjungi turis asing di Seoul — dan itu ada alasannya. Jalan pejalan kaki utamanya (Myeongdong-gil) membentang sekitar 800 m dari ujung ke ujung: toko flagship K-beauty (Olive Young, Innisfree, Laneige, Sulwhasoo) di kedua sisi, pedagang street food mengisi celahnya sejak jam 5 sore. ==g:Street food terbaik: hotteok (pancake gula merah), tteokbokki, corn dog keju, sate lobster.== Untuk makan yang layak, ==**Myeongdong Kyoja**== (sejak 1966) adalah yang pantas diantre — kalguksu (mi potong pisau) dan mandu (pangsit). Kebanyakan toko tutup sekitar jam 10 malam; bus bandara kembali ke Incheon berhenti ==g:tepat di luar Stasiun Myeongdong==. ==g:Bawa paspor Anda — toko K-beauty menawarkan tax refund di tempat.==

<div style="display:flex;gap:6px;margin:14px 0">
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-myeongdong1.webp" alt="Eksterior restoran Myeongdong Kyoja — spot kalguksu dan mandu paling terkenal di Seoul sejak 1966, selalu penuh turis dan warga lokal" loading="lazy" decoding="async" width="480" height="640" sizes="(max-width: 768px) 31vw, 220px" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Myeongdong Kyoja — mi potong pisau sejak 1966, selalu antre</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-myeongdong2.webp" alt="Halte Bus Bandara di Stasiun Myeongdong — layanan bus langsung ke Bandara Incheon dari pusat Seoul" loading="lazy" decoding="async" width="480" height="640" sizes="(max-width: 768px) 31vw, 220px" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Halte bus bandara tepat di Stasiun Myeongdong → Incheon ~60 mnt</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-myeongdong3.webp" alt="Jalan belanja Myeongdong dipenuhi toko K-beauty dan apotek berwarna-warni — Holika Holika terlihat di sebelah kanan" loading="lazy" decoding="async" width="480" height="640" sizes="(max-width: 768px) 31vw, 220px" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Myeongdong-gil — 800 m K-beauty, street food, dan papan neon</p>
  </div>
</div>

**Jalan malam:** Dari Myeongdong, berjalan ke utara sekitar 10 menit dan Anda akan turun ke ==**Cheonggyecheon Stream**== (청계천). Kanal urban sepanjang 11 km yang mengalir melintasi jantung pusat kota Seoul — dulu terkubur di bawah jalan tol setelah Perang Korea, digali dan direstorasi tahun 2005. Jalurnya berada di bawah permukaan jalan, diapit tembok batu, pohon willow, dan naungan musim panas. ==g:Burung kuntul berdiri di tengah aliran air, mencari ikan.== Pada malam hari instalasi lampu memantul di permukaan air dan warga lokal duduk di tepiannya dengan minuman dari minimarket. Exit subway muncul setiap beberapa menit di sepanjang jalur, jadi Anda bisa berjalan sebanyak atau sesedikit yang Anda mau dan muncul di mana saja.

<div style="display:flex;gap:6px;margin:14px 0">
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-cheonggyecheon1.webp" alt="Pemandangan lebar Cheonggyecheon Stream — jalur setapak dan batu pijakan dengan gedung pencakar langit pusat Seoul terlihat di balik deretan pohon" loading="lazy" decoding="async" width="480" height="640" sizes="(max-width: 768px) 31vw, 220px" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Kanal 11 km yang mengalir melintasi pusat Seoul — gratis, 24 jam</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-cheonggyecheon2.webp" alt="Close-up Cheonggyecheon Stream — burung kuntul putih berdiri di air yang mengalir, sepasang orang duduk di bebatuan tepi kanal" loading="lazy" decoding="async" width="480" height="640" sizes="(max-width: 768px) 31vw, 220px" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Burung kuntul mencari ikan di kanal — di tengah kota besar</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-cheonggyecheon3.webp" alt="Jalur setapak Cheonggyecheon Stream — pohon willow menjuntai di sisi kanal, pejalan kaki menyusuri jalur tepi sungai" loading="lazy" decoding="async" width="480" height="640" sizes="(max-width: 768px) 31vw, 220px" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Exit subway setiap beberapa menit di sepanjang jalur — berjalan sejauh yang Anda mau</p>
  </div>
</div>

**Larut malam:** Korean BBQ. Duduk di meja dengan panggangan arang tertanam, pesan samgyeopsal (perut babi panggang), bungkus dengan daun selada bersama bawang putih dan pasta ssamjang, padukan dengan soju. Kebanyakan restoran Korean BBQ buka sampai jam 2 pagi.

> 🕌 **Catatan untuk yang mencari halal.** Samgyeopsal itu daging babi dan soju itu alkohol — keduanya inti dari Korean BBQ pada umumnya, jadi bagian di atas tidak berlaku kalau Anda butuh halal. Arahkan langkah ke **Itaewon**: **Masjid Pusat Seoul** (berdiri sejak 1976, masjid pertama dan terbesar di Korea) ada di sana, dan gang-gang di sekitarnya penuh restoran halal serta toko bahan makanan Timur Tengah. Beberapa nama yang bersertifikat: **Makan Halal Korean Restaurant** (Itaewon, sertifikat KMF — tteokbokki, bibimbap, kimchi stew versi halal), **Siti Sarah** (Itaewon, masakan Melayu dan Mesir), dan **Osegyehyang** (Insadong, vegetarian bersertifikat halal).
>
> **Cara cepat memastikan:** cari salah satu dari tiga penanda ini — logo **halal certified by KMF** (Korea Muslim Federation, yang paling diakui), stiker **Muslim Friendly Restaurant**, atau tulisan **No Pork, No Alcohol**. Aplikasi **Halal Navi**, **Muslim Pro**, dan **Halal Korea** bisa menunjukkan pilihan terdekat. Untuk jaga-jaga: minimarket **CU** dan **GS25** menjual mie cup berlabel halal KMF — praktis untuk sarapan sebelum Day 1 atau makan larut setelah bustout.

---

### Hari 3 — Seoul Modern: Gangnam, Seongsu & Sungai Han

**Pagi:** Naik Line 2 ke ==**Gangnam**==. Gangnam yang sesungguhnya bukan seperti yang digambarkan lagu itu — ini adalah distrik bisnis dan hiburan kelas atas Seoul, rumah bagi COEX Mall (kota bawah tanah), Kuil Buddha Bongeunsa, dan beberapa kedai kopi terbaik di negeri ini.

**Sore:** ==**Seongsu-dong**== ("Brooklyn-nya Seoul") berjarak 10 menit taksi dari Gangnam. Bekas gudang industri yang diubah menjadi kafe spesial, studio desain, dan concept store. Kawasan ini menjadi yang paling banyak di-Instagram di Seoul dalam dua tahun terakhir. Perhentian sore yang bagus sebelum kembali menuju Incheon.

**Malam:** Kalau penerbangan Anda esok pagi, posisikan diri di ==**Itaewon**== atau dekat ==**Stasiun Hongik University**== untuk malam terakhir keluar — kedua kawasan punya bar dan restoran internasional yang buka sampai jam 3–5 pagi. Lalu AREX kembali ke bandara.

---

## Rincian Anggaran Korea untuk Trip Poker

<div style="overflow-x:auto;margin:20px 0">

| Item | Kisaran Anggaran |
|:---|:---:|
| Tiket pulang-pergi (dari hub besar Asia) | $200–$600 |
| Tiket pulang-pergi (dari AS/Eropa) | $700–$1.400 |
| Hotel dekat turnamen (Paradise City) | $150–$350/malam |
| Hotel di Seoul (kelas menengah) | $60–$120/malam |
| Makan harian (street food + restoran) | $20–$40/hari |
| Transportasi Seoul (kartu T-money) | $5–$10/hari |
| Buy-in APT Main Event | ==**~$1.703**== |
| Side event hemat (Mini Main, Ultra Stack) | ==g:**$270–$610**== |
| **Estimasi total trip poker 7 hari** | ==**~$800–$4.200 + buy-in poker**== |

</div>

*Batas bawah = penerbangan Asia + hotel kelas menengah di Seoul; batas atas = penerbangan jarak jauh + hotel Paradise City (7 malam, dari baris-baris di atas).*

Dibanding EPT Barcelona (Main ~€5.300) atau WPT Australia (Main ~AUD $5.000), ==g:APT Incheon memberi Anda turnamen GTD $1 juta dengan buy-in di bawah $2.000== — dan salah satu kota terbaik di Asia sebagai latarnya.

---

## Syarat Masuk: Visa & Akses Kasino

**Visa Korea:**

| Kewarganegaraan | Perlu visa? | Cara masuk |
|---|---|---|
| ==r:**Indonesia**== | ==r:**Perlu visa**== | **Visa turis C-3-9** lewat Kedubes Korea di Jakarta / Korea Visa Portal. ⚠️ Bebas visa 30 hari itu **khusus Pulau Jeju** — harus mendarat langsung di Jeju dan **tidak boleh melanjutkan ke Incheon/Seoul** |
| AS, Kanada, Inggris, Australia, NZ, Jepang, Singapura, sebelas negara UE, Norwegia, Swiss, Liechtenstein | **Tanpa visa** | Dibebaskan dari K-ETA sampai 31 Desember 2026 — cukup e-Arrival Card gratis |
| Malaysia | **Tanpa visa** | Tidak masuk daftar pembebasan → **perlu K-ETA** (₩10.000, k-eta.go.kr) |
| Hong Kong, Makau, Taiwan | **Tanpa visa** | Juga dibebaskan dari K-ETA sampai 31 Desember 2026 — cukup e-Arrival Card gratis |
| Sebagian besar negara lain | Cek [mofa.go.kr](https://www.mofa.go.kr) | Mungkin perlu visa turis |

==r:Ajukan K-ETA sebelum Anda memesan tiket.== Biasanya instan tapi cek 2 minggu sebelumnya di [k-eta.go.kr](https://www.k-eta.go.kr).

**Masuk kasino (Paradise City):**
- Wajib paspor yang valid
- Usia 19+
- ==r:Warga negara Korea tanpa izin tinggal tetap di luar negeri tidak boleh masuk==
- Warga Korea perantauan (penduduk tetap di luar negeri) boleh masuk dengan paspor Korea + green card atau surat keterangan emigrasi

---

:::readnext[Lanjut membaca]
/id/blog/holdem-tournament | Panduan Turnamen Texas Hold'em | /images/holdem-tournament-hero.webp
/id/blog/korea-poker-marathon-2026 | Panduan Korea Poker Marathon 2026 | /images/korea-poker-marathon-2026-hero.webp
:::

## FAQ

**Q. Apakah Paradise City Casino benar-benar bisa dijangkau berjalan kaki dari Bandara Incheon?**

A. Sangat dekat. Paradise City menjalankan shuttle gratis dari Terminal 1 (Gate 3C) setiap 20 menit — perjalanannya sekitar 10 menit. Terminal 2 punya shuttle sendiri setiap 60 menit dari Gate 4A. Resor ini adalah kompleks terpadu yang bersebelahan dengan bandara, dirancang khusus untuk kedatangan internasional.

**Q. Bisakah saya bermain di APT Incheon Main Event kalau belum pernah main turnamen live?**

A. Edisi 2026 sudah usai — tapi jawabannya tetap berlaku untuk perhentian APT berikutnya: bisa. Buy-in-nya (~$1.703) memang besar tapi strukturnya ramah pemula — starting stack yang dalam dan level yang panjang. Kalau Anda nyaman dengan home game atau poker online, transisi ke live cukup mudah. Mini Main Event (~$610, KRW 900K) dan Ultra Stack (~$477) adalah opsi stakes lebih rendah untuk memulai.

**Q. Bagaimana cara saya pergi dari Paradise City ke Seoul?**

A. Naik shuttle gratis kembali ke Terminal 1 Bandara Incheon, lalu naik AREX. Dua opsi: **Express** (non-stop ke Seoul Station, ~43 mnt, ₩11.000) atau **All-Stop** (langsung ke Stasiun Hongdae/Hongik University tanpa transfer, ~60 mnt, ₩4.750). All-Stop lebih murah dan menurunkan Anda di Hongdae tanpa transfer apa pun.

**Q. Apakah saya perlu bisa berbahasa Korea untuk menjelajahi Seoul?**

A. Tidak. Papan penunjuk subway Seoul sepenuhnya dwibahasa (Korea/Inggris), Google Maps bekerja akurat di seluruh kota, dan sebagian besar bisnis yang melayani turis punya staf yang bisa berbahasa Inggris. Aplikasi Papago (penerjemah dari Naver) menangani menu dan papan jalan dengan baik.

**Q. Kapan waktu terbaik menambahkan hari di Seoul — sebelum atau sesudah turnamen?**

A. Keduanya bisa, tapi ==g:disarankan tiba 2–3 hari lebih awal==. Anda pulih dari jet lag, menjelajahi kota dengan santai, dan tiba di Day 1 dalam kondisi segar dan sudah menyesuaikan diri. Setelah turnamen, hari bustout mana pun otomatis menjadi hari gratis untuk menjelajahi Seoul.

---

## Poin Penting

1. **APT Incheon 2026** sudah berlangsung ==**7–16 Agustus**== di Paradise City Casino — 10 menit dari Bandara Incheon dengan shuttle gratis, GTD $2,7 juta.
2. **Main Event** buy-in: ==**~$1.703**== dengan ==g:garansi $1 juta==. Sekitar sepertiga buy-in Main EPT Barcelona dan separuh buy-in Championship WPT Australia.
3. **Kualifikasi online tutup bersamaan dengan dibukanya festival** — Mega $35 dan OnLive tutup 2 Agustus, jalur terakhir (Step $20 → Mega $200) tutup 7 Agustus, hari pembukaan. Di sisa festival, pendaftaran di lokasi adalah satu-satunya jalan masuk.
4. **Seoul berjarak 60 mnt** dan merupakan salah satu destinasi wisata terbaik di Asia — ==g:istana, BBQ, kehidupan malam, dan street food tak tertandingi.==
5. **Dengan paspor asing Anda bisa masuk** — turnamen ini dibuat untuk pemain internasional. Paspor Anda adalah kartu akses Anda.

Menuju APT Incheon? [Panduan turnamen vs cash game](/id/blog/holdem-tournament-vs-cash-game "thumb:/images/tournament-table-action.webp") membahas perbedaan strukturalnya kalau Anda beralih dari cash. Dan untuk format poker yang akan Anda temui di luar NLHE, [aksi taruhan dalam poker](/id/blog/holdem-betting-actions) adalah referensi rapi untuk dibaca sebelum Day 1.

---

## Sumber

Semua angka dicocokkan dengan sumber primer. **Diverifikasi: 17 Agustus 2026.**

- **Halaman resmi seri** — [APT — Incheon, South Korea 2026](https://www.theasianpokertour.com/series/apt-incheon-south-korea-2026) · tanggal, buy-in dan garansi Main Event #20, struktur flight
- **Preview resmi** — [APT Incheon 2026: The Preview](https://www.theasianpokertour.com/news/apt-incheon-2026-the-preview) · total garansi, starting stack 40.000, **sembilan kursi APTC Taipei dan pembagiannya**
- **APT Championship 2026** — [Full APTC 2026 Schedule](https://www.theasianpokertour.com/news/apt-announces-full-aptc2026-schedule) · 13–29 November, Taipei; Main Event 23–27 Nov, $10.000, garansi $5.000.000
- **Ulang tahun ke-20** — [APT Gets Ready to Celebrate 20 Years](https://www.theasianpokertour.com/news/apt-2026-schedule)
- **Satelit dan paket** — [Natural8 — Journey to APT Incheon 2026](https://www.natural8.com/en/blog/journey-to-apt-incheon-2026) · [Country Exclusives 2026](https://www.natural8.com/en/poker/tournaments/apt-incheon-country-exclusives-2026)
- **Field dan hasil 2025 (laporan resmi penyelenggara)** — [APT — China's Junjie He Wins APT's Largest Korean Main Event](https://www.theasianpokertour.com/news/china-s-junjie-he-wins-apt-s-largest-korean-main-event-for-krw-380-5-m-usd-275-k-plus-apt-championship-seat-worth-krw-13-8-m-usd-10-k) · 1.281 entri dari 754 pemain · prize pool KRW 2.766.960.000 · rincian per negara
- **Aturan masuk kasino** — [Paradise City resmi](https://www.p-city.com/front/contents/casino/casinoInfo)

> ⚠️ **Festival ini berakhir pada 16 Agustus 2026.** Jadwal, pendaftaran, dan info shuttle di atas adalah catatan edisi 2026 — cek halaman event resmi APT untuk ketentuan musim berikutnya.
>
> 📌 **Prinsip sumber:** jadwal, buy-in, jumlah kursi, dan aturan diambil dari penyelenggara (APT), mitra online (Natural8), dan pihak kasino. Media industri hanya dipakai untuk hal yang tidak dipublikasikan ketiganya. Ranking afiliasi tidak dikutip.

---

## Postingan Terkait

<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/id/blog/holdem-tournament" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Dasar Turnamen</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Cara Kerja Turnamen Poker</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Buy-in, blind, satelit & checklist pemula</div>
  </a>
  <a href="/id/blog/holdem-tournament-vs-cash-game" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Strategi</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Turnamen vs Cash Game</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Bagaimana strategi berubah saat ICM berlaku</div>
  </a>
  <a href="/id/blog/wpt-australia-2026-guide" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Asia-Pasifik</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Panduan WPT Australia 2026</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">The Star Sydney — 10–30 Sep, Championship AUD $5.000</div>
  </a>
  <a href="/id/blog/ept-barcelona-2026-guide" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Eropa</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Panduan EPT Barcelona 2026</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Casino Barcelona — 16–29 Agu, Main Event €5.300</div>
  </a>
</div>
`.trim(),
};
