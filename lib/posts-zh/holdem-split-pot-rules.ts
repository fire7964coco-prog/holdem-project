...
:::tip[如果公共牌通杀而有人在河牌下注，**反射性弃牌才是错的**。如果没有任何牌能盖过公共牌，平分就是确定的，跟注仍然会把底池里原有的筹码拿回一半——弃牌等于白白把这一半送人。如果公共牌还可能被盖过，就用频率来算：面对满池下注，需要对手同样只有公共牌的概率达到约三分之二；面对半池下注，约一半就够。这道门槛其实很高：面对满池下注，只要他握着真牌的比例**超过三分之一**，弃牌就是对的——而在公共牌仍有可能被盖过的河牌上，那才是常态。]:::
...
> **快速回答**
> 多出来的那枚==单数筹码==不会被撕成两半，也不看谁的牌更漂亮：它按座位归属。荷官先把它换成场上流通的最小面额，实在分不平时，这枚零头才按座位发出去。所以“平分”极少是分毫不差的对半。

有时底池没法均分——两人之间一个 101 枚筹码的底池，每人 50 枚，剩一枚，而且没有半枚筹码这回事。标准规则是：==剩下的那一枚归**按钮左手边**第一个平局玩家==，也就是从庄家顺时针数、第一个还没盖牌的赢家座位。
