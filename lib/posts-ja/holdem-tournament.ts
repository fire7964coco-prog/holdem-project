...
> **先に結論**
> ポーカーのトーナメントとは、参加者全員が同じバイインを払って同じ枚数のチップを受け取り、脱落していく中で最後まで残った順に賞金を分ける競技形式です。キャッシュゲームと決定的に違うのは、チップが現金ではなく「生き残るための持ち点」だという一点で、ここから戦略の違いがすべて派生します。

トーナメントとは、全員が同じ参加費(**バイイン**)を払い、同じ枚数のスタートチップを受け取り、誰か1人がテーブル上の全チップを持つまでプレーする競技です。

**一文でまとめると:** キャッシュゲームではチップがそのまま現金で、いつでも席を立てます。フリーズアウトのトーナメントなら、あなたの最大損失はバイインちょうど——リエントリーやアドオンがあるとそれを超えます——その代わり、はるかに大きなプライズプールの取り分を狙って戦うことになります。
...
> **先に結論**
> バイインは「$100+$9」のように二つに分かれ、前者が賞金の原資、後者が会場の手数料で、大型ライブなら手数料はおおむね8〜10%です。対価として受け取るスタートスタックは「何BBから始まるか」で読むのが正しく、レベル1で100〜300BBが標準。チップの枚数そのものに現金価値はありません。
...
その対価として受け取るのが**スタートスタック**——一般的に10,000〜50,000トーナメントチップで、レベル1では通常100〜300ビッグブラインド分の深さです——「何BBから始まるか」はこの数字で読みます。
...
> **先に結論**
> トーナメントのブラインドはレベル制で、ライブなら20〜40分ごとに段階的に上がります。レベルの長さが決めるのは「同じスタックが何BBの価値でいられる時間」で、短いほど早くプッシュ判断を迫られ、長いほどポストフロップの腕が効きます。中盤からはアンティが加わり、待つコストがさらに増えます。
...
==g:目安:20ビッグブラインドを切ったら「プッシュ・オア・フォールド(押すか降りるか)」の領域。10ビッグブラインドを切ったら——特にレイトポジションやスモールブラインドからは——ブラインドに食い尽くされる前に、ファーストインならプレーできる手はほぼすべてオールインすべきです(先行アクション・ポジション・実効スタック・ICMで調整)。==
...
> **先に結論**
> 4つのステージで変わるのは「何が判断を支配するか」です。序盤はハンドの質、中盤はアンティで膨らんだポットの価値、バブルは生き残りそのもの、ファイナルテーブルは順位ごとの賞金差。自分のスタックのBB数に加え、残り人数・入賞人数・賞金差を見れば今どのステージにいるかが分かり、それが打つべきレンジを決めます。
...
> **先に結論**
> 形式を分ける主な軸は二つ。「チップをなくしたら終わりか、買い直せるか」(フリーズアウト/リバイ・リエントリー)と、「賞金が順位だけで決まるか、脱落させた相手にも付くか、現金ではなく席か」(通常/バウンティ・PKO/サテライト)。この二軸で表の大半が読めます(ディープスタックやMTT/SNGは深さ・テーブル数で分ける別軸で、重なります)。

| 形式 | 仕組み | 向いている人 |
|:---|:---|:---|
| **フリーズアウト** | バイイン1回、リバイなし。飛んだら終わり。 | 初心者 — 費用が固定 |
| **リバイ / リエントリー** | 序盤のリバイ期間中にもう一度チップを買う——リバイでは多くの場合、飛ばなくても買える | バンクロールに余裕のある積極的なプレイヤー |
| **バウンティ / KO** | 誰かを飛ばすたびに固定額の賞金を獲得 | アクション派 — 1ノックアウトごとの追加収入 |
| **PKO(プログレッシブKO)** | ノックアウトごとに賞金が増える — 一部は現金で、一部は自分の頭に上乗せ | 分散が大きく、大きな上振れを狙う人 |
| **ディープスタック** | 同じシリーズの標準イベントよりスタートスタックが明確に大きく、レベルもゆっくり | ポストフロップのプレーを多くしたい人 |
| **サテライト** | 賞品は現金ではなく、より大きなトーナメントへの参加権 | 大型イベントを狙う予算重視の人 |
| **ターボ / ハイパーターボ** | ブラインドレベルが標準イベントよりずっと短く、スタックがすぐ浅くなってプッシュ/フォールドが早く来る | 短時間で遊びたい人 — シューブかフォールドの判断に慣れた人 |
| **ミステリーバウンティ** | バウンティ形式の一種で、ノックアウトごと(多くは所定のステージ以降)にランダムな賞金を引く — ほとんどは小額で、いくつかはジャックポット | 一発の大きな配当を狙う人 |
...
フリーズアウトのトーナメントでは、参加できるのは1人1回だけです。チップを失えば脱落——リバイもリエントリーもなし。これが最も古典的なトーナメント形式で、登録した瞬間に総費用が固定されるため、初心者に最適です。
...
ディープスタックのトーナメントは、同じシリーズの標準イベントよりブラインドに対してはるかに多いチップからスタートし、たいていブラインドレベルも長めに設定されています。**規格化された基準値はありません**——「ディープスタック」は常に相対的な呼び方です。ストラクチャーシートでレベル1のスタックが何ビッグブラインドに当たるかを計算し、標準イベントの100〜200BBと比べてください。チップが多くクロックがゆっくりということは、ポストフロップのプレーが増え、ミスから立て直す余地も増え、そして1日が長くなるということです。

**リバイとアドオンは?** リバイイベントでは、序盤の決められた期間中にもう一度チップを買えます——多くのイベントでは、スタックが開始スタック以下ならいつでも、飛ばなくても買えます;アドオンは、その期間が終わるときに一度だけできる任意のチップ購入です。それが終われば、あとはフリーズアウトと同じように進行します。
...
> **先に結論**
> サテライトは、勝っても現金ではなく上位大会の参加権(シート)がもらえる小規模トーナメントです。複数人に同額のシートを配る形式では、席の数に入れる見込みが立った瞬間から「勝つ」より「飛ばない」が正解になり(優勝者1人だけがシートを得る形式は別で、チップEVで優勝を目指します)、通常のトーナメントとは逆向きの判断が求められます。
...
**連鎖サテライト**はさらに安くなります。$5のスーパーサテライト → $55のクオリファイア → $215のイベント → $1,050のオンラインメインイベント。大型イベントの多くのプレイヤーが、直接バイインのほんの一部の費用でサテライトの連鎖から参加しています。

==g:サテライトの戦略は通常のトーナメントとは違います——複数シートを配る形式なら、シートを確保できるだけのチップがあれば、もうリスクを取るのをやめること。バブルで飛ばないように、良い手でさえフォールドします。==
...
> **先に結論**
> 参加ルートは三つ。会場の登録デスクで直接バイインする、大型イベントならオンラインで事前登録して列を飛ばす、サテライトでシートを勝ち取る。どれも身分証は必須です。開始後もレイトレジストレーション(途中参加)の期間中ならフルスタックで座れるので、時間に遅れても諦める必要はありません。
...
- イベントのプラットフォームでアカウントを作成(例:WSOPならWSOP LIVEアプリとCaesars Rewardsアカウントの両方、EPT/APPT系ならPokerStarsソフトのロビー内「Events」「Live」タブ)
- バイインをオンラインで支払う
- 会場に到着 → 本人確認 → キオスクでシートカードを印刷、またはデスクで受け取り
- 登録の列を飛ばせる——大型イベントではやる価値あり

### 方法C:サテライトクオリファイア
- サテライトをオンライン(PokerStars Power Path、GGPokerのWSOPサテライト)または現地で探す
...
> **先に結論**
> ステージごとに主役が入れ替わります。序盤はポジションとハンド選び、中盤はアンティが乗ったポットを盗む攻め、20BBを切ればプッシュ・オア・フォールドの算数、バブル以降は生き残りの価値。一つの戦略を貫くのではなく、スタックとステージに合わせて切り替えることがトーナメント攻略の骨格です。
...
> **先に結論**
> 初参加で驚くのは、開始から数時間はほとんど何も起きないのに、レイトレジが締まった途端にフィールドが減り始め、ディナーブレイク後は一気に緊張が高まることです。昼に座って夜に賞金圏、という10時間超の流れを前提に、体力と集中力の配分を組んでおくと終盤の判断が崩れません。

ここは、多くの初心者が痛い目に遭ってからしか学べない部分です。12時開始の$300フリーズアウト(ライブ)の、現実的なDay1のタイムラインです。
...
<div style="color:hsl(var(--foreground))">レイトレジ締切。最終フィールド数が発表。プライズプール確定。</div>
</div>
<div style="display:grid;grid-template-columns:80px 1fr;gap:12px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
...
> **先に結論**
> 賞金がもらえるのは通常フィールドの上位10〜15%で、この割合をインマネ率(ITM率)と呼びます。最下位の入賞はバイインが少し増えて戻る程度の額で、参加者が多い大会ほど賞金総額に占める優勝賞金の割合は一般に小さくなります。配分の基準は事前に確認できますが、確定額は登録締切後に出る場合があります。

**典型的な配分:** フィールドの上位10〜15%が賞金をもらいます——この割合がインマネ率(ITM率)です。
...
- 入賞:180人(フィールドの約12.5%)
- ミニマムキャッシュ:バイインの約1.83倍
- 1位:$662,200(プライズプールの約14%)

賞金配分の基準は事前に確認できます(確定表は登録締切後の主催者発表を確認)。登録時に**ストラクチャーシート**をもらいましょう——ブラインドレベル、アンティ、スタートスタック、賞金スケジュールが載っています。
...
> **先に結論**
> テーブルで判断を変えるのは、この中でも「バブル」「ハンド・フォー・ハンド」「レイトレジ」の三語です。前二つは賞金圏直前の進行を、最後は途中参加とリエントリーの締切を決めます。残りは知っていれば会話に困らない程度なので、まずこの三つの意味と自分への影響を押さえてください。

この16語で、テーブルで耳にすることのほとんどをカバーできます。AからZまでの完全版は、[ポーカー用語集](/ja/blog/holdem-glossary)をご覧ください。
...
| **レイトレジ** | レイトレジストレーション(途中参加)期間 — トーナメント開始後でも参加できる |
| **リエントリー** | 飛んだあとに再度バイインすること(レイトレジ期間中のみ) |
| **サテライト** | 賞品がより大きなイベントのシートであるクオリファイア |
| **PKO** | プログレッシブノックアウト — 賞金が増えていくバウンティ系トーナメント |
| **ミステリーバウンティ** | ノックアウトごとの賞金が抽選で決まるバウンティ形式 |
| **ターボ** | ブラインドレベルがずっと短い構造 — ハイパーターボはさらに短い |
| **アドオン** | リバイ期間の終わりに全員へ一度だけ提供される追加チップの購入 — スタック量は問わない |
...
> **先に結論**
> 抜けて困る順に言えば、身分証がなければ登録そのものができず、現金が足りなければカード不可の会場で席を失い、ストラクチャーシートがなければアンティと休憩の時間を読み違えます。到着は開始30〜45分前が目安で、着席したらまずチップを数える——ここまでが「参加の前半戦」です。
...
**Q. ポーカーのトーナメントのルールはキャッシュゲームと違いますか?**

A. カードの配り方や役の強さ、ベットの順番といったゲーム自体のルールは同じです。違うのはゲームを囲む枠組みで、チップに現金価値がないこと、ブラインドが時間ごとに上がること、チップを持ったまま途中で席を立てないこと、そして賞金が最終順位で決まることの4点です。細かい比較は[トーナメントとキャッシュゲームの違い](/ja/blog/holdem-tournament-vs-cash-game)にまとめています。
...
A. リバイトーナメントでは、決められたリバイ期間中にもう一度チップを買えます——多くのイベントでは、スタックが開始スタック以下ならいつでも、飛ばなくても買えます——ただし通常は最初の数ブラインドレベルだけです。アドオンは一度だけできる任意のチップ購入で、たいていリバイ期間の終わりにスタックの大小に関係なく全員に提供されます。その期間が終われば、あとはフリーズアウトとして進行します。正確なルールは会場によって異なるので、ストラクチャーシートで確認してください。
...
A. 地域の法律によります。これを一言で片づける目安はありません——線引きは現地の法令にあり、ひとつのテストで判定できると書いているガイドは、あなたの代わりに推測しているだけです。法律は国や州で大きく異なるので、開催前に地域の規制を確認してください。

**Q. ポーカーのITMとは何ですか?**

A. ITM =「イン・ザ・マネー」、日本では「インマネ」と略します。賞金の支払いが確定する順位に到達したことです。200人のトーナメントで25位まで賞金が出るなら、175人が脱落して25人が残った時点であなたはITMです。ミニマムキャッシュはたいていバイインの1.5〜2倍です。

**Q. トーナメントに途中参加(レイトレジストレーション)はできますか?**
...
A. いいえ。キャッシュゲームと違い、トーナメントのチップに現金価値はなく、途中で換金することはできません。もし席を離れても、あなたのチップはプレー中のまま残り、なくなるまでブラインドとアンティを払い続けます。通常の順位賞金は、賞金圏(ITM)の順位でフィニッシュしたときに受け取ります(KO・PKOでは順位賞金とは別にノックアウト報酬を得ることがあります)。
