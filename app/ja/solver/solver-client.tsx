...
const SPEC: [string, string][] = [
  ...
  ["アカウント", "不要（ログインで学習スポット・今日の問題の履歴を同期 · 自分で計算したスポットと練習履歴はログインしてもこの端末だけ）"],
  ...
  // 🔴 2026-09-06 갱신 — 앱은 **12개 언어**다(`solver/src/i18n.ts:18` `Locale` 축어 · 라이브 12/12 고유 문안 확인).
  //    **«언어 셀렉터»는 2026-08-27에 제거됐다**(본진 언어 페깅 · `?lang=`만 받는다) → 종전 「…に切替可」는 사실이 아니게 됐다.
  //    그래서 형제 랜딩(de·es·pt·zh·zh-hant)과 같은 «다른 언어판이 있다» 형태로 정렬했다(문안 이식 · 창작 0).
...
const FEATURES: [string, "yes" | "no", string][] = [
  ...
  ["ノードロック後の再計算", "yes", "特定ノードのアクション頻度を自分で決めて解き直せます"],
  ...
  ["自分で計算したスポットで練習", "yes", "結果画面のワンクリックで問題として保存 · 問題と練習履歴はログインしてもこの端末だけ"],
];
