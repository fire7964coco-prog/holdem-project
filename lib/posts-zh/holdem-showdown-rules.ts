...
> **快速回答**
> 有人全押（all-in）、后续不可能再有下注时，**所有底牌都翻开**，通常在发完剩余公共牌之前或期间——全押局面里谁都不该有机会策略性盖牌。河牌圈的全押被跟注，全押者作为最后进攻者先亮，锦标赛里跟注方也必须摊牌。边池先结算只是派彩顺序，不影响亮牌时机。
...
一个细节：如果存在**边池**（其他玩家还有筹码、继续下注），先结算边池，再结算主池。这只是**派彩顺序**，跟什么时候亮牌是两回事：全押玩家的牌，在不可能再有下注的那一刻就已经摊开了。TDA 第 16 条的图解说得很直白——不要等边池分完了才去翻全押玩家的牌。
...
在摊牌阶段——也就是所有下注都结束之后——没有任何规则禁止你在正式轮到自己之前就把牌翻开。但只要这手牌还在进行、还有动作没走完，情况就完全相反：亮牌要按 ==WSOP 第 117 条== 受罚。==g:如果你拿着坚果牌或超强牌，立刻亮出来。==其他玩家会领情，牌局也更快。这正是 slow roll 的反面。
...
A. 必须——如果你是河牌圈最后一个下注或加注的人，被跟注后你要先亮牌。如果你是跟注方，看过对方的牌后确认自己输了，可以牌面朝下盖掉。例外是锦标赛中的全押：按 TDA 第 16 条，跟注方同样必须摊牌。而在锦标赛里，只有手上仍有牌或已经摊牌的人才能要求看牌——不可剥夺的，只限最后进攻者那一手。
...
A. 当有玩家全押且后续不可能再有下注时，该底池涉及的所有底牌通常会在发完剩余公共牌之前或期间翻开亮出。如果存在边池，先结算边池、再结算主池——但那只是派彩顺序，全押玩家的牌早在那之前就已经翻开了。至于其他还有筹码、还能继续下注的玩家，在下注走完之前，他们的牌都还扣着。

**Q. 扑克里的 "cards speak" 是什么意思？**

A. Cards speak（牌说话，以牌面为准）的意思是：胜负由牌面实际构成决定，而不是由玩家嘴上说的决定。看错自己的牌、报错牌型的玩家，只要实际牌面是最好的牌，照样获胜。反过来，没有确认输赢就盖牌的玩家对底池没有任何主张权，哪怕他的牌本来能赢。
