...
const HAND_TABLE: TierEntry[] = [
  ...
  [4, "66",  "셋 드로우 목적, 임플라이드 오즈 필요", "LP에서 첫 진입이면 레이즈, 레이즈 앞에선 콜 금액의 15배 이상이 뒤에 남았을 때 콜"],
  [4, "55",  "셋 아니면 가치 낮음", "LP에서 첫 진입이면 레이즈, LP에서 콜 금액의 15배 이상이 뒤에 남았을 때 싱글 레이즈에 콜"],
  ...
  //    🔴 배지만 올렸다 — 액션 문구는 그대로다.
  [4, "44",  "셋 없으면 가치 거의 없음", "LP에서 첫 진입이면 레이즈, LP에서 콜 금액의 15배 이상이 뒤에 남았을 때 싱글 레이즈에 콜"],
...
// BTN 50% 레인지는 663콤보다: 페어 78 + 수티드 312를 빼면 오프수트 몫이 273콤보 = 약 22타입 —
// 에이스 12 + K9o+ 4 + Q9o+ 3 + J9o+ 2 + 10-9o. 아래 오프수트 줄이 그 선이다.
