...
==是的，撲克也「算牌」——只是不是算那副牌本身。你算的是補牌、阻擋牌，還有已出現的死牌，而且這全部都完全合法。== 這篇會講清楚：為什麼 21 點那套一坐上撲克桌就失效、撲克版的算牌到底是什麼、這些做法犯不犯規，還有老派算牌真的管用的那一系撲克玩法。
...
> **快速解答**
> 能，也不能——你沒辦法像 21 點那樣算整副牌，但你絕對可以算補牌、抓阻擋牌、追蹤死牌，而且這全部合法。卡住多數人的是「算牌」被當成一個詞在用：==記整副牌的高低平衡==在撲克裡給你零優勢，==算你自己這手牌的機率==卻是每個贏家每一手都在做的事。
...
> **快速解答**
> 21 點算牌能成立，靠的是三個條件：一副牌靴連打好幾十手、看得到的牌夠多、對面是規則固定的莊家。==撲克把這三個全部打破了==——所以問題不是你算得不夠準，是這裡根本沒有東西可以累積。

為什麼搬不過來，說白了就這三點：

:::card
🔀 | 每一手都重新洗牌 | 21 點算牌需要一副牌靴連發幾十手，讓資訊一手一手累積。撲克每一手都重新洗，什麼都不會延續——每一手都從一副完整、隨機的牌重新開始
🙈 | 露出的牌太少 | 每個玩家的底牌都是蓋著的。你看得到的只有自己的兩張底牌、公共牌，以及攤牌時亮出來的牌——就那麼幾張——根本不夠讓你追蹤整副牌的組成
👥 | 你打的是對手，不是莊家 | 沒有一個規則固定的莊家讓你去佔便宜。「一副高牌很多的牌」在這裡毫無意義——不管牌怎麼分，一對 A 就是一對 A。你靠的是有更好的牌、或做出更好的決策，而不是靠一個對你有利的 count
:::

在 21 點裡，一副高牌偏多的牌在數學上對你有利，所以 count 好的時候你就下大注。撲克裡根本沒有等價的「有利的牌」——你的優勢來自打==對手==，以及你此刻看得到的牌：補牌、阻擋牌、牌面。
...
> **快速解答**
> 兩種遊戲要的資訊完全不同：21 點要的是==已經走掉的牌==，撲克要的是==此刻攤在檯面上的牌==。一個往回看，一個往前看——方法跨不過來，是因為連要記的東西都不是同一種。
...
==補牌（out）== 就是任何一張還沒現身、能把你的牌升級成大概會贏的牌。一個同花聽牌有 ==9 張補牌==（同花色 13 張，扣掉你已經看到的 4 張）——牌面上那些同花色的牌已經算在這 9 張裡扣掉了，別再把它們當成死牌扣第二次。用 ==四二法則（Rule of 4 and 2）== 把補牌換成一個粗估勝率：還剩兩張牌要發時乘以 4，只剩一張時乘以 2。

一個 9 張補牌的同花聽牌，到河牌大約有 ==g:35%== 的機率成牌（9 × 4 = 36% 是快速估算——真實值是 35.0%）。這個數字把剩下兩張牌都算進去了，所以它能決定一次跟注，只在你真的兩張都看得到的時候——後面不會再有下注，例如你已經全下，或你跟了別人的全下。要是翻牌圈面前有一注、轉牌還得再付一次，就只算下一張牌：==9 ÷ 47 = 19.1%==。完整的方法——髒補牌、複合聽牌、精確百分比——都在[算補牌完全教學](/zh-hant/blog/holdem-outs)裡，而每一種聽牌背後的機率則收在[德州撲克機率與勝率表](/zh-hant/blog/holdem-probability "thumb:/images/holdem-probability-hero.webp")。
...
阻擋牌也能==部分==發揮作用。在一個 ==b:Q-J-9== 的牌面上，堅果順子是 K-10。K-10 正常有 16 種組法（4 張 K × 4 張 10）；如果 K 或 10 有一張正握在你自己手裡，就把它砍到 ==12 種組合==，於是對手範圍裡的堅果順子組合就少了 25%。這正是現代詐唬選牌的核心——更多細節在 [3bet 與阻擋牌指南](/zh-hant/blog/holdem-3bet) 和[讀牌面與範圍思維](/zh-hant/blog/holdem-reading-the-board)裡。

### 移除效應與死牌（Card Removal）

你看得到的每一張牌，都在移除一些可能性。在德州撲克裡，補牌不可能躺在牌面上——真躺在那裡，你的牌早就成了——所以要追蹤的==死牌==是那些在牌面**之外**露出來的：不小心翻出來的一張、進蓋牌堆前被亮出來的一手、你剛好瞄到的鄰座棄牌。其中每一張補牌，都是你已經不再擁有的補牌；其他露出的牌，則只是讓沒看見的牌堆變小。好手就是在每一條街上不停地做這種安靜的微調。這就是算牌，只是不需要維持一個 running total 的那種。
...
> **快速解答**
> 合法——在腦子裡算補牌、算賠率、用阻擋牌，是一般的撲克技術，不是外來協助。真正要留意的界線，是牌局進行中的電子設備和別人給的建議，這部分由各撲克室或賽事自己規定。

拿 21 點來比，差別在於你對上的是誰。在撲克桌上，你競爭的對象是==其他玩家==；撲克室收的是開局的費用——也就是[抽水（Rake）](/zh-hant/blog/holdem-glossary)——而不是下場跟你對賭。在腦子裡算補牌本來就是這個遊戲的一部分，光憑這一點，並不會讓你被當成 21 點那種算牌者。

那條界線劃在「腦子」和「工具」之間。採用 [2026 年版 Poker TDA 規則](https://www.pokertda.com/poker-tda-rules/)的錦標賽裡，第 5C 條禁止手上還有活牌時操作電子或通訊設備；第 5D 條更進一步：下注 App、圖表和其他策略工具都不能在牌桌上使用，外來的策略資料也不被允許（現金桌依各場館規定）。規則擋的是工具，不是心算——所以同一套機率，記在腦子裡完全合法，開在手機上就不行。工具留到牌桌外研究，桌上的決定自己做。

:::tip[把腦子裡的算術，和做記號的牌、串通、私下交換底牌資訊分開來看。線上軟體另有各平台自己的規定：例如 [PokerStars 的工具政策](https://www.pokerstars.com/poker/room/prohibited/)禁止即時的行動建議，並限制在客戶端開著時使用解算器（solver）。先查清楚你所在的平台允許什麼，別把每一種工具都當成跟心算一樣。]:::

---

## 能老派算牌的那一系撲克玩法：七張梭哈 (Seven Card Stud)

> **快速解答**
> 在七張梭哈裡，每個玩家有一大部分的牌是攤開發出來的——所以你真的可以用老派方式去算那副牌。

如果你需要某張特定的牌來完成你的手牌，你可以環視整桌，實實在在地數出你的補牌裡有幾張已經出現在對手的明牌上了。每數到一張，就是一張死掉的補牌。

在德州撲克裡，唯一會正面發出的牌就是那五張公共牌——其他牌都蓋著，除非在攤牌時亮出、在全下時翻開，或是意外露出（不小心翻出的牌、棄牌時被亮出的一手），所以幾乎沒有東西可以追蹤。但梭哈——以及同樣發明牌的梭哈系玩法，像 Razz 和梭哈高低分——獎勵的，正是 21 點算牌者最擅長的那種記牌本領。這是撲克裡最接近電影版的一種玩法。
...
這也連到一個進階觀念——**阻擋牌要看你擋掉的是對手「會跟」的牌，還是「會蓋」的牌。** 擋掉他會跟你的那些強牌（例如你握著 A♠，他就湊不出堅果同花），你的詐唬就更容易成功；反過來，如果你手上那張牌擋掉的是他==本來就會蓋==的牌——比如他沒中的聽牌——那你逼他蓋牌的空間反而變小了。把阻擋牌用在「該不該詐唬」的判斷上，而不是無腦當成詐唬許可證，這才是進階牌手和背公式的人之間的差距。想把這套範圍思維打底，先讀[起手牌範圍表](/zh-hant/blog/holdem-starting-hands-chart)。
...
> **快速解答**
> 你不需要一套系統——三個習慣就夠，而且全在腦子裡完成。它們的==順序不能顛倒==：先數，再問自己擋掉了什麼，最後才扣掉你看到的死牌。倒過來做，你會拿一個還沒扣乾淨的數字去做決定。

:::steps
每個聽牌都算補牌 | 一有聽牌，就數出能讓它成牌的那些牌，然後乘一乘——×4 只在兩張牌都會發出來時用（你已經全下，或者後面沒有下注），否則就只算下一張牌 ×2。當這個機率（只算乾淨補牌）打過那個價錢，或隱含賠率補得上差距時就跟
先問這手牌擋掉了什麼 | 詐唬之前，先檢查你手裡有沒有一張牌，讓對手最強的那個跟注牌型變不可能、或變得更不可能
按你看到的死牌做調整 | 扣掉任何你在牌面之外看到的補牌——不小心翻出來的一張、被亮出來的一手、剛好瞄到的鄰座棄牌。你看得到的牌，就是對手不可能有的牌——但只限剛好瞄到的：故意去看別人的底牌不屬於這套方法——只算意外露出的牌
:::
...
A. 合法。在腦子裡算你自己的補牌和阻擋牌是合法的，也是打撲克的正常一環。撲克室和平台限制的，是牌局進行中的外來協助：電子設備、圖表，以及別人的建議。

**Q. 算牌在德州撲克裡有用嗎？**

A. 21 點那種算整副牌的方式沒用——牌每一手都重洗，露出的牌又太少。但撲克版的算牌在德州撲克裡絕對管用：算你的補牌、抓阻擋牌、按你看到的死牌做調整，都是不可或缺的本領。

**Q. 為什麼算牌在 21 點有用、在撲克卻沒用？**

A. 21 點是你對上一個規則固定的莊家、用一副牌靴連打好幾手，所以一副高牌偏多的牌在數學上對你有利，你就照這個下注。撲克每一手都重洗、而且你面對的是其他玩家，所以沒有「有利的牌」可記——優勢來自讀對手，以及你看得到的牌：補牌、阻擋牌、牌面。

**Q. 撲克裡對應算牌的東西是什麼？**

A. 算補牌（能升級你手牌的牌）、用阻擋牌（你手裡那些能減少對手組合的牌），還有追蹤死牌（你已經看著它離開牌局的補牌——不小心翻出來的一張、棄牌時被亮出來的一手）。三者合起來，讓你讀出接下來可能來什麼、以及對手不可能有什麼。

**Q. 七張梭哈可以算牌嗎？**

A. 可以，而且比德州撲克能算得多。在梭哈裡，每個玩家有好幾張牌是攤開發出來的，所以你可以環視整桌，數出你的補牌裡有幾張已經現身了。那是在同一手牌之內追蹤死牌的算牌，在梭哈裡是實實在在的優勢。

**Q. 算牌會害我被撲克室趕出去嗎？**

A. 不會——只要你是在腦子裡算自己的補牌、用阻擋牌。這些是對上其他玩家的遊戲裡的正常技術，而且不管誰贏，撲克室都照樣抽水；這跟衝著莊家去的 21 點算牌不一樣。
...
3. **它是技術，不是秘密。** 自己動腦算，外部工具留到牌桌外研究。算你的補牌、問你擋掉了什麼、扣掉你已經看到的死牌——每一手都做。
