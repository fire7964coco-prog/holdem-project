...
### 🗓️ 대회 종료 후 반드시 할 일 (날짜 예약)

Event 구조화 데이터(JSON-LD)를 내보내는 곳이 **두 군데**다. 대회가 끝나면 "지난 행사"가 되므로 종료 처리·결과 아카이브 전환이 필요하다.

| 대회 | 종료일 | 손봐야 할 곳 |
|---|---|---|
| **APT 인천 2026** | **2026-08-16** | `lib/posts/apt-incheon-2026-guide.ts`의 `event:{}` + `app/tournaments/tournaments-client.tsx`의 `eventSchemas` 2번째 |
| **APPT 코리아 2026** | **2026-09-14** | `app/tournaments/tournaments-client.tsx`의 `eventSchemas` 3번째 (블로그 글엔 `event:{}` 없음) |
| WSOP 2026 | 브레이슬릿 7/15 종료 / **ME 파이널 8/3~5** | ✅ 2026-07-28 처리 완료. **8/5 이후 다시 손봐야 함** — 우승자 확정되면 결과 반영 |
