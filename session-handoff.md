...
| 순환 | ✅ (09-16 추가 지시) 홈 피드·/blog 둘 다 마지막 카드 다음에 1번부터 다시 이어짐(클라이언트 전용 · 초기 HTML엔 카드 1회 · 이음새에 «처음부터 다시»+정책 링크). 5단계 캡처 때 이음새·중복 키·모바일도 같이 본다 | `community-client.tsx` cycles · `blog-index-client.tsx` loop() |
