...
==g:目安:20ビッグブラインドを切ったら「プッシュ・オア・フォールド(押すか降りるか)」の領域。10ビッグブラインドを切ったら——特にレイトポジションやスモールブラインドからは——ブラインドに食い尽くされる前に、プレーできる手はほぼすべてオールインすべきです。==
...
> **先に結論**
> 4つのステージで変わるのは「何が判断を支配するか」です。序盤はハンドの質、中盤はアンティで膨らんだポットの価値、バブルは生き残りそのもの、ファイナルテーブルは順位ごとの賞金差。自分のスタックのBB数を見れば今どのステージにいるかが分かり、それが打つべきレンジを決めます。
...
> **先に結論**
> 形式を分ける軸は二つだけです。「チップをなくしたら終わりか、買い直せるか」(フリーズアウト/リバイ・リエントリー)と、「賞金が順位だけで決まるか、脱落させた相手にも付くか、そもそも現金ではなく席か」(通常/バウンティ・PKO/サテライト)。この二軸を押さえれば、表のどの行も自分で読み解けます。
...
> **先に結論**
> サテライトは、勝っても現金ではなく上位大会の参加権(シート)がもらえる小規模トーナメントです。シートの価値はどれも同じなので、席の数に入れる見込みが立った瞬間から「勝つ」より「飛ばない」が正解になり、通常のトーナメントとは逆向きの判断が求められます。
...
> **先に結論**
> 賞金がもらえるのは通常フィールドの上位10〜15%で、この割合をインマネ率(ITM率)と呼びます。最下位の入賞はバイインが少し増えて戻る程度の額で、1位の取り分はフィールドが大きいほど薄く広く分配されます。正確な配分は開始前にストラクチャーシートで必ず公開されます。
