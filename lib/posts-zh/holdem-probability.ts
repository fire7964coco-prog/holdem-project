...
所以下次有人说"我从来发不到 A"，他说得大致没错——你被发到*特定*的一对（比如口袋 A）平均只有 ==大约每 221 手一次==。但**任意**口袋对每 17 手就来一次，这就是为什么 set-mine 是真实可行的打法，不是幻想。哪些对子和同花手值得从哪个座位打，可以看按位置排的起手牌图表。
...
扑克奖励那些把这些变成本能的人。学会这张表，练熟 2 和 4 法则，开始在行动*之前*就问"概率多少？"，而不是行动之后。接下来，把数学用起来，学学哪些起手牌该从每个位置打，或者温习一下 [为什么同花大过顺子](/zh/blog/holdem-flush-vs-straight)，这样你永远清楚自己的 outs 到底值多少。
