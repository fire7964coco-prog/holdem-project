...
export const POST: Post = {
  ...
  tags: [
    ...
    "side pot poker",
    "apa itu side pot",
    "aturan re-raise setelah all in",
  ],
...
Saya pernah ada di posisi itu. Pertama kali saya all-in di cash game live, saya tidak tahu apakah saya masih bisa menang, apakah pemain lain boleh re-raise, bahkan tumpukan chip mana yang jadi milik saya. Tidak ada yang menjelaskan.
...
**2. Dorong semua chip ke depan** — Geser seluruh stack Anda ke tengah dalam satu gerakan bersih. Mendorong chip sebagian-sebagian bisa terlihat seperti string bet, jadi pindahkan semuanya sekaligus. ==r:Mendorong saja tidak selalu cukup: kalau Anda menghadapi bet dan setiap chip Anda dibutuhkan hanya untuk call, dorongan tanpa suara dinilai sebagai call, bukan all-in (TDA 2024 Rule 45-A, WSOP Tournament Rule 92).== Di luar itu, mendorong chip terakhir Anda **memang** all-in (TDA 2024 Rule 45-B) — satu-satunya pengecualian adalah satu chip terakhir berukuran besar yang didorong tanpa suara ke sebuah bet, yang hanya dihitung call (TDA 2024 Rule 44).
...
| Pemain | Stack | Aksi |
|--------|-------|--------|
| Pemain A | 100 chip | All-in |
| Pemain B | 300 chip | Call 100, lalu bet 50 lagi |
| Pemain C | 300 chip | Call 100, lalu call 50 |

**Pot utama:** 100 × 3 = **300 chip** (A, B, dan C semuanya berhak)
...
Di sinilah semuanya jadi rumit — dan di sinilah kebanyakan pemula tersesat.

| Pemain | Stack | All-in sebesar |
|:---|:---:|:---:|
| A | 100 | 100 |
| B | 200 | 200 |
| C | 500 | 500 |
...
| Side pot 2 | 300 × 2 = **600** | C, D (A dan B dibatasi) |
| **Total** | **1.300** | — |

</div>

Aturannya: ==setiap side pot dibentuk dengan mengambil selisih sampai stack terkecil berikutnya × jumlah pemain yang menyamainya.== Hitung dari stack terkecil ke yang terbesar.
...
Kenapa aturan ini ada? Untuk melindungi pemain agar tidak dipaksa masuk ke raise yang lebih besar gara-gara all-in parsial. Raise penuh menandakan agresi sungguhan — all-in receh dari stack pendek tidak.

### Kasus Lanjutan: Bagaimana Kalau Beberapa Pemain All-In Pendek?

Ini versi yang bikin tersandung bahkan pemain reguler. Beberapa all-in pendek bisa **terakumulasi** menjadi satu raise penuh — dan kalau gabungan inkremen-nya mencapai ambang, taruhan terbuka kembali bagi seorang pemain yang sudah beraksi. ==r:Ujiannya dilakukan pemain per pemain, bukan sekali untuk seluruh meja:== taruhan hanya terbuka kembali bagi pemain yang, **saat aksi kembali kepadanya, menghadapi setidaknya satu raise penuh di atas yang sudah ia masukkan** (==TDA 2024 Rule 47==).

Ini aturan resmi TDA soal "re-opening the bet", dan mayoritas card room mengikutinya.

**Contoh (blinds $1/$2, di flop):**

1. Pemain A bet $10.
2. Pemain B all-in sebesar **$14** (inkremen +$4 — sendirian bukan raise penuh)
3. Pemain C all-in sebesar **$21** (inkremen +$7 — sendirian bukan raise penuh)

Gabungan inkremen: $4 + $7 = **$11** — itu memenuhi ambang raise minimum $10.

**Hasilnya: taruhan TERBUKA KEMBALI untuk Pemain A.** A memasukkan $10 dan kini menghadapi $21 — $11 lebih banyak, setidaknya satu raise penuh — jadi A boleh fold, call, atau re-raise, meskipun B maupun C secara individual tidak membuat raise penuh. Pemain yang sempat call $14 dari B di tengah jalan hanya akan menghadapi $7 lebih saat gilirannya kembali, dan baginya tidak ada yang terbuka kembali: call atau fold.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| All-in B | All-in C | Gabungan inkremen | Terbuka kembali untuk A? |
|:---|:---:|:---|:---|
...
Ambang raise minimum selalu berdasarkan *bet atau raise penuh terakhir yang sah* — bukan total kumulatif apa pun.
...
|---|---|
| Satu all-in < raise penuh | ❌ Tidak — hanya call atau fold |
...
==g:Satu pemain bisa memenangkan pot utama tapi kalah di side pot. Dua-duanya hasil yang sah.==

**Kasus khusus:** kalau di satu side pot hanya tersisa satu pemain (semua yang lain fold), pemain itu langsung mengambil kembali chip tersebut — untuk pot itu tidak perlu showdown.

---

## Apa yang Terjadi Kalau Anda All-In dengan Salah? — 5 Kesalahan yang Harus Dihindari

Setelah cukup sering terlibat pot all-in, Anda akan sadar kekacauannya hampir selalu berasal dari lima kesalahpahaman yang itu-itu saja — bukan soal hitungannya, tapi soal aturan di sekelilingnya, terutama siapa berhak atas pot yang mana.

### Kesalahan 1: Mengira pemain all-in bisa memenangkan side pot

Tidak bisa. Begitu pemain all-in dibatasi, chip tambahan apa pun yang dipertaruhkan stack yang lebih besar masuk ke pot yang sama sekali bukan haknya.
...
Anda all-in untuk pot utama. Dua pemain lain bertarung memperebutkan side pot. Di turnamen, ini selesai sendiri — begitu taruhan mereka berakhir, ==TDA 2024 Rule 16== mewajibkan semua hand dibuka, termasuk milik Anda. Di cash game tidak: saya pernah melihat seorang short stack melempar kartunya begitu showdown side pot berbalik melawannya — lupa bahwa dia bahkan tidak ikut di pot itu, dan pot utama masih bisa ia menangkan. Begitu dealer menyapunya ke dalam muck, kartu itu tak lagi bisa diidentifikasi — mati, dan pot utama jatuh ke pemain lain. (Kartu yang masih bisa dikenali dengan jelas kadang boleh diambil kembali atas kebijakan floor, tapi jangan pernah mengandalkan itu.) Jangan muck — kartu Anda masih hidup untuk pot utama. ==Selalu tunggu dealer menyelesaikan setiap pot sebelum menyentuh kartu Anda.==

### Kesalahan 5: All-in karena frustrasi

All-in adalah gerakan paling kuat di meja. Ia memaksa lawan mengambil keputusan hidup-mati. Kekuatan itu hilang kalau Anda asal dorong. Pakai di momen yang tepat — tekanan stack pendek, kartu value yang Anda ingin di-call, atau gertakan dengan fold equity sungguhan.
...
/id/blog/holdem-showdown-rules | Aturan Showdown Dijelaskan Tuntas | /images/holdem-showdown-rules-hero.webp
:::
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/id/blog/texas-holdem-rules-for-beginners" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Panduan Pilar</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Aturan Texas Hold'em untuk Pemula</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Aturan lengkap dari blinds sampai showdown</div>
  </a>
  <a href="/id/blog/holdem-split-pot-rules" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Aturan Split Pot dan Chop</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Kapan pot dibagi rata dan kenapa</div>
  </a>
  <a href="/id/blog/holdem-showdown-rules" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Aturan Showdown</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Siapa yang membuka duluan dan kapan Anda boleh muck</div>
  </a>
</div>
`.trim(),
};
