...
import { clusterForSlug, EN_CLUSTERS, JA_CLUSTERS, ES_CLUSTERS, PT_CLUSTERS, DE_CLUSTERS, ZH_CLUSTERS, type PillarCluster } from "@/lib/pillar-clusters";
...
export default function IntlBlogPostClient({
  ...
  // 클러스터 미니맵: 전 필라를 완역한 로케일만(en·ja·es·pt·de·zh). 라벨은 로케일별 클러스터, UI라벨은 EN 유지.
  const localeClusters: PillarCluster[] | null =
    locale === "en" ? EN_CLUSTERS : locale === "ja" ? JA_CLUSTERS : locale === "es" ? ES_CLUSTERS : locale === "pt" ? PT_CLUSTERS : locale === "de" ? DE_CLUSTERS : locale === "zh" ? ZH_CLUSTERS : null;
