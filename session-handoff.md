...
| **C-2** | 구형 워드마크 잔여 13장 | 인포 11 + 실사 2 | 🟠 미결 표 참조(둘 다 **재제작/재생성**이라 별건). 🔴 **08-31 회차로 줄지 않았다** — 신규 17장은 신형 워드마크로 나갔지만 **옛 파일을 지운 게 아니라 새로 추가한 것**이고, 옛 파일은 다국어가 그대로 쓴다(`rules-texas-holdem` 전체 145파일). 커밋 `ac6f0acd` 메시지의 「C-2 상당 부분 해소」는 **오기이므로 인용하지 마라** |
