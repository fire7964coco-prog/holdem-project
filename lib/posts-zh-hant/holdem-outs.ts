...
export const POST: Post = {
  ...
  tldr: "補牌就是牌堆裡還能把你這手變成大概會贏的牌。先數清楚，再換算：翻牌圈補牌 × 4、轉牌圈 × 2，就是你成牌的大概機率。同花聽牌是 9 張補牌，到河牌約 35%。",
  category: "odds",
  date: "2026-07-23",
  updated: "2026-09-26",
  ...
  content: `
我打牌的第一年，一直「靠聽牌在打」，卻從來沒真的數過補牌。同花聽牌跟卡順聽牌在我眼裡感覺差不多——反正都是「還可能來的牌」——所以兩種我都下一樣的注，然後納悶自己怎麼一直輸。真正救我的不是什麼策略課，而是一個五分鐘就能養成的習慣：==停一下，把真正能救我的牌，一張一張數出來。==
...
> **快速解答**
> 補牌就是牌堆裡任何一張，能把你這手變成==大概會贏==的牌。多數人第一次數就錯，錯在把「能讓牌變好的牌」通通算了進去——標準不是變好，是會贏。這一節只做「數」這個動作，換算留到後面。

那個「大概會贏」四個字是有分量的。真正的補牌得要*真的贏下*這一手，而不只是把牌變好一點。牌面上已經有三張同花、對手又真的拿著同花時，你配對了自己的 10，那不是補牌——你的牌是進步了，但你還是輸。學會數補牌，其實就是學會只數「能贏的牌」，並且把那些只是*看起來*有用的牌全部忽略掉。
...
> **快速解答**
> 前兩步數出來的是==原始補牌==——同花聽牌永遠是 9 張，跟對手拿什麼無關。第三步才換一個問題：這 9 張真的每一張都讓你贏嗎？牌面成對、或你追的不是堅果同花時，能算數的就不到 9 張。那一步剔掉的不是牌，是你對那幾張牌的信心。
...
數補牌只用你看得見的牌。你不會去扣對手那兩張看不見的牌；每一張沒露出來的牌，都當成還活在牌堆裡。這也是為什麼下面那些原始補牌數，不管別人手上拿什麼都一樣——它們是起點，後面還要再劃掉髒補牌。
...
> **快速解答**
> 這張表不用整張背，記住==同花 9、兩頭順 8==這兩個錨點就夠；其餘（高張、口袋對、卡順）都是直接數點數——每個點數 4 張，扣掉你看得見的。真正該花時間的不是背數字，是認出自己手上到底是哪一種聽牌。

![兩種補牌數法並排——13 張黑桃裡劃掉 4 張，旁邊是大大的 9；右邊是兩端標了記號的四張連牌和大大的 8](/images/holdem-outs-nine-and-eight.webp "左邊是同花聽牌，右邊是兩頭順聽牌——其他聽牌都拿這兩個數來比")

這些是原始補牌數，先拿來當起點，再用下面的髒補牌檢查去打折：

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| 你的聽牌 | 補牌 | 為什麼 |
|:---|:---:|:---|
| 同花 + 兩頭順 | 15 | 9 張同花 + 8 張兩頭順 − 2 張重複——怪物 |
| 同花 + 卡順 | 12 | 9 張同花 + 4 張卡順 − 1 張重複 |
...
> **快速解答**
> 實戰上先看「翻牌 → 轉牌」那一欄：只算下一張牌的數字，才是你面對這一條街的下注時真正買到的。「到河牌」那一欄要你兩張牌都看得到才成立。==多數跟注買到的只有一張牌==，所以拿錯欄，你會把自己的勝率高估將近一倍。

補牌要換成一個你能拿去跟價錢比的百分比，才真的有用。這是那張主表——到河牌成牌的機率，外加從翻牌圈只看下一張牌的機率：

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| 補牌 | 翻牌 → 轉牌（1 張） | 到河牌（2 張） | 河牌賠率 |
|:---|:---:|:---:|:---:|
| 2 | 4.3% | 8.4% | 11 比 1 |
| 4 | 8.5% | 16.5% | 5 比 1 |
| 6 | 12.8% | 24.1% | 3.1 比 1 |
...
每個聽牌都有兩個數字要看。**「到河牌」** 把剩下兩張牌都算進去，只有在後面不可能再有下注時才適用——你已經全下（all-in），或是你跟了別人的全下。**「翻牌 → 轉牌」** 只算下一張牌（9 ÷ 47 = 19.1%；從轉牌到河牌則變成 9 ÷ 46 = 19.6%）——只要後面還有下注，就用這個，因為你一次只保證看得到一張牌。新手明明面對的是轉牌圈的注，卻拿「到河牌」那個又肥的數字說服自己跟注，然後付出代價。

注意那個 15 張的怪物聽牌：還剩兩張牌要發時，它有 54.1% 會成牌——對上單純一對，這通常讓它成為**優勢方**，是那種你可以開開心心在翻牌圈就全下的罕見聽牌。對上暗三條就不是了：牌面可能成對、把暗三條補成葫蘆——下面那個 9♠ 8♣ 2♠ 上的 J♠ 10♠，對上口袋 9 只有大約 40%。
...
> **快速解答**
> 四二法則是拿來判斷「可不可以追」的，不是拿來精算的。乘完拿去跟所需勝率比，差距明顯就直接做決定。真正要小心的是補牌多的時候——==×4 從 7 張起就略微偏高==，補牌越多差得越遠，別讓一個捷徑替你做掉一個很接近的決定。
...
:::tip[×4 那版悄悄假設了你會看到*兩張*牌、中間沒有更多下注——只有在後面不可能再下注時（你已經全下，或你跟了別人的全下）才有保證。如果你面前還有一注要打，就用 ×2（單張牌）的數字，看你眼前這條街到底是哪一街。]:::

有一個地方這條法則會騙人：**翻牌圈補牌數很大的時候。** 因為 ×4 把「兩街都中」的那一點點小機率重覆算進去了，所以它從 7 張補牌起就開始略微高估，補牌越多差距越大；下面那個常用修正，則是用在補牌超過 8 張的時候。
...
> **快速解答**
> 兩個聽牌疊在一起時，別把兩個補牌數直接相加——先問「有沒有哪一張同時完成兩邊」。同花加兩頭順看起來是 9 + 8 = 17，實際是 15，因為那兩張既是同花又是順子的牌只能算一次。要相加的不是數字，是==彼此不重疊==的那幾組牌。
...
**同花 + 卡順**也是同樣的道理：9 張同花補牌 + 4 張卡順補牌，但那四張裡有一張是你的花色 → 9 + 3 = **12**。只要兩個聽牌共用牌，就把重疊的扣掉——同花＋卡順共用一張，同花＋兩頭順共用兩張。這是牌手最常見的多算方式，也是為什麼表裡那幾行複合聽牌，看起來總比兩數相加來得少（同花＋卡順少一張，同花＋兩頭順少兩張）。
...
> **快速解答**
> 髒補牌不是「不算」，是==打折算==。一張讓你成同花、卻同時把牌面配成對的牌，還是有機會贏——只是沒有你算的那麼有機會。與其糾結該扣幾張，不如養成一個習慣：牌面或下注在對你示警時，先把補牌數往下砍，再去乘。
...
:::card
♠ | 非堅果同花 | 你拿 8♠ 7♠ 面對 K♠ 9♠ 2♣，帳面上有 9 張黑桃「補牌」——但如果來一張黑桃、而對手正拿著更大的黑桃追同一個同花，你湊成了同花卻還是輸。當你追的不是堅果同花時，就得把補牌打折
...
A. 補牌是牌堆裡還沒露出來、能把你這手變成大概會贏的那些牌。同花聽牌有 9 張補牌（沒看見的 9 張同花色——前提是做成的同花真的會贏）；兩頭順聽牌有 8 張。你數補牌是為了算出成牌機率，進而判斷跟注划不划算。
...
A. 八張——兩端各四張能填滿順子。卡順（內順）聽牌只有 4 張補牌，因為只有一個點數能補那個洞。雙卡順（double gutshot）也是 8 張，跟兩頭順一樣。
...
A. 不用。你只扣你真正看得見的牌——你的底牌和公共牌面。其他每一張看不見的牌都當成活牌，這也是為什麼那些原始補牌數（同花 9 張、兩頭順 8 張）不管對手拿什麼都一樣。至於每一張是不是真的會贏，還是要看對手的牌——那就是髒補牌的檢查。
...
2. **用四二法則換算。** 翻牌圈補牌 × 4、轉牌圈 × 2。在翻牌圈，大聽牌（超過 8 張補牌）的 ×4 估值要再減掉 *(補牌 − 8)* 修一下。
