import type { Post } from "../posts";

export const POST: Post = {
  slug: "korea-poker-marathon-2026",
  title: "2026 韩国跑步 + 打牌 — 一趟行程搞定马拉松和扑克",
  seoTitle: "2026 韩国扑克 + 马拉松 — 春川、JTBC 首尔 + Paradise City 指南",
  desc: "2026 年外国人报名窗口已全部关闭——春川、JTBC、Garmin 全数截止。赛程表、报名费，以及仁川机场旁 Paradise City 的扑克，照 2027 年的模板读。",
  tldr: "韩国最好的秋季马拉松（春川 10 月 25 日、JTBC 首尔 11 月 1 日、Garmin Run 11 月 15 日）正好赶上完美的跑步天气。仁川机场旁的 Paradise City Casino 全年开放现金局——只对外国人开放，离航站楼 10 分钟。一趟行程两样都能拿下。2026 年的外国人报名已全部结束；2027 年的春川窗口约在 7 月。",
  category: "tournament",
  date: "2026-06-19",
  updated: "2026-09-10",
  masterUpdated: "2026-09-10",
  keepImagesInBody: true,
  readTime: "14 分钟",
  emoji: "🏃",
  image: "/images/korea-poker-marathon-2026-hero.webp",
  imageAlt: "跑者在春川冲过终点线，背景是秋日红叶和衣岩湖——2026 韩国马拉松",
  tags: [
    "韩国马拉松赛事",
    "韩国马拉松 2027",
    "春川马拉松2026报名",
    "JTBC 首尔 马拉松 2026",
    "Garmin Run Korea 2026",
    "韩国 扑克 跑步 行程",
    "韩国 跑步 打牌",
    "Paradise City 赌场",
  ],
  content: `
2026 年 5 月 17 日，BTS 的 Suga 早上 6 点在帕罗奥图的酒店醒来，跑了旧金山的 12K Bay to Breakers，成绩 1:04:43——当天晚上又在斯坦福体育场对 5 万名观众开唱。RM 后来解释了背景：==g:Suga 每天跑 10K。== 他听说有这场比赛，就直接报名了。

这个故事的共鸣超出了 ARMY 粉丝圈。它戳中了一个早就存在的重叠：打牌的人和跑步的人。这两个圈子共享的东西，比谁记录下来的都多——自律、对波动的容忍、对优化的痴迷。

而韩国，就是这两者完美交汇的地方。==**世界级的秋季马拉松。离机场 10 分钟、只对外国人开放的赌场。夹在中间的，是全亚洲最好的城市美食。**==

<div style="display:flex;align-items:flex-start;gap:16px;margin:20px 0">
  <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-marathon-record.webp" alt="2026 高阳半程马拉松官方成绩卡——芯片计时 1:51:10，男子半程组" width="600" height="899" sizes="160px" style="width:160px;flex-shrink:0;border-radius:8px" />
  <div style="font-size:14px;line-height:1.7;color:hsl(var(--muted-foreground))">
    <strong style="display:block;margin-bottom:6px;color:hsl(var(--foreground))">个人成绩——2026 高阳半程马拉松</strong>
    芯片计时：<strong>1:51:10</strong> · 男子半程组<br />
    韩国的赛事组织意外地精细——官方芯片计时、即时电子成绩卡、实时追踪。外国跑者拿到的体验和本地人完全一样。
  </div>
</div>

---

> **快速回答**
> 韩国扑克赛事：==**APT Incheon 8 月 7–16 日**==、==**APT Jeju 9 月 25 日–10 月 7 日**==（都在只对外国人开放的赌场）。韩国马拉松：==**春川 10 月 25 日**==、==**JTBC 首尔 11 月 1 日**==、==**Garmin Run 11 月 15 日**==。APT Jeju 10 月 7 日结束 → 春川 10 月 25 日开跑——==g:中间隔 18 天，一趟行程两样都占。== ==r:2026 年春川和 Garmin 的外国人报名已经结束，JTBC 则要看渠道（日本 RUNNET 渠道 9 月 4 日关闭，worldsmarathons.com 请以官方页面现况为准）——下面的日期请当 2027 年的模板读。==

---

## 为什么韩国是跑步 + 打牌的完美目的地？

> **快速回答**
> 理由就两条：时机和地点。10 月和 11 月是东亚最值得去的月份，8–15°C、空气干燥、满山秋叶，韩国的马拉松赛程就是围着它排的；而 Paradise City Casino 紧挨仁川机场，免费摆渡 10 分钟，全年开现金局，只对外国人开放。首尔地铁双语，到春川的快速列车 90 分钟，一切都连得上。

拆开来看：

**天气窗口：** 韩国的夏天（6–8 月）又热又极其潮湿——跑步和旅游都不理想。但 ==**10 月和 11 月大概是东亚最值得去的月份**==。气温降到 8–15°C。空气干燥通透。乡野被秋叶染成橙红。韩国每个认真的跑者都懂这一点。整张马拉松赛程表就是围着它排的。

**赌场位置：** Paradise City Casino 坐落在一个度假综合体里，==g:紧挨着仁川国际机场==。从 1 号航站楼出发的免费摆渡车每 20 分钟一班。赌场全年运营现金局牌桌——不需要参加锦标赛。对国际牌手来说，它是亚洲最好进的赌场：落地仁川，办理入住，直接开打。

**交通：** 首尔的地铁便宜、快、双语标识。从 Paradise City（经 AREX 机场快线）到首尔市中心大约 1 小时。从首尔坐快速列车到春川（马拉松所在城市）90 分钟。一切都连得上。

---

## 韩国 2026 马拉松赛程表——外国跑者

> **快速回答**
> 2026 年秋季的四场：YTN 首尔 Tour Marathon 9 月 6 日、春川马拉松 10 月 25 日（全程、10K）、JTBC 首尔马拉松 11 月 1 日（全程、10K）、Garmin Run Korea 11 月 15 日（半程、10K）。==r:外国人报名窗口 2026 年已全部关闭==——YTN 6 月 26 日额满、春川售罄、Garmin 报名期已过、JTBC 按渠道而定（日本 RUNNET 渠道 9 月 4 日关闭）。下面的日期请当 2027 年的模板来读：目标窗口是 10 月或 11 月。

<div style="overflow-x:auto;margin:20px 0">

| 赛事 | 日期 | 距离 | 地点 | 外国人报名 | 状态 |
|---|---|---|---|---|---|
| Dong-A 首尔国际马拉松 | 2026 年 3 月 | 全程、10K | 首尔 | [dongamarathon.com](https://www.dongamarathon.com) | 2026 已结束 |
| ==**YTN 首尔 Tour Marathon**== | ==**9 月 6 日**== | ==**11KM**==、半程 | Seoul Plaza | [run.ytn.co.kr](http://run.ytn.co.kr) | 已截止——6 月 26 日提前额满 |
| ==g:**春川马拉松**== | ==g:**10 月 25 日**== | ==g:全程、10K== | ==g:春川== | ==g:[chuncheonmarathon.com](https://www.chuncheonmarathon.com)== | ==g:报名截止（售罄）== |
| ==**JTBC 首尔马拉松**== | ==**11 月 1 日**== | 全程、10K | 首尔（上岩） | [worldsmarathons.com](https://worldsmarathons.com/marathon/international-seoul-marathon) | ==r:外国人报名截止日按渠道而定==（日本 RUNNET 渠道已于 2026 年 9 月 4 日关闭）——名额有限，抽签分配 |
| Garmin Run Korea | 11 月 15 日 | 半程、10K | 高阳 | [runable.me](http://runable.me/product/18977?comp=18962) | 已截止（报名为 7 月 1–3 日） |

</div>

==r:韩国的夏天（6–8 月）又热又潮——比赛之所以集中在 3 月和 10–11 月，就是这个原因。== 如果你打算围着跑步安排一趟韩国行，10 月或 11 月就是目标窗口。

![韩国秋季马拉松终点线——跑者冲过终点，计时牌清晰可见，观众和摄影师聚在线前](/images/korea-poker-marathon-2026-marathon-finish.webp)

*一场韩国秋季赛事的终点氛围。计时钟显示 1:34 上下——竞技型本地跑者定下节奏，休闲跑者在他们身后一整个上午陆续完赛。*

---

## 2026 春川马拉松 — 10 月 25 日

> **快速回答**
> 春川马拉松创办于 1946 年，韩国第二古老、AIMS 认证赛道，10 月 25 日 9:00 从孔之川桥出发，绕衣岩湖一圈，全程 42.195K 和 10K，赛日 8–14°C。外国人在 chuncheonmarathon.com 的 Foreigner 标签报名，2026 年名额不到 3 分钟抢光——2027 年窗口约在 7 月，先备好护照号和付款方式。

==**春川**==（춘천）是首尔以东坐快速列车约 80–90 分钟的一座小城，位于江原道两条河交汇处的山谷之中。韩国人称它"浪漫"。它举办的这场马拉松，是亚洲最受尊敬的赛事之一。

**这场比赛：** 创办于 1946 年——韩国第二古老的马拉松。==g:经国际丈量与认证的赛道==（AIMS）。全程 42.195K + 10K。9:00 从孔之川桥（Gongjicheon Bridge）出发。赛道绕着 ==**衣岩湖**==（의암호）跑一圈，那是一座被群山环抱的水库，10 月底山色正在变。比赛当天气温通常在 8–14°C——正是能跑出个人最好成绩的条件。

![秋日韩国公园里挂着号码布的跑者——橙红落叶铺满赛道两侧，晨光斜照](/images/korea-poker-marathon-2026-marathon-race.webp)

*韩国秋季的比赛清晨。10 月的跑步就是这个样子——10–14°C、落叶满地、安静的公园道路。*

**赛道实况：** 不是平路。5K 和 30K 处有起伏丘陵，春川大坝附近有一段上坡，会把队伍拉开。但回报——沿着无声的群山湖泊、伴着秋叶、没有车流地跑——是任何城市马拉松都比不了的。多位国际跑者的赛记都说，这是他们在亚洲跑过最有风景的一场。

**怎么报名（外国人）：**
1. 打开 [chuncheonmarathon.com](https://www.chuncheonmarathon.com)
2. 点 **참가신청** → **신청하기** → 选 **Foreigner** 标签
3. 需要护照号码（用于保险）
4. 全程报名：7 月 14 日下午 2 点（KST）。缴费窗口：7 月 20–21 日
5. 10K 报名：7 月 16 日下午 2 点（KST）。缴费窗口：7 月 22–23 日

==r:2026 年名额不到 3 分钟就抢光。为 2027 年做准备：窗口一开之前，先把护照号码和付款方式备好——2026 年的窗口 7 月就关了。==

**从首尔怎么去：** 从龙山站坐 ITX-青春（ITX-Cheongchun）快速列车 → 春川，约 80–90 分钟，约 ₩8,800。或者坐地铁经义中央线到清凉里，再转春川线（总共约 100 分钟，更便宜）。便宜、有风景、不用开车。

**春川 → Paradise City 的路线：** 赛后坐快速列车回首尔（约 80 分钟）→ 从首尔站或弘大坐 AREX → 从 1 号航站楼坐 Paradise City 摆渡车。总共约 3 小时。赛后周日傍晚完全跑得完。

---

## 2026 JTBC 首尔马拉松 — 11 月 1 日

> **快速回答**
> JTBC 首尔马拉松是韩国头号城市赛事，11 月 1 日 8:00 从上岩世界杯体育场出发，穿过光化门和东大门，全程终点奥林匹克公园，关门 5 小时。海外跑者的唯一报名渠道是 worldsmarathons.com，10K €104、全程 €129，名额抽签分配，2026 年的截止日按渠道而定（日本 RUNNET 渠道 9 月 4 日关闭）。参赛包 10 月 29–31 日在江南 SETEC 凭护照领取。

==**JTBC 首尔马拉松**== 是韩国头号城市赛事——25,000–50,000 名跑者，八成是二三十岁的人，穿过首尔市中心的地标。如果说春川是风景乡野，JTBC 就是橱窗展示。

**赛道：** 从 ==上岩世界杯体育场==（6 号线，上岩世界杯体育场站）出发。经过德寿宫外围、穿过光化门广场和东大门——首尔历史与现代身份的完整扫描。全程终点在 ==奥林匹克公园==；10K 终点在 ==汝矣岛公园==。关门时间：全程 5 小时。

**赛事细节：**
- 日期：==**2026 年 11 月 1 日（周日）**==，8:00 出发
- 距离：全程 42.195K · 10K
- 起点：上岩世界杯体育场
- Expo（领取参赛包）：SETEC，江南——10 月 29–31 日，上午 10 点至下午 6 点
- 领包时需出示护照（不能代领）

**怎么报名（外国人）：**
居住在韩国境外的外国跑者，只能通过 ==**[worldsmarathons.com](https://worldsmarathons.com/marathon/international-seoul-marathon)**== 报名。==r:名额有限且常常很早满员==，请以官方页面的最新状态为准——==r:**截止日按你走的渠道而定**==：日本 RUNNET 渠道到 2026 年 9 月 4 日为止、已经关闭，而 worldsmarathons.com 是另一个渠道，可能仍显示有名额，所以请在那里确认后再做打算。无论走哪个渠道，取消／退款的截止时间都落在该渠道关闭当天的 23:59（KST）。持韩国护照者无法通过这个渠道报名。

该渠道的报名费为 ==**10K €104**==、==**全程 42.2K €129**==（仅限海外跑者）。由于过去两个项目都在首日售罄，==g:名额已改为抽签分配==——早报名不再等于占到位子，只是进入抽签。参赛包必须在赛前的 EXPO 领取，报名姓名须与护照一致。路线从 ==**世界杯公园（麻浦区）**== 出发，终点在 ==**奥林匹克公园（松坡区）**==。

==g:JTBC 的外国人名额有限，抽签分配——2027 年的窗口开了就要盯紧。==

**赛后 → 打牌：** 全程终点在 ==奥林匹克公园==（松坡，5/8 号线）。从那里搭地铁到首尔站或弘大，再换 AREX 普通车（All-Stop）→ 从 1 号航站楼坐 Paradise City 摆渡车。从冲线算起要留 ==**1.5–2 小时**==，还不含洗澡换衣。大多数跑者在 12 点到 13 点之间完赛——傍晚打一场现金局是现实的，中午则不行。

---

## 2026 Garmin Run Korea — 11 月 15 日

> **快速回答**
> Garmin Run Korea 11 月 15 日 8:00 在一山的高阳综合运动场开跑——K-pop 粉丝会认出这座场馆，BTS、BLACKPINK 都在这里开过唱。半程 21.1K 和 10K，报名费 ₩80,000 和 ₩60,000，报名只在 7 月 1–3 日、抽签选取，2026 年已截止。高阳坐 3 号线离首尔市中心约 50 分钟。

==**Garmin Run Korea**==（가민런 코리아）是赛程表上较新的一个——但场地本身就值得一提。

比赛在京畿道一山的 ==**高阳综合运动场**==（고양종합운동장）举办。国际跑者可能从另一个语境认出这座体育场：它就是 ==g:BTS 和 BLACKPINK 演唱会== 用的同一个场地，是韩国规模最大的 K-pop 活动之一。对既追 K 文化又跑步的游客来说，这个组合值得绕一趟。

**赛事细节：**
- 日期：2026 年 11 月 15 日（周日），8:00 出发
- 距离：半程 21.1K · 10K
- 场地：高阳综合运动场，一山（京畿道高阳）
- 报名费：半程 ₩80,000 · 10K ₩60,000
- 报名：[runable.me](http://runable.me/product/18977?comp=18962)——==r:仅 7 月 1–3 日，抽签选取==

高阳坐地铁离首尔市中心约 50 分钟（3 号线，往一山大化方向）。轻松一日游。

---

## 2026 YTN 首尔 Tour Marathon — 9 月 6 日

> **快速回答**
> YTN 首尔 Tour Marathon 于 9 月 6 日 7:30 从市政厅前的 Seoul Plaza 出发，只有 11KM 和半程两个组别、没有 10K，报名费 ₩70,000 和 ₩80,000。报名原定到 7 月 10 日，6 月 26 日就提前额满。报名原定到 7 月 10 日，6 月 26 日就提前额满。想在秋天跑全程的，目标是 10 月的春川，不是这一场。

对 9 月抵达的人来说，==**YTN 首尔 Tour Marathon**== 从 ==**Seoul Plaza**==（서울광장）出发——就在市政厅正前方，背景是德寿宫和市中心天际线。

- 日期：2026 年 9 月 6 日，7:30 出发
- 距离：==**11KM**== · 半程 —— ==r:这场比赛没有 10K 组别==
- 报名费：11KM ₩70,000 · 半程 ₩80,000
- 起点：Seoul Plaza（市政厅一带）
- 主办方：YTN
- 网站：[run.ytn.co.kr](http://run.ytn.co.kr)
- 报名：原定 6 月 17 日 – 7 月 10 日，但==r:先到先得，6 月 26 日就提前额满截止==

注意：韩国的暑热在 8 月达到顶峰。9 月仍然偏热（20–28°C），湿度到月底才缓下来。在这种条件下，半程比全程更好扛。

---

## 韩国扑克赛事：2026 年 9–12 月

> **快速回答**
> 三场扑克节正好落在秋季窗口：APT Jeju 9 月 25 日–10 月 7 日（济州 LES A Casino，135 项赛事、$3.5M+ 保底），GOP Incheon 10 月 30 日–11 月 8 日（Paradise City；JTBC 马拉松那天正好是它的第 3 天），AJPC Samurai Circuit 12 月 4–13 日（Paradise City）。APT Jeju 结束后隔 18 天，就是春川马拉松。

赛程有意思的地方就在这里。三场扑克节在韩国场馆举办，正好落在和马拉松同一个秋季窗口——而且其中一场直接和 JTBC 首尔撞上。

<div style="overflow-x:auto;margin:20px 0">
<table style="width:100%;border-collapse:collapse;font-size:14px;line-height:1.5">
  <thead>
    <tr style="border-bottom:2px solid rgba(255,255,255,0.12)">
      <th style="text-align:left;padding:10px 14px;color:hsl(var(--muted-foreground));font-weight:600;white-space:nowrap;width:160px">日期</th>
      <th style="text-align:left;padding:10px 14px;color:hsl(var(--muted-foreground));font-weight:600">赛事</th>
      <th style="text-align:left;padding:10px 14px;color:hsl(var(--muted-foreground));font-weight:600">场馆</th>
      <th style="text-align:left;padding:10px 14px;color:hsl(var(--muted-foreground));font-weight:600">关键信息</th>
    </tr>
  </thead>
  <tbody>
    <tr style="background:rgba(212,175,55,0.07);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:10px 14px;font-weight:700;white-space:nowrap">9 月 25 日 – 10 月 7 日</td>
      <td style="padding:10px 14px"><strong>APT Jeju 2026</strong></td>
      <td style="padding:10px 14px">LES A Casino, Jeju</td>
      <td style="padding:10px 14px;color:hsl(var(--muted-foreground))">$3.5M+ 总保底 · 135 项赛事 · 春川马拉松前 18 天结束</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.07);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:10px 14px;font-weight:700;white-space:nowrap">10 月 30 日 – 11 月 8 日</td>
      <td style="padding:10px 14px"><strong>GOP Incheon<br/><span style="font-size:12px;color:hsl(var(--muted-foreground))">The Labyrinth Trail</span></strong></td>
      <td style="padding:10px 14px">Paradise City, Incheon</td>
      <td style="padding:10px 14px"><span style="background:rgba(239,68,68,0.15);color:#f87171;padding:2px 7px;border-radius:4px;font-size:12px;font-weight:600">⚡ JTBC 马拉松 11 月 1 日 = 这场赛事的 Day 3</span></td>
    </tr>
    <tr style="background:rgba(212,175,55,0.07);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:10px 14px;font-weight:700;white-space:nowrap">12 月 4–13 日</td>
      <td style="padding:10px 14px"><strong>AJPC Samurai Circuit<br/><span style="font-size:12px;color:hsl(var(--muted-foreground))">Incheon 2026 III</span></strong></td>
      <td style="padding:10px 14px">Paradise City, Incheon</td>
      <td style="padding:10px 14px;color:hsl(var(--muted-foreground))">年末之选 · 单项赛程与买入待定</td>
    </tr>
  </tbody>
</table>
</div>

### APT Jeju 2026 — 9 月 25 日–10 月 7 日

==**济州岛**== 是韩国最受欢迎的本土度假胜地——火山海岸、亚热带气候、多数国籍免签（⚠️ ==r:中国内地护照的济州免签只在济州岛内有效，不能从济州飞首尔或仁川==——见 [APT 仁川指南](/zh/blog/apt-incheon-2026-guide) 的签证一节）。赛事在济州新华世界（Jeju Shinhwa World）内的 ==**LES A Casino**== 举办。

- **保底：** ==g:135 项奖杯赛事总保底 $3.5M+==（主赛事：买入 ==**KRW 270 万**== ≈ $1,800，==g:KRW 22 亿保底==）
- **完整赛程：** 已公布——13 天内 135 项赛事 · [theasianpokertour.com](https://www.theasianpokertour.com/series/apt-jeju-south-korea-2026/info)
- 仅限外国人 · 19+ · 持有效护照（海外韩国人：韩国护照 + 绿卡或移民证明）

==g:APT Jeju 10 月 7 日结束——春川马拉松是 10 月 25 日。中间隔 18 天。== 从济州飞首尔，休整、逛逛，然后开跑。

---

### GOP Incheon — The Labyrinth Trail（10 月 30 日–11 月 8 日）

==**Gods of Poker（GOP）**== 重返 Paradise City，举办 2026 年在仁川的第二站，时间是 10 月 30 日到 11 月 8 日。这个扑克节正好压在 JTBC 首尔马拉松的日期上。

==**11 月 1 日既是 JTBC 比赛日，也是 GOP 扑克节的 Day 3。**==

对一个已经在韩国参赛的牌手来说：比赛当天早上 6 点闹钟（8 点起跑，提前一小时左右集合），42K 穿过首尔市中心，在奥林匹克公园完赛。==r:如果你跑进 4 小时==，中午就结束，傍晚能坐上 Paradise City 的牌桌。若是 4:30–5:00（关门时间正是 5 小时），那就变成晚上的安排。牌桌 24 小时开着，所以两件事总归能塞进同一天。基本没人在写这个重叠。

🪶 **但要说句实在话：能打，不代表打得好。** 跑完 42K 之后，糖原耗尽、身体脱水、前一晚也没睡够——这恰好是牌手 call 得过宽、坐得过久的状态。如果那晚还是要坐下，就先定好一个短时段和止损线，或者干脆把牌局挪到第二天。牌桌全年都开，马拉松只有那一个早上。

- **日期：** 2026 年 10 月 30 日 – 11 月 8 日
- **场馆：** Paradise City Casino, Incheon（从仁川 1 号航站楼免费摆渡）
- **信息：** [pokerdiscover.com](https://pokerdiscover.com/series/gods-of-poker-gop-incheon-the-labyrinth-trail-incheon-30-oct-08-nov-2026)
- 完整赛程和买入待定——关注官方 GOP／Paradise City 渠道

---

### AJPC Samurai Circuit Incheon III（2026 年 12 月 4–13 日）

==**AJPC Samurai Circuit**== 以 12 月在 Paradise City 的一轮，为韩国扑克赛程收尾。对错过 10–11 月窗口的牌手，这是年末之选。

- **日期：** 2026 年 12 月 4–13 日（据 pokercalendar.asia 赛历 · 8 月 28 日核实）
- **场馆：** Paradise City Casino, Incheon
- **信息：** [pokercalendar.asia](https://pokercalendar.asia/en/pokertournaments/ajpc-samurai-circuit-incheon-2026-iii/)
- 赛程和买入待定

12 月没有马拉松重叠，但不管怎样，Paradise City 的现金局全年都在开。

---

## Paradise City Casino — 扑克这一面

> **快速回答**
> 这座赌场一年 365 天开门，不只在办锦标赛的时候——现金局牌桌每天都为国际牌手开着，以无限注德州扑克为主。它紧挨仁川机场，1 号航站楼 3C 门的免费摆渡每 20 分钟一班，五星酒店 Hotel Paradise 在场内，还有水疗和桑拿。仅限外国人、19 岁以上、持有效护照。

马拉松集中在 9–11 月的那几个周日，而 ==**Paradise City Casino**== 一年 365 天都在发牌。

**基本情况：**
- 位置：仁川，紧挨着仁川国际机场
- 免费摆渡：1 号航站楼（3C 门），每 20 分钟一班，车程约 10 分钟
- 酒店：Hotel Paradise，五星，就在场内
- ==r:仅限外国人——无海外永久居留权的韩国公民不得入场==
- 海外韩国人（在海外的永久居民）可凭韩国护照 + 绿卡或移民证明入场
- 年龄：19+ 且持有效护照
- 牌局：无限注德州扑克现金局、其他桌上游戏、老虎机
- 场内有水疗和桑拿——刚跑完 42K 的话，这一点很实用

想看完整的锦标赛情况（APT Incheon 已于 8 月 7–16 日举办，$2.7M GTD），见 [2026 APT Incheon 指南](/zh/blog/apt-incheon-2026-guide "thumb:/images/apt-incheon-2026-guide-hero.webp")。

**完整的 2026 韩国扑克 + 马拉松赛程表：**

<div style="overflow-x:auto;margin:20px 0">
<table style="width:100%;border-collapse:collapse;font-size:13px;line-height:1.5">
  <thead>
    <tr style="border-bottom:2px solid rgba(255,255,255,0.12)">
      <th style="text-align:left;padding:9px 12px;color:hsl(var(--muted-foreground));font-weight:600;white-space:nowrap">日期</th>
      <th style="text-align:left;padding:9px 12px;color:hsl(var(--muted-foreground));font-weight:600">活动</th>
      <th style="text-align:left;padding:9px 12px;color:hsl(var(--muted-foreground));font-weight:600">场馆</th>
      <th style="text-align:left;padding:9px 12px;color:hsl(var(--muted-foreground));font-weight:600">详情</th>
    </tr>
  </thead>
  <tbody>
    <tr style="opacity:0.5;border-bottom:1px solid rgba(255,255,255,0.05)">
      <td style="padding:8px 12px;white-space:nowrap">5 月 15–24 日</td>
      <td style="padding:8px 12px">🃏 GOP Incheon 2026 <em>（已结束）</em></td>
      <td style="padding:8px 12px">Paradise City, Incheon</td>
      <td style="padding:8px 12px;color:hsl(var(--muted-foreground))">已完成</td>
    </tr>
    <tr style="opacity:0.5;border-bottom:1px solid rgba(255,255,255,0.05)">
      <td style="padding:8px 12px;white-space:nowrap">5 月 15–25 日</td>
      <td style="padding:8px 12px">🃏 TLPT Jeju 2026 <em>（已结束）</em></td>
      <td style="padding:8px 12px">LES A Casino, Jeju</td>
      <td style="padding:8px 12px;color:hsl(var(--muted-foreground))">已完成</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">8 月 7–16 日</td>
      <td style="padding:9px 12px">🃏 <strong>APT Incheon 2026</strong></td>
      <td style="padding:9px 12px">Paradise City, Incheon</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">$2.7M GTD · 主赛事 $1M GTD</td>
    </tr>
    <tr style="background:rgba(34,197,94,0.05);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">9 月 6 日</td>
      <td style="padding:9px 12px">🏃 YTN 首尔 Tour Marathon</td>
      <td style="padding:9px 12px">Seoul Plaza</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">11KM · 半程 · 7:30 · 报名已截止</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">9 月 25 日–10 月 7 日</td>
      <td style="padding:9px 12px">🃏 <strong>APT Jeju 2026</strong></td>
      <td style="padding:9px 12px">LES A Casino, Jeju</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">$3.5M+ GTD · 135 项赛事 · 距春川 18 天</td>
    </tr>
    <tr style="background:rgba(34,197,94,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">10 月 25 日（周日）</td>
      <td style="padding:9px 12px">🏃 <strong>春川马拉松</strong></td>
      <td style="padding:9px 12px">孔之川，春川</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">全程 · 10K · 9:00 · 7 月 14 日报名</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">10 月 30 日–11 月 8 日</td>
      <td style="padding:9px 12px">🃏 <strong>GOP Incheon<br/><span style="font-size:11px;font-weight:400;opacity:0.7">The Labyrinth Trail</span></strong></td>
      <td style="padding:9px 12px">Paradise City, Incheon</td>
      <td style="padding:9px 12px"><span style="background:rgba(239,68,68,0.18);color:#fca5a5;padding:1px 6px;border-radius:3px;font-size:11px;font-weight:700">⚡ JTBC 马拉松在 Day 3</span></td>
    </tr>
    <tr style="background:rgba(239,68,68,0.06);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">11 月 1 日（周日）</td>
      <td style="padding:9px 12px">🏃 <strong>JTBC 首尔马拉松</strong></td>
      <td style="padding:9px 12px">上岩世界杯体育场 → 奥林匹克公园</td>
      <td style="padding:9px 12px"><span style="background:rgba(239,68,68,0.18);color:#fca5a5;padding:1px 6px;border-radius:3px;font-size:11px;font-weight:700">⚡ GOP 赛事同周进行</span></td>
    </tr>
    <tr style="background:rgba(34,197,94,0.05);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">11 月 15 日（周日）</td>
      <td style="padding:9px 12px">🏃 Garmin Run Korea</td>
      <td style="padding:9px 12px">高阳综合运动场</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">半程 · 10K · 8:00 · 7 月 1–3 日报名</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">12 月 4–13 日</td>
      <td style="padding:9px 12px">🃏 AJPC Samurai Circuit III</td>
      <td style="padding:9px 12px">Paradise City, Incheon</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">单项赛程与买入待定</td>
    </tr>
    <tr style="border-top:1px solid rgba(255,255,255,0.08)">
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">全年</td>
      <td style="padding:9px 12px">🃏 现金局（NLHE）</td>
      <td style="padding:9px 12px">Paradise City, Incheon</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">24/7 开放 · 仅限外国人</td>
    </tr>
  </tbody>
</table>
</div>

==g:没人提的甜蜜点：== **APT Jeju 10 月 7 日结束——春川马拉松是 10 月 25 日。** 中间隔 18 天。打完锦标赛，留在韩国，再去跑比赛。一趟机票两样都占。而且如果你愿意留到 11 月 1 日，JTBC 首尔也就在那儿。

最干净的一鱼两吃：APT Incheon（8 月 7–16 日）→ 在首尔休整 → 春川马拉松（10 月 25 日）或 JTBC（11 月 1 日）。或者：APT Jeju（9 月 25 日–10 月 7 日）→ 在韩国待 18 天 → 春川（10 月 25 日）。⚠ 2026 年的外国人报名已全部结束——上面这两条线请当作 2027 年的模板来看。

---

## 行程样例——两样怎么串起来？

> **快速回答**
> 两个方案：A 是 APT Incheon（8 月 7–16 日）之后在韩长住，10 月 25 日跑春川、11 月 1 日跑 JTBC——覆盖最大化，但 8 月的起点已经过了，请按 2027 年规划；B 是 APT Jeju（9 月 25 日–10 月 7 日）结束后留 18 天，10 月 25 日跑春川，11 月 1 日可选 JTBC 背靠背。

**方案 A：APT Incheon + JTBC 首尔（长住）—— ==r:请按 2027 年规划==，8/7 的起点已经过了**

| 日 | 计划 |
|---|---|
| Day 1（8 月 7 日） | 飞抵 → Paradise City · APT Incheon 开赛 |
| 8 月 7–16 日 | ==g:APT Incheon 2026 赛事== |
| 8 月 17 日–10 月 24 日 | 在韩长住——首尔、釜山、庆州一日游 · 现金局 |
| 10 月 25 日 | ==**春川马拉松**== · 回首尔／Paradise City |
| 11 月 1 日 | ==**JTBC 首尔马拉松**== |
| 11 月 2 日起 | 离境 |

*这是覆盖最大化的方案。多数牌手不会待满两个多月，但它展示了照着这张赛程表能玩到什么程度。*

**方案 B：APT Jeju + 春川马拉松（隔 18 天）** ⚠️ ==r:本方案假设你持可入境韩国本土的签证或免签资格==。若用济州免签入境，10 月 8 日的济州 → 首尔这一步走不了。

| 日 | 计划 |
|---|---|
| 9 月 25 日 | 飞抵 → 济州新华世界 · APT Jeju 开赛 |
| 9 月 25 日–10 月 7 日 | ==g:APT Jeju 2026 赛事== · 打牌间隙逛济州 |
| 10 月 8–9 日 | 从济州飞首尔 · 入住首尔或 Paradise City 酒店 |
| 10 月 10–24 日 | 首尔时间——景福宫、弘大、明洞、现金局 |
| ==**10 月 25 日**== | ==**春川马拉松**==（从首尔坐 ITX，约 80 分钟） |
| 10 月 26–31 日 | 恢复 · 首尔或 Paradise City 现金局 |
| 11 月 1 日 | 可选：==**JTBC 首尔马拉松**==（背靠背一周） |
| 11 月 2 日 | 从仁川机场离境 |

---

## 韩国跑步 + 打牌 7 天怎么排？（JTBC 版）

> **快速回答**
> 以周日的 JTBC 为轴排 7 天：周四飞抵入住，周五逛景福宫、北村、广藏市场，周六去江南 SETEC 领参赛包、轻跑早睡，周日 8:00 开跑后恢复休息，周一弘大、明洞、清溪川，周二转去 Paradise City 打一场现金局，周三早上打牌后坐 AREX 离境。

| 日 | 计划 |
|---|---|
| Day 1（周四） | 飞抵 → 入住 Paradise City 或首尔酒店 · 晚上现金局或休息 |
| Day 2（周五） | 首尔观光——景福宫、北村、广藏市场 |
| Day 3（周六） | 到 SETEC（江南）Expo 领参赛包 · 轻跑 · 早睡 |
| Day 4（周日） | ==**JTBC 首尔马拉松**==（8:00 出发）· 恢复餐 · 休息 |
| Day 5（周一） | 首尔一日：弘大、明洞、清溪川散步 |
| Day 6（周二） | 转去 Paradise City · 下午加晚上打一场现金局 |
| Day 7（周三） | 早上打牌 · AREX 到仁川机场 · 离境 |

---

## 汉江跑步——首尔 80 公里的江边步道

> **快速回答**
> 汉江两岸有 80 多公里平坦、无车、24 小时开放的跑道。新手选汝矣岛 5K 环线（汝矣渡口站，场内 Runner Station 有淋浴和储物柜），傍晚选盘浦 5K（入夜有彩虹喷泉），想安静就去纛岛。最佳时段清晨或傍晚 6–8 点；跑完在公园便利店买碗汉江拉面，₩4,000–5,000。

在比赛日或打牌间隙，有一个地方是首尔每个跑者最终都会去的：==**汉江公园（Hangang Park）**==。

汉江从首尔正中穿过，两岸有超过 ==g:80 公里平坦的柏油跑道和骑行道==——连贯、无车、24 小时开放。没有坡。没有红绿灯。只有江风、城市天际线，和其他跑者。

<div style="display:flex;gap:12px;margin:18px 0;flex-wrap:wrap">
  <div style="flex:1;min-width:240px">
    <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-hangang1.webp" alt="汉江公园的骑行跑步道——前景一对情侣坐在长椅上，江边有骑行者和行人，对岸是首尔天际线" width="650" height="867" sizes="(max-width: 520px) 100vw, 330px" style="width:100%;border-radius:8px;display:block" />
    <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:6px;text-align:center">汝矣岛汉江公园——跑者最爱的一段。平坦、宽阔、无车、24 小时开放。</p>
  </div>
  <div style="flex:1;min-width:240px">
    <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-hangang2.webp" alt="汉江公园两旁绿树的跑道——江边步道上有一名慢跑者和一名骑行者，对岸是公寓高楼" width="560" height="700" sizes="(max-width: 520px) 100vw, 330px" style="width:100%;border-radius:8px;display:block" />
    <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:6px;text-align:center">江边步道向两个方向延伸好几公里——本地人日常跑步、骑行和清晨散步都在这里。</p>
  </div>
</div>

**热门跑步路线：**

- **汝矣岛 5K 环线**——从汝矣渡口站（5 号线，2 号出口）出发／结束。绕汝矣岛一圈的平坦环线，经过国会和 63 大厦。最适合新手。
- **汝矣岛"红薯" 8K**——从汝矣渡口延伸出去的环线，GPS 轨迹长得像一只红薯。从 **Runner Station**（在汝矣渡口站内）出发——那里有淋浴、储物柜和更衣室。
- **盘浦 5K 段**——在铜雀桥和东湖桥之间，南岸。经高速巴士客运站（3／7／9 号线）进入。安静、有风景，适合傍晚跑（盘浦大桥的彩虹喷泉入夜后亮起）。
- **纛岛**——7 号线，纛岛游园地站。连着首尔林。本地、人少。

==g:最佳时段：清晨（6–8 点）或傍晚（6–8 点）。== 步道夜间照明良好。多数公园入口的自行车租赁 ₩3,000–5,000／小时。

---

### 汉江拉面（한강라면）——唯一算数的赛后一餐

还有一个没人告诉跑者、但人人都记得的部分：==**한강라면（汉江拉面）**==。

你走进公园里的 GS25、CU 或 7-Eleven。买一包泡面（辛拉面、Buldak、真拉面——你说了算）。店员给你一个 ==g:带条形码的特制纸碗==。你把它拿到外面的拉面煮面机，扫条形码，按启动。机器会自动量好并倒入恰好加热的水。四分钟后，打一个蛋进去、搅一搅，端着碗走到江边的长椅上。

就这么简单。这就是那个在韩国社交媒体上被发过几百万次、如今专门把外国游客吸引来照做一遍的体验。

<div style="display:flex;gap:12px;margin:18px 0;flex-wrap:wrap">
  <div style="flex:1;min-width:200px">
    <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-ramen-machine.webp" alt="GS25 便利店里的汉江拉面煮面机——为纸碗泡面自动出热水" width="525" height="700" sizes="(max-width: 520px) 100vw, 330px" style="width:100%;height:280px;object-fit:cover;border-radius:8px;display:block" />
    <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:6px;text-align:center">公园便利店里的机器。扫你带条形码的碗，按启动——剩下的机器搞定。用机器免费。</p>
  </div>
  <div style="flex:1;min-width:200px">
    <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-ramen.webp" alt="一碗汉江拉面——纸碗里的辛拉面加一个蛋，在江边吃" width="700" height="525" sizes="(max-width: 520px) 100vw, 330px" style="width:100%;height:280px;object-fit:cover;border-radius:8px;display:block" />
    <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:6px;text-align:center">成品。热腾腾的辛拉面加蛋，在汉江边吃。总花费 ₩4,000–5,000（约 $3）。可能是首尔最好的赛后一餐。</p>
  </div>
</div>

**要花多少钱：**

| 项目 | 价格（KRW） | 约 USD |
|:---|:---:|:---:|
| 泡面 + 条形码纸碗（在 GS25／CU／7-Eleven） | ₩3,500–4,000 | ~$2.60–3.00 |
| 生鸡蛋 | ₩500 | ~$0.40 |
| 泡菜一包 | ₩1,500 | ~$1.10 |
| 野餐垫租借 | ₩3,000–5,000 | ~$2.20–3.70 |

:::note[本文里的美元数字是在不同时间做的粗略换算——上面的 APT 济州主赛事买入用的是约 ₩1,500/$，这张表用的是约 ₩1,350/$。做预算前请先查一下当前汇率。]:::

机器本身 ==g:免费使用——你只需为泡面付钱==。说明用韩英双语贴出。多数公园还设有外卖区，你想吃炸鸡也行。

**吃汉江拉面最好的几个公园：**
- **汝矣岛**（여의도）——新手之选，设施最全，离 Runner Station 最近。5 号线汝矣渡口，2 号出口。
- **盘浦**（반포）——傍晚之选，入夜后有彩虹喷泉大桥表演。3／7／9 号线高速巴士客运站。
- **纛岛**（뚝섬）——最安静，本地氛围。7 号线纛岛游园地。

**从 Paradise City 到汉江公园：** 免费摆渡 → 1 号航站楼 → AREX 普通车（约 60 分钟）到首尔市中心任一站，然后挑你的公园：

- **汝矣岛**——汝矣渡口站（5 号线，2 号出口）· 场内 Runner Station 有淋浴／储物柜
- **盘浦**——高速巴士客运站（3／7／9 号线）· 步行 5 分钟 · 夜间彩虹喷泉
- **二村**（이촌）——二村站（4 号线／京义中央线）· 更安静，本地人为主
- **纛岛**——纛岛游园地站（7 号线）· 连着首尔林

==g:每个汉江公园都有自己的 GS25、CU 或 7-Eleven，配着煮面机——你不必只盯着一个点。== 挑你在首尔当下最近的那个公园就行。

---

## 马拉松赛程参考

> **快速回答**
> 完整的韩国马拉松赛事赛历（所有距离、报名链接和日期）看 marathon.pe.kr。每年的日期都会挪，赛事也会增减——机票下单之前，以官方页面为准，本文的赛程表只当路线图。2027 年的春川窗口通常在 7 月；JTBC 的海外报名窗口每年不同，以官方页面为准。

想看最新的完整韩国马拉松赛程（所有距离、报名链接和日期）：[marathon.pe.kr](http://marathon.pe.kr/index_calendar.html)

下一年的具体日期，通常在前一年冬天陆续公布。

---

:::readnext[继续读]
/zh/blog/holdem-tournament | 德州扑克锦标赛指南 | /images/holdem-tournament-hero.webp
/zh/blog/apt-incheon-2026-guide | 2026 APT Incheon 指南 | /images/apt-incheon-2026-guide-hero.webp
:::

## FAQ

**Q. 跑韩国马拉松需要会说韩语吗？**

A. 不需要。春川和 JTBC 马拉松都有专门给外国跑者的英文报名通道。领号码布、赛道指示牌、终点线的流程都对外国人友好。剩下的事 Google 翻译就能搞定。

**Q. 我能在同一趟行程里既跑春川（10 月 25 日）又跑 JTBC 首尔（11 月 1 日）吗？**

A. 从赛历上看能——它们相隔一周。春川坐火车离首尔约 80 分钟。周日跑春川，周一到周六恢复，隔周周日跑 JTBC。很多韩国跑者就是这样背靠背连跑两周。2026 年注意：春川名额已售罄，所以这套完整组合实际上是 2027 年的计划。多留些恢复时间；春川赛道有坡。

**Q. 如果我马拉松后周日抵达，Paradise City Casino 开着吗？**

A. 开。Paradise City Casino 24/7 运营。国际客人周日赛后办理入住是常态。场内的水疗和桑拿在马拉松跑者里尤其受欢迎，很多人会先恢复一下再打晚上的现金局。

**Q. 外国人没有韩国地址能报名春川马拉松吗？**

A. 能——流程每年都一样（2026 年名额已售罄；下一个窗口约在 2027 年 7 月开启）。chuncheonmarathon.com 上的外国人报名标签就是给海外参赛者设计的。你需要护照号码、邮箱和付款方式。参赛包在赛前于赛事现场发放。

**Q. Garmin Run 场地和 K-pop 有什么关系？**

A. 高阳综合运动场是韩国最大的演唱会体育场之一——BTS、BLACKPINK 以及多数主流 K-pop 艺人都在那里办京畿道场演唱会。对既追 K 文化又跑步的人来说，体育场本身就是吸引力的一部分。比赛和这个场地的名气是两回事，但它们共享同一批观众。

---

## 总结一句话

> **快速回答**
> 韩国有东亚最好的秋季跑步条件、一座离机场 10 分钟的赌场，和一座非跑步时间同样值回票价的城市。三个日期——10 月 25 日春川、11 月 1 日 JTBC、11 月 15 日 Garmin；赌场全年开放。把 2026 当模板，2027 的报名窗口从 7 月开始。

一趟行程，两样都占。

那群手里既攥着扑克 bankroll、又记着一本马拉松训练日志的人，是真实存在的。BTS 的 Suga 没有发明这个重叠——他只是让它被看见了。

值得记下的日期：==**春川 10 月 25 日**==、==**JTBC 首尔 11 月 1 日**==、Garmin Run 11 月 15 日——2026 年的报名窗口都已关闭，照 2027 年的模板记。Paradise City 全年开放。

想看完整的扑克锦标赛情况（APT Incheon，$2.7M GTD，8 月 7–16 日）：[2026 APT Incheon 指南](/zh/blog/apt-incheon-2026-guide)。想了解锦标赛与现金局的策略差异：[锦标赛 vs 现金局](/zh/blog/holdem-tournament-vs-cash-game)。

---

## 相关文章

<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/zh/blog/apt-incheon-2026-guide" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">锦标赛</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">2026 APT Incheon 指南</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">8 月 7–16 日，Paradise City Casino · $2.7M GTD</div>
  </a>
  <a href="/zh/blog/holdem-tournament" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">基础</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">扑克锦标赛怎么打</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">买入、盲注、卫星赛和新手清单</div>
  </a>
  <a href="/zh/blog/wpt-australia-2026-guide" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">亚太</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">2026 WPT Australia 指南</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">The Star Sydney — 9 月 10–30 日</div>
  </a>
</div>
`.trim(),
};
