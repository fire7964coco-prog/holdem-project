...
08-13에 `/hands`를 **색인 개시**하고 같은 날 압축·경화·적대검수 4렌즈까지 마쳤다
(근거는 `app/hands/page.tsx` 주석 + WORKLOG 08-13 (1)(2)).

🔴 **「홀덤 족보 순위」만 보면 판정이 안 된다** — 그 쿼리는 실볼륨이 **390**뿐이다.
두 페이지가 실제로 붙는 곳은 **헤드텀**이므로 세 개를 따로 재라:
...
**판정 규칙**: 같은 쿼리에서 **두 URL이 번갈아 노출되면(URL 스와핑) 그게 카니발의 결정적 지문**이다.
각자 다른 쿼리를 가져가면 역할 분리 성공. **필라 노출이 떨어지면 되돌린다.**
hand-chart도 같은 시한(09-05경)이 걸려 있다.
