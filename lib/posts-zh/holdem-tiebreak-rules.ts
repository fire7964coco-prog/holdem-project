...
这个问题老有人问，是因为花色排序在扑克里==确实存在==——只是不在这个游戏里。梭哈（stud）和 razz 用它来决定谁先强制下注（bring-in）、以及那枚分不开的筹码归谁。德州扑克这两处都不用（唯一沾边的是开局抽牌定按钮位，那选的是座位不是牌——见下方常见问题）。
...
A. 先比大对，再比小对，最后比那唯一一张踢脚牌——所以 A 带 3 赢过 K 带 Q，哪怕第二对小得多。让人栽跟头的是像 K-K-9-9-5 这样的双对牌面：除非有人拿着 K、9、口袋 5-5，或比 9 大的口袋对子，否则每个人都是同样的两对，整手牌就坍缩成一张踢脚牌，全桌最大的那张底牌拿走底池——要是没人的底牌能压过牌面上的 5，那就全员打公共牌，平分底池。两对正好只带一张踢脚牌，绝不会有两张。
...
A. 不会——但德州扑克牌桌上确实有一个用到花色的地方，只是跟底池无关。现金桌（以及多数牌室的场规）在牌局开始时每人抽一张牌决定庄家按钮放在哪，两张抽到的牌点数相同时，就靠花色顺序定。（WSOP 锦标赛不抽牌：==锦标赛规则第 85 条==规定按钮从荷官右手边第一个筹码堆开始，只在剩 3、2、1 桌时才重新抽签——和花色无关。）不管哪种，那选的都是一个*座位*，从来不是一手牌。锦标赛规则书里唯一用到花色排序的地方，全都属于梭哈和 razz。两手最佳五张点数一张对一张全相同，无论花色如何都平分底池。
