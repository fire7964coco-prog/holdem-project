...
### ▶▶ 🔴 이 세션의 본체 = **B6 통화 전환** (축 B의 마지막 · 글별 판정 필요)

> **8/13에 네 트랙이 종료됐다.** 검수장 조치목록 **순위1~5**(남은 건 🔒8/16 순위6뿐) ·
> **구조화 데이터 66페이지** · **§6 축 B의 B1·B2·B5**(`ab991c3`) · **B3 족보 PDF**(`a8cb5ce`).
>
> 남은 건 **B6 통화**뿐인데 **기계적 치환이 아니다** — 아래 「§6 축 B」 절에 상세가 있다.
> 🔴 지시서의 「2편」이 **실측 9편**으로 깨졌고, 그중 일부는 **바꾸면 사실 왜곡**이다
> (`rake` 캡은 [PS] 1차 출처의 실제 USD · 이벤트 가이드는 실제 바이인 통화).
> **예시 금액만 골라내는 글별 판정**이 필요하다.
>
> 🪶 축 B가 끝나면 남는 큰 트랙은 **🔒8/16 묶음(4건)** 과 **1차 확인 필요 잔여 3건**이다.
>
