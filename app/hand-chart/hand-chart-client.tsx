    ...
    <>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 space-y-10">
        ...
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground text-center font-medium tracking-wide uppercase">
        ...
        <div className="overflow-x-auto">
          <div className="inline-block rounded-xl border border-border/50 overflow-hidden shadow-2xl">
            {/* Column headers */}
            <div className="flex">
              ...
              {RANKS.map((r) => (
                <div
                  key={r}
                  className="w-9 h-7 md:w-11 md:h-9 shrink-0 flex items-center justify-center text-[10px] md:text-xs font-bold text-muted-foreground bg-card/60 border-b border-border/40"
                >
                  {r}
                </div>
              ))}
            ...
            {RANKS.map((rowRank, row) => (
              <div key={row} className="flex">
                {/* Row header */}
                <div className="w-7 md:w-9 shrink-0 flex items-center justify-center text-[10px] md:text-xs font-bold text-muted-foreground bg-card/60 border-r border-border/40">
                  {rowRank}
                </div>
                ...
                {RANKS.map((_, col) => {
                  ...
                  let opacity = 1;
                  let bgColor = TIER_COLORS[tier] || "#1a1a1a";
                  let textColor = tier === 0 ? "#333" : "#fff";
        ...
        {/* Color Legend */}
        <div className="rounded-xl border border-border/50 p-5 bg-card/30 space-y-3">
          <h2 className="text-sm font-semibold text-foreground uppercase tracking-wider">색상 범례</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {POSITIONS.map((pos) => (
              <div key={pos.id} className="flex items-center gap-2">
                <div
                  className="w-8 h-8 rounded-md shrink-0 flex items-center justify-center text-white font-bold text-xs shadow"
                  style={{ backgroundColor: pos.color }}
                >
                  {pos.label}
                </div>
                <div className="text-xs leading-tight">
                  <div className="font-medium text-foreground">{pos.full}</div>
                  <div className="text-muted-foreground">{pos.pct}</div>
                </div>
              </div>
            ))}
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-md shrink-0 bg-[#1a1a1a] border border-border" />
              <div className="text-xs leading-tight">
                <div className="font-medium text-foreground">폴드</div>
                ...
                {POSITIONS.map((pos, idx) => {
                  ...
                  return (
                    <tr key={pos.id} className={`border-b border-border/30 ${idx % 2 === 0 ? "bg-card/20" : ""}`}>
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{pos.pct}</td>
        ...
        <section className="space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[
              ...
              <div key={item.title} className="rounded-xl border border-border/40 bg-card/30 p-5 space-y-2">
        ...
        <section className="space-y-3">
          ...
          {[
            ...
            >
              <summary className="flex justify-between items-center px-5 py-4 cursor-pointer list-none text-foreground font-medium hover:text-primary transition-colors">
                {item.q}
