...
| 🆕 🔴 **EN 필라 2편의 구글 canonical이 외부 도박 사이트 `747live.bet`로 잡혀 있다** — `/en/blog/holdem-pot-odds` · `/en/blog/holdem-reading-the-board` | 라이브 DOM 실측 결과 **우리 쪽은 정상**(자기참조 canonical · `index, follow` · hreflang 9) → 코드 결함이 아니라 **구글이 우리 글을 그쪽 복제본으로 판정**한 것(스크래핑 정황). 🔴 **콘텐츠 작업으로 못 고친다 — 별건이다.** 후보 조치: 내부링크 강화 · 재색인 요청 · (판단 필요) DMCA. 사장님 판단 대기 |
