...
| 무엇 | 막힌 지점 · 해소 조건 |
|---|---|
| **TMT 19 편입**(01.22~02.09) | 🔴 **베뉴·도시 미확정 하나 때문에 보류했다.** CTP 자체 페이지(`/festivals/tmt-19`)는 **날짜만** 있고 Schedule·Results·Information이 전부 «No data», somuchpoker는 **`* multiple venues *`**(162이벤트·32,875엔트리·40개국·GTD 60M TWD — TMT 20보다 큰 판이다). **해소 조건 = 베뉴 1차 확인 하나.** `archive.ctpclub.app`을 아직 안 열었다 |
| **Harbour Poker Cup 2026-1**(06.17~21) | `hpc.poker`가 **현재 판(9.30~10.04)만** 띄우고 6월분 페이지가 없다 → 1차 부재. CTP 목록에만 남아 있다. 🪶 이 사이트는 «현재 판만» 띄우는 구조라 **다음 판이 열리면 9월분도 사라진다** — 지금 값은 라이브 확인분이다 |
...
| 🟠 **`verifiedAt`이 07-29에 멈춘 TW 외 대회들** | 이번에 TW만 재대조했는데 **12건 중 2건이 결함**이었다(취소·날짜). 같은 밀도면 다른 나라에도 있다. 🔴 **가장 값싼 다음 수 = KR 17건·US 21건을 같은 방법으로**(주최사 목록 DOM 긁어 한 줄씩 대조). 방법은 이번 회차가 확립했다 |
