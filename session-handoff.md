...
> 🧰🔴 **2026-09-11 (11) 신설 — 헤드 잔여 작업은 `Holdem-queue` 레인(`harden-queue`)이 회차로 한다.** 정본 = `docs/hardening-protocol.md` **§7-F** · 회차 표 = **`docs/harden-queue-진행.md` §1**(Q1 결재 소품 · Q2 사진 5장 · ~~Q3~~ EN `updated` 전수 = 실측 종결 · Q4·Q5 EN-먼저 · Q6 로케일 판정 묶음 · Q7 게이트 신설 · Q8 용어 분열 · Q9 = 검수장 몫 · **합계 12~13세션 · 모델·세션은 표의 열**). **아래 절·미결의 항목은 그 표의 «재료»다 — 여기서 지우지 말고 회차가 닫히면 헤드가 지운다.** 헤드(이 창)는 머지·빌드·배포·우편함·핸드오프·정본 승격만. 창 띄우기 = §1-B 「🧰 queue 창」 · 첫 마디 「`HARDEN.md` 읽고 회차 Q1 시작해」.
...
| **MB-032** | 🆕 ja 회차 10 머지·배포 통지(`4291087c`) — 회귀 앵커 갱신(ja 직답 25자리/15편) · 게이트 LEN 로케일 분리(zh·zh-hant 수치 68/131→137/53 = 규격 변경이지 결함 증감 아님) · EN-먼저 J#6·J#7 «예고»(앵커 지금 걸지 말 것) · 요청 0 | 검수장 |
...
**레인 상태(여기만 유지)**: zh **42/42 완주** · zh-hant **42/42 완주**(회차 7 소급까지) · ja **회차 1~12 머지 완료**(회차 12 = ff `d847d2e1`) · ja **회차 13 진행 중**(미커밋) · 🧰 **queue 회차 0**(09-11 신설 · 회차 표 `docs/harden-queue-진행.md`).
