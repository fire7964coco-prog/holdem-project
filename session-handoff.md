...
### 🔴 스탬프 정본 정책 — 사장님 결재 대기 (실측·판정은 끝났다)

사장님 축어: *「현재 페이지들 보면 업데이트날짜가 전부 9/9 일로 되어있는데, 어제날짜거든? 어제까지 검수폴더에서
검수와 경화작업을 같이 했었는데 업데이트가 전체페이지로 다 덮어진건 아닌지 확인해봐」*

**🟢 데이터 사고는 없었다 — 두 겹으로 확인 끝.**
① 09-09 구간 커밋 72개 전수 분해 = 내용까지 바뀐 파일 **311편** / 스탬프만 바뀐 파일 **1편**.
② 그 1편(`lib/posts-de/holdem-positions.ts`)도 **09-10 (5) 회차에 오탐으로 판정됐다** —
de는 EN보다 **먼저**(2026-08-24 `fc404ca8` · M-041 원장) 같은 사실을 더 자세히 고쳐 놨었다.
🔴 **이 건을 다시 열지 마라.** 경위 = WORKLOG 「2026-09-10 (5)」 ①.

**🟠 남는 것은 «정책 판정» 하나다** — 이건 실측으로 못 정하고 사장님이 정해야 한다.
판정할 것 셋: ① 회차 `updated`는 **착수일인가 마감일인가** ② **전파본이 원본(EN) 날짜를 쓸 것인가 전파일을 쓸 것인가**
③ 한 회차가 300편을 만질 때 스탬프를 **분산할 것인가**. 정하면 프로토콜 §5-⑧에 한 줄.
🪶 판정이 걸려 있어 **손대지 않고 남긴 자리 2곳**: de `holdem-positions`(`updated` 08-17인데 실제 편집은 08-24 = 스탬프가 «과소») ·
de `holdem-position-play`(미결 절 참조).

**🪶 재현 명령**(다시 재고 싶을 때 — 커밋 구간만 바꾸면 된다)
...
🔴 **다만 «스탬프만 바뀐 파일»이 곧 결함은 아니다**(위 de 오탐이 그 실증) — 게이트를 만들면
«그 로케일이 그 사실을 이미 갖고 있나»를 사람이 판정하는 단계를 반드시 붙여라.
