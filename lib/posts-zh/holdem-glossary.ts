...
| **下注（Bet） vs 加注（Raise）** | 这一轮**第一个**投筹码叫下注；前面已经有人下了、你再往上加才叫加注。**轮到你时的口头声明是有约束力的，而且先说的算**——什么都不说只推筹码，推的钱不到上一个下注额的一半就只算跟注；到了一半却不够最小加注，则会被要求补足成一次完整的最小加注。 |
...
> **快速回答**
> 轮到你时的基本动作只有 5 个：过牌、下注、跟注、加注、弃牌。all in 是把筹码一次推光，它可以是下注、跟注也可以是加注；3bet、4bet、c-bet 则是加注在不同时机的叫法——名字再多，底层动作还是这 5 个。
