...
const COPY = {
  ...
  /**
   * ★2026-08-24 — `/zh-hant/solver` 신설과 함께 추가(8번째 = 마지막). 표기는 앱 번체 축어
   *   («GTO 解算器» · «打開瀏覽器就能算» · «不用裝軟體»)와 랜딩 훅(«免下載·免註冊»)에서 왔다.
   * 🔴 **solver의 번체 라벨은 «解算器»다 — 간체(求解器)를 자형만 바꿔 복사하지 마라**
   *   (앱 `?lang=zh-hant` title 실측 · 용어집 §7-C 일치).
   * 🔴 **`GTO`도 `solver`도 단독으로 두지 않는다** — 오염 규율은 간체와 동일(뱅크 §1-①·②).
   * 🔴 차별화어는 «무료»가 아니라 **«免下載 · 免註冊»**다 — 중화권 무료 축의 실질 경쟁자
   *   (TexasSolver 등)가 전부 설치형이라 SERP 전체와 갈린다.
   */
  "zh-hant": {
    href: "/zh-hant/solver",
