...
### 🔴 스탬프 드리프트 25로케일 223편 — 착수 지시 대기 (2026-09-10 (7) 발견)

> 🔴 **정책은 결재돼 정본에 있다**(`docs/settled-decisions.md` **§1-C** · 절차 `hardening-protocol` §5-⑦-B). **재론 금지.**
> 여기 남는 것은 «정책을 과거분에 적용한 적이 없다»는 **실행 부채**다 — §1-C를 뒤집는 게 아니다.

🔴 **본체 = `docs/stamp-drift-audit-2026-09-10.md`**(커밋 55개 전건 판정표 · 재현 절차). 요지 셋만:
① **BEHIND 223편은 «후보»이지 결함이 아니다** — 판정 단위는 **파일이 아니라 커밋**이다.
② 최대 덩어리 `8d2aba44`(08-18 HSL 토큰) **104편은 결함 아님**(순수 CSS 수리) · 실제 대상은 **09-10 묶음 ~106편 + 08-12 미러 10커밋 × 16편**(소수 언어).
③ 🔴 **착수 전 판정 필요** — 106편을 올리면 sitemap `lastmod` 106개가 동시에 움직인다(대외 조치). **일괄 처리 금지.**
