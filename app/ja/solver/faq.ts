...
export const SOLVER_FAQ_JA: FaqItem[] = [
  ...
  {
    q: "このGTOツールは本当に無料ですか?",
    a: "無料です。全機能に回数制限がなく、支払い方法の登録もロックされた機能もありません。アカウントも不要で、ログインは任意です。ログインすると学習スポット・今日の問題の履歴を端末間で同期できます。自分で計算したスポットとその練習履歴は、ログインしてもこの端末だけに残ります。オープンソースのWASM Postflop（AGPL-3.0）を土台にしており、HoldemMasterが改良したソースコードも同じライセンスで公開しています。",
  },
  {
    q: "有料プランや課金はありますか?",
  ...
  {
    q: "画面は日本語に対応していますか?",
    a: "対応しています。メニュー、5つの入力ステップ、結果画面の項目名、学習スポットの解説文、GTOトレーナーの採点表示まで日本語です。ソルバーは用語が多く、英語のままだと「Equity Realization」や「Exploitability」で手が止まりがちですが、日本語表示ならそこでつまずきません。",
  },
  {
    q: "ブラウザで動くソルバーの精度はどのくらいですか?",
    a: "目標エクスプロイタビリティを自分で設定し、その目標に向けて反復計算します。目標を下げるほど計算時間がかかり、反復回数の上限で終了する場合もあるため、計算後のエクスプロイタビリティを確認してください。結果は入力したレンジとツリーの条件に対するものです。WebAssemblyエンジンが使えるメモリは約4GBなので、極端に大きなツリーはデスクトップソルバーの方が向いています。",
  },
  {
  ...
  {
    q: "GTOトレーナーとは何ですか?",
    a: "計算済みの学習スポットや自分で計算したスポットを使った練習モードです。実際の意思決定ポイントでハンドが配られ、アクションを選ぶと採点されます。配られるハンドは実際のGTOレンジのウェイトに比例するので、「そのスポットで本当に持ちうる頻度」でその手が回ってきます。シングルレイズポット、3ベットポット、ブラインド戦を絞って練習することもできます。",
  },
  {
  ...
  {
    q: "学習の記録はどこに保存されますか?",
    a: "初期状態では端末内に保存され、アカウントは不要です。HoldemMasterのアカウントでログインすると、学習スポット・今日の問題の履歴をアカウントに保管して、別の端末でも続きから練習できます。自分で計算したスポットとその練習履歴は、ログインしてもこの端末だけに残り、ほかの端末には同期されません。連続正解の記録、シナリオ別の苦手分野の内訳、EVロスが大きかった問題を集めた復習キューは、それぞれの履歴をもとに動いています。",
  },
  {
