...
export const POST: Post = {
  ...
  desc: "名字跟着按钮走，不跟着椅子走。UTG、劫持位、CO、按钮位每个位置名称，加上座位编号、6人桌图解，还有谁先行动。",
  ...
  content: `
我第一次打现场现金局，坐的那个位置后来我才知道叫 UTG。我低头一看是 J♥ J♠，加注。劫持位跟了。CO 跟了。按钮位跟了。大盲 3bet。我完全不知道该怎么办 —— 我 call 了，然后连续三条街一路流血送筹码。
...
> **快速回答**
> 德州扑克的位置是==以庄家按钮为基准来命名的座位== —— UTG（枪口位）、lojack、劫持位、CO（关煞位）、按钮位、小盲位、大盲位 —— 随着按钮移动，它们==每手牌都顺时针移动一个座位==。翻前 UTG 最先行动、大盲最后行动。翻牌后小盲最先行动、按钮位最后行动。
...
你还会看到更宽泛的区域标签：==**EP**（前位）==涵盖 UTG 那几个座位，==**MP**（中位）==涵盖 lojack 和劫持位，==**LP**（后位）==涵盖 CO 和按钮位。老一点的书把 lojack 和劫持位归成「MP1/MP2」—— 同样的座位，不同的标签。
...
位置正好相反 —— 它们随着按钮==每一手都顺时针移动一个座位==。5 号座这一手可能是按钮位，下一手是 CO，再下一手是劫持位。

:::compare
座位编号（物理） | 位置（扑克）
钉死在椅子上 —— 1 号座通常是庄家正左边 | 每手牌跟着庄家按钮移动
工作人员用：「5 号座，筹码来了」 | 策略上用：「CO 开池」
...
## 劫持位和 lojack —— 以及它们为什么叫这名字

**劫持位（HJ）**是按钮右边第二个座位。**Lojack（LJ）**再往前一个，是按钮右边第三个座位。它们俩合起来构成了现代 9 人局里的中位。

这些名字没有任何有据可查的官方来源 —— 扑克黑话很少有 —— 但流传最广的说法是这样的：

- **劫持位：** CO 和按钮位是经典的偷盲座位。当前面一个座位的玩家先加注时，他就==**「劫」（hijack）走了**==那些后位座位本来等着做的偷盲 —— 于是这个座位就得了这个名字。
- **Lojack：** 是后来才有的，==对「hijack」的一个玩笑式变体== —— 在座次里「低」一档的座位。多数说法里还能听出防盗品牌 LoJack 的谐音：一个劫持位，往下挪一档。

这两个都当作牌桌传说听听就好，别当词源学。不是传说的是：劫持位和 lojack 是真实、标准的名称，你在每一张现代范围图和每一个训练网站上都会看到，所以值得把它们记得滚瓜烂熟。

---

## CO（关煞位）和按钮位（庄家位）

**CO（关煞位）**是==按钮右边第一个座位== —— 庄家前面的最后一个位置。有两个来源说法在流传：一个说这个座位「切断」（cut off）了按钮先加注偷盲的机会；一个更老的说法是，在自己发牌的家庭局里，庄家右边的玩家会在洗牌后==切牌==（cut the deck）。不管哪种，名字就这么定下来了，CO 普遍被算作后位。
...
位置名称不随桌子大小改变 —— 随着玩家被移走，==它们是从前位先剥掉的==。按钮、盲位、CO 和劫持位撑得最久；UTG 那些座位只有在满桌时才存在。下面是从 2 人到 10 人的地图，按翻前行动顺序列出：
...
**6 人桌 vs 满员桌**纯粹就是做减法：那三个多出来的座位（UTG+1、UTG+2 和一个中位座位）不存在，所以 6 人桌跑的是 UTG → HJ → CO → BTN → SB → BB。实际后果并不是某个座位打得「更后」——关煞位在两种桌型里身后都是同样的三个人。而是==少了前位的那几个座位，你坐到盲注位和后位的频率高得多，在你前面开牌的人也更少== —— 6 人桌的 UTG 面对五个对手，而不是八个 —— 这就是为什么在短手局里各个位置的范围都会放宽。逐座位的数字在[位置策略指南](/zh/blog/holdem-position-play)里，填进每个范围的具体牌型则在[起手牌图表](/zh/blog/holdem-starting-hands-chart)里映射好了。
...
A. 劫持位（HJ）是庄家按钮右边第二个座位，就在 CO 前面。它是 9 人局里两个中位座位中较靠后的那个，在 6 人桌里则是翻前第二个行动的座位。名字背后流传最广的说法是：从这个座位加注会「劫」走 CO 和按钮位本来占位准备做的偷盲。
...
A. 六个：UTG（也叫 lojack）、劫持位、CO（关煞位）、按钮位、小盲位和大盲位。跟 9 人桌相比，UTG+1、UTG+2 和一个中位座位干脆就不存在 —— 名字是从前位先被移走的。每个座位身后剩下的人数跟满员桌的同名座位一样，但少了前位的那几个座位，你坐到盲注位和后位的频率高得多，所以平均范围更宽。
...
A. 现场里「几号座」（1 号座、5 号座）说的是钉在椅子上的物理编号，跟位置无关。牌友口语说的「前面位」「后面位」才是位置：靠前的是 UTG 那几个「前位」座位，靠后的是 CO、按钮那种「后位」座位。同一把椅子这手是按钮、下手就成了 CO —— 判断该轮到谁行动，要看座位离庄家按钮多远，而不是它的编号。
