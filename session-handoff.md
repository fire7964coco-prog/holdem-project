...
## ▶▶▶▶▶▶▶ 새 세션 START HERE (2026-07-27 새벽 — **Wave 1 9/12** → 다음 액션 = **NV/베가스 글 + 위성글 2편**)

> ### 지금 상태 한 줄
> es-US Wave 1이 **9편/12편**. 남은 건 **#10 NV(라스베이거스)**, **#11 TX 위성**(card house 멤버십 심화), **#12 NJ 위성**(VPN·지오로케이션). 발행 57 blog + **457 intl**.

**✅ 이번 세션(2026-07-27 새벽)에 끝낸 것**
- **Fable5 7편 통합감사 반영 37건**. 최대 건 = ★**"텍사스에 카지노 0개"가 사실오류** → Kickapoo Lucky Eagle 등 **부족 카지노 실재**(연방법·21+). "상업 카지노 0개"로 정정 + TX 글에 전용 섹션(포커룸 여부는 **미확인 명시**). NJ 브랜드 수 4곳 통일, 헬프라인 **1-800-MY-RESET** 우선 7편 통일, `crap`→`craps`, 스페인 티 5건.
- **허브 A↔B 카니발 분리**: A=50개주 합법성 지도 / B=계정·앱·결제·세금. A에서 MSIGA 운영사표·주별 표·3규칙·SSN/30일/VPN FAQ 제거 → B 일원화.
- **신규 2편 작성 완료**(검수 결과 반영 후 푸시): `/es/blog/poker-en-arizona-y-colorado`, `/es/blog/poker-en-chicago`. 히어로 인포그래픽 2장 + 본문 1장(전부 HTML+Playwright, 로고+URL 워터마크, 각 ~31KB).

**★★이번 세션 최대 교훈 — 심판이 검수자를 뒤집은 게 2번째다**
- Fable5가 "루이지애나는 sweeps 금지 아님 / 네바다는 금지함"이라 지적 → **원문 판독 결과 둘 다 오판**, 기존 서술이 옳았음.
  - **LA는 금지 맞음**: HB 883→**Act 182**(2026-05-15). 법문에 "sweepstakes" **단어가 없고** *"dual-currency system"*으로 포섭 → **단어검색만 하면 반드시 오판한다.**
  - **NV는 금지 아님**: 검수자가 근거로 댄 **AB 380은 정신건강 위기대응팀(988) 법안**. 완전 무관.
- → `docs/us-poker-primary-sources.md` §6에 **재논쟁 방지 기록** 박아둠. 같은 지적 또 오면 그걸로 종결.
- **원칙 재확인: 검수 에이전트 지적을 원문 확인 없이 반영하지 마라.** 이 트랙에서 검수자 오지적 누적 **5건**.

**⚠️ 기존 문서의 사실오류 정정(중요)**
- 이 파일 아래 옛 §13 경고의 "**AZ 18/21 이중구조**"는 **틀렸다. 애리조나는 포커 포함 전 게임 21세**(A.R.S. §5-601(B)(1) + 2021 컴팩트 §3(w) + 카지노 공식 문구 3중 확인). 키워드뱅크도 정정 완료. **18세 예외는 FL(포커만)·MN·TX 클럽·일부 부족(Turning Stone 등)**.
...
**▶ 다음 액션(우선순위)**
1. **#10 NV/라스베이거스 글** — 1차출처 조사 진행 중이었음(WSOP 참가 외국인 요건·ITIN/30% 원천징수·조세조약·저가 데일리 토너·룸별 운영현황). 조사 결과 없으면 재실행.
2. **#11 TX 위성**(card house 멤버십 심화) / **#12 NJ 위성**(VPN·지오로케이션 차단). ※Fable5 판정: 주별 5편은 서로 카니발 없음. 위성글은 허브와 겹치지 않게 **각도 분리** 먼저 정할 것.
3. Wave 2(티어 A 16편) 착수 전에 **`docs/es-us-ranking-plan.md`** 재확인.

**⚠️ 남은 품질 부채**
- 주별 글 다수가 **본문 이미지 0~1장**(12분 분량). AZ+CO에만 본문 1장 추가함. 실사 사진은 사용자 Gemini 생성 필요, 인포그래픽은 HTML+Playwright로 자율 제작 가능(`scripts/gen-*.html` + `_shot.mjs` 패턴).
- 허브 A 페이지 이미지 총합 196KB — 200KB 경계. 형제 썸네일 늘면 재압축 필요.
