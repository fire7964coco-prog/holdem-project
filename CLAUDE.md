...
| 파일당 | 🔴 **상한을 걸지 마라 · quality 88** (2026-08-30 사장님 지시 — 옛 «≤60KB·q65»는 **폐기**). 근거 §9-2 |
...
🔴 **§9-2-A 인코딩 품질 = q88 · 용량 상한 금지** (2026-08-30 사장님 지시 축어: *「60KB를 강제하지마..
상황에 맞게 해야지...이미지가 선명하게 보여야 유저들이 포스팅을 이해하는데 도움이 되지..
조금 넘어도 괜찮아」*). **이 규칙을 어긴 회차가 같은 날 두 번 나왔다** — ①60KB 상한이 q40 강등을
만들었고 ②상한을 뺀 뒤에도 **q65 고정이 남아** 49장이 7~9% 작아졌다(세대 손실).
**판단은 렌더 경로 끝단에서 측정한다**: 모바일(750px)에서는 q65↔q88이 사실상 동일하지만
(축소가 아티팩트를 흡수), 히어로 `sizes`가 **1200px까지라 데스크톱은 축소 없이 받는다** →
그 경로에서 **최대 국소오차 40→24**로 개선되고 **전송량은 동일**하다. 디스크 1.5배는 전송 부담이 아니다.
도구 = `scripts/wm-fix.mjs`(q88) · 감사 = `scripts/wm-audit.mjs`.
