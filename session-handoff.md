...
### ▶ 0-❸. 2026-08-08 (3) — GTO 솔버 통합 · 13편 시리즈 ①편 발행 ⏳ (진행 중 · 12편 남음)

> **★다음 세션이 이어서 할 일. `docs/gto-solver-series-spec.md`만 읽으면 그대로 이어진다.**
> 커밋 `c00d2af`(랜딩) · `0624b67`(①편+캡처 26장). ⚠ 착수 전 `git pull`.
...
2. 🔴 **캡처 파일명의 `_ip`/`_oop`는 플레이어 구분이 아니다.** 픽셀 비교로 확인했다 —
   26장 전부 **OOP 화면**이고 IP 대응 캡처는 없다. "IP의 대응은 이렇습니다"로 쓰면 **§13 사실오류**다.
   「체크로 가는 핸드 vs 벳으로 가는 핸드」 액션 대비로 쓸 것. **매 편 이미지를 Read로 열어 확인.**
