...
```
1. git pull
2. docs/gto-solver-series-spec.md 통독           ← §3(캡처)·§4(키워드)·§5(체크리스트)
...
1. 🔴 **형제 글을 인용할 땐 «카드·무늬»도 원문에서 복사하라.** 이번에 수치는 전부 대조했는데
   보드 표기를 기억으로 적어 두 건 틀렸다 — ①은 `A♥7♦2♣`, ③은 `Q♠J♦T♠`(**투톤**)이다.
   무늬가 틀리면 「드라이/투톤」 판정까지 따라 틀린다. 적대검수 검산 렌즈가 단독으로 잡았다.
2. 🔴 **범위 서술("①~⑦에서 96~99.9% 체크")을 쓰기 전에 전 편 값을 실제로 훑어라.**
   ④는 76.2%, ⑤는 88.8%다. 세 렌즈가 동시에 잡은 만큼 눈에 잘 띄는 결함이다.
   확정된 값: 리드 벳 ① 1.9 · ② 0.2 · ③ 0.1 · ④ 23.7 · ⑤ 11.2 · ⑥ 3.0 · ⑦ 3.2 /
   OOP EQR ① 84.0 · ② 80.7 · ③ 77.9 · ④ 93.2 · ⑦ 84.3 · ⑧ 109.6 · ⑨ 117.8 · ⑩ 106.9 · ⑪ 103.1
3. **3벳팟 3편의 BB 레인지는 14종 고정**이고 콤보가 전부 정수로 맞는다(WORKLOG 표 참조).
   ⑫⑬은 블라인드전이라 레인지가 538/525콤보로 훨씬 넓다 — 콤보 분해를 억지로 시도하지 마라.
4. **⑫⑬ 키워드**: 「홀덤 포지션」 390 · 「홀덤 포지션별 핸드레인지」 70 · 「홀덤 트립스」 10 ·
   「홀덤 블러프」 10. ⑪이 「홀덤 포지션」을 이미 잡았으니 **⑫는 롱테일 쪽으로 갈라라.**
5. **seoTitle에 GTO 필수** · 「씨벳」 절대 금지(베팅 사이트 브랜드, 아래 참조).

---

### ▶ 0-❺. 2026-08-08 (5) — GTO 시리즈 ⑦~⑪ 5편 발행 ✅ (11/13)

> **build 67 blog posts** · 게이트 10편 🔴 0건 · canonical 0건 · FAQ 편당 4문항.
