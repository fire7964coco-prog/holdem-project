...
實戰口訣：==你是最後主動下注的人，就得翻牌——被跟到的詐唬也一樣。== 蓋牌不亮是跟注方的選擇，而且要在看過下注方的牌之後。自己下注卻搶著蓋牌，等於虧兩次：跟注方本來就能要求看你的牌；而且牌面說話，不少底池是被「以為輸了」的人丟掉的，其實他的 A 高還領先。
...
| 河牌上的全下被跟到 | 全下的人作為最後主動下注者先亮牌——但 ==r:這裡誰都不能蓋牌==：依 TDA 第 16 條，跟注方同樣必須攤牌 |
...
這條規則兩面都成立。如果你以為自己輸了、沒亮牌就直接蓋掉，但你那手牌其實才是贏家——==r:底池就飛了==。只有等荷官把牌推進蓋牌堆、再也認不出來時，這手牌才算死牌——只是蓋著放下並不會自動作廢（TDA 第 14 條）。話雖如此，永遠別把希望押在這上面。只要你不是 100% 確定自己輸，攤牌前一定先讓荷官把你的牌讀完再說。
