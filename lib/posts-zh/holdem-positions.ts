...
export const POST: Post = {
  ...
  tldr: "德州扑克的位置是以庄家按钮为基准来命名的座位 —— UTG（枪口位）、lojack、劫持位、CO（关煞位）、按钮位，还有两个盲位 —— 通常每手牌它们都顺时针移动一个座位。翻前 UTG 最先行动、大盲最后行动；翻牌后小盲最先行动、按钮位最后行动。物理座位编号永远不动，位置才会动。",
  category: "strategy",
  date: "2026-06-13",
  updated: "2026-09-26",
...
三手之后我坐到了按钮位，还是同样的 J♥ J♠。我加注。所有人都弃牌。我一个翻牌都没看到就赢了 $14。
...
> **快速回答**
> 德州扑克的位置是==以庄家按钮为基准来命名的座位== —— UTG（枪口位）、lojack、劫持位、CO（关煞位）、按钮位、小盲位、大盲位 —— 随着按钮移动，它们==通常每手牌都顺时针移动一个座位==。翻前 UTG 最先行动、大盲最后行动。翻牌后小盲最先行动、按钮位最后行动。
...
> **快速回答**
> 位置不是一把椅子，而是你相对于庄家按钮所在位置的名称，它决定了你在每条街的行动顺序。9 人桌的位置顺序是：UTG、UTG+1、UTG+2、LJ、HJ、CO、BTN、SB、BB —— 按钮通常每手牌顺时针移动一个座位，所以每个人每一手都换一个名字。

在正常的牌局里，按钮每手牌之后都顺时针移动一个座位，所以桌上每个玩家从这一手到下一手都戴着一个不同的名字。
...
> **快速回答**
> 德州扑克的位置缩写全是英文首字母：UTG、LJ、HJ、CO、BTN、SB、BB，按区域再归成 EP（前位）、MP（中位）、LP（后位）。牌桌上、教学文里、范围图上用的都是这套缩写，中文名反而少见 —— 所以先认缩写，中文名当注解记就行。
...
> **快速回答**
> 当值台员喊“12 号桌，5 号座”时，那个数字跟位置毫无关系 —— 座位卡上的编号钉死在椅子上，1 号座通常是庄家正左边；位置则跟着按钮通常每手牌顺时针挪一个座位。

现场牌局的头一晚，几乎每个人都要在这上面栽一次：把“几号座”当成了自己的位置。

在大多数扑克室里，物理座位是从庄家正左边开始编号的 —— ==1 号座位习惯上是庄家左边的第一把椅子==，顺时针数一圈到庄家右边的 9 号或 10 号座。这些数字是钉死在椅子上的。工作人员用它们来处理杂务：安排新玩家入座、送筹码、叫时间。

位置正好相反 —— 它们随着按钮==顺时针移动一个座位，通常每一手都挪==。5 号座这一手可能是按钮位，下一手是 CO，再下一手是劫持位。

:::compare
座位编号（物理） | 位置（扑克）
钉死在椅子上 —— 1 号座通常是庄家正左边 | 每手牌跟着庄家按钮移动
工作人员用：“5 号座，筹码来了” | 策略上用：“CO 开池”
一整个牌局中永不改变 | 通常每一手都变，顺时针挪一个座位
...
> **快速回答**
> 枪口位（UTG）指翻前第一个行动的座位，就在大盲左手边。满员 9 人局其实有三个前位座位 —— UTG、UTG+1、UTG+2 —— 但只有第一个是真正盲着开口的，+1、+2 只是名字上的枪口位，压力小得多。

这个名字点出了这个位置的压力：你必须在看到任何一个对手怎么做之前就投入筹码，就像被枪顶着行动一样。
...
> **快速回答**
> 劫持位（HJ）是按钮右边第二个座位，lojack（LJ）再往前一个，两者合起来是现代 9 人局的中位。这两个名字都没有可考的出处，只有牌桌上流传的说法 —— 当作行话记住位置就好。
...
> **快速回答**
> 关煞位（CO）是按钮右边第一个座位 —— 庄家前面的最后一个位置，普遍算作后位。按钮位（BTN）则是标着庄家圆盘的座位，也是这张桌子上一切命名的原点。

**CO（关煞位）**这个名字有两个来源说法在流传：一个说这个座位“切断”（cut off）了按钮先加注偷盲的机会；一个更老的说法是，在自己发牌的家庭局里，庄家右边的玩家会在洗牌后==切牌==（cut the deck）。不管哪种，名字就这么定下来了，CO 普遍被算作后位。
...
> **快速回答**
> 小盲位（SB）是按钮左边第一个座位，大盲位（BB）是第二个。作为位置，它们的顺序是反着的：翻前最后开口，翻牌之后却第一个开口 —— 这个翻转正是盲位难打的根源。
...
> **快速回答**
> 翻前的位置顺序从 UTG 开始，绕一圈到大盲收尾；翻牌、转牌、河牌则从按钮左边重新起算。所以答案是分两半的：盲位在翻前最后说话，翻牌之后却最先说话。

关于位置被问得最多的一个问题，用一张表回答：

| 街 | 最先行动 | 最后行动 |
|:---|:---|:---|
| **翻前** | **UTG** —— 大盲左边第一个座位 | **大盲** —— 没人加注时可以过牌或加注 |
...
> **快速回答**
> 位置名称不随桌子大小改变 —— 人数减少时，座位是从前位先剥掉的。6 人桌的位置就是 UTG（也叫 LJ）→ HJ → CO → BTN → SB → BB；单挑时按钮下小盲，翻前最先行动、翻牌后仍最后行动。

按钮、盲位、CO 和劫持位撑得最久；UTG+1、UTG+2 这些带编号的座位只有在满员的 full ring 桌上才存在。下面是从 2 人到 10 人的地图，按翻前行动顺序列出：
...
**6 人桌 vs 满员桌**纯粹就是做减法：前位那三个座位（UTG、UTG+1、UTG+2）整组消失、由 LJ 接手“UTG”这个名字，所以 6 人桌跑的是 UTG → HJ → CO → BTN → SB → BB。实际后果并不是某个座位打得“更后”——关煞位在两种桌型里身后都是同样的三个人。而是==少了前位的那几个座位，你坐到盲注位和后位的频率高得多，在你前面开牌的人也更少== —— 6 人桌的 UTG 面对五个对手，而不是八个 —— 这就是为什么在短手局里第一个座位开得更宽、你整体玩的手牌也更多，尽管关煞位的范围几乎不变。逐座位的数字在[位置策略指南](/zh/blog/holdem-position-play)里，填进每个范围的具体牌型则在[起手牌图表](/zh/blog/holdem-starting-hands-chart)里映射好了。
...
A. 小盲在每条街上都在大盲之前行动。翻前，两个盲位都最后行动（大盲是所有人里最后的那个——没人加注时，有过牌或加注的选择）；翻牌后，小盲是桌上第一个行动的座位。唯一的例外是单挑，那时按钮下小盲，大盲翻牌后最先行动。

**Q. 6 人桌德扑有几个位置？**

A. 六个：UTG（也叫 lojack）、劫持位、CO（关煞位）、按钮位、小盲位和大盲位。跟 9 人桌相比，是前位那三个座位（UTG、UTG+1、UTG+2）整组消失、由 LJ 接手“UTG”这个名字 —— 名字是从前位先被移走的，所以两个中位座位都还在。每个保住自己名字的座位（劫持位、CO、按钮位和两个盲位）身后剩下的人数跟满员桌的同名座位一样，但少了前位的那几个座位，你坐到盲注位和后位的频率高得多，所以平均范围更宽。

**Q. 德扑位置每手牌都会变吗？**

A. 会。庄家按钮每手牌之后顺时针移动一个座位，而既然所有位置都是按离按钮的距离命名的，每个玩家的位置通常每手牌都会挪一个座位。在人员稳定的牌桌上打完一整圈，每个位置你都会正好坐到一次——例外是有玩家出局或离座、牌桌改用死按钮（这时同一个玩家可能连续两手最后行动），或者有人加入、拆桌、只剩单挑的时候。
...
1. **位置是名字，不是椅子。**每个座位都按离庄家按钮的距离命名，每个名字通常每手牌都顺时针移动一个座位。
