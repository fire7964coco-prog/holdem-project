...
> 🔴🔴 **먼저 사장님께 물을 것 — GA4 「원치 않는 추천」에 `solver.holdemmaster.com` 넣으셨나요?**
> 2026-08-13에 **8/14에 직접 하시겠다고** 하셨다. 상세·경로는 `docs/update-calendar.md` 8/14 항목.
> **안 하면 솔버가 붙인 UTM이 전부 무시돼 유입 측정이 통째로 안 잡힌다.** 완료되면 두 곳에서 지울 것.
...
> 🪶 그 밖: C층 잔여(`game-order` #31 · `betting-actions` #2 #7 · `kicker` #18 · `split-pot` #21 ·
> `card-counting` #20 · `texas-…-beginners` #28 #29) · 적대 QA 잔여 8항목 · 검수장 회신 잔여 #2·#3·#5.
>
...
- 🔴 **사장님 액션 1건 — GA4 「원치 않는 추천」에 `solver.holdemmaster.com` 추가.**
  GA4 관리 → 데이터 스트림 → 태그 설정 구성 → 원치 않는 추천 나열. **콘솔 설정이라 코드로 못 한다.**
  안 하면 같은 상위 도메인이라 자기 참조로 처리돼 솔버가 붙인 UTM이 통째로 무시될 수 있다.
...
| 지표 | 값 |
|---|---|
| build | **69 blog + 457 intl + static 60** (static은 `/hands` 제외로 61→60) |
