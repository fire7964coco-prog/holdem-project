...
- main의 마지막 콘텐츠 커밋은 **0c74affa**(Q8-a PT 머지). 마지막 push는 **086161eb**(Q14/H1/core 규칙 등). 이관 문서 커밋은 그 뒤에 별도로 존재할 수 있다.
...
- 기존 ja/zh/zh-hant 레인은 7c9f25a9에 남아 있다. **이번 이관에서는 lane:sync를 적용하지 않았다.** 미커밋 초안 보존이 먼저이며 동기화 완료로 보고하지 않는다.

## 새 세션에서 할 일 — 한 묶음만

1. **우선 이미 push한 086161eb 배포의 최종 확인·마감만** 처리하는 것을 권장한다. build 773 및 로컬 118경로/FAQ1325 통과했지만 라이브 검증은 첫 URL 구문면 불일치로 중단되어 미확정이다. 라이브 성공·IndexNow·문서/우편함 마감이 남았다.
2. 재개 상세 = **docs/backlog-closeout-2026-09-22.md의 “새 세션 이관 체크포인트”**. 먼저 그 절을 읽고 작업 한 묶음을 고정한다. main의 미커밋 초안을 해당 배포분에 섞지 않는다.
3. 그다음 후보는 main 규칙 초안의 제한 검수·마감 또는 calc EN7 독립 검수다. 한 번에 전부 열지 않는다. 다른 작업을 원하면 사용자의 새 지시가 우선이다.
