...
| **4** | 🔴 **`/ranking` 체류 0:23 개선 — 다음 세션 착수 확정** | ⏭ **사장님 지시(2026-08-29) 축어: «다음새션에서는 ranking 체류 0:23 … 이거 작업하자»** — 상세는 바로 아래 절 |
| — | 🪶 원래 합의했던 3회차가 무엇이었는지는 기록이 없다 | 사장님이 4회차를 지정하셔서 **실질적으로 해소됨.** 기록 부재 = 합의 부재가 아니다([[absence-may-be-the-standard]]) — 나중에 떠오르시면 이 표에 추가 |

#### ▶ 4회차 착수 카드 — `/ranking` 체류 0:23 (다음 세션은 여기부터)

**무엇** — `/ranking`이 **참여율 73.1%인데 세션당 체류 0:23**이다(정본 = `docs/seo-report-2026-08-28.md` §7 약점).
참여율은 높은데 23초에 나간다 = **들어는 오는데 볼 게 없다**는 신호다.

**왜 지금인가** — 08-28 (4) 회차가 **이 페이지로 유입을 몰아줬다**: 홈에 `page:ranking` 카드를 신설해
링크 0→4개, 「홀덤사이트」 앵커 0→4회 + 허브 3곳 앵커 교체. 「홀덤사이트」(2,970)의
**홈 CTR 0.4% ↔ `/ranking` CTR 1.5%** 분배를 `/ranking` 쪽으로 넘기는 조치였다.
🔴 **유입을 늘려놓고 도착지가 23초면 그 조치가 값을 못 낸다.** 순서상 지금이 맞다.
...
**착수 순서**
1. 🔴 **원인부터 실측하라 — 「콘텐츠 부족」은 아직 가설이다.** 23초가 ⓐ 내용이 얇아서인지
   ⓑ 찾던 게 없어서(의도 불일치)인지 ⓒ 상단이 무거워서인지 가르기 전에 고치지 마라.
   [[holdem-site-keyword-intent-verdict]]가 실증했듯 **「온라인 홀덤사이트」는 의도 불일치로 이미 제외**됐고
   **「홀덤사이트」(2,970)만 살렸다** — 그 살린 축의 검색자가 뭘 기대하는지 **라이브 SERP로 확인**하라.
2. `node scripts/gsc-page.mjs ranking --days 28`로 쿼리 단위를 뽑는다(어떤 쿼리로 들어오는가).
3. `npm run analytics`로 체류·PV 기준선을 찍는다 — 🔴 **GA4 화면에서 읽지 마라**([[analytics-snapshot-gate.md]]).
4. 실제 페이지를 **모바일 390px로 열어 육안 확인**(모바일×신규 축이 약점 — [[mobile-engagement-gap-new-visitors-only]]).
   ⚠ 단 `/ranking`은 **모바일이 이기는 자리**로 기록돼 있으니 전면 처방 금지.
5. 처방 → 빌드·게이트 🔴 0 → 배포 → 라이브 확인 → WORKLOG·이 표 갱신.

**모델 배치** — 무게중심이 «판정·카피»면 §5-A(본체 Fable), «코드»면 3회차처럼 Opus 단일 + 교열 1렌즈.
🔴 **md에 적힌 대로 실행하지 말고 작업 성격으로 다시 짜라**(사장님 지시 · settled-decisions §5-A 단서).

**남은 단점 (§7) — 6개 중 4개 미착수**
| 약점 | 수치 | 상태 |
|---|---|---|
| `/calculator` | 1:10 · 1.40PV | 🟢 메타 조치(1회차) + 기능 보완(3회차) → **관측 중** |
| `blind-meaning` | **42.1%** (세션 50+ 랜딩 최저) | 🟡 «제목·직답은 건드리지 마라» 확정(settled §1). 3회차가 도구→글 링크로 지원. **본문 이탈 원인은 미착수** |
| `/ranking` | 참여율 73.1%인데 체류 **0:23** | ❌ 미착수 |
| `/en/blog/holdem-tiebreak-rules` | 45.0% · 0:38 | ❌ 미착수 |
| 이미지 검색 | 1,598노출 · **5클릭** | ❌ 미착수 |
| 독일 실독자 | 0:14 | ❌ 미착수(§8 중복) |
...
🪶 **관측은 아직 데이터 1점뿐** — `docs/analytics-log.md`에 기준선 **08-26 한 행**만 있다.
주 1회 `npm run analytics`를 돌리기로 했는데 이후 실행이 없어 SPR·홀덤사이트 조치 효과는 **아직 못 잰다**(정상 — 재크롤 대기).
