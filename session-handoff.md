...
- **다음 후보**: ① 초반 완료분(hand-rankings·rules·game-order·betting-actions 등)에 트슬래시·raw img lazy 재점검(이번에 이벤트가이드에서 다수 발견됐으니 룰/전략글도 있을 수 있음) ② GSC 색인 재요청(이벤트가이드는 "크롤됨-색인안됨" 상태 다수 — 오늘 개선 배포 후 재요청) ③ 신규 한국어 포스트(목표 50개, 현재 51개는 이미 달성상태 확인 필요) ④ APT 인천 8/16 후 자산화
