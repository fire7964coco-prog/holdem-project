...
| 🆕 🟠 **GA4 오가닉 랜딩 ↔ GSC 클릭이 2배 어긋난다**(blind-meaning 65 ↔ 31 · `/ranking`도 73 ↔ 39) | Discover·이미지 검색 등 web 외 유형 가설이 있으나 **미확인이다 — 추정을 사실로 적지 마라.** 확인 전까지 두 수치를 같은 표에 나란히 놓지 말 것(층이 다를 수 있다). GSC searchType별 조회로 가릴 수 있다 |
