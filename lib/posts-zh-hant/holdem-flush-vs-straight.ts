...
> **快速解答**
> 同花大。它排第 5、順子排第 6，兩者只差一階，而這一階在標準德州撲克裡永遠不會變。唯一會翻轉的是短牌（6+），但那動到的是同花和葫蘆的關係，同花對順子照樣不變。
...
> **快速解答**
> 因為同花更難湊出來。52 張牌能發出的 2,598,960 種五張組合裡，同花有 5,108 種、順子有 10,200 種——順子的組合數是同花的兩倍，所以它排在下面一階。
...
順子湊成的方法大約是同花的 ==r:**兩倍**==——在 2,598,960 手五張牌裡是 10,200 種對 5,108 種。算到河牌的七張牌上，差距縮到 ==約 1.5 倍==（4.62% 對 3.03%），但方向沒變：順子出現得更頻繁——這就讓它成為比較弱的一手。同一條以五張牌計算的頻率法則，解釋了整個牌型階梯（算到七張牌時，單張高牌其實比兩對還少見，但排名是按五張牌定的）；每一手牌的精確數字都在[德州撲克機率與勝率表](/zh-hant/blog/holdem-probability "thumb:/images/holdem-probability-hero.webp")裡。

### 為什麼這感覺是反過來的

順子只需要五個連號，而且==**花色不用管**==。這份自由造出了非常多的組合。同花剛好相反：五張牌**每一張都得同一種花色**，而且四種花色一次只有一種能成。==g:能達成的方法少太多，同花就更罕見——而越罕見的永遠贏。==

:::tip[如果你拿的是同花聽牌，對手在追順子，那兩邊撞在一起時是你贏——**兩邊都成牌**時，攤牌是你的同花壓過他的順子。但這不等於你是優勢方：如果他的順子聽牌還帶著一對或更大的牌，河牌之前他仍可能領先。]:::
...
> **快速解答**
> 三種牌面最容易讓人推錯底池：你做成順子但檯面有三張同花色、你做成同花但檯面連號、以及同花和順子共用同一批牌時其實是同花順。共通點是——先看檯面允許什麼，再看自己有什麼。
...
> **快速解答**
> 比同花大的只有四種牌型（外加更大的同花）——但真正會讓你賠錢的不是記不住它們，而是牌面一成對，葫蘆和四條就同時「活」了。

你手上的堅果同花，在成對牌面上從來不是堅果——而葫蘆大小怎麼比，先看三條，不看那對。
...
你手裡是 **A♠ 5♠**，做成堅果同花：**A♠ K♠ 9♠ 5♠ 4♠**。對手手裡是 **K♦ 9♦**，攤出 **9♦ 9♠ 9♥ K♦ K♠**——三條 9 帶一對 K 的葫蘆。==r:葫蘆比同花大==，沒有任何同花活得下來。在沒成對的牌面上，堅果同花只會輸給同花順；可是牌面一旦成對，葫蘆和四條就都進場了。
...
> **快速解答**
> 兩者的差別在「要比幾張」：同花大小可能一路比到第五張才分出來，順子大小只有一個數字要看。所以同花對同花常常要逐張比到底，順子對順子卻只看最高那一張——最大張一樣就是同一副牌，直接平分。
...
玩家 A 的 A 在**第一張**就壓過玩家 B 的 K，所以 A 勝。黑桃同花**不會**因為花色而贏過紅心同花——只有點數算數。（在真正的德州撲克手牌裡，兩手同花一定是*同一種*花色，因為都是靠共用的公牌湊出來的；這個例子用不同花色只是為了示範。）

順子更簡單：只比**最大那張**——沒有踢腳牌。

- **A-K-Q-J-10**（A 當高張，「百老匯／大順」）是最大的順子。
- **A-2-3-4-5**（「輪子／小順」，也就是大家常說的「12345」，A 當 1）是最小的順子。
...
> **快速解答**
> 同花順不是「同花加順子」，而是同樣那五張牌既同花色又連號。如果你的順子用的是一批牌、同花用的是另一批，兩者不能相加——你打的只是其中比較大的那一手，也就是同花。

![9♥ 8♥ 7♥ 6♥ 5♥——一手紅心同花順，撲克裡第 2 大的牌型](/images/holdem-flush-vs-straight-sf.webp "同花順——五張紅心連號，只有更大的同花順或皇家同花順贏得了它")

**同花順**是五張*連號*又*同花色*的牌——像 9♥ 8♥ 7♥ 6♥ 5♥。它是**撲克裡第 2 大的牌型**：只有更大的同花順，或皇家同花順（其實就是 A 高的同花順，A-K-Q-J-10 同花色）贏得了它。它只有 **36 種組合**（五張牌裡約 0.00139%；德州撲克到河牌約 0.028%），除了皇家同花順本身之外比任何牌型都罕見。
...
> **快速解答**
> 會——但動到的是同花和葫蘆的相對位置，同花對順子的關係不變。所以你在 App 或短牌桌上看到「同花贏葫蘆」，不是規則寫錯，是牌堆換了。

短牌（6+）德州撲克是唯一一種會把這些牌型重新排序的常見玩法。在**短牌（6+）德州撲克**裡，2 到 5 的牌會從牌堆裡拿掉。牌變少，同花就變得*比*葫蘆*更難*做出來——所以在這種玩法裡排名會變，==r:**同花會比葫蘆大**==。原則從來沒變過：==越罕見的牌越贏==。變的只是牌堆。在用完整 52 張牌的標準德州撲克裡，==g:同花永遠比順子大、輸給葫蘆==。
...
A. 是。同花是第 5 名、順子是第 6 名，所以在標準德州撲克裡同花永遠贏。五張同花色在統計上比五張連號更難做出來，而在五張牌的牌型裡，越罕見的排名永遠越高。
...
A. 純粹是數學。順子不用管花色，所以大約有 10,200 種湊法，而同花只有 5,108 種。這讓同花在五張牌裡大約罕見兩倍；算到河牌的七張牌上，仍留有約 1.5 倍（3.03% 對 4.62%）。按五張牌來算——排名正是這樣定的——越罕見的牌型排名永遠越高。
...
A. 贏不了——在標準德州撲克裡葫蘆（第 4 名）永遠比同花（第 5 名）大，因為五張裡湊出三條加一對（3,744 種組合）比湊出五張同花色（5,108 種）更難。只有短牌（6+）玩法會把這兩個對調。（順帶一提，《賭俠》那句「同花打得過 full house？除非你爸變成兔子！」其實**說對了**：牌理上就是打不過。）
...
A. 可以。兩手同花從頂張開始逐張比，所以 A 高同花贏過 K 高同花。如果頂張相同，就比第二大的那張，依此類推比完五張。
...
用[完整牌型大小排名](/zh-hant/blog/holdem-hand-rankings)把整個順序記牢，在[比牌與踢腳牌規則](/zh-hant/blog/holdem-tiebreak-rules)裡學會接近的牌怎麼分勝負，如果你完全是新手，[德州撲克新手入門規則](/zh-hant/blog/texas-holdem-rules-for-beginners)會把這一切串起來。
