...
| **B** | **클러스터 본문 이미지 재사용** — §9 「고유화」 규율 저촉 | 🔴 **KO 코퍼스 48장이 2편 이상 공유**(최악 `tournament-table-action` **8편** · `holdem-check-raise-counter` 7편 · `gto-srp-dry-ace-oop`·`holdem-3bet-defense`·`holdem-pub-players-table`·`rules-texas-holdem` 각 6편) | 🔵 **다음 세션 착수 확정**(사장님 2026-08-30: *「b 는크다고했으니 다음새션에서 하고」*). 🔴 **착수 전 선별 필수** — `gto-*` 레인지 매트릭스는 **같은 스팟 캡처라 재사용이 정당**하다(솔버 출력). 진짜 대상은 **실사·인포그래픽 재사용**이다. 18언어 공용 자산이라 교체 = 신규 이미지 제작 회차 |
| C-2 | 구형 워드마크 잔여 13장 | 인포 11 + 실사 2 | 🟠 미결 표 참조(둘 다 **재제작/재생성**이라 별건) |
