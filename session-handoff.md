...
| 항목 | 메모 |
|---|---|
| 🔴 **같은 결함이 컴포넌트에 1,457건** | 산출물 609개 스윕 결과. **①로케일 블로그 목록 카드** `lib/intl-blog-index.tsx:72` — 42장/페이지·원본 합 **1,514KB**·`h-40`(160px)에 600x338 원본. 25개 로케일 전부. **②로케일 홈** 12장·310KB·**6장 eager**. **③대회 가이드 히어로** `components/tournament-guide-post.tsx:105` — `eager`+`fetchPriority=high`라 **LCP인데 원본 그대로**(59.6KB, w=750이면 31.9KB) |
| ⚠ 히어로를 고칠 땐 **preload와 반드시 짝** | `app/blog/[slug]/page.tsx:299~311`이 `optUrl`/`imageSrcSet`으로 preload를 낸다. **한쪽만 고치면 조용히 두 번 받는다**(그 파일 주석에 이미 경고돼 있다) |
