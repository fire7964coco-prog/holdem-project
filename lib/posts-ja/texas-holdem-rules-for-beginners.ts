...
> **先に結論**
> 特に断りがなければ**ノーリミット**です。カードの配り方と役は3形式とも同じで、主に違うのは**一度にいくらベットできるか** — ノーリミットはビッグブラインド以上から全チップまで、フィックスドリミットは決まった刻みだけ、ポットリミットは現在のポットの額まで。ノーリミットは理解しやすい分、1回のミスで全部失える形式でもあります。

「テキサスホールデム」と言うとき、ほぼ確実に指しているのは **ノーリミットホールデム（NLHE）** です — WSOPメインイベントでも、ほとんどのホームゲームでも採用されている形式です。カードの配り方と役の強さは3形式とも完全に同じ。主に変わるのは **一度にいくらベットできるか** です — そしてフィックスドリミットでは、1ラウンドに許されるレイズの回数にもたいてい上限があります（WSOPでは1ベットと4レイズまで）。
...
ドローハンド（例：フラッシュまであと1枚）を持っているときは、自分の **アウツ** — ドローを勝てそうな手に変えてくれるカードの枚数 — を数えます。
...
A. ディーラーボタンの左隣2人が、カードが配られる前に払う強制ベットです。1人目がスモールブラインド、2人目がビッグブラインド（通常はその2倍）を払います。これにより、常にポットの中に、奪い合う対象のお金がある状態が保証されます。他のプレイヤーはハンドに残るために最低でもビッグブラインド額を出す必要があります（それが全チップなら、それより少ない額でオールインしても構いません）。
...
次のステップとして、[ポーカーの役の強さ一覧](/ja/blog/holdem-hand-rankings) を復習し、[スターティングハンド表](/en/hand-chart) で練習し、コールが割に合うかどうかを知りたくなったら [ポーカー確率計算機](/ja/calculator) を使ってみてください。
