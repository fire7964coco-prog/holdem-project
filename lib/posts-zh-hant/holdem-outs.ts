...
每個聽牌都有兩個數字要看。**「到河牌」** 把剩下兩張牌都算進去，只有你在翻牌圈全下（all-in）、後面沒有下注時才適用。**「翻牌 → 轉牌」** 只算下一張牌（9 ÷ 47 = 19.1%；從轉牌到河牌則變成 9 ÷ 46 = 19.6%）——只要後面還有下注，就用這個，因為你一次只保證看得到一張牌。新手明明面對的是轉牌圈的注，卻拿「到河牌」那個又肥的數字說服自己跟注，然後付出代價。
...
:::tip[×4 那版悄悄假設了你會看到*兩張*牌、中間沒有更多下注——只有你已經全下時才有保證。如果你面前還有一注要打，就用 ×2（單張牌）的數字，看你眼前這條街到底是哪一街。]:::
