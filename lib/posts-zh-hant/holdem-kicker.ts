...
你們誰都湊不出比 A 更高的，所以雙方都「用公牌打」，==g:平分底池==——但前提是你把底牌翻開；把牌蓋掉就一毛都拿不到，就算在這種局面也一樣（TDA 第 19 條）。順子沒有踢腳，所以那些底牌是死重。當你聽到「the board plays（公牌自己成牌）」，講的就是這個 —— 這也是唯一一個看起來很強的底牌，實際上一文不值的場合。
