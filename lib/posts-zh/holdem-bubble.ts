...
A. 泡沫系数衡量的是——按真金白银（ICM）来算，输掉一个底池给你造成的损失，比赢下同一个底池给你带来的好处，要大出多少。泡沫系数为 1.0 意味着筹码等于钱；为 1.5 意味着出局的痛是赢下的爽的 1.5 倍。它能直接换算成你做一次跟注所需要的胜率：泡沫系数 ÷（1 + 泡沫系数）。泡沫系数为 1.5 时，你需要 60% 而不是 50% 才能跟——这正是为什么一次筹码 EV 上的抛硬币对局，到了泡沫期就变成了该弃。
