...
> **快速解答**
> 這張表不用整張背，記住==同花 9、兩頭順 8==這兩個錨點就夠；其餘（高張、口袋對、中洞）都是直接數點數——每個點數 4 張，扣掉你看得見的。真正該花時間的不是背數字，是認出自己手上到底是哪一種聽牌。
...
**同花 + 中洞**也是同樣的道理：9 張同花補牌 + 4 張中洞牌，但那四張裡有一張是你的花色 → 9 + 3 = **12**。只要兩個聽牌共用牌，就把重疊的扣掉——同花＋中洞共用一張，同花＋兩頭順共用兩張。這是牌手最常見的多算方式，也是為什麼表裡那幾行複合聽牌，看起來總比兩數相加「少一張」。
