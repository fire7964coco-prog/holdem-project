...
大家最容易搞混的是這一段：算牌在 ==21 點== 裡其實也不違法——那不是犯罪，就只是心算。但在多數地區，賭場是一門私人生意，它可以禁入或拒絕一位疑似算牌者，因為算牌者是在賺==賭場==的錢。撲克把這件事完全反過來：你打的是==其他玩家==，賭場不管誰贏都只抽[抽水（Rake）](/zh-hant/blog/holdem-glossary)。沒有人有任何理由阻止你算你的補牌——籠罩 21 點的那個「被趕出去」問題，在這裡不會因為算牌而發生。（撲克室當然保有因其他理由拒絕任何人的權利；替自己的聽牌做算術不是其中之一。）

那條界線劃在「腦子」和「工具」之間：採用 TDA 規則的錦標賽，第 5 條明文寫著手上還有活牌的玩家不得使用下注 App 或圖表（現金桌依各場館規定）。規則擋的是工具，不是心算——所以同一套機率，記在腦子裡完全合法，開在手機上就不行。
...
3. **它是技術，不是秘密。** 沒人會為此趕你。算你的補牌、問你擋掉了什麼、扣掉你已經看到的死牌——每一手都做。
