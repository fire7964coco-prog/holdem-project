...
### 솔버 랜딩·앱
- **MS 랜딩 로컬 완료·배포 전**(2026-09-14 직접 지시): `/ms/solver` 3파일·메뉴/프로모/사이트맵·11언어 hreflang 등록. Google MY 설정 조사와 현지 전문가 페르소나 검수 근거 = `docs/ms-solver-landing-brief.md`. 최종 빌드 및 FAQ **229문항** 화면/JSON-LD 통과, 390px/데스크톱 확인. EN·JA의 옛 기본 오차 0.5% 안내도 정정. 전체 typecheck는 기존 596건 실패(변경 파일 진단 0). 미커밋 변경 보존. 실제 배포 뒤 솔버 `outbound.ts` MS `/solver` 매핑 필요. 미구현 잔여=hi.
- **솔버 랜딩 FAQ 정리 배포 대기**(2026-09-14 사용자 직접 지시): 10개 언어 저장 범위·8개 언어의 옛 선택기 FAQ·KO ⑩ 탑페어 한정 수정은 로컬 검증 완료. 빌드와 FAQ 206문항의 화면/JSON-LD 일치 확인. 아직 커밋·배포 전이므로 일반 작업 중 변경분을 덮어쓰지 말 것. 경위 = WORKLOG 같은 날짜 「솔버 랜딩 FAQ 동기화 정리」.
