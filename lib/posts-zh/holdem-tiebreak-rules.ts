...
A. 先比大对，再比小对，最后比那唯一一张踢脚牌——所以 A 带 3 赢过 K 带 Q，哪怕第二对小得多。让人栽跟头的是像 K-K-9-9-5 这样的双对牌面：每个人手里已经是同样的两对，整手牌就坍缩成一张踢脚牌，全桌最大的那张底牌拿走底池。两对正好只带一张踢脚牌，绝不会有两张。
...
A. 不会——但德州扑克牌桌上确实有一个用到花色的地方，只是跟底池无关。牌局开始时每人抽一张牌决定庄家按钮放在哪，两张抽到的牌点数相同时，就靠花色顺序定。那选的是一个*座位*，从来不是一手牌。规则书里其余用到花色排序的地方，全都属于梭哈和 razz。两手最佳五张点数一张对一张全相同，无论花色如何都平分底池。
