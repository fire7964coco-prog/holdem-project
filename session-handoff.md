...
4. **`audit:hard`의 C1(형제 글 표 교차 대조)이 계속 0쌍이다** — 헤더가 달라 매칭에 실패한다.
   이번에도 손으로 대조했다(결과는 정상: confusing=5장 기준 명시 / game-order=7장 기준 명시).
   **매칭 규칙을 느슨하게 고치지 않으면 이 검사는 영원히 아무것도 검증하지 않는다.**
