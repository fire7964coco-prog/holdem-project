# CLAUDE.md 원본 백업 (2026-08-11 슬림화 직전)

> 532줄 / 33.5KB 버전. 슬림화로 뭐가 빠졌는지 대조할 때 쓴다.
> 여기 적힌 수치(포스트 개수 등)는 **당시에도 이미 낡아 있었다** — 신뢰하지 마라.
...
>
> - `session-handoff.md`는 **통독한다**(2026-08-09에 15KB로 정리했다 — «지금 할 일»만 담는다).
>   과거 로그는 `docs/handoff-archive-2026-08-09.md`로 내렸고, **거기 적힌 지시는 전부 낡았다.**
> - 🔴 `WORKLOG.md`는 **603KB라 통독하지 마라**(Read 상한 256KB 초과 → 에러). 쓰는 법은 둘뿐이다:
>   **① 최상단 몇 개 항목만** `Read`에 `limit`을 줘서 보고 **② 손댈 글은 `slug`로 grep**해
>   이미 작업한 적 있는지 확인한다(안 읽어서 2026-08-02에 2건 오판했다).
...
- [ ] **`npm run audit:hard -- --slug=<slug>` 🔴 0건** (§13 검산·메타·링크·중복 기계 검사 — §14-A 9)
- [ ] (핸드 예시 수정 시) §13 베스트 5장 검산 완료 — 게이트가 시나리오를 못 잡았다면(커버리지 출력 확인) 손으로
- [ ] `npm run build` 성공 확인 (sitemap 출력의 post 수도 확인)
- [ ] (FAQ 손댔으면) `npm run audit:hard -- --schema` — 산출물 기준 확인
...
## 12-A. 🔴 템플릿 리터럴 백틱 금지 (빌드 파손 방지)

포스트 `content`는 **백틱 템플릿 리터럴**(`` `...` ``)이다. 본문 안에 백틱을 쓰면 문자열이 그 자리에서 끊겨 **빌드가 즉시 깨진다**(exit=1).

- ❌ 본문에 백틱으로 코드 표기 — 2026-07-30에만 **3회 파손**(`texas-beginners` s/o/T 표기, `bluffing` 계산식 등)
- ✅ 대체 표기: `**굵게**` / `==하이라이트==` / `"따옴표"`
- 계산식도 백틱 금지 — `==0.6 ÷ (1 + 0.6) = 약 37.5%==` 처럼 하이라이트로
- 편집 후 반드시 `npm run build`로 확인

---

## 12-B. 🔴 서치 절대 규칙 — 요약 금지, 원문 직접 (§13과 동급)

> 상세는 `.cursor/rules/posting.mdc` 최상단. 여기엔 핵심만.

**사실은 "요약"이 아니라 "원문"에서만 가져온다.** 2026-07-29 하루에 같은 원인으로 **두 번** 틀렸다 — 중국 우승자 2명(실제 4명), 일본 브레이슬릿 3개(실제 4개). 둘 다 **WebFetch에 "누가 우승했나?"라고 질문을 던지고 그 답을 받아쓴** 탓이다. WebFetch는 읽고 **요약해서** 답한다.

| 도구 | 허용 | 금지 |
|---|---|---|
| **WebSearch** | **URL 찾기 전용** | 검색 요약의 **내용을 사실로 쓰기** |
| **WebFetch** | 단일 사실. **축어 인용 요구** 필수 | **열거·개수·목록** 질문 |
| **Playwright** | ★ 열거·개수의 유일한 정답. `browser_evaluate`로 DOM 텍스트를 긁어 **내 정규식으로 직접 센다** | — |

- 파싱 후 **빠진 번호가 없는지** 완전성 검증까지 할 것
- WebFetch 응답에 **질문과 무관한 항목이 섞여 나오면 그 응답은 불완전하다** — 실제로 이 신호를 보고도 넘어가서 틀렸다
- 2차 출처밖에 없으면 **쓰지 않는다**
