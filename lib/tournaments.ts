...
const RAW_TOURNAMENTS: Tournament[] = [
     ...
     아래 두 행은 카지노가 아닌 호텔 컨벤션·시내 베뉴라 «내국인 가능» 축이다. 값은 주최 공식에서 직접 읽었다(본체 Playwright 09-03). */
  {
     ...
     * 🔴 2026-09-06 판정 — **「DAY1 9/11·12 = 플라이트 2회」는 오독이다. NOTE_EN 「Day 1 Sep 11–12」(범위)를 고치지 마라.**
     *   핸드오프 미결이 «en-dash가 이틀 연속 한 플라이트로 읽힌다»며 정정을 걸어 뒀는데, 원문이 정확히 그 뜻이다.
     *   NHN 뉴스룸 축어(2026-09-06 라이브 재확인): 「**11일, 12일 양일 간 진행되는** ‘메인 이벤트’ DAY1에서 이용자는
     *   오프라인 토너먼트 참가권 2장을 사용해 출전할 수 있다. **DAY1 일정 내에서는 참가권 2장을 사용해 재참여(리엔트리)도 가능하다.**
     *   DAY1에서 생존한 이용자는 9월 13일 펼쳐질 ‘메인 이벤트’ DAY2에서…」 — 「플라이트」라는 낱말은 **0건**이다.
     *   즉 DAY1은 «이틀에 걸친 하나의 DAY1 + 그 안에서 리엔트리»이지 별개 플라이트 2회가 아니다. 범위 표기가 옳다.
     *   🪶 교훈 = 지시서·미결의 «근거»와 «결론»은 갈라서 판정한다. 여기서는 결론이 원문에 반증됐다.
     */
    id: "hpt-5",
  ...
  },
  {
     ...
     * 🔴 2026-09-06 — 「전 이벤트 참가권(SEAT) 방식」의 **1차 출처는 일정 «이미지»다.** 텍스트 페이지에는 SEAT·참가권이 0건이고
     *   (`/apl-winter-series-2026-winter-prelims-circuit-i/` 본문 605자 · 「Full schedule as shown below」),
     *   실제 근거는 `wp-content/uploads/2026/08/WINTER_CIRCUIT1_스케줄확정.png`(2160×2880)다. 본체가 내려받아 육안 판독:
     *   각주 축어 「※ SEAT = 해당 이벤트 참가권, 획득처는 추후 공지 · FLIGHT · DAY 표기 상금은 이벤트 통합 상금 · MINS = 블라인드 레벨 시간」 ·
     *   BUY-IN 열이 **진입 가능한 16행 전부 SEAT**이고 「—」는 둘뿐인데 그 둘은 «APLC MAIN EVENT · DAY 2»·«· FINAL DAY»로
     *   **같은 이벤트 #1의 연속일**이다(진입 불가 = 바이인 없음). 그래서 「전 이벤트」는 참이다.
     *   → NOTE_EN 에서 탈락해 있던 이 **범위 한정어를 en·ja·zh·zh-hant·es 다섯에 소급**했다(2026-09-06).
     *   🔴 **텍스트 grep 0건을 «근거 없음»으로 읽지 마라** — 이 자산은 처음부터 이미지 안에 있었다.
     *   🪶 09-04에 확보한 「After two years…」 축어는 **다른 URL**(`/apl-seoul-returns-…`)에 있다. 2026-09-06 라이브 재확인 1건 ✅.
     */
  ...
  },
  {
    ...
    emoji: "🇵🇭",
    color: "bg-sky-500/15 text-sky-400 border-sky-500/30",
    // 공식 페이지는 이 행에 **이름·기간만** 게시한다(다른 행과 달리 GTD·메인 바이인 칸이 비어 있다).
    // 🔴 2026-09-04 재실측 — 「All events are subject to regulatory approval」은
    //   이 대회 전용 조건이 아니라 **페이지 하단 NOTE의 전체 공통 각주**다(M-084 정밀화 d).
    //   종전 note는 그걸 이 대회만의 특질처럼 읽히게 했다 — 되돌리지 마라.
    note: "공식은 이름·기간만 게시 · 상금·바이인 미발표",
    sourceUrl: "https://www.pokerstarslivemanila.com/tournaments/",
    sourceTier: "A",
    verifiedAt: "2026-09-03",
  },
  ...
  },
  {
    ...
    city: "Castellón",
    country: "ES",
    venue: "Orenes Gran Casino de Castellón",
    buyin: "€150~€1,000",
    emoji: "🎲",
    color: "bg-orange-500/15 text-orange-400 border-orange-500/30",
    note: "메인 €500 / €100,000 GTD (10/8~11) · 미니메인 €150 / €50,000 GTD",
    sourceUrl: "https://www.partypoker.com/en/poker/p/party-poker-tour-2026/castellon",
    sourceTier: "A",
    verifiedAt: "2026-09-04",
  },
  {
    ...
    sourceUrl: "https://www.partypoker.com/en/poker/p/party-poker-tour-2026/cork",
    sourceTier: "A",
    verifiedAt: "2026-09-04",
  },
  {
  ...
  },
  {
    ...
    emoji: "🎲",
    color: "bg-orange-500/15 text-orange-400 border-orange-500/30",
    note: "시즌 최종 스톱 · 메인이벤트 £150,000 GTD",
    sourceUrl: "https://www.partypoker.com/en/poker/p/party-poker-tour-2026/london2",
    sourceTier: "A",
    verifiedAt: "2026-09-04",
  },
