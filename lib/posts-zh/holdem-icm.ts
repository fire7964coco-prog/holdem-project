...
经典的冲突是*跟*一次边缘的 all in。在 chip EV 里，为一个大底池打一次 coin flip 可能没问题、甚至很好——你赢的筹码和输的一样多。在 ICM 里它可能是个明确的 ==弃牌==，因为出局会让你失去你在**每一档高于已锁定名次**的奖金里的权益（已经锁定的那份保底本身仍然是你的），而你要赢的那些筹码，值得比面值更少。
...
A. chip EV 用赢来或输掉的筹码衡量一个决策；ICM 用真实的钱衡量。它们在锦标赛前期一致，逼近钱圈时分道扬镳，因为在那里出局丢的是真实的奖金权益——在 bubble 上，丢的是你进钱的全部机会；一旦进了钱，丢的是高于已锁定名次的一切。一次在 chip EV 里没问题的 coin-flip all in，在 ICM 下可能是个明确的弃牌。
