...
export const POST: Post = {
  ...
  content: `
我第一次被 ICM 坑掉錢的時候，根本不知道它的存在。桌上剩四個人，三個進錢圈，我低頭看到一手口袋 J，籌碼中等。我推了全下，籌碼領先者拿 A-10 跟，結果我在泡泡時間爆掉、一毛沒拿。==好幾年來，我都把它歸檔成「這一手全下是錯的」的證據。它並不錯==——我只是根本不知道泡泡時間究竟*在哪一步*向你收費，而那正是錦標賽撲克裡最重要的一個觀念。
...
關鍵的思維轉換在這：現金桌裡，一顆籌碼就是一塊錢，句號。錦標賽裡，一顆籌碼是一張押在固定獎金組上的*彩券*。ICM 就是幫這張彩券定價。它只適用於錦標賽和 SNG（Sit-and-Go）——**[現金桌從來用不上](/zh-hant/blog/holdem-tournament-vs-cash-game "thumb:/images/tournament-table-action.webp")**，因為那裡的籌碼本來就等於它的面額。
...
數字全在這了：領先者手握==一半的籌碼，卻只佔 38.4% 的獎金==，而短碼那 20% 的籌碼，值 28.9%。你不用在牌桌上手算這些——ICM 計算工具一秒就跑完（後期為什麼由它主導，見[錦標賽指南](/zh-hant/blog/holdem-tournament)）——但親眼看過一次這台機器怎麼運轉，才是讓策略真正刻進腦子的關鍵。
...
最經典的衝突，是*去跟*一手邊緣全下。在籌碼EV 裡，為一個大底池打一場五五開可以是不錯、甚至是好的——你贏到的籌碼跟你可能輸掉的一樣多。但在 ICM 裡，它可能是清清楚楚該==蓋==，因為爆掉賠掉的是你在「已經鎖住的那個名次之上」每一個名次的權益（保底那一份本身還是你的）——而在泡泡時間，還什麼都沒鎖住，賠掉的就是你全部的獎金權益——而你贏到的那些籌碼，價值又低於面額。

我正是在這裡把那手口袋 J 想反了。稅是收在*跟注*那一側的，而它的反面才是泡泡時間之所以能打的原因：正因為所有人的跟注範圍都收緊了，你的棄牌價值比用籌碼衡量時**更值錢**。在泡泡時間搶先全下是中等籌碼的武器，不是它的漏洞——我只是撞上了那個唯一能跟得最寬的人，那是變異數，不是策略錯誤。==籌碼EV 問的是「這會不會把我的籌碼疊高？」ICM 問的是「這會不會把我的資金堆高？」==——只有第二個問題會真的付你錢。
...
感受最深的，是**泡泡時間的中碼**——大到有實實在在的權益可以輸，又沒短到被逼著非上不可。他背著最高的風險溢價，該打得最緊。籌碼領先者背著*最低*的風險溢價，這正是 ICM 壓力（ICM pressure）背後的整台引擎。

---

![泡泡時間裡，一個中碼在錢圈邊緣蓋掉面對大碼全下的牌，籌碼與獎金階梯都在畫面裡——ICM 壓力把一個正常的跟注變成蓋牌的那一刻](/images/holdem-icm-pressure.webp "ICM 壓力：中碼蓋牌，因為在泡泡時間爆掉會賠掉整個 min-cash 以及它之上的每一階獎金")
...
短碼從 ICM 交易拿到的，比籌碼平分多 ==$97==，因為 ICM 把他早已賺到的那些獎金躍升算了進去。所以規則很好記：==你是短碼，就要求 ICM 交易；你是籌碼領先者，就提議籌碼平分。== 實務上，籌碼領先者常常會談到*略高於*他 ICM 數字的位置（短碼也接受略低一點），換取把錢當場鎖死的確定性——這沒問題，只要你先知道自己的 ICM 數字。在你答應任何事之前，先把自己的籌碼和獎金結構丟進 ICM 計算工具跑一遍。
...
- **[錢圈泡泡時間（bubble）](/zh-hant/blog/holdem-bubble)**——所有躍升裡最大的一次，就是從 $0 到進錢，所以風險溢價在這裡達到頂峰。
- **決賽桌泡泡時間，以及決賽桌上的每一次獎金躍升**——每一級階梯都是真金白銀。
- **衛星賽（satellite）**——最極端的例子：在發出多個席位的衛星賽裡，每一個晉級席位都同樣值錢，所以一旦你的籌碼夠贏一個席位，多出來的籌碼幾乎*一文不值*，你會蓋掉幾乎所有牌（只發一個席位的贏家全拿衛星賽，要按籌碼EV 去搶第一）。
...
A. 籌碼EV 用贏或輸多少籌碼衡量一個決策；ICM 用真實獎金衡量。兩者在錦標賽早期一致，接近錢圈就分岔。在泡泡時間，爆掉賠掉的是你進錢圈的全部機會；一旦進了錢，賠掉的是「已鎖住的名次之上」的一切。一手在籌碼EV 沒問題的五五開全下，在 ICM 下可能是清楚的蓋牌。
...
A. 三個大的。第一，*過度*套用 ICM——獎金躍升明明還遠，卻一路蓋牌「想爬階梯」，而不是累積籌碼。第二，當泡泡時間附近的中碼卻跟得太寬，那正是你風險溢價最高的地方——那時你還什麼都沒鎖住，所以在那裡爆掉，賠掉的是你的全部權益，連 min-cash 都算在內。第三，當短碼卻答應籌碼平分（或當領先者卻答應 ICM 交易）而事先沒算過數字。ICM 是後期工具：用得太早、或在決賽桌無視它，都會漏錢。
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  ...
  <a href="/zh-hant/blog/holdem-when-to-fold" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">泡泡時間蓋掉現金桌會跟的牌</div>
  </a>
</div>
`.trim(),
};
