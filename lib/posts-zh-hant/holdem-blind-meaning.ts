...
**盲注（Blind）** 這個名字就來自「盲下」——牌還沒看就得先押。那為什麼一定要逼人先付這筆錢？==g:為了讓牌局活下去。== ==r:沒有盲注的話，每個人都可以一直蓋牌、無止盡地枯等一手大牌==；底池永遠是空的，詐唬也變得毫無意義，整場牌局就停擺了。==g:正常情況下每手牌兩個強制注，保證檯面上永遠有東西可以爭。==（唯一的例外是死按鈕，那一手可能沒有小盲。）
...
> **快速解答**
> 盲注不是固定在某兩個人身上，而是跟著莊家鈕走：每打完一手，鈕往左移一個座位，盲注也跟著移。所以每個人每一圈都會各付一次小盲和一次大盲，想靠離席跳過自己那一輪也不行——回座時要嘛補上錯過的盲注，要嘛等大盲輪回到你的座位。
...
> **快速解答**
> 分兩種情況：新坐下的人通常要補一個大盲才能立刻開打，或乾脆等盲注輪到自己；而中途離桌漏掉盲注的人回座時，要嘛補上、要嘛等大盲輪回來；選擇補的話，漏掉的小盲會變成一枚死籌碼，直接收進底池、不算你的注。

各家撲克室的規矩不一樣，所以坐下前問荷官一句最穩。常見的兩種做法是——**補下一個大盲** 立刻加入，或等盲注輪到你的座位；也有撲克室不要求無限注／底池限注新入座的玩家補盲（WSOP 現金局（Live Action）規則 104.a 就是這樣）。如果你離桌一下，==**漏掉了你的盲注**==，多數撲克室會要你回座時補下才能重新加入——漏掉的大盲當成一個有效的活注，漏掉的小盲則是一枚 ==**死籌碼**==、直接被收進底池。線上牌桌會自動幫你處理這一切。
...
A. 多數撲克室會要你回座時補下漏掉的盲注——漏掉的大盲當成有效的活注，漏掉的小盲則是一枚死籌碼、直接進底池。或者，你也可以等大盲自然輪回你的座位再入局。線上牌桌會自動處理。
