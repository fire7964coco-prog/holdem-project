...
| ✅ **DFS 복구 완료(2026-08-27) — «인증 만료»는 오진이었다.** 자격증명은 유효(직접 REST 20000 Ok·유료 잔액 실재 — 사장님 지적이 맞았다). 진짜 원인 = `.mcp.json`의 `${VAR}` 확장이 참조하는 **환경변수가 시스템에 없었던 것** → 사용자 환경변수 등록 완료. 🔴 **MCP는 세션 재시작 후 붙는다** — 그전엔 curl REST 직접(경로·CJK 이스케이프 처방 = `docs/rakko-playbook.md` 2026-08-27 절) | 재시작 후 첫 세션에서 MCP로 fr 4종·id 헤드텀 교차검증 실행(각 뱅크 머리 참조) |
