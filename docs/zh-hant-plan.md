...
8. [x] ✅ `holdem-strategy` — 策略 로드맵. rakko headline+경쟁강도 스캔 → 대만 검색 본류=技巧 리스티클(sixpoker 14大·happypoker 10招)이나 얇은 娛樂城稿 → EN 5-decisions 골격으로 차별화. **대만 winnable 흡수 FAQ**: 剝削(Exploitative, monsterstack 전용글)·資金控管(sixpoker)·實力vs運·GTO·變強·緊兇TAG. 用語 緊兇/鬆兇/緊弱/鬆弱·剝削·資金控管·半詐唬·快打·防守大盲(SERP 실측). 링크=position-play+starting-hands-chart+pot-odds+probability+glossary(5편). 적대적 검수 §13 전항 통과(暗三條11.8%·A♣K♣ 미스·5♠5♦ set·c-bet 빈도·資金 20-40/100+). 배포.
9. [x] ✅ `holdem-tournament` — 錦標賽 (★필라 9/9 완주!). EN 정보허브 재저작 + **대만 특화 winnable 흡수**: FAQ `錦標賽在台灣合法嗎?`(suggest 상위, #2 정합·신중표현)·`台灣哪裡打?`(撲克協會 일반화, 구체 클럽/요금 창작금지) + 報名에 撲克協會 옵션. 用語 升盲/盲注結構/底注/結構表/泡泡圈/錢圈ITM/衛星賽/PKO/手手制/撲克協會. 링크=starting-hands+glossary+strategy+rules(4편). 적대적 검수: **§13 전 수치 통과·WPT Seminole 완전 자립**(3200×1435=4.592M·181/1435=12.6%·662200/4.592M=14%·1만籌 200→10BB). 🟢 Day1/체크리스트 카드 다크테마 스타일=전 언어 공통 백로그. 배포.
