...
### ▶ 0-E. ⚠ 다음 경화 1순위 — 프리플랍 LEGACY 2편의 FAQ가 죽어 있다
...
합쳐 **13문항이 스키마로 안 잡힌다.** 형식은 `**Q. 질문**` + **빈 줄** + `A. 답변`
(감사는 소스 raw 말고 `.next/server/app/blog/*.html`의 `"@type":"Question"` 수로).
둘 다 레인지 클러스터 핵심 형제 글이라 순위에도 직결된다.
