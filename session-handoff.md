...
| 🆕 🟠 **대회·펍 강점 축의 남은 자리는 «데이터 공급»이 병목이다**(2026-08-29 (2) 실측 결론) | 강점 축을 강화하려 GSC를 팠는데 **지금 당장 할 것이 적었다**: 「참가비」 카니발=무해 · 「홀덤대회」 CTR=쿼리 오염(둘 다 settled-decisions §1로 종결) · `how-to-enter`는 자기 축에서 **r1.0·CTR 40.4%로 이미 최적**. 남은 건 **브랜드 롱테일**(「ksop 참가 방법」 6노출 r9.5 · 「apl 참가 방법」 5 r9.8 · 「jopt 참가 방법」 · 「hgp 참가권」 — 전부 클릭 0)인데 **KSOP·APL·JOPT·HGP는 KO 코퍼스 언급이 0건**이다. 🔴 `country-tournament-playbook.md` §3 규율 1(데이터 공급 확정 전 착수 금지)에 정확히 걸린다 → **1차 출처 확보가 선행되는 별도 리서치 회차**. 볼륨은 합 13노출로 작으니 우선순위는 낮다 |
