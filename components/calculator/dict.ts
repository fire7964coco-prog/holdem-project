...
export const CALC_DICT_EN: CalcDict = {
  ...
  pushfold: {
    ...
    huNote: {
      ...
      p1: ": the SB is assumed to either shove or fold. The ante setting adds 0.125bb per player (12.5% of the big blind each — what a big-blind ante works out to at an 8-handed table; heads-up that is only 0.25bb in total). Ranges widen as stacks get shorter — and below ~3-4bb the BB correctly calls ",
      em: "wider",
    ...
    mwNote: {
      ...
      p2: " Nash-style equilibrium — the calling ranges of the players behind are solved simultaneously — using the standard approximation that a called pot is resolved heads-up against the first caller (multi-caller pots are ignored). Ranges widen as fewer players are left behind you (UTG → BTN → SB), as stacks get shorter, and with antes ON (0.125bb per player). Because it is pure chip-EV, the early-position (UTG/MP) 10bb ranges come out tighter than popular rules of thumb (with antes off, small pairs fold) — adjust for ICM and opponents in practice.",
    },
  ...
  quickRef: [
    {
      ...
      intro: "Equity when two hands go all-in preflop, averaged over every suit combination of the matchup (precomputed offline: all 1,712,304 boards for each one). A specific suit combination can differ by more than two points (A♠K♠ vs Q♠J♠: 66.0%) — enter the exact cards in the equity calculator above.",
  ...
  guide: {
    ...
    cards: [
      ...
      { icon: "⚡", title: "Nash push/fold chart", body: "A 13×13 Nash chart for heads-up, 6-max and 9-max: which hands to open-shove at 1–25bb (and, heads-up, which to call with). A must-have for late-stage tournaments." },
    ],
  },
