...
### 작업 내용
- 홈 피드 디자인 전면 리뉴얼: 다크 카지노 → 에디토리얼 크림 뉴스페이퍼 스타일
- 폰트: DM Sans → Inter(UI) + Lora(세리프 헤드라인) + Noto Sans KR 조합 적용
- 마스트헤드 헤더("글로벌 포커 커뮤니티 / 홀덤 라운지"), 언더라인 필터 탭, 다크그린 하단 탭바
- post-card.tsx: 크림 팔레트(BG #f4f0e7, INK #20312a) + 뉴스페이퍼 카드 레이아웃 (키커·저자 하단배치)
- layout.tsx globals.css: Inter/Lora 폰트 변수 추가
...
### 작업 내용
- 성능 개선: 홈(/) 및 13개 로케일 피드 페이지 CSR 전환 — force-dynamic 제거, 전체 Static(○) 전환
- Supabase 데이터 fetch를 CommunityClient useEffect 클라이언트 사이드로 이동
- 예상 TTFB 6초(cold) → 0.1초(Vercel CDN HIT)
- 커밋: `9c0b8d6`
