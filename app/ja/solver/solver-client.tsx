 ...
 *     ポーカー gtoツール 90 · **ポーカー gtoツール 無料 50(SEO난이도 24 «低» — 이 판 유일)** ·
...
const SPEC: [string, string][] = [
  ...
  ["表示言語", "日本語（韓国語・英語・スペイン語・ポルトガル語・ドイツ語に切替可）"],
 ...
 * 🪶 `slug`가 하나도 없다 — GTO 시리즈 해설 13편이 **ja에는 아직 없기 때문**이다.
 *    ja판을 발행하면 그 행에 채운다. **없는 링크를 미리 걸지 않는다.**
 */
const SPOT_GROUPS = [
  {
    ...
    items: [
      { board: "A♥7♦2♣", name: "ドライなAハイボード", note: "レンジ優位の教科書的スポット。Aはオープンレイザーに有利なカードです" },
      { board: "K♠8♦3♣", name: "ドライなKハイボード", note: "KハイでもBTN優位ですが、チェックがわずかに増えます — Aハイと比べてみてください" },
      { board: "Q♠J♦T♠", name: "ブロードウェイのコネクトボード（2トーン）", note: "両者のレンジがボードと強く噛み合います。ハンド分類パネルでどちらに厚みがあるか確かめてください" },
      { board: "9♥8♥7♣", name: "ミドルのコネクトボード（2トーン）", note: "コーラー優位ボードの代表例。「常にCベット」がなぜ間違いなのかを学べます" },
  ...
  },
  {
    ...
    items: [
      { board: "A♦K♠2♥", name: "3ベッター優位のAハイボード", note: "3ベットレンジにとって最高のボード。低SPRで小さなベットがレンジ全体に圧力をかけます" },
      { board: "Q♥T♥7♠", name: "ダイナミックな2トーンボード", note: "生きたドローが2つ。それでもレンジの98.4%が同じ2/3サイズで仕掛けます" },
      { board: "8♦5♣2♠", name: "ロー・ドライボード", note: "3ベットレンジがほぼ丸ごと外れるボード。それでもオーバーペアが圧力をかけ続けます" },
    ],
  },
  {
    ...
    items: [
      { board: "K♥T♦6♠", name: "KTハイボード", note: "レンジが広く、お互いに弱いのが特徴。BTN vs BBと頻度がどう違うか比べてみてください" },
      { board: "7♦6♦5♣", name: "ローのコネクトボード（2トーン）", note: "ワイドレンジ同士が非常にコネクトしたボードでぶつかり、ツーペアとドローが続出します" },
      // 🔴 2026-08-21 정정 — 앱 화면은 「Aを持つスリーカードが珍しい」라고 쓰지만
      //    **ja 코퍼스 정본은 `トリップス`다**(`lib/posts-ja/holdem-glossary.ts` L47·L136 축어:
      ...
      //    ⚠ 이건 **솔버 앱 ja 번역의 결함**이기도 하다 — 별건으로 솔버 쪽에 통지할 것.
      //    🪶 EN 랜딩은 "Trips are rare with two aces down"으로 **처음부터 정확했다.**
      { board: "A♠A♥6♦", name: "Aペアボード", note: "Aを持つトリップスはSBが88コンボ、BBが66コンボ。数でも質でも上回るSBが80.1%を仕掛けます" },
    ],
  },
];
    ...
    <div className="mx-auto max-w-3xl px-4 pb-16">
      ...
      */}
      <section className="mt-12">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          順番があります。GTOの勉強がうまくいかない理由のほとんどは、
      ...
      <section className="mt-12">
        ...
        <ul className="mt-4 list-disc space-y-2 pl-6">
          <li>
            ...
            この記事のレンジ表にあり、①と②に貼り付けるレンジもここから来ています
          </li>
          <li>
