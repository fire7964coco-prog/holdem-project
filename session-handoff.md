...
### ▶ 0-◗. 2026-08-07 — 생 `<img>`가 최적화를 우회하던 것: 포스트 본문분 마감 ✅ (여기서 멈춤)

> **작업 트리 깨끗 · build 56+457 intl(615) · canonical 0건 · 라이브 확인.** 커밋 `14e43af`.
> 상세는 `WORKLOG.md` 「2026-08-07」.
>
> ⚠ 착수 전 **`git pull` 먼저.**

**한 일** — 「폭 750px 미만 15장」을 확인하러 갔다가 **진짜 원인이 렌더 경로**임을 찾았다.
포스트가 갤러리를 raw HTML로 쓰는 자리(3개 글·17개 파일·**113개**)가 `/_next/image`를
**아예 안 타고 있었다.** 렌더러에 마지막 패스를 넣고, 포스트에 `width/height`(CLS 13장)와
레이아웃별 `sizes`를 박았다. **갤러리 414,380 → 183,268 bytes (−56%, Playwright 실측).**
...
3. **글자가 든 이미지는 하한을 올려라.** 독자가 확대해도 브라우저는 더 큰 후보를 **다시 안 받는다.**
   일정표 9장은 `sizes=750px` 고정(§9-2 가독성 우선).
...
| 다음 시한 | **8/10 K Poker Cup·APPT 마닐라 종료**(스파인 기록) · **8/16 APT 인천 종료 → 9개 언어 아카이브** |
