...
> **현재 상태 (2026-08-08 마감 시점)**
> **build 69 blog posts** · GTO 솔버 시리즈 ✅ **13 / 13 완결**(⑫⑬ 발행 + 체인 닫기 + `/solver` 허브).
> 시리즈 작업은 **끝났다.** 아래는 그다음 할 일이다.

## ▶▶ 다음 세션이 할 일 — **다국어 동기화 S2 (판정 끝남, 적용만 남았다)**

```
1. git pull
2. docs/multilang-sync-plan.md 의 §0-C(규칙·세션분할) + §0-D(판정표) 만 읽으면 된다
   ★S1(판정)은 2026-08-09에 끝났다. 다시 판정하지 마라.
3. §0-D 「S1 판정 완료 — 최종 적용 대상」 표 그대로 적용
4. 편집 → audit:hard --locale=<code> → build → 커밋·배포
```

### 착수 전 반드시 알 것

1. 🔴 **판정을 다시 하지 마라.** 12항목 × 로케일이 §0-D 표에 확정돼 있다.
   원문을 직접 읽고 판정한 결과이고, **오탐 3건과 «정상이라 손대지 않는 것» 2건까지 기록**돼 있다.
2. 🔴 **한글·다국어 파일을 PowerShell로 치환하지 마라** (CLAUDE.md §16).
   Node `.mjs`에서 `readFileSync(p, "utf8")`.
3. 🔴 **E1(사이드팟)은 값 교체가 아니라 «문장 재작성»이다** — 22개 언어로 한 문장씩 써야 한다.
   나머지와 같은 세션에 넣으면 둘 다 급해진다. **별도 세션 권장.**
4. **작업 순서는 «항목별», 그리고 `de`·`id`·`pt` 먼저.** 12항목 중 11항목이 이 셋이다.

### 적용 대상 요약 (상세는 계획서 §0-D)

| 유형 | 항목 | 파일 |
|---|---|---:|
| **기계적 값 교체** (먼저 하면 좋다) | E2 족보표 +키커 · E3 tiebreak 동일 · E4 `$25(+$10)`→`$24(+$9)` · E5 `1 in 407`→`408` · C1 임플라이드 FAQ 공식 · C2 스트래들 두 자리→한 자리 · C4 `80–86%`→`80–85%` | ~41 |
| **문장 수정** | C7 케이스 킹 반증 · C8 3bb 버튼 이유 · C10 차트 평균 주장 · C11 하이카드 설명 | ~19 |
| **문장 재작성(별도 세션)** | E1 사이드팟 | 22 |

### 아직 판정 안 끝난 3항목

`C5`(머킹 공개 요청권한) · `C6`(핸드포핸드 동시탈락) · `C9`(보드 플러시 개선 조건) —
셋 다 **`de`는 오류 확인**됐고 나머지 로케일만 미확인이다. 적용하는 김에 같이 판정하면 된다.
