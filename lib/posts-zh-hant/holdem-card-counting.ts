...
==補牌（out）== 就是任何一張還沒現身、能把你的牌升級成大概會贏的牌。一個同花聽牌有 ==9 張補牌==（同花色 13 張，扣掉你已經看到的 4 張）——牌面上那兩張同花色的牌已經算在這 9 張裡扣掉了，別再把它們當成死牌扣第二次。用 ==四二法則（Rule of 4 and 2）== 把補牌換成一個粗估勝率：還剩兩張牌要發時乘以 4，只剩一張時乘以 2。
...
按你看到的死牌做調整 | 扣掉任何你在牌面之外看到的補牌——發錯牌時翻出來的一張、被亮出來的一手、剛好瞄到的鄰座棄牌。是剛好瞄到，不是刻意去看——故意去看別人的底牌，在任何一家牌房都算違反禮儀。你看得到的牌，就是對手不可能有的牌
:::
