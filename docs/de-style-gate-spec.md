...
## 6-C. 🔴 3차 튜닝 루프 (2026-08-10 · Session 2 착수 전) — **표기 42편 일괄 확정 + D11·D12**

핸드오프의 「사장님 판단 대기」 2건(ASCII 따옴표 · Gedankenstrich)을 **정본 §7-1 기준으로 확정**하고
42편을 일괄 정리했다. 셀프테스트 **87 → 99개**.

| # | 넓힌 규칙 | 왜 필요했나 (실측) | 정리한 것 |
|---|---|---|---|
| ⑬ | **D11 산문 ASCII 따옴표** | 🔴 D10은 「여는 `„`가 있을 때」만 봤다. 그래서 **처음부터 끝까지 ASCII로 쓴 25편 294쌍은 원리상 침묵**했다. 그 침묵을 «정상»으로 고정하는 픽스처까지 있었다(`ASCII 따옴표만 쓴 줄 — 판정 대상 아님`) | 본문 294쌍 + 메타 6쌍 → `„…“` |
| ⑭ | **D12 Gedankenstrich** | ` — ` **2,364곳** vs ` – ` **9곳**. 즉 «소수가 옳고 다수가 틀린» 상태였다. 정본 §7-1도 –로 적혀 있다 | 2,364곳 → ` – ` |

### 이 확장이 드러낸 것 2가지

1. 🔴 **「공백 양쪽」으로 규칙을 좁게 잡으면 삽입구를 닫는 `—,`가 통째로 빠져나간다.**
   일괄 치환 뒤 **정확히 그 자리에 5곳이 남아 있었다**(hand-rankings·split-pot·strategy·texas ×2,
   그리고 하이라이트 마커에 붙은 `==—`). → D12를 **em dash 전부**로 넓혔다.
   독일어 산문에 em dash가 옳은 자리는 없다.
2. 🔴 **`stripNonProse`의 `\]\([^)]*\)`는 링크 타이틀 안에 괄호가 있으면 샌다.**
   실측: `holdem-straddle` L70 `"Ein Button- (Mississippi-) Straddle …"`.
   → D11 전용으로 **한 겹 중첩까지 허용하는 `stripLinkTitles`**를 먼저 건다.
   안 그러면 `"thumb:…"`가 산문 인용부호로 오탐된다.

### 치환 절차 (한 건도 어긋나지 않은 이유)

`.mjs` 드라이런 → **변환 대상 300쌍 전문 출력 → 육안** → 사후조건(`](…„…)`·`<…„…>` 누출 0) → `--apply`.
사후조건이 없었다면 위 ②는 **빌드는 통과하고 렌더만 깨지는** 결함이 됐을 것이다.
