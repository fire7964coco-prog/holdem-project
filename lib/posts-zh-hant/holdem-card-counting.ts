...
阻擋牌也能==部分==發揮作用。在一個 ==b:Q-J-9== 的牌面上，堅果順子是 K-10。K-10 正常有 16 種組法（4 張 K × 4 張 10）；如果 K 或 10 有一張正握在你自己手裡，就把它砍到 ==12 種組合==，於是堅果順子出現在對手範圍裡的機率就少了 25%。這正是現代詐唬選牌的核心——更多細節在 [3bet 與阻擋牌指南](/zh-hant/blog/holdem-3bet) 和[讀牌面與範圍思維](/zh-hant/blog/holdem-reading-the-board)裡。

### 移除效應與死牌（Card Removal）

你看得到的每一張牌，都在移除一些可能性。在德州撲克裡，補牌不可能躺在牌面上——真躺在那裡，你的牌早就成了——所以要追蹤的==死牌==是那些在牌面**之外**露出來的：不小心翻出來的一張、進蓋牌堆前被亮出來的一手、你剛好瞄到的鄰座棄牌。你看到的每一張，都是你已經不再擁有的補牌。好手就是在每一條街上不停地做這種安靜的微調。這就是算牌，只是不需要維持一個 running total 的那種。
...
在德州撲克裡，唯一會正面發出的牌就是那五張公共牌——其他你看到的都是意外（不小心翻出的牌、棄牌時被亮出的一手），所以幾乎沒有東西可以追蹤。但梭哈——以及同樣發明牌的梭哈系玩法，像 Razz 和梭哈高低分——獎勵的，正是 21 點算牌者最擅長的那種記牌本領。這是撲克裡最接近電影版的一種玩法。
...
:::steps
每個聽牌都算補牌 | 一有聽牌，就數出能讓它成牌的那些牌，然後乘一乘——×4 只在兩張牌都會發出來時用（你已經全下，或者後面沒有下注），否則就只算下一張牌 ×2。當這個勝率打過那個價錢時就跟
