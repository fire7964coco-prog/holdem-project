...
export const CALC_DICT_MS: CalcDict = {
  ...
  outs: {
    ...
    exactNote: "Nombor besar itu ialah angka yang tepat",
  ...
  //    1010 «do not open-limp» · AJs·KJs «raise or fold» · A10s·Q10s «fold from EP».
    hands: {
      ...
      "88": { desc: "Pocket pair dengan potensi set yang baik", action: "Raise di posisi lewat; di posisi awal boleh call raise — jangan open-limp" },
      ...
      "77": { desc: "Tangan untuk cari set, berjaga-jaga dengan overcard", action: "Raise di posisi lewat; di posisi awal call raise — jangan open-limp" },
      A9s: { desc: "Suited ace dengan potensi flush", action: "Raise di posisi lewat" },
      "66": { desc: "Cari set; perlukan implied odds", action: "Call raise apabila anda dan pe-raise itu ada sekurang-kurangnya ~15× saiz call itu di belakang; raise di posisi lewat" },
      "55": { desc: "Kurang nilai tanpa set", action: "Call satu raise di posisi lewat dengan syarat baki ~15× yang sama; raise first in dari posisi lewat" },
      ...
      "87s": { desc: "Suited connector yang baik", action: "Call/raise di posisi lewat" },
      "76s": { desc: "Suited connector", action: "Raise first in dari posisi lewat; call raise hanya dengan posisi dan stack deep" },
      "44": { desc: "Hampir tiada nilai tanpa set", action: "Call raise hanya apabila ada ~15× saiz call itu di belakang" },
      "33": { desc: "Perlu cari set; spekulatif", action: "Pot multiway dengan stack deep sahaja" },
      "22": { desc: "Pocket pair paling rendah", action: "Pot multiway dengan stack deep sahaja" },
      ...
      "65s": { desc: "Suited connector; mahukan multiway/implied odds", action: "Call raise di posisi lewat" },
      "54s": { desc: "Suited connector rendah; spekulatif", action: "Call raise di posisi lewat, pot multiway yang murah" },
    },
  },
