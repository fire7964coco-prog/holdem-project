...
🟢 **B의 입력 = 브리프 + EN 마스터 파일(읽기 전용 · 본문 골격 복사용)** — 🅱 파일럿(09-26)이 정한 해석을 헤드가 승인했다. 편당 20KB 산문을 브리프에 복제하지 않는다. 대신 **웹·MCP·다른 로케일 파일은 B에서 열지 않는다.** 사실·수치·카드의 출처는 EN 축어뿐이다.
🔴 `/clear`로 끊는 이유: 브리프(+EN)가 **유일한 입력**이어야 B가 기억으로 사실을 쓰지 않는다(`hardening-protocol` §2 «B 구간에 도구 왕복을 넣지 마라»). 컨텍스트를 비우면 Opus도 싸진다.
...
- 🔴 **원시 HTML 줄도 구조다** — `<a id="…"></a>` 앵커 · `<div style=…>` 카드 그리드 · `<br/>`를 마크다운과 같이 목록에 넣는다. B는 이 줄들을 **축어로** 옮긴다.
  (09-26 🅱 파일럿: EN `holdem-probability` L160의 `<a id="pot-odds"></a>`를 놓치고 «EN 앵커가 죽어 있다»고 오판했다. 페이지 내 앵커 `(#…)`의 대상은 **빌드 산출물에서 확인**한다: `grep -o 'id="<앵커>"' ../Holdem_Project/.next/server/app/en/blog/<slug>.html` — slugify 규칙으로 추론하지 마라.)
- 🔴 **«EN이 틀렸다»(EN-먼저)는 렌더 결과를 봐야 하는 주장이면 빌드 산출물로 확인한 뒤에만 올린다.**
