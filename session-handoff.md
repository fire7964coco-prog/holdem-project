...
### 1. 🆕 「홀덤 gto 표」 210 — 유일하게 큰 미공략 키워드

의도는 «프리플랍 레인지 차트를 달라»에 가깝다. **13편 시리즈는 플랍 전략이라 매치가 어긋난다.**
「홀덤 gto 차트」20 + 「홀덤 레인지 차트」40과 묶으면 **270**이다.
→ **별도 글**을 쓰거나 **`/solver` 랜딩**이 받아야 한다. 기존 `/hand-chart` 도구와의 카니발 여부를 먼저 볼 것.

### 2. 솔버 세션에 전달할 URL 13줄

`docs/gto-solver-series-spec.md` **§6**에 「파일id: URL」 13줄이 확정돼 있다. 그대로 복사하면 된다.
솔버 교육 예제 화면에 「해설 보기」 링크로 연결하는 것이 목적이다.

### 3. 이미 해결된 것 (지난 핸드오프의 «남은 일»에서 내려옴)

- ~~`/solver` 랜딩이 「무료」를 안 노린다~~ → **「무료」는 이미 반영돼 있다.** title이
  「무료 GTO 솔버 — 설치 없이 브라우저에서 실행」이고 desc·FAQ에도 여러 번 들어간다.
  지난 핸드오프의 이 항목은 그 부분에 한해 **낡은 정보였다.**
  ⚠ **다만 「프로그램」(140)·「계산기」(20)라는 «단어»는 아직 메타에 없다** — desc는 "직접 계산하세요"까지다.
  desc에 두 단어를 자연스럽게 넣는 것은 여전히 값싼 개선이다(글 하나 안 쓰고 160자 안에서 끝난다).
- ~~`/solver`에 13편 허브 목록~~ → **완료.** `app/solver/solver-client.tsx`의 `SPOT_GROUPS`
...
### ▶ 0-❺. 2026-08-08 (5) — GTO 시리즈 ⑦~⑪ 5편 발행 ✅ (11/13)

> **커밋 `f3a7b1c`** · build 67 blog posts · 게이트 10편 🔴 0건 · canonical 0건 ·
> FAQ 편당 4문항 · 라이브 5편 200 확인.
> 상세는 `WORKLOG.md` 「2026-08-08 (5)」. ⚠ 착수 전 **`git pull` 먼저.**

**한 일** — ⑦ `low-board-check-raise` · ⑧ `3bet-pot-cbet` · ⑨ `3bet-pot-bet-sizing` ·
⑩ `3bet-pot-low-board` · ⑪ `blind-battle-cbet` 발행 + 기존 글 4곳에 역링크 +
⑥의 readnext를 ①→⑦로 교체(체인 연장) + 키워드 뱅크 §3-B 신설.

**★적대검수가 기계 게이트 뒤에서 사실오류 4건을 잡았다** (게이트는 10편 전부 「0건」이었다)

| 편 | 결함 | 잡은 렌즈 |
|---|---|---|
| ⑩ | "셋만 빼면 오버페어를 이기는 핸드 없음" — BTN에게도 오버페어 24콤보 | 실전·검산 |
| ⑧⑪ | "①~⑦에서 96~99.9% 체크" — ④ 76.2%·⑤ 88.8% | SEO·교열·검산 |
| ⑨ | "①~③의 드라이 보드" — ③은 투톤(드로우 없음 24.7%) | SEO·교열·검산 |
| ⑦⑪ | **형제 글 보드 무늬 오기**(① `A♥7♦2♣` · ③ `Q♠J♦T♠`) | **검산 단독** |

**★키워드 함정** — 🔴 **「씨벳」 월 6,600은 포커 용어가 아니다**(자동완성 「씨벳 먹튀·씨벳 코드」 =
베팅 사이트 브랜드). 「홀덤 c벳」 20인데 「씨벳」만 6,600 = **자릿수 차이 자체가 이상 신호**다.
...
`holdem-cbet-strategy`·`holdem-check-raise`·`holdem-value-bet-sizing`에 역링크.

**★다음 세션이 반드시 알 것**

1. 🔴 **원본 md를 옮겨 쓰지 마라. 라이브에서 다시 뽑아라.**
   `node scripts/capture-solver-spots.mjs <key>` → `make-solver-range-charts.mjs` →
   `convert-solver-captures.mjs <key>`. 수치는 `.solver-captures/data.json`이 원천이다.
   원본 md와 라이브가 **실제로 달랐다**(①편 체크 99.0% → 98.2%, ②편 99.9% → 99.8%).
2. 🔴 **옛 `_ip` 캡처 26장은 삭제했다.** 보드가 달라도 전부 같은 그림(IP 프리플랍 레인지)이었다.
   핸드오프에 적혀 있던 "차이 0.2~0.9%"는 **비교 대상을 잘못 짝지은 오진**이다.
   같은 스팟 `-oop`↔`-ip`는 54.8% 차이다. 이제 히어로는 `-oop`, 본문은 `-ranges`(자체 인포그래픽)다.
3. 🔴 **⑧~⑬은 조건이 다르다.** 3벳팟 = OOP가 **BB(3벳터)** · 팟 22.5bb · 스택 89bb /
   블라인드전 = OOP가 **SB(오픈레이저)** · 팟 6bb · 스택 97bb. SRP의 「팟 5.5bb」 표를 복사하면 전부 틀린다.
4. **템플릿은 ①이 아니라 ②(`k-high-board-cbet.ts`)를 복사하라.** ①은 초판의 흉터를 고친 글이다.
5. **CLUSTERS에 이 시리즈를 넣지 마라.** 넣었더니 편당 🔴 11~12건이 났고 전부 오탐이었다
   (편마다 보드가 달라 표 값이 달라야 정상). 근거는 `audit-hardening.mjs` 주석과 spec §5-1.
6. **적대적 검수는 렌즈를 갈라야 값이 나온다.** 이번에 기계 게이트가 초록인 상태에서
   「현역 플레이어」 렌즈만 **①편의 «A7=탑 페어» 사실오류**를 잡았다(실제로는 투 페어).
   4렌즈(검산·실전·교열·SEO) 병렬 + **2차 교열 패스**까지 돌리는 것을 유지할 것.
...
> ⚠ **이 항목의 «이미지·수치» 내용은 0-❹에서 뒤집혔다.** 아래는 기록용이다.
> 커밋 `c00d2af`(랜딩) · `0624b67`(①편+캡처 26장).

**사장님 지시 (2026-08-08 확정)**
- 이 세션은 ①편까지만. **다음 세션부터 «자율 모드»** — 확인 없이 판단해서 배포까지 진행한다
- **한 세션 5편** → `/clear` → 새 세션 5편 → 마지막 2편
- **편당 적대적 검수 필수 · ★Agent 툴(서브에이전트)로 돌린다**(사장님 허가). 리포트가 본 세션
  컨텍스트를 안 먹어야 5편이 안정적으로 들어간다. 렌즈를 갈라라(검산/실전/교열/SEO)
- **배포는 묶어서** — 5편 모아 한 번에. 시리즈 내부링크를 함께 엮기 위해

**끝난 것**
- `solver.holdemmaster.com` 배포됨 · **noindex**(검색은 본체 `/solver`로 일원화 — 서브도메인은
  구글에게 별도 사이트라 본체 도메인 권위를 못 받는다. 반대 증거가 `/calculator` 노출 463·6.5위)
- `/solver` 랜딩 발행 — 원본의 **사실 오류 2건 수정**: 「모바일 미지원」(아이폰에서 정상 렌더 확인) ·
  경쟁사 가격 수치(1차 출처 확인 불가 → §12-B에 따라 제거)
- 캡처 **26장** 크롭·변환 (`scripts/convert-solver-captures.mjs`)
- **①편 `a-high-board-cbet` 발행** — 이 파일이 나머지 12편의 **템플릿**이다

**★반드시 알 것**
1. **이 시리즈는 §8 「경험담 필수」를 적용하지 않는다**(사장님 결정). 1차 데이터가 본체이고
   **재현 가능성이 출처를 대신한다** — 계산 조건 표 · 0.1bb 단위 안내 · 교육 예제 CTA 3종 필수.
2. ~~캡처 파일명의 `_ip`/`_oop`는 플레이어 구분이 아니다~~ → **0-❹에서 오진으로 판명.**
   `-ip`는 IP의 프리플랍 레인지 화면이라 보드별 정보가 0이었다(13장 삭제). spec §3-2 참조.
3. 🔴 **`audit:hard`가 이 시리즈를 «미검사»로 흘린다**(커버리지 "카드 문단 0개"). 0건은 검증이 아니다.
   **손 검산 필수**: 팟 = 오픈+콜+SB · EV 합 = 팟 · **EQR 역산 = EV ÷ (에퀴티 × 팟)**.
   ①편은 이 검산을 전부 통과했다(오차 0.2%p) → 솔버 출력이 정합하니 나머지도 수치는 신뢰 가능.
4. 어투는 **존댓말+평서체**(본체 톤). 참고했던 포커고수 글은 반말이지만 **가져온 건 «전개»뿐**이다 —
   독자의 오해를 먼저 꺼내 반박 / 조건을 바꿔 대비 / 개념을 그림으로 증명.

**남은 것** — ②~⑬ 12편(목록·키워드·slug는 spec 문서 §4) · 완료 후 허브-스포크 링크 ·
`audit-hardening.mjs` CLUSTERS 등록 · **솔버 세션에 "파일id: URL" 목록 전달**.

### ▶ 0-❶. 2026-08-08 — FCP 병목은 «폰트 선언 파싱»이었다 → 자체 서브셋으로 마감 ✅

> **작업 트리 깨끗(문서 커밋 제외) · build 609 HTML · 라이브 검증 완료.**
> 커밋 `77fb021`(psi 계측) · `2526df2`(서브셋). 상세는 `WORKLOG.md` 「2026-08-08」.
>
> ⚠ 착수 전 **`git pull` 먼저.**

**한 일** — 지난 세션이 남긴 «`npm run psi` 먼저»를 실행했고, 그게 **작업 우선순위를 뒤집었다.**

`@font-face` **438 → 69개** · 렌더차단 CSS **414KB → 179KB** · 폰트 CSS 전송 **24,399 → 2,020B** ·
woff2 요청 **48 → 8개** · 홈 **FCP −18% · LCP −32%** · 블로그 **FCP −19%**.
생성기는 `scripts/subset-korean-font.py`(`pip install fonttools brotli`, 이 머신에 Python 3.12 있음).

**★다음 세션이 반드시 알 것**

1. 🔴 **PSI 랩 점수를 판정에 쓰지 마라.** 두 번 데였다.
   - **PSI는 같은 URL의 리포트를 캐시한다** — 3회 반복이 같은 값을 3번 돌려준다(범위가 정확히 0이면 그 신호다).
     캐시 우회는 `?psi=1,2,3` 같은 더미 쿼리.
   - 우회해도 편차가 크다. 배포 후 홈이 **97 / 62 / 65점**으로 나왔다.
   - **결론은 통제 조건에서만 선다**: Playwright + CDP(`Emulation.setCPUThrottlingRate` 4×,
     `Network.emulateNetworkConditions` Slow 4G) · n=4 중앙값.
   - 필드(CrUX)는 **URL별 표본이 아예 없다** — `origin_fallback: true`라 전부 사이트 전체 값이다.
2. 🔴 **내가 처음에 "홈은 98점이라 빠르다"고 보고한 건 캐시된 이상치였다.** 실제는 64~65점.
   **한 번 잰 값으로 우선순위를 뒤집지 마라.**
3. **"바이트"와 "속도"는 다른 축이다.** 지난 세션이 줄인 건 바이트였고, 이번 FCP 병목은
   **네트워크가 아니라 메인스레드**였다(홈은 모든 요청이 576ms에 끝났는데 FCP가 4.05s).
   **`mainthread-work-breakdown`을 먼저 봐라** — Style & Layout이 761~929ms였다.
4. **A/B는 네트워크 왕복을 그대로 태워야 한다.** `route.fulfill`로 빈 응답을 주면 다운로드까지
   건너뛰어 **부당하게 유리한 수치**가 나온다. `route.fetch()`로 실제 요청 후 본문만 바꿔라.
5. **폰트 커버리지를 `document.fonts.check`로 검증하면 안 된다** — h1의 첫 폰트가 Inter라
   **전부 ✓로 나온다**(실제로 이 함정에 빠졌다). `fontTools`로 **cmap을 직접 읽어라.**
6. 스크래치패드의 `.mjs`를 `node file.mjs`로 직접 실행하면 **exit 127**이 났다(원인 미상).
   `node --input-type=module -e "import('./file.mjs')"`로 우회했다.

### ▶ 0-❷. 2026-08-08 (2) — 카니발 재점검 · 색인 실태 전수 · 사이트맵 lastmod ✅

> 커밋 `2372559` · `a4ada32` · `d0954d4`. 상세는 `WORKLOG.md` 「2026-08-08 (2)」.

**한 일** — `/hand-chart` noindex 해제(당일 색인됨) · **색인 실태 전수 점검 도구 신설** ·
사이트맵 lastmod를 실제 변경일로(오늘 날짜 **89 → 4개**) · 고아 글 본문링크 2건.

**★다음 세션이 반드시 알 것**

1. 🔴 **「노출 0」과 「색인 0」은 다른 문제다.** `buy-in-cost`가 노출 0이라 카니발을 의심했는데
   실제로는 **크롤조차 안 된 상태**였다(`Discovered - currently not indexed`).
   **색인 안 된 페이지는 GSC 실적 데이터에 아예 안 나온다** — `gsc-fetch`·`gsc-page`로는 안 보인다.
   → **`node scripts/gsc-index-audit.mjs`** (한국어 전편) · `--tools` · `--all`
2. 🔴 **내부링크를 셀 때 `lib/` 전체를 grep하면 다국어 12개 언어가 합산된다.**
   그래서 "vs-cash-game이 83회 독식"이라 보고했다가 **실제 한국어 본문 8회**로 정정했다.
   한국어만: `grep -roh "](/blog/<slug>)" lib/posts.ts lib/posts/`
3. **카니발을 의심하기 전에 검색어가 실제로 겹치는지 봐라.** 토너먼트 4편은 하나도 안 겹쳤다
   (참가방법 / 세금 / 캐시게임비교). 합치거나 몰아주면 오히려 손해였다.
4. **사이트맵 lastmod 함정 둘** — ①`layout.tsx` 같은 공통 파일을 소스로 넣으면 또 전부 같은 날짜가 된다
   ②로케일은 동적 `[locale]`이 아니라 **언어별 실제 디렉터리**(`app/en`·`app/ja`…)다.
5. **블록주석 안 `**` + 슬래시**는 주석을 끝내버린다(빌드 1회 파손). §12-A 백틱 금지와 같은 계열.

**⚠ 남긴 것 — 다음 1순위 후보**

| 항목 | 메모 |
|---|---|
| 🔴 **홀덤펍 지역 10개 전부 미색인** | `/pub` 허브만 색인됐고 지역 10개는 전부 밖이다(`/pub/suwon`·`daegu`는 구글이 URL 존재조차 모름). 원인 확정: **10개가 사실상 같은 페이지** — h2가 지역명만 바뀐 동일 구조, 지역명 제거 시 본문 **79% 일치**, 2,000자. 링크 문제 아님(허브가 10개 다 링크). ★**사장님 보류** — 현장 방문·촬영이 필요하다. 검증하려면 **강남 1곳만** 제대로 채워 색인되는지 먼저 볼 것 |
| **`/hand-chart` 4주 뒤 재측정** | 2026-09-05경. **포스팅(`starting-hand-range`) 노출이 떨어지면 진짜 카니발이니 되돌린다.** 기준선: 포스팅 노출 16·클릭 1 / 차트 0 |
| `buy-in-cost`·`community-event-guide` | 색인 요청 완료·본문링크 추가 완료 → **2주 뒤 `gsc-index-audit`로 재확인** |
| `/holdem-practice` 미색인 | 8/5에 SSR을 복구했는데 아직 색인 전. 시간 문제일 수 있다 |
| **필라 구조 정리** | `holdem-glossary`는 **미발행**, `holdem-tournament`는 단일 대표글이 없다(4편으로 분산). 문서에 대응표는 박았지만 **구조 자체는 사장님과 정리 필요** |
| **Inter/Lora `@font-face` 65개** | 남은 69개 중 66개가 이쪽이다. A/B에서 「전부 제거」와 「Noto만 제거」 차이가 **56ms**였으니 이득은 작다. 라틴이라 서브셋도 쉽다 — **값어치 대비 판단 필요** |
| 🔴 **홈 raw `<img>` 1,008건** | 지난 세션의 1순위. 홈 총 페이로드가 **3,015KB**다. 다만 **LCP엔 거의 영향 없다**(전부 lazy·화면 아래). 얻는 건 모바일 데이터·대역폭. 홈 피드 카드 출처 파일을 **먼저 찾아야 한다** |
| ja/zh CJK 폴백 | 서브셋에 한자가 없어 시스템 폰트로 간다. **넣으려면 1.37MB라 불가.** 실제 독자 기기(iOS/Android)에서 어떻게 보이는지는 **아직 안 쟀다** |
| 🔴 **CLAUDE.md 필라 슬러그 6개가 404** | §8에 대응표를 박아뒀다. `holdem-glossary`는 **미발행**이고 `holdem-tournament`는 단일 글이 없다 — **필라 구조 자체를 사장님과 정리할 필요** |
| `apt-incheon-2026-guide`(ko) 메타 🔴 | desc 120자·seoTitle 34자. 2026-08-07분 그대로 |
| 고아 이미지 59장 · 60~80KB 41장 | 규정 내라 미착수 |
| `npm run typecheck` 480건 | `ignoreBuildErrors: true`라 빌드는 통과 |
| 🔴 **다음 시한 (임박)** | **8/10 K Poker Cup·APPT 마닐라 종료** · **8/16 APT 인천 종료 → 9개 언어 결과 아카이브**. 상세는 `docs/update-calendar.md` |

### ▶ 0-◗. 2026-08-07 — 생 `<img>`가 최적화를 우회하던 것 (본문 113개 + 카드 + 히어로) ✅

> **작업 트리 깨끗 · build 56+457 intl(615) · canonical 0건 · 산출물 609개 스윕 · 라이브 A/B 실측 완료.**
> 커밋 **`14e43af` ~ `f801fe6`**(5개). 상세는 `WORKLOG.md` 「2026-08-07」.
> 보고서: <https://claude.ai/code/artifact/f61831c6-00b2-4ab0-bfe5-c730cde30294>
>
> ⚠ 착수 전 **`git pull` 먼저.**

**한 일** — 「폭 750px 미만 15장」을 확인하러 갔다가 **진짜 원인이 렌더 경로**임을 찾았다.
포스트 본문(3개 글·17개 파일·**113개**)·**로케일 목록 카드**·**대회 가이드 히어로**가
`/_next/image`를 **아예 안 타고 있었다.**

| 대상 | before | after (라이브 실측) |
|---|---:|---:|
| `/de/blog` 카드 42장 | 1,488,918 B | **311,315 B (−79%)** |
| apt-incheon 갤러리 8장 | 414,380 B | **87,432 B (−79%)** |
| korea-poker-marathon 5장 | 209,715 B | **62,503 B (−70%)** |
| 대회 가이드 히어로(배너+본문) | 80,905 B | **29,709 B (−63%)** |
| └ 배너 단독 | 61,072 B | **9,876 B (−84%)** |

사이트 전체 최적화 미적용 `<img>` **1,457 → 1,008건**.
⚠ 로컬 빌드로 재면 수치가 나쁘게 나온다(−56%) — **Vercel이 AVIF로 협상**하므로 라이브가 기준이다.

**★다음 세션이 반드시 알 것**

1. **`next/image`는 확대는 안 하되 재인코딩은 한다.** 그래서 "폭<750 = 원본 그대로"는
   **의심해야 정상**이었다. 실측하니 정말 같았다(74,688 == 74,688). **바이트로 재라.**
2. **표시 폭은 «레이아웃»이 정한다.** 컨테이너가 672px이어도 2·3단 그리드면 336·222px다.
   `sizes`가 틀리면 최적화를 태워도 절감이 0이다. **Playwright로 `getBoundingClientRect()`를 재라.**
   🔴 **본문 폭도 레이아웃마다 다르다** — 일반 블로그 **672px** vs **대회 가이드 906px**
   (`tournament-guide-post.tsx`). 906px 자리에 `sizes=750px`를 넣어 «더 무르게» 만들 뻔했고
   **라이브 실측에서야** 잡혔다. 빌드 산출물 검사로는 안 잡힌다.
3. **글자가 든 이미지는 하한을 올려라.** 독자가 확대해도 브라우저는 더 큰 후보를 **다시 안 받는다.**
   일정표 9장은 `sizes="(max-width: 768px) 100vw, 1080px"` — 데스크톱은 **원본 900px 그대로**,
   모바일만 480으로 준다. 용량은 AVIF로만 줄었다(장당 78,029 → 41,660 B, **해상도 손실 0**).
   §9-2 「가독성이 용량보다 위」. ⚠ 처음에 `750px`로 넣었다가 906px 레이아웃이라 되돌렸다(아래 2번).
4. 🔴 **PowerShell `Set-Content`로 소스 파일을 쓰지 마라.** BOM+CRLF가 들어가 diff가 9→243줄로
   터졌다. node로 쓸 것.
5. 🔴 **Grep 출력의 백슬래시는 표시 artifact다.** `src="\images\…"`로 보여 «경로 누수»로
   오판했는데 파일 원문·라이브 모두 정상이었다. **Read로 확인하고 결론 내라.**
6. `audit:hard`·`canonical:check`는 통과. 다만 **게이트는 이미지 전송량을 안 본다** —
   이 결함은 게이트가 아니라 **실측**으로만 잡힌다.

**★Vercel 이미지 변환 쿼터 — 실측(2026-08-08)과 결론**

이미지를 최적화 경로로 옮기면 **Vercel 이미지 «변환»이 과금·쿼터 대상**이 된다. 확인 결과:

| 항목 | 사용 / 한도(Hobby) |
|---|---|
| Image Transformations | **2,238 / 5,000** (30일) |
| Image Cache Reads | 9,219 / 300,000 |
| Image Cache Writes | 6,381 / 100,000 |

여유가 있었다(차트의 큰 봉우리는 8/1 본문 최적화분이지 8/7분이 아니다).
`minimumCacheTTL` 1년이라 **변형 종류 수만큼의 일회성 비용**이지 페이지뷰마다 나가지 않는다.
→ **사장님이 Pro 전환을 결정하셨다.** 쿼터 압박은 사실상 해소.

🔴 **그리고 «사다리 깎기»는 값어치가 거의 없다는 걸 확인했다** (`6f74c0a`).
고정 크기 카드에서 `sizes`를 빼 후보를 **12 → 2**(480·1080)로 줄였고 전송 바이트는 동일했지만,
사이트 전체 조합은 **1,878 → 1,834(−44)**밖에 안 줄었다. 이유:
- 변형 집합은 **페이지 합집합**이다. 카드가 쓰는 파일은 히어로와 **같은 파일**이라
  다른 소비처가 이미 전체 사다리를 깔아 두면 카드만 줄여도 합집합이 안 준다
- 남은 사다리 생성원은 **관련글 썸네일**(`fill`이라 `sizes`가 **필수** — 뺄 수 없다)과
  반응형 히어로다. **개별 소비처로는 못 줄인다**
- 유일한 레버는 **`next.config.mjs`의 `deviceSizes`/`imageSizes` 축소**(현재 8+7=15종)인데
  **전 이미지에 영향**을 주므로 별도 측정이 필요하다. **Pro 전환으로 급하지 않다 — 하지 마라.**

### ✅ [완료 2026-08-08] 다음 세션 **맨 처음** 할 일 — `npm run psi` 먼저 (사장님 지시)

> **실행했고 결론이 나왔다 — 위 0-❶ 참조.** 요약: 이 작업은 «바이트»엔 도움이 됐지만
> FCP의 진짜 병목은 이미지가 아니라 **폰트 선언 파싱**이었다. 그리고 psi 스크립트 자체가
> 키를 안 읽어 고장나 있었다(`77fb021`). 아래는 그때의 문제의식 기록.

> 사장님 질문: *"지금 하고 있는 이미지 정리 작업이 속도 개선에 도움이 되는 건 맞는 거지?"*
> → **정직한 답은 «반쪽»이다. 나는 «바이트»를 쟀지 «속도»를 재지 않았다.**

| 이번 작업 | 속도에 실제로 도움? |
|---|---|
| 대회 가이드 히어로 (−51KB, 이중 preload 제거) | ✅ **진짜다** — `eager`+최우선순위라 **첫 화면 그리기 경로**에 있었다 |
| `width`/`height` 없던 104장에 치수 주입 | ✅ **CLS 개선** — 코어 웹 바이탈·순위 신호 |
| 카드·갤러리 −79% (절감량 최대) | ⚠️ **LCP엔 거의 영향 없다** — 전부 `lazy`+화면 아래다. 얻는 건 모바일 데이터·스크롤 부드러움·대역폭 경쟁 |

⚠ **반대 신호도 있었다**: apt-incheon 페이지의 **LCP 요소는 이미지가 아니라 `<p>` 문단**이었다.
그 페이지에선 애초에 이미지가 병목이 아니었을 수 있다.
⚠ 작은 반대급부: `/_next/image`는 캐시 없는 첫 요청에 변환이 한 번 돈다 →
**드문 페이지의 첫 방문자는 오히려 살짝 느릴 수 있다**(이후엔 엣지 캐시).

**→ 그래서 홈 작업에 들어가기 전에 `npm run psi`로 현재 LCP·CLS·TBT를 먼저 확보한다.**
그래야 홈 1,008장을 고친 뒤 «정말 빨라졌는가»를 **바이트가 아니라 속도로** 증명할 수 있다.
지금은 근거가 반쪽이다. (참고: 무한 애니메이션이 LCP를 착시로 부풀린 전례가 있다 —
`docs`의 속도 진단 기록과 `perf-lessons-lcp-and-bundle` 메모 참조.)

**⚠ 남긴 것 — ★다음 세션 1순위 후보**

| 항목 | 메모 |
|---|---|
| 🔴 **★다음 1순위 — 홈 페이지들에 raw `<img>` 1,008건** | 카드·히어로를 마감해 1,457→1,008로 줄었고, **남은 건 전부 «홈»이다**: **`/`(한국어 홈) 혼자 114장** · `/es` 94 · `/en`·`/ja`·`/zh-hant`·`/zh` 각 86 · `/de` 84 · `/id` 82 · `/pt` 80 · 소규모 로케일 홈 15종 각 12. **사이트에서 가장 중요한 페이지**이고 일부는 `eager`다. 후보 파일: `app/home-client.tsx`(2곳)·`app/rules/rules-client.tsx`·`app/tournaments/tournaments-client.tsx`(3곳) — 다만 홈 피드 카드가 어디서 나오는지 **먼저 찾아야 한다**(위 파일들만으로는 114장이 안 나온다) |
| ✅ 카드·히어로는 마감 | `6de535d`·`c87650d`. /de/blog 42장 **1,488,918→311,315 B(−79%)** · 히어로 **80,905→29,709 B(−63%)** |
| ★**이중 preload는 실제로 발생 중이었다** | `page.tsx:294~298` 주석이 경고하던 그 함정이 **대회 가이드에서 현실이 돼 있었다** — `fetchPriority="high"` 생 `<img>`를 보고 **React가 raw 경로 preload를 자동 생성**해, 페이지의 최적화본 preload와 어긋난 채 둘 다 받고 있었다. `next/image`의 `priority`로 바꾸면 srcset·sizes가 일치해 사라진다. **다른 곳에도 같은 조합(`생 <img>` + `fetchPriority=high`)이 있으면 의심하라** |
| ★**장식 이미지엔 1:1 밀도를 주지 마라** | 배너는 opacity 40%라 `sizes=50vw`+`q=55`로 **61,072→9,876 B(−84%)**. 실제 표시 조건을 재현해 **육안 비교로 확인하고** 적용했다. ⚠ **본문 이미지엔 쓰지 말 것** |
| `apt-incheon-2026-guide`(ko) 메타 🔴 | `audit:hard`가 desc **120자**(기준 ~95)·seoTitle **34자**(기준 ~32)로 잡는다. **이번 작업과 무관한 기존 건**이고 CTR 훅 판단이 필요해 손대지 않았다(§17: 호기심형 제목을 정보형으로 단독 교체 금지) |
| 고아 이미지 59장 · 60~80KB 41장 | 2026-08-06분 그대로. 규정 내라 미착수 |
| `/en/win-rate-quiz` | ko는 채웠지만 en은 얇다(2.81 kB) |
| `npm run typecheck` 480건 | `lib/posts.ts`의 `Category`가 한국어 전용. `ignoreBuildErrors: true`라 빌드는 통과 |
| 🔴 **다음 시한 (임박)** | **8/10 K Poker Cup·APPT 마닐라 종료** — 우리 글 없음, `docs/tournament-spine.md`에 결과만 기록 · **8/16 APT 인천 종료 → 9개 언어 결과 아카이브 전환**(ko·en·ja·zh·zh-hant·es·de·pt·id). ja판 훅이 "2025년 일본 37.8%"라 **2026년 국별 내분이 나오면 훅을 갱신**해야 한다. 상세는 `docs/update-calendar.md` |

### ▶ 0-◖. 2026-08-06 (사장님 부재 중 자율) — 시한 대회 결과 2건 · 퀴즈 SSR · 기계 결함 ✅

> **작업 트리 깨끗 · selftest 64/64 · canonical 0건 · build 56+457 intl · 라이브 확인 완료.**
> 커밋 `aa2189f`~`55d4836`(6개). 상세는 `WORKLOG.md` 「2026-08-06」 + **`docs/autonomous-run-2026-08-06.md`**.
>
> ⚠ 착수 전 **`git pull` 먼저.** 이번엔 사장님 허가로 이 폴더에서 포스트를 손댔다(예외).

**한 일** — 시한이 지나 있던 WSOP 2026 ME 결과(6개 언어) · 홀덤 마스터스 제7회 결과 + 제8회 DAY1 28회 ·
`/win-rate-quiz` SSR 본문 0줄 복구 · 기계 결함 6종.

**★다음 세션이 반드시 알 것 — 이번에 밟은 함정**

1. **공식 페이지의 `End Date` 필드를 "사건이 일어난 시각"으로 읽지 마라.**
   WSOP 결과의 `End Date: Aug 05 01:48 AM`을 우승 확정 순간으로 읽어 6개 언어에 박았는데,
   라이브 리포팅은 8월 5일 **저녁**에 헤즈업이 시작됐다고 적고 있었다. 필드는 필드로만 인용하라.
2. **`sed` 전 파일 일괄 치환은 출처·인용 줄을 오염시킨다.**
   `s/$85,634,400/$87,568,080/g`가 ESPN 인용 줄까지 바꿔 **출처가 말하지 않은 수치를 귀속**시켰다.
3. **수치 grep은 축약형까지.** `$85,634,400`만 찾아 en의 `$85.6M`·ja의 `8,560万ドル`를 놓쳤다.
4. **`best7fast`는 `Card` 객체가 아니라 `NCard` 튜플 `[rank 2..14, suit 0..3]`을 받는다.**
   객체를 넘기면 **모든 승률이 정확히 50.00%**로 나온다(전부 동점 처리). 랭크는 `"10"`, `"T"` 아님.
5. **WPL 사이트는 페이지가 스스로 이탈한다** — `browser_evaluate` 안에 `location.href` 가드를
   넣지 않으면 **직전 페이지 내용을 그대로 반환**한다(실제로 WSOP 내용을 받았다).
6. **1층 게이트는 사실·구조 결함을 못 잡는다.** 이번에 `audit:hard`가 잡은 건 메타 길이뿐이고,
   결함은 전부 2·3층에서 나왔다. **2차 교열을 건너뛰면 "한 곳만 고치고 놓친 짝"이 남는다**(이번 4건).
...
| 고아 이미지 59장 · 60~80KB 41장 | 삭제·재압축 판단 필요. 규정 내라 손대지 않음 |
| `/en/win-rate-quiz` | ko는 본문을 채웠지만 en은 여전히 얇다(2.81 kB) |
| 다음 시한 | **8/10 K Poker Cup·APPT 마닐라 종료**(스파인 기록) · **8/16 APT 인천 종료 → 9개 언어 아카이브 전환**(캘린더엔 6개로 과소 기재돼 있었다) |

### ▶ 0-◕. 2026-08-05 (6) — 승률 시뮬레이터 마감 · 연습장 SSR · 모바일 네비 ✅

> **작업 트리 깨끗 · build 615 · canonical 0건 · typecheck 0건 · 라이브 확인 완료.**
> 마지막 커밋 `31689bd`. 커밋 10개(`f6f2803`~`31689bd`). 상세는 `WORKLOG.md` 「2026-08-05 (6)」.
>
> ⚠ 착수 전 **`git pull` 먼저.** 포스트 검수는 이 폴더가 아니다(0-◇ 참조).

**한 일 요약**
- `/holdem-practice` — 정적 HTML에 콘텐츠가 **0**이던 것 복구(h1 0→1). GSC 노출 0의 원인이었다
- 승률 시뮬레이터 — 공개 승률 병기 · 아웃츠/암산 패널 · 모바일 성능 · 레이아웃 · 검수 4건
- **허브 셸** — 모바일에 이동 수단이 없던 것 복구(가로 스크롤 탭)

**🔴 되돌리면 안 되는 것 (이번 세션분)**

1. **`hub-shell`의 모바일 가로 탭을 빼지 말 것.** 빼면 모바일에서 도구·가이드로 갈 방법이
   **푸터밖에 없다**(`/calculator`는 스크롤 7,768px). 하단 탭바는 커뮤니티 4개뿐이라
   대체가 안 된다 — 이 오해가 원래 주석에 그대로 적혀 있었다.
2. **탭 목록을 `hub-shell`에 직접 적지 말 것.** `side-rail.ts`의 `hubPagesFor(locale)`가
   **단일 출처**다. ja·es가 2개뿐인 건 그 언어에 도구 페이지가 없기 때문이다(없는 라우트 금지).
3. **좌석 배치를 화면별로 통일하지 말 것.** 모바일 3줄 / 데스크톱 5줄이다.
   「위아래 배치면 세로가 비싸고, 좌우 배치면 가로가 비싸다」 — 이번에 세 번 갈아엎고 얻었다.
4. **양보·표본을 개수로 고정하지 말 것.** `SLICE_MS`(시간 기준 양보)와
   `BUDGET_MS`/`MIN_SAMPLES`(적응형 표본). 개수로 고정하면 느린 기기에서 그대로 배가 된다.
5. **진행 버튼의 모바일 `sticky`를 빼지 말 것.** 하단 내비 62px + 상단 바 120px이라
   실가용 높이가 782px뿐이다.
6. `/holdem-practice`의 헤더·본문을 `ready` 게이트 **안으로** 되돌리지 말 것(SSR이 빈 껍데기가 된다).
7. **`_equity.ts` 평가 로직은 여전히 무변경.**

**★다음 세션이 바로 쓸 수 있는 것**

- **모바일 검증은 CPU 스로틀링으로**: `Emulation.setCPUThrottlingRate` 4x·6x.
  데스크톱만 보고 "long task 0"이라고 결론 냈다가 이번에 틀렸다.
- **카니발은 GSC로 실측**: 쿼리→페이지 방향으로 API를 직접 치면 "누가 그 검색어를 갖고 있나"가 나온다.
  `/holdem-practice` 본문 주제를 그렇게 정했다(연습 = 빈 영역).
- **서버 정리는 커맨드라인 매칭으로**: `pkill -f "next start"`는 **안 먹는다**
  (실제는 `.../next/dist/bin/next s`). 이번에 34개가 쌓였고 그중 `next dev` 하나가
  `.next`를 덮어써 빌드가 `PageNotFoundError`로 깨졌다.
- 좌석·요소가 도형 밖으로 나가는지는 **순수 타원식으로 재면 오탐**이 난다(실제는 46%/40% 라운드렉트).
  최종 판정은 스크린샷으로.
...
| 항목 | 메모 |
|---|---|
| `/win-rate-quiz` 본문 0줄 | 색인 요청(`priority 0.8`) 중인데 h1+부제뿐. 승률표·해설 본문 + FAQ 스키마가 후보 |
| 리버 승부 해설 | 복기 표에 승률만 있다. "키커로 갈렸다"를 자동 생성할 수 있다(`score5`가 키커를 이미 갖고 있다) |
| 프로덕션 E2E | 검수 세션에 `BASE=https://www.holdemmaster.com`으로 요청하면 마감된다 |
| 모바일 탭 41px | 세로를 41px 쓴다. 항목 순서·높이는 조정 가능 |

---

### ▶ 0-◔. 2026-08-05 (5) — 실전 모드 검수: 결함 3건 ✅

> **작업 트리 깨끗 · typecheck 0 · build 615 · canonical 0건 · selftest 64/64.**
> 마지막 커밋 `fa25e6a`. 상세는 `WORKLOG.md` 「2026-08-05 (5)」.

사장님 *"검수 안 해도 됨???"* 지적으로 돌린 검수. **결함 3건이 나왔다.**

**🔴 도구 페이지에는 적용 가능한 기계 게이트가 없다 — 이걸 먼저 알 것**
```
npm run audit:hard -- --slug=win-rate-quiz  →  🔴 [F0] 슬러그가 존재하지 않음
```
`audit:hard`는 **블로그 포스트 전용**이다. `/calculator`·`/quiz`·`/hand-chart`·
`/win-rate-quiz` 같은 도구 페이지는 대상이 아니다. 여기서 "0건"은 **"안 봤다"**다.
→ 도구 페이지는 **성능 실측 + 손 검수**가 유일한 방어선이다.

**나온 결함**
1. 🔴 **D유형** — 상대가 레이즈해도 승률을 "폴드만 안 한 레인지"로 계산 → 최대 **+64.7%p**
   과대평가, 팟오즈 판정이 **정반대로 뒤집힘**. 상대별 액션 이력으로 레인지를 만들어 고침
2. 🟠 **성능** — 새 핸드마다 **1,617ms** 메인스레드 프리즈 → **0ms**.
   진짜 원인은 `makeTableSim`에 남아 있던 **아무도 안 쓰는 MC 10만 샘플**이었다
3. 🔴 **무한로딩** — `useEffect`가 `setSim`으로 자기 의존성을 바꿔 계산을 스스로 취소

**🔴 되돌리면 안 되는 것 (이번 세션분)**
1. **`eligibleHoles`에서 액션 일치 조건(`!== step.action`)을 "폴드만 아니면"으로 되돌리지 말 것.**
   그 순간 승률이 최대 65%p 부풀려지고 폴드 자리를 "콜"로 가르친다.
2. **`makeTableSim`에 계산을 다시 넣지 말 것.** 딜만 한다. 승률은 `_engine.ts`,
   쇼다운 판정은 `_simulator.tsx`가 §13 평가기로 직접 한다.
3. **`_simulator.tsx`의 effect 의존성에 `sim`을 넣지 말 것.** 새 판은 `handId`로 돌린다.
4. **MC 루프의 CHUNK 양보를 빼지 말 것.** 빼면 1.6초 프리즈가 돌아온다.

**★검수 방법도 남긴다 (도구 페이지용)**
- 메인스레드: `PerformanceObserver({entryTypes:['longtask']})`로 실측. 50ms 넘으면 결함
- 성능 원인은 **추측 말고 프로파일** — 이번에 유력 후보 둘(`eligibleHoles`·`heroEquity`) 다 아니었다
- 승률 도구는 **알려진 값과 대조**(AA vs 랜덤 85.2% · 72o 34.6% · AA vs KK 82.64%)
- 팟오즈는 **사이트 글의 공식과 대조**(하프팟 25% · 풀팟 33.3%)

---

### ▶ 0-◓. 2026-08-05 (4) — 승률 시뮬레이터 「실전 모드」 ✅

> **작업 트리 깨끗 · build 615 · `canonical:check` 0건 · 링크 8개 200 · 1440/390 실측.**
> 마지막 커밋 `1e153fd`. 상세는 `WORKLOG.md` 「2026-08-05 (4)」.

사장님과 설계를 주고받으며 만든 것. **도구의 성격이 바뀌었다** —
「승률 갈리는 걸 구경」 → **「내 패로 어떻게 판단했어야 하나 배우기」**.
상대 패를 가리고, 스트리트마다 내 승률 + 팟오즈 필요 승률을 보여주고, 끝나면 복기한다.

**🔴 되돌리면 안 되는 것 — 전부 이유가 있다**
1. **상대 패를 기본으로 공개하지 말 것.** 실전과 같은 정보 상태여야 승률에 의미가 생긴다.
   보고 싶으면 「상대 패 보기」 버튼이 있다.
2. **나에게 폴드 버튼을 주지 말 것.** 폴드하면 거기서 핸드가 끝나 **복기를 못 한다.**
   사후 평가("여기서 끊었어야")로 가르치는 게 이 도구의 전부다.
3. **폴드 규칙과 승률 계산을 분리하지 말 것.** 상대를 폴드시키는 그 규칙이 **곧 상대 레인지**다.
   그래서 화면에서 폴드한 사람이 계산에서도 빠진다 — 보이는 것과 근거가 어긋날 수 없다.
   따로 레인지 표를 만들면 이 일치가 깨진다.
4. **`STACK`을 없애지 말 것.** 없으면 팟이 60 → 240 → 960 → 3,840으로 폭발한다.
   모두 같은 액션을 해서 스택이 항상 같으므로 **사이드팟이 안 생긴다**(절단선 유지).
5. **규칙 설명(`ruleText`)을 지우지 말 것.** "실제 플레이어는 이렇게 안 한다"는 문장이
   빠지면 잘못된 전략을 가르치는 도구가 된다(§13이 못 잡는 D유형).
6. **`_equity.ts`의 평가 로직은 여전히 무변경.** `EVAL` export만 추가했다.

**★KO 슬러그가 CLAUDE.md §8 필라 목록과 다르다 — 이번에 두 번 밟았다**
...
둘 다 그대로 뒀으면 404였다. **링크 적을 때 §8 말고 실제 파일을 grep할 것.**

**⚠ 합의된 절단선 — 넘지 말 것**
사장님과 *"레이즈 응수부터가 복잡해진다"*고 합의하고 그 앞에서 잘랐다.
**레이즈에 내가 응수하기 시작하면 포커 엔진이 된다**(액션 순서·베팅 라운드 종료·사이드팟).
지금은 상대가 금액만 정하고 나는 항상 콜한다. 이 선을 넘으려면 지시를 받고 할 것.

**⚠ 남은 것**
- 이 페이지는 **여전히 본문 텍스트가 없다.** 사이트맵 `priority 0.8`로 색인 요청 중인데
  h1 + 부제뿐이다. 승률표·해설 본문 + FAQ 스키마가 다음 후보
- 리버 승부 해설(키커로 왜 갈렸는지)은 아직 없다 — 복기 표에는 승률만 나온다

---

### ▶ 0-◒. 2026-08-05 (3) — 승률 시뮬레이터 6인 테이블 ✅

> **작업 트리 깨끗 · build 615 · `canonical:check` 0건 · 링크 6개 200 · 1440/390 실측.**
> 마지막 커밋 `5e82a54`. 상세는 `WORKLOG.md` 「2026-08-05 (3)」.

사장님 제안(*"보통 6인 게임 많이 하니까"*)으로 `/win-rate-quiz`·`/en/win-rate-quiz`를
**6좌석 + 포지션 + 폴드 머크**로 바꿨다.

**★설계의 핵심 — 되돌리기 전에 반드시 읽을 것**
「6인 테이블」과 「6명이 쇼다운까지 간다」는 **다르다.** 실전에서 6명 전원이 리버까지
가는 핸드는 거의 없다. 그래서 좌석만 6개고 **끝까지 가는 인원은 2~4명 그대로**다.

**🔴 되돌리면 안 되는 것**
1. **폴드 좌석의 카드를 보여주지 말 것.** 보여주는 순간 "누가 폴드하는가"가
   프리플랍 레인지 판단이 되고 *"UTG가 AA를 폴드했네?"* 가 나온다. 안 보여줘야
   전략적 주장을 안 하게 되고 **D유형 사고가 원천 차단**된다.
   `_table.ts`는 폴드 좌석의 카드를 **뽑지도 않는다.**
2. **`_equity.ts`를 건드리지 말 것.** §13 검증 통과분이고 이번에 한 줄도 안 고쳤다.
   좌석 배치는 `_table.ts`, UI는 `_simulator.tsx`다.
3. **`_simulator.tsx`를 ko·en으로 복사하지 말 것.** 원래 두 클라이언트가 UI를 통째로
   복사해 갖고 있어서 이번에 합쳤다. 언어별로 다른 건 **`QuizUI` 문자열뿐**이다.
4. **`/blog/holdem-position-play`를 KO에 쓰지 말 것 — 없다.**
   EN·JA·ES 등에만 있고 **KO 슬러그는 `position-is-everything-in-holdem`**이다.

**⚠ 남긴 것 (사장님과 합의된 다음 단계)**
이 페이지는 아직 **본문 텍스트가 h1 + 부제 한 줄뿐**인데 사이트맵엔 `priority 0.8`로
색인 요청 중이다(`generate-sitemap.mjs:34`). 순위를 먹을 텍스트가 없다.
- 다음 후보 ①: 시뮬레이터 아래 **승률표·해설 본문 + FAQ 스키마**
- 다음 후보 ②: 클래식 매치업 프리셋(AA vs KK 등) · 내 카드 직접 지정 ·
  리버에서 **"왜 이겼는지"(키커) 설명** — 지금은 셋 다 "원페어"인데 왜 한 명이 이기는지
  페이지가 설명하지 않는다
- 별건: 승률 바의 **`정확 계산` 라벨이 프리플랍에선 엄밀히 틀리다**(MC 10만, ±0.15%).
  플랍부터는 완전 열거라 정확하다. 이번 범위 밖이라 손대지 않았다

---

### ▶ 0-◑. 2026-08-05 (2) — APT 인천 D-3 정정 미러 8종 반영 ✅

> **작업 트리 깨끗 · build 615 · 9개 파일 마감경로 권유 0건 · 라이브 확인.**
> 마지막 커밋 `6ab3bf0`. 상세는 `WORKLOG.md` 「2026-08-05 (2)」.

**⛔ `apt-incheon-2026-guide`는 이제 KO + 미러 8종 전부 정정 완료다. 다시 손대지 말 것.**

검수 폴더의 `73bcf04`(8/4)이 KO만 고치고 미러를 안 건드려서, 개막 D-2에
8개 언어권이 **8월 2일에 닫힌 위성으로 유도되던** 상태였다. 반영한 사실:
마감 경로($20 스텝 → $200 메가만 8/7까지) · Superstar 8/14~15 · 상금풀 ₩2,766,960,000 ·
국적 기준(국적이 아니라 **해외 영주권 보유 여부**) · OnLive 종료.

**★교훈 3가지 — 다음에도 그대로 밟는다**
1. **KO만 고치면 미러 8종이 남는다.** 대회·이벤트 글은 `lib/posts/`와
   `lib/posts-*/` 9곳이 한 세트다. 검수 폴더가 KO를 고쳤다면 **미러 동기화는 별도 작업**이다.
2. **`$3,50`(쉼표 소수점)은 `3.50` grep에 안 잡힌다.** de·id가 처음에 "해당 없음"으로
   보였지만 실제로는 전부 해당됐다. 숫자 표기는 로케일마다 다르다.
3. **날짜만 보고 일괄 치환하면 오히려 깨진다.** `#37 High Roller – Ultra Stack`이
   진짜 8/10~11이라, Superstar 8/10~11을 sed로 밀었으면 멀쩡한 사실을 틀리게 만들었다.

**★KO 마스터에도 다섯 번째 자리가 있었다** — `73bcf04`이 "네 곳"을 고쳤다고 적었는데
FAQ가 여전히 `$3.50`을 권했다. **커밋 메시지가 센 개수를 믿지 말고 직접 grep할 것.**

---

### ▶ 0-◐. 2026-08-05 — `/tournaments` 날짜 하드코딩 제거 ✅

> **작업 트리 깨끗 · build 615 · `canonical:check` 0건 · 미래 7개 시점 시뮬레이션 통과.**
> 마지막 커밋 `6e442c3`. 상세는 `WORKLOG.md` 「2026-08-05」.

사장님 질문(*"대회 포스팅은 날짜 지날 때마다 업데이트 해줘야 해?"*)에서 나온 작업.

**답부터**: `/tournaments` 보드는 원래 자동이다(`computeStatus`+매일 00:20 KST 크론 리빌드).
사람이 봐야 하는 건 **블로그 본문**과 **`lib/tournaments.ts` 데이터**뿐이다.
문제는 **자동이어야 할 자리가 손으로 박혀 있던 것**이었고, 그걸 고쳤다.

**★가장 중요 — 또 밟기 쉽다**
`components/seo.tsx`는 props를 useEffect에서 **무조건** `document.title`에 덮어쓴다.
그래서 `layout.tsx`의 `generateMetadata`가 아무리 옳아도 **클라이언트가 <SEO>에 다른 문자열을
넘기면 브라우저에서는 그게 이긴다.** `/tournaments`가 정확히 그 상태였다
(정적 HTML은 "APT 인천 8/7 개막", 브라우저는 "WSOP 파이널 8/3~5" = 지난 일정).
**`curl`로는 원리상 못 잡는다 — 브라우저로 봐야 한다.**

**🔴 되돌리면 안 되는 것**
1. **`<SEO>`에 title·description 문자열을 직접 적지 말 것.** `lib/tournaments-digest.ts`의
   함수를 쓴다. `layout.tsx`와 **같은 함수**여야 메타와 화면이 안 갈린다.
2. **`tournaments-digest.ts`에서 `new Date()`를 부르지 말 것.** `todayISO`는 서버 prop이다.
3. **`ongoing`을 재정렬하지 말 것.** `TOURNAMENTS`가 이미 시작일 오름차순이고,
   `highlight` 우선 정렬은 기존 메타 문구를 바꾼다(만들어 봤다가 되돌렸다).
4. 문장에 **날짜·개수를 손으로 적지 말 것.** 이번에 뺀 폭탄이 셋이었다
   (기준일 · "8/3~5 남았다" · **"99개 수여"** — 브레이슬릿 마지막 1개가 8/5 결정이라 8/6이면 거짓).

**★검증 방법을 남긴다 — 오늘 날짜만 보면 결함이 안 보인다**
미래 날짜를 넣어 돌려 보면 3건이 더 나왔다(8/17 괄호 충돌 · 12/31·2027-03 문장 덜렁거림).
날짜에 반응하는 문자열을 만들면 **반드시 미래 시점 여러 개로 시뮬레이션할 것.**

**⚠ 남긴 것(검수 영역)**: `lib/tournaments.ts`의 WSOP 카드 note —
*"브레이슬릿 100개는 7/15 종료 · 메인이벤트 파이널 테이블만 8/3~5 (ESPN)"*.
대회 데이터라 손대지 않았다. 8/5가 지나면 시제가 어색해진다.

---

### ▶ 0-◈. 2026-08-04 (6) — 허브 셸 ja·es 확장 ✅ **사장님이 정한 범위 완료**
...
| 순위 | 할 일 | 메모 |
|---:|---|---|
| ~~1~~ ✅ | ~~`apt-incheon` 미러 8종~~ | **2026-08-05 완료** (`6ab3bf0`) — 위 `0-◑` |
| ~~2~~ ✅ | ~~`tournaments-client.tsx:601` 하드코딩 날짜~~ | **2026-08-05 완료** (`6e442c3`) — 위 `0-◐` |
| **1** | **`/win-rate-quiz` 본문 0줄** | 도구는 다 됐다(6인→실전모드→검수→모바일). **남은 건 본문·FAQ 스키마뿐.** `priority 0.8`로 색인 요청 중인데 h1+부제만 있다 |
| **2** | 프로덕션 E2E 마감 | 검수 세션에 `BASE=https://www.holdemmaster.com`으로 요청. 배포는 `31689bd`까지 끝났다 |
| ~~3~~ ✅ | ~~`/holdem-practice` h1 0개~~ | **2026-08-05 완료** (`f6f2803`) — 실제로는 h1이 아니라 **콘텐츠 전체**가 없었다 |
| ~~4~~ ⛔ | ~~EN 도구 페이지 `<title>`~~ | **오해였다.** 6개 전부 이미 `title:{absolute}`를 쓴다. EN이 한국어 template를 물려받으면 안 되니 브랜드를 직접 적는 게 맞다 |
| 5 | 다국어 정책 페이지 | `/privacy`·`/terms`·`/contact`가 한국어뿐. 링크는 영어 라벨로 걸려 있다 |
| 6 | `es` 국기 판단 (아래 ⚠) | 사장님 결정 필요 |
| 7 | 리버 승부 해설 | 복기 표에 승률만 있다. "키커로 갈렸다"를 `score5`의 키커로 자동 생성 가능 |

> 🔴 **CLAUDE.md §8의 KO 필라 슬러그 목록이 실제 파일과 다르다.** 링크 적을 땐 반드시 grep할 것.
> `holdem-pot-odds`(X) → **`holdem-pot-odds-calculation`** · `holdem-position-play`(X, EN·JA·ES 전용) → **`position-is-everything-in-holdem`**
> 2026-08-05에 이걸로 두 번 404 직전까지 갔다.

### ⏰ 1순위 상세 — `apt-incheon` KO만 고쳐졌다 (2026-08-05 확인)

> ⚠ **한국어 글은 이미 끝났다.** 검수 폴더가 `73bcf04`(8/4 18:46)로 D-3 긴급 정정을 마쳤고
> 라이브 반영·`dateModified: 2026-08-04`까지 확인했다. **다시 검수하지 말 것.**
> (이 파일의 옛 0-◆에 있던 "8/7 개막이니 최우선" 메모를 그대로 옮겨 적었다가
>  slug로 grep해보고 오판이었음을 확인했다 — WORKLOG는 상단만 보지 말고 slug로 grep할 것.)

**진짜 구멍은 미러다.** `73bcf04`가 건드린 파일은 `lib/posts/apt-incheon-2026-guide.ts`와
`public/sitemap.xml` **둘뿐**이고, 8개 언어 미러는 정정 전 상태 그대로다.

| 항목 | KO (정정됨) | 미러 8종 (en·ja·zh·zh-hant·es·de·pt·id) |
|---|---|---|
| $3.50 스텝 위성 | ~~취소선~~ + "8월 2일 종료" note | **여전히 권함** (글당 5~8회) |
| Superstar Challenge | **8/14~15** (APT 공식 #83 Day 1 대조) | **8/10~11** (옛 오류) |
| `updated` | 2026-08-04 | 07-28 (ja만 07-30) |

★de·id는 `$3,50`(쉼표 소수점) 표기라 `3.50`으로 grep하면 **안 잡힌다.** 8종 전부 해당된다.

**개막 8/7.** D-3 정정문이 스스로 "🔴 최우선"이라 적은 항목(닫힌 경로로 유도)이
바로 그 미러들에 남아 있다. 좌석 수(45/12 vs 52/22)도 KO와 갈리는 것으로 보이나
거친 grep이라 단정하지 않았다 — **검수 폴더에서 fact-instance 전수 diff로 확인할 것.**
...
좌측 레일의 가이드 버튼들이 **도착지마다 레이아웃이 제각각**이던 문제를 하나의 셸로 통일했다.
레일이 아예 없던 페이지가 8개(전략·규칙·용어사전·퀴즈·핸드차트·홀덤펍·승률·연습장)였다.

- `components/hub-shell.tsx` — 홈과 같은 3열(마스트헤드 + 레일200 + 본문 + 사이드바240).
  적용 13개 라우트. `page.tsx`는 `<HubPage title="…">`로 감싸기만 하면 된다.
- `components/hub-sidebar.tsx`(서버) · `lib/hub-trending.ts` · `lib/theme.ts` · `lib/hub-routes.ts` 신설
- 「전체 글 보기」 레일에서 제거(사장님 지시). `/blog` 라우트는 유지 — 검색이 거기 산다.
  그 페이지의 회색 풀블리드 히어로는 걷어냈다.
- 좌측 레일 「글 쓰기」 → `/?write=1` (홈이 읽어 모달을 연다)

**🔴 되돌리면 안 되는 것 4가지**
1. **서버 컴포넌트는 `@/lib/theme`에서 색 토큰을 가져온다.** `app/community/post-card`는
   "use client"라 서버에서 읽으면 프리렌더가 깨진다(실제로 밟았다).
2. **HubShell은 children을 한 번만 렌더한다.** 모바일/데스크톱 블록으로 쪼개면 자식이
   두 번 마운트돼 `id="search"`·heading id가 문서에 2개씩 생긴다.
3. **우측 사이드바는 `xl:` 이상.** lg에서 3열이면 가운데가 504px로 뭉갠다.
4. **새 허브 페이지는 `lib/hub-routes.ts` 등록 + `<HubPage>` 감싸기를 둘 다** 해야 한다.
   하나만 하면 상단바가 2겹이 되거나 셸이 안 붙는다.

**남은 것**: `/holdem-practice` h1 0개(기존 문제) · 다국어 툴 페이지는 이 셸 미적용(한국어만)

---

### ▶ 0-◆. 2026-08-04 (2) 세션 — 「사이트 기본기 6종」 ✅ **6/6 완료**

> **작업 트리 깨끗 · build 614 · sitemap 56 blog posts + 53 static · 배포 완료.**
> 상세는 `WORKLOG.md` 「2026-08-04 (2)」 항목. 1440/360 Playwright로 전부 실측 확인했다.

| # | 결과 |
|---:|---|
| 1 | 푸터 부활(`components/footer.tsx` 재작성) + `/privacy`·`/terms`·`/contact` 신설. 홈은 무한 스크롤이라 푸터 대신 **좌측 레일 바닥 + 모바일 피드 끝** |
| 2 | `app/not-found.tsx` — 검색창(JS 불필요) + 인기 페이지 6 + 제보 링크 |
| 3 | 문단 폭 928 → **704px**, 한 줄 52 → **41.9자**. 표(862px)·이미지·H2는 그대로 |
| 4 | 태그 — 링크화했다가 **블록 통째로 제거**(사장님 판단: 목적지가 목록 페이지라 뒤로 되돌리는 동선). 360px에서 본문 시작이 1655 → **1495px**. `post.tags`·`?tag=` 필터는 유지 |
| 5 | `/blog?q=` 검색 — 진입점 5곳. 공백 무시 정규화(「팟오즈」→「팟 오즈」 글도 잡힘) |
| 6 | 시각 브레드크럼 `홈 → 블로그 → 글제목` (JSON-LD와 **같은 3단**) |

**덤**: `/blog` 목록이 클라에서 `import { POSTS }` 하던 것을 서버 메타 전달로 교체
— 56편치 본문 마크다운이 번들에 실리던 누수 제거(청크·HTML 검색 0건).

**🔴 되돌리면 안 되는 것 3가지**
1. `blog-index-client`는 **`useSearchParams()`를 쓰지 않는다.** 쓰는 순간 `blog.html`의
   글 링크가 56 → 1개가 된다(이번에 실제로 밟고 되돌렸다). `window.location` + `popstate`가 정답.
2. 필터 해제는 `<Link href="/blog">`가 아니라 **button + 상태 초기화**. 같은 라우트는 재마운트가 없다.
3. `.legal-prose` **안**에 Tailwind로 스타일한 카드를 넣지 말 것 — `.legal-prose a`가
   특이도로 이겨 링크가 골드 밑줄이 된다. `LegalPage`의 `intro` 슬롯(prose 밖)을 쓸 것.

**⚠ 시한**: `apt-incheon-2026-guide`가 **8/7 개막**이다. 이제 이게 최우선.

**점검했더니 문제 없던 것 (다시 파지 말 것)**
- 대비비 **15.12** (기준 4.5), 보조 텍스트 5.53 — 통과
- `h1` 1개 · 이미지 alt 누락 0 (`alt=""` 2건은 장식용 썸네일이라 정답)
- 탭 타겟: 처음 "71개 작음"으로 나왔으나 **오탐**. 닫힌 `<details>`가
  content-visibility로 레이아웃만 잡힌 것 + 판정식(`hit.contains(e)`)이 조상까지 참으로
  받은 탓. **열면 세 패널 전부 344px 정상.**
  ★교훈: 가시성 판정은 `getClientRects().length` + `elementFromPoint`의 **정확한 동일성**으로.
- 읽기 진행바·목차·공유·이전/다음·관련 글·작성자 박스·업데이트 날짜 — 전부 있음
- 학습맵(클러스터 미니맵)은 경쟁 사이트에 드문 **강점**이므로 건드리지 말 것

**남은 부스러기 (급하지 않음)**
- CLAUDE.md §8 필라 목록의 `holdem-pot-odds`는 **실재하지 않는 slug**다
  (실제는 `holdem-pot-odds-calculation`). 문서만 틀린 것 — 라우트는 정상.
- `/blog?tag=` 필터는 코드에 남아 있지만 **이제 사이트 안에 링크가 없다**(태그 블록 제거).
  태그 탐색을 되살리려면 목록으로 되돌리는 방식 말고 다른 목적지를 먼저 정할 것 —
  그게 이번에 뺀 이유다.
...
> 이 폴더는 **사이트 구조·UI·인프라** 작업을 맡는다.
> 그래서 아래 목록에서 검수성 항목(대회 가이드 소급 등)은 순위에서 뺐다.
> 검수 결과가 원격에 올라오므로, 이 폴더에서 작업 시작 전 **`git pull` 먼저** 할 것.

| 순위 | 할 일 | 왜 |
|---:|---|---|
| ~~1~~ | ~~다국어 브레드크럼 소급~~ | ✅ **완료** (457/457 · 대회 가이드 마크업 불일치도 같이 정정) |
| **1** | 다국어 툴 페이지에 허브 셸 소급 | 지금은 한국어 13개 라우트만 적용됨 |
| **2** | `/holdem-practice` h1 0개 | 기존 결함. 셸 통일 검증 중 발견 |
| 3 | `tournaments-client.tsx:601` 하드코딩 | "2026년 7월 28일 기준 … 8월 3~5일"이 낡았다 |
| 4 | 데스크톱 네비 잔여 (handoff 0-★ B항목) | `blog-top-bar.tsx` 좌우 버튼이 아직 같은 href |

> **브레드크럼 규칙 (앞으로 지킬 것)**: 화면 단계 수 == `BreadcrumbList` JSON-LD 단계 수.
> 어긋나면 구글이 리치결과를 아예 안 준다. 실제로 대회 가이드 5편이 화면 4단 ↔ 마크업 3단으로
> 어긋나 있었다. 새 레이아웃을 만들 때 둘을 **같이** 손댈 것.
> 다국어 첫 단계 라벨은 `CHROME[locale].brand` — 25개 언어에 「홈」을 새로 번역해 넣지 말 것.

**검수 폴더 쪽 진행 상황 (참고용 — 여기서 다시 하지 말 것)**

| 글 | 상태 |
|---|---|
| `apt-incheon-2026-guide` | ✅ **완료·배포** (`73bcf04`, 2026-08-04 D-3 긴급 정정) — 마감된 $35 메가 위성·OnLive 예선 취소선 처리, Superstar Challenge 8/10~11 → **8/14~15** 정정, KYC 타임라인·국적 조건 복원 |
| `holdem-masters-7th-guide` | 미확인 — 8/3 제8회 개막이 지났다. 결과 아카이브 전환 필요 |
| 대회·이벤트 가이드 나머지 8편 | 미확인 — [[event-guide-consistency-review]] fact-instance 전수 diff |
| `docs/update-calendar.md` | 8/5 WSOP 결과 6개 언어 |

---

### ▶ 0-★. 2026-08-04 세션 — 모바일 네비 재설계 · 계산기 압축 · 카테고리 통일 (여기서 멈춤)

> **작업 트리 깨끗 · build 611 · sitemap 56 blog posts · 배포 완료.**
> 마지막 커밋 `76b1315`. 상세는 `WORKLOG.md` 2026-08-04 항목.

**이번 세션은 콘텐츠가 아니라 UI/구조 세션이었다.** 하단 탭바를 컴포넌트로 추출해
전역화하고, 블로그 글·계산기에 62px 상단 섹션 네비를 신설했다. 계산기 모바일은
한 화면에 들어오도록 압축했고, 카테고리 taxonomy(`확률·수학` → `확률`)를 통일했다.
...
| 순위 | 할 일 | 왜 급한가 |
|---:|---|---|
| **1** | `apt-incheon-2026-guide` | **8/7 개막** — 사흘 남았다 |
| **2** | `holdem-masters-7th-guide` | 8/3 제8회 개막 — 이미 지났다. 결과 아카이브로 전환 |
| **3** | 대회·이벤트 가이드 나머지 8편 | [[event-guide-consistency-review]] fact-instance 전수 diff |
| 4 | `docs/update-calendar.md` 확인 | 8/5 WSOP 결과 6개 언어 |

**B. 네비 작업 잔여 (이번 세션에서 명시적으로 미룬 것)**

1. **데스크톱 네비** — 사장님 지시로 손대지 않았다("모바일만 하고 있으니 데스크탑은 나중에")
2. **다국어 블로그 글** — `components/intl-blog-post-client.tsx`는 아직 **옛 상단바**
   (계산기·목차·학습맵). 새 4버튼 섹션 네비로 안 바뀌었다
3. `/blog` 목록·`/tournaments`도 섹션 네비 패턴 미적용
4. `blog-top-bar.tsx` 좌우 버튼이 아직 같은 `homeHref` (네비 결정 대기로 보류)
5. `tournaments-client.tsx:601` — "2026년 7월 28일 기준 … 8월 3~5일" 하드코딩 줄이 낡음

**C. 장기 (반복 사고 차단)**

- `lib/facts` SSOT + 게이트 확장: 지금 게이트는 **표만** 교차 대조한다. 8/4에 드러난
  오류는 전부 **산문·행동 순서·글 간 주장**에 있었다
- 교차 모델 검수를 상시화 — GPT-5.6이 잡은 14건을 우리 게이트는 🔴 0으로 통과시켰다

## ⚠️ 이번 세션 함정 (반복 방지)

| 함정 | 교훈 |
|---|---|
| `offsetParent`로 가시성 판정 | `position:fixed`는 **항상 null** → `getComputedStyle(display)` |
| `sticky`를 `translate-y-full`로 숨기려 함 | sticky는 문서 흐름을 차지한다 → `fixed` |
| `opacity-0`이 클릭을 막을 거라 가정 | 안 막는다 → `pointer-events-none` 필수 |
| 일부만 확인하고 "전부 그대로"라고 보고 | 모바일/데스크톱을 **분리 실측·분리 보고** |
| 커밋에 "검산했다"고 적고 안 함 | ★거짓 "검증됨"은 그 항목을 **영구히 재검사에서 제외**시킨다 |
| CRLF 파일에 `
` 기준 치환 | 파일의 개행을 먼저 감지하거나 Edit 도구를 쓸 것 |
...
**★7/28에 손댄 글인데 조문 원문을 다시 보니 두 곳이 달랐다**
7/28 개정은 국세청 안내 문구가 근거였다. 이번엔 **법제처 조문 원문을 직접 추출**해 대조했다
(법령 본문이 `iframe#lawService` 안이라 `contentDocument.body.innerText`로 긁어야 한다).
① 시행령 제87조 제1호 **단서**를 새로 찾았다 — "실제 소요된 필요경비가 80%를 초과하면 초과분도
산입" = **80%는 상한이 아니라 최소 보장선**. ② 300만 원 분리과세 서술의 **방향이 뒤집혀 있었다**
(제14조 제3항 제8호 가목: 합산하지 아니함이 원칙, 합산이 납세자 선택).

**★적대 렌즈 2종이 독립적으로 같은 것을 지목** — 본문은 고쳤는데 **FAQ만 옛 서술로 남아** 정면 모순.
FAQ는 리치결과로 노출되는 자리다. 그리고 **2차 교열이 또 3건**을 잡았다(전부 내 1차 수정이 만든 것).
세 세션 연속 같은 패턴 — 1차 반영 후 2차 교열은 선택이 아니다.

**⚠ 검수자 오탐 1건** — 세무 렌즈가 "IRS 원문에 그 문장 없음"이라 했으나 Playwright 재확인 결과
내 인용문도 실재했다. 근거는 기각하고 조치(더 명확한 문장으로 교체)만 채택.

**★이미지 렌더 파이프라인 개선 + 정식 스크립트 승격**
사장님 지적("글자가 흐릿하다") 원인 = 2배 렌더 → 축소 → webp q72.
→ **3배 렌더 · lanczos3 + 언샵마스크 · q88 · 대비/굵기 상향.**
`scripts/render-infographic.mjs`로 승격(공통 스타일 `docs/infographic-style.css`).
★**출처는 이미지에 넣지 않는다**(사장님 지시) — 본문 조문 자리와 하단 출처 섹션에만.

### ▶ 0-⊚-1. 🔴 남은 것 — 새 세션은 여기서 시작

1. **토너먼트 클러스터 잔여 2편** — `tournament-buy-in-cost`(🔴2: 질문형 H2 56% + 구형 `### Q` 6문항) ·
   `tournament-schedule-check`(🔴2: 질문형 H2 30% + 구형 `### Q` 6문항). **처방 확정 — 1순위.**
   버블·세금 글에서 쓴 순서를 그대로 재사용: 자동완성 실측 → 미수신 수요를 절로 신설 →
   1차 출처 직접 추출 → 적대 렌즈 → **2차 교열** → 이미지(그 글 것만).
2. **★소급 대상 — 세금 서술이 글마다 어긋난다**(SEO 렌즈 발견, 이번엔 손대지 않음)
   - `apt-incheon-2026-guide`: "필요경비 공제 후 약 22%" — 틀린 말은 아니나 세금 글의
     "지급액 기준 4.4%"와 나란히 놓이면 독자가 혼동한다
   - `holdem-tournament-how-to-enter`: "원칙적으로 국내 소득세 신고 대상" — 세금 글보다 단정 수위가 높다
   → 각 글 경화 차례에 세금 글 기준으로 맞출 것.
3. **8/3 제8회 홀덤 마스터스 개막** — `holdem-masters-7th-guide` 갱신. `docs/update-calendar.md`.
4. **색인 요청 목록에 2편 추가** — `holdem-bubble-strategy`(1순위-C) · `holdem-tournament-tax-guide`.
5. 나머지(대회 가이드 9편 · C1 형제 표 3건 · `holdem-raise-how-much`)는 아래 0-⊙-1·0-◎-1 그대로.

### ▶ 0-⊙. 2026-08-02 (10) 세션 — `holdem-bubble-strategy` 경화

> **작업 트리 깨끗 · 게이트 43/57편(🔴 24 → 22 · 🟠 31 → 28) · build 611 · 커밋 `b9cd5a5`.**
> 상세는 `WORKLOG.md` 2026-08-02 (10).

**⚠ 명령어 두 개가 인수인계에 틀리게 적혀 있었다 — 고쳤다**
- 게이트 총계는 **`npm run audit:hard -- --all`**. 플래그 없이 돌리면 **41편 목록만** 봐서 "37/41"이 나온다.
  지금까지의 "N/57" 수치는 전부 `--all` 기준이다.
- **`npm run canonical`은 없다. `npm run canonical:check`** 다.

**★자동완성을 보고 절을 신설한 게 이번 방식의 핵심**
GSC 28일 실적이 **쿼리 1개·노출 1·클릭 0** = 검색에 사실상 없는 글이었다. rakko로 구글 자동완성을
실측하니 본문이 하나도 안 받던 수요가 무더기였다(스톤버블·하드버블·버블 프로텍션·버블타임).
→ 3개 절 신설, 전부 **1차 출처 직접 추출**(WSOP 룰 PDF pdftotext / GGPoker·용어집 Playwright DOM).
**핸드 포 핸드 절**(WSOP 룰 126조)이 한국어 경쟁 글에 없는 최대 차별점이다.

**★§13은 계산으로만 잡혔다** — QQ vs AK "약 54%" → **56%**(전수 열거). 수티드 수치를 무늬 미지정
AK에 붙인 오류였다. 장면 3의 "15BB 코인플립"도 거짓(A2s 36.7%, 몬테카를로 33만 표본).

**★게이트 🔴 0인데 D유형이 6건 나왔다.** 「20BB 이하 올인 레인지」 표가 바로 위 표와 정면 충돌
(→ 10~15BB로 재라벨), "콜해도 안 죽는 올인은 콜"의 필요조건/충분조건 혼동, KQs 오버폴드 등.
**GGPoker 표 4행 누락은 세 렌즈가 독립 지목**했다 — 내가 원문 표를 축약하다 잘랐다.

**★2차 교열이 연속 두 세션째 값을 했다** — 1차 반영이 만든 모순 4건(뒤집은 판정이 앞 원칙과 충돌,
마무리가 격하된 옛 규칙 재진술, 좁힌 표를 위쪽 행이 여전히 가리킴, 신설 문단끼리 축어 중복).

**🔴 이미지 5장 중 4장이 못 쓸 상태였다** — AI가 깨뜨린 인포그래픽 2장(프롬프트 잔재 `(Rule/hand
errors fixed)`가 그대로 렌더, **그중 하나가 og:image**) + 블랙잭 테이블 사진 + 맥락 무관 라이터 사진.
전부 제거하고 **HTML+Playwright**로 2장 신규 제작.
> ★**게이트도 적대 렌즈도 이미지는 안 본다.** 경화 때 이미지는 반드시 Read로 직접 열 것.
> **다른 글에도 같은 시기 인포그래픽이 살아 있을 가능성이 높다 — 전수 점검 대상이다.**

### ▶ 0-⊙-1. 🔴 남은 것 — 새 세션은 여기서 시작

1. **토너먼트 클러스터 잔여 3편** — `tournament-buy-in-cost`(🔴2: 질문형 H2 56% + 구형 FAQ 6문항) ·
   `tournament-schedule-check`(🔴2: 질문형 H2 30% + 구형 FAQ 6문항) · `tournament-tax-guide`(🔴1: desc 115자).
   **처방이 이미 정해져 있다 — 1순위.** 버블 글에서 쓴 방식(자동완성 실측 → 미수신 수요를 절로 신설
   → 1차 출처 직접 추출 → 적대 4렌즈 → 2차 교열)을 그대로 재사용하면 된다.
2. **★이미지 결함은 이미 전부 조사돼 있었다 — `docs/image-audit/defects.md`**
   2026-07-11에 만들어진 이 파일에 **DEFECT 117건**이 교체용 프롬프트까지 붙어 정리돼 있다.
   그런데 **그중 100건이 KO 포스트에서 아직 살아 있다.** 버블 글 4장도 여기 다 등재돼 있었고
   3주 넘게 그대로였다(`holdem-bubble-pressure`만 7/12에 교체됨 — 그 항목은 이제 낡은 기록이다).

   > ★**사장님 지시(2026-08-02): 일괄 교체 프로젝트로 돌리지 말 것. 글을 하나씩 경화할 때
   > 그 글의 이미지만 같이 처리한다. 다국어는 범위 밖 — KO만 본다.**

   → **경화 착수 시 고정 절차에 추가**: 대상 slug의 이미지를 `defects.md`에서 grep해
   해당 항목이 있으면 Read로 현물 확인 → 필요 시 교체 → `defects.md`의 그 줄을 정리.
   글자 인포그래픽은 **HTML+Playwright**로 만든다(이번 버블 2장이 그 방식, 18/21.7KB).

   ✅ 이번에 미참조 결함 이미지 **16장 삭제**(재사용 사고 방지). 삭제 후 KO 포스트 이미지 참조 전부 실존 확인.
3. **8/3 제8회 홀덤 마스터스 개막** — `holdem-masters-7th-guide` 갱신. `docs/update-calendar.md` 참조.
4. **대회·이벤트 가이드 9편** — [[event-guide-consistency-review]] 방식(fact-instance 전수 diff)이 별도로 필요.
5. **C1 형제 표 수치 불일치 3건** · **`holdem-raise-how-much`** 1편 (아래 0-◎-1 그대로).
6. **색인 요청** — 0-★ 목록에 **`holdem-bubble-strategy` 추가**(FAQ 0→8문항 + 신설 3개 절 + og:image 교체.
   재크롤 전엔 전부 효과 0이다).

### ▶ 0-◎. 2026-08-02 (4) 세션 — 구조 결함 전수 + 프리미엄 핸드 3부작 경화
...
★적대 렌즈가 잡은 것 중 이번 편집 유래 — 축어 중복 8건, 사실 오류("QQ·JJ·TT를 붙잡아 앞선 상태로"
→ AK는 43%로 **뒤진다**), 문체 드리프트(합쇼체 글에 해요체)가 이번 편집에서 생겼다.
...
**★기준이 바뀌었다 — GSC·GA4는 28일로 본다**
> "90일로 보는 것도 좋지만 최근 게 더 유효하다. 계속 경화 작업을 하는데 너무 길게 보면
> 변화한 게 섞여 있어서 정확한 판단이 안 됨. 28일이 제일 좋을 듯."

경화 대상 선별 기준 = **참여율 70% 미만이거나 이벤트가 저조한 페이지**.
`npm run ga -- --pages --days 28` · `npm run gsc -- --days 28`. 메모리 `analytics-window-28days`에 저장.

**한 일**
1. `holdem-tournament-how-to-enter` 경화 — GA4 이탈 1위(80세션 63.7%)와 게이트 🔴 3건이 같은 글을 가리켰다.
   **★FAQ 6문항이 `### Qn.` 형식이라 스키마 0이었다** → 0 → 10문항. 8/1 홀덤펍과 같은 유형이다.
2. **`CLUSTERS`에 '토너먼트' 신설**(7편) — 아예 정의가 없어 대회 글 전체가 형제 대조를 못 받고 있었다.
   교차 대조 0쌍 → **4쌍·19행**.
...
- **`holdem-hand-rankings`(족보 필라): 체류 0m16s · 이벤트/세션 1.90** (사이트 평균 4.77).
  8/1에 그림까지 넣은 필라가 16초 만에 나간다. `/hands`(도구, noindex)가 같은 쿼리에서
...
3. ~~**미흡수 수요 2건**~~ — **2026-08-02 둘 다 처리·정정 완료.**
   - 「홀덤펍 가격」 ✅ **흡수함**. 7일 10.7위인데 `first-visit-guide` 본문에 "가격" 표기가 **0회**였다
     (참가비 27·바이인 13). H2를 「홀덤펍 가격·참가비는 얼마인가요?」로 바꾸고 **가격=참가비=바이인이
     같은 돈**임을 본문·stripe·바로 답·tldr에 명시. seoTitle은 「홀덤펍 혼자」 6.4위·CTR 6%로 살아 있어 §17대로 유지.
     ⚠ **형제 글(`pub-guide`·`pub-legal`)에는 "가격"을 넣지 않았다** — 이 쿼리의 랜딩은 first-visit이고,
     같은 표기를 형제에 뿌리면 내부 경쟁을 새로 만든다.
   - 「홀덤펍 대회」 ❌ **오진이었다.** "어느 글도 답하지 않는다"고 적혀 있었으나 실제로는
     **`/tournaments`가 17노출 9.4위·클릭 1**로 받고 있다. 흡수 대상 아님.
...
**🔴 1순위-C — 버블 전략 (8/2 경화, `b9cd5a5`)**
```
https://www.holdemmaster.com/blog/holdem-bubble-strategy
```
> GSC 28일 **쿼리 1개·노출 1·클릭 0**이던 글이다. FAQ **0 → 8문항**, 신설 3개 절(스톤버블·핸드 포 핸드·
> 버블 프로텍션), og:image가 **깨진 인포그래픽 → 실사**로 바뀌었다. 재크롤 전엔 전부 효과 0.

**🔴 1순위-D — 세금 가이드 (8/2 재검수, `0898cb0`)**
```
https://www.holdemmaster.com/blog/holdem-tournament-tax-guide
```
