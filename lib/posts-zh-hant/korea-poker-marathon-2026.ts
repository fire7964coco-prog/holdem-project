import type { Post } from "../posts";

export const POST: Post = {
  slug: "korea-poker-marathon-2026",
  title: "韓國撲克馬拉松 2026 全攻略——賽事、馬拉松與仁川賭場一趟玩透",
  seoTitle: "韓國撲克馬拉松 2026——賽事×馬拉松行程指南",
  desc: "JTBC 首爾（11/1）外國人報名 2026 年 9 月 4 日截止——春川與 Garmin 已滿。賽程表、報名費，加上仁川機場十分鐘就到的 Paradise City，一趟全串起來。",
  tldr: "韓國秋季頂級馬拉松（春川 10/25、JTBC 首爾 11/1、Garmin Run 11/15）正好落在最舒服的跑步天氣。仁川機場旁的 Paradise City 賭場全年開放現金桌——僅限外國人、離航廈十分鐘。撲克錦標賽（APT 仁川 8/7–16、APT 濟州 9/25–10/7）也在同一個秋季窗口——一趟行程兩邊都吃得到。2026 年外國人報名只有 JTBC，截止 9/4。",
  category: "tournament",
  date: "2026-07-23",
  updated: "2026-08-31",
  masterUpdated: "2026-08-31",
  keepImagesInBody: true,
  readTime: "14 分鐘",
  emoji: "🏃",
  image: "/images/korea-poker-marathon-2026-hero.webp",
  imageAlt: "跑者在春川衝過終點線，背景是秋天的楓紅與衣岩湖——韓國馬拉松 2026",
  tags: [
    "韓國撲克馬拉松",
    "Korea Poker Marathon 2026",
    "韓國德州撲克大賽",
    "首爾馬拉松 外國人報名",
    "春川馬拉松 2026",
    "APT 仁川 濟州 2026",
    "Paradise City 賭場 撲克",
    "韓國 撲克 大會",
  ],
  content: `
2026 年 5 月 17 日，BTS 的 Suga 早上六點起床，用 1 小時 04 分 43 秒跑完舊金山的 12K「Bay to Breakers」路跑——當晚就在座無虛席的史丹佛體育場開唱。RM 後來補上背景：==g:Suga 每天固定跑 10K。== 他聽說有這場賽事，直接就報名了。

這個故事打動的不只是 ARMY。它打中的，是一個早就存在的重疊圈子：一群玩牌的人，同時也是一群跑步的人。這兩個社群共享的，比任何人記錄下來的都多——那份紀律、對變異數的忍耐、對優化近乎偏執的追求。

而韓國，正是這兩件事完美交會的地方。==**世界級的秋季馬拉松、離機場十分鐘的外國人專屬賭場，中間還夾著全亞洲最強的都市美食。**==

<div style="display:flex;align-items:flex-start;gap:16px;margin:20px 0">
  <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-marathon-record.webp" alt="2026 高陽半程馬拉松官方成績卡——晶片時間 1:51:10，男子半馬組" width="600" height="899" sizes="160px" style="width:160px;flex-shrink:0;border-radius:8px" />
  <div style="font-size:14px;line-height:1.7;color:hsl(var(--muted-foreground))">
    <strong style="display:block;margin-bottom:6px;color:hsl(var(--foreground))">個人紀錄——2026 高陽半程馬拉松</strong>
    晶片時間：<strong>1:51:10</strong> · 男子半馬組<br />
    韓國的賽事籌辦意外地精緻——官方晶片時間、即時發放的數位成績卡、還有實時追蹤。外國跑者拿到的體驗和當地人完全一樣。
  </div>
</div>

---

## 韓國為何是「跑步＋玩牌」的完美目的地

> **快速解答**
> 兩個賽曆會在同一個窗口重疊：韓國的秋季馬拉松集中在十月底到十一月中，撲克系列賽也在九月底到十二月排了三個，而且賭場就在機場旁邊。所以「跑一場、打一場」不需要特別安排，只要把班機落在這個窗口裡。

理由不複雜。就是時機和地點。

**天氣窗口：** 韓國的夏天（六到八月）又熱又極度潮濕——跑步或觀光都不理想。但 ==**十月和十一月大概是造訪東亞最好的兩個月**==。氣溫降到 8–15°C，空氣乾爽通透，鄉間的楓葉轉成橙紅。韓國每個認真的跑者都懂這一點，整份馬拉松賽曆都是繞著這個窗口排的。

**賭場位置：** Paradise City 賭場座落在一個度假村園區裡，==g:與仁川國際機場實體相連==。第一航廈免費接駁車每 20 分鐘一班。賭場全年營運、有現金桌——不必打錦標賽也能玩。對國際牌手來說，這是全亞洲最好進的賭場：仁川落地、入住、開打。

**交通：** 首爾的地鐵又便宜又快，還有雙語標示。從 Paradise City（搭 AREX 機場快線）進首爾市中心約 1 小時。從首爾到春川（馬拉松所在城市）搭特快列車 90 分鐘。所有點都串得起來。

---

## 韓國 2026 馬拉松賽曆——外國跑者

> **快速解答**
> 五場比賽的外國人報名管道各不相同：有的走官網的 Foreigner 分頁、有的走 worldsmarathons、有的走韓國本地平台抽籤，截止日從六月到九月都有。2026 年的名額已全部關閉——這張表現在的用途，是替 2027 年抓每一場的開放時間。

<div style="overflow-x:auto;margin:20px 0">

| 賽事 | 日期 | 距離 | 地點 | 外國人報名 | 狀態 |
|---|---|---|---|---|---|
| 東亞首爾國際馬拉松 | 2026 年 3 月 | 全馬、10K | 首爾 | [dongamarathon.com](https://www.dongamarathon.com) | 2026 已結束 |
| ==**YTN 首爾旅遊馬拉松**== | ==**9/6**== | ==**11KM**==、半馬 | 首爾廣場 | [run.ytn.co.kr](http://run.ytn.co.kr) | 已截止——6/26 提前額滿 |
| ==g:**春川馬拉松**== | ==g:**10/25**== | ==g:全馬、10K== | ==g:春川== | ==g:[chuncheonmarathon.com](https://www.chuncheonmarathon.com)== | ==g:報名已截止（額滿）== |
| ==**JTBC 首爾馬拉松**== | ==**11/1**== | 全馬、10K | 首爾（上岩） | [worldsmarathons.com](https://worldsmarathons.com/marathon/international-seoul-marathon) | 已截止（外國人報名 9/4 結束）——名額有限，抽籤分配 |
| Garmin Run 韓國 | 11/15 | 半馬、10K | 高陽 | [runable.me](http://runable.me/product/18977?comp=18962) | 已截止（報名期為 7/1–3） |

</div>

==r:韓國夏天（六到八月）又熱又潮濕——賽事之所以集中在三月與十至十一月，就是這個原因。== 如果你要繞著跑步排一趟韓國行，十月或十一月就是目標窗口。

![韓國秋季馬拉松終點線——跑者衝線、計時看板清晰可見，觀眾與攝影師守在終點](/images/korea-poker-marathon-2026-marathon-finish.webp)

*韓國秋季賽事的終點氛圍。時鐘讀數約 1:34——競技型的當地跑者定下配速，休閒跑者則在整個上午陸續完賽。*

---

## 春川馬拉松 2026——10/25

> **快速解答**
> 春川的分量在履歷上：1946 年創辦、韓國第二古老，賽道經 AIMS 國際認證，繞衣岩湖跑、幾乎零車流。代價是名額——2026 年不到三分鐘就額滿，2027 年要在七月中開放的當下就送出，把護照號碼和信用卡先放在手邊。

==**春川**==（춘천）是一座從首爾往東搭特快列車約 80–90 分鐘的小城，坐落在江原道兩河交匯處的山谷裡。韓國人形容它「浪漫」。它舉辦的這場馬拉松，是全亞洲最受敬重的賽事之一。

**賽事本身：** 創辦於 1946 年——韓國第二古老的馬拉松。==g:經國際丈量與認證的賽道==（AIMS）。全馬 42.195K ＋ 10K。起點在孔之川橋，早上 9:00 起跑。賽道繞行 ==**衣岩湖**==（의암호）——一座被群山環抱的水庫，在十月底層林盡染。比賽日氣溫通常落在 8–14°C，是能跑出個人最佳的條件。

![秋天韓國公園裡別著號碼布的跑者——橙紅落葉沿著賽道，晨光灑落](/images/korea-poker-marathon-2026-marathon-race.webp)

*韓國秋季的比賽早晨。這就是十月跑步的樣子——10–14°C、滿地落葉、安靜的公園道路。*

**賽道實況：** 不是平路。5K 與 30K 附近有起伏丘陵，靠近春川大壩處有一段上坡，會把人群拉開。但回報——沿著寂靜的山中湖泊、伴著楓紅、零車流地奔跑——是任何都市馬拉松都給不了的。多篇國際跑者的賽事報告都說，這是他們在亞洲跑過最美的一場。

**如何報名（外國人）：**
1. 前往 [chuncheonmarathon.com](https://www.chuncheonmarathon.com)
2. 點 **참가신청（報名）** → **신청하기（申請）** → 選 **Foreigner（外國人）** 分頁
3. 需要護照號碼（供保險用）
4. 全馬報名：7/14 下午 2 點（韓國時間 KST）。繳費時段：7/20–21
5. 10K 報名：7/16 下午 2 點（KST）。繳費時段：7/22–23

==r:2026 年名額不到 3 分鐘就售罄。為 2027 年做準備：開放的當下，護照號碼和付款方式都要先備好——2026 年的報名 7 月就關了。==

**從首爾怎麼去：** ITX-青春特快列車，龍山站 → 春川，約 80–90 分鐘、約 ₩8,800。或搭地鐵京義中央線到清涼里，再轉春川線（全程約 100 分鐘，更便宜）。經濟、風景好、不用開車。

**春川 → Paradise City 路線：** 賽後搭特快回首爾（約 80 分鐘）→ 從首爾站或弘大搭 AREX → 在第一航廈轉 Paradise City 接駁車。全程約 3 小時。賽後週日傍晚完全跑得完。

---

## JTBC 首爾馬拉松 2026——11/1

> **快速解答**
> JTBC 是這幾場裡唯一能「跑完直接去玩牌」的：終點就在奧林匹克公園旁的地鐵站，轉 AREX 到仁川約兩小時。外國人只能透過 worldsmarathons 報名，而且改成抽籤——2026 年已於 9/4 截止，2027 年一開放就要登記。

==**JTBC 首爾馬拉松**== 是韓國的頂級都市賽事——25,000 到 50,000 名跑者，八成落在 20、30 歲，穿過首爾市中心的地標。如果說春川是鄉間美景，JTBC 就是門面秀。

**賽道：** 起點在 ==上岩世界盃體育場==（地鐵 6 號線，上岩世界盃體育場站）。路線行經德壽宮外側、穿過光化門廣場與東大門——把首爾的歷史與現代身分一次掃過。全馬終點在 ==奧林匹克公園==；10K 終點在 ==汝矣島公園==。全馬關門時間：5 小時。

**賽事細節：**
- 日期：==**2026 年 11 月 1 日（週日）**==，早上 8:00 起跑
- 距離：全馬 42.195K · 10K
- 起點：上岩世界盃體育場
- 博覽會（領取賽事包）：SETEC，江南——10/29–31，上午 10 點至下午 6 點
- 領取時須出示護照（不可代領）

**如何報名（外國人）：**
居住在韓國以外的外國跑者，一律透過 ==**[worldsmarathons.com](https://worldsmarathons.com/marathon/international-seoul-marathon)**== 報名。==r:名額有限、很快就滿==，請上官方頁面查詢最新剩餘名額——==r:**報名截止日就是 2026 年 9 月 4 日**==，同一天 23:59（KST）也是取消／退費的截止時間。持韓國護照者不適用此管道。

這個管道的報名費是 ==**10K €104**== 、==**全馬 42.2K €129**== （限海外跑者）。由於過去兩個項目都在開放首日就額滿，==g:名額已改為抽籤分配== ——早報名不等於卡到位子，只是進入抽籤。物資包必須在賽前的 EXPO 領取，報名姓名要與護照一致。路線從 ==**世界盃公園（麻浦區）**== 起跑，終點在 ==**奧林匹克公園（松坡區）**== 。

==g:JTBC 的外國人名額有限——2026 年已於 9/4 截止，2027 年開放時要趁早。==

**賽後 → 玩牌：** 全馬終點在 ==奧林匹克公園==（松坡，5/8 號線）。從那裡搭地鐵到首爾站或弘大，再轉 AREX 各站停 → 在第一航廈轉 Paradise City 接駁車。從衝線算起要抓 ==**1.5–2 小時**==，還沒算洗澡換衣。多數跑者在 12 點到 13 點之間完賽——傍晚安排一場現金桌很實際，中午則不行。

---

## Garmin Run 韓國 2026——11/15

> **快速解答**
> Garmin Run 的門檻不在距離而在報名：2026 年只開放 7 月 1 到 3 日、三天，還要抽籤。場地是高陽綜合運動場——BTS 和 BLACKPINK 開過演唱會的那座——從首爾市區搭 3 號線不到一小時，當天來回沒問題。

==**Garmin Run 韓國**==（가민런 코리아）是賽曆上較新的一場——但場地讓它值得一提。

賽事辦在京畿道一山的 ==**高陽綜合運動場**==（고양종합운동장）。國際跑者可能會從另一個脈絡認出這座體育場：這裡也是 ==g:BTS 與 BLACKPINK 演唱會== 的場地，是韓國規模最大的 K-pop 活動之一。對同時愛跑步的 K 文化旅客來說，這個組合值得繞一趟。

**賽事細節：**
- 日期：2026 年 11 月 15 日（週日），早上 8:00 起跑
- 距離：半馬 21.1K · 10K
- 場地：高陽綜合運動場，一山（京畿道高陽）
- 報名費：半馬 ₩80,000 · 10K ₩60,000
- 報名：[runable.me](http://runable.me/product/18977?comp=18962)——==r:僅 7/1–3，抽籤選出==

高陽從首爾市中心搭地鐵約 50 分鐘（3 號線，往大化／一山方向）。輕鬆當天來回。

---

## YTN 首爾旅遊馬拉松 2026——9/6

> **快速解答**
> 2026 年的 YTN 已在 9 月 6 日跑完，而且 6 月 26 日就提前額滿——它是先到先得，不是抽籤。留著這一節是因為它的位置特別：起點在市政廳前的首爾廣場，是唯一從市中心起跑的一場；九月仍偏暖，這一場適合跑半馬而不是全馬。

九月的賽事是 ==**YTN 首爾旅遊馬拉松**==（2026 年已於 9/6 舉行），起點在 ==**首爾廣場**==（서울광장）——就在市政廳正前方，背景是德壽宮與市中心天際線。

- 日期：2026 年 9 月 6 日，早上 7:30 起跑
- 距離：==**11KM**== · 半馬 —— ==r:這場沒有 10K 組別==
- 報名費：11KM ₩70,000 · 半馬 ₩80,000
- 起點：首爾廣場（市政廳一帶）
- 主辦：YTN
- 網站：[run.ytn.co.kr](http://run.ytn.co.kr)
- 報名：原訂 6/17 – 7/10，但==r:採先到先得，6/26 就提前額滿截止==

提醒：韓國的暑熱在八月達到頂點。九月仍偏暖（20–28°C），濕度到月底才緩和。在這種條件下，半馬比全馬好駕馭。

---

## 韓國撲克錦標賽：2026 年 9–12 月

> **快速解答**
> 三個系列賽的用法是「挑一個當軸」：APT 濟州 10/7 結束後隔 18 天就是春川馬拉松；GOP 仁川 10/30–11/8 直接壓在 JTBC 的 11/1 上；AJPC 12 月則沒有馬拉松可配。想兩件事都做，前兩個窗口才成立。

這裡賽曆就變得有意思了。有三個撲克賽事系列在韓國場館開打，時間點正好落在跟馬拉松同一個秋季窗口——其中一個還跟 JTBC 首爾直接重疊。

<div style="overflow-x:auto;margin:20px 0">
<table style="width:100%;border-collapse:collapse;font-size:14px;line-height:1.5">
  <thead>
    <tr style="border-bottom:2px solid rgba(255,255,255,0.12)">
      <th style="text-align:left;padding:10px 14px;color:hsl(var(--muted-foreground));font-weight:600;white-space:nowrap;width:160px">日期</th>
      <th style="text-align:left;padding:10px 14px;color:hsl(var(--muted-foreground));font-weight:600">錦標賽</th>
      <th style="text-align:left;padding:10px 14px;color:hsl(var(--muted-foreground));font-weight:600">場館</th>
      <th style="text-align:left;padding:10px 14px;color:hsl(var(--muted-foreground));font-weight:600">重點資訊</th>
    </tr>
  </thead>
  <tbody>
    <tr style="background:rgba(212,175,55,0.07);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:10px 14px;font-weight:700;white-space:nowrap">9/25 – 10/7</td>
      <td style="padding:10px 14px"><strong>APT 濟州 2026</strong></td>
      <td style="padding:10px 14px">LES A Casino，濟州</td>
      <td style="padding:10px 14px;color:hsl(var(--muted-foreground))">總保證獎池 $3.5M+ · 136 場賽事 · 距春川馬拉松 18 天</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.07);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:10px 14px;font-weight:700;white-space:nowrap">10/30 – 11/8</td>
      <td style="padding:10px 14px"><strong>GOP 仁川<br/><span style="font-size:12px;color:hsl(var(--muted-foreground))">The Labyrinth Trail</span></strong></td>
      <td style="padding:10px 14px">Paradise City，仁川</td>
      <td style="padding:10px 14px"><span style="background:rgba(239,68,68,0.15);color:#f87171;padding:2px 7px;border-radius:4px;font-size:12px;font-weight:600">⚡ JTBC 馬拉松 11/1 = 本賽事第 3 天</span></td>
    </tr>
    <tr style="background:rgba(212,175,55,0.07);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:10px 14px;font-weight:700;white-space:nowrap">12/4–13</td>
      <td style="padding:10px 14px"><strong>AJPC Samurai Circuit<br/><span style="font-size:12px;color:hsl(var(--muted-foreground))">Incheon 2026 III</span></strong></td>
      <td style="padding:10px 14px">Paradise City，仁川</td>
      <td style="padding:10px 14px;color:hsl(var(--muted-foreground))">年末選項 · 單項賽程與買入未定</td>
    </tr>
  </tbody>
</table>
</div>

### APT 濟州 2026——9/25–10/7

==**濟州島**== 是韓國最熱門的國內度假勝地——火山海岸線、亞熱帶氣候，多數國籍免簽。賽事辦在濟州神話世界（Jeju Shinhwa World）裡的 ==**LES A Casino**==。

- **保證獎池：** ==g:136 場獎盃賽事、總保證獎池 $3.5M+==（主賽事：買入 ==**KRW 270 萬**== ≈ $1,800，==g:KRW 22 億保證==）
- **完整賽程：** 已公布——13 天內 136 場賽事 · [theasianpokertour.com](https://www.theasianpokertour.com/series/apt-jeju-south-korea-2026/info)
- 僅限外國人 · 19 歲以上 · 持有效護照（海外韓國人：韓國護照＋綠卡或移民證明）

==g:APT 濟州 10/7 結束——春川馬拉松是 10/25。中間隔 18 天。== 從濟州飛首爾，休息、逛逛，然後開跑。

---

### GOP 仁川——The Labyrinth Trail（10/30–11/8）

==**Gods of Poker（GOP）**== 回到 Paradise City，舉辦 2026 年第二站仁川賽事，10 月 30 日打到 11 月 8 日。這個系列賽的日期，正好壓在 JTBC 首爾馬拉松那天。

==**11/1 既是 JTBC 比賽日，也是 GOP 系列賽的第 3 天。**==

對一個已經在韓國打錦標賽的牌手來說：比賽早晨是 6 點的鬧鐘（8 點起跑，提前約一小時集合）、42K 穿過首爾市中心、在奧林匹克公園完賽。==r:如果你跑進 4 小時==，中午就結束，傍晚能坐上 Paradise City 的牌桌；若是 4:30–5:00（關門時間正好 5 小時），那就變成晚上的行程。牌桌 24 小時開著，所以兩件事總能塞進同一天。幾乎沒人在寫這個重疊。

🪶 **但要說句實在話：能打，不代表打得好。** 跑完 42K 之後，肝醣耗盡、身體脫水、前一晚也沒睡飽——這正好是牌手 call 得太寬、坐得太久的狀態。如果那晚還是要坐下，就先定好一段短時間和停損線，或者乾脆把牌局挪到隔天。牌桌全年都開，馬拉松只有那一個早上。

- **日期：** 2026 年 10 月 30 日 – 11 月 8 日
- **場館：** Paradise City 賭場，仁川（仁川機場第一航廈免費接駁）
- **資訊：** [pokerdiscover.com](https://pokerdiscover.com/series/gods-of-poker-gop-incheon-the-labyrinth-trail-incheon-30-oct-08-nov-2026)
- 完整賽程與買入待公布——請關注官方 GOP／Paradise City 管道

---

### AJPC Samurai Circuit Incheon III（2026 年 12 月 4–13 日）

==**AJPC Samurai Circuit**== 為韓國撲克賽曆收尾，12 月在 Paradise City 開打。對錯過 10–11 月窗口的牌手，這是年末的選項。

- **日期：** 2026 年 12 月 4–13 日（據 pokercalendar.asia 賽曆 · 8/28 查核）
- **場館：** Paradise City 賭場，仁川
- **資訊：** [pokercalendar.asia](https://pokercalendar.asia/en/pokertournaments/ajpc-samurai-circuit-incheon-2026-iii/)
- 賽程與買入待公布

12 月沒有馬拉松重疊，但 Paradise City 的現金桌無論如何全年都在跑。

---

## Paradise City 賭場——撲克這一面

> **快速解答**
> 對這篇的讀者，賭場最重要的一項不是錦標賽而是現金桌：全年、每天、24 小時開，不必等系列賽也能玩。門檻是護照、19 歲、非韓國公民（有海外永居除外）；跑完馬拉松想去的話，園區裡的 Spa 和桑拿比牌桌更該先去。

馬拉松集中在 10–11 月，但 ==**百樂達斯城（Paradise City）賭場**== 是全年營運的。它不是只辦錦標賽的場館——現金桌每天都為國際牌手開著。

**基本資訊：**
- 地點：仁川，與仁川國際機場實體相連
- 免費接駁：第一航廈（3C 登機門），每 20 分鐘一班，車程約 10 分鐘
- 飯店：Hotel Paradise，五星級，園區內
- ==r:僅限外國人——無海外永久居留權的韓國公民不得進入==
- 海外韓國人（在海外的永久居民）可持韓國護照＋綠卡或移民證明進入
- 年齡：19 歲以上，持有效護照
- 遊戲：無限注德州撲克現金桌、其他桌上遊戲、吃角子老虎
- 園區內有 Spa 與桑拿——剛跑完 42K 的話，這點很有用

想看完整的錦標賽全貌（APT 仁川 8 月 7–16 日，保證獎池 $2.7M），請見[APT 仁川 2026 錦標賽指南](/zh-hant/blog/apt-incheon-2026-guide "thumb:/images/apt-incheon-2026-guide-hero.webp")；想先熟悉錦標賽怎麼運作，就看[德州撲克錦標賽入門](/zh-hant/blog/holdem-tournament)。

**2026 韓國撲克＋馬拉松完整賽曆：**

<div style="overflow-x:auto;margin:20px 0">
<table style="width:100%;border-collapse:collapse;font-size:13px;line-height:1.5">
  <thead>
    <tr style="border-bottom:2px solid rgba(255,255,255,0.12)">
      <th style="text-align:left;padding:9px 12px;color:hsl(var(--muted-foreground));font-weight:600;white-space:nowrap">日期</th>
      <th style="text-align:left;padding:9px 12px;color:hsl(var(--muted-foreground));font-weight:600">活動</th>
      <th style="text-align:left;padding:9px 12px;color:hsl(var(--muted-foreground));font-weight:600">場館</th>
      <th style="text-align:left;padding:9px 12px;color:hsl(var(--muted-foreground));font-weight:600">細節</th>
    </tr>
  </thead>
  <tbody>
    <tr style="opacity:0.5;border-bottom:1px solid rgba(255,255,255,0.05)">
      <td style="padding:8px 12px;white-space:nowrap">5/15–24</td>
      <td style="padding:8px 12px">🃏 GOP 仁川 2026 <em>（已結束）</em></td>
      <td style="padding:8px 12px">Paradise City，仁川</td>
      <td style="padding:8px 12px;color:hsl(var(--muted-foreground))">已完賽</td>
    </tr>
    <tr style="opacity:0.5;border-bottom:1px solid rgba(255,255,255,0.05)">
      <td style="padding:8px 12px;white-space:nowrap">5/15–25</td>
      <td style="padding:8px 12px">🃏 TLPT 濟州 2026 <em>（已結束）</em></td>
      <td style="padding:8px 12px">LES A Casino，濟州</td>
      <td style="padding:8px 12px;color:hsl(var(--muted-foreground))">已完賽</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">8/7–16</td>
      <td style="padding:9px 12px">🃏 <strong>APT 仁川 2026</strong></td>
      <td style="padding:9px 12px">Paradise City，仁川</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">保證獎池 $2.7M · 主賽事 $1M GTD</td>
    </tr>
    <tr style="background:rgba(34,197,94,0.05);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">9/6</td>
      <td style="padding:9px 12px">🏃 YTN 首爾旅遊馬拉松</td>
      <td style="padding:9px 12px">首爾廣場</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">11KM · 半馬 · 7:30 · 報名已截止</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">9/25–10/7</td>
      <td style="padding:9px 12px">🃏 <strong>APT 濟州 2026</strong></td>
      <td style="padding:9px 12px">LES A Casino，濟州</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">$3.5M+ GTD · 136 場賽事 · 距春川 18 天</td>
    </tr>
    <tr style="background:rgba(34,197,94,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">10/25（日）</td>
      <td style="padding:9px 12px">🏃 <strong>春川馬拉松</strong></td>
      <td style="padding:9px 12px">孔之川，春川</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">全馬 · 10K · 9:00 · 報名 7/14</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">10/30–11/8</td>
      <td style="padding:9px 12px">🃏 <strong>GOP 仁川<br/><span style="font-size:11px;font-weight:400;opacity:0.7">The Labyrinth Trail</span></strong></td>
      <td style="padding:9px 12px">Paradise City，仁川</td>
      <td style="padding:9px 12px"><span style="background:rgba(239,68,68,0.18);color:#fca5a5;padding:1px 6px;border-radius:3px;font-size:11px;font-weight:700">⚡ JTBC 馬拉松在第 3 天</span></td>
    </tr>
    <tr style="background:rgba(239,68,68,0.06);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">11/1（日）</td>
      <td style="padding:9px 12px">🏃 <strong>JTBC 首爾馬拉松</strong></td>
      <td style="padding:9px 12px">上岩世界盃體育場 → 奧林匹克公園</td>
      <td style="padding:9px 12px"><span style="background:rgba(239,68,68,0.18);color:#fca5a5;padding:1px 6px;border-radius:3px;font-size:11px;font-weight:700">⚡ 同週 GOP 錦標賽進行中</span></td>
    </tr>
    <tr style="background:rgba(34,197,94,0.05);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">11/15（日）</td>
      <td style="padding:9px 12px">🏃 Garmin Run 韓國</td>
      <td style="padding:9px 12px">高陽綜合運動場</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">半馬 · 10K · 8:00 · 報名 7/1–3</td>
    </tr>
    <tr style="background:rgba(212,175,55,0.08);border-bottom:1px solid rgba(255,255,255,0.06)">
      <td style="padding:9px 12px;font-weight:700;white-space:nowrap">12/4–13</td>
      <td style="padding:9px 12px">🃏 AJPC Samurai Circuit III</td>
      <td style="padding:9px 12px">Paradise City，仁川</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">單項賽程與買入待公布</td>
    </tr>
    <tr style="border-top:1px solid rgba(255,255,255,0.08)">
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">全年</td>
      <td style="padding:9px 12px">🃏 現金桌（NLHE）</td>
      <td style="padding:9px 12px">Paradise City，仁川</td>
      <td style="padding:9px 12px;color:hsl(var(--muted-foreground))">24 小時營業 · 僅限外國人</td>
    </tr>
  </tbody>
</table>
</div>

==g:沒人在講的甜蜜點：== **APT 濟州 10 月 7 日結束——春川馬拉松是 10 月 25 日。** 中間隔 18 天。打完錦標賽，留在韓國，跑那場馬拉松。一張機票兩件事全包。如果你願意待到 11 月 1 日，JTBC 首爾也就在那裡。

最乾淨的一趟雙吃：APT 仁川（8/7–16）→ 在首爾休養 → 春川馬拉松（10/25）或 JTBC（11/1）。或者：APT 濟州（9/25–10/7）→ 在韓國待 18 天 → 春川（10/25）。⚠ 2026 年的馬拉松外國人報名只有 JTBC，截止 9/4——春川這條路線請當作 2027 年的模板。

---

## 行程範例——兩件事怎麼併

> **快速解答**
> 兩個範例的差別只在「以哪場撲克賽為軸」：APT 仁川版是八月進場、長住到十月底跑春川；APT 濟州版是九月底進場、從濟州回首爾、跑春川、再加碼 JTBC。不管哪一版，馬拉松那週都排在撲克之後，因為恢復比籌碼更難補。

**方案 A：APT 仁川 ＋ JTBC 首爾（長停留）—— ==r:請以 2027 年來規劃==，8/7 的起點已經過了**

| 天 | 計畫 |
|---|---|
| 第 1 天（8/7） | 飛抵 → Paradise City · APT 仁川開賽 |
| 8/7–16 | ==g:APT 仁川 2026 錦標賽== |
| 8/17–10/24 | 韓國長停留——首爾、釜山、慶州當天來回 · 打現金桌 |
| 10/25 | ==**春川馬拉松**== · 回首爾／Paradise City |
| 11/1 | ==**JTBC 首爾馬拉松**== |
| 11/2 起 | 離境 |

*這是覆蓋最大的方案。多數牌手不會待到 2 個月以上，但它秀出這份賽曆能玩到什麼程度。*

**方案 B：APT 濟州 ＋ 春川馬拉松（隔 18 天）**

| 天 | 計畫 |
|---|---|
| 9/25 | 飛抵 → 濟州神話世界 · APT 濟州開賽 |
| 9/25–10/7 | ==g:APT 濟州 2026 錦標賽== · 場次間逛濟州 |
| 10/8–9 | 濟州飛首爾 · 入住首爾或 Paradise City 飯店 |
| 10/10–24 | 首爾時光——景福宮、弘大、明洞、打現金桌 |
| ==**10/25**== | ==**春川馬拉松**==（從首爾搭 ITX，約 80 分鐘） |
| 10/26–31 | 恢復 · 首爾或 Paradise City 現金桌 |
| 11/1 | 加碼選項：==**JTBC 首爾馬拉松**==（連兩週背靠背） |
| 11/2 | 從仁川機場離境 |

---

## 7 天韓國「跑步＋玩牌」行程範例（JTBC 版）

> **快速解答**
> 七天是最短的可行單位，卡點是博覽會：JTBC 的賽事包只能本人在賽前三天到 SETEC 領，所以週六一定要在首爾。倒過來排就是——週四落地、週六領包、週日跑、週一休、週二才移到 Paradise City 打牌。

| 天 | 計畫 |
|---|---|
| 第 1 天（四） | 飛抵 → 入住 Paradise City 或首爾飯店 · 傍晚打現金桌或休息 |
| 第 2 天（五） | 首爾觀光——景福宮、北村、廣藏市場 |
| 第 3 天（六） | 到 SETEC（江南）博覽會領賽事包 · 輕鬆慢跑 · 早睡 |
| 第 4 天（日） | ==**JTBC 首爾馬拉松**==（8:00 起跑）· 恢復餐 · 休息 |
| 第 5 天（一） | 首爾一日：弘大、明洞、清溪川散步 |
| 第 6 天（二） | 移動到 Paradise City · 下午＋晚上一場現金桌 |
| 第 7 天（三） | 早上玩牌 · 搭 AREX 到仁川機場 · 離境 |

---

## 漢江跑步——首爾 80 公里的河濱步道

> **快速解答**
> 沒有比賽的日子也能跑：漢江兩岸有八十多公里的步道，平坦、沒有紅綠燈、整夜都開，從 Paradise City 搭 AREX 約一小時就到。新手選汝矣島 5K 環線，那裡的 Runner Station 有淋浴和置物櫃；傍晚想看噴泉就去盤浦。

比賽日之間、或牌局場次之間，首爾每個跑者最後都會去的地方只有一個：==**漢江公園（Hangang Park）**==。

漢江橫貫首爾市中心，兩岸都有超過 ==g:80 公里平坦的跑步與自行車鋪面步道==——連續、無車、24 小時開放。沒有坡、沒有紅綠燈。只有河風、城市天際線，還有其他跑者。

<div style="display:flex;gap:12px;margin:18px 0;flex-wrap:wrap">
  <div style="flex:1;min-width:240px">
    <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-hangang1.webp" alt="漢江公園自行車與跑步步道——前景長椅上的情侶，河濱上的自行車與行人，對岸是首爾天際線" width="650" height="867" sizes="(max-width: 520px) 100vw, 330px" style="width:100%;border-radius:8px;display:block" />
    <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:6px;text-align:center">汝矣島漢江公園——跑者最愛的一段。平坦、寬闊、無車、24 小時開放。</p>
  </div>
  <div style="flex:1;min-width:240px">
    <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-hangang2.webp" alt="漢江公園林蔭跑步步道——河濱步道上的慢跑者與自行車騎士，對岸是公寓高樓" width="560" height="700" sizes="(max-width: 520px) 100vw, 330px" style="width:100%;border-radius:8px;display:block" />
    <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:6px;text-align:center">河濱步道往兩個方向都延伸數公里——當地人天天用來跑步、騎車、清晨散步。</p>
  </div>
</div>

**熱門跑步路線：**

- **汝矣島 5K 環線**——起／終點在汝矣渡口站（5 號線，2 號出口）。繞汝矣島的平坦環線，經過國會與 63 大廈。最適合新手。
- **汝矣島「地瓜」8K**——從汝矣渡口延伸的環線，GPS 軌跡長得像一顆地瓜。起點在 **Runner Station**（汝矣渡口站內）——那裡有淋浴、置物櫃和更衣室。
- **盤浦 5K 路段**——在銅雀大橋與東湖大橋之間，南岸。經由高速巴士客運站（3／7／9 號線）進入。安靜、風景好，適合傍晚跑（盤浦大橋的彩虹噴泉入夜後會亮起）。
- **纛島**——7 號線，纛島遊園地站。連通首爾林。當地味、人較少。

==g:最佳時段：清晨（6–8 點）或傍晚（6–8 點）。== 步道夜間照明良好。多數公園入口的自行車租借約 ₩3,000–5,000／小時。

---

### 漢江泡麵（한강라면）——唯一算數的賽後一餐

還有一件沒人會告訴跑者、但每個人都記得的事——==**한강라면（漢江泡麵）**==。

你走進公園裡的 GS25、CU 或 7-Eleven，買一碗即食泡麵（辛拉麵、火辣雞（Buldak）、金拉麵（Jin）——你挑）。店員給你一個 ==g:附條碼的特製紙碗==。你把它端到外面的泡麵烹煮機，掃條碼，按開始。機器會自動量取、注入剛好溫度的熱水。四分鐘後，你打進一顆蛋、攪一攪，端著碗走到河邊的長椅上。

就這樣。這就是那個在韓國社群被貼過上百萬次、如今專門吸引國際旅客特地來複製的體驗。

<div style="display:flex;gap:12px;margin:18px 0;flex-wrap:wrap">
  <div style="flex:1;min-width:200px">
    <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-ramen-machine.webp" alt="GS25 便利商店的漢江泡麵烹煮機——為紙碗即食麵自動供應熱水" width="525" height="700" sizes="(max-width: 520px) 100vw, 330px" style="width:100%;height:280px;object-fit:cover;border-radius:8px;display:block" />
    <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:6px;text-align:center">公園便利商店的機器。掃你的條碼紙碗、按開始——剩下的交給機器。機器免費使用。</p>
  </div>
  <div style="flex:1;min-width:200px">
    <img loading="lazy" decoding="async" src="/images/korea-poker-marathon-2026-ramen.webp" alt="一碗漢江泡麵——紙碗裡的辛拉麵加一顆蛋，在河邊享用" width="700" height="525" sizes="(max-width: 520px) 100vw, 330px" style="width:100%;height:280px;object-fit:cover;border-radius:8px;display:block" />
    <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:6px;text-align:center">成品。漢江邊一碗加蛋的熱辛拉麵。總花費 ₩4,000–5,000（約 $3）。大概是首爾最棒的賽後一餐。</p>
  </div>
</div>

**大概要花多少：**

| 項目 | 價格（韓元） | 約合美元 |
|:---|:---:|:---:|
| 泡麵＋條碼碗（GS25／CU／7-Eleven） | ₩3,500–4,000 | ~$2.60–3.00 |
| 生蛋 | ₩500 | ~$0.40 |
| 泡菜包 | ₩1,500 | ~$1.10 |
| 野餐墊租借 | ₩3,000–5,000 | ~$2.20–3.70 |

機器本身 ==g:免費使用——你只付泡麵的錢==。使用說明有韓文和英文。多數公園也有外送區，想吃炸雞也行。

**漢江泡麵最佳公園：**
- **汝矣島**（여의도）——新手首選，設施最齊、離 Runner Station 最近。5 號線汝矣渡口站，2 號出口。
- **盤浦**（반포）——傍晚首選，入夜有彩虹噴泉橋秀。3／7／9 號線高速巴士客運站。
- **纛島**（뚝섬）——最安靜、當地味。7 號線纛島遊園地站。

**從 Paradise City 到漢江公園：** 免費接駁 → 第一航廈 → AREX 各站停（約 60 分鐘）到任一首爾市中心車站，再挑你的公園：

- **汝矣島**——汝矣渡口站（5 號線，2 號出口）· 現場有含淋浴／置物櫃的 Runner Station
- **盤浦**——高速巴士客運站（3／7／9 號線）· 步行 5 分鐘 · 夜間彩虹噴泉
- **二村**（이촌）——二村站（4 號線／京義中央線）· 較安靜、當地人多
- **纛島**——纛島遊園地站（7 號線）· 連通首爾林

==g:每個漢江公園都有自己的 GS25、CU 或 7-Eleven，都配泡麵機——你不必只守著一個地方。== 挑一個離你在首爾當下位置最近的公園就好。

---

## 馬拉松賽曆參考

> **快速解答**
> 這個連結的用途是看 2027 年：韓國馬拉松的報名多半在賽前三到五個月就開放，而且熱門場次幾分鐘就滿。訂機票之前，先到各賽事官網確認日期沒動，再到這份賽曆核對一次。

想看最新完整的韓國馬拉松賽程（所有距離、報名連結與日期）：[marathon.pe.kr](http://marathon.pe.kr/index_calendar.html)

日期與活動可能變動——訂機票前，請先查各賽事官網與這份賽曆。

---

:::readnext[繼續閱讀]
/zh-hant/blog/holdem-tournament | 德州撲克錦標賽入門指南 | /images/holdem-tournament-hero.webp
/zh-hant/blog/holdem-tournament-vs-cash-game | 錦標賽 vs 現金桌差異 | /images/holdem-tournament-vs-cash-hero.webp
:::

## 常見問題 FAQ

**Q. 不會韓文也能跑韓國馬拉松嗎？**

A. 可以。春川與 JTBC 馬拉松都有專為外國跑者設的英文報名管道。領號碼布、賽道標示、終點動線都對外國人友善。其餘的用 Google 翻譯就能搞定。

**Q. 同一趟能同時跑春川（10/25）和 JTBC 首爾（11/1）嗎？**

A. 就賽曆而言可以——兩者只差一週。春川離首爾搭車約 80 分鐘。週日跑春川，週一到週六恢復，隔週日跑 JTBC。很多韓國跑者就是這樣連兩週跑。2026 年注意：春川名額已售罄，所以這套完整組合實際上是 2027 年的計畫。記得多留恢復時間；春川賽道有坡。

**Q. 週日跑完馬拉松抵達，Paradise City 賭場還開嗎？**

A. 開。Paradise City 賭場 24 小時營運。週日賽後入住對國際旅客是常態。現場的 Spa 與桑拿特別受馬拉松跑者歡迎，很適合在傍晚牌局前先恢復一下。

**Q. 沒有韓國地址，外國人能報名春川馬拉松嗎？**

A. 可以——流程每年都一樣（2026 年名額已售罄；下一個窗口約在 2027 年 7 月開啟）。chuncheonmarathon.com 的外國人報名分頁就是為海外參賽者設計的。你需要護照號碼、電子郵件和付款方式。賽事包在賽前於現場發放。

**Q. Garmin Run 場地跟 K-pop 有什麼關聯？**

A. 高陽綜合運動場是韓國最大的演唱會體育場之一——BTS、BLACKPINK 及多數主流 K-pop 團體，都在這座京畿道場地開唱。對同時愛跑步的 K 文化粉絲來說，這座體育場本身就是吸引力。賽事和場地的名氣是兩件事，但共享同一群受眾。

**Q. 台灣玩家赴韓，Paradise City 該注意什麼？**

A. 帶好護照（無海外永久居留權的韓國公民不得進入）、年滿 19 歲。從仁川機場第一航廈搭免費接駁約 10 分鐘就到，落地不必先進市區。想先熟悉錦標賽的買入、盲注結構與衛星賽怎麼運作，可以先看[德州撲克錦標賽入門指南](/zh-hant/blog/holdem-tournament)。

---

## 結語

韓國擁有東亞最好的秋季跑步條件、一座離國際機場十分鐘的賭場，還有一座在非跑步時段同樣回報你的城市。

同時握著一份撲克資金和一本馬拉松訓練日誌的人，是真實存在的社群。BTS 的 Suga 沒有發明這個重疊——他只是讓它被看見。

要記下的日期：==**春川 10/25**==、==**JTBC 首爾 11/1**==、Garmin Run 11/15——2026 年的報名管道都已關閉（最後一個是 JTBC，9/4 截止）。Paradise City 全年開放。

想看完整的撲克錦標賽全貌（APT 仁川，保證獎池 $2.7M，8 月 7–16 日）與賽事運作，先讀[德州撲克錦標賽入門指南](/zh-hant/blog/holdem-tournament)。想搞懂錦標賽與現金桌的策略差異：[錦標賽 vs 現金桌](/zh-hant/blog/holdem-tournament-vs-cash-game)。

---

## 相關文章

<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/zh-hant/blog/holdem-tournament" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">錦標賽</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">德州撲克錦標賽入門指南</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">買入、盲注結構、衛星賽與新手清單</div>
  </a>
  <a href="/zh-hant/blog/holdem-tournament-vs-cash-game" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">賽制</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">錦標賽 vs 現金桌</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">兩種賽制的策略與心態差在哪</div>
  </a>
  <a href="/zh-hant/blog/holdem-short-stack" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">策略</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">短籌碼怎麼打</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">錦標賽後段活下來的 push-fold 心法</div>
  </a>
</div>
`.trim(),
};

export default POST;
