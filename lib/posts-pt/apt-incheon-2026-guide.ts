import type { Post } from "../posts";

export const POST: Post = {
  slug: "apt-incheon-2026-guide",
  title: "APT Incheon 2026 — Festival de Pôquer + Guia de Viagem pela Coreia",
  seoTitle: "Voar pra Coreia pra jogar pôquer? APT Incheon 2026 no Paradise City + Seul em 3 dias",
  desc: "APT Incheon 2026 — 7 a 16 de ago, Paradise City perto do Aeroporto de Incheon. $2.7M GTD, Main Event ~$1.703. Classifique a partir de $3.50 pela Natural8.",
  tldr: "O APT Incheon 2026 rola de 7 a 16 de agosto no Paradise City Casino, em Incheon — 10 min de shuttle grátis a partir do Terminal 1. Main Event: buy-in de KRW 2.500.000 (~$1.703), $1M GTD. Classifique a partir de $3.50 pelos step satellites da Natural8. Só estrangeiros (cidadãos coreanos não podem entrar). Seul fica a ~1 hora e é uma das melhores cidades da Ásia para comida, cultura e vida noturna.",
  category: "tournament",
  date: "2026-06-18",
  updated: "2026-07-28",
  masterUpdated: "2026-07-28",
  keepImagesInBody: true,
  readTime: "16 min",
  emoji: "🇰🇷",
  image: "/images/apt-incheon-2026-guide-hero.webp",
  imageAlt: "APT Incheon 2026 no Paradise City — uma sala premium de torneio de pôquer à noite com mesas de feltro verde, pilhas de fichas e o skyline do porto de Incheon através das janelas do chão ao teto",
  tags: [
    "APT Incheon 2026",
    "torneio de pôquer Coreia 2026",
    "Paradise City Casino pôquer",
    "viagem de pôquer Seul",
    "satellite Natural8 APT",
    "pôquer viagem Coreia",
    "poker tour Ásia Incheon",
    "viagem de pôquer Coreia",
  ],
  content: `
Seul entrou na lista de viagem de todo mundo desde que Parasita ganhou o Oscar, Round 6 dominou toda conversa e o BTS virou um fenômeno global. Agora tem mais um motivo pra fechar aquela passagem: ==**um dos maiores festivais de pôquer da Ásia acontece bem do lado do Aeroporto Internacional de Incheon**==.

APT Incheon 2026. 7 a 16 de agosto. Paradise City Casino. $2.7 milhões em premiações garantidas. E quando as cartas descansam — bibimbap, noites à beira do Rio Han, passeios por palácios e frango frito coreano até as 4 da manhã.

A viagem de pôquer e a viagem pela Coreia são a mesma viagem. ==g:O Paradise City fica a 10 minutos do aeroporto, e Seul fica a 1 hora de metrô.== Você pode vir só pelo cassino, só pelo turismo, ou fazer os dois — a localização deixa fácil de qualquer jeito.

---

> **Resposta rápida**
> O APT Incheon 2026 rola de ==**7 a 16 de ago**== no Paradise City Casino, em Incheon. O resort fica a ==g:10 min de shuttle grátis do Terminal 1 do Aeroporto de Incheon== — sai a cada 30 min, sem custo. Buy-in do Main Event: ==**KRW 2.500.000 (~$1.703)**==, $1M garantido. Os step satellites começam a partir de ==g:**$3.50 pela Natural8**==. O cassino é ==r:só para estrangeiros== — cidadãos coreanos não podem entrar.

---

## Por que a Coreia em 2026? (O argumento fora do pôquer)

Antes do pôquer: a Coreia do Sul virou silenciosamente um dos melhores países pra visitar na Ásia, e o resto do mundo está sacando isso rápido.

- **Segurança:** Seul aparece consistentemente como uma das cidades mais seguras do mundo. Andar sozinho às 3 da manhã é rotina tanto pra locais quanto pra turistas.
- **Comida:** a comida de rua, a cultura de churrasco e os restaurantes com estrela Michelin de Seul rivalizam com qualquer capital gastronômica do mundo — pela metade do preço.
- **Transporte:** o sistema de metrô é mais barato, mais limpo e mais confiável do que o da maioria das cidades. Um cartão T-money resolve tudo.
- **Cultura K:** o Palácio Gyeongbokgung, a Bukchon Hanok Village e a cena de música ao vivo de Hongdae são experiências culturais legitimamente de nível mundial.
- **Custo:** comparada a Tóquio, Hong Kong ou Sydney, Seul é notavelmente acessível — um jantar completo de churrasco coreano com bebidas sai por menos de $20 por pessoa.

E pra visitantes internacionais: ==g:a maioria dos portadores de passaporte ocidental entra na Coreia sem visto ou via um K-ETA gratuito (autorização eletrônica de viagem) que leva minutos pra solicitar online.==

**E tem um motivo que vale só pra este ano.** O próprio anúncio da APT diz sem rodeios: *"with the APT turning twenty in 2026"* — o tour completa vinte anos em 2026. Ele nasceu em 2006, quando Antanas "Tony G" Guoga venceu o Main Event inaugural em Singapura. A temporada de aniversário fecha com o APT Championship em Taipei, e é por isso que **os assentos para o Championship distribuídos em Incheon valem mais que o normal neste ano**.

---

## APT Incheon 2026 — O Torneio num Relance

<div style="overflow-x:auto;margin:20px 0">

| Detalhe | Info |
|---|---|
| **Datas** | ==**7 a 16 de agosto de 2026**== |
| **Local** | Paradise City Casino, Incheon, Coreia do Sul |
| **Total Garantido** | ==g:**Mais de KRW 4 bilhões (~$2.7M USD)**== |
| **Buy-In do Main Event** | KRW 2.500.000 ==**($1.703 USD)**== |
| **GTD do Main Event** | ==g:KRW 1.500.000.000 (~$1M USD)== |
| **Entrada de Satellite Mais Barata** | ==g:**$3.50 pela Natural8**== |
| **Patrocinador** | Natural8 |
| **Quem Pode Entrar** | ==r:Só estrangeiros — cidadãos coreanos não podem entrar== |
| **Idade Mínima** | 19+ com documento de foto válido |
| **Inscrição** | registrations@apt.poker |

</div>

---

## Agenda Completa de Eventos

<div style="overflow-x:auto;margin:20px 0">

| Evento | Datas | Buy-In (USD) | GTD (USD) |
|:---|:---|:---:|:---:|
| Korea National Cup | 7–8 de ago | ~$477 | ~$136K |
| Mystery Bounty Hunter | 8–9 de ago | ~$610 | ~$204K |
| APT Super High Roller | 8–9 de ago | ==r:~$10.200== | ~$340K |
| ==**APT Main Event**== | ==**9–15 de ago**== | ==**~$1.703**== | ==g:**~$1M**== |
| Superstar Challenge | 10–11 de ago | ~$23.800 | — |
| Zodiac Classic | 13–14 de ago | ~$2.000 | ~$340K |
| Mini Main Event | 14–16 de ago | ~$610 | ~$272K |
| APT High Roller | 15–16 de ago | ~$3.400 | ~$340K |
| Ultra Stack | 15–16 de ago | ~$475 | ~$102K |
| Micro Main Event | 16 de ago | ~$270 | — |

</div>

**O que torna o APT Incheon único:** a programação tem formatos que você não vai achar no WPT nem no EPT — "Choose Your Own Adventure" (o BB escolhe o jogo), Omaha 21, um mix só de stud com 12 jogos e ==g:nove eventos femininos, mais do que a maioria dos grandes tours faz num ano inteiro==. Se você está cansado do mesmo grind de NLHE, esse é o festival pra isso.

---

---

## Quanto valem os assentos do APT Championship?

Ao longo do festival são distribuídos **nove assentos para o APT Championship (APTC) Taipei 2026** — cada um avaliado em **$10.000**. A maior parte da cobertura menciona só "três assentos no Main Event". É incompleto, e essa diferença pesa na hora de escolher em quais torneios entrar.

<div style="overflow-x:auto;margin:20px 0">

| Torneio | Buy-in (USD) | Assentos APTC |
|:---|:---:|:---:|
| **APT Main Event** | ~$1.703 | **3** (top 3) |
| APT Super High Roller | ~$10.200 | 1 |
| APT High Roller | ~$3.400 | 1 |
| Superstar Challenge | ~$23.800 | 1 |
| **Ultra Stack** | **~$475** | **1** |
| **Mini Main Event** | **~$610** | **1** |
| Zodiac Classic | ~$2.000 | 1 |
| **Total** | | **9** |

</div>

==g:Olha as duas linhas mais baratas.== Um **Ultra Stack de $475** ou um **Mini Main de $610** carrega cada um um assento inteiro de $10.000. Ganhar um desses é uma proposta completamente diferente de terminar no top 3 de um Main Event com mais de mil entradas — e o ticket que você leva é o mesmo.

**Pra onde esse ticket te leva:** o APT Championship 2026 acontece de **13 a 29 de novembro no Red Space, em Taipei**. O Main Event do Championship é de **23 a 27 de novembro, um freezeout de $10.000 com $5.000.000 garantidos** — o maior evento do ano da APT. E Taipei, diferente do Paradise City, **não tem restrição de nacionalidade na entrada**.

## Como se Classificar pela Natural8 (A Partir de $3.50)

A Natural8 é a parceira online oficial do APT e roda a campanha de satellites ==**Journey to APT Incheon 2026**==. Dois caminhos pro Main Event:

<div style="background:rgba(255,248,210,0.08);border:1px solid rgba(255,240,180,0.25);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Rota | Vagas GTD | Como Funciona |
|---|---|---|
| **Mega Satellite → Live Day 1** | 52 vagas | Ganhe online → jogue o Main Event ao vivo completo desde o Day 1 |
| **OnLive Day 1** | 22 vagas | Jogue o Day 1 online → se você embolsar fichas, continue ao vivo a partir do Day 3 |

</div>

**Caminho dos step satellites (rota mais barata):**

<div style="background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.10);border-radius:10px;padding:14px 20px;margin:16px 0;font-size:13px;line-height:1.8">
Step 1 (~$3.50) → ganhe o ticket pro Step 2 (~$15) → ganhe o ticket pro Mega Satellite (~$50) → ganhe a vaga no Main Event (valor ~$1.703)
</div>

==g:A Natural8 também roda um bônus de 8% sobre a premiação== para os classificados via satellite — se você ganhar $10.000 no evento ao vivo, recebe $800 extras por cima. Cadastre-se em natural8.com, filtre os torneios por "APT Incheon" pra achar a agenda de satellites atual.

> **Dica OnLive:** a rota OnLive te deixa jogar o Day 1 do seu próprio fuso horário e só voar pra Coreia se você sobreviver até o Day 3. ==g:Se o seu orçamento de viagem está apertado, esse é o caminho de classificação mais esperto.==

**Existe uma terceira rota que quase nenhum guia cita: os Country Exclusives.** A Natural8 roda classificatórios restritos por país de residência que distribuem **seis pacotes live** — e pacote não é só o assento: inclui **a entrada do Main Event (~$1.684) mais cerca de $500 em despesas de viagem**. Satélites normais te dão a vaga e o voo fica por sua conta. A elegibilidade depende do país de residência, então confere no lobby se o seu país tem bracket aberto antes de grindar a rota geral.

---

## Quem realmente joga esse torneio? — O field de 2025 por nacionalidade

Antes de comprar a passagem, vale saber em que mesa você vai sentar. A APT publica a divisão por país, e o Main Event de Incheon 2025 ficou assim:

<div style="overflow-x:auto;margin:20px 0">

| # | País | Entradas | Fatia |
|:---:|:---|---:|:---:|
| 1 | **Japão** | 484 | 37,8% |
| 2 | **China** | 299 | 23,3% |
| 3 | Tailândia | 107 | 8,4% |
| 4 | Hong Kong | 106 | 8,3% |
| 5 | Taiwan | 49 | 3,8% |
| 6 | Rússia | 40 | 3,1% |
| 7 | Singapura | 37 | 2,9% |
| 8 | Vietnã | 29 | 2,3% |
| 9 | Estados Unidos | 26 | 2,0% |
| 10 | Malásia | 21 | 1,6% |

</div>

==r:Japão e China sozinhos são 61% do field.== Foram 27 países representados, mas isso aqui é um torneio **asiático de ponta a ponta** — nenhum país lusófono aparece no top 10. Se você vem do Brasil ou de Portugal, vai ser raridade na mesa.

Na prática isso significa duas coisas. Primeira: na mesa se ouve mais japonês e mandarim do que inglês, e o floor lida com isso com naturalidade. Segunda — e essa joga a seu favor — **ninguém tem read nenhum sobre você**. Você chega sem histórico num circuito onde muitos regs se conhecem há anos.

**Os números de 2025:** 1.718 entradas de **850 jogadores únicos** (o resto foram re-entries), formando um prize pool de **KRW 2.766.660.000 (~$2,0 mi)** — o maior Main Event que a APT já tinha rodado na Coreia até ali. Quem levou foi o chinês **Junjie He**, por ₩380.520.000 (~$275.140) mais um assento do APT Championship, depois de um acordo ICM a três com Kouki Okumiya e Kuroda Kiyoto.

> **Leia o número de entradas direito.** "1.718 entradas" não são 1.718 pessoas: são 850 jogadores mais re-entries. Field com re-entry sempre parece maior do que é.

---

---

## O Local: Paradise City Casino

![Interior do Paradise City Casino — cassino resort de luxo conectado ao Aeroporto Internacional de Incheon com mesas de pôquer, máquinas caça-níqueis e lobby de hotel sofisticado](/images/apt-incheon-2026-guide-venue.webp)

O Paradise City não é um cassino isolado — é um ==**resort integrado construído como parte da expansão do Aeroporto Internacional de Incheon**==, projetado pra capturar o fluxo de trânsito e chegada internacional.

**O que isso significa pra você:**

- ==g:Ônibus shuttle grátis sai a cada 30 min do Terminal 1 (Portões 3C e 14C)== — ~10 min, sem custo, sem problemas com o clima
- Hotel 5 estrelas (Hotel Paradise) no local com acesso direto ao cassino
- Praça de alimentação internacional, spa e complexo de entretenimento
- 45–60 minutos até o centro de Seul via trem AREX a partir do aeroporto

**Regras do local pra você saber:**
- Dress code: sem chinelo, sem pijama
- Sem câmeras ou fotografia dentro do cassino
- ==r:Cidadãos coreanos não podem entrar== — você vai precisar do seu passaporte não coreano na porta
- Idade: 19+ com documento

---

## Como Chegar Lá: Aeroporto de Incheon ao Paradise City

O Paradise City roda o seu próprio transfer gratuito a partir do aeroporto:

| Opção | Trajeto | Custo | Observações |
|:---|:---:|:---|:---|
| ==g:**Shuttle grátis (do T1)**== | ==g:~10 min== | ==g:Grátis== | Melhor opção — Portão 3C ou 14C, a cada 30 min (5h–23h) |
| Shuttle grátis (do T2) | ~20 min | Grátis | Portão 2A, a cada 60 min |
| Táxi do T1 | ~15–20 min | ~$15–20 | Taxímetro, disponível 24/7 |
| AREX até a Estação Unseo → táxi | ~20 min no total | ~$8 | Útil se o horário do trem bater |

Se você está passando por Incheon e tem uma escala longa: ==g:o cassino é acessível durante uma parada de trânsito== — confirme com a sua companhia aérea as exigências de visto pra sua nacionalidade durante o trânsito.

---

## Bate-Volta a Seul: 1 Hora do Paradise City

Aqui está o esquema que a maioria dos jogadores de torneio não sacam até chegar: ==**o Paradise City é a sua base, e Seul fica a só 1 hora**==.

Você pode ficar no hotel do cassino a viagem toda — jogue suas sessões, durma, repita. Ou pode fazer o que a maioria dos regulars faz: terminar a sessão da manhã, pegar o metrô, passar a tarde em Hongdae ou Myeongdong, e voltar pro feltro à noite. O transporte deixa isso totalmente realista.

**Como ir do Paradise City a Seul:**

<div style="overflow-x:auto;margin:14px 0">

| Rota | Destino | Trajeto | Custo |
|:---|:---|:---:|:---:|
| ==g:**Shuttle grátis → T1 → AREX Express**== | ==g:Estação de Seul== | ==g:~53 min== | ==g:~₩11.000 (~$8)== |
| Shuttle grátis → T1 → AREX All-Stop | Hongdae (Univ. Hongik) | ~70 min | ~₩4.750 (~$3.50) |
| Ônibus do Aeroporto 6001 | Myeongdong / City Hall | ~60 min | ~₩17.000 (~$13) |
| Ônibus do Aeroporto 6015 | Gangnam / Yangjae | ~70 min | ~₩17.000 (~$13) |
| Táxi direto | Hongdae / Myeongdong | ~50–60 min | ~$35–50 |

</div>

==g:Duas opções de AREX a partir do T1==: o **Express** (sem paradas até a Estação de Seul em 43 min, ₩11.000 — depois baldeie pra Linha 2 ou 4) ou o **All-Stop** (direto até a Estação Hongdae/Universidade Hongik sem baldeação, ~60 min, ₩4.750). Os trens All-Stop saem a cada 6–10 minutos, das 5h à meia-noite. Os ônibus do aeroporto vão direto pra Myeongdong e Gangnam, se você preferir.

Aqui está o que fazer com 1–3 dias livres:

![Visão geral do Google Maps mostrando o Paradise City em Incheon e a distância até o centro de Seul — aproximadamente 50km a oeste da cidade](/images/apt-incheon-2026-guide-map-wide.webp)

![Google Maps com zoom no centro de Seul com o Namsan (N Seoul Tower) marcado — Hongdae a oeste, Rio Han ao sul](/images/apt-incheon-2026-guide-map-seoul.webp)

### Dia 1 — Seul Antiga: Palácios, Hanoks e Comida de Rua

**Manhã:** comece no ==**Palácio Gyeongbokgung**== (Estação Gyeongbokgung, Linha 3). Construído em 1395, o terreno do palácio é enorme. A entrada custa ~₩3.000 — ou alugue um hanbok (~₩20.000) nas lojas próximas pra ==g:entrar de graça no palácio==. Reserve 2 horas.

**Meio-dia:** ande 15 minutos até a ==**Bukchon Hanok Village**== — um bairro residencial na encosta com mais de 900 casas tradicionais coreanas (hanoks) que datam da dinastia Joseon. Visite entre 10h–17h; ainda é uma área residencial, então espera-se manter o barulho baixo. A vista do topo de Gahoe-dong é o ponto mais fotografado de Seul.

**Noite:** vá pro ==**Mercado Gwangjang**== (Estação Jongno 5-ga, Linha 1) pra jantar. O mercado coberto mais antigo de Seul tem mais de 5.000 barracas — o roteiro gastronômico cobre bindaetteok (panqueca de feijão-mungo), bibimbap, mayak gimbap e sannakji (polvo vivo, opcional). ==g:Uma das melhores experiências gastronômicas da Ásia por menos de $10.==

---

### Dia 2 — Hongdae, Myeongdong e Cheonggyecheon

**Manhã:** ==**Ikseon-dong**== — um conjunto de prédios hanok centenários convertidos em cafés de brunch e cafeterias especiais, a 10 minutos de caminhada do Gyeongbokgung. Vielas estreitas, sem turistas ainda, café bom. Ótimo jeito de começar o dia devagar antes de encher.

**Tarde:** pegue o metrô pro oeste até ==**Hongdae**== (Estação Universidade Hongik — Linha 2, também AREX direto de Incheon). Batizado em homenagem à Universidade Hongik, uma das melhores escolas de arte da Coreia, a área tem três zonas: a ==**rua de pedestres central**== onde grupos de dança de K-pop e músicos de rua se apresentam a partir da tarde; ==**Yeonnam-dong**== ao norte, um conjunto mais tranquilo de cafés independentes ao longo do Gyeongui Line Forest Park (um parque linear construído sobre trilhos de trem antigos); e o ==**distrito de baladas**== entre as Saídas 6 e 9 pra mais tarde. A área toda funciona até o amanhecer — venha pela energia da tarde, fique o quanto quiser.

<div style="display:flex;gap:6px;margin:14px 0">
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-hongdae.webp" alt="Entrada do metrô da Estação Universidade Hongik (Hongdae) — Linhas 2, Airport Express e Gyeongui-Jungang, com bicicletas de aluguel estacionadas do lado de fora" loading="lazy" decoding="async" style="width:100%;height:auto;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Estação Hongdae — AREX All-Stop direto do Aeroporto de Incheon (~60 min, sem baldeação)</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-kbeauty.webp" alt="Loja Olive Young em Hongdae — a maior rede de saúde e beleza da Coreia com milhares de produtos de K-beauty e skincare" loading="lazy" decoding="async" style="width:100%;height:auto;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Olive Young — todas as marcas de K-beauty que você viu no TikTok</p>
  </div>
</div>

**Noite:** metrô até ==**Myeongdong**== (Linha 4, Saída 6). O bairro mais visitado por turistas estrangeiros em Seul — e por um bom motivo. A principal rua de pedestres (Myeongdong-gil) tem cerca de 800m de ponta a ponta: lojas-conceito de K-beauty (Olive Young, Innisfree, Laneige, Sulwhasoo) dos dois lados, barracas de comida de rua preenchendo os espaços a partir das 17h. ==g:Melhor comida de rua: hotteok (panqueca de açúcar mascavo), tteokbokki, corn dogs de queijo, espetinhos de lagosta.== Pra uma refeição de verdade, o ==**Myeongdong Kyoja**== (desde 1966) é o que vale a fila — kalguksu (macarrão cortado à faca) e mandu (guioza). A maioria das lojas fecha por volta das 22h; o ônibus do aeroporto de volta pra Incheon para ==g:bem em frente à Estação Myeongdong==. ==g:Leve o seu passaporte — as lojas de K-beauty oferecem reembolso de impostos na hora.==

<div style="display:flex;gap:6px;margin:14px 0">
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-myeongdong1.webp" alt="Fachada do restaurante Myeongdong Kyoja — o spot mais famoso de kalguksu e mandu de Seul desde 1966, sempre lotado de turistas e locais" loading="lazy" decoding="async" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Myeongdong Kyoja — macarrão cortado à faca desde 1966, sempre com fila</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-myeongdong2.webp" alt="Ponto de ônibus do aeroporto na Estação Myeongdong — serviço direto de ônibus pro Aeroporto de Incheon a partir do centro de Seul" loading="lazy" decoding="async" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Ponto de ônibus do aeroporto bem na Estação Myeongdong → Incheon ~60 min</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-myeongdong3.webp" alt="Rua comercial de Myeongdong repleta de lojas coloridas de K-beauty e farmácias — Holika Holika visível à direita" loading="lazy" decoding="async" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Myeongdong-gil — 800m de K-beauty, comida de rua e letreiros de neon</p>
  </div>
</div>

**Caminhada noturna:** de Myeongdong, ande pro norte por uns 10 minutos e você desce até o ==**Riacho Cheonggyecheon**== (청계천). Um curso d'água urbano de 11km que atravessa o coração do centro de Seul — soterrado sob uma via expressa depois da Guerra da Coreia, escavado e restaurado em 2005. O caminho fica abaixo do nível da rua, ladeado por muros de pedra, salgueiros e sombra de verão. ==g:Garças fazem ninho na água.== À noite, as instalações de luz se refletem na superfície e os locais sentam nas margens com bebidas de loja de conveniência. As saídas de metrô aparecem a cada poucos minutos ao longo de todo o caminho, então você pode caminhar o quanto quiser e emergir em qualquer ponto.

<div style="display:flex;gap:6px;margin:14px 0">
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-cheonggyecheon1.webp" alt="Vista ampla do Riacho Cheonggyecheon — passarela elevada e pedras de travessia com os arranha-céus do centro de Seul visíveis atrás da linha das árvores" loading="lazy" decoding="async" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Riacho de 11km atravessando o centro de Seul — grátis, 24h</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-cheonggyecheon2.webp" alt="Close do Riacho Cheonggyecheon — garça branca em pé na água corrente, casal sentado nas pedras à beira" loading="lazy" decoding="async" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Garças fazendo ninho no riacho — no meio de uma capital</p>
  </div>
  <div style="flex:1;min-width:0;text-align:center">
    <img src="/images/apt-incheon-2026-guide-cheonggyecheon3.webp" alt="Trilha de caminhada do Riacho Cheonggyecheon — salgueiros-chorões ao lado do riacho, pedestres passeando pela trilha à beira-rio" loading="lazy" decoding="async" style="width:100%;height:200px;object-fit:cover;border-radius:6px" />
    <p style="font-size:11px;color:#888;margin:4px 0 0">Saídas de metrô a cada poucos minutos ao longo do caminho — caminhe até onde quiser</p>
  </div>
</div>

**Alta madrugada:** churrasco coreano. Sente numa mesa com uma grelha a carvão embutida, peça samgyeopsal (barriga de porco grelhada), enrole numa folha de alface com alho e pasta ssamjang, acompanhe com soju. A maioria dos restaurantes de churrasco coreano funciona até as 2h.

---

### Dia 3 — Seul Moderna: Gangnam, Seongsu e o Rio Han

**Manhã:** pegue a Linha 2 até ==**Gangnam**==. A Gangnam de verdade não é o que a música descreveu — é o distrito sofisticado de negócios e entretenimento de Seul, lar do COEX Mall (cidade subterrânea), do Templo Budista Bongeunsa e de algumas das melhores cafeterias do país.

**Tarde:** ==**Seongsu-dong**== ("o Brooklyn de Seul") fica a 10 minutos de táxi de Gangnam. Antigos galpões industriais convertidos em cafés especiais, estúdios de design e concept stores. Virou o bairro mais instagramado de Seul nos últimos dois anos. Ótima parada de tarde antes de voltar rumo a Incheon.

**Noite:** se o seu voo é na manhã seguinte, se posicione em ==**Itaewon**== ou perto da ==**Estação Universidade Hongik**== pra uma última noite de rolê — os dois bairros têm bares e restaurantes internacionais abertos até 3h–5h. Depois, AREX de volta pro aeroporto.

---

## Orçamento da Coreia para a Viagem de Pôquer

<div style="overflow-x:auto;margin:20px 0">

| Item | Faixa de Orçamento |
|:---|:---:|
| Passagens ida e volta (de hubs asiáticos importantes) | $200–$600 |
| Passagens ida e volta (dos EUA/Europa) | $700–$1.400 |
| Hotel perto do torneio (Paradise City) | $150–$350/noite |
| Hotel em Seul (intermediário) | $60–$120/noite |
| Refeições diárias (comida de rua + restaurantes) | $20–$40/dia |
| Transporte em Seul (cartão T-money) | $5–$10/dia |
| Buy-in do APT Main Event | ==**~$1.703**== |
| Side events baratos (Mini Main, Ultra Stack) | ==g:**$270–$610**== |
| **Total estimado de uma viagem de pôquer de 7 dias** | ==**$2.500–$5.000 + buy-ins de pôquer**== |

</div>

Comparado ao EPT Barcelona (Main ~€5.300) ou ao WPT Australia (Main ~AUD $5.000), ==g:o APT Incheon te dá um torneio com $1M GTD por um buy-in abaixo de $2.000== — e com uma das melhores cidades da Ásia como cenário.

---

## Exigências de Entrada: Vistos e Acesso ao Cassino

**Visto pra Coreia:**

| Nacionalidade | Precisa de visto? | Como entrar |
|---|---|---|
| EUA, Canadá, Reino Unido, Austrália, Nova Zelândia, maioria da UE | **Sem visto** | K-ETA (gratuito, online, leva 5 min) |
| Japão, Singapura, Hong Kong, Taiwan | **Sem visto** | K-ETA |
| Maioria dos outros países | Confira [mofa.go.kr](https://www.mofa.go.kr) | Pode precisar de visto de turista |

==r:Solicite o K-ETA antes de fechar as passagens.== Costuma ser instantâneo, mas confira com 2 semanas de antecedência em [k-eta.go.kr](https://www.k-eta.go.kr).

**Entrada no cassino (Paradise City):**
- Passaporte não coreano válido obrigatório
- Idade 19+
- ==r:Cidadãos coreanos são legalmente barrados — sem exceções==
- Coreanos no exterior (residentes permanentes fora do país) podem entrar com passaporte coreano + green card ou certificado de emigração

---

:::readnext[Continue lendo]
/pt/blog/holdem-tournament | Guia de Torneio de Texas Hold'em | /images/holdem-tournament-hero.webp
/pt/blog/korea-poker-marathon-2026 | Korea Poker Marathon 2026 | /images/korea-poker-marathon-2026-hero.webp
:::

## FAQ

**Q. O Paradise City Casino é mesmo caminhável a partir do Aeroporto de Incheon?**

A. Bem pertinho. O Paradise City roda um shuttle grátis a partir do Terminal 1 (Portões 3C e 14C) a cada 30 minutos — a viagem leva cerca de 10 minutos. O Terminal 2 tem o seu próprio shuttle a cada 60 minutos a partir do Portão 2A. O resort é um complexo integrado adjacente ao aeroporto, projetado especificamente pra chegadas internacionais.

**Q. Posso jogar o Main Event do APT Incheon se nunca joguei um torneio ao vivo?**

A. Sim. O buy-in (~$1.703) é considerável, mas a estrutura é amigável pra iniciante — stacks iniciais profundos e níveis longos. Se você se vira bem em home games ou pôquer online, a transição pro ao vivo é administrável. O Mini Main Event (~$610, KRW 900K) e o Ultra Stack (~$475) são opções de stakes mais baixos pra começar.

**Q. Como vou do Paradise City a Seul?**

A. Pegue o shuttle grátis de volta pro Terminal 1 do Aeroporto de Incheon, e depois embarque no AREX. Duas opções: **Express** (sem paradas até a Estação de Seul, ~43 min, ₩11.000) ou **All-Stop** (direto até a Estação Hongdae/Universidade Hongik sem baldeação, ~60 min, ₩4.750). O All-Stop é mais barato e te deixa em Hongdae sem nenhuma baldeação.

**Q. Preciso falar coreano pra me virar em Seul?**

A. Não. A sinalização do metrô de Seul é totalmente bilíngue (coreano/inglês), o Google Maps funciona com precisão pela cidade toda, e a maioria dos negócios voltados pra turistas tem funcionários que falam inglês. O app Papago (o tradutor da Naver) resolve bem cardápios e placas de rua.

**Q. Qual o melhor momento pra encaixar os dias em Seul — antes ou depois do torneio?**

A. Os dois funcionam, mas ==g:é recomendado chegar 2–3 dias antes==. Você se recupera do jet lag, explora a cidade num ritmo tranquilo, e chega no Day 1 descansado e aclimatado. Depois do torneio, qualquer dia de bustout vira um dia livre em Seul automaticamente.

---

## Os Pontos-Chave

1. **APT Incheon 2026** rola de ==**7 a 16 de ago**== no Paradise City Casino — 10 min do Aeroporto de Incheon de shuttle grátis, $2.7M GTD.
2. **Main Event** buy-in: ==**~$1.703**== com ==g:$1M garantido==. Cerca de um terço do Main do EPT Barcelona e metade do buy-in do Championship do WPT Australia.
3. **Classifique a partir de $3.50** pelos step satellites da Natural8. O formato OnLive te deixa começar o Day 1 de casa.
4. **Seul fica a 60 min** e é um dos melhores destinos de viagem da Ásia — ==g:palácios, churrasco, vida noturna e comida de rua imbatível.==
5. **Só estrangeiros** — esse torneio foi feito pra jogadores internacionais. O seu passaporte é o seu cartão de acesso.

Indo pro APT Incheon? O [guia de torneio vs cash game](/pt/blog/holdem-tournament-vs-cash-game "thumb:/images/tournament-table-action.webp") cobre as diferenças estruturais se você está migrando do cash. E pros formatos de pôquer que você vai encontrar além do NLHE, [as ações de aposta no pôquer](/pt/blog/holdem-betting-actions) são uma referência limpa pra ter lido antes do Day 1.

---

## Fontes

Todos os números foram checados contra fontes primárias. **Verificado em: 28 de julho de 2026.**

- **Página oficial da série** — [APT — Incheon, South Korea 2026](https://www.theasianpokertour.com/series/apt-incheon-south-korea-2026) · datas, buy-in e garantido do Main Event #20, estrutura de flights
- **Preview oficial** — [APT Incheon 2026: The Preview](https://www.theasianpokertour.com/news/apt-incheon-2026-the-preview) · garantido total, stack inicial de 40.000, **os nove assentos APTC Taipei e como são divididos**
- **APT Championship 2026** — [Full APTC 2026 Schedule](https://www.theasianpokertour.com/news/apt-announces-full-aptc2026-schedule) · 13–29 de novembro, Taipei; Main Event 23–27 nov, $10.000, $5.000.000 GTD
- **20º aniversário** — [APT Gets Ready to Celebrate 20 Years](https://www.theasianpokertour.com/news/apt-2026-schedule)
- **Satélites e pacotes** — [Natural8 — Journey to APT Incheon 2026](https://www.natural8.com/en/blog/journey-to-apt-incheon-2026) · [Country Exclusives 2026](https://www.natural8.com/en/poker/tournaments/apt-incheon-country-exclusives-2026)
- **Field e resultado 2025 (relatório oficial do organizador)** — [APT — China's Junjie He Wins APT's Largest Korean Main Event](https://www.theasianpokertour.com/news/china-s-junjie-he-wins-apt-s-largest-korean-main-event-for-krw-380-5-m-usd-275-k-plus-apt-championship-seat-worth-krw-13-8-m-usd-10-k) · 1.718 entradas de 850 jogadores · prize pool KRW 2.766.660.000 · divisão por país
- **Regras de entrada no cassino** — [Paradise City oficial](https://www.p-city.com/front/contents/casino/casinoInfo)

> ⚠️ **O festival começa em 7 de agosto.** Número de assentos dos satélites, prazos de late reg e horários do shuttle ainda podem mudar. **Confere de novo a página da APT e o lobby da Natural8 no dia em que for se registrar.**
>
> 📌 **Critério de fontes:** calendário, buy-ins, número de assentos e regras vêm do organizador (APT), do parceiro online (Natural8) e do próprio cassino. Mídia especializada é usada só para o que esses três não publicam. Rankings de afiliados não são citados.

---

## Posts relacionados

<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/pt/blog/holdem-tournament" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--primary);margin-bottom:6px">Básico de Torneio</div>
    <div style="font-size:14px;font-weight:700;color:var(--foreground);line-height:1.4">Como Funcionam os Torneios de Pôquer</div>
    <div style="font-size:12px;color:var(--muted-foreground);margin-top:4px">Buy-ins, blinds, satellites e checklist de estreante</div>
  </a>
  <a href="/pt/blog/holdem-tournament-vs-cash-game" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--primary);margin-bottom:6px">Estratégia</div>
    <div style="font-size:14px;font-weight:700;color:var(--foreground);line-height:1.4">Torneio vs Cash Game</div>
    <div style="font-size:12px;color:var(--muted-foreground);margin-top:4px">Como a estratégia muda quando o ICM entra em jogo</div>
  </a>
  <a href="/pt/blog/wpt-australia-2026-guide" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--primary);margin-bottom:6px">Ásia-Pacífico</div>
    <div style="font-size:14px;font-weight:700;color:var(--foreground);line-height:1.4">Guia do WPT Australia 2026</div>
    <div style="font-size:12px;color:var(--muted-foreground);margin-top:4px">The Star Sydney — 10–30 de set, Championship AUD $5.000</div>
  </a>
  <a href="/pt/blog/ept-barcelona-2026-guide" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--primary);margin-bottom:6px">Europa</div>
    <div style="font-size:14px;font-weight:700;color:var(--foreground);line-height:1.4">Guia do EPT Barcelona 2026</div>
    <div style="font-size:12px;color:var(--muted-foreground);margin-top:4px">Casino Barcelona — 16–29 de ago, Main Event €5.300</div>
  </a>
</div>
`.trim(),
};

export default POST;
