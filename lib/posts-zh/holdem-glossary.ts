...
export const POST: Post = {
  ...
  seoTitle: "从 nuts 到 fish——看得懂的德州扑克术语大全",
  ...
  tags: ["扑克术语", "德州扑克术语", "德州扑克术语英文", "德州扑克术语大全", "德州扑克意思", "扑克术语英文", "德州扑克名词", "扑克 是什么意思"],
  content: `
我第一次坐进现场局的时候，整桌人简直像在说另一种语言。有人说自己在“枪口位”，另一个人“3bet 了关煞位（CO）”，荷官问我要不要“run it twice（分两次发）”，等我拿着一对 K 输牌，别人还告诉我“这都算不上 bad beat，只是个 [cooler](/zh/blog/holdem-cooler "thumb:/images/holdem-cooler-hero.webp")”。我像听懂了一样点头。其实我一句都没懂。

扑克有它自己的一套词，懂这套词能帮你两件事：让你看起来不像个 [fish](/zh/blog/holdem-fish "thumb:/images/holdem-fish-hero.webp")，也让你真正跟得上那些能帮你赢钱的策略。这份术语表收的是德州扑克牌桌上真会冒出来的词——按 ==g:你实际会怎么遇到它们== 来分组，而不是按 A 到 Z 堆成一面看不完的墙。先看大家最爱搞混的那几个词，再按你需要的分类翻。凡是有完整指南的术语，你都能找到一条直达链接。

新马、港澳牌桌上多直接喊英文原词（fold/call/raise/all-in），下面的中文译名便于理解，实战中喊动作还是以英文为主。
...
> **快速回答**
> 最容易搞混的一共 8 个：过牌 vs 跟注、盲注 vs 前注、set vs trips、cooler vs bad beat、价值下注 vs 诈唬、底池赔率 vs 隐含赔率、VPIP vs PFR，再加上 3bet 的数法。每个都用一句话分清，再看说错动作的后果。
...
上面那组是“概念”层面的混淆。下面这几对更接地气——是新手在牌桌上**动作**和**叫牌**时真会说错、说错就当场吃亏的配对：

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| 这对别说反了 | 用错会怎样 |
|:---|:---|
| **过牌（Check） vs 跟注（Call）** | 你不需要再补筹码跟上下注时才能过牌；如果还需要补筹码，想留在牌里就得跟注或加注。**面对下注还喊“过牌”，你就只剩跟注或弃牌，不能再加注**。 |
| **下注（Bet） vs 加注（Raise）** | 这一轮**第一个**投筹码叫下注；前面已经有人下了、你再往上加才叫加注。**轮到你时的口头声明是有约束力的，而且先说的算**——未先声明加注时，单枚大筹码及符合多筹码跟注规则的投入先按跟注处理；其余非全下的超额投入，看的是**超过跟注额的部分**：不到最小加注增量的一半算跟注，达到一半但不足完整增量就须补足最小加注。 |
| **set（暗三条） vs trips（明三条）** | 两者牌力一样，但 **set 用一对口袋对子撞中公共牌**、隐蔽性极强，**trips 是你一张底牌配上公共牌上明摆着的一对**、对手一眼看得见。把 trips 当 set 去价值下注，常被对方拿更好的踢脚反杀。 |
| **关煞位（Cutoff / CO） vs 按钮位（Button / BTN）** | 按钮位是**庄家按钮所在的那个座位**、翻后**最后**行动，是全桌最好的座位；CO 是按钮位**右边**、次好的座位。把 CO 当按钮位去偷盲、结果按钮位还没弃牌，你就成了无位置的那个。 |
...
## 下注动作都是什么意思？过牌、下注、跟注、加注、弃牌

> **快速回答**
> 轮到你时的基本动作只有 5 个：过牌、下注、跟注、加注、弃牌。all in 是把筹码一次推光，它可以是下注、跟注也可以是加注；3bet、4bet 是再加注，c-bet 则是翻前加注者在翻牌圈的持续下注——名字再多，底层动作还是这 5 个。

这一组讲的是轮到你时能做的每一个动作。要是你是全新的新手，先从 [下注顺序](/zh/blog/holdem-betting-actions "thumb:/images/holdem-betting-actions-hero.webp") 看起。

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| 术语 | 意思 |
|:---|:---|
| **过牌（Check）** | 不下注就把行动权让出去——只有你不需要再补筹码跟上下注时才可以过。 |
...
| **隔离加注（Isolation raise / iso）** | 在有人平跟之后加注，把底池打成单挑——把那个弱牌手隔离出来单独对付。 |
...
## 牌桌位置都是什么意思？枪口位、劫持位（HJ）、关煞位（CO）、按钮位

> **快速回答**
> 位置指的是你相对庄家按钮的座位，不是椅子号，每一手都顺时针挪一格。按钮位翻后最后行动、全桌最好；枪口位翻前第一个说话、要打得最紧。从早到晚依次是枪口位、Lojack（LJ）、劫持位（HJ）、关煞位（CO）、按钮位（BTN），再加小盲和大盲。

这一组讲的是你坐在哪儿，以及它决定了你什么时候行动——而最后行动是一种永久的优势。至于怎么真正用好它，见 [位置打法](/zh/blog/holdem-position-play)。
...
| **小盲（Small blind / SB）** | 三人或以上时，在按钮位左边交强制注，翻后第一个行动（翻后最差的座位）。单挑时小盲就在按钮位，翻后最后行动。 |
| **大盲（Big blind / BB）** | 两个盲注中较大的那个；级别就是按盲注大小命名的（$1/$2），而一个大盲是衡量筹码量的标准单位。 |
| **枪口位（UTG）** | 翻前第一个行动——需要最紧的开池范围。 |
| **关煞位（Cutoff / CO）** | 按钮位右边；第二好的座位，非常适合偷盲。 |
| **劫持位（Hijack / HJ）** | 按钮位右边第二个座位；中位（MP），就在关煞位之前。 |
| **Lojack（LJ）** | 劫持位右边；同样是中位（MP）（叫法会随桌上人数变动）。 |
...
## 牌型和牌面的术语都是什么意思？底牌、公共牌、nuts、听牌

> **快速回答**
> 你的 2 张底牌加 5 张公共牌，从 7 张里挑最好的 5 张，就是你的牌。nuts 是当前牌面下能组出的最强牌；听牌是还没成型、要靠后面的牌补齐的牌；踢脚是牌力相同时用来分胜负的那张边张。

![一张信息图：金色庄家按钮加两张盖着的底牌，绿色桌布上是 K♦ 7♣ 2♠ 的翻牌](/images/holdem-button-dealer-board.webp "牌面和你的底牌组合成你最好的五张牌——大多数扑克词汇讲的正是这个怎么组")

这一组讲牌本身，以及你能用它们组成什么。对各条街的流程还不熟？先从 [游戏顺序](/zh/blog/holdem-game-order) 看起。
...
| **两对（Two pair）** | 两对不同的对子。 |
| **葫芦的俚语（Boat / Full boat）** | **葫芦（full house）** 的俗称——三条加一对（意为“填满了”）。 |
| **四条（Quads）** | **四条（four of a kind）** 的口语说法。 |
...
| **连撞 / 后门（Runner-runner）** | 靠转牌 **和** 河牌连续两张都命中才成的牌——凑成的后门听牌（比如“runner-runner 同花”）。 |
...
## 玩家类型的黑话都是什么意思？fish、shark、nit、whale
...
## 钱和牌局的术语都是什么意思？盲注、买入、抽水、bankroll

> **快速回答**
> 盲注是小盲、大盲两个座位每手必交的强制注；前注也是起底池的强制注，传统上全桌都交、如今锦标赛多由大盲一个座位代交；买入是坐下或报名要付的钱；抽水是牌房从大多数底池抽走的佣金；bankroll 是你留给扑克的全部资金，不是桌上那一摞筹码。

这一组讲筹码、级别，还有两种赛制。最大的分岔是 [现金局 vs 锦标赛](/zh/blog/holdem-tournament-vs-cash-game "thumb:/images/tournament-table-action.webp")。
...
| **前注（Ante）** | 传统上是每个人都交的一小笔强制下注，用来起底池，跟盲注分开算——如今大多数锦标赛改用大盲前注，由一个座位替全桌交。 |
| **底池（Pot）** | 正在争夺的全部筹码。 |
| **边池（Side pot）** | 当一名玩家 all in、其余人继续下注时另建的一个独立底池。 |
| **筹码量（Stack）** | 一名玩家面前的筹码。 |
| **bankroll（资金）** | 你为打扑克整体留出的钱——不是桌上的筹码。 |
| **买入（Buy-in）** | 进入一场牌局或锦标赛所需的金额。 |
| **抽水（Rake）** | 牌房从大多数底池里抽的一份——[完整指南](/zh/blog/holdem-rake)。 |
...
| **无限注 / 限注（No-limit / Limit）** | 无限注（NLH）的首次下注最低为一个大盲，最高到全部筹码（筹码不足一个大盲时可全下）；限注则把每次下注卡死在固定大小。德扑几乎清一色是无限注；PLO 采用的底池限注，把下注或加注的上限设为底池大小。 |
| **PLO** | 底池限注奥马哈（Pot-Limit Omaha）——一种很火的变体，发四张底牌、且必须刚好用其中两张（跟德扑不是同一个游戏，但你会常听人提到它）。 |
| **锦标赛（Tournament）** | 固定买入、盲注递增，一直打到你出局或夺冠。 |
| **深跑（Deep run）** | 在一场锦标赛里挺进得很深——远超钱圈、接近决赛桌的那种成绩。 |
| **无重买赛（Freezeout）** | 一种不能重买的锦标赛——出局就是出局。 |
| **保底奖池（GTD / 保底）** | 锦标赛承诺的最低奖池金额，就算报名人数不够也照付。 |
| **手对手（Hand-for-hand）** | 临近钱圈泡沫时，所有桌一次只打一手牌、同步进行，谁都别想靠拖时间磨进钱圈。 |
...
| **bad beat 大奖池（Bad beat jackpot / JP）** | 当一手极强的牌输掉时派发的一份彩金——[怎么运作](/zh/blog/holdem-bad-beat)。 |

</div>

---

## 情境、数据和礼仪的术语都是什么意思？摊牌、上头、VPIP、range

> **快速回答**
> 这一组词分三类：摊牌、分池、单挑说的是牌局走到了哪一步；VPIP、PFR、EV、range 是衡量打法的数字和模型；上头、slow roll、泼筹码则是牌桌上的情绪和规矩——学会它们，你就听得懂别人在评价谁。

这一组讲牌桌上正在发生什么，以及这期间该怎么表现。
...
| **run it twice（分两次发）** | all in 的玩家把剩下的牌面发两次，每次争一半底池，用来降低波动——只限现金局，且所有相关玩家都得同意。 |
| **单挑（Heads-up）** | 一对一地打——要么是只有两名玩家的对局，要么是锦标赛打到最后两人。 |
| **率先加注（RFI / raise first in）** | 范围和数据用语里对开池加注的简称：前面没人进池时，你率先加注进池的频率有多高。 |
| **泼筹码（Splash the pot）** | 把筹码乱扔进底池、不在自己面前整齐堆放——不被鼓励，因为这样投入的金额没法核对。 |
...
A. 过牌是不投任何筹码就把行动权让出去，而且只有你不需要再补筹码跟上下注时才允许。跟注是跟上某人已经做出的下注。过牌是免费的；跟注要补上你距当前下注还差的金额。把这两个搞混，是新手最常犯的头号错误。
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/zh/blog/holdem-fish" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">术语表</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">fish 是什么？</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">解读各种玩家类型</div>
  </a>
  <a href="/zh/blog/holdem-cooler" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">大家都会混淆的两种输法</div>
  </a>
  <a href="/zh/blog/holdem-betting-actions" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">下注动作</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">过牌、下注、跟注、加注、弃牌</div>
  </a>
  <a href="/zh/blog/holdem-hand-rankings" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">谁大谁小</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">完整的牌型大小顺序</div>
  </a>
</div>
`.trim(),
};
