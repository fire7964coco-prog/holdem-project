...
#### ▶ 🔴 신규 미결 — **다국어 39편이 히어로를 두 번 그린다** (2026-08-31 · 사장님 발견)

사장님이 라이브에서 「히어로가 바로 아래에 또 있다」고 지적 → 내 글은 고쳤고(`9f4d0f2d`),
전수 조사하니 **39편이 더 같은 상태**다(산출물의 `<img …-hero.webp…>` 태그가 2개 · **오탐 0**).
...
**처방**: 각 글 `content`의 자기 히어로 마크다운 이미지 1줄 삭제. 기계적·저위험.
⚠ `holdem-game-order`는 6회차 색인 관측 중이지만 **메타를 안 건드리므로 관측과 무관**하다.
🔴 **사장님 판정 대기** — 39편 일괄 스윕할지.

🪶 **함께 판정할 것(별건)**: 다국어 글은 content에 히어로가 없어 **og:image가 «첫 본문 이미지»**가 된다
(`getBlogLcpInfo` — KO·다국어 공통). 본문 이미지가 없으면 `/opengraph.jpg`로 떨어진다
(예: `zh-hant/holdem-tournament`). **og를 `post.image`로 고정할지**는 472편에 영향이라 별도 판정.
