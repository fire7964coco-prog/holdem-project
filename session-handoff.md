...
### 🟢 직전 회차 (2026-09-10 (5)(6) · Opus 헤드 + Fable 집필 구간)
`35446d45`→`5e37d976` 배포·라이브 확인(Playwright) · 워킹트리 깨끗. **경위 = WORKLOG 「2026-09-10 (5)(6)」.**
요지만: **APT 제주 135 라이브 재실측 45자리 전파** · **결재 2-ⓐ 8항목 25로케일 전수 판정(잔존 1)** ·
**`check:drift` 핵심 21 → 🟢 0 완주**(de·id·pt 14편은 스탬프만 지연이던 «가짜 부채» · ar 7편은 실제 이식) ·
**아랍어 GPT 교차 렌즈 최초 가동**(14건 재판정 = ar 2 · EN-먼저 11 · 기각 3) · **발신함 자가삼킴 사고 복구**.
...
| 🔴 **EN-먼저 11건 — ar GPT 교차가 올린 규칙 클러스터**(`docs/en-first-queue.md` **§2-A** · 룰북 축어 동봉). §13급 4 = 누적 재개방 «전원» 일반화(TDA §47A) · cards-speak FAQ 자기모순(TDA §14) · 올인 전원공개를 캐시까지 일반화(LA §149) · WSOP §117 룰북 미구분. 🔴 **EN을 고치면 8로케일 전파가 따라온다** — 회차를 따로 열어라 | 중~대 |
...
### 솔버 랜딩·앱
- 🟢 **아랍어 원어민 렌즈 — 1회 실시했다(2026-09-10 · GPT 교차 팩 2개).** 조어 의심 4건 중 **셋이 해소**됐다:
  ⓐ `الورقة المحروقة`는 **조어가 아니다**(GGPoker ar 표제 `ورقة محروقة` · 대체형 `بطاقة الحرق`) — 되살리지 마라.
  ⓑ `Ratholing` 정의는 EN과 대응(설명+원어 병기) ⓒ board 역어 3계열이나 **사실을 바꾸는 오용은 없다**(퍼즐도 `الطاولة (5 أوراق)`로 문맥 유지).
  🟠 **남은 것 둘**: ⓓ FAQ 어형의 **실검색 빈도 미실측**(ar은 키워드 실측 기록이 아예 없다 · 자동완성 실측 필요) ·
  🪶 board 역어 **통일 여부는 문안 판단**(결함이 아니므로 급하지 않다). 🔴 ar 팩은 **규칙 7편만** 봤다 — `holdem-tournament-vs-cash-game` 외 나머지는 다음 렌즈에.
...
🟢 **ⓖ(TDA 연도) 종결** — 2024 v1.0을 취득해 2022와 11개 조항 전수 대조, **번호가 안 밀렸다** → 연도 불요.
🔴 **대신 반대 규율이 생겼다: WSOP는 룰북이 둘이고 같은 번호가 다른 조항이다**(§117 = 토너 「카드 노출」 ↔ 캐시 「결함 있는 덱」 · §104도 다르다).
「WSOP Rule N」 인용에는 **어느 룰북인지** 반드시 붙인다. 근거·전문 = `docs/sources/README.md` + 룰북 3종 사본.
