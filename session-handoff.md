...
> 마지막 갱신 2026-09-17 (본체 HEAD = pt 계산기 회차 커밋 `73f2fbea` + 마감 문서 · `/pt/calculator` 재조준 배포(Equity 탭·quickRef 6표·FAQ 18(TDA 2024 Rule 5 고유)·브라질식 숫자 · MB-062) · 다음 = **de 계산기 회차** · 미커밋 0).

## ▶ 다음 할 일 (사장님이 고른다)

1. 🧮 **de 계산기 랜딩 회차 — 첫 마디 「핸드오프 읽고 de 계산기 회차 시작해」** (🔴 **Opus 창에서 연다** · 모델 분담 = 브리프 §3-D(pt도 Fable 없이 완주 — 헤드·집필·렌즈 3종+2차 교열 Opus · 구조 추출 sonnet 2 · 전사 = `check:calc-parity`). 절차 = `docs/harden-brief/calculator-landing-rewrite.md` **§3-A**를 de로 + **§3-B·§3-C·§3-E·§3-F·§3-G가 남긴 것**(🔴 `npm run check:calc-parity -- de`를 렌즈 전에 — **소수 콤마 로케일은 이제 게이트가 정규화**(`numL`) · 🔴 **숫자 표기는 de 코퍼스를 세고 정한다**(pt는 콤마 1,637 : 0) · 🔴 **ICM 약어가 무엇에 덮이는지 자동완성으로 먼저**(es = IMC · pt = ICMS 세금까지 · de «ICM Rechner»?) · 딜 명사 오염도 같이 · Monte Carlo «run» 역어가 게임 용어(라운드)와 겹치는지 · 탭 이름≠페이지 이름 · 방위어 금지 · FAQ 질문 뒤 명사 덧붙이기 금지 · 제목 앞머리 = 실측 머리어(CTA «Poker-Odds-Rechner»는 볼륨 확인) · 영문 «poker odds calculator»가 자국어와 같은 체급일 수 있다 · board 역어 충돌 · 형제 글 FAQ **+ H2까지** 의도 grep · captionMw 관사 금지 · «call LP/EP»는 «레이즈에 콜» 명시 · 오버카드 정의는 «보드보다 높은 카드» · fold equity 역어는 코퍼스 grep · TDA 2024판). 권역 = DACH(`docs/local-voice/de-dach.md` · `docs/keyword-bank/de-core-volumes.md` · 볼륨 location = Germany 2276). 🔴 합법성 축을 열지 마라. 선례 = `ja/zh/zh-hant/es/pt-calculator.md` · EN 정본 = `en-calculator.md`. 수치 EN 그대로(§13) · 정의형 FAQ 금지 · 딜러 렌즈 12항 + §5 누적 회灌 동반. 끝나면 §0 de ✅ + 다음 로케일(fr)을 이 자리에.
