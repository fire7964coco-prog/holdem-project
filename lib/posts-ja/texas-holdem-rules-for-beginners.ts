...
> **先に結論**
> 特に断りがなければ**ノーリミット**です。カードの配り方・ブラインド・役は3形式とも同じで、違うのは**一度にいくらベットできるか**だけ — ノーリミットはビッグブラインド以上から全チップまで、フィックスドリミットは決まった刻みだけ、ポットリミットは現在のポットの額まで。ノーリミットは理解しやすい分、1回のミスで全部失える形式でもあります。
...
初心者は ==g:特に断りがなければノーリミットだと思ってください。== ノーリミットは *理解する* のは最も簡単（ビッグブラインド以上なら好きなだけベットできる）ですが、*下手にプレイする* と最も高くつきます。たった1回の間違ったコールでスタック全部を失えるからです。だからこそ、後述のポットオッズと [ベットアクション — チェック・コール・レイズ・フォールド](/ja/blog/holdem-betting-actions) が重要になります。
...
> **先に結論**
> 最初の目標は「勝つ」ではなく**チップを寄付しない**ことです。そのために守るのは5つ — 弱いハンドの参加を減らす、強いハンドはレイズする、アーリーポジションでは多めに降りる、リバーの大きなベットは信じる、ドローを何でも追わない。手札は**ペアはTT以上、それにAKとAQ**（上位約5%）から始めれば十分です。
...
> **先に結論**
> やってはいけない振る舞いを教わらないまま席に着く人がほとんどです。TDAルール70が「エチケット違反」として挙げ、ルール71で罰則対象にする例は8つ——遅延、他人の体・カード・チップに触る、順番無視、カード・チップを見えにくくする、ディーラーに届かない場所へのベット、暴言、不快な身だしなみ、過度なおしゃべり。避ければ初めてでも浮きません。
