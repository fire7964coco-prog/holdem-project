...
A. 不会——但德州扑克牌桌上确实有一个用到花色的地方，只是跟底池无关。现金桌（以及多数牌室的场规）在牌局开始时每人抽一张牌决定庄家按钮放在哪，两张抽到的牌点数相同时，就靠花色顺序定。（WSOP 锦标赛不抽牌：==第 85 条==规定按钮从荷官右手边第一个筹码堆开始，只在剩 3、2、1 桌时才重新抽签——和花色无关。）不管哪种，那选的都是一个*座位*，从来不是一手牌。锦标赛规则书里唯一用到花色排序的地方，全都属于梭哈和 razz。两手最佳五张点数一张对一张全相同，无论花色如何都平分底池。
