...
A. 可以不亮，直接把牌盖掉（Muck）——前提是你明确输了。**但别指望反悔：只有当牌被荷官推进弃牌堆、再也无法 100% 辨认和取回时，它才成为死牌（dead hand）；在那之前，牌面朝下放着并不会自动作废（TDA 第 14 条）。一旦成了死牌，哪怕它本来能赢，也再拿不回底池。** 赢家亮牌后，输的玩家才可以牌面朝下盖牌；例外有两个：一是你的牌被跟注过——跟注你的那个人花了钱，有权要求荷官翻开查看；二是锦标赛里只要有人全押，按 TDA 第 16 条谁都不能盖牌。只要对谁赢还有任何不确定，就绝不要在荷官读完两手牌之前盖牌。
