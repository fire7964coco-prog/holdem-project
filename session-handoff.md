...
> 커밋 `f3a7b1c` · 작업 트리 깨끗 · **build 67 blog posts** · 라이브 200 확인.
> GTO 솔버 시리즈 **11 / 13 완료**. 나머지는 ⑫⑬ 2편과 마감 작업뿐이다.

## ▶▶ 다음 세션이 할 일 — GTO 시리즈 ⑫⑬ 2편 + 마감

```
1. git pull
2. docs/gto-solver-series-spec.md 통독
     ★§4-B「편별 확정 수치표」가 새로 생겼다 — 형제 글 인용은 전부 여기서만 가져온다
3. docs/keyword-bank/ko-gto-solver.md 통독       ← ★§3-B(2차 실측 100개)가 새로 생겼다
4. node scripts/capture-solver-spots.mjs sb-connected sb-paired-ace
   node scripts/make-solver-range-charts.mjs
   node scripts/convert-solver-captures.mjs sb-connected sb-paired-ace
5. lib/posts/blind-battle-cbet.ts 를 템플릿으로 복사   ← ⑪이 같은 «블라인드전» 조건이라 가장 가깝다
   (팟 6bb · 스택 97bb · SPR 16.2 · OOP = SB 오픈레이저 · 벳 사이즈 33% 한 가지)
6. 편당 §13 손 검산 → audit:hard --slug → --uncovered
7. 적대적 검수 4렌즈 병렬(Agent 툴) + ★2차 교열 패스
8. 빌드·커밋·배포 → 보고
9. ★마감 작업 (spec §6): 체인 닫기(⑬ → ①) · /solver 랜딩에 13편 허브 목록 ·
   솔버 세션에 "파일id: URL" 13줄 전달
```

**★⑫⑬ 착수 전 반드시 알 것**

1. 🔴 **형제 글을 인용할 땐 «카드·무늬»까지 원문에서 복사하라.**
   지난 세션에 수치는 전부 대조했는데 보드 표기를 기억으로 적어 두 건 틀렸다 —
   ①은 `A♥7♦2♣`, ③은 `Q♠J♦T♠`(**투톤**)이다. 무늬가 틀리면 「드라이/투톤」 판정까지 따라 틀린다.
   적대검수 **검산 렌즈가 단독으로** 잡았다.
2. 🔴 **범위 서술("①~⑦에서 96~99.9% 체크")을 쓰기 전에 전 편 값을 실제로 훑어라.**
   ④는 76.2%, ⑤는 88.8%였다. 세 렌즈가 동시에 잡았다.
   → **1·2번을 막으려고 `docs/gto-solver-series-spec.md` §4-B에 편별 확정 수치표를 만들어 뒀다.**
   보드·무늬·조건·빈도·에퀴티·EQR이 한 표에 있다. **핸드오프에 값을 다시 적지 않는다**(중복이 원인이었다).
3. **3벳팟 3편(⑧⑨⑩)의 BB 레인지는 14종 고정**이고 보드 블로커를 반영하면 콤보가 정수로 정확히 맞는다
   (63 / 73 / 83). 손 검산의 강력한 무기다. **단 ⑫⑬은 블라인드전이라 538·525콤보로 넓다 —
   콤보 분해를 억지로 시도하지 마라.**
4. **⑫⑬ 키워드**: 「홀덤 포지션」 390 · 「홀덤 포지션별 핸드레인지」 70 · 「홀덤 트립스」 10 ·
   「홀덤 블러프」 10. ⑪이 「홀덤 포지션」을 이미 잡았으니 **⑫는 롱테일 쪽으로 갈라라.**
5. **seoTitle에 GTO 필수.** 🔴 **「씨벳」(월 6,600) 절대 금지** — 베팅 사이트 브랜드다.
   키워드는 볼륨만 보지 말고 **자동완성으로 의도까지** 확인한다.
6. ⚠ **PowerShell로 한글 문서를 일괄 치환하지 마라.** 지난 세션에 `Get-Content -Raw` + `Set-Content`로
   spec 문서 한글을 통째로 깨뜨렸다(git으로 복구). **문서 편집은 Edit 도구.** CLAUDE.md §16 참조.

---

## ▶▶ 시리즈와 별개로 남아 있는 것 (미착수 · 우선순위 순)

1. 🔴 **`/solver` 랜딩이 「무료」를 안 노린다.** 자동완성이 「홀덤 솔버 → 사용법·다운로드·**무료**·추천」인데
   랜딩 메타에 «무료»가 없다. 우리 솔버는 무료·설치 불필요·회원가입 없음이다. **가장 값싼 개선.**
   대상: 「홀덤 gto 프로그램」140 · 「홀덤 gto 계산기」20 · 「gto 솔버 무료」10 · 「홀덤 솔버 무료」
2. 🆕 **「홀덤 gto 표」 210 — 이 판 2위 키워드인데 아무 글도 안 노린다.**
   의도는 «프리플랍 레인지 차트를 달라»에 가까워 이 시리즈(플랍 전략)와 어긋난다.
   「홀덤 gto 차트」20 + 「홀덤 레인지 차트」40과 묶으면 270. **별도 글 또는 `/solver` 랜딩이 받아야 한다.**
3. **「gto wizard」 월 4,400** — 브랜드 키워드라 직접 조준은 사장님 판단 필요.
