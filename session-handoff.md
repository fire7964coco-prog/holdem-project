...
> 드리프트 **90** · 추적불가 84 (✅ 236)
> ⚠ **드리프트 82 → 90(+8)의 정체** — FAQ 스키마 복구로 EN `texas-…-beginners`·`game-order`의
> `updated`를 08-10으로 올린 결과다. **미러의 masterUpdated는 일부러 안 올렸다** —
> 이번 변경은 형식뿐이고 `texas-…-beginners`의 **내용 재동기화는 여전히 미결**이라,
> 여기서 올리면 진짜 드리프트가 숨는다. **90은 지금 옳은 숫자다.**
> 「판 전체 재동기화」를 실행할 때 한 번에 정리하면 된다.
...
>
> ### ✅ 다국어 FAQ 스키마 — **완결.** 게이트 신설 + 25편 전건 복구
> `npm run check:faq-schema`(셀프테스트 **16/16**) 신설 → **455/455편 · 25개 로케일 전부 🔴 0건.**
> EN 마스터 2편 + 번역본 23편, 빌드 산출물로 전건 확인. 경위 = `docs/faq-schema-audit-2026-08-10.md`.
> ⚠ 수치 이력: 41(**오보** — `.ts` 원본을 CRLF째로 훑었다) → 25(실측) → 0.
> 🔴 **masterUpdated는 일부러 안 올렸다** — 형식만 고쳤고 `texas-…-beginners`는 「판 전체 재동기화」가
> 미결이다. 드리프트 90은 「내용이 아직 안 왔다」를 정확히 말하고 있으니 **그대로 두는 게 맞다.**
