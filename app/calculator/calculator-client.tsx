...
const HAND_TABLE: TierEntry[] = [
  ...
  [1, "1010", "JJ 이상 나오면 밸류 낮아짐", "모든 포지션에서 레이즈 — 오픈 림프 금지"],
  [1, "AKs", "최강 드로우 핸드. 팟을 키워라", "항상 레이즈/리레이즈 가능"],
  [1, "AKo", "AKs보다 약하지만 여전히 프리미엄", "항상 레이즈, 리레이즈 콜 가능"],
  [2, "AQs", "강한 핸드, 포지션 유리할수록 가치↑", "대부분 포지션에서 레이즈"],
  [2, "AJs", "버튼/CO에서 최강, UTG에선 약함", "중후기 레이즈, UTG에선 레이즈 아니면 폴드 — 림프 금지"],
  [2, "A10s", "스윗드 에이스 중 최상위권", "중후기 레이즈, UTG에선 레이즈 아니면 폴드 — 림프 금지"],
  [2, "KQs", "플러시+스트레이트 드로우 잠재력↑", "대부분 포지션에서 레이즈"],
  [2, "KJs", "강한 드로우 핸드", "LP 레이즈, EP에선 레이즈 아니면 폴드 — 림프 금지"],
  [2, "99",  "중간 포켓페어, 오버카드 플랍 주의", "대부분 포지션 레이즈, 딥스택 주의"],
  [2, "88",  "드로우 잠재력 좋은 포켓페어", "대부분 포지션 레이즈, 레이즈에 콜 — 오픈 림프 금지"],
  [2, "AQo", "오프수트라 약화, 포지션 중요", "중후기 레이즈, UTG에선 레이즈 아니면 폴드 — 림프 금지"],
  [3, "AJo", "EP에서는 약함, LP에서 강함", "CO/BTN 레이즈, EP 조심"],
  [3, "KQo", "오프수트 커넥터 중 최상위", "LP 레이즈, EP에선 아무도 안 들어왔으면 레이즈 아니면 폴드, 레이즈 앞에선 대체로 폴드"],
  [3, "K10s", "스윗드 킹, LP에서 강함", "LP 레이즈, EP 폴드"],
  [3, "QJs", "두-웨이 드로우 강력", "LP 레이즈, 딥스택에서 가치↑"],
  [3, "J10s", "최고의 스윗드 커넥터 중 하나", "LP 레이즈, 포지션 있으면 싱글 레이즈에 콜"],
  [3, "109s", "강한 스윗드 커넥터", "LP에서 첫 진입이면 레이즈, 포지션 있으면 싱글 레이즈에 콜"],
  [3, "77",  "셋 드로우 기대, 오버카드 주의", "LP 레이즈, EP에선 레이즈에 콜, 첫 진입이면 레이즈 아니면 폴드 — 오픈 림프 금지"],
  [3, "A9s", "스윗드 에이스, 플러시 드로우 가치", "LP 레이즈"],
  [3, "K9s", "스윗드 킹, K가 맞아도 더 강한 킹에게 지배될 수 있음", "앞 전원 폴드면 CO/BTN에서 오픈 레이즈, 레이즈 앞에서는 포지션·가격·유효 스택을 함께 판단"],
  [3, "Q9s", "스트레이트 가능성이 있는 스윗드 퀸, 더 강한 퀸에 주의", "앞 전원 폴드면 CO/BTN에서 오픈 레이즈, 레이즈 앞에서는 포지션·가격·유효 스택을 함께 판단"],
  [3, "J9s", "스트레이트·플러시 가능성이 있는 스윗드 원갭 핸드", "앞 전원 폴드면 CO/BTN에서 오픈 레이즈, 레이즈 앞에서는 포지션·가격·유효 스택을 함께 판단"],
  [4, "66",  "셋 드로우 목적, 임플라이드 오즈 필요", "LP에서 첫 진입이면 레이즈, 레이즈 앞에선 나와 레이즈한 상대 둘 다 콜 금액의 15배 이상이 뒤에 남았을 때 콜"],
  [4, "55",  "셋 아니면 가치 낮음", "LP에서 첫 진입이면 레이즈, LP에서 나와 레이즈한 상대 둘 다 콜 금액의 15배 이상이 뒤에 남았을 때 싱글 레이즈에 콜"],
  ...
  [4, "A3s", "휠 카드를 낀 너트 플러시 드로우, A2s보다 한 단계 위", "LP에서만"],
  [4, "A2s", "휠+너트 플러시, 하지만 약함", "LP에서만"],
  [4, "KJo", "도미네이션 위험, LP에서만", "CO/BTN에서 레이즈, 리레이즈엔 폴드"],
  [4, "QJo", "적당한 연결성, 포지션 필요", "LP에서만"],
  [4, "98s", "강한 스윗드 커넥터", "LP에서 첫 진입이면 레이즈, 레이즈 앞에선 포지션이 있을 때 콜"],
  [4, "87s", "좋은 스윗드 커넥터", "LP에서 첫 진입이면 레이즈, 레이즈 앞에선 포지션이 있을 때 콜"],
  [4, "76s", "스윗드 커넥터", "LP에서 첫 진입이면 레이즈, 레이즈 앞에선 포지션과 딥스택이 있을 때만 콜"],
  // 🔴 2026-09-21 — 44·33·22도 Tier 4다. 아래 폴백의 콤보 모델이 「페어 78」 = 13개 페어 **전부**를
  //    버튼 레인지 안으로 세는데 표만 이 셋을 T5로 찍어, 미등재 43s·32s(수티드 → T4)가 44보다 위로 떴다.
  //    수티드에 바닥을 두는 해법은 09-20에 기각됐다 → 페어 승격이 모델과 맞는 쪽이다.
  //    🔴 components/calculator/calculator-tool.tsx의 HAND_TABLE과 같은 값이다 — 한쪽만 고치지 마라.
  //    🔴 배지만 올렸다 — 액션 문구는 그대로다.
  [4, "44",  "셋 없으면 가치 거의 없음", "LP에서 첫 진입이면 레이즈, LP에서 나와 레이즈한 상대 둘 다 콜 금액의 15배 이상이 뒤에 남았을 때 싱글 레이즈에 콜"],
  [4, "33",  "셋 드로우 필요, 도박성↑", "LP에서 첫 진입이면 레이즈, 그 외에는 멀티웨이 팟·딥스택에서만"],
  [4, "22",  "가장 낮은 포켓페어", "LP에서 첫 진입이면 레이즈, 그 외에는 멀티웨이 팟·딥스택에서만"],
  // K10o·Q10o·J10o는 Tier 4다 — 폴백이 계산한 K9o·Q9o·J9o가 Tier 4인데 이들이 Tier 5면
  // 「더 센 핸드가 더 약하게」 뜬다(09-20 딜러 렌즈). 세 핸드 다 BTN 표준 오픈이다.
  [4, "K10o", "K-10 오프수트, 약함", "앞 전원 폴드면 CO/BTN에서 오픈 레이즈"],
  [4, "Q10o", "연결성 낮은 오프수트", "BTN에서만"],
  [4, "J10o", "좋은 오프수트지만 취약", "BTN에서만"],
  [3, "Q10s", "강한 스윗드 브로드웨이, 포지션에서 우수", "LP 레이즈, EP에선 폴드"],
  [4, "A10o", "마지널 오프수트 에이스, 도미네이션 주의", "LP에서만"],
  [4, "65s", "스윗드 커넥터, 멀티웨이/임플라이드 오즈 필요", "LP에서 첫 진입이면 레이즈, 레이즈 앞에선 포지션과 딥스택이 있을 때만 콜"],
  [4, "54s", "낮은 스윗드 커넥터, 투기적", "LP에서 첫 진입이면 레이즈, 레이즈 앞에선 포지션이 있고 싼 멀티웨이 팟일 때만 콜"],
];
...
// 표 밖 119핸드는 폴백 문구 하나를 같이 받지만 세기가 같지 않다 — K8s·A9o는 스틸 오픈이고 72o는 폴드다.
// 전부 Tier 5로 박으면 화면에 「🚫 약함」과 「BTN에서 오픈」이 나란히 뜬다.
// 폴백 문구의 분기와 같은 조건으로 계산한다(rank는 RANKS 인덱스: 1="3", 7="9", 12="A").
// 🔴 components/calculator/calculator-tool.tsx의 fallbackTier와 같은 규칙이다 — 한쪽만 고치지 마라.
//
// 오프수트 줄의 출처. 전제 조건: 9맥스 · ~100bb · 버튼 앞이 전부 폴드.
// 🔴 **총량은 구성을 증명하지 않는다 — 개수만 묶을 뿐이다.** 아래 둘은 «~50%»에서 따라 나오는 결론이
// 아니라 통용 버튼 오픈 차트에서 가져온 «모델 선택»이다: ① 페어 13개 전부(78콤보) ② 수티드 78개
// 전부(312콤보). 그 둘을 두고 나면 1,326의 50% = 663에서 오프수트 몫이 273콤보다. 오프수트는 타입당
// 12콤보라 273은 도달 불가 — 22타입 = 264(합 654 = 49.32%)와 23타입 = 276(합 666 = 50.23%)이
// 663을 양쪽에서 끼운다. 우리는 아래쪽 22타입을 쓴다: 에이스 12 + K9o+ 4 + Q9o+ 3 + J9o+ 2 + 10-9o —
// «오프수트를 세기 순으로 줄 세운다»는 것 역시 총량이 대신 정해 주지 못하는 선택이다.
// 🔴 «50%»라고 적지 마라 — Tier 1~4는 654콤보 = **49.32%**다.
// 🔴 기각한 확장(K5o+/Q8o+/J8o+/98o = 29타입)은 738 = **55.66%**이지 «58%+»가 아니다 —
//    그 수치는 재현 불가라 2026-09-21에 철회했다(MA-159 ③). 네 타입만 더하는 읽기는 26타입 = 702 = 52.94%.
//    넓힐지 말지는 산술이 아니라 판단 자리다 — `docs/en-first-queue.md` §2-H H-3.
// 수티드는 312가 통째로 들어가므로 버튼은 전부 오픈한다. CO 제한은 문구에만 두고 여기선 안 가른다.
function fallbackTier(hi: Card, lo: Card): 4 | 5 {
  if (hi.suit === lo.suit) return 4;              // 같은 무늬는 전부 버튼 오픈
  return hi.rank === 12 || lo.rank >= 7 ? 4 : 5;  // 오프수트 에이스, 또는 두 장 다 9 이상
}
...
function StartingHandCalc() {
  ...
  const result = useMemo(() => {
    ...
    if (!entry) {
      ...
      return {
        name, tier: fallbackTier(hi, lo),
        desc: "기본 오픈 차트에 없는 핸드",
        action: "스윗드(같은 무늬): 앞 전원 폴드면 BTN에서는 어떤 스윗드든 오픈, CO에서는 스윗드 K·두 장 다 8 이상(Q8s·10-8s)·54s까지의 커넥터로 좁힌다. 오프수트: BTN에서만, A를 든 핸드(A9o·A5o)나 두 장 다 9 이상(K9o·10-9o). 나머지(J2o·93o·72o)는 폴드",
      };
    }
          ...
          <motion.div initial={false} animate={{ opacity:1, y:0 }} className="w-full">
            <div className={`rounded-xl border p-5 mb-4 ${tierMeta?.bg}`}>
              ...
              <div className="flex flex-col sm:flex-row items-start gap-1 sm:gap-2 mt-3">
                <span className="shrink-0 whitespace-nowrap text-xs font-bold text-foreground uppercase tracking-wide">추천 액션:</span>
      ...
      <div>
        ...
        <div className="space-y-2">
          {([
            ...
            [3,"AJo KQo K10s Q10s QJs J10s 10-9s 77 A9s K9s Q9s J9s","후기 포지션 레이즈"],
            [4,"66~22 A8s~A2s KJo QJo A10o~J10o 98s~54s","LP에서 선택적"],
            [5,"약한 오프수트(J2o·93o·72o)","대부분 폴드"],
...
const BB_DEPTH_BANDS = [
  { max: 10,       name: "푸시/폴드", range: "≤10",    color: "text-red-400",    bg: "bg-red-500/10 border-red-500/40",
    /* 🔴 「폴드 에퀴티가 남아 있다」를 ≤10 **전 구간**에 걸지 마라 — 5BB 이하에서 거짓이 된다.
          `holdem-bubble-strategy.ts:212` 「폴드 에퀴티가 아직 남아 있는 **10~15BB** 구간이 기준선…
          좋은 패를 기다리다 **5BB가 되면 그때는 어떤 핸드로 올인해도 콜을 받습니다**」.
          숏스택 밴드가 15BB로 이분된 것과 **같은 유형**이다(2026-08-29 (3) 2차 대조에서 검출). */
    desc: "올인 아니면 폴드입니다. 레이즈하고 물러설 칩이 없어요. 10BB 근처면 아직 상대를 접게 만들 수 있지만, 5BB 아래로 내려가면 어떤 핸드로 올인해도 콜을 받습니다." },
