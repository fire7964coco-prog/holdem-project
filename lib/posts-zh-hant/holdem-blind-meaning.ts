...
為什麼一定要逼人先付這筆錢？==g:為了讓牌局活下去。== ==r:沒有盲注的話，每個人都可以一直蓋牌、無止盡地枯等一手大牌==；底池永遠是空的，詐唬也變得毫無意義，整場牌局就停擺了。==g:每手牌兩個強制注，保證檯面上永遠有東西可以爭。==
...
兩個劣勢會疊在一起：牌是隨機發的，位置又永遠最差——翻牌之後每一條街，你都得在資訊最少的時候先開口。所以在小盲，正確的目標是把每一手的損失壓小，而不是想著在這個座位上贏錢。
...
**大盲注（Big Blind, BB）** 由 **小盲左邊** 緊挨著的玩家下。那個[過牌](/zh-hant/blog/holdem-betting-actions)的權利之所以值錢，是因為你已經把一個完整注放進底池了——沒人加注時，這一手你等於免費看到翻牌。
...
> **快速解答**
> 底注和盲注最大的差別是「誰要出」：盲注只有兩個座位付，底注則是整桌每一手都躲不掉。這一條之所以重要，是因為它直接改變你該打多兇——當每一圈光是坐著就在流血，等好牌的成本就變高了。
...
坐進一場現場現金局時，各家撲克室的規矩不一樣：通常沒辦法免費開打，多數撲克室會要求你二選一——**補下一個大盲** 立刻加入，或等盲注輪到你的座位；也有撲克室不要求無限注／底池限注新入座的玩家補盲（WSOP 現金局（Live Action）規則 104.a 就是這樣），坐下前先問荷官一句最穩。如果你離桌一下，==**漏掉了你的盲注**==，多數撲克室會要你回座時補下才能重新加入——漏掉的大盲當成一個有效的活注，漏掉的小盲則是一枚 ==**死籌碼**==、直接被收進底池。線上牌桌會自動幫你處理這一切。
