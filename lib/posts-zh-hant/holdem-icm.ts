...
數字全在這了：領先者手握==一半的籌碼，卻只佔 38.4% 的獎金==，而短碼那 20% 的籌碼，值 28.9%。你不用在牌桌上手算這些——ICM 計算工具一秒就跑完（實戰上怎麼用，見[錦標賽指南](/zh-hant/blog/holdem-tournament)）——但親眼看過一次這台機器怎麼運轉，才是讓策略真正刻進腦子的關鍵。
...
![泡泡時間上，一個中碼在錢圈邊緣蓋掉面對大碼全下的牌，籌碼與獎金階梯都在畫面裡——ICM 壓力把一個正常的跟注變成蓋牌的那一刻](/images/holdem-icm-pressure.webp "ICM 壓力：中碼蓋牌，因為爆掉會賠上籌碼再也買不回來的鎖定獎金")
...
> **快速解答**
> 談交易的順序是：先算、再開口。籌碼平分對大碼有利、ICM 交易對短碼有利，兩種算法的差距在下面的例子裡是 $97——這筆錢會流向先知道自己數字的那個人。大碼多要一點、短碼少拿一點換取確定性都合理，但底線得先算出來。
...
> **快速解答**
> 真的可以無視 ICM 的場合比想像中多：早期、中期、盲注還小、以及最後的單挑——這些時候下一次獎金躍升不是很遠就是不存在，照籌碼 EV 打就對了。ICM 只在階梯逼近的那幾手放大，把它帶進整場比賽，反而會把籌碼白白蓋掉。
...
A. 三個大的。第一，*過度*套用 ICM——獎金躍升明明還遠，卻一路蓋牌「想爬階梯」，而不是累積籌碼。第二，當泡泡時間附近的中碼卻跟得太寬，那正是你風險溢價最高、爆掉會賠上鎖定獎金的地方。第三，當短碼卻答應籌碼平分（或當領先者卻答應 ICM 交易）而事先沒算過數字。ICM 是後期工具：用得太早、或在決賽桌無視它，都會漏錢。
