...
你看得到的每一張牌，都在移除一些可能性。在德州撲克裡，補牌不可能躺在牌面上——真躺在那裡，你的牌早就成了——所以要追蹤的==死牌==是那些在牌面**之外**露出來的：發錯牌時翻出來的一張、進蓋牌堆前被亮出來的一手、你剛好瞄到的鄰座棄牌。你看到的每一張，都是你已經不再擁有的補牌。好手就是在每一條街上不停地做這種安靜的微調。這就是算牌，只是不需要維持一個 running total 的那種。
...
德州撲克只攤開五張公共牌，所以這種算法在這裡只限於牌面。但梭哈——以及同樣發明牌的梭哈系玩法，像 Razz 和梭哈高低分——獎勵的，正是 21 點算牌者最擅長的那種記牌本領。這是撲克裡最接近電影版的一種玩法。
...
按你看到的死牌做調整 | 扣掉任何你在牌面之外看到的補牌——發錯牌時翻出來的一張、被亮出來的一手、剛好瞄到的鄰座棄牌。你看得到的牌，就是對手不可能有的牌——但只限剛好瞄到的：故意去看別人的底牌，在任何一家牌房都算違規，也可能被罰
:::
...
A. 算補牌（能升級你手牌的牌）、用阻擋牌（你手裡那些能減少對手組合的牌），還有追蹤死牌（你已經看著它離開牌局的補牌——發錯牌時翻出來的一張、棄牌時被亮出來的一手）。三者合起來，讓你讀出接下來可能來什麼、以及對手不可能有什麼。
