...
A. 不會 —— 但德州撲克的牌桌上確實有一個地方會用到花色，只是跟底池無關。現金桌、以及大多數撲克室的館規，開局時每人抽一張牌決定按鈕從哪裡開始，兩張抽到同點數就用花色順序決定。（WSOP 錦標賽不抽牌：==Rule 85== 規定按鈕從荷官右手邊第一位開始，只在剩三桌、兩桌、一桌時重抽 —— 完全不看花色。）不管哪一種，決定的都是*座位*，從來不是牌。錦標賽規則裡唯一的花色順序屬於梭哈和 Razz。兩手最強五張一張一張都對上，不管花色如何都平分底池。
