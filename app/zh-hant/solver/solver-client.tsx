 ...
 *   · 산수: 결함 0(«唯一 리드»·«사라진다»형 지적 2건은 zh 동일 문구가 검수장 전수 0 판정 — 기각).
 *     🔄 **2026-08-26 정정 — «사라진다»형 기각은 더 이상 유효하지 않다.** 기각 근거가
 *     «앱 축어 그대로라 고치면 앱과 갈라진다»였는데, **앱이 `190d293`에서 완화형으로 바뀌어
 *     그 근거가 소멸했다.** 재지적이 아니라 «근거 뒤집힘»이다(검수장 M-067 §2 · zh 동형).
 *     → 학습 스팟 ⑤ note는 신형 축어로 재동기했다. 이 줄을 근거로 되돌리지 마라.
 */
...
const SPEC: [string, string][] = [
  ...
  // 🟢 이 열거는 앱 셀렉터와 1:1이다 — 번체가 자기 자리, 간체는 «另有»의 첫 항.
  //    ★2026-08-24 갱신 — 앱이 fr을 배포해 셀렉터가 9개가 됐다(法語 추가).
  // ★2026-09-05 갱신 — 앱이 id·ms를 배포해 셀렉터가 11개가 됐다(라이브 실측 · ③ 마감 회차).
  ["語言", "繁體中文（另有簡體中文、韓語、英語、日語、西班牙語、葡萄牙語、德語、法語、印尼語、馬來語、印地語）"],
 ...
 */
// ✅ 2026-09-03 — zh-hant 해설 13편을 발행하며 slug를 전 행에 채웠다(lib/posts-zh-hant/<slug>.ts · EN·ja·es·zh 랜딩과 같은 대응). slug는 lib/gto-series.ts와 동일.
const SPOT_GROUPS = [
  {
    ...
    items: [
      ...
      { board: "A♥7♦2♣", slug: "a-high-board-cbet", name: "乾燥的 A 高牌面", note: "範圍優勢的教科書——A 這張牌正好打中開池方的範圍，BB 在這裡 98.2% 過牌" },
      { board: "K♠8♦3♣", slug: "k-high-board-cbet", name: "乾燥的 K 高牌面", note: "和 A 高牌面比一比：K 高牌面同樣是 BTN 佔優，但過牌會稍微多一點。為什麼呢？" },
      // 🟢 앱 zh-hant 축어가 이미 정정본. 다만 «在 13 個案例裡»의 편 수 하드코딩만
      //    «這套案例裡»로 갈았다(RP-08 · zh와 같은 처방).
      { board: "Q♠J♦T♠", slug: "broadway-board-strategy", name: "百老匯連張雙色牌面", note: "看著像兩邊都打中的牌面。可 BB 的勝率實現在這套案例裡是最低的——77.9%，BTN 是 119.4%——BB 有 99.9% 都在過牌" },
      ...
      //    ⚠ 아래 L128 주석의 «消失 지적 기각»은 근거가 «앱 축어 그대로»였다 — **그 근거가 소멸했다.**
      //       재지적이 아니라 근거 뒤집힘이다(검수장 §2). 🔴 간체 «变少了»를 자형만 바꿔 복사하지 마라 —
      //       앱 번체 축어는 «變少了，主要剩下»다(190d293 `lessonZhHant` 실측).
      { board: "Q♠9♠2♠", slug: "monotone-board-strategy", name: "單色牌面（3 張同花）", note: "看看大注為什麼變少了，主要剩下小注和過牌——連已經成同花的牌都經常只過牌" },
      { board: "6♣6♦3♥", slug: "paired-board-strategy", name: "對子牌面", note: "誰都不太容易打中的牌面，詐唬（bluff）的頻率就上去了。到詳情表裡找找看，是哪些手牌被當作詐唬來下注" },
      ...
      { board: "6♠5♥2♦", slug: "low-board-check-raise", name: "低張彩虹牌面", note: "一場高張（overcard）之爭。研究過牌加注怎麼搭建的案例——畫面上 BB 的第一個動作是 96.8% 過牌、3.2% 下注" },
    ],
  },
  {
    ...
    items: [
      ...
      { board: "A♦K♠2♥", slug: "3bet-pot-cbet", name: "3bet 方佔優的 A 高牌面", note: "對 3bet 範圍（一堆 AK、AA、KK）來說最好的翻牌。小注之所以能壓住對手整個範圍，靠的是這個範圍的形狀——不是因為 SPR 低" },
      // 🟢 앱 zh-hant 축어가 이미 정정본이다(98.4 · 0.8).
      // 🔴 2026-09-06 EN 정렬 — 노트 꼬리의 «0.8 %만 체크» 절을 뗐다(구형 문자열은 주석에도 인용하지 않는다 — 회귀 앵커 계수 보호). 98.4는 «큰 사이즈» 몫이고 벳 합계는 99.1(작은 사이즈 0.7 포함)이라
      //    98.4와 0.8을 나란히 두면 독자가 뺄셈으로 0.8%p를 잃는다. en 문안이 정본.
      //    🔴 100−99.1로 «빼서» 구하면 0.9가 나온다 — 개별 반올림값의 합은 100이 아니다(§4-B 주의).
      { board: "Q♥T♥7♠", slug: "3bet-pot-bet-sizing", name: "多變的雙色牌面", note: "雖然是 3bet 底池，但這個牌面對跟注方也不差。可 3bet 方並不會收手——98.4% 的範圍都用同一個 2/3 尺寸打出去" },
  ...
  },
  {
    ...
    items: [
      ...
      { board: "K♥T♦6♠", slug: "blind-battle-cbet", name: "K 高帶 10 的牌面", note: "盲位對戰雙方範圍都寬，所以到了翻牌兩邊都比較弱。同樣是 K 高牌面，和 BTN vs BB 時的頻率比一比，差在哪裡" },
      { board: "7♦6♦5♣", slug: "blind-battle-connected-board", name: "低張連張雙色牌面", note: "兩個寬範圍撞上一個連張性極強的牌面。兩對、順子、聽牌滿地都是——這是「手牌/聽牌」面板最熱鬧的一個局面" },
      // 🟢 앱 zh-hant 축어가 이미 정정본이다(88 대 66 · 80.1) — 明三條(trips) 용어도 코퍼스 정본과
      //    일치한다(`zh-hant/holdem-glossary` 暗三條/明三條 구분 · M-034 「코퍼스 정본 먼저」).
      { board: "A♠A♥6♦", slug: "ace-paired-board-strategy", name: "A 對子牌面", note: "牌面上擺著兩張 A 的特殊局面。明三條（trips）並不少見——SB 有 88 個組合，BB 只有 66 個，所以 SB 打出 80.1%。誰手裡的 A 更多，就是這個牌面的全部" },
    ],
  },
];
