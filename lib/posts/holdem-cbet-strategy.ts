...
export const POST: Post = {
  ...
  title: "홀덤 C벳이란? — 아무 플랍에나 쏘면 칩 버립니다",
  seoTitle: "C벳 무조건 하면 칩 샙니다 — 하면 안 되는 보드",
  desc: "프리플랍 레이즈했다고 플랍에 무조건 쏘나요? C벳 뜻과 보드별 사이즈, 그리고 쏘면 안 되는 보드를 먼저 짚습니다.",
  ...
  image: "/images/holdem-cbet-player-action.webp",
  imageAlt: "어두운 조명 아래 칩을 쥔 손이 플랍에서 C벳 타이밍을 재는 홀덤 실전 장면",
  tags: ["c벳", "컨티뉴에이션 벳", "c벳 타이밍", "더블 배럴", "드라이 보드"],
  content: `
![어두운 조명 아래 칩을 쥔 손이 플랍에서 C벳 타이밍을 재는 홀덤 테이블](/images/holdem-cbet-player-action.webp "C벳 = 프리플랍 레이저가 플랍에서 이어서 쏘는 첫 베팅 — 패 연결 여부와 무관")
...
특히 [스몰블라인드](/blog/holdem-small-blind-role)에서 "내가 레이즈했으니 C벳해야지" 하고 자동으로 쏘는 습관은 위험합니다. C벳은 공격권을 이어가는 도구지만, 포지션이 나쁘고 보드가 상대에게 맞았으면 [체크](/blog/holdem-check-meaning) 후 폴드가 더 싸게 막는 선택일 수 있습니다.

---

:::stripe
보드별 | 드라이 25~40% · 일반 50~60% · 웻 65~75%
헤즈업·드라이 | C벳 효과가 가장 큰 조건
선별 | 빈도보다 어떤 보드에서 쏘느냐가 핵심
멀티웨이 | 상대 많을수록 효율 급감
:::
...
> **바로 답**
> C벳은 **프리플랍에서 레이즈한 사람이 플랍에서도 이어 거는 베팅**입니다. 상대는 "저 사람이 강한 패로 열었다"는 전제를 갖고 있어서, 내 패가 미스했어도 폴드를 받아낼 수 있습니다.
...
> 가장 중요한 건 빈도가 아니라 **선별**입니다. "레이즈했으니 무조건 쏜다"는 자동 C벳이 초보가 칩을 흘리는 대표 경로입니다 — 보드가 상대 레인지에 맞았으면 체크가 정답일 때가 많습니다.
...
강한 핸드를 들었을 때 C벳으로 팟을 키운다. 상대가 레이즈하면 **강한 핸드와 블러프가 섞인 좁은 레인지**로 정리되고, 콜하면 드로우나 미드 페어일 가능성이 크며, 폴드하면 팟을 가져온다. 세 가지 시나리오 모두에서 정보가 생긴다. 다만 레이즈를 곧바로 "강함 확인"으로 읽으면 안 된다 — 아래 «더블 배럴을 피해야 할 때»에서 보듯 체크레이즈 레인지에는 블러프가 함께 들어 있어서, 전부 밸류로 읽으면 블러프 레이즈에 과하게 접게 된다.
...
**너무 크면(90%+)**: 블러프가 본전을 맞추는 데 필요한 폴드율이 ==0.9 ÷ (1 + 0.9) = 약 47.4%==까지 올라간다(하프팟은 33.3%, 풀팟은 50%). 접히면 더 벌지만 실패했을 때 잃는 금액이 커지고, 밸류 쪽으로는 나보다 약한 핸드의 콜을 못 받는다.
...
![녹색 펠트 위 딜러 버튼과 플랍 보드 — C벳 여부는 상대 수·포지션·보드 텍스처로 결정한다](/images/holdem-button-dealer-board.webp "C벳 타이밍은 상대 수·포지션·보드 텍스처를 함께 보고 결정")
...
> 자주 헷갈리는 자리 하나. **"타이트-피시"(강할 때만 콜하는 상대)는 블러프 C벳이 안 통하는 상대가 아니라 가장 잘 통하는 상대**입니다. 플랍에서 쓸 만한 패가 되는 건 세 판에 한 판꼴이라 나머지는 그냥 접어 주니까요. 이런 상대에게 통하지 않는 쪽은 **밸류 C벳**입니다 — 나보다 약한 핸드가 콜을 안 해 주기 때문에 얇은 밸류는 체크가 낫습니다.
...
- 미드 페어(A9·T9 등 9를 한 장만 든 핸드) 가능 — 99·66은 보드와 겹쳐 페어가 아니라 **셋**이 된다
...
- 상대가 플랍에서 [체크레이즈](/blog/holdem-check-raise "thumb:/images/holdem-check-raise-counter.webp")한 경우 (강한 핸드·블러프 혼합 — 더블 배럴 대부분 중단)
...
C벳 빈도를 조절하는 가장 쉬운 방법은 **밸류 C벳과 블러프 C벳 비율**을 의식하는 것이다. 이 비율은 하나로 고정된 값이 아니라 **사이즈가 정한다.** 상대를 무차별하게 만드는 블러프 비중은 ==벳 ÷ (팟 + 벳×2)==라서, 풀팟이면 1/3(밸류 2 : 블러프 1)이지만 이 글이 권장하는 33~75% 구간에서는 ==약 4 : 1 ~ 2.3 : 1==로 밸류 쪽이 더 두꺼워진다. 흔히 인용되는 "2:1"은 **풀팟 벳 기준값**이라는 뜻이다. 게다가 이 값은 카드가 더 나오지 않는 리버 기준이고, 플랍은 턴·리버 두 장이 남아 블러프를 그보다 넉넉히 섞을 수 있다. [홀덤 레인지란?](/blog/holdem-range-meaning)에서 폴라라이즈드·머지드 레인지 개념을 익히면 C벳 빈도 조절이 더 쉬워진다.
...
:::readnext[이어서 읽기]
/blog/holdem-small-blind-role | 스몰블라인드 역할 | /images/holdem-small-blind-position.webp
/blog/holdem-check-meaning | 홀덤 체크 뜻 | /images/holdem-check-meaning-mistakes.webp
:::

## FAQ — 자주 묻는 질문

**Q. C벳을 상대가 콜할 때마다 더블 배럴 해야 하나요?**

A. 아닙니다. 더블 배럴은 선별적으로 해야 합니다. 상대가 드로우로 콜했을 가능성이 높고, 턴 카드가 드로우를 미스시켰거나 내 레인지를 강하게 보이게 하는 경우에 시도하세요. 막연히 "지난 번 C벳이 콜됐으니 또 한 번 더"는 좋지 않습니다.

**Q. OOP(포지션 불리)에서 C벳은 무조건 피해야 하나요?**

A. 무조건은 아닙니다. 강한 밸류 핸드(탑 페어+)나 세미블러프 드로우면 OOP에서도 C벳이 맞습니다. 다만 에어 핸드의 블러프 C벳은 포지션 불리에서 더 위험하다는 점을 인식해야 합니다.

**Q. 상대가 C벳에 레이즈하면 어떻게 해야 하나요?**

A. 밸류 핸드면 3벳 또는 콜, 블러프 C벳이면 대부분 폴드가 맞습니다. 상대의 레이즈 빈도와 레인지를 파악하는 게 핵심입니다. 타이트한 상대의 레이즈는 강한 핸드일 확률이 높고, 루즈한 상대는 블러프 레이즈도 섞여 있습니다.

**Q. 드라이 보드에서는 항상 C벳해야 하나요?**

A. 드라이 보드는 C벳 효율이 높지만 "항상"은 아닙니다. 멀티웨이나 상대가 너무 타이트해서 C벳에만 강한 핸드로 반응하는 경우라면 에어 C벳의 가치가 낮아집니다. 기본적으로 헤즈업 드라이 보드는 강하게 C벳하는 방향이 맞습니다.

**Q. C벳 빈도가 높다는 걸 어떻게 알 수 있나요?**

A. 상대가 C벳에 레이즈하거나 콜 후 턴에서 나를 베팅으로 압박하는 빈도가 늘어나면 C벳 블러프가 읽히고 있다는 신호입니다. C벳 이후 콜 비율이 지나치게 높다면 사이즈를 키우거나 빈도를 줄여야 합니다.
