...
| 这对别说反了 | 用错会怎样 |
|:---|:---|
| **过牌（Check） vs 跟注（Call）** | 前面没人下注你才能过牌；一旦有人下了注，你想留在牌里就只能跟注或加注。**面对下注还喊「过牌」，你就只剩跟注或弃牌，不能再加注**。 |
...
> **快速回答**
> 盲注是小盲、大盲两个座位每手必交的强制注；前注也是起底池的强制注，传统上全桌都交、如今锦标赛多由大盲一个座位代交；买入是坐下或报名要付的钱；抽水是牌房从每个底池抽走的佣金；bankroll 是你留给扑克的全部资金，不是桌上那一摞筹码。
...
| **无限注 / 限注（No-limit / Limit）** | 无限注（NLH）随时可以下到你的全部筹码；限注则把每次下注卡死在固定大小。德扑几乎清一色是无限注。 |
