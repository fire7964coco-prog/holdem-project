...
==out（出路牌）==是任何一张能把你这手牌变成大概率赢牌的、尚未现身的牌。一个同花听牌有==9 张 outs==（一种花色 13 张减去你能看到的 4 张）。用==二四法则==把 outs 换成一个大致胜率：还剩两张要发时乘 4，只剩一张时乘 2。
...
按已死的牌做调整 | 把任何你在牌面之外看到的 out 减掉——发牌时闪出来的一张、亮开过的一手、恰好瞄到的邻座弃牌。你能看到的牌，就是对手不可能拿到的牌
:::
