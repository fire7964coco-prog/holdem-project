...
| 🔴 **앱 UI 축어 사전** | `.solver-captures/data-<loc>.json`(스팟명·등급 라벨·플레이어 라벨) + `scripts/capture-solver-spots.mjs`의 `L10N.<loc>`(내비·버튼) | **「自分で確かめる」 절의 경로·라벨·표 등급명은 여기서만 가져온다.** ja 첫 회차에서 이걸 안 주자 배치마다 EN 화면 용어(Study Spots · View results · ノーメイド)와 앱 축어(学習スポット · 結果をすぐ見る · 役なし)로 갈려 ~270곳을 사후 통일했다 |
...
## 4-B. 배치 간 통일 표기 (ja 1회차 정규화 결과 — 다음 로케일도 «앱 축어 우선» 원칙)
学習スポット · ⚡ 結果をすぐ見る · このスポットを自分で計算する · GTOトレーナー · 役なし · Aハイ/Kハイ · ローペア(앱)=弱いペア(산문 허용) · 実効スタック · テクスチャ · コールレンジ · シングルレイズポット · ツートーン(앱 스팟명 안의 「（2トーン）」만 예외) · ロー(ロウ 금지) · %p→ポイント · 카드 심볼 10♠(T♠ 금지, 레인지 표기 T9s/TT는 유지) · 「〜です：」 금지(→「——」) · H2의 BB/BTN/SB는 풀 표기 · H2 「この数字が出た計算条件は?」 · 「## 自分で確かめる」 · 「## 実戦で変えることは?」 · 태그 「ポーカー GTO」.
...
🔴 **정정도 원문 대조 후에** — 렌즈가 「0.1%〜1.9%」를 지적해 파일 전체를 23.7로 바꿨더니 EN 원문이 1.9%인 표 행까지 덮여 게이트 🔴 4가 났다. 같은 문자열이 «틀린 자리»와 «맞는 자리»에 공존한다.
