...
### 이 세션의 본체 = **A층 묶음 7** (남은 3건 · 2단계 마지막)

검수장 인계 **4단계 중 2단계** 진행 중 — **12/15건 완료**.

> ✅ 1단계(미러 전파 47곳) `a66ef97` · ✅ 2단계 묶음 1~6 = `3d51c1c` `2b4d6cd` `77de935` `0c35439` `cba8b3f` `30b820d`
...
### 상태 (2026-08-11 마감 · **묶음 6 후에도 지표 전부 불변** — 실측 재확인)
...
| ja 원장 | 유효 **1368** / 무효 **12** — 무효 12건 = A층 기수정분(묶음 1~6)과 **1:1** ✅ |
...
| **7** | `holdem-strategy` #9 · `holdem-rake` #32 · `holdem-drawing-odds` #17 | ◀ **다음. 2단계 마지막 → 끝나면 푸시.** strategy: 예외에 **BB 디펜스** 추가(2.5bb 오픈 → 필요 ≈27%, 표준 디펜스의 30~50%가 플랫콜) + **본문↔FAQ 정합**(FAQ는 이미 셋마이닝 콜을 인정한다). rake: 「상위 레이트로」에 **뱅크롤·에지 조건**(본문이 이미 「같은 실력·같은 에지」를 전제했다). drawing-odds: 35%는 코인플립 아님 → **「3회에 1회에 가깝다」**, 같은 글 다른 구절과 정합 |
