...
**当两名玩家做成同样的对子时，踢脚牌是==从大到小、一张一张往下比==的：先比第一张，打平了才比第二张，再打平才比第三张。**一对手里带三张踢脚，所以这场比较最深能拆到第三张牌。
...
**踢脚牌的张数由牌型决定：一对有 3 张有效踢脚，三条有 2 张，两对和四条各只有 1 张。**记住这张表，你在摊牌时就知道一场平局最多能往下拆几层——这也是全网几乎没人做全的一张"杀手表"。
...
**当公共牌上比你底牌更大的牌，把你的踢脚从 best-5 里挤了出去，你手里那张牌就变成了"废牌"——两个人可能都拿着 A，最后却平分底池。**这是牌桌上最容易看错的一种情况之一：明明我有一张 A，怎么会跟他打平？
...
**有——四条带一张踢脚牌，但在德州里它几乎从不决定胜负：那需要两名玩家拿到同一副四条，也就是四张全摊在公共牌上——这种牌面很少见。**这就是大多数教程搞砸的那个例外，它们把四条一股脑归进了"没有踢脚牌的五张牌型"。
