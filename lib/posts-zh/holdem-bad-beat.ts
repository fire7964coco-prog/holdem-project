...
你把钱推进去时是“好牌位”，在数学上领先——这通常就是正确的打法——牌堆却偏偏发出了那张能击败你的牌。这一败不怪你；是 variance（方差／运气波动）在使坏。

机制永远是一次 **suckout（逆袭）**：一张在钱进池之后才到来的牌——翻牌、转牌或河牌都可能——把一手输牌变成了赢牌。你的一对 A 本来把对手的一对 5 打得死死的，直到那第三张 5 冒了出来。你的顶对本来领先对手的同花听牌，直到最后一张红桃落下。那个瞬间——热门被一个本不该到岸的听牌逆袭——正是这个词的核心。搞懂它，也是不让它毁掉你整个牌局的第一步，这份情绪自律正是把高手和 [fish（鱼／菜鸟）](/zh/blog/holdem-fish) 区分开来的东西。
...
> **快速回答**
> 两者没有统一的官方分界，按本文采用的严格区分只看一条：钱推进去时你领先、之后被一张幸运牌反超，是 bad beat；钱进池时你就已经落后、只是牌太强弃不掉，是 cooler。前者该气这副牌，后者只能佩服它——两者都疼，但方向相反。

![把 bad beat 和 cooler 分开的信息图——一对 A 对上提升成暗三条的一对 7，旁边是一对 K 撞上从头就领先、不必提升的一对 A](/images/holdem-bad-beat-litmus.webp "严格的分法：那对 A 进池时领先、后来被反超，是 bad beat；那对 K 进池时就落后、始终没追上，是 cooler")

人们常把“bad beat”和“cooler”混着用，两者之间也没有统一的官方分界——但按本文采用的严格说法，它俩是相反的，而搞清楚刚刚发生的是哪一个，能告诉你该气这副牌，还是该佩服它。整个区别就在于**钱推进去时谁领先，以及有没有发生 suckout：**

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| | bad beat | cooler（严格意义） |
|:---|:---|:---|
...
一个快速的试金石：**如果钱推进去时你是大热门，而你的对手需要*提升*牌力才能赢，那就是 bad beat。**如果钱推进去时他就已经领先、你只是弃不掉自己的怪兽牌，那就是严格意义上的 [cooler](/zh/blog/holdem-cooler)——没有 suckout，就没有 bad beat。还要注意这个陷阱：**翻牌成的暗三条对翻牌成的暗三条（flopped set over set）不是 bad beat。**当你的暗三条 Q 输给同一个翻牌上做出的暗三条 K 时，没有人在 river 河牌走运——更大的那个暗三条从头到尾都领先。（如果钱已经进池、那对 K 才在转牌或河牌击中暗三条，按这条试金石就是 suckout——那一次*确实*是 bad beat。如果是更大的暗三条先成形、筹码才推进去，那仍然是 cooler。）那是一场披着 bad beat 外衣的 cooler。
...
最标志性的是**一对 A 被暗三条击碎。**你拿着一对 A 翻前全下对上一对 7——你大约是 80% 的热门，一个 4:1 稳赢的局面。可这副牌里还有另外两张 7，只要一张出现在牌面上，他们的三条几乎总能击败你的对子——只有一张 A，或者罕见的牌面走势（同花、顺子，或公共牌上的三条）才救得了你。五次里有四次你收下底池；第五次，你就有了一个没人想听的 bad beat 故事。数学从来没错——你只是落在了它错误的那一边，这也正是为什么单独一手牌根本 [说明不了你打得好不好](/zh/blog/holdem-cooler)。
...
> **快速回答**
> bad beat jackpot（官方就叫这名，中文辅助说“爆冷大奖”）是一个累积奖池：从一个个底池里额外抽一小笔攒起来，当一手非常强的牌在摊牌时输掉，就派给输家。门槛条件和分配比例全看牌室规则。
...
**jackpot 有一样东西是不看的：本页开头那条判别法。** 它的门槛是按**牌力大小**写的，而不是按“钱推进去的时候谁在前面”。拿最经典的触发手试一下：你拿 A♠A♥，公共牌 A♣ J♠ J♦ 7♥ 2♣，成葫芦 A 带 J；对手拿 J♥J♣，成四条 J。两手牌在翻牌圈就都已经成形，筹码也是在那里推进去的——所以钱进去之后谁也没被反超。按上面的标准，这是一手 **cooler**，而 jackpot 恰恰就是为这种牌型派奖的。把“bad beat jackpot”当成牌室的产品名，而不是这个词的第二个定义。
...
> **快速回答**
> 源源不断的 bad beat，通常说明你正坐在一个可击败的牌局里。愿意在落后时把钱推进去的对手，就是长期给你送钱的对手；一次 suckout 是那份收入的入场费，不是账单。

这不是安慰话，而是一条能推到底的道理。想想一次 bad beat 需要什么：一个把钱在*落后*时、作为数学上的弱势方推进去，然后走运的对手。多数情况下，那是一个在做**亏钱决策**的玩家——正是你想要的那种对手。如果你牌桌上从来没人做那些亏钱的跟注，那就意味着每个人都在正确地弃掉自己的弱牌——那是一个难打得多的牌局。正如一句广为人知的教练格言所说，来自弱手的一次 suckout 是一份*礼物*：它是你另外四次拿走他们筹码的入场费。

在一个足够大的样本里，运气会围绕你的期望值拉平。如果你是那个领先着把钱推进去的人，你遭遇的 bad beat 就会比你*施加*给别人的多——它们是你作为热门赢下的所有底池的另一面。真正驱动你长期结果的，是你决策的质量。**把钱在领先时推进去然后输了，从长期看，在每一个重要的意义上仍然是一次胜利。**优势会在成千上万手牌里累积；单独一次 bad beat 只是围绕它的噪音。
...
只要你确认过当时推进去是对的——下注尺寸、筹码深度、锦标赛压力都看过——一次 bad beat 关于你的打法就没有任何教训可言，它主要的危险在于它对你*接下来*几手牌的影响。保护好它们：
...
4. **相信你的 bankroll（资金）。**bad beat 正是你要留一份足以吸收 variance 的资金的原因。在现金局里，跨越几万手牌，一次 bad beat 只是一个舍入误差——真正留下长久伤害的，是让它改变你的打法。在锦标赛里，它可能直接结束你的这一程；那是你报名时就接受了的 variance。
...
A. 在一次 bad beat 里，钱推进去时你是热门，然后被逆袭了——发生了一次 suckout。在一场严格意义上的 cooler 里，钱推进去时你就落后、手握一手太强弃不掉的牌，且没有发生 suckout（也有牌手把 cooler 用得更宽，泛指任何一手大牌输给更大的牌）。快速测试：如果钱推进去时你是大热门、而你的对手需要*提升*牌力才能赢，那就是 bad beat；如果钱推进去时他就已经领先，那就是 cooler。比如说，两边都在翻牌就击中暗三条、筹码也在翻牌圈推进去的暗三条 vs 暗三条就是 cooler，不是 bad beat——但如果筹码进池后更大的那个暗三条才在转牌或河牌到来，那就是一次 suckout。
...
A. 不——绝大多数情况下它俩是相反的。一次 bad beat 意味着你的钱是作为热门推进去的——多数情况下正是因为你做出了*正确*的决策——然后输给了运气。但“当时领先”本身并不能证明决策没问题：下注尺寸选错，或者 ICM 上这个底池本就不值得去争，在领先时把筹码推进去这件事照样是错误。因为*你自己*做了一次糟糕的跟注或一次松散的打法而输牌，那不是 bad beat，那是错误。诚实的玩家会很小心，不把自己的错误归档到“bad beat”名下，因为漏洞就是这么一直不被修好的。

---

## 要记住的 3 件事

1. **bad beat 就是作为大热门输给一次幸运的 suckout**——钱推进去时你是领先的，却被逆袭了。如果推进去是正确的打法，那就是 variance，不是错误。
2. **按严格定义，它是 cooler 的反面。**bad beat 需要很大的优势加上一次 suckout（弱势方提升牌力）；严格意义上的 cooler 没有 suckout（领先方始终领先）。如果钱推进去时你是大热门、而你的对手需要提升牌力才能赢，那就是一次 bad beat。
3. **bad beat 其实对你是好事。**它们意味着对手正在落后时把钱推进去然后走运——多数时候，那些亏钱决策给你带来的回报，远远多过它们烧到你的次数。领先着推进去，把这一败一笑置之，让 variance 自己抹平。

bad beat 是你为参与一个能赢的牌局所交的税。最好的玩家遭遇的 bad beat 一点也不少——往往还更多，因为他们更常是热门——他们只是学会了把这些记为 variance、保护下一手牌不受 tilt 影响，然后回到 [把满桌人打穿](/zh/blog/holdem-fish) 上去。只要你足够频繁地领先着把钱推进去，这副牌的残忍就会变成你的利润。
