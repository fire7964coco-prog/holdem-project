...
> **快速回答**
> 位置不是一把椅子，而是你相对于庄家按钮所在位置的名称，它决定了你在每条街的行动顺序。9 人桌的位置顺序是：UTG、UTG+1、UTG+2、LJ、HJ、CO、BTN、SB、BB —— 按钮每手牌顺时针移动一个座位，所以每个人每一手都换一个名字。

因为按钮每手牌之后都顺时针移动一个座位，所以桌上每个玩家每一手都戴着一个不同的名字。
...
> **快速回答**
> 当值台员喊「12 号桌，5 号座」时，那个数字跟位置毫无关系 —— 座位卡上的编号钉死在椅子上，1 号座通常是庄家正左边；位置则跟着按钮每手牌顺时针挪一个座位。
