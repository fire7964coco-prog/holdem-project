...
> **快速回答**
> 手握口袋对，翻牌中暗三条或更好的概率是 11.8%，约 7.5 比 1；两张同花色翻牌直接成同花只有 0.84%，翻出同花听牌则有 10.9%；同花连张翻牌成顺子 1.3%；两张不成对的牌翻出两对 2.0%。翻牌大多数时候谁都没中，这些数字告诉你「中了」到底有多难得。
...
经典场面：你翻牌中一手**同花听牌**（9 张 outs）。你到河牌 ==有 35% 会做成==——比三分之一还高一点，所以别把它当成"多半会落空"的牌来打。再看右边那一栏：转牌一旦落空，剩下那一张牌的成牌概率就只有 19.6%——每多追一条街，你买到的都是更小的一份机会。
