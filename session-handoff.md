...
> 마지막 갱신 **2026-09-22** · HEAD = **`471b03c5`+** · **미커밋 0 · 배포·라이브 확인·IndexNow 200 완료.**
>
> **최근 회차(전부 종결 — 경위는 WORKLOG, 되묻지 말 것)**
> · **09-22 (1)** **queue Q13-a 머지**(`471b03c5` · 회신 **MB-075**) — MA-147 ②③ 1/2 cooler·bad-beat·fish **8로케일 24파일**. 🔴 **원장이 «정본»으로 든 de 문면이 공집합을 가리켰다**(「top end만 진짜 쿨러」 — 상단 = 너츠라 지는 쪽이 될 수 없다 = D유형) · 🔴 **레인이 자기 브리프 §4-8을 어긴 쪽이 옳았다**(`masterUpdated` 금지는 무조건이 아니다). 정본 승격 **§3-P·§3-Q + §1-C 조건부**.
>
...
1. **🧰 queue 다음 회차 — 사장님이 번호를 고른다.** 레인 창 첫 마디 = 「`HARDEN.md` 읽고 회차 Q<번호> 시작해」 · 회차 표 = `docs/harden-queue-진행.md` §1.
   🟢 **지금 레인에 도는 회차가 없다**(Q13-a 머지 완료 · 세 로케일 레인도 0커밋·clean). 사장님이 번호를 고르면 레인 창을 연다.
   머지 절차 = `git log main..harden-queue` → 레인 보고(진행 파일 §1-Q<번호>) 통독 → **헤드 독립 검산**(§13·산수·기계 대조) → 머지 → 빌드 → sitemap bump ↔ 편집 파일 수 **대조** → 푸시 → **라이브 확인은 «신규 있음 + 구 문면 사라짐» 둘 다**(🔴 `innerText` 말고 **`page.content()`** — 접힌 `<details>`를 못 본다) → IndexNow `--urls`로 그 URL만 → MB 회신·정본 승격·§5 요청 이행 → WORKLOG → 핸드오프 3단 마감.
   🔴 **검수장 MA-147 — ①은 Q12-a·Q12-b로, ② ⓐⓑⓕ + ③ 중 2건은 Q13-a로 종결.**
   **1순위 = `Q13-b`** MA-147 ②③ 2/2 = `holdem-glossary`·`holdem-rake`·`holdem-straddle`(G1~G3 · R1~R5 · S1~S4 + es rake FAQ 「Solo se cobra…」).
   🔴 **자리 수를 「21」로 잡지 마라 — 8로케일 전수 대조면 약 110자리다**(Q13-a가 약 100이었다). 브리프 = `docs/harden-brief/queue-Q13-용어클러스터.md` §1 「Q13-b」 표 + §2 1차 출처 축어 + §3 처방 문면(**B 구간이 룰북을 다시 열 필요 없다**). 착수 전 진행 파일 §2 「Q13-a가 남긴 것」을 먼저 읽어라.
   **그다음 = Q14** MA-148 zh+zh-hant 정정 13 + EN-먼저 4(c-bet 직답 「四到五成」→「約 30–45%」는 **5로케일**).
...
- **우편함 — 회신 대기 중**: 🆕 **MB-075**(09-22 · Q13-a · **이견 2**: ⓐ 원장 #24가 정본으로 든 de 문면이 **공집합 처방**이라 근거란 정정 요청 ⓑ MA-147 ②의 **로케일 개수가 여섯 축에서 작다**(앵커 축어 grep이 표기를 못 맞춤) · **신설 결함 2**(ja 「イディオットエンド」 검색 0건 · zh-hant 콜아웃 전방참조 무조건형) · **앵커 갱신 요청**) · **MB-072**(09-21 · MA-158·159·163 이행 + MA-147·148 미이행 통지 · **요청 3**: ⓐ ms 66 «정밀형» 오판 정정 + 원장에 «분배 표지» 축 신설 ⓑ ⭐배지↔종료 공존 0 회귀 앵커 ⓒ 147-②ⓐ 앵커를 「In every one」으로 수정). 그 밖 미처리는 «→검수장»(MB-033~062 + S-022·023). 🔴 MB-054는 **MB-053 ②(상금풀 $87,568,080)를 뒤집는 통지**. 🪶 `check:mailbox` 기존 흠 둘(미수정): «MB-054 파이프 9» 🔴 · «MB-067 열 결손» 🟠(남의 회차 행이라 손대지 않았다).
