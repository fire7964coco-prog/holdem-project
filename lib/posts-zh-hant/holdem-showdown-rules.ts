...
| 前面的街全下（河牌沒有下注） | 錦標賽：所有手牌在發完剩下的公牌前／發牌時全部攤開（TDA 第 16 條）。現金桌：全下的人先翻牌——有邊池時邊池玩家先亮（現金桌規則第 149 條） |
...
| 全下情境 | 攤牌規則 |
|----------------|---------------|
| 有人在前面的街全下、其他人跟注、後面不可能再下注 | 錦標賽：所有手牌在發完剩牌前或發牌時全部正面朝上。現金桌：全下者先翻牌——有邊池時邊池玩家先亮（現金桌規則第 149 條） |
...
在多數撲克室，你可以要求荷官把一手蓋掉的牌翻開——但僅限那手 **被跟到、而且進到攤牌** 的牌。已經蓋掉的棄牌是死牌，永遠拿不回來。這條「要求看牌」的規則，是用來防串通作弊的，不是用來滿足好奇心的，濫用它會被當成失禮。要省著用。
...
A. 在錦標賽裡，當一名玩家全下、後面不可能再有下注時，涉及那個池的所有手牌會在發完剩下的公牌前或發牌時全部正面朝上（TDA 第 16 條）。如果有邊池，先結算邊池、再結算主池——但全下玩家的牌早在那之前就已經翻開了。現金桌上，WSOP 現金桌規則讓全下的人先翻牌，邊池參與者要在只爭主池的全下者之前亮牌（第 149 條）。至於其他還有籌碼、還能下注的玩家，在下注走完之前，他們的牌都還蓋著。
