...
| **time rake（计时费）** | 每人一笔固定费，每 30 分钟 | 约 $10–$15 每小时 | 高级别现场（$10/$20+），以及所有无法从底池抽水的场合 |
...
- **抽水封顶。**牌局绝不会在一个巨大底池上按满百分比抽——到一个上限就停，现场常见 **$3–$6**、线上 **$1–$3**。级别上去封顶也会上去，但不是按比例——它是一级一级跳的，所以好几个级别常常共用同一个封顶。此外，发到桌的人越少往往越缩水（一个单挑底池可能封顶在 $1）。
- **用 time rake 代替底池抽水。**在更高级别，牌局常常不抽底池，改成收一笔固定费——比如每人每小时 $10–$15，每半小时收一次。这对赢大底池的牌手有利——不过你省下的是*封顶那部分*抽水，而不是底池的一个比例：面对 $3–$6 的封顶，一个 $2,000 的底池本来也只交出几美元。
...
| 封顶 **$4** 的房 | 约 8–9 bb/100 | +8 bb/100 打平或变成**输家（0 到 −1）** |
...
对一个休闲牌手来说，rakeback 是个小福利。对一个高手数的常客来说它巨大无比：一个 20% 和一个 40% 的返水，差距是随你实际产生的抽水放大的——所以只有你在**够高的级别上打出足够手数**时，它才会变成一笔大钱，而对很多打平的 grinder 来说，rakeback *就是*他们的利润。它实际上拉低了你真正的抽水，所以在挑去哪儿打之前值得先看看。只是要留个心眼：网上很多 rakeback 的攻略都是联盟推广驱动的——对那些「点这里注册」的页面，要用你对付任何一句推销词的那份怀疑去看待。
...
那笔费用——也叫 **juice** 或 **vig**——就是锦标赛版的抽水。它通常是**买入的 5–20%**，而且是固定的：不管你第一个出局还是拿下整场冠军，都要交。买入越低，费用按比例越高（一个 $3 + $0.30 的 sit-and-go 就是 10%），而快节奏的 **turbo 赛制会压缩你的优势**，所以手续费在那里咬得最狠——百分比越低，你的技术能留下来的就越多。既然锦标赛的结构跟现金局完全不同，你为打牌付费的方式也就不一样——这个区别值得跟[锦标赛 vs 现金局](/zh/blog/holdem-tournament-vs-cash-game)的基础一起搞懂。
...
- **现场抽水**往往是**更高的百分比（常常 10%）加上更高的封顶（$3–$6）**——但你每小时只打约 30 手，所以你交它的次数更少。
...
A. 在一个抽水的局里你没法完全躲开抽水，但你能把它压小。尽量拿到最好的 rakeback；挑封顶对牌手友好的房。往上打级别也能压低抽水——固定封顶占每个底池的比例更小——但这一步只在你的资金扛得住波动、*而且*你面对更硬的牌桌仍然有优势时才走；否则对手从你身上拿走的，会远比抽水多。多打大底池、少打一堆到封顶的小底池，避开极短台——每小时的手数更多、交盲注也更频繁，被抽水的底池你会参与得多得多，偏向高级别的计时费（time rake）局，这些都能拉低你的实际抽水。单看成本，完全不抽水的家庭局是最便宜的扑克——底池里什么都不会被拿走。

**Q. 抽水合法吗？收抽水在扑克里为什么会犯法？**

A. 收一笔费用来主办牌局，本来就是有牌照的牌局、赌场或受监管线上网站的整个商业模式——这怎么监管，各地规定不一样。家庭局通常是分摊成本，而不是从底池抽水：主办者一旦开始抽一个百分比，这局牌就不再是大家分摊的开销，而变成了有人在经营的生意——电影《茉莉牌局》（*Molly's Game*）讲的正是这个转折。
...
A. 影响很大——最重的是低级别，那里封顶不随级别一起降下来。短台还多了一个跟封顶无关的因素：同一个底池的抽水由更少的人分摊，而且每 100 手里你交盲注的次数多得多——所以你每手承担的那一份变大了。（按*小时*算你还会付得更多，那只是因为手数更多，但那和 bb/100 是两个问题。）抽水能把一个小赢家变成输家：同一个 +8 bb/100 的牌手，仅仅换到一个抽水封顶更高的房，就可能变成小幅亏损。永远要在抽完水之后再来量你的赢率。
