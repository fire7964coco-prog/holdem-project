...
>
> ⚠ **남은 것 — 다국어 17편**: `holdem-hand-rankings-showdown.webp`를 EN·JA 등 전 언어가
> "두 플레이어의 베스트 5장"이라는 **같은 잘못된 캡션**으로 쓰고 있다(그 보드로는 성립 불가능한 그림).
> 새 `holdem-showdown-flush-vs-fullhouse.webp`는 **글자가 없어** 경로만 갈아끼우면 각 언어 캡션이 참이 된다.
> KO는 이번에 처리했다. 다국어는 미착수.
...
1. **다국어 17편 이미지 경로 교체** — 위 0-F의 남은 것. 기계적이고 §13 리스크를 17개 언어에서 동시에 없앤다.
