...
| 5 | B | id rankings 6편 | + hand-rankings hero `image`/`imageAlt` 복원(미결 표 참조). 🆕 **뱅크 = `docs/keyword-bank/id-core-volumes.md`**(2026-08-27 라쿠 실측 — urutan kartu poker 1.900 · tertinggi 1.300↑ · susunan/kombinasi 각 320↑. 착수 전 통독) |
...
| ⚠ **fr·id 뱅크는 라쿠 단일 벤더** — DFS 인증 만료(40100) 지속. 2026-08-27 재실측: `.env.local` 자격증명은 있으나 서버가 거부 → **대시보드(app.dataforseo.com/api-access)에서 API 비밀번호 재발급 필요(사장님 몫)** | DFS 복구 시 ① fr: `solver poker`·`gto poker`·`range poker`·`solver poker gratuit` 4종 교차검증 + France SERP(뱅크 §7) ② id: 헤드텀 4종 교차검증 + Indonesia SERP(`id-core-volumes.md` 머리 참조) |
