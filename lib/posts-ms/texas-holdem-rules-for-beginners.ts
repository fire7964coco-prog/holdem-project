import type { Post } from "../posts";

export const POST: Post = {
  slug: "texas-holdem-rules-for-beginners",
  title: "Cara Main Texas Hold'em untuk Pemula — Peraturan, Cip, Kedudukan Tangan & Strategi Pertama",
  seoTitle: "Cara Main Poker Texas Hold'em untuk Pemula — Peraturan & Cip",
  desc: "Tak pernah main poker? Belajar cara main Texas Hold'em langkah demi langkah — blinds, susunan cip, kedudukan tangan dan helaian rujukan mudah untuk pemula.",
  tldr: "Dalam Texas Hold'em, setiap pemain menerima 2 kad pemula, 5 kad komuniti dikongsi bersama, dan tangan poker 5 kad terbaik menang selepas empat pusingan pertaruhan.",
  category: "rules",
  date: "2026-06-11",
  updated: "2026-08-14",
  masterUpdated: "2026-07-12",
  keepImagesInBody: true,
  readTime: "14 minit",
  emoji: "♠️",
  image: "/images/rules-texas-holdem.webp",
  imageAlt: "Infografik asas Texas Hold'em — kad pemula A♣ K♦ di sebelah kipas lima kad komuniti A♠ K♥ Q♦ J♣ 10♠",
  tags: [
    "cara main poker texas holdem",
    "peraturan poker untuk pemula",
    "peraturan asas texas holdem",
    "siapa mula dulu dalam texas holdem",
    "helaian rujukan poker",
    "cip poker untuk pemula",
    "berapa pemain texas holdem",
    "no limit texas holdem",
  ],
  content: `
Jika anda mencari ==peraturan Texas Hold'em untuk pemula==, anda mungkin belum perlukan buku peraturan kasino lagi.

Anda perlu tahu apa itu blinds, bila giliran anda, bagaimana lima kad komuniti berfungsi, tangan mana yang menang, dan berapa banyak cip perlu diberikan kepada setiap orang bila bermain di rumah.

Panduan ini menerangkan ==cara main Texas Hold'em untuk pemula== dalam bahasa yang mudah, dengan urutan permainan yang tepat, susunan cip untuk pemula, asas mengedar kad, dan helaian rujukan (cheat sheet) yang boleh dicetak dan diletakkan di tepi meja.

### Cara main Texas Hold'em dalam 30 saat

1. Dua pemain meletakkan **blinds** (pertaruhan wajib)
2. Semua pemain menerima **2 kad pemula** secara tertutup
3. Bertaruh, call, raise, atau fold — **pre-flop**
4. Pengedar membuka **3 kad komuniti** (flop) → bertaruh lagi
5. Pengedar membuka **1 kad lagi** (turn) → bertaruh lagi
6. Pengedar membuka **kad terakhir** (river) → pusingan pertaruhan terakhir
7. Pemain yang masih tinggal membandingkan tangan — **tangan 5 kad terbaik menang**

Fakta penting:
- Anda boleh guna kedua-dua kad pemula, satu sahaja, atau langsung tiada — mana-mana yang membentuk tangan terbaik
- Butang pengedar berpindah setiap tangan, jadi blinds dan urutan giliran berubah setiap pusingan
- Anda menang tanpa showdown jika semua pemain lain fold pada bila-bila masa

---

## Peraturan Asas Texas Hold'em

Peraturan asas Texas Hold'em sebenarnya mudah sebaik sahaja anda melihat meja mengikut urutan.

Setiap tangan bermula dengan butang pengedar. Dua pemain di sebelah kiri butang meletakkan pertaruhan wajib yang dipanggil **Small Blind** dan **Big Blind** — jika ia kedengaran mengelirukan, baca [apa itu blinds dan bagaimana Small Blind dan Big Blind berfungsi](/ms/blog/holdem-blind-meaning). Selepas itu, setiap pemain menerima dua kad secara tertutup. Inilah kad pemula (hole cards) anda.

Kemudian, pengedar membuka lima kad kongsi di tengah meja:

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Peringkat | Kad yang dibuka | Apa yang berlaku |
|------|----------------|--------------|
| Pre-flop | 0 kad komuniti | Pemain bertindak dengan 2 kad pemula sahaja |
| Flop | 3 kad komuniti | Kad kongsi pertama muncul |
| Turn | 1 kad lagi | Kad komuniti keempat muncul |
| River | 1 kad terakhir | Kad komuniti kelima muncul |
| Showdown | Tiada kad baru | Pemain yang tinggal membandingkan tangan 5 kad terbaik |

</div>

==r:Anda **tidak** wajib menggunakan kedua-dua kad pemula anda.== Anda boleh guna dua, satu, atau langsung sifar kad pemula jika board itu sendiri membentuk tangan terbaik — kemahiran yang dipanggil membaca board.

Contohnya:

| Kad anda | Board | Tangan terbaik anda |
|-----------|-------|----------------|
| A♠ K♠ | A♦ 7♣ 7♥ 2♠ 9♣ | Two Pair, Ace dan tujuh |
| 8♠ 8♦ | K♣ 8♥ 4♠ 4♦ J♣ | Full House, tiga lapan dengan sepasang empat |
| 2♣ 3♦ | A♠ K♠ Q♠ J♠ 10♠ | Royal Flush di atas board |

Jika kedudukan tangan poker masih baru bagi anda, buka [kedudukan tangan Texas Hold'em](/ms/blog/holdem-hand-rankings) sebelum bermain. Tahu sama ada Flush mengalahkan Straight lebih penting daripada menghafal strategi lanjutan.

---

## Cara Main Texas Hold'em — Ringkasan Aliran untuk Pemula

Artikel ini memberi anda **versi pemula bagi aliran permainan** supaya anda boleh duduk di meja tanpa terkaku. Jika anda mahu panduan lebih mendalam jalan demi jalan dengan satu tangan penuh, urutan pertaruhan dan contoh, baca [urutan permainan Texas Hold'em](/ms/blog/holdem-game-order "thumb:/images/blog-holdem-game-flow.webp") selepas ini.

| Urutan | Peringkat | Apa yang pemula perlu ingat |
|------:|-------|--------------------------------|
| 1 | Butang pengedar | Menentukan blinds dan urutan giliran |
| 2 | Small Blind / Big Blind | Pertaruhan wajib sebelum kad diedarkan |
| 3 | Dua kad pemula | Kad peribadi yang hanya anda boleh lihat |
| 4 | Pre-flop | Giliran bermula di kiri Big Blind |
| 5 | Flop, turn, river | Kad komuniti keluar 3, kemudian 1, kemudian 1 |
| 6 | Showdown | Pemain yang tinggal membandingkan tangan 5 kad terbaik |

Untuk sesi pertama anda, idea utamanya mudah: ==**setiap kali street baru muncul, satu lagi pusingan pertaruhan berlaku.**==

![Infografik pandangan atas meja Texas Hold'em sebelum flop — setiap pemain memegang dua kad tertutup dan board masih kosong](/images/rules-step2-preflop.webp "Cara main Texas Hold'em langkah demi langkah — giliran pre-flop selepas blinds")

Untuk versi terperinci, teruskan dengan [urutan permainan Texas Hold'em dari blinds hingga showdown](/ms/blog/holdem-game-order).

---

## Berapa Ramai Pemain Boleh Main Texas Hold'em?

Texas Hold'em boleh dimainkan dengan **2 hingga 10 pemain** di satu meja. Anda tidak perlukan meja penuh untuk bermula — cukup sekurang-kurangnya dua orang.

| Pemain | Apa namanya | Nota untuk pemula |
|--------:|------------------|----------------|
| 2 | Heads-up | Pantas dan agresif; blinds diterbalikkan (lihat di bawah) |
| 3–6 | Short-handed (6-max) | Paling biasa dalam talian; lebih banyak tangan boleh dimainkan |
| 7–10 | Full ring (9-max) | Permainan klasik rumah/kasino; main lebih ketat, lebih banyak fold |

Untuk permainan rumah pertama, **4 hingga 6 pemain** ialah bilangan paling sesuai — cukup rancak untuk belajar, cukup sedikit supaya setiap tangan bergerak pantas.

==g:Bila hanya dua pemain yang tinggal (atau anda bermula dengan dua orang), ia dipanggil heads-up.== Peraturannya sama dengan satu kelainan: ==Button meletakkan Small Blind dan bertindak dahulu sebelum flop, tetapi Big Blind bertindak dahulu pada flop, turn dan river.== Penukaran ini memastikan permainan adil kerana jika tidak, seorang pemain akan sentiasa bertindak terakhir.

Bilangan pemain juga mengubah strategi anda — lebih ramai lawan, lebih besar kemungkinan seseorang memegang tangan kuat, jadi anda fold tangan yang lebih lemah. Jika anda sedang memilih antara permainan santai dan format kompetitif, lihat [cash game vs pertandingan (tournament)](/ms/blog/holdem-tournament-vs-cash-game).

---

## Siapa Mula Dulu dalam Texas Hold'em?

Ini antara soalan pemula paling kerap ditanya kerana jawapannya berubah selepas flop.

| Pusingan | Pemain pertama bertindak | Kenapa |
|------|---------------------|-----|
| Pre-flop | Pemain di kiri Big Blind | Blinds sudah meletakkan pertaruhan wajib |
| Flop | Pemain aktif pertama di kiri Button | Button bertindak terakhir selepas flop |
| Turn | Pemain aktif pertama di kiri Button | Urutan post-flop yang sama |
| River | Pemain aktif pertama di kiri Button | Urutan post-flop yang sama |

Cara mudah untuk ingat:

==**Sebelum flop, lihat ke kiri Big Blind. Selepas flop, lihat ke kiri Button.**==

Sebab itulah ==g:Button ialah posisi yang begitu berkuasa==. ==Button biasanya bertindak terakhir pada flop, turn dan river==, yang bermakna dia dapat melihat apa yang semua pemain lain buat terlebih dahulu.

---

## Cip Apa Anda Mula dalam Texas Hold'em?

Di kasino, nilai cip sudah ditetapkan. Dalam permainan rumah, susunan cip anda tentukan sendiri.

Untuk pemula, kekalkan nombor yang mudah. Anda tidak perlukan lima warna cip berbeza. Tiga atau empat warna sudah memadai.

| Jenis permainan rumah | Cadangan stack permulaan | Contoh blinds |
|:---|:---|:---:|
| Permainan santai untuk belajar | 100 cip | 1 / 2 |
| Rasa cash game standard di rumah | 200 cip | 1 / 2 atau 2 / 4 |
| Malam panjang gaya pertandingan | 1,000 hingga 2,000 cip | 10 / 20 |

Untuk permainan rumah pertama, susunan ini berfungsi dengan baik:

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Warna cip | Nilai | Cip setiap pemain | Jumlah nilai |
|:---|:---:|:---:|:---:|
| Putih | 1 | 20 | 20 |
| Merah | 5 | 16 | 80 |
| Biru | 25 | 4 | 100 |
| **Jumlah** | | **40 cip** | **200** |

</div>

Dengan 200 cip dan blinds 1/2, ==g:setiap pemain bermula dengan 100 Big Blind==. Itu memberi pemula ruang yang cukup untuk fold, call, raise dan melihat flop ==tanpa kehabisan cip terlalu cepat==.

---

## Berapa Banyak Wang Anda Mula dalam Texas Hold'em?

Jawapannya bergantung pada sama ada anda bermain dengan wang sebenar atau sekadar berlatih.

Jika anda masih belajar, gunakan cip permainan dahulu. Matlamatnya ialah memahami urutan permainan, bukan mencipta tekanan.

Jika anda bermain cash game kecil di rumah, pilih buy-in yang semua orang selesa untuk kalah. Struktur biasa untuk pemula ialah:

| Tahap blinds | Buy-in pemula | Nota |
|:---|:---:|:---|
| $0.01 / $0.02 | $2 hingga $5 | Terbaik untuk belajar dengan taruhan sebenar |
| $0.05 / $0.10 | $10 hingga $20 | Masih kecil, tetapi keputusan mula terasa nyata |
| $0.10 / $0.25 | $25 hingga $50 | Lebih sesuai selepas pemain faham peraturan |

==r:Jangan mula dengan taruhan yang buat orang gementar.== Pemula yang gementar tidak belajar lebih cepat. Mereka cuma fold terlalu banyak, call ikut emosi, atau bertengkar pasal peraturan.

Perhatikan bahawa buy-in **pertandingan (tournament)** berfungsi berbeza daripada cash game: anda bayar satu yuran masuk tetap, terima stack cip yang tidak boleh ditunaikan, dan bermain sehingga seorang pemain memiliki semuanya. Jika anda tidak pasti mana satu yang anda mahu, baca [cash game vs pertandingan](/ms/blog/holdem-tournament-vs-cash-game) dahulu.

Untuk latihan dalam talian, gunakan meja percuma sehingga anda boleh menerangkan urutan giliran tanpa berfikir. Kemudian naik taraf perlahan-lahan.

---

## No-Limit, Limit, atau Pot-Limit? Texas Hold'em Mana Yang Anda Main?

Bila orang sebut "Texas Hold'em", mereka hampir selalu maksudkan **No-Limit Hold'em (NLHE)** — format yang digunakan dalam WSOP Main Event dan hampir semua permainan rumah. Cara edar, blinds dan kedudukan tangan adalah sama dalam ketiga-tiga format. Satu-satunya yang berubah ialah **berapa banyak anda dibenarkan bertaruh.**

| Format | Berapa banyak boleh bertaruh? | Di mana anda akan jumpa |
|--------|-----------------------|---------------------|
| **No-Limit (NLHE)** | Apa-apa jumlah sehingga semua cip anda ("all-in") | Permainan lalai — permainan rumah, kebanyakan kasino, WSOP |
| **Fixed-Limit** | Kenaikan tetap sahaja (pertaruhan kecil ditetapkan, kemudian berganda pada turn/river) | Meja kasino lama; kurang perubahan besar |
| **Pot-Limit** | Sehingga saiz pot semasa | Jarang untuk Hold'em; standard untuk Omaha (PLO) |

Untuk pemula, ==g:anggap anda bermain No-Limit melainkan ada yang cakap sebaliknya.== No-Limit paling mudah untuk *difahami* (bertaruh berapa sahaja anda mahu) tetapi paling menghukum jika *dimainkan* dengan teruk, kerana satu call yang salah boleh melenyapkan seluruh stack anda. Sebab itulah bahagian pot odds di bawah dan [tindakan pertaruhan anda — cek, call, raise, fold](/ms/blog/holdem-betting-actions) sangat penting.

---

## Cara Mengedar Kad Texas Hold'em

Anda tidak perlu mengedar dengan sempurna seperti pengedar kasino, tetapi anda perlu ikut urutan yang kemas.

Ini urutan mengedar yang mesra pemula:

1. Kocok dek kad.
2. Letakkan butang pengedar.
3. Small Blind dan Big Blind meletakkan cip mereka.
4. Edar satu kad kepada setiap pemain mengikut arah jam, bermula di kiri Button.
5. Edar kad kedua kepada setiap pemain dengan cara yang sama.
6. Jalankan pusingan pertaruhan pre-flop.
7. Buang satu kad (burn), kemudian edar flop.
8. Buang satu kad, kemudian edar turn.
9. Buang satu kad, kemudian edar river.
10. Semasa showdown, bandingkan tangan lima kad terbaik.

Kad burn ialah kad teratas yang diletakkan tertutup sebelum flop, turn dan river. Ia membantu melindungi dek dan menjadi standard dalam poker langsung.

![Infografik pandangan atas meja Texas Hold'em ketika flop A♠ K♦ 8♥ diedarkan terbuka di tengah](/images/rules-step3-flop.webp "Cara mengedar Texas Hold'em — flop muncul selepas pusingan pertaruhan pre-flop")

Dalam permainan rumah yang santai, perkara paling penting ialah konsistensi. Umumkan setiap street, pastikan board jelas, dan jangan tergesa-gesa semasa seseorang sedang membuat keputusan.

---

## Posisi dalam Texas Hold'em — Kenapa Tempat Duduk Anda Mengubah Segalanya

Posisi bermaksud **bila anda bertindak dalam setiap pusingan pertaruhan**. Bertindak kemudian bermakna anda sudah melihat apa yang pemain lain buat — dan maklumat itu lebih bernilai daripada yang kebanyakan pemula sedari.

| Tempat duduk | Nama | Urutan pre-flop (9-max) | Perkara penting |
|------|------|----------------------|-----------|
| Posisi awal | UTG (Under the Gun) | Ke-1 | Paling ramai pemain belum bertindak — main paling ketat |
| Tengah | MP / HJ | Ke-4–Ke-5 | Julat sederhana, masih berhati-hati |
| Satu tempat di kanan BTN | CO (Cutoff) | Ke-6 | Tempat duduk kedua terbaik — luaskan julat |
| Terakhir post-flop | BTN (Button) | Ke-7 pre; **terakhir** post | **Tempat duduk terbaik** — bertindak terakhir pada setiap jalan post-flop |
| Kiri pengedar | SB (Small Blind) | Ke-8 pre, ke-1 post | Tempat duduk paling teruk — bertindak dahulu pada setiap jalan post-flop |
| Dua kiri pengedar | BB (Big Blind) | Terakhir pre (ke-9), ke-2 post | Ada pilihan untuk raise pre-flop; luar posisi post-flop |

==g:Button ialah tempat duduk paling menguntungkan di meja.== Anda bertindak terakhir pada flop, turn dan river — bermakna setiap pemain menunjukkan kekuatan tangan mereka sebelum anda membuat keputusan.

---

## Strategi Texas Hold'em untuk Pemula

Sebaik sahaja peraturan sudah difahami, strategi pertama anda sepatutnya membosankan dan kukuh.

==r:Kebanyakan pemula hilang cip kerana bermain terlalu banyak tangan, mengejar draw yang lemah, atau call semata-mata sebab ingin tahu.== Matlamat pertama anda bukan untuk bluff semua orang. ==g:Matlamat pertama anda ialah berhenti mendermakan cip.==

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Peraturan pemula | Kenapa ia berkesan |
|--------------|--------------|
| Main lebih sedikit tangan lemah | Tangan permulaan yang buruk mencipta keputusan sukar |
| Raise tangan kuat | Tangan kuat patut membina pot lebih besar |
| Fold lebih banyak dari posisi awal | Terlalu ramai pemain bertindak selepas anda |
| Hormati pertaruhan besar di river | Pemula jarang bluff dalam banyak situasi river |
| Jangan kejar setiap draw | Draw perlukan harga yang betul untuk diteruskan |

</div>

Tangan permulaan yang baik untuk pemula termasuk:

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

**Carta Tangan Permulaan Pemula — 4 Tahap**

| Tahap | Tangan | Bila nak main |
|------|-------|--------------|
| 🟥 **Premium — sentiasa raise** | AA, KK, QQ, JJ, AKs, AKo | Setiap posisi, setiap stack |
| 🟧 **Kuat — biasanya raise** | TT, 99, AQs, AQo, AJs, KQs | Kebanyakan posisi; lebih ketat dari UTG |
| 🟦 **Boleh dimainkan — posisi penting** | 88, 77, ATs, AJo, KJs, QJs, JTs | Utamakan posisi lewat (CO, BTN) |
| ⬜ **Fold secara lalai** | Semua yang lain sebagai pemula | Terutamanya dari posisi awal |

**Peraturan mudah diingat:** Mula dengan Pair TT ke atas, tambah AK dan AQ. Itu lebih kurang 5% tangan teratas. Luaskan perlahan-lahan bila anda semakin berpengalaman.

</div>

Dan sebelum anda boleh menggunakan tangan permulaan dengan betul, anda perlu tahu [tindakan pertaruhan anda — cek, call, raise, fold](/ms/blog/holdem-betting-actions).

---

## Pot Odds — Satu Konsep Matematik Yang Menjimatkan Wang Pemula

Anda tidak perlu jadi pakar matematik. Anda cuma perlu faham satu soalan sebelum call sesuatu pertaruhan: **adakah harga yang saya bayar berbaloi dengan peluang saya untuk menang?**

### Bagaimana pot odds berfungsi (satu contoh)

Bayangkan pot ialah **$100** dan lawan anda bertaruh **$20**. Untuk kekal dalam tangan, anda perlu call $20. Sebelum call anda, pot ialah **$120**; selepasnya, **$140**.

Pot odds anda ialah **$120 kepada $20**, iaitu **6 kepada 1**. Bermakna anda perlu menang sekurang-kurangnya **1 daripada 7 kali** (kira-kira 14%) untuk call ini menguntungkan.

### Peraturan 2 dan 4 — jalan pintas kebarangkalian segera

Bila anda memegang tangan draw (contohnya empat kad ke arah Flush), kira **outs** anda — kad-kad yang akan melengkapkan tangan anda.

<div style="background:rgba(255,248,210,0.10);border:1px solid rgba(255,240,180,0.35);border-radius:14px;padding:4px 20px 20px;margin:24px 0">

| Situasi | Formula | Contoh (9 outs Flush) |
|-----------|---------|------------------------|
| Pada **flop** (2 kad akan datang) | Outs × 4 | 9 × 4 = **~36% peluang** |
| Pada **turn** (1 kad akan datang) | Outs × 2 | 9 × 2 = **~18% peluang** |

</div>

Jika peratusan peluang anda untuk menang **lebih tinggi** daripada peratusan yang diperlukan pot odds, call itu menguntungkan. Jika lebih rendah, fold.

==r:Inilah konsep matematik paling penting untuk pemula. Kuasainya dan anda secara automatik akan mengelakkan separuh daripada kesilapan mahal yang dilakukan pemain baru.==

---

## Helaian Rujukan Peraturan Texas Hold'em Boleh Cetak

Ini versi ringkas untuk diletakkan di tepi meja. Anda boleh salin ke dalam nota, cetak halaman ini, atau guna PDF kongsi (dalam bahasa Inggeris) sebelum permainan rumah.

[Muat turun PDF Peraturan Texas Hold'em untuk Pemula](/downloads/texas-holdem-rules-for-beginners.pdf)

| Topik | Jawapan untuk pemula |
|------|-----------------|
| Kad setiap pemain | 2 kad pemula peribadi |
| Kad komuniti | 5 kad kongsi di atas board |
| Tangan terbaik | Tangan poker 5 kad terbaik |
| Pusingan pertaruhan | Pre-flop, flop, turn, river |
| Pertaruhan wajib | Small Blind dan Big Blind |
| Giliran pertama pre-flop | Kiri Big Blind |
| Giliran pertama post-flop | Pemain aktif pertama di kiri Button |
| Kelebihan Button | Biasanya bertindak terakhir selepas flop |
| Menang tanpa showdown | Semua pemain lain fold |
| Menang semasa showdown | Tangan 5 kad terbaik menang |

Kedudukan tangan asas dari paling kuat ke paling lemah (dengan kekerapan anda membentuk setiap tangan menggunakan 5 kad terbaik daripada 7):

| Kedudukan | Tangan | Kekerapan anggaran |
|-----:|------|-----------------------|
| 1 | Royal Flush | 0.003% — mungkin bertahun-tahun tanpa melihatnya |
| 2 | Straight Flush | 0.03% — sangat jarang |
| 3 | Four of a Kind | 0.17% — jarang; bertaruh besar-besaran |
| 4 | Full House | 2.6% — kuat dan cukup biasa untuk dijangka |
| 5 | Flush | 3.0% — pemenang kukuh pada kebanyakan board |
| 6 | Straight | 4.6% — terdedah kepada Flush/Full House |
| 7 | Three of a Kind | 4.8% — tangan bagus, tetapi board penting |
| 8 | Two Pair | 23.5% — tangan "kuat" paling biasa |
| 9 | Pair | 43.8% — tangan paling kerap di showdown |
| 10 | High Card | 17.4% — tangan paling lemah di showdown; biasanya menang hanya apabila pemain lain pun tidak dapat apa-apa |

Jika dua pemain mempunyai jenis tangan yang sama, bandingkan kad tertinggi yang berkaitan — itulah peraturan kicker dan pemutus seri. Jika lima kad terbaik betul-betul sama, pot dibahagikan.

---

## Kesilapan Biasa Pemula

### Kesilapan 1: Menyangka anda mesti guna kedua-dua kad pemula

Anda boleh guna kedua-duanya, satu, atau langsung tiada. Tangan lima kad terbaik menang.

### Kesilapan 2: Terlupa urutan giliran berubah

Pre-flop bermula di kiri Big Blind. Selepas flop, giliran bermula di kiri Button.

### Kesilapan 3: Call sebab "manalah tahu kad tu keluar"

Draw perlukan harga yang betul. Jika pot kecil dan pertaruhan besar, mengejar draw lemah biasanya mahal.

### Kesilapan 4: Main setiap Ace

A♣4♦ nampak menarik bagi pemula, tetapi Ace lemah selalunya membentuk One Pair kedua terbaik sahaja. Ace besar seperti AK dan AQ jauh lebih kuat.

### Kesilapan 5: Mengabaikan posisi

Tangan lebih mudah dimainkan bila anda bertindak kemudian. Jika anda yang pertama bertindak, anda perlukan tangan lebih kuat kerana semua pemain lain masih ada kelebihan maklumat.

---

:::readnext[Teruskan membaca]
/ms/blog/holdem-game-order | Urutan Permainan | /images/blog-holdem-game-flow.webp
/ms/blog/holdem-hand-rankings | Kedudukan Tangan Poker | /images/holdem-hand-rankings-hero.webp
:::

## Soalan Lazim

**Q. Bagaimana cara main Texas Hold'em langkah demi langkah?**

A. Letakkan blinds, edar dua kad pemula kepada setiap pemain, jalankan pertaruhan pre-flop, buka flop, turn dan river dengan pertaruhan selepas setiap street, kemudian bandingkan tangan lima kad terbaik semasa showdown.

**Q. Siapa mula dulu dalam Texas Hold'em?**

A. Pre-flop, pemain di kiri Big Blind bertindak dahulu. Selepas flop, pemain aktif pertama di kiri butang pengedar bertindak dahulu, dan Button biasanya bertindak terakhir.

**Q. Cip apa anda mula dalam Texas Hold'em?**

A. Untuk permainan rumah pemula, beri setiap pemain cip bernilai sekitar 200 dan guna blinds 1/2. Stack mudah ialah 20 cip bernilai 1, 16 cip bernilai 5, dan 4 cip bernilai 25.

**Q. Berapa banyak wang anda mula dalam Texas Hold'em?**

A. Untuk belajar, mulakan dengan cip permainan. Untuk permainan rumah wang sebenar yang kecil, guna buy-in yang semua orang selesa untuk kalah, seperti $2 hingga $5 pada blinds $0.01/$0.02.

**Q. Adakah wujud straight kecil dalam Texas Hold'em?**

A. Ya. A-2-3-4-5 ialah Straight yang sah dan sering dipanggil "wheel". Ia Straight paling rendah yang mungkin. Ace tidak boleh menyambung melepasi King: J-Q-K-A-2 bukan Straight. Ace hanya boleh bermain sebagai kad tertinggi (A-K-Q-J-10) atau terendah (A-2-3-4-5), tidak pernah di tengah.

**Q. Berapa banyak tangan permulaan yang mungkin dalam Texas Hold'em?**

A. Terdapat 1,326 kombinasi tepat dua kad, tetapi pemain biasanya mengumpulkannya kepada 169 jenis tangan permulaan, seperti AA, AK suited, atau 76 offsuit.

**Q. Apakah peraturan Texas Hold'em paling mudah — versi paling ringkas?**

A. Versi paling ringkas: setiap pemain dapat 2 kad peribadi. Lima kad kongsi dibuka dalam tiga peringkat (3, kemudian 1, kemudian 1). Anda bertaruh selepas setiap peringkat. Tangan 5 kad terbaik menggunakan apa-apa gabungan kad anda dan kad kongsi menang. Jika semua pemain lain fold, anda menang — tidak kira kad apa yang anda pegang.

**Q. Peraturan Texas Hold'em untuk pemula — apa maksud blinds?**

A. Dua pemain di kiri butang pengedar mesti meletakkan pertaruhan wajib sebelum kad diedarkan. Pemain pertama meletakkan Small Blind, pemain kedua meletakkan Big Blind (biasanya dua kali ganda). Pertaruhan ini memastikan sentiasa ada wang dalam pot untuk direbut. Setiap pemain lain mesti sekurang-kurangnya menyamai Big Blind untuk kekal dalam tangan.

**Q. Apakah versi pantas peraturan Texas Hold'em?**

A. Letak blinds → edar 2 kad pemula → pertaruhan pre-flop → buka 3 kad komuniti (flop) + pertaruhan → buka 1 kad (turn) + pertaruhan → buka 1 kad (river) + pertaruhan → tangan terbaik menang. Jumlah: empat pusingan pertaruhan, lima kad komuniti, satu pemenang — atau pot dikongsi, kalau lima kad terbaik betul-betul sama.

**Q. Berapa ramai pemain diperlukan untuk main Texas Hold'em?**

A. Anda perlukan sekurang-kurangnya 2 pemain dan boleh muat sehingga 10. Dengan tepat dua pemain ia dipanggil heads-up, dan blinds diterbalikkan — Button meletakkan Small Blind dan bertindak dahulu pre-flop, manakala Big Blind bertindak dahulu selepas flop. Untuk permainan rumah pertama, 4 hingga 6 pemain memastikan permainan pantas dan mudah diikuti.

**Q. Apa maksud no-limit dalam Texas Hold'em?**

A. No-Limit bermaksud anda boleh bertaruh apa-apa jumlah sehingga semua cip anda pada mana-mana pusingan pertaruhan — itulah langkah "all-in". Ia format lalai dan paling popular, termasuk WSOP Main Event. Limit Hold'em menghadkan setiap pertaruhan kepada saiz tetap, dan Pot-Limit menghadkan pertaruhan anda kepada saiz pot semasa.

**Q. Berapa lama satu tangan Texas Hold'em?**

A. Di meja live, satu tangan biasanya mengambil masa kira-kira 30 saat hingga 2 minit, walaupun pot besar dengan ramai pemain dan keputusan sukar boleh berlarutan beberapa minit. Sesi santai di rumah selama beberapa jam akan melalui berpuluh-puluh tangan, jadi tiada siapa perlu meluangkan satu malam penuh untuk satu edaran sahaja.

---

## Kesimpulan

Texas Hold'em lebih mudah dipelajari bila anda asingkan peraturan daripada strategi.

Pertama, ==belajar alirannya==: blinds, dua kad pemula, lima kad komuniti, empat pusingan pertaruhan, dan tangan lima kad terbaik. ==g:Kemudian belajar posisi, tangan permulaan dan keputusan pot asas.==

Sebagai langkah seterusnya, semak [kedudukan tangan Texas Hold'em](/ms/blog/holdem-hand-rankings), berlatih dengan [carta tangan permulaan](/hand-chart), dan guna [kalkulator odds poker](/calculator) bila anda mahu faham kenapa sesuatu call itu menguntungkan atau tidak.

---

## Artikel Berkaitan

<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/ms/blog/holdem-game-order" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Aliran Permainan</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Urutan Permainan Texas Hold'em</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Aliran penuh satu tangan — pre-flop hingga showdown dengan contoh sebenar</div>
  </a>
  <a href="/ms/blog/holdem-hand-rankings" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Kedudukan Tangan</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Kedudukan Tangan Poker — Terbaik ke Terlemah</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Kesemua 10 tangan dengan odds, contoh dan teka-teki board</div>
  </a>
  <a href="/ms/blog/holdem-betting-actions" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Pertaruhan</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Cek, Call, Raise, Fold Dijelaskan</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Bila guna setiap tindakan + jadual keputusan</div>
  </a>
  <a href="/ms/blog/holdem-showdown-rules" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Showdown</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Peraturan Showdown: Siapa Tunjuk Dulu?</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Last aggressor, muck, slow roll dan peraturan all-in</div>
  </a>
  <a href="/ms/blog/holdem-all-in-rules" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">All-In</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Peraturan All-In: Side Pot & Raise Semula</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Pot utama, side pot dan kelayakan raise semula</div>
  </a>
</div>
`.trim(),
};
