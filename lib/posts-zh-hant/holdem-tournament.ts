...
就這一個差別，從根本改變了籌碼價值、盲注壓力和策略。→ 現金桌 vs 錦標賽的完整差別，另有專篇（籌碼價值、升盲、ICM）。
...
### 階段 3——泡泡圈（Bubble）
最緊張的階段。再淘汰一個人，剩下所有人就**進錢圈**（ITM = In The Money，拿到獎金）。短籌碼會僵住，大籌碼會霸凌。這裡聰明地打，能在不贏任何一個底池的情況下加到真實的權益——泡泡圈值得一篇專屬指南。

### 階段 4——決賽桌（Final Table）
通常剩 6–9 人。每淘汰一個人，賠付就大幅跳升。這裡由 **ICM（獨立籌碼模型）**主導決策——籌碼 EV 和真實獎金 EV 明顯背離。
