...
![一个中等码量的锦标赛牌手在钱圈 bubble 上向大码量的 all in 弃牌，画面里有筹码和一道奖金阶梯——正是 ICM 压力把一次正常的跟注变成弃牌的那一刻](/images/holdem-icm-pressure.webp "ICM 压力：中等码量弃牌，因为出局会让他失去高于已锁定名次的那部分奖金权益")
...
A. chip EV 用赢来或输掉的筹码衡量一个决策；ICM 用真实的钱衡量。它们在锦标赛前期一致，逼近钱圈时分道扬镳，因为在那里出局会让你失去高于已锁定名次的那部分奖金权益。一次在 chip EV 里没问题的 coin-flip all in，在 ICM 下可能是个明确的弃牌。
...
A. 主要有三个。第一，*过度*套用 ICM——在 pay jump 还很远时就一路弃牌"去 ladder up"，而不是积累筹码。第二，作为 bubble 附近的中等码量跟得太宽，而那里恰恰是你风险溢价最高、出局会失去高于已锁定名次那部分权益的地方。第三，当你是 short stack 时不先跑一遍数字就答应 chip chop（或当你是 leader 时答应 ICM deal）。ICM 是个后期工具：用得太早，或在 final table 上无视它，两头都在漏钱。
