...
那就是我的 8-8-8-A-K：我的 J-10 和他的 5-2 都在打牌面带 A-K 踢脚的三条 8——五张完全相同，两手牌一亮出来就是 chop。极端情况是像 A♠ K♠ Q♠ J♠ 10♠（皇家同花顺）这样的牌面：任何底牌都改进不了它，所以 ==g:剩下的所有人一起 chop==。
...
**而在牌桌上最要紧的一点：你的牌只有正面亮出来才算赢。** 一手被盖掉的牌就是死牌，哪怕它本来能平分——就算你打的是公共牌，也仍然必须把底牌正面亮出来，否则就等于放弃了属于你的那一份底池（WSOP 现金局规则第 172 条；WSOP 锦标赛规则在第 75 条里重申了同一点）。谁先亮牌、整个顺序怎么走，都在[摊牌规则](/zh/blog/holdem-showdown-rules)里。
