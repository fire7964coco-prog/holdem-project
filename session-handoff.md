...
> 순위 1·2는 그날 닫았고 **3~5가 남았다** — START HERE를 그쪽으로 교체했다.
> 그날 경위는 `WORKLOG.md` 2026-08-13 (1)~(6). `/hands` 트랙은 **열었다가 같은 날 되돌려 종료**됐다.)
...
> 그 전에 닫을 수 있는 건 아래 1~3번이다.

### ▶▶ 🔴 이 세션의 본체 = **검수장 조치목록 순위3 — `wpt-australia` WRONG 14**

가장 큰 미해결 덩어리다. **사실 오류이고 독자가 돈·시간을 잘못 쓰는 축**이라 §13 정신상 최우선이다.

| 결함 | 정정 | 자리 |
|---|---|---|
| WC 시트 「$10.400 zusätzlich」 | 공식 **$15.000 · 1위 상금에서 공제**(추가 지급 아님) | **×4** |
| 새틀 구조 Step $255/$1.075 · 1-in-8 · 일정 | **공식 2026 일람에 부재** → Prime **1-in-6 $290** · 첫 새틀 **7/24** | 6자리 |
| 송금 참조란 | **Member ID and Surname** | 3자리 |
| SMS 코드 포맷 | (상세는 7/7 리포트) | 1 |

⚠ **검수장이 「1차 접근 차단」을 보고했다** — worldpokertour.com **전 경로 403** · pokerstars.com 404 ·
casinobarcelona.com 거부. 즉 **내가 1차로 재확인 못 하는 항목이 나온다.**
→ 그때는 **오늘 GGPoker 건과 같은 방식**으로 처리하라: 확인된 것만 고치고,
**확인 못 한 고유명·수치는 «주장 자체를 빼는»** 쪽으로(고유명을 다른 고유명으로 바꾸면 오류가 이동할 뿐이다).
그리고 **검수장에 회신**해 근거 URL을 요청할 것.

🔴 **분모를 지시서대로 믿지 마라.** 오늘 순위2가 그랬다 — 「de 1곳」 지시였는데 실측은 **7로케일**이었고
**EN 마스터가 발원지**였다. `wpt-australia`도 착수 전 **전 로케일 grep으로 직접 세라.**

### ▶ 그다음 — 순위4·5

- **순위4 `korea-poker-marathon` 러닝 실측 4** — 종목 **11KM**(글은 10K×3) · 접수마감 **7/10 + 6/26 조기마감**
  (글은 「15. Jul」×2). ⚠ **desc가 이미 마감된 대회 참가를 권한다**(RISKY #1) — SERP 필드라 함께 고칠 것.
...
| **검수장 재검수 원장** | 42편 2,988주장 · OK 2,558 · **WRONG 30 · RISKY 116 · 미해결 146** — 이 중 **순위1·2만 닫혔다** |
