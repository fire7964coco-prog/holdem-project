...
> 마지막 갱신 **2026-09-21** · HEAD = **`209b0d3e`** · **미커밋 0 · 배포·라이브 확인·IndexNow 200 완료.**
>
...
> · **09-21 (7)** **queue Q12-b 머지**(`7263564a` → `76569eef` · 회신 **MB-074**) — **MA-147 ① ⓐ~ⓖ 전건 종결.** 🔴 **25로케일이 «전부» 팟오즈 문턱을 «완성 확률»로 말하고 있었다**(D유형) · 🔴 **원장이 준 처방 문면 자체가 덜 정확했다**(부정관사 → 정관사). 정본 승격 **§3-N·§3-O**.
>
> 🔴 **되살리기 금지 열넷**: ① (not set)은 구조적 동작이다 — «유실»로 되돌리지 마라(보정 ② 유지 · `RULES_VERSION` 1)
...
1. **🧰 queue 다음 회차 — 사장님이 번호를 고른다.** 레인 창 첫 마디 = 「`HARDEN.md` 읽고 회차 Q<번호> 시작해」 · 회차 표 = `docs/harden-queue-진행.md` §1.
   🔴 **지금 레인에 `Q13`이 돌고 있다**(2026-09-21 착수 · 🧰 `Holdem-queue`). **헤드가 이 세션에서 할 일 = 「queue 회차 Q13 끝났어, 머지해」를 받는 것**이다 — 새 회차를 열지 마라.
   머지 절차 = `git log main..harden-queue` → 레인 보고(진행 파일 §1-Q13) 통독 → **헤드 독립 검산**(§13·산수·기계 대조) → 머지 → 빌드 → sitemap bump ↔ 편집 파일 수 **대조** → 푸시 → **라이브 확인은 «신규 있음 + 구 문면 사라짐» 둘 다**(폴링) → IndexNow `--urls`로 그 URL만 → MB 회신·정본 승격·§5 요청 이행 → WORKLOG → 핸드오프 3단 마감.
   🔴 **검수장 MA-147·148 — MA-147 ①은 Q12-a·Q12-b로 ✅ 전건 종결(09-21 (6)(7)).** Q13 = MA-147 ②③ glossary 묶음(21자리 · 6편 × 8로케일 + es 고유 3 · `holdem-fish` 「izquierda→derecha」는 **WRONG**).
   **Q13 다음 = Q14** MA-148 zh+zh-hant 정정 13 + EN-먼저 4(c-bet 직답 「四到五成」→「約 30–45%」는 **5로케일**).
...
- **우편함 — 회신 대기 중**: **MB-072**(09-21 · MA-158·159·163 이행 + MA-147·148 미이행 통지 · **요청 3**: ⓐ ms 66 «정밀형» 오판 정정 + 원장에 «분배 표지» 축 신설 ⓑ ⭐배지↔종료 공존 0 회귀 앵커 ⓒ 147-②ⓐ 앵커를 「In every one」으로 수정). 그 밖 미처리는 «→검수장»(MB-033~062 + S-022·023). 🔴 MB-054는 **MB-053 ②(상금풀 $87,568,080)를 뒤집는 통지**. 🪶 `check:mailbox` 기존 흠 둘(미수정): «MB-054 파이프 9» 🔴 · «MB-067 열 결손» 🟠(남의 회차 행이라 손대지 않았다).
