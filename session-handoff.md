...
| Step | 무엇 | 산출물 | 상태 |
|---|---|---|---|
| **0** | **기준선 스냅샷** | WORKLOG grep ✅ · `market-profile/zh-hant.md`·`zh-hant-tag-volumes.md`·rakko-playbook 정독 ✅ · 🔴 **`gsc:lang`/`gsc:page` 스냅샷은 아직** | 🟡 부분 |
| **2** | **키워드·SERP 실측** 🔴 **중단 게이트** | ✅ **완료 → `docs/keyword-bank/zh-hant-tournament.md`.** 자동완성 453종 발굴 + DFS 볼륨 155종 + 모바일 SERP 6쿼리. **게이트가 실제로 작동해 전제가 뒤집혔다 — 아래 «Step 2 판정» 절** | ✅ |
| **1** | **데이터 층** 🔴 **중단 게이트** | `ctpclub.app` Playwright 파싱 · TW 12건 검증 · buyin 5건 · 중문 대회명. 🔴 **Step 2 결과로 «타이베이 12건»의 값어치가 재평가됐다 — 착수 전 아래 판정 먼저** | ⏸ 보류 |
...
| **5** | **배포·관측** | ✅ 푸시 완료(`9800a1e0`) · ⬜ **8주 GSC 추적**(기준선은 아래) | 🟡 |
