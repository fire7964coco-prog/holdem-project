...
> 마지막 갱신 2026-09-17 (본체 HEAD = es 계산기 회차 커밋 `f839bb1e` + 마감 문서 · `/es/calculator` 재조준 배포(Equity 탭·quickRef 6표·FAQ 18(TDA 2024 Rule 5 고유) · MB-061) · 다음 = **pt 계산기 회차** · 미커밋 0).

## ▶ 다음 할 일 (사장님이 고른다)

1. 🧮 **pt 계산기 랜딩 회차 — 첫 마디 「핸드오프 읽고 pt 계산기 회차 시작해」** (🔴 **Opus 창에서 연다** · 모델 분담 = 브리프 §3-D(es 회차도 Fable 없이 완주 — 헤드·집필·렌즈 3종+2차 교열 Opus · 구조 추출 sonnet 2 · 전사 = `check:calc-parity`). 절차 = `docs/harden-brief/calculator-landing-rewrite.md` **§3-A**를 pt로 + **§3-B·§3-C·§3-E·§3-F가 남긴 것**(🔴 `npm run check:calc-parity -- pt`를 렌즈 전에 · 탭 이름≠페이지 이름 · 방위어 금지 · FAQ 질문 뒤 명사 덧붙이기 금지 · 제목 앞머리는 실측 머리어(CTA «Calculadora de probabilidades»는 볼륨 0일 수 있다) · 🔴 **ICM↔IMC 약어 오염을 자동완성으로 먼저**(pt도 BMI = IMC) · 영문 «icm calculator»가 자국어보다 클 수 있다 · board 역어가 table(mesa)과 충돌하는지 · 형제 글 FAQ «의도» 충돌 grep · captionMw 관사 금지 · «call LP»는 «레이즈에 콜»로 명시 · fold equity 역어는 코퍼스 grep · TDA는 2024판). 🔴 **권역부터 판정**(pt-BR 중심인지 `docs/`의 pt 트랙 문서 확인 · 볼륨 location = Brazil 2076 · `language_code`는 볼륨을 안 나눈다). 🔴 합법성 축을 열지 마라 — TDA 같은 «규정» 사실만. 선례 = `ja/zh/zh-hant/es-calculator.md` · EN 정본 = `en-calculator.md`. 수치 EN 그대로(§13) · 정의형 FAQ 금지 · 딜러 렌즈 12항 동반. 끝나면 §0 pt ✅ + 다음 로케일(de)을 이 자리에.
...
- 우편함 미처리는 전부 «→검수장»(MB-033~061 + S-022·023). 본체 몫 0. 검수장 회신이 오면 MA 요청부터 판정. 🔴 MB-054는 **MB-053 ②(상금풀 $87,568,080)를 뒤집는 통지** · MB-055는 **계산기 랜딩 10로케일 검수 청구**(ja·en·zh·zh-hant·es 몫은 MB-057·058·059·060·061 해시로 대체) · 🪶 `check:mailbox`의 «MB-054 파이프 9» 🔴는 기존 파손(09-17 확인 · 미수정).
