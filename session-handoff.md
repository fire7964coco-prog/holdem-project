...
- `/blog?tag=` URL이 ~200개 생기지만 전부 canonical `/blog`라 중복 색인은 안 난다.
  태그별 실제 목록 페이지가 필요해지면 그때 라우트로 승격하면 된다.
- 다국어 글(`intl-blog-post-client`)에는 브레드크럼·태그 링크를 **아직 안 넣었다**(한국어만).
