...
A. 在一个抽水的局里你没法完全躲开抽水，但你能把它压小。尽量拿到最好的 rakeback；挑封顶对牌手友好的房。往上打级别也能压低抽水——固定封顶占每个底池的比例更小——但这一步只在你的资金扛得住波动、*而且*你面对更硬的牌桌仍然有优势时才走；否则对手从你身上拿走的，会远比抽水多。多打大底池、少打一堆到封顶的小底池，避开极短台——每小时的手数更多、交盲注也更频繁，被抽水的底池你会参与得多得多，偏向高级别的计时费（time rake）局，这些都能拉低你的实际抽水。至于完全不抽水的家庭局，那是最便宜的扑克了。
