...
> ✅ **Q2-b로 실행 완료(2026-09-11).** 실행 중 이 브리프가 **틀렸던 자리**는 진행 파일 §1-Q2-b에 있다 —
> ① es 세 번째 예외는 `buscar trío`가 아니라 `set-mining` ② 슬롯 ⑦에 통화 기호를 넣으면 de(€)와 어긋난다
> ③ 슬롯 ②의 `15–20×`는 분모 없이는 D유형 ④ §4의 영문 라벨은 §5 언어 중립 규칙에 따라 기호로 대체됐다.
> **이 브리프의 §4 사양을 그대로 다시 쓰지 마라 — 산출물은 `scripts/gen-*.html` 8개다.**
