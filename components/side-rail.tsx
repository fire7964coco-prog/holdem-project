...
export const HUB_PAGES = [
  ...
  /**
   * ★2026-08-28 추가 — `/ranking`이 이 레일에 **없었다.** `/solver`가 08-14까지 그랬던 것과 같은 상태다.
   *   이 페이지는 「홀덤사이트」(28일 2,970노출·클릭 27)의 답을 가진 자리인데, 정작 구글은 홈을
   *   골라 띄우고 있었고(홈 CTR 0.4% vs /ranking 1.5%) 홈에서 여기로 오는 링크는 **0개**였다.
   *   같은 날 홈 피드에 티저 카드를 넣었으나 그 카드는 `lg:hidden` 컨테이너라 **모바일에서만** 보인다.
   *   이 레일은 홈과 모든 허브에 함께 깔리므로 여기 한 줄이 데스크톱 진입점이자 전역 앵커다.
   *   ⚠ 라벨에 공백 없는 「홀덤사이트」를 담는다(2,970 vs 「홀덤 사이트」 287 — 실측 지배 표기).
   *   ⚠ 「추천/TOP5」로 바꾸지 마라 — 이 페이지는 JSON-LD에 "정보 제공 목적, 순위·보증 아님"을 선언한다.
   */
  { href: "/ranking",         icon: "🛡️", label: "홀덤사이트 고르기" },
