...
> **快速回答**
> 因为奖金是「头重脚轻」的，而且排在你下面的那几档已经锁定了。用来活下来的头几个筹码值很多，用来争冠军的最后几个筹码值得少。所以一个握着一半筹码的人拿不到一半奖池——他不可能比第一名更好，却仍然可能出局。
...
> **快速回答**
> chip EV 拿筹码当尺子，ICM（也叫 $EV）拿真实奖金当尺子。锦标赛前期两者一致；逼近钱圈和决赛桌时分道扬镳——一次在 chip EV 里没问题的 coin flip，到了 ICM 下往往该直接弃掉，因为出局丢的是已锁定的奖金。
...
经典的冲突是*跟*一次边缘的 all in。在 chip EV 里，为一个大底池打一次 coin flip 可能没问题、甚至很好——你赢的筹码和输的一样多。在 ICM 里它可能是个明确的 ==弃牌==，因为出局会让你失去那些拿不回来的、已锁定的奖金，而你要赢的那些筹码，值得比面值更少。
...
> **快速回答**
> ICM 在逼近 pay jump 时最重要：钱圈、决赛桌 bubble、卫星赛，风险溢价在这几处到峰值。pay jump 还很远时最不重要：前中期、深码小盲、争冠 heads-up，这时打 chip EV、专心累积筹码。用得太早和该用时无视，两头都在漏钱。
