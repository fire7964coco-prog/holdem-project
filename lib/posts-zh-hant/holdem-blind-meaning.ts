...
> **快速解答**
> 盲注是在牌發下來之前就被迫下的注——你什麼都還沒看到就先押，所以叫「盲下」。每手牌固定兩個座位付：莊家鈕左邊緊挨著的小盲和大盲。它存在的理由只有一個，就是讓牌局不至於停下來。

**盲注（Blind）** 這個名字就來自「盲下」——牌還沒看就得先押。那為什麼一定要逼人先付這筆錢？==g:為了讓牌局活下去。== ==r:沒有盲注的話，每個人都可以一直蓋牌、無止盡地枯等一手大牌==；底池永遠是空的，詐唬也變得毫無意義，整場牌局就停擺了。==g:每手牌兩個強制注，保證檯面上永遠有東西可以爭。==
...
各家撲克室的規矩不一樣，所以坐下前問荷官一句最穩。常見的兩種做法是——**補下一個大盲** 立刻加入，或等盲注輪到你的座位；也有撲克室不要求無限注／底池限注新入座的玩家補盲（WSOP 現金局（Live Action）規則 104.a 就是這樣），坐下前先問荷官一句最穩。如果你離桌一下，==**漏掉了你的盲注**==，多數撲克室會要你回座時補下才能重新加入——漏掉的大盲當成一個有效的活注，漏掉的小盲則是一枚 ==**死籌碼**==、直接被收進底池。線上牌桌會自動幫你處理這一切。
