...
export const POST: Post = {
  ...
  readTime: "14分",
  emoji: "🃏",
  tags: ["ポーカー 役", "ポーカー 役 強さ", "テキサスホールデム 役", "ポーカー 役 順位", "キッカー", "ポーカー 引き分け", "役 一覧", "ホールデム 役"],
  content: `
...
フォーカード同士なら、4枚の数字が高いほうが勝ちます。4枚がボードに並んでいる場合は**キッカー**で決まり、エースのキッカーが最強です。
...
**キッカー**とは、役には含まれないけれど引き分けを決めるために使うカードのこと。A-A-K と A-A-Q なら、どちらもエースのペアなので、Kのキッカーが勝ちます。だからこそ上級者は「ペアになったか」だけでなく、高いカードの「質」を気にするのです。
...
→ K♥ が5枚目のハートなので「フラッシュ」と思いがち。でも並びを見ると、**K♥ 10♥ 9♥ 8♥ 7♥** は同じハートの連続5枚——**Kハイのストレートフラッシュ**、第2位の役です。フラッシュのカードが連続していないか、決めつける前に必ず確認を。
...
→ A♥ がボードの A♠ とペアになり、**KKK + AA のフルハウス**。ただし相手もエースを持っていれば**同じフルハウスで引き分け**。相手がエースを持たず小さいペアだけなら、あなたのボートが勝ちます。教訓:ボードが仕事の大半をしているとき、あなたの手はカード1枚分しか強くないことが多い。
...
ここから一つの大きな例外も理解できます。2〜5を抜いた**シックスプラス(ショートデッキ)ホールデム**では、フラッシュがフルハウスより作りにくくなる——だからこの形式では**フラッシュがフルハウスに勝ちます**。確率が変われば順番も変わるのです。ゲーム別の違いは後述します。
...
**1. まずスート** — 同じスートが3枚以上ボードにある? あればフラッシュの可能性。自分のスートを確認。

**2. 次に連続性** — 近い数字(8-9-10など)が並んでいる? あればストレートが生きている。

**3. 最後にペア** — ボードがペアになっている? なればフルハウスやフォーカードが射程に入り、自分のフラッシュやストレートが危ない。
