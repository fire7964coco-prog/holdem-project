...
> 🧰 **queue 상태(09-13 (27))**: Q1·Q2-a·Q2-b·Q2-c·Q4-a·Q7-a·Q5-a ✅ · **Q5-b ✅**(EN 12편 + 8로케일 · 렌즈 63/반영 54 · 머지 `33acbaa1`). 남은 회차 = **Q6**(로케일 표기·ja 잔여) · **Q7-b**(직답 포인터·개수 게이트 + 🔴 `check:stamp` `.trim()` 오탐 튜닝 + `check:structure` «같은 파일 안 링크 개수» 신호) · **Q8-a/b**(pt·id·de 용어 분열) · 🏆 **Q4-b**(대회 묶음). 🆕 **신규 회차 재료** = `en-first-queue` §1 ㉒-c(직답 리드 4편) · ㉙(이미지 1장 제작). 다음 권장 = **Q7-b**(게이트 둘 다 실증된 구멍이다).
> 직전 회차 = **queue 회차 Q5-b 머지**(09-13 (27) · `33acbaa1` · EN-먼저 묶음 II 후반 — wpt 티켓 분포 표 · bubble 126.b/RP-8-A · bad-beat §13 예시 — 경위 WORKLOG).
...
| 🟠 **ja 회차 10·11이 헤드에 남긴 판정 2**(`harden-ja-진행` §5-H 3·5) — pot-odds L194 카드 행 中黒 이탈 · showdown 「マック 마너/손해」 FAQ 신설 여부(`ポーカー マック` 170 중 マナー·デメリット ≈40). 🟢 bubble 「複数シート」는 09-11 (8)에 tldr까지 닫았다(J#9). | 소 |
...
| 발신 | 무엇 | 상태 |
|---|---|---|
| **MB-033~049** | 🆕 09-11~13 발신 17건 회신 대기 — 🟢 **MB-049 = 솔버 S-024 이행 완료 회신**(랜딩 기능표 10로케일 40줄 · 커밋 `692c50cf` · 확인 칸 S-024) · MB-044~048 = queue 회차 Q4-a·Q2-c·Q7-a·Q5-a 통지 + 사장님 확인 2건 확정. 이하 구 목록: — MB-043 queue Q2-b(EN 7편 이미지 10자리 + 캡션 4 · 새 파일명 10 앵커 · F유형 2 정정) · MB-042 ja 회차 14(앵커 · 🔴 소급 회차 종료 결재 §1-D · EN 변경 0 · 예고 2) · MB-041 queue Q1(EN shc 변경 통지 · 요청 1 = M-102 §6 「€30+6」 0-감시 앵커를 capt-graz 카드로 좁혀 달라 · 「1,326」·「58」 신규 앵커) · MB-040 ja 회차 13(앵커 · EN 변경 0 · 예고 4 · 형제 개수 «결함 아님» 통지) · MB-033 EN-먼저 18건 배포(앵커 갱신 · 요청 1 = ar 원장 11건 «EN 완료·ar 미전파» 표시) · MB-034 ja 회차 11(앵커) · MB-035 회귀 🔴 2 재실행(재앵커 요청) · MB-036 S-013 ACK(솔버 · 라이브 대조 후 닫힘) · MB-037 MA-130·131 이행(앵커 · X-1 미결) · MB-038 MA-136 이행 · MB-039 ja 회차 12(앵커 · 훅 결함 통지). 전문 = `mailbox/out-본체.md` | 검수장 · 솔버 |
...
### 렌더·게이트·코퍼스
- 🔴 **`check:stamp`가 `.trim()` 한 줄을 «실질 변경»으로 센다 — 첫 실행 🔴 48건이 전건 오탐이었다**(2026-09-13 헤드 실측). Q5-a가 `content` 템플릿 끝을 `` ` `` → `` `.trim(), ``로 전 코퍼스 통일했는데, 그 48파일은 **diff가 그 한 줄뿐**이라 §1-C 「기계적 변경은 `updated`를 안 올린다」에 해당한다. 🔴 **이 48건을 «스탬프 미갱신»으로 읽고 고치지 마라** — 튜닝(분류기에 «코드 한 줄» 부류 추가)은 queue **Q7-b**. 판정 재현 = 파일별 diff에서 `+`/`-` 줄이 `` `, ``·`` `.trim(), ``뿐인지 보면 된다
- 🟢 **게이트 6종 신설 완료(queue Q7-a · 2026-09-12)** — `check:stamp`·`check:mailbox`·`check:tournament-numbers`·`check:image-refs`·`check:pdf-page`·`check:number-format`(셀프테스트 **126/126**). 위 «게이트 부재» 4건은 이것으로 닫혔다. 🔴 **쓰는 규율은 `docs/hardening-protocol.md` §7-F 「게이트(C)」 행이 정본** — 특히 **발신함에 행을 쓰기 «전»에 `npm run check:mailbox`**.
