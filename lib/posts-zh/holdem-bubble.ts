...
- ==**卫星赛钱圈（satellite bubble）**== ——那个另类。每一个晋级席位发的奖 ==完全一样==。一旦你的码量大到足够安全，多出来的筹码就*一文不值*——所以正确打法几乎变成了正常 bubble 的反面（下面「弃 A」那条规则会细讲）。
...
> **快速回答**
> 在送出多个同等席位的卫星赛里，每一个席位的奖都一样，所以一旦你的码量在数学上安全了，就什么都弃——包括口袋 A。（只送一个席位的 winner-take-all 卫星赛不适用：那种要按 chip EV 去争第一。）赢下一手只是拿到一个早已锁定的席位，输了却会出局：没有回报，只有风险。唯一的例外：你覆盖着对面那个 short stack，他一出局就打爆 bubble，而且就算这个底池输了你的席位也还稳。

这是扑克里最反直觉的一个局面，而它是对的。三条规矩：
