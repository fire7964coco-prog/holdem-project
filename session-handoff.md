...
### 🟢 직전 회차 (2026-09-10 (3) · Opus 헤드) — 구조 결손 A축 + 결재 이행 + 게이트 3종
`f1a88b65`→`048cebb0` 배포·라이브 확인 · 빌드 708p exit 0. 경위 = WORKLOG 「2026-09-10 (2)」·「(3)」.
**A축 실질 결손 11 → 2편(ar만)** · 결재 15건 전건 승격(`settled-decisions` §1-B) · 결재 11·14 이행.
게이트 = `check:structure` **링크 정규식 수리**(썸네일 링크 미검사 · link 44→**55**로 정직해짐) +
**예외 등재 ALLOW** + **«FAQ 절 이탈» 검사** 신설(셀프테스트 7→18).
🔴 **렌즈가 자체 0건을 뒤집었다** — 게이트 전부 통과인데 렌즈 2종이 독립으로 이식 사고를 잡았다.

### 🔴 1순위 — 구조 결손 전파 회차 (**A축 완료 · B/C축 잔여**)

`npm run check:structure`가 실증한 «EN 회차가 로케일에 통째로 안 간» 자리다. **본체는 `docs/en-first-queue.md` §0.**

| 축 | 실측(2026-09-10 (3) 갱신) | 대표 |
|---|---|---|
| **A** h2·row | 🟢 **11편 → 2편** — 남은 건 `ar/holdem-hand-rankings`·`ar/holdem-tournament-vs-cash-game`뿐이고 **아래 ar 판단에 걸려 있다** | 닫은 것 = tiebreak 무늬 절(de·id·pt) · zh-hant hand-rankings 3절 · es glossary 3항목 |
| **B** faq | 🔴 **~55편** | de `drawing-odds` 11문 중 3문 부재 · **id·pt `hand-rankings` 12 ↔ EN 20** · pt가 22편으로 최다 |
| **C** link | 🔴 **55편**(게이트 수리 후 정직한 수치 · 종전 44는 썸네일 링크 미검사) | `3bet-pot-cbet`(de·id·ja) · `low-board-check-raise`(de·es·id) |
...
| 실행 항목 | 어디서 |
|---|---|
| **ar** — 🔴 **아래 «사장님 판단 1건»에 걸려 있다.** 결재 40의 뜻을 정해야 규모가 정해진다 | 판단 후 |
...
### 🔴 사장님 판단 1건 — ar을 핵심으로 올린 뜻이 무엇인가

결재 40으로 ar이 핵심이 됐는데, **ar은 8편뿐이다**(핵심 나머지는 42~60편). 실측으로 분해했다:

> **ar 링크 결손 32건 중 31건이 «대상 글이 ar에 없어서» 구조적으로 불가능하다.** 실제로 걸 수 있는 건 1건.
> 즉 게이트가 낸 ar 7편은 **결함이 아니라 코퍼스 규모의 결과**다.

| 안 | 뜻 | 규모 |
|---|---|---|
| **ⓐ** 있는 8편만 EN 구조에 맞춘다 | 링크 결손은 «불가능»으로 등재해 닫고, faq/h2/row만 채운다 | 작다(1~2회차) |
| **ⓑ** ar도 42편 규격으로 간다 | 34편 신규 번역·발행 | 크다(별도 트랙) |

🪶 본체 권고 = **ⓐ 먼저.** ⓑ는 «아랍어 수요 실측»이 선행돼야 하고, 그건 키워드 회차다.
