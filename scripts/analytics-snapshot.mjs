 ...
 *   ② landingPage="(not set)" 제외 — user_engagement는 찍히는데 page_view가 0이라
 *      GA가 «참여 0»으로 센다. 제외는 계속 옳다. 단 **근거가 2026-09-20에 교체됐다**:
 *      🔴 옛 근거 «page_view만 유실된 진짜 사람» = 틀렸다. 유실이 아니라 **중복 계수**다.
 *         GA4는 30분 무활동 뒤 같은 탭에서 상호작용이 재개되면 새 세션을 연다. 새 문서
 *         로드가 없으니 page_view가 없고, 그래서 랜딩이 (not set)이 된다. 즉 **한 번의
 *         방문이 30분 경계로 쪼개진 «두 번째 조각»**이라 빼는 게 맞다.
 *      실측 근거(2026-09-20 · 창 08-18~09-14 · `docs/ga-notset-diagnosis-2026-09-20.md`):
 *        · 신규 2 / 재방문 195 / (not set) 57 → **신규가 0.8%**인데 사이트 전체는 75.7%다.
 *          태그 로딩 지연이 원인이면 캐시가 없는 «신규»에 몰려야 한다 → 그 가설은 기각.
 *        · 이벤트 구성 = session_start 255 · user_engagement 261 · scroll 14 · click 8,
 *          page_view **0**. 데스크톱 71%(전체 47%) — 탭을 오래 열어 두는 쪽이다.
 *        · 시계열 평탄(계단 없음) → 배포로 생긴 결함 아님.
 *      🔴 «2.3배 폭증»도 오독이었다 — 오가닉 대비 5.56% → 6.53%(+0.97p)로, 트래픽이
 *         1.93배 는 것에 거의 비례한다. **분모를 보고 판정하라.**
 *      ⚠ 보정 동작은 안 바뀌었으므로 RULES_VERSION은 1 그대로다(과거 행과 비교 유지).
