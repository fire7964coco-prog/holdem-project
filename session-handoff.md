...
### 🔴🔴 다음 세션 착수 확정 — «모바일 속도 부채: 폭<750px 15장» (사장님 지시 2026-08-30)

> 축어: *「수고했어 다음새션에서는 이미지 트랙 다음 1순위->이거 하자」*
> ([[round-plan-must-be-written-down]] 규율대로 **지시받은 그 턴에 적는다.**)

**무엇** — `public/images` 중 **폭 750px 미만 15장(합계 713KB)**. 이 그룹만 `next/image`
최적화를 못 타서 **디스크 크기가 곧 모바일 전송량**이다.

| 글 | 모바일이 통째로 받는 양 | 장수 |
|---|---|---|
| `apt-incheon-2026-guide` | **405KB** | 10 |
| `korea-poker-marathon-2026` 계열 | **309KB** | 5 |

🔴 **품질 조정으로는 못 고친다 — 이미 실측으로 막혔다.** 이 사진들은 이미 q45~50으로 강압축돼
있어서 **AVIF로 변환하면 오히려 62% 커진다**(72.9KB → 114.6KB 등). 그래서 next/image가
원본을 그대로 내보낸다. **q를 더 낮추는 것도 금지** — 화질만 깎이고 용량은 거의 안 준다
(2026-08-07에 9장을 q50까지 내려 총 −7%에 그쳐 전부 되돌린 선례. `IMAGE_OPTIMIZATION.md`).

**착수 순서**
| 단계 | 할 일 |
|---|---|
| 1 | `npm run check:images` — 현황·목록 재확인(수치를 기억으로 쓰지 마라) |
| 2 | 🔴 **각 장을 Read로 열어 «크롭 가능한가» 육안 판정** — 정보 없는 영역(빈 하늘·바닥·여백)이 있으면 크롭이 정답이다. **화질을 올리면서 용량이 준다**(선례: `korea-poker-marathon-2026-hangang2` 하단 38% 크롭 → 83.9KB q45실패 → **63.3KB q78**) |
| 3 | 크롭할 게 없는 장은 **폭 1200px 확대가 실익이 있는지 «측정»하라** — 업스케일은 화질을 못 올리지만 next/image 경로를 타게 만든다. 🔴 **가정하지 말고 640px AVIF 전송량을 재서 판정**(커지면 하지 마라) |
| 4 | 원본 사진이 따로 있으면 **더 큰 원본으로 교체가 정석**(사장님께 확인) |
| 5 | `npm run check:images` 🔴 0 → 빌드 → 배포 |

⚠ **이 사진들은 생성물이 아니라 현장 실사**로 보인다(펜션·음식·거리). **크롭 판단은 육안으로**,
피사체를 자르지 마라. 손대기 전 원본을 `git show`로 확보해 두면 되돌리기 쉽다.
...
| 🆕 🟠 **구형 워드마크 18장 = URL 부재**(2026-07-26 규율 「로고 + holdemmaster.com」 미달) — 인포그래픽 **12** + 실사 **6** | 🔴 **덮어쓰기 금지 — 인포그래픽 12장은 워드마크가 디자인에 구워져 있어 URL을 얹으면 겹친다**(apt 2장에서 실증: 새 워터마크가 구형과 겹쳐 판독 불가 → 되돌렸다). 처방은 **HTML 원본 수정 + Playwright 재촬영**(§9-1)이라 별건 회차. 실사 6장은 위치를 피해 URL만 추가 가능(`button-position` 선례 참조) |
...
| 🆕 🔴 **모바일 속도 부채 — 「폭 750px 미만」 15장·713KB**(`apt-incheon` 405KB · `korea-poker-marathon` 계열 309KB) | 🔴 **품질 조정으로 못 고친다** — 이들은 AVIF 변환 시 **오히려 62% 커져서**(이미 q45~50 강압축 세로 사진) next/image가 **원본을 그대로 내보낸다** = 디스크가 곧 전송량. 처방은 ①정보 없는 영역 **크롭**(화질을 올리면서 용량이 준다) ②**1200px 원본으로 교체**. 착수 전 `npm run check:images`로 현황부터. 사장님이 «모바일 속도»를 지목하셨으니 **이미지 트랙 다음 1순위** |
