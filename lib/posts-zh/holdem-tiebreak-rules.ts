...
> **快速回答**
> 如果你的底牌破不了公共牌自己的最佳五张，它们就不参与——你在"打公牌"（play the board）。当所有人都是这样，底池平分；只要有一个人能用底牌把牌面延伸成更大的牌，他就独赢。
...
A. 先比大对，再比小对，最后比那唯一一张踢脚牌——所以 A 带 3 赢过 K 带 Q，哪怕第二对小得多。让人栽跟头的是像 K-K-9-9-5 这样的双对牌面：除非有人拿着 K、9、口袋 5-5，或比 9 大的口袋对子，否则每个人都是同样的两对，整手牌就坍缩成一张踢脚牌，全桌最大的那张底牌拿走底池——要是没人的底牌能压过牌面上的 5，那就全员打公牌，平分底池。两对正好只带一张踢脚牌，绝不会有两张。

**Q. 德州扑克里第五张牌重要吗？**

A. 重要——而且这是玩家把一个自以为稳赢的底池输掉的最常见方式。把整个底池压在最后一张牌上的典型情形有这么几种：一对的第三张踢脚牌、两对那唯一一张踢脚牌、同花最小的那张、以及牌面四条旁边的那张单张。只有当公共牌本身大过你手里那张单张，它才不再重要——有时是整副公共牌都比你大、你的底牌整个掉出这手牌（打公牌），有时是只有一张底牌起作用、另一张永远轮不到：A♠ 3♠ 对 A♦ 2♦、公共牌 A♥ K♣ Q♦ J♠ 9♥ 就是平分，两人都是 A-A-K-Q-J。
...
A. 会，但不常见。真正的平手（平局，英文 tie / split pot）只在两名及以上玩家最佳五张点数完全相同时发生——最常见的就是公共牌自己是最强牌（"打公牌"），或是一副谁的底牌都改善不了的共用顺子／同花。这时底池对半分。踢脚牌存在的意义，恰恰就是在绝大多数"差点平分"的局面变成真平分之前，把胜负先分出来。
