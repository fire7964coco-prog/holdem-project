...
## ▶▶▶▶▶▶▶▶▶ 새 세션 START HERE (2026-07-31 밤 — 확률 클러스터 완료 / 전 클러스터 30/30 무결)
...
### 0-C. ⚠ 후속 작업 (이번 세션에서 남긴 것)

1. **`holdem-probability` 본문 이미지가 4장뿐이다.** 옛 인포그래픽 10장을 폐기했고
   새로 만든 건 hero·potodds·outs·starting 4장이다. 20분짜리 긴 글이라 **중간 가독성용
   실사 이미지 2~3장을 보강하면 좋다**(§9-1: 실사가 기본). 급하진 않다.
2. **다국어 영향 확인 필요** — 팟오즈 수치(하프팟 25%·풀팟 33%)와 Rule of 4 조건은
   §13 불변 항목이라 **번역본에도 같은 오류가 있을 가능성이 높다.**
   `lib/posts-{en,ja,zh,es,...}/holdem-pot-odds-calculation.ts` 등을 점검할 것.
   → 이게 다음 세션의 **가장 큰 미처리 건**이다.
3. 삭제한 이미지를 참조하던 `holdem-bankroll-management`의 readnext 썸네일은 고쳤다.
   다른 언어판에도 같은 참조가 있는지는 확인하지 않았다.
