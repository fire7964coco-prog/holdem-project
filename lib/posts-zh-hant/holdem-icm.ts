...
![泡泡時間裡，一個中碼在錢圈邊緣蓋掉面對大碼全下的牌，籌碼與獎金階梯都在畫面裡——ICM 壓力把一個正常的跟注變成蓋牌的那一刻](/images/holdem-icm-pressure.webp "ICM 壓力：中碼蓋牌，因為爆掉會賠掉「已鎖住的名次之上」那部分獎金權益")
...
> **快速解答**
> 可以直接拿籌碼 EV 當近似的場合比想像中多：早期、中期、盲注還小、以及最後的單挑——這些時候下一次獎金躍升不是很遠就是不存在，照籌碼 EV 打就夠準。ICM 只在階梯逼近的那幾手放大，把它帶進整場比賽，反而會把籌碼白白蓋掉。
...
A. 籌碼EV 用贏或輸多少籌碼衡量一個決策；ICM 用真實獎金衡量。兩者在錦標賽早期一致，接近錢圈就分岔，因為在那裡爆掉會賠掉「已鎖住的名次之上」那部分獎金權益。一手在籌碼EV 沒問題的五五開全下，在 ICM 下可能是清楚的蓋牌。
...
A. 三個大的。第一，*過度*套用 ICM——獎金躍升明明還遠，卻一路蓋牌「想爬階梯」，而不是累積籌碼。第二，當泡泡時間附近的中碼卻跟得太寬，那正是你風險溢價最高、爆掉會賠掉「已鎖住的名次之上」那部分權益的地方。第三，當短碼卻答應籌碼平分（或當領先者卻答應 ICM 交易）而事先沒算過數字。ICM 是後期工具：用得太早、或在決賽桌無視它，都會漏錢。
