...
사장님 우려(「업데이트 날짜가 전부 9/9인데 전체가 덮인 건 아니냐」)는 **실측·결재 둘 다 끝났다** — 데이터 사고 없음.
**정본 = `docs/settled-decisions.md` §1-C** · 절차 = `docs/hardening-protocol.md` §5-⑦-B · 경위 = WORKLOG 「2026-09-10 (4)(5)」.
🔴 **재론 금지.** 원칙 한 줄 = **`updated` = 그 파일이 실제로 손질된 날.**
🪶 남은 선택지 하나 = `check:stamp` 게이트를 만들 것인가(급하지 않다 · 만들면 «사람 판정 단계»를 반드시 붙여라 — «스탬프만 바뀐 파일»이 곧 결함은 아니다).
