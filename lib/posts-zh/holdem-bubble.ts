...
- **它怎么运作：** 赛事总监暂停计时，之后每打完一手就从当前级别固定扣掉 ==2 分钟==，不管这手牌实际打了多久（WSOP 锦标赛规则 126.a、126.c；TDA RP-8）——所以盲注在 bubble 期间照样往上走，只是按手数走、不按真实时间走。所有桌各发一手牌，任何一桌都要等到每一桌都打完了才开始下一手。如果**同一桌**有两个人在同一次 hand-for-hand 里出局，那手开始时筹码更少的那个拿更低（bubble）的名次；如果他们在**不同桌**出局，则视为并列，平分这两个名次的奖金。
...
- **别指望现场拖延。** 线上把时钟用满不花钱；现场为了爬名次刻意烧时间银行，按 WSOP 规则是明确可罚的——按正常速度弃牌，把出局的风险留给短码。
...
A. 「泡沫（bubble）」就是进钱前的最后一个名次，所以在那里出局的人——也就是 bubble boy——什么都拿不到，而其余还在场的人全部进钱；这就是人们说的「吃泡」（pay the bubble）。「泡沫破裂」（burst the bubble）指的则是那最后一次淘汰：它一发生，剩下的每个玩家就都进了钱，紧张的求生压力也随之松开。有些比赛会给 bubble boy 一份小小的安慰奖，但传统上钱圈边上就是发零奖。
