...
const STATIC_ROUTES = [
  ...
  /**
   * ★2026-08-22 신설 — `/es/solver`. `/en/solver`·`/ja/solver`와 같은 구조·같은 가중치(0.7)다.
   *   솔버 본체(`?lang=es`)는 noindex이므로 스페인어권 검색은 이 랜딩이 전담한다
   *   — 신설 전까지 그 진입로가 0개였다.
  ...
  /**
   * ★2026-08-24 신설 — `/de/solver`. 위 넷과 같은 구조·같은 가중치(0.7)다.
   *   솔버 본체(`?lang=de`)는 noindex이므로 독일어권 검색은 이 랜딩이 전담한다
   *   — 신설 전까지 그 진입로가 0개였다.
   *   🔴 빌드 후 `.next/server/app/de/solver.html`의 meta가 **index, follow**인지 확인할 것.
   */
  { path: "/de/solver", priority: "0.7", changefreq: "monthly" },

  /**
   * ★2026-08-24 신설 — `/zh/solver`. 위 다섯과 같은 구조·같은 가중치(0.7)다.
   *   솔버 본체(`?lang=zh`)는 noindex이므로 중국어권 검색은 이 랜딩이 전담한다
   *   — 신설 전까지 그 진입로가 0개였다.
   *   🔴 빌드 후 `.next/server/app/zh/solver.html`의 meta가 **index, follow**인지 확인할 것.
   */
  ...
  /**
   * ★2026-08-24 신설 — `/fr/solver`(9번째 솔버 랜딩 — 앱 fr 배포 당일). 위와 같은 구조·같은
   *   가중치(0.7)다. 솔버 본체(`?lang=fr`)는 noindex이므로 프랑스어권 검색은 이 랜딩이 전담한다.
   *   🔴 빌드 후 `.next/server/app/fr/solver.html`의 meta가 **index, follow**인지 확인할 것.
   */
  { path: "/fr/solver", priority: "0.7", changefreq: "monthly" },

  /**
   * ★2026-09-05 신설 — `/id/solver`(10번째 솔버 랜딩). 위와 같은 구조·같은 가중치(0.7)다.
   *   솔버 본체(`?lang=id`)는 noindex이므로 인니어권 검색은 이 랜딩이 전담한다 —
   *   신설 전까지 그 진입로가 0개였다(SERP에 인니어 «원문» 페이지가 하나뿐이고
   *   `translate.google.com`이 4개 랭크인 상태 = 무주공산).
 ...
 */
const TOURNAMENT_LOCALES = ["en", "ja", "zh", "zh-hant", "es", "de"];
