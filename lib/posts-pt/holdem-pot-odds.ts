...
export const POST: Post = {
  ...
  title: "Como calcular pot odds no poker — o método de 10 segundos",
...
Os números por trás dos seus projetos vêm da [tabela de odds e probabilidade do poker](/pt/blog/holdem-probability "thumb:/images/holdem-probability-hero.webp") — este guia é como você transforma esses números num call ou fold correto.
...
Esse número de "quantas vezes você precisa ganhar" é o ponto central. Receber 3 para 1 significa que o call se paga se você ganhar apenas **25% das vezes** ou mais. As pot odds transformam um vago "devo pagar?" num alvo concreto: *ganho com frequência suficiente para bater esse preço?*
...
O padrão é intuitivo: quanto mais o pote encobre o call, menor a fatia do bolo que você precisa para justificar o call.
...
**Q. Existe calculadora ou simulador de probabilidade para poker?**

A. Existe — mas na mesa você não pode usar uma, então o método de 10 segundos acima é a sua calculadora de verdade. Para estudar fora da mesa, o [solver GTO gratuito do site](/pt/solver) tem uma aba **Equity** que calcula o confronto na hora, direto no navegador, sem instalar nada. Use a ferramenta para conferir as suas contas até a regra do 2 e 4 virar reflexo.

**Q. Qual a diferença entre pot odds e implied odds?**

A. As pot odds só contam as fichas que estão no pote agora. As implied odds somam as fichas *extras* que você espera ganhar nas streets seguintes se completar a sua mão. As implied odds deixam você pagar com lucro alguns projetos que as pot odds sozinhas mandariam foldar — desde que os stacks sejam profundos o suficiente para te pagar.
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin:24px 0">
  <a href="/pt/blog/holdem-probability" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    ...
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Cada mão, flop e projeto — os números por trás do preço</div>
  </a>
  ...
  <a href="/pt/blog/holdem-tournament-vs-cash-game" style="display:block;padding:16px 18px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.10);border-radius:12px;text-decoration:none;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(212,175,55,0.45)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.10)'">
    <div style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:hsl(var(--primary));margin-bottom:6px">Cash vs Torneio</div>
    <div style="font-size:14px;font-weight:700;color:hsl(var(--foreground));line-height:1.4">Torneio vs Cash Game</div>
    <div style="font-size:12px;color:hsl(var(--muted-foreground));margin-top:4px">Por que as implied odds correm mais fundo em cash games</div>
  </a>
</div>
`.trim(),
};
