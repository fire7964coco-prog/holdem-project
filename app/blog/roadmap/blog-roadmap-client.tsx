    ...
    <>
      <SEO
        ...
        /* 🔴 2026-08-29 — page.tsx의 서버 metadata와 **같아야 한다**(seo.tsx가 런타임에 덮어쓴다).
           ⚠ 여기 있던 seoTitle="홀덤 완전 정복 로드맵 …" 을 **삭제**했다. 두 가지 이유:
              ① components/seo.tsx는 { title, description, path }만 구조분해한다 — seoTitle은 죽은 코드였다
              ② 그 문구가 §17 금지어(「완전 정복」)라 살아나면 안 되는 값이었다. 죽어 있다고 남겨 두면
                 다음 세션이 «구현만 고치면 되겠다»고 되살린다. */
        title="홀덤 학습 로드맵 — 뭐부터 읽어야 하나"
        description="홀덤을 처음 배우는 순서대로 글을 엮었습니다. 규칙과 족보에서 시작해 포지션·확률·전략까지, 어디까지 읽었는지 따라가며 볼 수 있는 학습 지도입니다."
