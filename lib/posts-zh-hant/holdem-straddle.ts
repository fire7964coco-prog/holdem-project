...
繞桌一圈 | 每個人都要跟到 $4（不是 $2）才能玩；照常可以蓋牌、跟注或加注——而且最小加注現在是 $8，即盲抓的兩倍，跟面對正常大盲時的規則一樣
...
| **密西西比** | 任意座位（常是按鈕/關煞位） | 盲抓者左邊 | 翻前——翻後只有從按鈕位才算* | 有 |
| **按鈕位** | 按鈕位 | 小盲 | 翻前＋翻後 | 有 |
| **沉睡** | 非槍口位的座位 | 正常（槍口位） | 無 | 看各家規則 |
| **再盲抓** | 盲抓者左邊 | 再盲抓者左邊 | 只有翻前 | 有 |

</div>

*密西西比盲抓只有下在==按鈕位==上，翻後才是最後行動。翻牌之後的順序完全由按鈕位決定，所以從關煞位抓的盲抓，只買到翻前的最後一句話。
...
- **沉睡盲抓（Sleeper straddle）** — 從非槍口位座位下的一注盲注，它「睡著」：**除非動作一路蓋到它那裡，否則不啟動**。位置它從來買不到；而它「睡醒」變成活盲後有沒有加注權，屬於各家規則的地帶。少見，線上幾乎看不到。
...
A. 沉睡盲抓是從非槍口位座位下的一注盲注，它保持不啟動（「睡著」），除非動作一路蓋到它那裡才生效。它不像活盲抓那樣給你位置；而它「睡醒」後有沒有加注權，因牌室而異。它不常見，線上也很少提供——一定要確認各家規則。
...
A. 對多數牌手來說，不划算——它是一個 -EV 的打法。你盲投錢，你縮小了自己的位置優勢（解算器在有盲抓的彩池裡開的牌*更少*，不是更多），又撐大了抽水。它只有在鬆弱桌、在所有人都已經盲抓的牌局、或純粹為了好玩時才站得住腳——絕不能拿來當賺錢的方法。
...
2. **決定位置的是座位，不是盲抓的名字。** 槍口位盲抓只在翻前最後行動。翻後也能最後行動的，只有==從按鈕位下的==盲抓——因為翻牌之後的順序永遠跟著按鈕位走。一切都看各家規則。
