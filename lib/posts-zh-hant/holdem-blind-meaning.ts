...
export const POST: Post = {
  ...
  tags: ["盲注是什麼", "大盲注", "小盲注", "大小盲", "德州撲克大盲小盲", "小盲大盲差別", "大盲底注", "德州撲克盲注"],
  content: `
...
> **快速解答**
> 盲注是在牌發下來之前就被迫下的注——你什麼都還沒看到就先押，所以叫「盲下」。正常情況下每手牌固定兩個座位付：莊家鈕左邊緊挨著的小盲和大盲。它存在的理由只有一個，就是讓牌局不至於停下來。

**盲注（Blind）** 這個名字就來自「盲下」——牌還沒看就得先押。那為什麼一定要逼人先付這筆錢？==g:為了讓牌局活下去。== ==r:沒有盲注的話，每個人都可以一直蓋牌、無止盡地枯等一手大牌==；底池永遠是空的，詐唬也變得毫無意義，整場牌局就停擺了。==g:正常情況下每手牌兩個強制注，保證檯面上永遠有東西可以爭。==（常見的例外有兩個：死按鈕那一手可能沒有小盲；單挑時下小盲的不是鈕左邊那一家，而是按鈕位自己——見下文。）
...
> **快速解答**
> 小盲是兩個強制注裡較小的那個，由莊家鈕正左邊的玩家下，通常是大盲的一半——$1/$2 的局裡就是 $1。它同時是全桌長期輸最多的座位，因為你用一手隨機牌付了錢，翻牌後卻要第一個動。

**小盲注（Small Blind, SB）** 難打，是因為兩個劣勢會疊在一起：牌是隨機發的，位置又永遠最差——翻牌之後每一條街，你都得在資訊最少的時候先開口。所以在小盲，正確的目標是把每一手的損失壓小，而不是想著在這個座位上贏錢。
...
> **快速解答**
> 大盲是完整的基本注，由小盲左邊的玩家下，$1/$2 的局裡就是 $2。它換到的補償是翻牌前最後一個行動——沒人加注時還握有「選擇權」，可以直接過牌免費看翻牌。

這裡有個容易被跳過的名詞：大盲在翻牌前沒人加注時握有的那個權利，正式叫 **「選擇權（option）」**——他可以直接[過牌](/zh-hant/blog/holdem-betting-actions)、不再加錢就看翻牌。它之所以值錢，是因為你已經把一個完整注放進底池了。
...
> **快速解答**
> 盲注不是固定在某兩個人身上，而是跟著莊家鈕走：每打完一手，鈕往左移一個座位，盲注也跟著移。正常輪轉下每個人每一圈都會各付一次小盲和一次大盲。現金桌離席漏掉的，回座時可以補上、等大盲輪回或在允許的撲克室以槍口位活盲抓入局；錦標賽則照扣，人不在也一樣。

兩個盲注都在 **發牌前** 下好——牌還沒發，錢已經在底池裡了。下面這張表把兩個座位的位置、金額和行動順序放在一起看。
...
> **快速解答**
> 級別寫成 SB/BB，小盲在前：$1/$2 就是小盲 $1、大盲 $2。真正的差別不在數字，而在「會不會動」——現金桌坐下之後固定不變，錦標賽則時間一到就升盲，你的籌碼即使一手沒輸，換算成大盲也一直在縮水。

級別牌上寫的就是這兩個數字，小的在前、大的在後。下面這張表把常見的現金桌級別和對應買入放在一起：
...
> **黃金守則：** ==坐下前一定要先看清楚級別牌==——有些撲克室會跑非標準的結構，像 $1/$3 或 $2/$3。
...
> **快速解答**
> 底注（Ante）和盲注最大的差別是「這筆錢算誰的」：盲注只有兩個座位付，底注是全桌每一手都躲不掉的成本。現代錦標賽常見的大盲底注只是它的簡化收法——由大盲一個人代替全桌放進去，不用每個人各丟籌碼。
...
還有一個你會在現金局看到的強制注遠親：**盲抓（Straddle）**——一個 *自願* 的額外盲注（通常是 2× 大盲），由 UTG（大盲左邊），或在允許的牌桌上由鈕位下，暫時把這一手的注碼拉高。WSOP 規則中，鈕位盲抓優先，但限注局不允許鈕位盲抓。它有自己的一套規則、種類與行動順序，完整拆解在[盲抓是什麼、該不該下](/zh-hant/blog/holdem-straddle "thumb:/images/holdem-straddle-hero.webp")裡查得到；這類牌桌用語如果你還很陌生，[德州撲克術語大全](/zh-hant/blog/holdem-glossary "thumb:/images/holdem-glossary-hero.webp")一次收齊。
...
> **快速解答**
> 單挑最反直覺的地方在於：按鈕位平常是全桌最好的座位，這時卻要自己掏小盲。代價換來的是翻牌之後每一條街都最後行動——而翻牌前先開口的那個「虧」，正是用這個位置優勢補回來的。

當桌上只剩兩名玩家（或本來就是單挑對戰）時，平常的規則會 ==r:反轉==。**莊家鈕下小盲注**，並在翻牌前 ==**第一個**== 行動；另一名玩家下大盲注、翻牌前 ==**最後**== 行動——然後從翻牌之後每一條街都變成他先動。
...
> **快速解答**
> 分兩種情況：新坐下的人通常要補一個大盲才能立刻開打，或等盲注輪到自己；中途離桌漏盲的人回座時可以補上、等大盲輪回，或在允許的撲克室以槍口位活盲抓入局——補的話漏掉的小盲會變成死籌碼直接進底池、不算你的注。

各家撲克室的規矩不一樣，所以坐下前問荷官一句最穩。常見的兩種做法是——**補下一個大盲** 立刻加入，或等盲注輪到你的座位；也有撲克室不要求無限注／底池限注新入座的玩家補盲（WSOP 現金局（Live Action）規則 104.a 就是這樣）。如果你離桌一下，==**漏掉了你的盲注**==，多數撲克室的做法是回座時補下再重新加入——漏掉的大盲當成一個有效的活注，漏掉的小盲則是一枚 ==**死籌碼**==、直接被收進底池。線上牌桌會自動幫你處理這一切。
...
> **快速解答**
> 盲注位的錢不是一次爆掉的，是每一圈漏一點。兩個習慣就修掉大半：小盲不要跛入，要嘛加注要嘛蓋牌；大盲則要比你以為的更常防守，因為你已經投了一個完整注在裡面。

![籌碼疊朝底池推過去的德州撲克桌面——從按鈕位發動的偷盲全推](/images/holdem-blind-steal.webp "從按鈕位發動的偷盲——沒人跟就贏下兩個盲注")

盲注位是我看新手最常默默流血的地方。下面三條依座位分開講，最後一條是反過來從別人盲注賺錢的：
...
A. 不行——沒有盲注，牌局就跑不起來。正常情況下，盲注是每手牌兩筆的強制注，保證底池裡永遠有籌碼可以爭，這樣才不會每個人都一直蓋牌枯等大牌、讓遊戲停擺。它同時也給了牌局「動機」：因為檯面上一開始就有錢，你才有理由去偷盲、去搶底池、去用位置施壓——盲注不只是規則，更是整場戰略的起點。
...
A. 1 BB 就是一個大盲注的金額。現金桌看級別牌：$1/$2 的局 1 BB = $2；錦標賽則以籌碼數算，25/50 級別的 1 BB = 50 籌碼，而且每升一級就變大。牌手習慣把籌碼深度和下注大小全都換算成 BB 來講（「100 BB 深」「下注 3 BB」），就是因為大盲是整桌的基本單位。

**Q. 錦標賽為什麼要加底注（Ante）？**

A. 為了逼大家動起來。只有盲注時，一圈裡多數位置蓋牌完全不用花錢，牌局會愈打愈慢、愈打愈緊；加了底注之後，每一手牌的底池一開始就更大，「枯等大牌」變成每一圈都在流血，偷盲、搶池才變得划算。換句話說，底注不是為了多收錢，是為了把牌局往前推。
...
A. 小盲確實是全桌最不利的座位：你用半個注的隨機牌入池，翻牌後每條街又都要第一個行動。實務上的解法是「加注或蓋牌」——不要只補半個注跛入（limp）。手牌不夠強就乾脆蓋掉；夠強就用加注 **3-bet**（再加注）取得主動權，逼對手在不利於他的情況下做決定。長期目標是在這個座位 *少輸*，而不是想著要贏。

**Q. 盲注跟花色（同花大小）有關係嗎？**

A. **完全沒有關係，這是兩碼事。** 有些娛樂城的文章會把「盲注」和「花色有沒有大小」混在一起講，很容易誤導人。盲注是發牌前的強制 *下注*，只跟位置與金額有關（小盲、大盲）；花色（黑桃♠、紅心♥、方塊♦、梅花♣）在德州撲克裡 **沒有大小之分**，比牌只看點數，兩手同花就從最大那張開始逐張比下去。想釐清哪一手牌贏哪一手，看[德州撲克牌型大小排名](/zh-hant/blog/holdem-hand-rankings)。

**Q. 下了盲注之後可以蓋牌嗎？**

A. 可以。輪到你時，你跟其他人一樣能蓋牌；小盲可以選擇放棄、不去補足到大盲。但要記住：已經下進去的盲注 **不會因為蓋牌而退還**，蓋掉就是把那筆籌碼留在底池裡。

**Q. 單挑（兩人局）時盲注誰下？**

A. 只剩兩人時規則會反轉：莊家鈕下小盲注，並在翻牌前 **第一個** 行動；另一名玩家下大盲注、翻牌前 **最後** 行動。

**Q. 沒下到盲注（漏盲）會怎樣？**

A. 多數撲克室會要你回座時補下漏掉的盲注——漏掉的大盲當成有效的活注，漏掉的小盲則是一枚死籌碼、直接進底池。或者，你也可以等大盲自然輪回你的座位再入局，或在允許這麼做的撲克室以槍口位活盲抓重新入局。線上牌桌會自動處理。
