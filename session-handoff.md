...
## ▶▶▶▶▶▶▶▶▶ 새 세션 START HERE (2026-07-31 — ★경화 기계 게이트 신설, 25/25 무결)

### 0-A. ★먼저 알아야 할 것 — 검수 방식이 바뀌었다 (커밋 `b5b56b0`)
...
npm run audit:hard -- --slug=holdem-outs-calculation
npm run audit:hard -- --worksheet   # §13 핸드 계산 결과 전부 출력
npm run audit:hard -- --schema      # FAQ 스키마 (npm run build 이후)
npm run audit:hard:selftest         # 게이트 자체 검증 13/13
```

**글 1편 편집할 때마다 `--slug=`로 돌려라.** 이게 §13 검산·메타·링크·중복을 한 번에 본다.

⚠ **이 게이트가 못 잡는 것** — D유형(전략적 유해 조언: "지는 자리를 밸류라고 가르침")과
F유형(사실 오류). 이 둘은 **여전히 렌즈(적대 검수)와 1차 출처의 몫**이다. 게이트가 초록이라고
검수가 끝난 게 아니다.

⚠ **커버리지를 반드시 읽어라.** 출력 맨 아래가 "시나리오를 못 잡은 글"을 나열한다.
그 글들의 "0건"은 **검증이 아니라 미검사**다. 현재 8편이 여기 해당한다.
