...
4. 🔴 **우편함을 연다** — `node ~/Downloads/홀덤검수/tools/mailbox-check.mjs` → **«검수장→본체» 행이 우리 몫**이다
   (도구는 검수장 시점이라 🟡 «내 할 일 아님»으로 찍혀도 본체에겐 할 일이다). 2026-09-25에 MA-144가 **9일** 묵은 채 발견됐다.
   팀 협업 규율 4종(세션 시작 점검 · 배포=MB 통지 · 받은 MA는 다음 세션 안에 회신+대기열 등재 · 큰 회차 착수 공지) 정본 = `docs/mailbox-protocol.md` **§6**
...
- [ ] 🔴 **본문을 바꾼 배포면 `mailbox/out-본체.md`에 MB 1행** — 커밋 해시·슬러그·로케일. 로케일 발행·랜딩 변경도 포함
      (2026-09-22~24에 17커밋이 통지 없이 나가 검수장이 원장 앵커 소거로 뒤늦게 알았다 · `docs/mailbox-protocol.md` §6)
