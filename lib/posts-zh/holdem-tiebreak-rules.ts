...
export const POST: Post = {
  ...
  tags: ["德州扑克 平局规则", "牌型一样谁赢", "同样一对谁赢", "两对平局", "第五张牌重要吗", "德州扑克 平手怎么算", "更大的顺子", "德州扑克花色大小"],
  content: `
...
> **快速回答**
> 德州扑克平局按固定顺序拆分：先比牌型，再比组成牌型的牌，最后从大到小比踢脚牌——第一处不同就定胜负；如果五张牌还是全一样，就平分底池。花色、谁先下注、谁筹码多，全都不参与比较。
...
> **快速回答**
> 两个玩家同样一对，第一张踢脚牌大的赢。一对带三张踢脚牌，从上往下一张一张比——第一处不同就定底池，后面的踢脚牌根本轮不到比。要留神的只有一点：进了最佳五张的才算踢脚牌，公共牌可能把你的底牌挤出去。
...
> **快速回答**
> 每种牌型都有自己的比较顺序：一对、两对、三条、四条、高牌会一路比到踢脚牌；顺子、同花、同花顺只看组成牌型的牌本身，葫芦先比三条再比对子。表格右侧的标记告诉你踢脚牌会不会登场。

:::tiebreak
皇家同花顺|只有牌面本身就是皇家同花顺时才可能撞车——全桌平分|-无踢脚牌
同花顺|只比最大的牌|-无踢脚牌
四条|四条点数 → 第五张牌|+踢脚牌
葫芦|三条点数 → 对子点数|-无踢脚牌
同花|五张全比，从大到小|-无踢脚牌
...
> **快速回答**
> 两个玩家都是两对，先比大对，再比小对，最后比那唯一一张踢脚牌——就按这个顺序。两对正好只带一张踢脚牌，所以对子比完之后只剩一张牌可争；而牌面上冒出来的第二个对子，还可能把你手里的小对整个作废。
...
> **快速回答**
> 能。顺子纯粹按最大的那张牌排：A-2-3-4-5 的轮子里 A 当最小，所以它是全游戏最小的顺子；A-K-Q-J-10 的 Broadway 是最大的顺子。中间每一副顺子都只凭顶张排位，两副顺子顶张相同就完全一样，平分底池。
...
阶梯的另一头，==**最大的顺子是 Broadway——A-K-Q-J-10**==。没有任何顺子能赢它（同花以及排在它之上的牌型照样能），轮子坐在最底层，所以游戏里每一副顺子都只凭顶张排在这两者之间。
...
> **快速回答**
> 重要。只要两手牌的前四张一样，第五张牌就是整个底池——一对的第三张踢脚牌、两对那唯一一张踢脚牌、同花最小的那张、牌面四条旁边的那张单张，都是这种情况。它只在公共牌本身大过你的底牌时才不再重要。
...
> **快速回答**
> 不分。在决定谁赢底池这件事上，花色只有一个任务：五张同花色凑成同花。除此之外它没有任何高低，所以两手点数一模一样的牌永远平分底池，也没有哪张牌会因为花色而压过另一张。

这个问题老有人问，是因为花色排序在扑克里==确实存在==——只是在这个游戏里从不用来比牌的大小。梭哈（stud）和 razz 用它来决定谁先强制下注（bring-in）、以及那枚分不开的筹码归谁。德州扑克这两处都不用（唯一沾边的是开局抽牌定按钮位，那选的是座位不是牌——见下方常见问题）。

最干净的证据，就是那一枚==*分不开*==的筹码。2026 年 WSOP 锦标赛规则第 73 条写道：==g:*"In button games with 2 or more high or low hands, the odd chip goes to the first seat left of the button"*==（按钮位游戏里出现两手及以上同样的高牌或低牌时，零头筹码归按钮左边第一个座位）。哪怕底池物理上分不均，规则依据的也是**座位**，不是花色——而同一条规则后半段按花色分零头的办法，只适用于梭哈和 razz。

还有一点值得知道：在德州扑克里两副同花永远是*同一种*花色，因为五张公共牌是共用的，一张牌面装不下三张红桃再加三张黑桃。所以“我的黑桃赢你的红桃”这种事，不是你输在了哪条规则上——那是一手根本发不出来的牌。
...
> **快速回答**
> 如果你的底牌破不了公共牌自己的最佳五张，它们就不参与——你在“打公共牌”（play the board）。当所有人都是这样，底池平分；只要有一个人能用底牌把牌面延伸成更大的牌，他就独赢。

看上面的公共牌：A♠ K♥ Q♣ J♦ 10♠，Broadway（顺高）已经现成。你的 9♥ 7♠ *确实*能做成顺子——K-Q-J-10-9——但它比台面上那副 A 高顺子**更小**，所以你的最佳五张就是公共牌本身。别人也一样。

这就是所谓的**打公共牌（play the board）**：你的两张底牌一张都用不上，直接用五张公共牌当作最佳五张。关键在于——==r:只要公共牌自己就成了顺子或同花，除非有人手里握着能“延伸”到更大的牌，否则全桌一起打公共牌、平分底池==。
...
| 玩家 | 底牌 | 最佳五张 | 结果 |
|--------|------|-----------|------|
| A | 2♠ 3♣ | 9-8-7-6-5（打公共牌） | 平分 |
| B | K♦ Q♠ | 9-8-7-6-5（打公共牌） | 平分 |
| C | ==g:10♣== 4♦ | ==g:10==-9-8-7-6 | **独赢** |

</div>

A 的 2、3 接不到顺子下端（要接成 3-4-5-6-7 得有 4，没有），B 的 K、Q 也够不到上端（要更大得有 10-J），所以两人都只能打公共牌 9-8-7-6-5，==g:打平==。可 C 手里那张 10 把顺子往上延伸成 10-9-8-7-6——**顶张 10 大过 9**，于是 C 独吞底池。同花是完全一样的道理：公共牌摆出五张♥，谁手里没♥或只有小♥就打公共牌，握着 A♥ 的人做成 A 高同花独赢。

更微妙的版本是你的牌参与了、但踢脚牌没参与。公共牌 A♥ K♣ Q♦ J♠ 9♥：A♠ 3♠ 对 A♦ 2♦。两人都成一对A，而且三个踢脚牌位都由公共牌填满——每个玩家都是 A-A-K-Q-J。那 3 和那 2 是死重；最佳五张完全相同，==g:平分。==

![信息图：在 A-K-Q-J-9 的公共牌上，A-3 和 A-2 都是 A-A-K-Q-J，所以完全相同的两手牌平分底池](/images/holdem-tiebreak-split.webp "当最佳五张一张对一张全部相同，底池就分掉——花色永远打不破平局")
...
A. 三道检查按顺序跑：先比牌型，再比组成牌型的牌，最后从大到小比踢脚牌——第一处不同就结束。同样重要的是什么永远不进比较：花色、谁最后下注、谁离按钮更近、每个人投了多少筹码。如果五张牌点数一张对一张全相同，无论下注过程发生了什么，荷官都平分底池。

**Q. 都是一对怎么比大小？要比单张吗？**

A. 要比——踢脚牌大的赢，但先看清你的哪几张牌真正进了最佳五张。牌面 A-Q-7-3-2 上大家都是一对 A，拿 A-9 的玩家打出 A-A-Q-9-7：牌面的 Q 跳到了 9 的前面，所以 9 只是*第二张*踢脚牌。对上 A-K，底池在第一张踢脚牌（K 对 Q）就定了，那张 9 根本没被比到。踢脚位有三个；大多数底池在第一个就结束。

**Q. 都是两对怎么比大小？要比单牌吗？**

A. 先比大对，再比小对，最后比那唯一一张踢脚牌——所以 A 带 3 赢过 K 带 Q，哪怕第二对小得多。让人栽跟头的是像 K-K-9-9-5 这样的双对牌面（没有三张同花色，谁都凑不成同花）：除非有人拿着 K、9、口袋 5-5，或比 9 大的口袋对子，否则每个人都是同样的两对，整手牌就坍缩成一张踢脚牌，全桌最大的那张底牌拿走底池——要是没人的底牌能压过牌面上的 5，那就全员打公共牌，平分底池。两对正好只带一张踢脚牌，绝不会有两张。

**Q. 德州扑克里第五张牌重要吗？**

A. 重要——而且这是玩家把一个自以为稳赢的底池输掉的最常见方式。把整个底池压在最后一张牌上的典型情形有这么几种：一对的第三张踢脚牌、两对那唯一一张踢脚牌、同花最小的那张、以及牌面四条旁边的那张单张。只有当公共牌本身大过你手里那张单张，它才不再重要——有时是整副公共牌都比你大、你的底牌整个掉出这手牌（打公共牌），有时是只有一张底牌起作用、另一张永远轮不到：A♠ 3♠ 对 A♦ 2♦、公共牌 A♥ K♣ Q♦ J♠ 9♥ 就是平分，两人都是 A-A-K-Q-J。

**Q. 底牌无效、打公共牌（play the board）是什么？我的两张牌完全没用怎么算？**

A. 当五张公共牌本身就是你能凑出的最强五张、两张底牌一张都进不了最佳五张时，你就是在“打公共牌”。这时全桌都用同一副公共牌，==g:通常直接平分底池==。例如公共牌 A♠ K♥ Q♣ J♦ 10♠ 已是 A 高顺子，你手里的 9♥ 7♠ 顶多做出更小的 K-Q-J-10-9，用不上，全桌一起打公共牌平分。唯一例外：有人手里那张牌能把牌面“延伸”成更大的牌（见下一条）。

**Q. 公共牌摆出顺子／同花，是不是所有人平分？**

A. 不一定。只有当**没人能延伸出更大的牌**时才全体平分。公共牌 5♦ 6♣ 7♥ 8♠ 9♦ 自己就是 9 高顺子：只握 2-3 或 K-Q 的人接不到两端，都只能打公共牌 9-8-7-6-5，平分；但握着 10 的人做成 10-9-8-7-6，==r:顶张更大就独赢==。同花同理——公共牌五张♥时，握着 A♥ 的人做成 A 高同花，把其他打公共牌的人全部盖过去。

**Q. 我的对子被河牌顶掉（counterfeit）是什么意思？**

A. 指公共牌自己配出比你更大的对子，把你手里那对“顶”出了最佳五张，让你原本领先的牌缩水甚至变成打公共牌。看一个会==g:平分==的例子：你 6♠ 6♦、对手 5♠ 5♦，公共牌走到 K♣ K♥ 9♠ 9♥ A♦。牌面已经自带 K-K 和 9-9 两对，都比你的 6 和他的 5 大；再加最大的单张 A，双方的最佳五张都是 ==g:K-K-9-9-A==——你的 6 和他的 5 全被顶掉、一张都没进牌，两人打公共牌平分。简体没有统一译法，一般说“公牌顶掉（counterfeit）”或“底牌作废”。正文里 5♠4♠ 被河牌那对 9 顶掉的例子，则是顶掉后一方独赢的版本。

**Q. 牌型完全一样，零头那一颗筹码归谁？**

A. 平分后除不尽的奇数筹码（零头）不会撕成两半，而是==g:从按钮位左侧第一个未盖牌的赢家开始，按顺时针方向分给最靠前的那一位==。所以“平分”极少是分毫不差的对半，零头有固定归属规则。具体到边池、三方分的分法都在[平分底池规则](/zh/blog/holdem-split-pot-rules)里。
...
**Q. 德州扑克顺子 12345 算数吗？A 能当 1 用吗？**

A. 算——A-2-3-4-5 是合法的顺子，叫“轮子”，此时 A 当作最小的牌，这也让轮子成为全游戏最小的顺子。A 只能这样当 1 用，不能从中间绕圈：Q-K-A-2-3 不是顺子。

**Q. 能有比对手更大的顺子吗？**

A. 能，而且实战里通常发生在顺子大半已经摆在牌面上的时候。牌面 5♦ 6♣ 7♠ 8♥ 2♦：拿 9♣ 4♠ 的玩家做成 9-8-7-6-5，拿 4♥ 3♦ 的玩家用同样四张牌做成 8-7-6-5-4。两人都“做成了顺子”，可从头到尾只数顶张，所以 9 拿走底池。顶张相同才是一样的顺子，平分。
...
A. 从最大的那张单张开始，一张一张往下比，一直比到第五张——第一处不同就定胜负。A-K-9-6-3 赢过 A-K-8-7-5：前两张 A、K 打平，第三张 9 大过 8，后面的 7 和 5 再大也轮不到比。五张点数全相同就平分底池，花色照样不参与。

**Q. 都是同花怎么比大小？**

A. 从最大的那张开始，一张一张往下比：A 高同花赢过 K 高同花；顶张一样就比下一张，一直比到第五张。==r:花色永远打不破平局==，所以五张点数全相同就平分底池。（德州扑克里两副同花一定是同一花色，因为大家共用公共牌。）
...
A. 不会——但德州扑克牌桌上确实有一个用到花色的地方，只是跟底池无关。现金桌（以及多数牌室的场规）在牌局开始时每人抽一张牌决定庄家按钮放在哪，两张抽到的牌点数相同时，就靠花色顺序定。（WSOP 锦标赛开局不抽牌：==锦标赛规则第 85 条==规定按钮从荷官右手边第一个筹码堆开始，只在剩 3、2、1 桌时才抽牌定按钮。）不管哪种，那选的都是一个*座位*，从来不是一手牌。除了抽牌定按钮——WSOP 规则书的术语表规定按“点数和花色”来定——锦标赛规则书里唯一的花色排序全都属于梭哈和 razz。两手最佳五张点数一张对一张全相同，无论花色如何都平分底池。
...
A. 会，但不常见。真正的平手（平局，英文 tie / split pot）只在两名及以上玩家最佳五张点数完全相同时发生——最常见的就是公共牌自己是最强牌（“打公共牌”），或是一副谁的底牌都改善不了的共用顺子／同花。这时底池对半分。踢脚牌存在的意义，恰恰就是在绝大多数“差点平分”的局面变成真平分之前，把胜负先分出来。
