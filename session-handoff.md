...
> 마지막 갱신 **2026-09-21** · HEAD = **`76569eef`+** · **미커밋 0 · 배포·라이브 확인·IndexNow 200 완료.**
>
...
1. **🧰 queue 다음 회차 — 사장님이 번호를 고른다.** 레인 창 첫 마디 = 「`HARDEN.md` 읽고 회차 Q<번호> 시작해」 · 회차 표 = `docs/harden-queue-진행.md` §1.
   🔴 **검수장 MA-147·148 이행 — MA-147 ①은 Q12-a·Q12-b로 ✅ 전건 종결(09-21 (6)(7)). 남은 둘**:
...
- 🆕 🟠 **`check:stamp` 🔴 24 — 사장님 판정 1건**(▶ 아님 · 결정만 하면 닫힌다): 전건 **선재**이고 Q12-a·Q12-b 파일은 0건. 24 중 **22가 `64c0fd9a`**(09-17 계산기 랜딩 신설 회차가 «본문 계산기 링크 66곳 재조준» 하며 `updated`를 안 올린 것) · 나머지 2는 `abdc7a21`. 헤드 실측 = 그 커밋이 그 파일들에 한 일은 **`/calculator` → `/<loc>/calculator` 링크 재조준뿐이고 산문은 바이트 동일**이다. `settled-decisions` **§1-C**는 「내용을 바꿨으면 반드시 올려라」인데 같은 절의 목적 축어는 *「날짜를 보고 업데이트 할지 말지 결정을 하지」* = **노후도 신호**다. → **ⓐ §1-C에 「링크 재조준 등 기계적 변경은 `updated` 대상 아님」을 명문화**(권고 · 29파일 무변경) **ⓑ 그 29파일의 `updated`를 09-17로 올린다** 중 택일. 🔴 **닫지 않으면 게이트가 매 회차 🔴 24를 띄우고 다음 세션이 또 판다.**
